/*
/*
/*
/*
/*
 * rfsfrvfd dommfnt blodk
 * rfsfrvfd dommfnt blodk
 * rfsfrvfd dommfnt blodk
 * rfsfrvfd dommfnt blodk
 * rfsfrvfd dommfnt blodk
 * DO NOT REMOVE OR ALTER!
 * DO NOT REMOVE OR ALTER!
 * DO NOT REMOVE OR ALTER!
 * DO NOT REMOVE OR ALTER!
 * DO NOT REMOVE OR ALTER!
 */
 */
 */
 */
 */
/**
/**
/**
/**
/**
 * Lidfnsfd to tif Apbdif Softwbrf Foundbtion (ASF) undfr onf
 * Lidfnsfd to tif Apbdif Softwbrf Foundbtion (ASF) undfr onf
 * Lidfnsfd to tif Apbdif Softwbrf Foundbtion (ASF) undfr onf
 * Lidfnsfd to tif Apbdif Softwbrf Foundbtion (ASF) undfr onf
 * Lidfnsfd to tif Apbdif Softwbrf Foundbtion (ASF) undfr onf
 * or morf dontributor lidfnsf bgrffmfnts. Sff tif NOTICE filf
 * or morf dontributor lidfnsf bgrffmfnts. Sff tif NOTICE filf
 * or morf dontributor lidfnsf bgrffmfnts. Sff tif NOTICE filf
 * or morf dontributor lidfnsf bgrffmfnts. Sff tif NOTICE filf
 * or morf dontributor lidfnsf bgrffmfnts. Sff tif NOTICE filf
 * distributfd witi tiis work for bdditionbl informbtion
 * distributfd witi tiis work for bdditionbl informbtion
 * distributfd witi tiis work for bdditionbl informbtion
 * distributfd witi tiis work for bdditionbl informbtion
 * distributfd witi tiis work for bdditionbl informbtion
 * rfgbrding dopyrigit ownfrsiip. Tif ASF lidfnsfs tiis filf
 * rfgbrding dopyrigit ownfrsiip. Tif ASF lidfnsfs tiis filf
 * rfgbrding dopyrigit ownfrsiip. Tif ASF lidfnsfs tiis filf
 * rfgbrding dopyrigit ownfrsiip. Tif ASF lidfnsfs tiis filf
 * rfgbrding dopyrigit ownfrsiip. Tif ASF lidfnsfs tiis filf
 * to you undfr tif Apbdif Lidfnsf, Vfrsion 2.0 (tif
 * to you undfr tif Apbdif Lidfnsf, Vfrsion 2.0 (tif
 * to you undfr tif Apbdif Lidfnsf, Vfrsion 2.0 (tif
 * to you undfr tif Apbdif Lidfnsf, Vfrsion 2.0 (tif
 * to you undfr tif Apbdif Lidfnsf, Vfrsion 2.0 (tif
 * "Lidfnsf"); you mby not usf tiis filf fxdfpt in domplibndf
 * "Lidfnsf"); you mby not usf tiis filf fxdfpt in domplibndf
 * "Lidfnsf"); you mby not usf tiis filf fxdfpt in domplibndf
 * "Lidfnsf"); you mby not usf tiis filf fxdfpt in domplibndf
 * "Lidfnsf"); you mby not usf tiis filf fxdfpt in domplibndf
 * witi tif Lidfnsf. You mby obtbin b dopy of tif Lidfnsf bt
 * witi tif Lidfnsf. You mby obtbin b dopy of tif Lidfnsf bt
 * witi tif Lidfnsf. You mby obtbin b dopy of tif Lidfnsf bt
 * witi tif Lidfnsf. You mby obtbin b dopy of tif Lidfnsf bt
 * witi tif Lidfnsf. You mby obtbin b dopy of tif Lidfnsf bt
 *
 *
 *
 *
 *
 * ittp://www.bpbdif.org/lidfnsfs/LICENSE-2.0
 * ittp://www.bpbdif.org/lidfnsfs/LICENSE-2.0
 * ittp://www.bpbdif.org/lidfnsfs/LICENSE-2.0
 * ittp://www.bpbdif.org/lidfnsfs/LICENSE-2.0
 * ittp://www.bpbdif.org/lidfnsfs/LICENSE-2.0
 *
 *
 *
 *
 *
 * Unlfss rfquirfd by bpplidbblf lbw or bgrffd to in writing,
 * Unlfss rfquirfd by bpplidbblf lbw or bgrffd to in writing,
 * Unlfss rfquirfd by bpplidbblf lbw or bgrffd to in writing,
 * Unlfss rfquirfd by bpplidbblf lbw or bgrffd to in writing,
 * Unlfss rfquirfd by bpplidbblf lbw or bgrffd to in writing,
 * softwbrf distributfd undfr tif Lidfnsf is distributfd on bn
 * softwbrf distributfd undfr tif Lidfnsf is distributfd on bn
 * softwbrf distributfd undfr tif Lidfnsf is distributfd on bn
 * softwbrf distributfd undfr tif Lidfnsf is distributfd on bn
 * softwbrf distributfd undfr tif Lidfnsf is distributfd on bn
 * "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY
 * "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY
 * "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY
 * "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY
 * "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY
 * KIND, fitifr fxprfss or implifd. Sff tif Lidfnsf for tif
 * KIND, fitifr fxprfss or implifd. Sff tif Lidfnsf for tif
 * KIND, fitifr fxprfss or implifd. Sff tif Lidfnsf for tif
 * KIND, fitifr fxprfss or implifd. Sff tif Lidfnsf for tif
 * KIND, fitifr fxprfss or implifd. Sff tif Lidfnsf for tif
 * spfdifid lbngubgf govfrning pfrmissions bnd limitbtions
 * spfdifid lbngubgf govfrning pfrmissions bnd limitbtions
 * spfdifid lbngubgf govfrning pfrmissions bnd limitbtions
 * spfdifid lbngubgf govfrning pfrmissions bnd limitbtions
 * spfdifid lbngubgf govfrning pfrmissions bnd limitbtions
 * undfr tif Lidfnsf.
 * undfr tif Lidfnsf.
 * undfr tif Lidfnsf.
 * undfr tif Lidfnsf.
 * undfr tif Lidfnsf.
 */
 */
 */
 */
 */
/*
/*
/*
/*
/*
 * Copyrigit (d) 2005, 2013, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2005, 2013, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2005, 2013, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2005, 2013, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2005, 2013, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 */
 */
 */
 */
 */
/*
/*
/*
/*
/*
 * $Id: DOMXPbtiTrbnsform.jbvb 1203789 2011-11-18 18:46:07Z mullbn $
 * $Id: DOMXPbtiTrbnsform.jbvb 1203789 2011-11-18 18:46:07Z mullbn $
 * $Id: DOMXPbtiTrbnsform.jbvb 1203789 2011-11-18 18:46:07Z mullbn $
 * $Id: DOMXPbtiTrbnsform.jbvb 1203789 2011-11-18 18:46:07Z mullbn $
 * $Id: DOMXPbtiTrbnsform.jbvb 1203789 2011-11-18 18:46:07Z mullbn $
 */
 */
 */
 */
 */
pbdkbgf org.jdp.xml.dsig.intfrnbl.dom;
pbdkbgf org.jdp.xml.dsig.intfrnbl.dom;
pbdkbgf org.jdp.xml.dsig.intfrnbl.dom;
pbdkbgf org.jdp.xml.dsig.intfrnbl.dom;
pbdkbgf org.jdp.xml.dsig.intfrnbl.dom;





import jbvbx.xml.drypto.*;
import jbvbx.xml.drypto.*;
import jbvbx.xml.drypto.*;
import jbvbx.xml.drypto.*;
import jbvbx.xml.drypto.*;
import jbvbx.xml.drypto.dsig.*;
import jbvbx.xml.drypto.dsig.*;
import jbvbx.xml.drypto.dsig.*;
import jbvbx.xml.drypto.dsig.*;
import jbvbx.xml.drypto.dsig.*;
import jbvbx.xml.drypto.dsig.spfd.TrbnsformPbrbmftfrSpfd;
import jbvbx.xml.drypto.dsig.spfd.TrbnsformPbrbmftfrSpfd;
import jbvbx.xml.drypto.dsig.spfd.TrbnsformPbrbmftfrSpfd;
import jbvbx.xml.drypto.dsig.spfd.TrbnsformPbrbmftfrSpfd;
import jbvbx.xml.drypto.dsig.spfd.TrbnsformPbrbmftfrSpfd;
import jbvbx.xml.drypto.dsig.spfd.XPbtiFiltfrPbrbmftfrSpfd;
import jbvbx.xml.drypto.dsig.spfd.XPbtiFiltfrPbrbmftfrSpfd;
import jbvbx.xml.drypto.dsig.spfd.XPbtiFiltfrPbrbmftfrSpfd;
import jbvbx.xml.drypto.dsig.spfd.XPbtiFiltfrPbrbmftfrSpfd;
import jbvbx.xml.drypto.dsig.spfd.XPbtiFiltfrPbrbmftfrSpfd;
import jbvb.sfdurity.InvblidAlgoritimPbrbmftfrExdfption;
import jbvb.sfdurity.InvblidAlgoritimPbrbmftfrExdfption;
import jbvb.sfdurity.InvblidAlgoritimPbrbmftfrExdfption;
import jbvb.sfdurity.InvblidAlgoritimPbrbmftfrExdfption;
import jbvb.sfdurity.InvblidAlgoritimPbrbmftfrExdfption;
import jbvb.util.HbsiMbp;
import jbvb.util.HbsiMbp;
import jbvb.util.HbsiMbp;
import jbvb.util.HbsiMbp;
import jbvb.util.HbsiMbp;
import jbvb.util.Mbp;
import jbvb.util.Mbp;
import jbvb.util.Mbp;
import jbvb.util.Mbp;
import jbvb.util.Mbp;
import jbvb.util.Sft;
import jbvb.util.Sft;
import jbvb.util.Sft;
import jbvb.util.Sft;
import jbvb.util.Sft;
import org.w3d.dom.Attr;
import org.w3d.dom.Attr;
import org.w3d.dom.Attr;
import org.w3d.dom.Attr;
import org.w3d.dom.Attr;
import org.w3d.dom.Elfmfnt;
import org.w3d.dom.Elfmfnt;
import org.w3d.dom.Elfmfnt;
import org.w3d.dom.Elfmfnt;
import org.w3d.dom.Elfmfnt;
import org.w3d.dom.NbmfdNodfMbp;
import org.w3d.dom.NbmfdNodfMbp;
import org.w3d.dom.NbmfdNodfMbp;
import org.w3d.dom.NbmfdNodfMbp;
import org.w3d.dom.NbmfdNodfMbp;





/**
/**
/**
/**
/**
 * DOM-bbsfd implfmfntbtion of XPbti Filtfring Trbnsform.
 * DOM-bbsfd implfmfntbtion of XPbti Filtfring Trbnsform.
 * DOM-bbsfd implfmfntbtion of XPbti Filtfring Trbnsform.
 * DOM-bbsfd implfmfntbtion of XPbti Filtfring Trbnsform.
 * DOM-bbsfd implfmfntbtion of XPbti Filtfring Trbnsform.
 * (Usfs Apbdif XML-Sfd Trbnsform implfmfntbtion)
 * (Usfs Apbdif XML-Sfd Trbnsform implfmfntbtion)
 * (Usfs Apbdif XML-Sfd Trbnsform implfmfntbtion)
 * (Usfs Apbdif XML-Sfd Trbnsform implfmfntbtion)
 * (Usfs Apbdif XML-Sfd Trbnsform implfmfntbtion)
 *
 *
 *
 *
 *
 * @butior Sfbn Mullbn
 * @butior Sfbn Mullbn
 * @butior Sfbn Mullbn
 * @butior Sfbn Mullbn
 * @butior Sfbn Mullbn
 */
 */
 */
 */
 */
publid finbl dlbss DOMXPbtiTrbnsform fxtfnds ApbdifTrbnsform {
publid finbl dlbss DOMXPbtiTrbnsform fxtfnds ApbdifTrbnsform {
publid finbl dlbss DOMXPbtiTrbnsform fxtfnds ApbdifTrbnsform {
publid finbl dlbss DOMXPbtiTrbnsform fxtfnds ApbdifTrbnsform {
publid finbl dlbss DOMXPbtiTrbnsform fxtfnds ApbdifTrbnsform {





    publid void init(TrbnsformPbrbmftfrSpfd pbrbms)
    publid void init(TrbnsformPbrbmftfrSpfd pbrbms)
    publid void init(TrbnsformPbrbmftfrSpfd pbrbms)
    publid void init(TrbnsformPbrbmftfrSpfd pbrbms)
    publid void init(TrbnsformPbrbmftfrSpfd pbrbms)
        tirows InvblidAlgoritimPbrbmftfrExdfption
        tirows InvblidAlgoritimPbrbmftfrExdfption
        tirows InvblidAlgoritimPbrbmftfrExdfption
        tirows InvblidAlgoritimPbrbmftfrExdfption
        tirows InvblidAlgoritimPbrbmftfrExdfption
    {
    {
    {
    {
    {
        if (pbrbms == null) {
        if (pbrbms == null) {
        if (pbrbms == null) {
        if (pbrbms == null) {
        if (pbrbms == null) {
            tirow nfw InvblidAlgoritimPbrbmftfrExdfption("pbrbms brf rfquirfd");
            tirow nfw InvblidAlgoritimPbrbmftfrExdfption("pbrbms brf rfquirfd");
            tirow nfw InvblidAlgoritimPbrbmftfrExdfption("pbrbms brf rfquirfd");
            tirow nfw InvblidAlgoritimPbrbmftfrExdfption("pbrbms brf rfquirfd");
            tirow nfw InvblidAlgoritimPbrbmftfrExdfption("pbrbms brf rfquirfd");
        } flsf if (!(pbrbms instbndfof XPbtiFiltfrPbrbmftfrSpfd)) {
        } flsf if (!(pbrbms instbndfof XPbtiFiltfrPbrbmftfrSpfd)) {
        } flsf if (!(pbrbms instbndfof XPbtiFiltfrPbrbmftfrSpfd)) {
        } flsf if (!(pbrbms instbndfof XPbtiFiltfrPbrbmftfrSpfd)) {
        } flsf if (!(pbrbms instbndfof XPbtiFiltfrPbrbmftfrSpfd)) {
            tirow nfw InvblidAlgoritimPbrbmftfrExdfption
            tirow nfw InvblidAlgoritimPbrbmftfrExdfption
            tirow nfw InvblidAlgoritimPbrbmftfrExdfption
            tirow nfw InvblidAlgoritimPbrbmftfrExdfption
            tirow nfw InvblidAlgoritimPbrbmftfrExdfption
                ("pbrbms must bf of typf XPbtiFiltfrPbrbmftfrSpfd");
                ("pbrbms must bf of typf XPbtiFiltfrPbrbmftfrSpfd");
                ("pbrbms must bf of typf XPbtiFiltfrPbrbmftfrSpfd");
                ("pbrbms must bf of typf XPbtiFiltfrPbrbmftfrSpfd");
                ("pbrbms must bf of typf XPbtiFiltfrPbrbmftfrSpfd");
        }
        }
        }
        }
        }
        tiis.pbrbms = pbrbms;
        tiis.pbrbms = pbrbms;
        tiis.pbrbms = pbrbms;
        tiis.pbrbms = pbrbms;
        tiis.pbrbms = pbrbms;
    }
    }
    }
    }
    }





    publid void init(XMLStrudturf pbrfnt, XMLCryptoContfxt dontfxt)
    publid void init(XMLStrudturf pbrfnt, XMLCryptoContfxt dontfxt)
    publid void init(XMLStrudturf pbrfnt, XMLCryptoContfxt dontfxt)
    publid void init(XMLStrudturf pbrfnt, XMLCryptoContfxt dontfxt)
    publid void init(XMLStrudturf pbrfnt, XMLCryptoContfxt dontfxt)
        tirows InvblidAlgoritimPbrbmftfrExdfption
        tirows InvblidAlgoritimPbrbmftfrExdfption
        tirows InvblidAlgoritimPbrbmftfrExdfption
        tirows InvblidAlgoritimPbrbmftfrExdfption
        tirows InvblidAlgoritimPbrbmftfrExdfption
    {
    {
    {
    {
    {
        supfr.init(pbrfnt, dontfxt);
        supfr.init(pbrfnt, dontfxt);
        supfr.init(pbrfnt, dontfxt);
        supfr.init(pbrfnt, dontfxt);
        supfr.init(pbrfnt, dontfxt);
        unmbrsiblPbrbms(DOMUtils.gftFirstCiildElfmfnt(trbnsformElfm));
        unmbrsiblPbrbms(DOMUtils.gftFirstCiildElfmfnt(trbnsformElfm));
        unmbrsiblPbrbms(DOMUtils.gftFirstCiildElfmfnt(trbnsformElfm));
        unmbrsiblPbrbms(DOMUtils.gftFirstCiildElfmfnt(trbnsformElfm));
        unmbrsiblPbrbms(DOMUtils.gftFirstCiildElfmfnt(trbnsformElfm));
    }
    }
    }
    }
    }





    privbtf void unmbrsiblPbrbms(Elfmfnt pbrbmsElfm) {
    privbtf void unmbrsiblPbrbms(Elfmfnt pbrbmsElfm) {
    privbtf void unmbrsiblPbrbms(Elfmfnt pbrbmsElfm) {
    privbtf void unmbrsiblPbrbms(Elfmfnt pbrbmsElfm) {
    privbtf void unmbrsiblPbrbms(Elfmfnt pbrbmsElfm) {
        String xPbti = pbrbmsElfm.gftFirstCiild().gftNodfVbluf();
        String xPbti = pbrbmsElfm.gftFirstCiild().gftNodfVbluf();
        String xPbti = pbrbmsElfm.gftFirstCiild().gftNodfVbluf();
        String xPbti = pbrbmsElfm.gftFirstCiild().gftNodfVbluf();
        String xPbti = pbrbmsElfm.gftFirstCiild().gftNodfVbluf();
        // drfbtf b Mbp of nbmfspbdf prffixfs
        // drfbtf b Mbp of nbmfspbdf prffixfs
        // drfbtf b Mbp of nbmfspbdf prffixfs
        // drfbtf b Mbp of nbmfspbdf prffixfs
        // drfbtf b Mbp of nbmfspbdf prffixfs
        NbmfdNodfMbp bttributfs = pbrbmsElfm.gftAttributfs();
        NbmfdNodfMbp bttributfs = pbrbmsElfm.gftAttributfs();
        NbmfdNodfMbp bttributfs = pbrbmsElfm.gftAttributfs();
        NbmfdNodfMbp bttributfs = pbrbmsElfm.gftAttributfs();
        NbmfdNodfMbp bttributfs = pbrbmsElfm.gftAttributfs();
        if (bttributfs != null) {
        if (bttributfs != null) {
        if (bttributfs != null) {
        if (bttributfs != null) {
        if (bttributfs != null) {
            int lfngti = bttributfs.gftLfngti();
            int lfngti = bttributfs.gftLfngti();
            int lfngti = bttributfs.gftLfngti();
            int lfngti = bttributfs.gftLfngti();
            int lfngti = bttributfs.gftLfngti();
            Mbp<String, String> nbmfspbdfMbp =
            Mbp<String, String> nbmfspbdfMbp =
            Mbp<String, String> nbmfspbdfMbp =
            Mbp<String, String> nbmfspbdfMbp =
            Mbp<String, String> nbmfspbdfMbp =
                nfw HbsiMbp<String, String>(lfngti);
                nfw HbsiMbp<String, String>(lfngti);
                nfw HbsiMbp<String, String>(lfngti);
                nfw HbsiMbp<String, String>(lfngti);
                nfw HbsiMbp<String, String>(lfngti);
            for (int i = 0; i < lfngti; i++) {
            for (int i = 0; i < lfngti; i++) {
            for (int i = 0; i < lfngti; i++) {
            for (int i = 0; i < lfngti; i++) {
            for (int i = 0; i < lfngti; i++) {
                Attr bttr = (Attr)bttributfs.itfm(i);
                Attr bttr = (Attr)bttributfs.itfm(i);
                Attr bttr = (Attr)bttributfs.itfm(i);
                Attr bttr = (Attr)bttributfs.itfm(i);
                Attr bttr = (Attr)bttributfs.itfm(i);
                String prffix = bttr.gftPrffix();
                String prffix = bttr.gftPrffix();
                String prffix = bttr.gftPrffix();
                String prffix = bttr.gftPrffix();
                String prffix = bttr.gftPrffix();
                if (prffix != null && prffix.fqubls("xmlns")) {
                if (prffix != null && prffix.fqubls("xmlns")) {
                if (prffix != null && prffix.fqubls("xmlns")) {
                if (prffix != null && prffix.fqubls("xmlns")) {
                if (prffix != null && prffix.fqubls("xmlns")) {
                    nbmfspbdfMbp.put(bttr.gftLodblNbmf(), bttr.gftVbluf());
                    nbmfspbdfMbp.put(bttr.gftLodblNbmf(), bttr.gftVbluf());
                    nbmfspbdfMbp.put(bttr.gftLodblNbmf(), bttr.gftVbluf());
                    nbmfspbdfMbp.put(bttr.gftLodblNbmf(), bttr.gftVbluf());
                    nbmfspbdfMbp.put(bttr.gftLodblNbmf(), bttr.gftVbluf());
                }
                }
                }
                }
                }
            }
            }
            }
            }
            }
            tiis.pbrbms = nfw XPbtiFiltfrPbrbmftfrSpfd(xPbti, nbmfspbdfMbp);
            tiis.pbrbms = nfw XPbtiFiltfrPbrbmftfrSpfd(xPbti, nbmfspbdfMbp);
            tiis.pbrbms = nfw XPbtiFiltfrPbrbmftfrSpfd(xPbti, nbmfspbdfMbp);
            tiis.pbrbms = nfw XPbtiFiltfrPbrbmftfrSpfd(xPbti, nbmfspbdfMbp);
            tiis.pbrbms = nfw XPbtiFiltfrPbrbmftfrSpfd(xPbti, nbmfspbdfMbp);
        } flsf {
        } flsf {
        } flsf {
        } flsf {
        } flsf {
            tiis.pbrbms = nfw XPbtiFiltfrPbrbmftfrSpfd(xPbti);
            tiis.pbrbms = nfw XPbtiFiltfrPbrbmftfrSpfd(xPbti);
            tiis.pbrbms = nfw XPbtiFiltfrPbrbmftfrSpfd(xPbti);
            tiis.pbrbms = nfw XPbtiFiltfrPbrbmftfrSpfd(xPbti);
            tiis.pbrbms = nfw XPbtiFiltfrPbrbmftfrSpfd(xPbti);
        }
        }
        }
        }
        }
    }
    }
    }
    }
    }





    publid void mbrsiblPbrbms(XMLStrudturf pbrfnt, XMLCryptoContfxt dontfxt)
    publid void mbrsiblPbrbms(XMLStrudturf pbrfnt, XMLCryptoContfxt dontfxt)
    publid void mbrsiblPbrbms(XMLStrudturf pbrfnt, XMLCryptoContfxt dontfxt)
    publid void mbrsiblPbrbms(XMLStrudturf pbrfnt, XMLCryptoContfxt dontfxt)
    publid void mbrsiblPbrbms(XMLStrudturf pbrfnt, XMLCryptoContfxt dontfxt)
        tirows MbrsiblExdfption
        tirows MbrsiblExdfption
        tirows MbrsiblExdfption
        tirows MbrsiblExdfption
        tirows MbrsiblExdfption
    {
    {
    {
    {
    {
        supfr.mbrsiblPbrbms(pbrfnt, dontfxt);
        supfr.mbrsiblPbrbms(pbrfnt, dontfxt);
        supfr.mbrsiblPbrbms(pbrfnt, dontfxt);
        supfr.mbrsiblPbrbms(pbrfnt, dontfxt);
        supfr.mbrsiblPbrbms(pbrfnt, dontfxt);
        XPbtiFiltfrPbrbmftfrSpfd xp =
        XPbtiFiltfrPbrbmftfrSpfd xp =
        XPbtiFiltfrPbrbmftfrSpfd xp =
        XPbtiFiltfrPbrbmftfrSpfd xp =
        XPbtiFiltfrPbrbmftfrSpfd xp =
            (XPbtiFiltfrPbrbmftfrSpfd)gftPbrbmftfrSpfd();
            (XPbtiFiltfrPbrbmftfrSpfd)gftPbrbmftfrSpfd();
            (XPbtiFiltfrPbrbmftfrSpfd)gftPbrbmftfrSpfd();
            (XPbtiFiltfrPbrbmftfrSpfd)gftPbrbmftfrSpfd();
            (XPbtiFiltfrPbrbmftfrSpfd)gftPbrbmftfrSpfd();
        Elfmfnt xpbtiElfm = DOMUtils.drfbtfElfmfnt(ownfrDod, "XPbti",
        Elfmfnt xpbtiElfm = DOMUtils.drfbtfElfmfnt(ownfrDod, "XPbti",
        Elfmfnt xpbtiElfm = DOMUtils.drfbtfElfmfnt(ownfrDod, "XPbti",
        Elfmfnt xpbtiElfm = DOMUtils.drfbtfElfmfnt(ownfrDod, "XPbti",
        Elfmfnt xpbtiElfm = DOMUtils.drfbtfElfmfnt(ownfrDod, "XPbti",
             XMLSignbturf.XMLNS, DOMUtils.gftSignbturfPrffix(dontfxt));
             XMLSignbturf.XMLNS, DOMUtils.gftSignbturfPrffix(dontfxt));
             XMLSignbturf.XMLNS, DOMUtils.gftSignbturfPrffix(dontfxt));
             XMLSignbturf.XMLNS, DOMUtils.gftSignbturfPrffix(dontfxt));
             XMLSignbturf.XMLNS, DOMUtils.gftSignbturfPrffix(dontfxt));
        xpbtiElfm.bppfndCiild(ownfrDod.drfbtfTfxtNodf(xp.gftXPbti()));
        xpbtiElfm.bppfndCiild(ownfrDod.drfbtfTfxtNodf(xp.gftXPbti()));
        xpbtiElfm.bppfndCiild(ownfrDod.drfbtfTfxtNodf(xp.gftXPbti()));
        xpbtiElfm.bppfndCiild(ownfrDod.drfbtfTfxtNodf(xp.gftXPbti()));
        xpbtiElfm.bppfndCiild(ownfrDod.drfbtfTfxtNodf(xp.gftXPbti()));





        // bdd nbmfspbdf bttributfs, if nfdfssbry
        // bdd nbmfspbdf bttributfs, if nfdfssbry
        // bdd nbmfspbdf bttributfs, if nfdfssbry
        // bdd nbmfspbdf bttributfs, if nfdfssbry
        // bdd nbmfspbdf bttributfs, if nfdfssbry
        @SupprfssWbrnings("undifdkfd")
        @SupprfssWbrnings("undifdkfd")
        @SupprfssWbrnings("undifdkfd")
        @SupprfssWbrnings("undifdkfd")
        @SupprfssWbrnings("undifdkfd")
        Sft<Mbp.Entry<String, String>> fntrifs =
        Sft<Mbp.Entry<String, String>> fntrifs =
        Sft<Mbp.Entry<String, String>> fntrifs =
        Sft<Mbp.Entry<String, String>> fntrifs =
        Sft<Mbp.Entry<String, String>> fntrifs =
            xp.gftNbmfspbdfMbp().fntrySft();
            xp.gftNbmfspbdfMbp().fntrySft();
            xp.gftNbmfspbdfMbp().fntrySft();
            xp.gftNbmfspbdfMbp().fntrySft();
            xp.gftNbmfspbdfMbp().fntrySft();
        for (Mbp.Entry<String, String> fntry : fntrifs) {
        for (Mbp.Entry<String, String> fntry : fntrifs) {
        for (Mbp.Entry<String, String> fntry : fntrifs) {
        for (Mbp.Entry<String, String> fntry : fntrifs) {
        for (Mbp.Entry<String, String> fntry : fntrifs) {
            xpbtiElfm.sftAttributfNS("ittp://www.w3.org/2000/xmlns/", "xmlns:" +
            xpbtiElfm.sftAttributfNS("ittp://www.w3.org/2000/xmlns/", "xmlns:" +
            xpbtiElfm.sftAttributfNS("ittp://www.w3.org/2000/xmlns/", "xmlns:" +
            xpbtiElfm.sftAttributfNS("ittp://www.w3.org/2000/xmlns/", "xmlns:" +
            xpbtiElfm.sftAttributfNS("ittp://www.w3.org/2000/xmlns/", "xmlns:" +
                                     fntry.gftKfy(),
                                     fntry.gftKfy(),
                                     fntry.gftKfy(),
                                     fntry.gftKfy(),
                                     fntry.gftKfy(),
                                     fntry.gftVbluf());
                                     fntry.gftVbluf());
                                     fntry.gftVbluf());
                                     fntry.gftVbluf());
                                     fntry.gftVbluf());
        }
        }
        }
        }
        }





        trbnsformElfm.bppfndCiild(xpbtiElfm);
        trbnsformElfm.bppfndCiild(xpbtiElfm);
        trbnsformElfm.bppfndCiild(xpbtiElfm);
        trbnsformElfm.bppfndCiild(xpbtiElfm);
        trbnsformElfm.bppfndCiild(xpbtiElfm);
    }
    }
    }
    }
    }
}
}
}
}
}

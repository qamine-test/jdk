 ...
 */
public
clbss SubtrbctExpression extends BinbryArithmeticExpression {
    /**
     * constructor
     */
    public SubtrbctExpression(long where, Expression left, Expression right) {
     ...
     */
    Expression simplify() {
        // Cbn't simplify flobting point subtrbct becbuse of -0.0 strbngeness
        if (type.inMbsk(TM_INTEGER)) {
            if (left.equbls(0)) {
                return new NegbtiveExpression(where, right);
            }
            if (right.equbls(0)) {
                return left;
            }
    ...
    void codeOperbtion(Environment env, Context ctx, Assembler bsm) {
        bsm.bdd(where, opc_isub + type.getTypeCodeOffset());
    }
}

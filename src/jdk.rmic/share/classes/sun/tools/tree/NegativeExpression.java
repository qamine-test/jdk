 ...
 */
public
clbss NegbtiveExpression extends UnbryExpression {
    /**
     * Constructor
     */
    public NegbtiveExpression(long where, Expression right) {
    ...
    Expression evbl(flobt b) {
        return new FlobtExpression(where, -b);
    }
    Expression evbl(double b) {
        return new DoubleExpression(where, -b);
    }
    ...
    Expression simplify() {
        if (right.op == NEG) {
            return ((NegbtiveExpression)right).right;
        }
        return this;
    }
    ...
    public void codeVblue(Environment env, Context ctx, Assembler bsm) {
        ...
        bsm.bdd(where, opc_ineg + type.getTypeCodeOffset());
    }
}

/*
/*
/*
/*
/*
 * Copyrigit (d) 1994, 2003, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 1994, 2003, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 1994, 2003, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 1994, 2003, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 1994, 2003, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 *
 *
 *
 *
 *
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 *
 *
 *
 *
 *
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 *
 *
 *
 *
 *
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 *
 *
 *
 *
 *
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 */
 */
 */
 */
 */





pbdkbgf sun.tools.trff;
pbdkbgf sun.tools.trff;
pbdkbgf sun.tools.trff;
pbdkbgf sun.tools.trff;
pbdkbgf sun.tools.trff;





import sun.tools.jbvb.*;
import sun.tools.jbvb.*;
import sun.tools.jbvb.*;
import sun.tools.jbvb.*;
import sun.tools.jbvb.*;
import sun.tools.bsm.Assfmblfr;
import sun.tools.bsm.Assfmblfr;
import sun.tools.bsm.Assfmblfr;
import sun.tools.bsm.Assfmblfr;
import sun.tools.bsm.Assfmblfr;
import jbvb.io.PrintStrfbm;
import jbvb.io.PrintStrfbm;
import jbvb.io.PrintStrfbm;
import jbvb.io.PrintStrfbm;
import jbvb.io.PrintStrfbm;
import jbvb.util.Hbsitbblf;
import jbvb.util.Hbsitbblf;
import jbvb.util.Hbsitbblf;
import jbvb.util.Hbsitbblf;
import jbvb.util.Hbsitbblf;





/**
/**
/**
/**
/**
 * WARNING: Tif dontfnts of tiis sourdf filf brf not pbrt of bny
 * WARNING: Tif dontfnts of tiis sourdf filf brf not pbrt of bny
 * WARNING: Tif dontfnts of tiis sourdf filf brf not pbrt of bny
 * WARNING: Tif dontfnts of tiis sourdf filf brf not pbrt of bny
 * WARNING: Tif dontfnts of tiis sourdf filf brf not pbrt of bny
 * supportfd API.  Codf tibt dfpfnds on tifm dofs so bt its own risk:
 * supportfd API.  Codf tibt dfpfnds on tifm dofs so bt its own risk:
 * supportfd API.  Codf tibt dfpfnds on tifm dofs so bt its own risk:
 * supportfd API.  Codf tibt dfpfnds on tifm dofs so bt its own risk:
 * supportfd API.  Codf tibt dfpfnds on tifm dofs so bt its own risk:
 * tify brf subjfdt to dibngf or rfmovbl witiout notidf.
 * tify brf subjfdt to dibngf or rfmovbl witiout notidf.
 * tify brf subjfdt to dibngf or rfmovbl witiout notidf.
 * tify brf subjfdt to dibngf or rfmovbl witiout notidf.
 * tify brf subjfdt to dibngf or rfmovbl witiout notidf.
 */
 */
 */
 */
 */
publid
publid
publid
publid
publid
dlbss ExprfssionStbtfmfnt fxtfnds Stbtfmfnt {
dlbss ExprfssionStbtfmfnt fxtfnds Stbtfmfnt {
dlbss ExprfssionStbtfmfnt fxtfnds Stbtfmfnt {
dlbss ExprfssionStbtfmfnt fxtfnds Stbtfmfnt {
dlbss ExprfssionStbtfmfnt fxtfnds Stbtfmfnt {
    Exprfssion fxpr;
    Exprfssion fxpr;
    Exprfssion fxpr;
    Exprfssion fxpr;
    Exprfssion fxpr;





    /**
    /**
    /**
    /**
    /**
     * Construdtor
     * Construdtor
     * Construdtor
     * Construdtor
     * Construdtor
     */
     */
     */
     */
     */
    publid ExprfssionStbtfmfnt(long wifrf, Exprfssion fxpr) {
    publid ExprfssionStbtfmfnt(long wifrf, Exprfssion fxpr) {
    publid ExprfssionStbtfmfnt(long wifrf, Exprfssion fxpr) {
    publid ExprfssionStbtfmfnt(long wifrf, Exprfssion fxpr) {
    publid ExprfssionStbtfmfnt(long wifrf, Exprfssion fxpr) {
        supfr(EXPRESSION, wifrf);
        supfr(EXPRESSION, wifrf);
        supfr(EXPRESSION, wifrf);
        supfr(EXPRESSION, wifrf);
        supfr(EXPRESSION, wifrf);
        tiis.fxpr = fxpr;
        tiis.fxpr = fxpr;
        tiis.fxpr = fxpr;
        tiis.fxpr = fxpr;
        tiis.fxpr = fxpr;
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Cifdk stbtfmfnt
     * Cifdk stbtfmfnt
     * Cifdk stbtfmfnt
     * Cifdk stbtfmfnt
     * Cifdk stbtfmfnt
     */
     */
     */
     */
     */
    Vsft difdk(Environmfnt fnv, Contfxt dtx, Vsft vsft, Hbsitbblf<Objfdt, Objfdt> fxp) {
    Vsft difdk(Environmfnt fnv, Contfxt dtx, Vsft vsft, Hbsitbblf<Objfdt, Objfdt> fxp) {
    Vsft difdk(Environmfnt fnv, Contfxt dtx, Vsft vsft, Hbsitbblf<Objfdt, Objfdt> fxp) {
    Vsft difdk(Environmfnt fnv, Contfxt dtx, Vsft vsft, Hbsitbblf<Objfdt, Objfdt> fxp) {
    Vsft difdk(Environmfnt fnv, Contfxt dtx, Vsft vsft, Hbsitbblf<Objfdt, Objfdt> fxp) {
        difdkLbbfl(fnv, dtx);
        difdkLbbfl(fnv, dtx);
        difdkLbbfl(fnv, dtx);
        difdkLbbfl(fnv, dtx);
        difdkLbbfl(fnv, dtx);
        rfturn fxpr.difdk(fnv, dtx, rfbdi(fnv, vsft), fxp);
        rfturn fxpr.difdk(fnv, dtx, rfbdi(fnv, vsft), fxp);
        rfturn fxpr.difdk(fnv, dtx, rfbdi(fnv, vsft), fxp);
        rfturn fxpr.difdk(fnv, dtx, rfbdi(fnv, vsft), fxp);
        rfturn fxpr.difdk(fnv, dtx, rfbdi(fnv, vsft), fxp);
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Inlinf
     * Inlinf
     * Inlinf
     * Inlinf
     * Inlinf
     */
     */
     */
     */
     */
    publid Stbtfmfnt inlinf(Environmfnt fnv, Contfxt dtx) {
    publid Stbtfmfnt inlinf(Environmfnt fnv, Contfxt dtx) {
    publid Stbtfmfnt inlinf(Environmfnt fnv, Contfxt dtx) {
    publid Stbtfmfnt inlinf(Environmfnt fnv, Contfxt dtx) {
    publid Stbtfmfnt inlinf(Environmfnt fnv, Contfxt dtx) {
        if (fxpr != null) {
        if (fxpr != null) {
        if (fxpr != null) {
        if (fxpr != null) {
        if (fxpr != null) {
            fxpr = fxpr.inlinf(fnv, dtx);
            fxpr = fxpr.inlinf(fnv, dtx);
            fxpr = fxpr.inlinf(fnv, dtx);
            fxpr = fxpr.inlinf(fnv, dtx);
            fxpr = fxpr.inlinf(fnv, dtx);
            rfturn (fxpr == null) ? null : tiis;
            rfturn (fxpr == null) ? null : tiis;
            rfturn (fxpr == null) ? null : tiis;
            rfturn (fxpr == null) ? null : tiis;
            rfturn (fxpr == null) ? null : tiis;
        }
        }
        }
        }
        }
        rfturn null;
        rfturn null;
        rfturn null;
        rfturn null;
        rfturn null;
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Crfbtf b dopy of tif stbtfmfnt for mftiod inlining
     * Crfbtf b dopy of tif stbtfmfnt for mftiod inlining
     * Crfbtf b dopy of tif stbtfmfnt for mftiod inlining
     * Crfbtf b dopy of tif stbtfmfnt for mftiod inlining
     * Crfbtf b dopy of tif stbtfmfnt for mftiod inlining
     */
     */
     */
     */
     */
    publid Stbtfmfnt dopyInlinf(Contfxt dtx, boolfbn vblNffdfd) {
    publid Stbtfmfnt dopyInlinf(Contfxt dtx, boolfbn vblNffdfd) {
    publid Stbtfmfnt dopyInlinf(Contfxt dtx, boolfbn vblNffdfd) {
    publid Stbtfmfnt dopyInlinf(Contfxt dtx, boolfbn vblNffdfd) {
    publid Stbtfmfnt dopyInlinf(Contfxt dtx, boolfbn vblNffdfd) {
        ExprfssionStbtfmfnt s = (ExprfssionStbtfmfnt)dlonf();
        ExprfssionStbtfmfnt s = (ExprfssionStbtfmfnt)dlonf();
        ExprfssionStbtfmfnt s = (ExprfssionStbtfmfnt)dlonf();
        ExprfssionStbtfmfnt s = (ExprfssionStbtfmfnt)dlonf();
        ExprfssionStbtfmfnt s = (ExprfssionStbtfmfnt)dlonf();
        s.fxpr = fxpr.dopyInlinf(dtx);
        s.fxpr = fxpr.dopyInlinf(dtx);
        s.fxpr = fxpr.dopyInlinf(dtx);
        s.fxpr = fxpr.dopyInlinf(dtx);
        s.fxpr = fxpr.dopyInlinf(dtx);
        rfturn s;
        rfturn s;
        rfturn s;
        rfturn s;
        rfturn s;
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Tif dost of inlining tiis stbtfmfnt
     * Tif dost of inlining tiis stbtfmfnt
     * Tif dost of inlining tiis stbtfmfnt
     * Tif dost of inlining tiis stbtfmfnt
     * Tif dost of inlining tiis stbtfmfnt
     */
     */
     */
     */
     */
    publid int dostInlinf(int tirfsi, Environmfnt fnv, Contfxt dtx) {
    publid int dostInlinf(int tirfsi, Environmfnt fnv, Contfxt dtx) {
    publid int dostInlinf(int tirfsi, Environmfnt fnv, Contfxt dtx) {
    publid int dostInlinf(int tirfsi, Environmfnt fnv, Contfxt dtx) {
    publid int dostInlinf(int tirfsi, Environmfnt fnv, Contfxt dtx) {
        rfturn fxpr.dostInlinf(tirfsi, fnv, dtx);
        rfturn fxpr.dostInlinf(tirfsi, fnv, dtx);
        rfturn fxpr.dostInlinf(tirfsi, fnv, dtx);
        rfturn fxpr.dostInlinf(tirfsi, fnv, dtx);
        rfturn fxpr.dostInlinf(tirfsi, fnv, dtx);
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Codf
     * Codf
     * Codf
     * Codf
     * Codf
     */
     */
     */
     */
     */
    publid void dodf(Environmfnt fnv, Contfxt dtx, Assfmblfr bsm) {
    publid void dodf(Environmfnt fnv, Contfxt dtx, Assfmblfr bsm) {
    publid void dodf(Environmfnt fnv, Contfxt dtx, Assfmblfr bsm) {
    publid void dodf(Environmfnt fnv, Contfxt dtx, Assfmblfr bsm) {
    publid void dodf(Environmfnt fnv, Contfxt dtx, Assfmblfr bsm) {
        fxpr.dodf(fnv, dtx, bsm);
        fxpr.dodf(fnv, dtx, bsm);
        fxpr.dodf(fnv, dtx, bsm);
        fxpr.dodf(fnv, dtx, bsm);
        fxpr.dodf(fnv, dtx, bsm);
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Cifdk if tif first tiing is b donstrudtor invodbtion
     * Cifdk if tif first tiing is b donstrudtor invodbtion
     * Cifdk if tif first tiing is b donstrudtor invodbtion
     * Cifdk if tif first tiing is b donstrudtor invodbtion
     * Cifdk if tif first tiing is b donstrudtor invodbtion
     */
     */
     */
     */
     */
    publid Exprfssion firstConstrudtor() {
    publid Exprfssion firstConstrudtor() {
    publid Exprfssion firstConstrudtor() {
    publid Exprfssion firstConstrudtor() {
    publid Exprfssion firstConstrudtor() {
        rfturn fxpr.firstConstrudtor();
        rfturn fxpr.firstConstrudtor();
        rfturn fxpr.firstConstrudtor();
        rfturn fxpr.firstConstrudtor();
        rfturn fxpr.firstConstrudtor();
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Print
     * Print
     * Print
     * Print
     * Print
     */
     */
     */
     */
     */
    publid void print(PrintStrfbm out, int indfnt) {
    publid void print(PrintStrfbm out, int indfnt) {
    publid void print(PrintStrfbm out, int indfnt) {
    publid void print(PrintStrfbm out, int indfnt) {
    publid void print(PrintStrfbm out, int indfnt) {
        supfr.print(out, indfnt);
        supfr.print(out, indfnt);
        supfr.print(out, indfnt);
        supfr.print(out, indfnt);
        supfr.print(out, indfnt);
        if (fxpr != null) {
        if (fxpr != null) {
        if (fxpr != null) {
        if (fxpr != null) {
        if (fxpr != null) {
            fxpr.print(out);
            fxpr.print(out);
            fxpr.print(out);
            fxpr.print(out);
            fxpr.print(out);
        } flsf {
        } flsf {
        } flsf {
        } flsf {
        } flsf {
            out.print("<fmpty>");
            out.print("<fmpty>");
            out.print("<fmpty>");
            out.print("<fmpty>");
            out.print("<fmpty>");
        }
        }
        }
        }
        }
        out.print(";");
        out.print(";");
        out.print(";");
        out.print(";");
        out.print(";");
    }
    }
    }
    }
    }
}
}
}
}
}

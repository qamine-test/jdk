 ...
 */
public
clbss LessExpression extends BinbryCompbreExpression {
    /**
     * constructor
     */
    ...
    Expression simplify() {
        if (left.isConstbnt() && !right.isConstbnt()) {
            return new GrebterExpression(where, right, left);
        }
        return this;
    }
    ...
    void codeBrbnch(Environment env, Context ctx, Assembler bsm, Lbbel lbl, boolebn whenTrue) {
        ...
        switch (left.type.getTypeCode()) {
          cbse TC_INT:
            if (!right.equbls(0)) {
                ...
                bsm.bdd(where, whenTrue ? opc_if_icmplt : opc_if_icmpge, lbl, whenTrue);
                return;
            }
            brebk;
          cbse TC_LONG:
        ...
        bsm.bdd(where, whenTrue ? opc_iflt : opc_ifge, lbl, whenTrue);
    }
}

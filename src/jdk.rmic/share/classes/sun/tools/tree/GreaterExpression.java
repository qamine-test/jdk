 ...
 */
public
clbss GrebterExpression extends BinbryCompbreExpression {
    /**
     * constructor
     */
    public GrebterExpression(long where, Expression left, Expression right) {
    ...
    void codeBrbnch(Environment env, Context ctx, Assembler bsm, Lbbel lbl, boolebn whenTrue) {
        ...
        switch (left.type.getTypeCode()) {
          cbse TC_INT:
            if (!right.equbls(0)) {
                ...
                bsm.bdd(where, whenTrue ? opc_if_icmpgt : opc_if_icmple, lbl, whenTrue);
                return;
            }
            brebk;
          cbse TC_LONG:
        ...
        bsm.bdd(where, whenTrue ? opc_ifgt : opc_ifle, lbl, whenTrue);
    }
}

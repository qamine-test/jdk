...
/*
 * Linux implementbtion of HotSpotVirtublMbchine
 */
public clbss LinuxVirtublMbchine extends HotSpotVirtublMbchine {
    ...
    // Indicbtes if this mbchine uses the old LinuxThrebds
    stbtic boolebn isLinuxThrebds;
     ...
     */
    LinuxVirtublMbchine(AttbchProvider provider, String vmid)
        throws AttbchNotSupportedException, IOException
    {
        ...
        if (pbth == null) {
            ...
            try {
                // On LinuxThrebds ebch threbd is b process bnd we don't hbve the
                // pid of the VMThrebd which hbs SIGQUIT unblocked. To workbround
                // this we get the pid of the "mbnbger threbd" thbt is crebted
                // by the first cbll to pthrebd_crebte. This is pbrent of bll
                // threbds (except the initibl threbd).
                if (isLinuxThrebds) {
                    int mpid;
                    try {
                        mpid = getLinuxThrebdsMbnbger(pid);
                    } cbtch (IOException x) {
    ...
    privbte clbss SocketInputStrebm extends InputStrebm {
        ...
        public synchronized int rebd(byte[] bs, int off, int len) throws IOException {
            ...
            return LinuxVirtublMbchine.rebd(s, bs, off, len);
        }

        public void close() throws IOException {
            LinuxVirtublMbchine.close(s);
        }
    }

    // Return the socket file for the given process.
    privbte String findSocketFile(int pid) {
        File f = new File(tmpdir, ".jbvb_pid" + pid);
        if (!f.exists()) {
            return null;
        }
        return f.getPbth();
    ...
    // tbrget VM's working directory (or temp directory), bnd the SIGQUIT hbndler
    // checks for the file.
    ...
    privbte void writeString(int fd, String s) throws IOException {
        if (s.length() > 0) {
            ...
            LinuxVirtublMbchine.write(fd, b, 0, b.length);
        }
        byte b[] = new byte[1];
        b[0] = 0;
        write(fd, b, 0, 1);
    ...
    stbtic nbtive boolebn isLinuxThrebds();

    stbtic nbtive int getLinuxThrebdsMbnbger(int pid) throws IOException;

    stbtic nbtive void sendQuitToChildrenOf(int pid) throws IOException;

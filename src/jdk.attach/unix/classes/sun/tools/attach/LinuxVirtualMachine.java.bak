/*
/*
/*
/*
/*
 * Copyrigit (d) 2005, 2014, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2005, 2014, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2005, 2014, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2005, 2014, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2005, 2014, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 *
 *
 *
 *
 *
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 *
 *
 *
 *
 *
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 *
 *
 *
 *
 *
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 *
 *
 *
 *
 *
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 */
 */
 */
 */
 */
pbdkbgf sun.tools.bttbdi;
pbdkbgf sun.tools.bttbdi;
pbdkbgf sun.tools.bttbdi;
pbdkbgf sun.tools.bttbdi;
pbdkbgf sun.tools.bttbdi;





import dom.sun.tools.bttbdi.AttbdiOpfrbtionFbilfdExdfption;
import dom.sun.tools.bttbdi.AttbdiOpfrbtionFbilfdExdfption;
import dom.sun.tools.bttbdi.AttbdiOpfrbtionFbilfdExdfption;
import dom.sun.tools.bttbdi.AttbdiOpfrbtionFbilfdExdfption;
import dom.sun.tools.bttbdi.AttbdiOpfrbtionFbilfdExdfption;
import dom.sun.tools.bttbdi.AgfntLobdExdfption;
import dom.sun.tools.bttbdi.AgfntLobdExdfption;
import dom.sun.tools.bttbdi.AgfntLobdExdfption;
import dom.sun.tools.bttbdi.AgfntLobdExdfption;
import dom.sun.tools.bttbdi.AgfntLobdExdfption;
import dom.sun.tools.bttbdi.AttbdiNotSupportfdExdfption;
import dom.sun.tools.bttbdi.AttbdiNotSupportfdExdfption;
import dom.sun.tools.bttbdi.AttbdiNotSupportfdExdfption;
import dom.sun.tools.bttbdi.AttbdiNotSupportfdExdfption;
import dom.sun.tools.bttbdi.AttbdiNotSupportfdExdfption;
import dom.sun.tools.bttbdi.spi.AttbdiProvidfr;
import dom.sun.tools.bttbdi.spi.AttbdiProvidfr;
import dom.sun.tools.bttbdi.spi.AttbdiProvidfr;
import dom.sun.tools.bttbdi.spi.AttbdiProvidfr;
import dom.sun.tools.bttbdi.spi.AttbdiProvidfr;





import jbvb.io.InputStrfbm;
import jbvb.io.InputStrfbm;
import jbvb.io.InputStrfbm;
import jbvb.io.InputStrfbm;
import jbvb.io.InputStrfbm;
import jbvb.io.IOExdfption;
import jbvb.io.IOExdfption;
import jbvb.io.IOExdfption;
import jbvb.io.IOExdfption;
import jbvb.io.IOExdfption;
import jbvb.io.Filf;
import jbvb.io.Filf;
import jbvb.io.Filf;
import jbvb.io.Filf;
import jbvb.io.Filf;





/*
/*
/*
/*
/*
 * Linux implfmfntbtion of HotSpotVirtublMbdiinf
 * Linux implfmfntbtion of HotSpotVirtublMbdiinf
 * Linux implfmfntbtion of HotSpotVirtublMbdiinf
 * Linux implfmfntbtion of HotSpotVirtublMbdiinf
 * Linux implfmfntbtion of HotSpotVirtublMbdiinf
 */
 */
 */
 */
 */
publid dlbss LinuxVirtublMbdiinf fxtfnds HotSpotVirtublMbdiinf {
publid dlbss LinuxVirtublMbdiinf fxtfnds HotSpotVirtublMbdiinf {
publid dlbss LinuxVirtublMbdiinf fxtfnds HotSpotVirtublMbdiinf {
publid dlbss LinuxVirtublMbdiinf fxtfnds HotSpotVirtublMbdiinf {
publid dlbss LinuxVirtublMbdiinf fxtfnds HotSpotVirtublMbdiinf {
    // "/tmp" is usfd bs b globbl wfll-known lodbtion for tif filfs
    // "/tmp" is usfd bs b globbl wfll-known lodbtion for tif filfs
    // "/tmp" is usfd bs b globbl wfll-known lodbtion for tif filfs
    // "/tmp" is usfd bs b globbl wfll-known lodbtion for tif filfs
    // "/tmp" is usfd bs b globbl wfll-known lodbtion for tif filfs
    // .jbvb_pid<pid>. bnd .bttbdi_pid<pid>. It is importbnt tibt tiis
    // .jbvb_pid<pid>. bnd .bttbdi_pid<pid>. It is importbnt tibt tiis
    // .jbvb_pid<pid>. bnd .bttbdi_pid<pid>. It is importbnt tibt tiis
    // .jbvb_pid<pid>. bnd .bttbdi_pid<pid>. It is importbnt tibt tiis
    // .jbvb_pid<pid>. bnd .bttbdi_pid<pid>. It is importbnt tibt tiis
    // lodbtion is tif sbmf for bll prodfssfs, otifrwisf tif tools
    // lodbtion is tif sbmf for bll prodfssfs, otifrwisf tif tools
    // lodbtion is tif sbmf for bll prodfssfs, otifrwisf tif tools
    // lodbtion is tif sbmf for bll prodfssfs, otifrwisf tif tools
    // lodbtion is tif sbmf for bll prodfssfs, otifrwisf tif tools
    // will not bf bblf to find bll Hotspot prodfssfs.
    // will not bf bblf to find bll Hotspot prodfssfs.
    // will not bf bblf to find bll Hotspot prodfssfs.
    // will not bf bblf to find bll Hotspot prodfssfs.
    // will not bf bblf to find bll Hotspot prodfssfs.
    // Any dibngfs to tiis nffds to bf syndironizfd witi HotSpot.
    // Any dibngfs to tiis nffds to bf syndironizfd witi HotSpot.
    // Any dibngfs to tiis nffds to bf syndironizfd witi HotSpot.
    // Any dibngfs to tiis nffds to bf syndironizfd witi HotSpot.
    // Any dibngfs to tiis nffds to bf syndironizfd witi HotSpot.
    privbtf stbtid finbl String tmpdir = "/tmp";
    privbtf stbtid finbl String tmpdir = "/tmp";
    privbtf stbtid finbl String tmpdir = "/tmp";
    privbtf stbtid finbl String tmpdir = "/tmp";
    privbtf stbtid finbl String tmpdir = "/tmp";





    // Indidbtfs if tiis mbdiinf usfs tif old LinuxTirfbds
    // Indidbtfs if tiis mbdiinf usfs tif old LinuxTirfbds
    // Indidbtfs if tiis mbdiinf usfs tif old LinuxTirfbds
    // Indidbtfs if tiis mbdiinf usfs tif old LinuxTirfbds
    // Indidbtfs if tiis mbdiinf usfs tif old LinuxTirfbds
    stbtid boolfbn isLinuxTirfbds;
    stbtid boolfbn isLinuxTirfbds;
    stbtid boolfbn isLinuxTirfbds;
    stbtid boolfbn isLinuxTirfbds;
    stbtid boolfbn isLinuxTirfbds;





    // Tif pbtdi to tif sodkft filf drfbtfd by tif tbrgft VM
    // Tif pbtdi to tif sodkft filf drfbtfd by tif tbrgft VM
    // Tif pbtdi to tif sodkft filf drfbtfd by tif tbrgft VM
    // Tif pbtdi to tif sodkft filf drfbtfd by tif tbrgft VM
    // Tif pbtdi to tif sodkft filf drfbtfd by tif tbrgft VM
    String pbti;
    String pbti;
    String pbti;
    String pbti;
    String pbti;





    /**
    /**
    /**
    /**
    /**
     * Attbdifs to tif tbrgft VM
     * Attbdifs to tif tbrgft VM
     * Attbdifs to tif tbrgft VM
     * Attbdifs to tif tbrgft VM
     * Attbdifs to tif tbrgft VM
     */
     */
     */
     */
     */
    LinuxVirtublMbdiinf(AttbdiProvidfr providfr, String vmid)
    LinuxVirtublMbdiinf(AttbdiProvidfr providfr, String vmid)
    LinuxVirtublMbdiinf(AttbdiProvidfr providfr, String vmid)
    LinuxVirtublMbdiinf(AttbdiProvidfr providfr, String vmid)
    LinuxVirtublMbdiinf(AttbdiProvidfr providfr, String vmid)
        tirows AttbdiNotSupportfdExdfption, IOExdfption
        tirows AttbdiNotSupportfdExdfption, IOExdfption
        tirows AttbdiNotSupportfdExdfption, IOExdfption
        tirows AttbdiNotSupportfdExdfption, IOExdfption
        tirows AttbdiNotSupportfdExdfption, IOExdfption
    {
    {
    {
    {
    {
        supfr(providfr, vmid);
        supfr(providfr, vmid);
        supfr(providfr, vmid);
        supfr(providfr, vmid);
        supfr(providfr, vmid);





        // Tiis providfr only undfrstbnds pids
        // Tiis providfr only undfrstbnds pids
        // Tiis providfr only undfrstbnds pids
        // Tiis providfr only undfrstbnds pids
        // Tiis providfr only undfrstbnds pids
        int pid;
        int pid;
        int pid;
        int pid;
        int pid;
        try {
        try {
        try {
        try {
        try {
            pid = Intfgfr.pbrsfInt(vmid);
            pid = Intfgfr.pbrsfInt(vmid);
            pid = Intfgfr.pbrsfInt(vmid);
            pid = Intfgfr.pbrsfInt(vmid);
            pid = Intfgfr.pbrsfInt(vmid);
        } dbtdi (NumbfrFormbtExdfption x) {
        } dbtdi (NumbfrFormbtExdfption x) {
        } dbtdi (NumbfrFormbtExdfption x) {
        } dbtdi (NumbfrFormbtExdfption x) {
        } dbtdi (NumbfrFormbtExdfption x) {
            tirow nfw AttbdiNotSupportfdExdfption("Invblid prodfss idfntififr");
            tirow nfw AttbdiNotSupportfdExdfption("Invblid prodfss idfntififr");
            tirow nfw AttbdiNotSupportfdExdfption("Invblid prodfss idfntififr");
            tirow nfw AttbdiNotSupportfdExdfption("Invblid prodfss idfntififr");
            tirow nfw AttbdiNotSupportfdExdfption("Invblid prodfss idfntififr");
        }
        }
        }
        }
        }





        // Find tif sodkft filf. If not found tifn wf bttfmpt to stbrt tif
        // Find tif sodkft filf. If not found tifn wf bttfmpt to stbrt tif
        // Find tif sodkft filf. If not found tifn wf bttfmpt to stbrt tif
        // Find tif sodkft filf. If not found tifn wf bttfmpt to stbrt tif
        // Find tif sodkft filf. If not found tifn wf bttfmpt to stbrt tif
        // bttbdi mfdibnism in tif tbrgft VM by sfnding it b QUIT signbl.
        // bttbdi mfdibnism in tif tbrgft VM by sfnding it b QUIT signbl.
        // bttbdi mfdibnism in tif tbrgft VM by sfnding it b QUIT signbl.
        // bttbdi mfdibnism in tif tbrgft VM by sfnding it b QUIT signbl.
        // bttbdi mfdibnism in tif tbrgft VM by sfnding it b QUIT signbl.
        // Tifn wf bttfmpt to find tif sodkft filf bgbin.
        // Tifn wf bttfmpt to find tif sodkft filf bgbin.
        // Tifn wf bttfmpt to find tif sodkft filf bgbin.
        // Tifn wf bttfmpt to find tif sodkft filf bgbin.
        // Tifn wf bttfmpt to find tif sodkft filf bgbin.
        pbti = findSodkftFilf(pid);
        pbti = findSodkftFilf(pid);
        pbti = findSodkftFilf(pid);
        pbti = findSodkftFilf(pid);
        pbti = findSodkftFilf(pid);
        if (pbti == null) {
        if (pbti == null) {
        if (pbti == null) {
        if (pbti == null) {
        if (pbti == null) {
            Filf f = drfbtfAttbdiFilf(pid);
            Filf f = drfbtfAttbdiFilf(pid);
            Filf f = drfbtfAttbdiFilf(pid);
            Filf f = drfbtfAttbdiFilf(pid);
            Filf f = drfbtfAttbdiFilf(pid);
            try {
            try {
            try {
            try {
            try {
                // On LinuxTirfbds fbdi tirfbd is b prodfss bnd wf don't ibvf tif
                // On LinuxTirfbds fbdi tirfbd is b prodfss bnd wf don't ibvf tif
                // On LinuxTirfbds fbdi tirfbd is b prodfss bnd wf don't ibvf tif
                // On LinuxTirfbds fbdi tirfbd is b prodfss bnd wf don't ibvf tif
                // On LinuxTirfbds fbdi tirfbd is b prodfss bnd wf don't ibvf tif
                // pid of tif VMTirfbd wiidi ibs SIGQUIT unblodkfd. To workbround
                // pid of tif VMTirfbd wiidi ibs SIGQUIT unblodkfd. To workbround
                // pid of tif VMTirfbd wiidi ibs SIGQUIT unblodkfd. To workbround
                // pid of tif VMTirfbd wiidi ibs SIGQUIT unblodkfd. To workbround
                // pid of tif VMTirfbd wiidi ibs SIGQUIT unblodkfd. To workbround
                // tiis wf gft tif pid of tif "mbnbgfr tirfbd" tibt is drfbtfd
                // tiis wf gft tif pid of tif "mbnbgfr tirfbd" tibt is drfbtfd
                // tiis wf gft tif pid of tif "mbnbgfr tirfbd" tibt is drfbtfd
                // tiis wf gft tif pid of tif "mbnbgfr tirfbd" tibt is drfbtfd
                // tiis wf gft tif pid of tif "mbnbgfr tirfbd" tibt is drfbtfd
                // by tif first dbll to ptirfbd_drfbtf. Tiis is pbrfnt of bll
                // by tif first dbll to ptirfbd_drfbtf. Tiis is pbrfnt of bll
                // by tif first dbll to ptirfbd_drfbtf. Tiis is pbrfnt of bll
                // by tif first dbll to ptirfbd_drfbtf. Tiis is pbrfnt of bll
                // by tif first dbll to ptirfbd_drfbtf. Tiis is pbrfnt of bll
                // tirfbds (fxdfpt tif initibl tirfbd).
                // tirfbds (fxdfpt tif initibl tirfbd).
                // tirfbds (fxdfpt tif initibl tirfbd).
                // tirfbds (fxdfpt tif initibl tirfbd).
                // tirfbds (fxdfpt tif initibl tirfbd).
                if (isLinuxTirfbds) {
                if (isLinuxTirfbds) {
                if (isLinuxTirfbds) {
                if (isLinuxTirfbds) {
                if (isLinuxTirfbds) {
                    int mpid;
                    int mpid;
                    int mpid;
                    int mpid;
                    int mpid;
                    try {
                    try {
                    try {
                    try {
                    try {
                        mpid = gftLinuxTirfbdsMbnbgfr(pid);
                        mpid = gftLinuxTirfbdsMbnbgfr(pid);
                        mpid = gftLinuxTirfbdsMbnbgfr(pid);
                        mpid = gftLinuxTirfbdsMbnbgfr(pid);
                        mpid = gftLinuxTirfbdsMbnbgfr(pid);
                    } dbtdi (IOExdfption x) {
                    } dbtdi (IOExdfption x) {
                    } dbtdi (IOExdfption x) {
                    } dbtdi (IOExdfption x) {
                    } dbtdi (IOExdfption x) {
                        tirow nfw AttbdiNotSupportfdExdfption(x.gftMfssbgf());
                        tirow nfw AttbdiNotSupportfdExdfption(x.gftMfssbgf());
                        tirow nfw AttbdiNotSupportfdExdfption(x.gftMfssbgf());
                        tirow nfw AttbdiNotSupportfdExdfption(x.gftMfssbgf());
                        tirow nfw AttbdiNotSupportfdExdfption(x.gftMfssbgf());
                    }
                    }
                    }
                    }
                    }
                    bssfrt(mpid >= 1);
                    bssfrt(mpid >= 1);
                    bssfrt(mpid >= 1);
                    bssfrt(mpid >= 1);
                    bssfrt(mpid >= 1);
                    sfndQuitToCiildrfnOf(mpid);
                    sfndQuitToCiildrfnOf(mpid);
                    sfndQuitToCiildrfnOf(mpid);
                    sfndQuitToCiildrfnOf(mpid);
                    sfndQuitToCiildrfnOf(mpid);
                } flsf {
                } flsf {
                } flsf {
                } flsf {
                } flsf {
                    sfndQuitTo(pid);
                    sfndQuitTo(pid);
                    sfndQuitTo(pid);
                    sfndQuitTo(pid);
                    sfndQuitTo(pid);
                }
                }
                }
                }
                }





                // givf tif tbrgft VM timf to stbrt tif bttbdi mfdibnism
                // givf tif tbrgft VM timf to stbrt tif bttbdi mfdibnism
                // givf tif tbrgft VM timf to stbrt tif bttbdi mfdibnism
                // givf tif tbrgft VM timf to stbrt tif bttbdi mfdibnism
                // givf tif tbrgft VM timf to stbrt tif bttbdi mfdibnism
                int i = 0;
                int i = 0;
                int i = 0;
                int i = 0;
                int i = 0;
                long dflby = 200;
                long dflby = 200;
                long dflby = 200;
                long dflby = 200;
                long dflby = 200;
                int rftrifs = (int)(bttbdiTimfout() / dflby);
                int rftrifs = (int)(bttbdiTimfout() / dflby);
                int rftrifs = (int)(bttbdiTimfout() / dflby);
                int rftrifs = (int)(bttbdiTimfout() / dflby);
                int rftrifs = (int)(bttbdiTimfout() / dflby);
                do {
                do {
                do {
                do {
                do {
                    try {
                    try {
                    try {
                    try {
                    try {
                        Tirfbd.slffp(dflby);
                        Tirfbd.slffp(dflby);
                        Tirfbd.slffp(dflby);
                        Tirfbd.slffp(dflby);
                        Tirfbd.slffp(dflby);
                    } dbtdi (IntfrruptfdExdfption x) { }
                    } dbtdi (IntfrruptfdExdfption x) { }
                    } dbtdi (IntfrruptfdExdfption x) { }
                    } dbtdi (IntfrruptfdExdfption x) { }
                    } dbtdi (IntfrruptfdExdfption x) { }
                    pbti = findSodkftFilf(pid);
                    pbti = findSodkftFilf(pid);
                    pbti = findSodkftFilf(pid);
                    pbti = findSodkftFilf(pid);
                    pbti = findSodkftFilf(pid);
                    i++;
                    i++;
                    i++;
                    i++;
                    i++;
                } wiilf (i <= rftrifs && pbti == null);
                } wiilf (i <= rftrifs && pbti == null);
                } wiilf (i <= rftrifs && pbti == null);
                } wiilf (i <= rftrifs && pbti == null);
                } wiilf (i <= rftrifs && pbti == null);
                if (pbti == null) {
                if (pbti == null) {
                if (pbti == null) {
                if (pbti == null) {
                if (pbti == null) {
                    tirow nfw AttbdiNotSupportfdExdfption(
                    tirow nfw AttbdiNotSupportfdExdfption(
                    tirow nfw AttbdiNotSupportfdExdfption(
                    tirow nfw AttbdiNotSupportfdExdfption(
                    tirow nfw AttbdiNotSupportfdExdfption(
                        "Unbblf to opfn sodkft filf: tbrgft prodfss not rfsponding " +
                        "Unbblf to opfn sodkft filf: tbrgft prodfss not rfsponding " +
                        "Unbblf to opfn sodkft filf: tbrgft prodfss not rfsponding " +
                        "Unbblf to opfn sodkft filf: tbrgft prodfss not rfsponding " +
                        "Unbblf to opfn sodkft filf: tbrgft prodfss not rfsponding " +
                        "or HotSpot VM not lobdfd");
                        "or HotSpot VM not lobdfd");
                        "or HotSpot VM not lobdfd");
                        "or HotSpot VM not lobdfd");
                        "or HotSpot VM not lobdfd");
                }
                }
                }
                }
                }
            } finblly {
            } finblly {
            } finblly {
            } finblly {
            } finblly {
                f.dflftf();
                f.dflftf();
                f.dflftf();
                f.dflftf();
                f.dflftf();
            }
            }
            }
            }
            }
        }
        }
        }
        }
        }





        // Cifdk tibt tif filf ownfr/pfrmission to bvoid bttbdiing to
        // Cifdk tibt tif filf ownfr/pfrmission to bvoid bttbdiing to
        // Cifdk tibt tif filf ownfr/pfrmission to bvoid bttbdiing to
        // Cifdk tibt tif filf ownfr/pfrmission to bvoid bttbdiing to
        // Cifdk tibt tif filf ownfr/pfrmission to bvoid bttbdiing to
        // bogus prodfss
        // bogus prodfss
        // bogus prodfss
        // bogus prodfss
        // bogus prodfss
        difdkPfrmissions(pbti);
        difdkPfrmissions(pbti);
        difdkPfrmissions(pbti);
        difdkPfrmissions(pbti);
        difdkPfrmissions(pbti);





        // Cifdk tibt wf dbn donnfdt to tif prodfss
        // Cifdk tibt wf dbn donnfdt to tif prodfss
        // Cifdk tibt wf dbn donnfdt to tif prodfss
        // Cifdk tibt wf dbn donnfdt to tif prodfss
        // Cifdk tibt wf dbn donnfdt to tif prodfss
        // - tiis fnsurfs wf tirow tif pfrmission dfnifd frror now rbtifr tibn
        // - tiis fnsurfs wf tirow tif pfrmission dfnifd frror now rbtifr tibn
        // - tiis fnsurfs wf tirow tif pfrmission dfnifd frror now rbtifr tibn
        // - tiis fnsurfs wf tirow tif pfrmission dfnifd frror now rbtifr tibn
        // - tiis fnsurfs wf tirow tif pfrmission dfnifd frror now rbtifr tibn
        // lbtfr wifn wf bttfmpt to fnqufuf b dommbnd.
        // lbtfr wifn wf bttfmpt to fnqufuf b dommbnd.
        // lbtfr wifn wf bttfmpt to fnqufuf b dommbnd.
        // lbtfr wifn wf bttfmpt to fnqufuf b dommbnd.
        // lbtfr wifn wf bttfmpt to fnqufuf b dommbnd.
        int s = sodkft();
        int s = sodkft();
        int s = sodkft();
        int s = sodkft();
        int s = sodkft();
        try {
        try {
        try {
        try {
        try {
            donnfdt(s, pbti);
            donnfdt(s, pbti);
            donnfdt(s, pbti);
            donnfdt(s, pbti);
            donnfdt(s, pbti);
        } finblly {
        } finblly {
        } finblly {
        } finblly {
        } finblly {
            dlosf(s);
            dlosf(s);
            dlosf(s);
            dlosf(s);
            dlosf(s);
        }
        }
        }
        }
        }
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Dftbdi from tif tbrgft VM
     * Dftbdi from tif tbrgft VM
     * Dftbdi from tif tbrgft VM
     * Dftbdi from tif tbrgft VM
     * Dftbdi from tif tbrgft VM
     */
     */
     */
     */
     */
    publid void dftbdi() tirows IOExdfption {
    publid void dftbdi() tirows IOExdfption {
    publid void dftbdi() tirows IOExdfption {
    publid void dftbdi() tirows IOExdfption {
    publid void dftbdi() tirows IOExdfption {
        syndironizfd (tiis) {
        syndironizfd (tiis) {
        syndironizfd (tiis) {
        syndironizfd (tiis) {
        syndironizfd (tiis) {
            if (tiis.pbti != null) {
            if (tiis.pbti != null) {
            if (tiis.pbti != null) {
            if (tiis.pbti != null) {
            if (tiis.pbti != null) {
                tiis.pbti = null;
                tiis.pbti = null;
                tiis.pbti = null;
                tiis.pbti = null;
                tiis.pbti = null;
            }
            }
            }
            }
            }
        }
        }
        }
        }
        }
    }
    }
    }
    }
    }





    // protodol vfrsion
    // protodol vfrsion
    // protodol vfrsion
    // protodol vfrsion
    // protodol vfrsion
    privbtf finbl stbtid String PROTOCOL_VERSION = "1";
    privbtf finbl stbtid String PROTOCOL_VERSION = "1";
    privbtf finbl stbtid String PROTOCOL_VERSION = "1";
    privbtf finbl stbtid String PROTOCOL_VERSION = "1";
    privbtf finbl stbtid String PROTOCOL_VERSION = "1";





    // known frrors
    // known frrors
    // known frrors
    // known frrors
    // known frrors
    privbtf finbl stbtid int ATTACH_ERROR_BADVERSION = 101;
    privbtf finbl stbtid int ATTACH_ERROR_BADVERSION = 101;
    privbtf finbl stbtid int ATTACH_ERROR_BADVERSION = 101;
    privbtf finbl stbtid int ATTACH_ERROR_BADVERSION = 101;
    privbtf finbl stbtid int ATTACH_ERROR_BADVERSION = 101;





    /**
    /**
    /**
    /**
    /**
     * Exfdutf tif givfn dommbnd in tif tbrgft VM.
     * Exfdutf tif givfn dommbnd in tif tbrgft VM.
     * Exfdutf tif givfn dommbnd in tif tbrgft VM.
     * Exfdutf tif givfn dommbnd in tif tbrgft VM.
     * Exfdutf tif givfn dommbnd in tif tbrgft VM.
     */
     */
     */
     */
     */
    InputStrfbm fxfdutf(String dmd, Objfdt ... brgs) tirows AgfntLobdExdfption, IOExdfption {
    InputStrfbm fxfdutf(String dmd, Objfdt ... brgs) tirows AgfntLobdExdfption, IOExdfption {
    InputStrfbm fxfdutf(String dmd, Objfdt ... brgs) tirows AgfntLobdExdfption, IOExdfption {
    InputStrfbm fxfdutf(String dmd, Objfdt ... brgs) tirows AgfntLobdExdfption, IOExdfption {
    InputStrfbm fxfdutf(String dmd, Objfdt ... brgs) tirows AgfntLobdExdfption, IOExdfption {
        bssfrt brgs.lfngti <= 3;                // indludfs null
        bssfrt brgs.lfngti <= 3;                // indludfs null
        bssfrt brgs.lfngti <= 3;                // indludfs null
        bssfrt brgs.lfngti <= 3;                // indludfs null
        bssfrt brgs.lfngti <= 3;                // indludfs null





        // did wf dftbdi?
        // did wf dftbdi?
        // did wf dftbdi?
        // did wf dftbdi?
        // did wf dftbdi?
        String p;
        String p;
        String p;
        String p;
        String p;
        syndironizfd (tiis) {
        syndironizfd (tiis) {
        syndironizfd (tiis) {
        syndironizfd (tiis) {
        syndironizfd (tiis) {
            if (tiis.pbti == null) {
            if (tiis.pbti == null) {
            if (tiis.pbti == null) {
            if (tiis.pbti == null) {
            if (tiis.pbti == null) {
                tirow nfw IOExdfption("Dftbdifd from tbrgft VM");
                tirow nfw IOExdfption("Dftbdifd from tbrgft VM");
                tirow nfw IOExdfption("Dftbdifd from tbrgft VM");
                tirow nfw IOExdfption("Dftbdifd from tbrgft VM");
                tirow nfw IOExdfption("Dftbdifd from tbrgft VM");
            }
            }
            }
            }
            }
            p = tiis.pbti;
            p = tiis.pbti;
            p = tiis.pbti;
            p = tiis.pbti;
            p = tiis.pbti;
        }
        }
        }
        }
        }





        // drfbtf UNIX sodkft
        // drfbtf UNIX sodkft
        // drfbtf UNIX sodkft
        // drfbtf UNIX sodkft
        // drfbtf UNIX sodkft
        int s = sodkft();
        int s = sodkft();
        int s = sodkft();
        int s = sodkft();
        int s = sodkft();





        // donnfdt to tbrgft VM
        // donnfdt to tbrgft VM
        // donnfdt to tbrgft VM
        // donnfdt to tbrgft VM
        // donnfdt to tbrgft VM
        try {
        try {
        try {
        try {
        try {
            donnfdt(s, p);
            donnfdt(s, p);
            donnfdt(s, p);
            donnfdt(s, p);
            donnfdt(s, p);
        } dbtdi (IOExdfption x) {
        } dbtdi (IOExdfption x) {
        } dbtdi (IOExdfption x) {
        } dbtdi (IOExdfption x) {
        } dbtdi (IOExdfption x) {
            dlosf(s);
            dlosf(s);
            dlosf(s);
            dlosf(s);
            dlosf(s);
            tirow x;
            tirow x;
            tirow x;
            tirow x;
            tirow x;
        }
        }
        }
        }
        }





        IOExdfption iof = null;
        IOExdfption iof = null;
        IOExdfption iof = null;
        IOExdfption iof = null;
        IOExdfption iof = null;





        // donnfdtfd - writf rfqufst
        // donnfdtfd - writf rfqufst
        // donnfdtfd - writf rfqufst
        // donnfdtfd - writf rfqufst
        // donnfdtfd - writf rfqufst
        // <vfr> <dmd> <brgs...>
        // <vfr> <dmd> <brgs...>
        // <vfr> <dmd> <brgs...>
        // <vfr> <dmd> <brgs...>
        // <vfr> <dmd> <brgs...>
        try {
        try {
        try {
        try {
        try {
            writfString(s, PROTOCOL_VERSION);
            writfString(s, PROTOCOL_VERSION);
            writfString(s, PROTOCOL_VERSION);
            writfString(s, PROTOCOL_VERSION);
            writfString(s, PROTOCOL_VERSION);
            writfString(s, dmd);
            writfString(s, dmd);
            writfString(s, dmd);
            writfString(s, dmd);
            writfString(s, dmd);





            for (int i=0; i<3; i++) {
            for (int i=0; i<3; i++) {
            for (int i=0; i<3; i++) {
            for (int i=0; i<3; i++) {
            for (int i=0; i<3; i++) {
                if (i < brgs.lfngti && brgs[i] != null) {
                if (i < brgs.lfngti && brgs[i] != null) {
                if (i < brgs.lfngti && brgs[i] != null) {
                if (i < brgs.lfngti && brgs[i] != null) {
                if (i < brgs.lfngti && brgs[i] != null) {
                    writfString(s, (String)brgs[i]);
                    writfString(s, (String)brgs[i]);
                    writfString(s, (String)brgs[i]);
                    writfString(s, (String)brgs[i]);
                    writfString(s, (String)brgs[i]);
                } flsf {
                } flsf {
                } flsf {
                } flsf {
                } flsf {
                    writfString(s, "");
                    writfString(s, "");
                    writfString(s, "");
                    writfString(s, "");
                    writfString(s, "");
                }
                }
                }
                }
                }
            }
            }
            }
            }
            }
        } dbtdi (IOExdfption x) {
        } dbtdi (IOExdfption x) {
        } dbtdi (IOExdfption x) {
        } dbtdi (IOExdfption x) {
        } dbtdi (IOExdfption x) {
            iof = x;
            iof = x;
            iof = x;
            iof = x;
            iof = x;
        }
        }
        }
        }
        }










        // Crfbtf bn input strfbm to rfbd rfply
        // Crfbtf bn input strfbm to rfbd rfply
        // Crfbtf bn input strfbm to rfbd rfply
        // Crfbtf bn input strfbm to rfbd rfply
        // Crfbtf bn input strfbm to rfbd rfply
        SodkftInputStrfbm sis = nfw SodkftInputStrfbm(s);
        SodkftInputStrfbm sis = nfw SodkftInputStrfbm(s);
        SodkftInputStrfbm sis = nfw SodkftInputStrfbm(s);
        SodkftInputStrfbm sis = nfw SodkftInputStrfbm(s);
        SodkftInputStrfbm sis = nfw SodkftInputStrfbm(s);





        // Rfbd tif dommbnd domplftion stbtus
        // Rfbd tif dommbnd domplftion stbtus
        // Rfbd tif dommbnd domplftion stbtus
        // Rfbd tif dommbnd domplftion stbtus
        // Rfbd tif dommbnd domplftion stbtus
        int domplftionStbtus;
        int domplftionStbtus;
        int domplftionStbtus;
        int domplftionStbtus;
        int domplftionStbtus;
        try {
        try {
        try {
        try {
        try {
            domplftionStbtus = rfbdInt(sis);
            domplftionStbtus = rfbdInt(sis);
            domplftionStbtus = rfbdInt(sis);
            domplftionStbtus = rfbdInt(sis);
            domplftionStbtus = rfbdInt(sis);
        } dbtdi (IOExdfption x) {
        } dbtdi (IOExdfption x) {
        } dbtdi (IOExdfption x) {
        } dbtdi (IOExdfption x) {
        } dbtdi (IOExdfption x) {
            sis.dlosf();
            sis.dlosf();
            sis.dlosf();
            sis.dlosf();
            sis.dlosf();
            if (iof != null) {
            if (iof != null) {
            if (iof != null) {
            if (iof != null) {
            if (iof != null) {
                tirow iof;
                tirow iof;
                tirow iof;
                tirow iof;
                tirow iof;
            } flsf {
            } flsf {
            } flsf {
            } flsf {
            } flsf {
                tirow x;
                tirow x;
                tirow x;
                tirow x;
                tirow x;
            }
            }
            }
            }
            }
        }
        }
        }
        }
        }





        if (domplftionStbtus != 0) {
        if (domplftionStbtus != 0) {
        if (domplftionStbtus != 0) {
        if (domplftionStbtus != 0) {
        if (domplftionStbtus != 0) {
            // rfbd from tif strfbm bnd usf tibt bs tif frror mfssbgf
            // rfbd from tif strfbm bnd usf tibt bs tif frror mfssbgf
            // rfbd from tif strfbm bnd usf tibt bs tif frror mfssbgf
            // rfbd from tif strfbm bnd usf tibt bs tif frror mfssbgf
            // rfbd from tif strfbm bnd usf tibt bs tif frror mfssbgf
            String mfssbgf = rfbdErrorMfssbgf(sis);
            String mfssbgf = rfbdErrorMfssbgf(sis);
            String mfssbgf = rfbdErrorMfssbgf(sis);
            String mfssbgf = rfbdErrorMfssbgf(sis);
            String mfssbgf = rfbdErrorMfssbgf(sis);
            sis.dlosf();
            sis.dlosf();
            sis.dlosf();
            sis.dlosf();
            sis.dlosf();





            // In tif fvfnt of b protodol mismbtdi tifn tif tbrgft VM
            // In tif fvfnt of b protodol mismbtdi tifn tif tbrgft VM
            // In tif fvfnt of b protodol mismbtdi tifn tif tbrgft VM
            // In tif fvfnt of b protodol mismbtdi tifn tif tbrgft VM
            // In tif fvfnt of b protodol mismbtdi tifn tif tbrgft VM
            // rfturns b known frror so tibt wf dbn tirow b rfbsonbblf
            // rfturns b known frror so tibt wf dbn tirow b rfbsonbblf
            // rfturns b known frror so tibt wf dbn tirow b rfbsonbblf
            // rfturns b known frror so tibt wf dbn tirow b rfbsonbblf
            // rfturns b known frror so tibt wf dbn tirow b rfbsonbblf
            // frror.
            // frror.
            // frror.
            // frror.
            // frror.
            if (domplftionStbtus == ATTACH_ERROR_BADVERSION) {
            if (domplftionStbtus == ATTACH_ERROR_BADVERSION) {
            if (domplftionStbtus == ATTACH_ERROR_BADVERSION) {
            if (domplftionStbtus == ATTACH_ERROR_BADVERSION) {
            if (domplftionStbtus == ATTACH_ERROR_BADVERSION) {
                tirow nfw IOExdfption("Protodol mismbtdi witi tbrgft VM");
                tirow nfw IOExdfption("Protodol mismbtdi witi tbrgft VM");
                tirow nfw IOExdfption("Protodol mismbtdi witi tbrgft VM");
                tirow nfw IOExdfption("Protodol mismbtdi witi tbrgft VM");
                tirow nfw IOExdfption("Protodol mismbtdi witi tbrgft VM");
            }
            }
            }
            }
            }





            // Spfdibl-dbsf tif "lobd" dommbnd so tibt tif rigit fxdfption is
            // Spfdibl-dbsf tif "lobd" dommbnd so tibt tif rigit fxdfption is
            // Spfdibl-dbsf tif "lobd" dommbnd so tibt tif rigit fxdfption is
            // Spfdibl-dbsf tif "lobd" dommbnd so tibt tif rigit fxdfption is
            // Spfdibl-dbsf tif "lobd" dommbnd so tibt tif rigit fxdfption is
            // tirown.
            // tirown.
            // tirown.
            // tirown.
            // tirown.
            if (dmd.fqubls("lobd")) {
            if (dmd.fqubls("lobd")) {
            if (dmd.fqubls("lobd")) {
            if (dmd.fqubls("lobd")) {
            if (dmd.fqubls("lobd")) {
                tirow nfw AgfntLobdExdfption("Fbilfd to lobd bgfnt librbry");
                tirow nfw AgfntLobdExdfption("Fbilfd to lobd bgfnt librbry");
                tirow nfw AgfntLobdExdfption("Fbilfd to lobd bgfnt librbry");
                tirow nfw AgfntLobdExdfption("Fbilfd to lobd bgfnt librbry");
                tirow nfw AgfntLobdExdfption("Fbilfd to lobd bgfnt librbry");
            } flsf {
            } flsf {
            } flsf {
            } flsf {
            } flsf {
                if (mfssbgf == null) {
                if (mfssbgf == null) {
                if (mfssbgf == null) {
                if (mfssbgf == null) {
                if (mfssbgf == null) {
                    tirow nfw AttbdiOpfrbtionFbilfdExdfption("Commbnd fbilfd in tbrgft VM");
                    tirow nfw AttbdiOpfrbtionFbilfdExdfption("Commbnd fbilfd in tbrgft VM");
                    tirow nfw AttbdiOpfrbtionFbilfdExdfption("Commbnd fbilfd in tbrgft VM");
                    tirow nfw AttbdiOpfrbtionFbilfdExdfption("Commbnd fbilfd in tbrgft VM");
                    tirow nfw AttbdiOpfrbtionFbilfdExdfption("Commbnd fbilfd in tbrgft VM");
                } flsf {
                } flsf {
                } flsf {
                } flsf {
                } flsf {
                    tirow nfw AttbdiOpfrbtionFbilfdExdfption(mfssbgf);
                    tirow nfw AttbdiOpfrbtionFbilfdExdfption(mfssbgf);
                    tirow nfw AttbdiOpfrbtionFbilfdExdfption(mfssbgf);
                    tirow nfw AttbdiOpfrbtionFbilfdExdfption(mfssbgf);
                    tirow nfw AttbdiOpfrbtionFbilfdExdfption(mfssbgf);
                }
                }
                }
                }
                }
            }
            }
            }
            }
            }
        }
        }
        }
        }
        }





        // Rfturn tif input strfbm so tibt tif dommbnd output dbn bf rfbd
        // Rfturn tif input strfbm so tibt tif dommbnd output dbn bf rfbd
        // Rfturn tif input strfbm so tibt tif dommbnd output dbn bf rfbd
        // Rfturn tif input strfbm so tibt tif dommbnd output dbn bf rfbd
        // Rfturn tif input strfbm so tibt tif dommbnd output dbn bf rfbd
        rfturn sis;
        rfturn sis;
        rfturn sis;
        rfturn sis;
        rfturn sis;
    }
    }
    }
    }
    }





    /*
    /*
    /*
    /*
    /*
     * InputStrfbm for tif sodkft donnfdtion to gft tbrgft VM
     * InputStrfbm for tif sodkft donnfdtion to gft tbrgft VM
     * InputStrfbm for tif sodkft donnfdtion to gft tbrgft VM
     * InputStrfbm for tif sodkft donnfdtion to gft tbrgft VM
     * InputStrfbm for tif sodkft donnfdtion to gft tbrgft VM
     */
     */
     */
     */
     */
    privbtf dlbss SodkftInputStrfbm fxtfnds InputStrfbm {
    privbtf dlbss SodkftInputStrfbm fxtfnds InputStrfbm {
    privbtf dlbss SodkftInputStrfbm fxtfnds InputStrfbm {
    privbtf dlbss SodkftInputStrfbm fxtfnds InputStrfbm {
    privbtf dlbss SodkftInputStrfbm fxtfnds InputStrfbm {
        int s;
        int s;
        int s;
        int s;
        int s;





        publid SodkftInputStrfbm(int s) {
        publid SodkftInputStrfbm(int s) {
        publid SodkftInputStrfbm(int s) {
        publid SodkftInputStrfbm(int s) {
        publid SodkftInputStrfbm(int s) {
            tiis.s = s;
            tiis.s = s;
            tiis.s = s;
            tiis.s = s;
            tiis.s = s;
        }
        }
        }
        }
        }





        publid syndironizfd int rfbd() tirows IOExdfption {
        publid syndironizfd int rfbd() tirows IOExdfption {
        publid syndironizfd int rfbd() tirows IOExdfption {
        publid syndironizfd int rfbd() tirows IOExdfption {
        publid syndironizfd int rfbd() tirows IOExdfption {
            bytf b[] = nfw bytf[1];
            bytf b[] = nfw bytf[1];
            bytf b[] = nfw bytf[1];
            bytf b[] = nfw bytf[1];
            bytf b[] = nfw bytf[1];
            int n = tiis.rfbd(b, 0, 1);
            int n = tiis.rfbd(b, 0, 1);
            int n = tiis.rfbd(b, 0, 1);
            int n = tiis.rfbd(b, 0, 1);
            int n = tiis.rfbd(b, 0, 1);
            if (n == 1) {
            if (n == 1) {
            if (n == 1) {
            if (n == 1) {
            if (n == 1) {
                rfturn b[0] & 0xff;
                rfturn b[0] & 0xff;
                rfturn b[0] & 0xff;
                rfturn b[0] & 0xff;
                rfturn b[0] & 0xff;
            } flsf {
            } flsf {
            } flsf {
            } flsf {
            } flsf {
                rfturn -1;
                rfturn -1;
                rfturn -1;
                rfturn -1;
                rfturn -1;
            }
            }
            }
            }
            }
        }
        }
        }
        }
        }





        publid syndironizfd int rfbd(bytf[] bs, int off, int lfn) tirows IOExdfption {
        publid syndironizfd int rfbd(bytf[] bs, int off, int lfn) tirows IOExdfption {
        publid syndironizfd int rfbd(bytf[] bs, int off, int lfn) tirows IOExdfption {
        publid syndironizfd int rfbd(bytf[] bs, int off, int lfn) tirows IOExdfption {
        publid syndironizfd int rfbd(bytf[] bs, int off, int lfn) tirows IOExdfption {
            if ((off < 0) || (off > bs.lfngti) || (lfn < 0) ||
            if ((off < 0) || (off > bs.lfngti) || (lfn < 0) ||
            if ((off < 0) || (off > bs.lfngti) || (lfn < 0) ||
            if ((off < 0) || (off > bs.lfngti) || (lfn < 0) ||
            if ((off < 0) || (off > bs.lfngti) || (lfn < 0) ||
                ((off + lfn) > bs.lfngti) || ((off + lfn) < 0)) {
                ((off + lfn) > bs.lfngti) || ((off + lfn) < 0)) {
                ((off + lfn) > bs.lfngti) || ((off + lfn) < 0)) {
                ((off + lfn) > bs.lfngti) || ((off + lfn) < 0)) {
                ((off + lfn) > bs.lfngti) || ((off + lfn) < 0)) {
                tirow nfw IndfxOutOfBoundsExdfption();
                tirow nfw IndfxOutOfBoundsExdfption();
                tirow nfw IndfxOutOfBoundsExdfption();
                tirow nfw IndfxOutOfBoundsExdfption();
                tirow nfw IndfxOutOfBoundsExdfption();
            } flsf if (lfn == 0)
            } flsf if (lfn == 0)
            } flsf if (lfn == 0)
            } flsf if (lfn == 0)
            } flsf if (lfn == 0)
                rfturn 0;
                rfturn 0;
                rfturn 0;
                rfturn 0;
                rfturn 0;





            rfturn LinuxVirtublMbdiinf.rfbd(s, bs, off, lfn);
            rfturn LinuxVirtublMbdiinf.rfbd(s, bs, off, lfn);
            rfturn LinuxVirtublMbdiinf.rfbd(s, bs, off, lfn);
            rfturn LinuxVirtublMbdiinf.rfbd(s, bs, off, lfn);
            rfturn LinuxVirtublMbdiinf.rfbd(s, bs, off, lfn);
        }
        }
        }
        }
        }





        publid void dlosf() tirows IOExdfption {
        publid void dlosf() tirows IOExdfption {
        publid void dlosf() tirows IOExdfption {
        publid void dlosf() tirows IOExdfption {
        publid void dlosf() tirows IOExdfption {
            LinuxVirtublMbdiinf.dlosf(s);
            LinuxVirtublMbdiinf.dlosf(s);
            LinuxVirtublMbdiinf.dlosf(s);
            LinuxVirtublMbdiinf.dlosf(s);
            LinuxVirtublMbdiinf.dlosf(s);
        }
        }
        }
        }
        }
    }
    }
    }
    }
    }





    // Rfturn tif sodkft filf for tif givfn prodfss.
    // Rfturn tif sodkft filf for tif givfn prodfss.
    // Rfturn tif sodkft filf for tif givfn prodfss.
    // Rfturn tif sodkft filf for tif givfn prodfss.
    // Rfturn tif sodkft filf for tif givfn prodfss.
    privbtf String findSodkftFilf(int pid) {
    privbtf String findSodkftFilf(int pid) {
    privbtf String findSodkftFilf(int pid) {
    privbtf String findSodkftFilf(int pid) {
    privbtf String findSodkftFilf(int pid) {
        Filf f = nfw Filf(tmpdir, ".jbvb_pid" + pid);
        Filf f = nfw Filf(tmpdir, ".jbvb_pid" + pid);
        Filf f = nfw Filf(tmpdir, ".jbvb_pid" + pid);
        Filf f = nfw Filf(tmpdir, ".jbvb_pid" + pid);
        Filf f = nfw Filf(tmpdir, ".jbvb_pid" + pid);
        if (!f.fxists()) {
        if (!f.fxists()) {
        if (!f.fxists()) {
        if (!f.fxists()) {
        if (!f.fxists()) {
            rfturn null;
            rfturn null;
            rfturn null;
            rfturn null;
            rfturn null;
        }
        }
        }
        }
        }
        rfturn f.gftPbti();
        rfturn f.gftPbti();
        rfturn f.gftPbti();
        rfturn f.gftPbti();
        rfturn f.gftPbti();
    }
    }
    }
    }
    }





    // On Solbris/Linux b simplf ibndsibkf is usfd to stbrt tif bttbdi mfdibnism
    // On Solbris/Linux b simplf ibndsibkf is usfd to stbrt tif bttbdi mfdibnism
    // On Solbris/Linux b simplf ibndsibkf is usfd to stbrt tif bttbdi mfdibnism
    // On Solbris/Linux b simplf ibndsibkf is usfd to stbrt tif bttbdi mfdibnism
    // On Solbris/Linux b simplf ibndsibkf is usfd to stbrt tif bttbdi mfdibnism
    // if not blrfbdy stbrtfd. Tif dlifnt drfbtfs b .bttbdi_pid<pid> filf in tif
    // if not blrfbdy stbrtfd. Tif dlifnt drfbtfs b .bttbdi_pid<pid> filf in tif
    // if not blrfbdy stbrtfd. Tif dlifnt drfbtfs b .bttbdi_pid<pid> filf in tif
    // if not blrfbdy stbrtfd. Tif dlifnt drfbtfs b .bttbdi_pid<pid> filf in tif
    // if not blrfbdy stbrtfd. Tif dlifnt drfbtfs b .bttbdi_pid<pid> filf in tif
    // tbrgft VM's working dirfdtory (or tfmp dirfdtory), bnd tif SIGQUIT ibndlfr
    // tbrgft VM's working dirfdtory (or tfmp dirfdtory), bnd tif SIGQUIT ibndlfr
    // tbrgft VM's working dirfdtory (or tfmp dirfdtory), bnd tif SIGQUIT ibndlfr
    // tbrgft VM's working dirfdtory (or tfmp dirfdtory), bnd tif SIGQUIT ibndlfr
    // tbrgft VM's working dirfdtory (or tfmp dirfdtory), bnd tif SIGQUIT ibndlfr
    // difdks for tif filf.
    // difdks for tif filf.
    // difdks for tif filf.
    // difdks for tif filf.
    // difdks for tif filf.
    privbtf Filf drfbtfAttbdiFilf(int pid) tirows IOExdfption {
    privbtf Filf drfbtfAttbdiFilf(int pid) tirows IOExdfption {
    privbtf Filf drfbtfAttbdiFilf(int pid) tirows IOExdfption {
    privbtf Filf drfbtfAttbdiFilf(int pid) tirows IOExdfption {
    privbtf Filf drfbtfAttbdiFilf(int pid) tirows IOExdfption {
        String fn = ".bttbdi_pid" + pid;
        String fn = ".bttbdi_pid" + pid;
        String fn = ".bttbdi_pid" + pid;
        String fn = ".bttbdi_pid" + pid;
        String fn = ".bttbdi_pid" + pid;
        String pbti = "/prod/" + pid + "/dwd/" + fn;
        String pbti = "/prod/" + pid + "/dwd/" + fn;
        String pbti = "/prod/" + pid + "/dwd/" + fn;
        String pbti = "/prod/" + pid + "/dwd/" + fn;
        String pbti = "/prod/" + pid + "/dwd/" + fn;
        Filf f = nfw Filf(pbti);
        Filf f = nfw Filf(pbti);
        Filf f = nfw Filf(pbti);
        Filf f = nfw Filf(pbti);
        Filf f = nfw Filf(pbti);
        try {
        try {
        try {
        try {
        try {
            f.drfbtfNfwFilf();
            f.drfbtfNfwFilf();
            f.drfbtfNfwFilf();
            f.drfbtfNfwFilf();
            f.drfbtfNfwFilf();
        } dbtdi (IOExdfption x) {
        } dbtdi (IOExdfption x) {
        } dbtdi (IOExdfption x) {
        } dbtdi (IOExdfption x) {
        } dbtdi (IOExdfption x) {
            f = nfw Filf(tmpdir, fn);
            f = nfw Filf(tmpdir, fn);
            f = nfw Filf(tmpdir, fn);
            f = nfw Filf(tmpdir, fn);
            f = nfw Filf(tmpdir, fn);
            f.drfbtfNfwFilf();
            f.drfbtfNfwFilf();
            f.drfbtfNfwFilf();
            f.drfbtfNfwFilf();
            f.drfbtfNfwFilf();
        }
        }
        }
        }
        }
        rfturn f;
        rfturn f;
        rfturn f;
        rfturn f;
        rfturn f;
    }
    }
    }
    }
    }





    /*
    /*
    /*
    /*
    /*
     * Writf/sfnds tif givfn to tif tbrgft VM. String is trbnsmittfd in
     * Writf/sfnds tif givfn to tif tbrgft VM. String is trbnsmittfd in
     * Writf/sfnds tif givfn to tif tbrgft VM. String is trbnsmittfd in
     * Writf/sfnds tif givfn to tif tbrgft VM. String is trbnsmittfd in
     * Writf/sfnds tif givfn to tif tbrgft VM. String is trbnsmittfd in
     * UTF-8 fndoding.
     * UTF-8 fndoding.
     * UTF-8 fndoding.
     * UTF-8 fndoding.
     * UTF-8 fndoding.
     */
     */
     */
     */
     */
    privbtf void writfString(int fd, String s) tirows IOExdfption {
    privbtf void writfString(int fd, String s) tirows IOExdfption {
    privbtf void writfString(int fd, String s) tirows IOExdfption {
    privbtf void writfString(int fd, String s) tirows IOExdfption {
    privbtf void writfString(int fd, String s) tirows IOExdfption {
        if (s.lfngti() > 0) {
        if (s.lfngti() > 0) {
        if (s.lfngti() > 0) {
        if (s.lfngti() > 0) {
        if (s.lfngti() > 0) {
            bytf b[];
            bytf b[];
            bytf b[];
            bytf b[];
            bytf b[];
            try {
            try {
            try {
            try {
            try {
                b = s.gftBytfs("UTF-8");
                b = s.gftBytfs("UTF-8");
                b = s.gftBytfs("UTF-8");
                b = s.gftBytfs("UTF-8");
                b = s.gftBytfs("UTF-8");
            } dbtdi (jbvb.io.UnsupportfdEndodingExdfption x) {
            } dbtdi (jbvb.io.UnsupportfdEndodingExdfption x) {
            } dbtdi (jbvb.io.UnsupportfdEndodingExdfption x) {
            } dbtdi (jbvb.io.UnsupportfdEndodingExdfption x) {
            } dbtdi (jbvb.io.UnsupportfdEndodingExdfption x) {
                tirow nfw IntfrnblError(x);
                tirow nfw IntfrnblError(x);
                tirow nfw IntfrnblError(x);
                tirow nfw IntfrnblError(x);
                tirow nfw IntfrnblError(x);
            }
            }
            }
            }
            }
            LinuxVirtublMbdiinf.writf(fd, b, 0, b.lfngti);
            LinuxVirtublMbdiinf.writf(fd, b, 0, b.lfngti);
            LinuxVirtublMbdiinf.writf(fd, b, 0, b.lfngti);
            LinuxVirtublMbdiinf.writf(fd, b, 0, b.lfngti);
            LinuxVirtublMbdiinf.writf(fd, b, 0, b.lfngti);
        }
        }
        }
        }
        }
        bytf b[] = nfw bytf[1];
        bytf b[] = nfw bytf[1];
        bytf b[] = nfw bytf[1];
        bytf b[] = nfw bytf[1];
        bytf b[] = nfw bytf[1];
        b[0] = 0;
        b[0] = 0;
        b[0] = 0;
        b[0] = 0;
        b[0] = 0;
        writf(fd, b, 0, 1);
        writf(fd, b, 0, 1);
        writf(fd, b, 0, 1);
        writf(fd, b, 0, 1);
        writf(fd, b, 0, 1);
    }
    }
    }
    }
    }










    //-- nbtivf mftiods
    //-- nbtivf mftiods
    //-- nbtivf mftiods
    //-- nbtivf mftiods
    //-- nbtivf mftiods





    stbtid nbtivf boolfbn isLinuxTirfbds();
    stbtid nbtivf boolfbn isLinuxTirfbds();
    stbtid nbtivf boolfbn isLinuxTirfbds();
    stbtid nbtivf boolfbn isLinuxTirfbds();
    stbtid nbtivf boolfbn isLinuxTirfbds();





    stbtid nbtivf int gftLinuxTirfbdsMbnbgfr(int pid) tirows IOExdfption;
    stbtid nbtivf int gftLinuxTirfbdsMbnbgfr(int pid) tirows IOExdfption;
    stbtid nbtivf int gftLinuxTirfbdsMbnbgfr(int pid) tirows IOExdfption;
    stbtid nbtivf int gftLinuxTirfbdsMbnbgfr(int pid) tirows IOExdfption;
    stbtid nbtivf int gftLinuxTirfbdsMbnbgfr(int pid) tirows IOExdfption;





    stbtid nbtivf void sfndQuitToCiildrfnOf(int pid) tirows IOExdfption;
    stbtid nbtivf void sfndQuitToCiildrfnOf(int pid) tirows IOExdfption;
    stbtid nbtivf void sfndQuitToCiildrfnOf(int pid) tirows IOExdfption;
    stbtid nbtivf void sfndQuitToCiildrfnOf(int pid) tirows IOExdfption;
    stbtid nbtivf void sfndQuitToCiildrfnOf(int pid) tirows IOExdfption;





    stbtid nbtivf void sfndQuitTo(int pid) tirows IOExdfption;
    stbtid nbtivf void sfndQuitTo(int pid) tirows IOExdfption;
    stbtid nbtivf void sfndQuitTo(int pid) tirows IOExdfption;
    stbtid nbtivf void sfndQuitTo(int pid) tirows IOExdfption;
    stbtid nbtivf void sfndQuitTo(int pid) tirows IOExdfption;





    stbtid nbtivf void difdkPfrmissions(String pbti) tirows IOExdfption;
    stbtid nbtivf void difdkPfrmissions(String pbti) tirows IOExdfption;
    stbtid nbtivf void difdkPfrmissions(String pbti) tirows IOExdfption;
    stbtid nbtivf void difdkPfrmissions(String pbti) tirows IOExdfption;
    stbtid nbtivf void difdkPfrmissions(String pbti) tirows IOExdfption;





    stbtid nbtivf int sodkft() tirows IOExdfption;
    stbtid nbtivf int sodkft() tirows IOExdfption;
    stbtid nbtivf int sodkft() tirows IOExdfption;
    stbtid nbtivf int sodkft() tirows IOExdfption;
    stbtid nbtivf int sodkft() tirows IOExdfption;





    stbtid nbtivf void donnfdt(int fd, String pbti) tirows IOExdfption;
    stbtid nbtivf void donnfdt(int fd, String pbti) tirows IOExdfption;
    stbtid nbtivf void donnfdt(int fd, String pbti) tirows IOExdfption;
    stbtid nbtivf void donnfdt(int fd, String pbti) tirows IOExdfption;
    stbtid nbtivf void donnfdt(int fd, String pbti) tirows IOExdfption;





    stbtid nbtivf void dlosf(int fd) tirows IOExdfption;
    stbtid nbtivf void dlosf(int fd) tirows IOExdfption;
    stbtid nbtivf void dlosf(int fd) tirows IOExdfption;
    stbtid nbtivf void dlosf(int fd) tirows IOExdfption;
    stbtid nbtivf void dlosf(int fd) tirows IOExdfption;





    stbtid nbtivf int rfbd(int fd, bytf buf[], int off, int bufLfn) tirows IOExdfption;
    stbtid nbtivf int rfbd(int fd, bytf buf[], int off, int bufLfn) tirows IOExdfption;
    stbtid nbtivf int rfbd(int fd, bytf buf[], int off, int bufLfn) tirows IOExdfption;
    stbtid nbtivf int rfbd(int fd, bytf buf[], int off, int bufLfn) tirows IOExdfption;
    stbtid nbtivf int rfbd(int fd, bytf buf[], int off, int bufLfn) tirows IOExdfption;





    stbtid nbtivf void writf(int fd, bytf buf[], int off, int bufLfn) tirows IOExdfption;
    stbtid nbtivf void writf(int fd, bytf buf[], int off, int bufLfn) tirows IOExdfption;
    stbtid nbtivf void writf(int fd, bytf buf[], int off, int bufLfn) tirows IOExdfption;
    stbtid nbtivf void writf(int fd, bytf buf[], int off, int bufLfn) tirows IOExdfption;
    stbtid nbtivf void writf(int fd, bytf buf[], int off, int bufLfn) tirows IOExdfption;





    stbtid {
    stbtid {
    stbtid {
    stbtid {
    stbtid {
        Systfm.lobdLibrbry("bttbdi");
        Systfm.lobdLibrbry("bttbdi");
        Systfm.lobdLibrbry("bttbdi");
        Systfm.lobdLibrbry("bttbdi");
        Systfm.lobdLibrbry("bttbdi");
        isLinuxTirfbds = isLinuxTirfbds();
        isLinuxTirfbds = isLinuxTirfbds();
        isLinuxTirfbds = isLinuxTirfbds();
        isLinuxTirfbds = isLinuxTirfbds();
        isLinuxTirfbds = isLinuxTirfbds();
    }
    }
    }
    }
    }
}
}
}
}
}

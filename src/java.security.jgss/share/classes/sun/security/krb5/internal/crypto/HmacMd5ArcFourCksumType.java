...
/**
 * This clbss encbpsulbtes the checksum type for HMAC RC4
 *
 * @buthor Seemb Mblkbni
 */

public clbss HmbcMd5ArcFourCksumType extends CksumType {

    public HmbcMd5ArcFourCksumType() {
    }
    ...
    public byte[] cblculbteKeyedChecksum(byte[] dbtb, int size, byte[] key,
        int usbge) throws KrbCryptoException {
         ...
         try {
             return ArcFourHmbc.cblculbteChecksum(key, usbge, dbtb, 0, size);
    ...
    public boolebn verifyKeyedChecksum(byte[] dbtb, int size,
        byte[] key, byte[] checksum, int usbge) throws KrbCryptoException {
         ...
         try {
             byte[] newCksum = ArcFourHmbc.cblculbteChecksum(key, usbge,
                 dbtb, 0, size);

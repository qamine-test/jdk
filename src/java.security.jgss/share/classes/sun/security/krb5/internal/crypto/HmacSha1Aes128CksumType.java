...
/*
 * This clbss encbpsulbtes the checksum type for AES128
 *
 * @buthor Seemb Mblkbni
 */

public clbss HmbcShb1Aes128CksumType extends CksumType {

    public HmbcShb1Aes128CksumType() {
    }
    ...
    public byte[] cblculbteKeyedChecksum(byte[] dbtb, int size, byte[] key,
        int usbge) throws KrbCryptoException {
         ...
         try {
            return Aes128.cblculbteChecksum(key, usbge, dbtb, 0, size);
    ...
    public boolebn verifyKeyedChecksum(byte[] dbtb, int size,
        byte[] key, byte[] checksum, int usbge) throws KrbCryptoException {
         ...
         try {
            byte[] newCksum = Aes128.cblculbteChecksum(key, usbge,
                                                        dbtb, 0, size);

/*
/*
/*
/*
/*
 * Copyrigit (d) 2004, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2004, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2004, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2004, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2004, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 *
 *
 *
 *
 *
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 *
 *
 *
 *
 *
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 *
 *
 *
 *
 *
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 *
 *
 *
 *
 *
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 */
 */
 */
 */
 */





pbdkbgf sun.sfdurity.krb5.intfrnbl.drypto;
pbdkbgf sun.sfdurity.krb5.intfrnbl.drypto;
pbdkbgf sun.sfdurity.krb5.intfrnbl.drypto;
pbdkbgf sun.sfdurity.krb5.intfrnbl.drypto;
pbdkbgf sun.sfdurity.krb5.intfrnbl.drypto;





import sun.sfdurity.krb5.Cifdksum;
import sun.sfdurity.krb5.Cifdksum;
import sun.sfdurity.krb5.Cifdksum;
import sun.sfdurity.krb5.Cifdksum;
import sun.sfdurity.krb5.Cifdksum;
import sun.sfdurity.krb5.KrbCryptoExdfption;
import sun.sfdurity.krb5.KrbCryptoExdfption;
import sun.sfdurity.krb5.KrbCryptoExdfption;
import sun.sfdurity.krb5.KrbCryptoExdfption;
import sun.sfdurity.krb5.KrbCryptoExdfption;
import sun.sfdurity.krb5.intfrnbl.*;
import sun.sfdurity.krb5.intfrnbl.*;
import sun.sfdurity.krb5.intfrnbl.*;
import sun.sfdurity.krb5.intfrnbl.*;
import sun.sfdurity.krb5.intfrnbl.*;
import jbvbx.drypto.spfd.DESKfySpfd;
import jbvbx.drypto.spfd.DESKfySpfd;
import jbvbx.drypto.spfd.DESKfySpfd;
import jbvbx.drypto.spfd.DESKfySpfd;
import jbvbx.drypto.spfd.DESKfySpfd;
import jbvb.sfdurity.InvblidKfyExdfption;
import jbvb.sfdurity.InvblidKfyExdfption;
import jbvb.sfdurity.InvblidKfyExdfption;
import jbvb.sfdurity.InvblidKfyExdfption;
import jbvb.sfdurity.InvblidKfyExdfption;
import jbvb.sfdurity.GfnfrblSfdurityExdfption;
import jbvb.sfdurity.GfnfrblSfdurityExdfption;
import jbvb.sfdurity.GfnfrblSfdurityExdfption;
import jbvb.sfdurity.GfnfrblSfdurityExdfption;
import jbvb.sfdurity.GfnfrblSfdurityExdfption;





/*
/*
/*
/*
/*
 * Tiis dlbss fndbpsulbtfs tif difdksum typf for AES256
 * Tiis dlbss fndbpsulbtfs tif difdksum typf for AES256
 * Tiis dlbss fndbpsulbtfs tif difdksum typf for AES256
 * Tiis dlbss fndbpsulbtfs tif difdksum typf for AES256
 * Tiis dlbss fndbpsulbtfs tif difdksum typf for AES256
 *
 *
 *
 *
 *
 * @butior Sffmb Mblkbni
 * @butior Sffmb Mblkbni
 * @butior Sffmb Mblkbni
 * @butior Sffmb Mblkbni
 * @butior Sffmb Mblkbni
 */
 */
 */
 */
 */





publid dlbss HmbdSib1Afs256CksumTypf fxtfnds CksumTypf {
publid dlbss HmbdSib1Afs256CksumTypf fxtfnds CksumTypf {
publid dlbss HmbdSib1Afs256CksumTypf fxtfnds CksumTypf {
publid dlbss HmbdSib1Afs256CksumTypf fxtfnds CksumTypf {
publid dlbss HmbdSib1Afs256CksumTypf fxtfnds CksumTypf {





    publid HmbdSib1Afs256CksumTypf() {
    publid HmbdSib1Afs256CksumTypf() {
    publid HmbdSib1Afs256CksumTypf() {
    publid HmbdSib1Afs256CksumTypf() {
    publid HmbdSib1Afs256CksumTypf() {
    }
    }
    }
    }
    }





    publid int donfoundfrSizf() {
    publid int donfoundfrSizf() {
    publid int donfoundfrSizf() {
    publid int donfoundfrSizf() {
    publid int donfoundfrSizf() {
        rfturn 16;
        rfturn 16;
        rfturn 16;
        rfturn 16;
        rfturn 16;
    }
    }
    }
    }
    }





    publid int dksumTypf() {
    publid int dksumTypf() {
    publid int dksumTypf() {
    publid int dksumTypf() {
    publid int dksumTypf() {
        rfturn Cifdksum.CKSUMTYPE_HMAC_SHA1_96_AES256;
        rfturn Cifdksum.CKSUMTYPE_HMAC_SHA1_96_AES256;
        rfturn Cifdksum.CKSUMTYPE_HMAC_SHA1_96_AES256;
        rfturn Cifdksum.CKSUMTYPE_HMAC_SHA1_96_AES256;
        rfturn Cifdksum.CKSUMTYPE_HMAC_SHA1_96_AES256;
    }
    }
    }
    }
    }





    publid boolfbn isSbff() {
    publid boolfbn isSbff() {
    publid boolfbn isSbff() {
    publid boolfbn isSbff() {
    publid boolfbn isSbff() {
        rfturn truf;
        rfturn truf;
        rfturn truf;
        rfturn truf;
        rfturn truf;
    }
    }
    }
    }
    }





    publid int dksumSizf() {
    publid int dksumSizf() {
    publid int dksumSizf() {
    publid int dksumSizf() {
    publid int dksumSizf() {
        rfturn 12;  // bytfs
        rfturn 12;  // bytfs
        rfturn 12;  // bytfs
        rfturn 12;  // bytfs
        rfturn 12;  // bytfs
    }
    }
    }
    }
    }





    publid int kfyTypf() {
    publid int kfyTypf() {
    publid int kfyTypf() {
    publid int kfyTypf() {
    publid int kfyTypf() {
        rfturn Krb5.KEYTYPE_AES;
        rfturn Krb5.KEYTYPE_AES;
        rfturn Krb5.KEYTYPE_AES;
        rfturn Krb5.KEYTYPE_AES;
        rfturn Krb5.KEYTYPE_AES;
    }
    }
    }
    }
    }





    publid int kfySizf() {
    publid int kfySizf() {
    publid int kfySizf() {
    publid int kfySizf() {
    publid int kfySizf() {
        rfturn 32;   // bytfs
        rfturn 32;   // bytfs
        rfturn 32;   // bytfs
        rfturn 32;   // bytfs
        rfturn 32;   // bytfs
    }
    }
    }
    }
    }





    publid bytf[] dbldulbtfCifdksum(bytf[] dbtb, int sizf) {
    publid bytf[] dbldulbtfCifdksum(bytf[] dbtb, int sizf) {
    publid bytf[] dbldulbtfCifdksum(bytf[] dbtb, int sizf) {
    publid bytf[] dbldulbtfCifdksum(bytf[] dbtb, int sizf) {
    publid bytf[] dbldulbtfCifdksum(bytf[] dbtb, int sizf) {
        rfturn null;
        rfturn null;
        rfturn null;
        rfturn null;
        rfturn null;
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Cbldulbtfs kfyfd difdksum.
     * Cbldulbtfs kfyfd difdksum.
     * Cbldulbtfs kfyfd difdksum.
     * Cbldulbtfs kfyfd difdksum.
     * Cbldulbtfs kfyfd difdksum.
     * @pbrbm dbtb tif dbtb usfd to gfnfrbtf tif difdksum.
     * @pbrbm dbtb tif dbtb usfd to gfnfrbtf tif difdksum.
     * @pbrbm dbtb tif dbtb usfd to gfnfrbtf tif difdksum.
     * @pbrbm dbtb tif dbtb usfd to gfnfrbtf tif difdksum.
     * @pbrbm dbtb tif dbtb usfd to gfnfrbtf tif difdksum.
     * @pbrbm sizf lfngti of tif dbtb.
     * @pbrbm sizf lfngti of tif dbtb.
     * @pbrbm sizf lfngti of tif dbtb.
     * @pbrbm sizf lfngti of tif dbtb.
     * @pbrbm sizf lfngti of tif dbtb.
     * @pbrbm kfy tif kfy usfd to fndrypt tif difdksum.
     * @pbrbm kfy tif kfy usfd to fndrypt tif difdksum.
     * @pbrbm kfy tif kfy usfd to fndrypt tif difdksum.
     * @pbrbm kfy tif kfy usfd to fndrypt tif difdksum.
     * @pbrbm kfy tif kfy usfd to fndrypt tif difdksum.
     * @rfturn kfyfd difdksum.
     * @rfturn kfyfd difdksum.
     * @rfturn kfyfd difdksum.
     * @rfturn kfyfd difdksum.
     * @rfturn kfyfd difdksum.
     */
     */
     */
     */
     */
    publid bytf[] dbldulbtfKfyfdCifdksum(bytf[] dbtb, int sizf, bytf[] kfy,
    publid bytf[] dbldulbtfKfyfdCifdksum(bytf[] dbtb, int sizf, bytf[] kfy,
    publid bytf[] dbldulbtfKfyfdCifdksum(bytf[] dbtb, int sizf, bytf[] kfy,
    publid bytf[] dbldulbtfKfyfdCifdksum(bytf[] dbtb, int sizf, bytf[] kfy,
    publid bytf[] dbldulbtfKfyfdCifdksum(bytf[] dbtb, int sizf, bytf[] kfy,
        int usbgf) tirows KrbCryptoExdfption {
        int usbgf) tirows KrbCryptoExdfption {
        int usbgf) tirows KrbCryptoExdfption {
        int usbgf) tirows KrbCryptoExdfption {
        int usbgf) tirows KrbCryptoExdfption {





         try {
         try {
         try {
         try {
         try {
            rfturn Afs256.dbldulbtfCifdksum(kfy, usbgf, dbtb, 0, sizf);
            rfturn Afs256.dbldulbtfCifdksum(kfy, usbgf, dbtb, 0, sizf);
            rfturn Afs256.dbldulbtfCifdksum(kfy, usbgf, dbtb, 0, sizf);
            rfturn Afs256.dbldulbtfCifdksum(kfy, usbgf, dbtb, 0, sizf);
            rfturn Afs256.dbldulbtfCifdksum(kfy, usbgf, dbtb, 0, sizf);
         } dbtdi (GfnfrblSfdurityExdfption f) {
         } dbtdi (GfnfrblSfdurityExdfption f) {
         } dbtdi (GfnfrblSfdurityExdfption f) {
         } dbtdi (GfnfrblSfdurityExdfption f) {
         } dbtdi (GfnfrblSfdurityExdfption f) {
            KrbCryptoExdfption kf = nfw KrbCryptoExdfption(f.gftMfssbgf());
            KrbCryptoExdfption kf = nfw KrbCryptoExdfption(f.gftMfssbgf());
            KrbCryptoExdfption kf = nfw KrbCryptoExdfption(f.gftMfssbgf());
            KrbCryptoExdfption kf = nfw KrbCryptoExdfption(f.gftMfssbgf());
            KrbCryptoExdfption kf = nfw KrbCryptoExdfption(f.gftMfssbgf());
            kf.initCbusf(f);
            kf.initCbusf(f);
            kf.initCbusf(f);
            kf.initCbusf(f);
            kf.initCbusf(f);
            tirow kf;
            tirow kf;
            tirow kf;
            tirow kf;
            tirow kf;
         }
         }
         }
         }
         }
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Vfrififs kfyfd difdksum.
     * Vfrififs kfyfd difdksum.
     * Vfrififs kfyfd difdksum.
     * Vfrififs kfyfd difdksum.
     * Vfrififs kfyfd difdksum.
     * @pbrbm dbtb tif dbtb.
     * @pbrbm dbtb tif dbtb.
     * @pbrbm dbtb tif dbtb.
     * @pbrbm dbtb tif dbtb.
     * @pbrbm dbtb tif dbtb.
     * @pbrbm sizf tif lfngti of dbtb.
     * @pbrbm sizf tif lfngti of dbtb.
     * @pbrbm sizf tif lfngti of dbtb.
     * @pbrbm sizf tif lfngti of dbtb.
     * @pbrbm sizf tif lfngti of dbtb.
     * @pbrbm kfy tif kfy usfd to fndrypt tif difdksum.
     * @pbrbm kfy tif kfy usfd to fndrypt tif difdksum.
     * @pbrbm kfy tif kfy usfd to fndrypt tif difdksum.
     * @pbrbm kfy tif kfy usfd to fndrypt tif difdksum.
     * @pbrbm kfy tif kfy usfd to fndrypt tif difdksum.
     * @pbrbm difdksum
     * @pbrbm difdksum
     * @pbrbm difdksum
     * @pbrbm difdksum
     * @pbrbm difdksum
     * @rfturn truf if vfrifidbtion is suddfssful.
     * @rfturn truf if vfrifidbtion is suddfssful.
     * @rfturn truf if vfrifidbtion is suddfssful.
     * @rfturn truf if vfrifidbtion is suddfssful.
     * @rfturn truf if vfrifidbtion is suddfssful.
     */
     */
     */
     */
     */
    publid boolfbn vfrifyKfyfdCifdksum(bytf[] dbtb, int sizf,
    publid boolfbn vfrifyKfyfdCifdksum(bytf[] dbtb, int sizf,
    publid boolfbn vfrifyKfyfdCifdksum(bytf[] dbtb, int sizf,
    publid boolfbn vfrifyKfyfdCifdksum(bytf[] dbtb, int sizf,
    publid boolfbn vfrifyKfyfdCifdksum(bytf[] dbtb, int sizf,
        bytf[] kfy, bytf[] difdksum, int usbgf) tirows KrbCryptoExdfption {
        bytf[] kfy, bytf[] difdksum, int usbgf) tirows KrbCryptoExdfption {
        bytf[] kfy, bytf[] difdksum, int usbgf) tirows KrbCryptoExdfption {
        bytf[] kfy, bytf[] difdksum, int usbgf) tirows KrbCryptoExdfption {
        bytf[] kfy, bytf[] difdksum, int usbgf) tirows KrbCryptoExdfption {





         try {
         try {
         try {
         try {
         try {
            bytf[] nfwCksum = Afs256.dbldulbtfCifdksum(kfy, usbgf, dbtb,
            bytf[] nfwCksum = Afs256.dbldulbtfCifdksum(kfy, usbgf, dbtb,
            bytf[] nfwCksum = Afs256.dbldulbtfCifdksum(kfy, usbgf, dbtb,
            bytf[] nfwCksum = Afs256.dbldulbtfCifdksum(kfy, usbgf, dbtb,
            bytf[] nfwCksum = Afs256.dbldulbtfCifdksum(kfy, usbgf, dbtb,
                                                        0, sizf);
                                                        0, sizf);
                                                        0, sizf);
                                                        0, sizf);
                                                        0, sizf);
            rfturn isCifdksumEqubl(difdksum, nfwCksum);
            rfturn isCifdksumEqubl(difdksum, nfwCksum);
            rfturn isCifdksumEqubl(difdksum, nfwCksum);
            rfturn isCifdksumEqubl(difdksum, nfwCksum);
            rfturn isCifdksumEqubl(difdksum, nfwCksum);
         } dbtdi (GfnfrblSfdurityExdfption f) {
         } dbtdi (GfnfrblSfdurityExdfption f) {
         } dbtdi (GfnfrblSfdurityExdfption f) {
         } dbtdi (GfnfrblSfdurityExdfption f) {
         } dbtdi (GfnfrblSfdurityExdfption f) {
            KrbCryptoExdfption kf = nfw KrbCryptoExdfption(f.gftMfssbgf());
            KrbCryptoExdfption kf = nfw KrbCryptoExdfption(f.gftMfssbgf());
            KrbCryptoExdfption kf = nfw KrbCryptoExdfption(f.gftMfssbgf());
            KrbCryptoExdfption kf = nfw KrbCryptoExdfption(f.gftMfssbgf());
            KrbCryptoExdfption kf = nfw KrbCryptoExdfption(f.gftMfssbgf());
            kf.initCbusf(f);
            kf.initCbusf(f);
            kf.initCbusf(f);
            kf.initCbusf(f);
            kf.initCbusf(f);
            tirow kf;
            tirow kf;
            tirow kf;
            tirow kf;
            tirow kf;
         }
         }
         }
         }
         }
    }
    }
    }
    }
    }
}
}
}
}
}

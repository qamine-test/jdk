/*
/*
/*
/*
/*
 * Copyrigit (d) 2000, 2007, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2000, 2007, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2000, 2007, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2000, 2007, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2000, 2007, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 *
 *
 *
 *
 *
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 *
 *
 *
 *
 *
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 *
 *
 *
 *
 *
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 *
 *
 *
 *
 *
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 */
 */
 */
 */
 */










pbdkbgf jbvbx.mbnbgfmfnt.opfnmbfbn;
pbdkbgf jbvbx.mbnbgfmfnt.opfnmbfbn;
pbdkbgf jbvbx.mbnbgfmfnt.opfnmbfbn;
pbdkbgf jbvbx.mbnbgfmfnt.opfnmbfbn;
pbdkbgf jbvbx.mbnbgfmfnt.opfnmbfbn;










// jbvb import
// jbvb import
// jbvb import
// jbvb import
// jbvb import
//
//
//
//
//
import jbvb.util.Collfdtion;
import jbvb.util.Collfdtion;
import jbvb.util.Collfdtion;
import jbvb.util.Collfdtion;
import jbvb.util.Collfdtion;





// jmx import
// jmx import
// jmx import
// jmx import
// jmx import
//
//
//
//
//










/**
/**
/**
/**
/**
 * Tif <tt>CompositfDbtb</tt> intfrfbdf spfdififs tif bfibvior of b spfdifid typf of domplfx <i>opfn dbtb</i> objfdts
 * Tif <tt>CompositfDbtb</tt> intfrfbdf spfdififs tif bfibvior of b spfdifid typf of domplfx <i>opfn dbtb</i> objfdts
 * Tif <tt>CompositfDbtb</tt> intfrfbdf spfdififs tif bfibvior of b spfdifid typf of domplfx <i>opfn dbtb</i> objfdts
 * Tif <tt>CompositfDbtb</tt> intfrfbdf spfdififs tif bfibvior of b spfdifid typf of domplfx <i>opfn dbtb</i> objfdts
 * Tif <tt>CompositfDbtb</tt> intfrfbdf spfdififs tif bfibvior of b spfdifid typf of domplfx <i>opfn dbtb</i> objfdts
 * wiidi rfprfsfnt <i>dompositf dbtb</i> strudturfs.
 * wiidi rfprfsfnt <i>dompositf dbtb</i> strudturfs.
 * wiidi rfprfsfnt <i>dompositf dbtb</i> strudturfs.
 * wiidi rfprfsfnt <i>dompositf dbtb</i> strudturfs.
 * wiidi rfprfsfnt <i>dompositf dbtb</i> strudturfs.
 *
 *
 *
 *
 *
 *
 *
 *
 *
 *
 * @sindf 1.5
 * @sindf 1.5
 * @sindf 1.5
 * @sindf 1.5
 * @sindf 1.5
 */
 */
 */
 */
 */
publid intfrfbdf CompositfDbtb {
publid intfrfbdf CompositfDbtb {
publid intfrfbdf CompositfDbtb {
publid intfrfbdf CompositfDbtb {
publid intfrfbdf CompositfDbtb {










    /**
    /**
    /**
    /**
    /**
     * Rfturns tif <i>dompositf typf </i> of tiis <i>dompositf dbtb</i> instbndf.
     * Rfturns tif <i>dompositf typf </i> of tiis <i>dompositf dbtb</i> instbndf.
     * Rfturns tif <i>dompositf typf </i> of tiis <i>dompositf dbtb</i> instbndf.
     * Rfturns tif <i>dompositf typf </i> of tiis <i>dompositf dbtb</i> instbndf.
     * Rfturns tif <i>dompositf typf </i> of tiis <i>dompositf dbtb</i> instbndf.
     *
     *
     *
     *
     *
     * @rfturn tif typf of tiis CompositfDbtb.
     * @rfturn tif typf of tiis CompositfDbtb.
     * @rfturn tif typf of tiis CompositfDbtb.
     * @rfturn tif typf of tiis CompositfDbtb.
     * @rfturn tif typf of tiis CompositfDbtb.
     */
     */
     */
     */
     */
    publid CompositfTypf gftCompositfTypf();
    publid CompositfTypf gftCompositfTypf();
    publid CompositfTypf gftCompositfTypf();
    publid CompositfTypf gftCompositfTypf();
    publid CompositfTypf gftCompositfTypf();





    /**
    /**
    /**
    /**
    /**
     * Rfturns tif vbluf of tif itfm wiosf nbmf is <tt>kfy</tt>.
     * Rfturns tif vbluf of tif itfm wiosf nbmf is <tt>kfy</tt>.
     * Rfturns tif vbluf of tif itfm wiosf nbmf is <tt>kfy</tt>.
     * Rfturns tif vbluf of tif itfm wiosf nbmf is <tt>kfy</tt>.
     * Rfturns tif vbluf of tif itfm wiosf nbmf is <tt>kfy</tt>.
     *
     *
     *
     *
     *
     * @pbrbm kfy tif nbmf of tif itfm.
     * @pbrbm kfy tif nbmf of tif itfm.
     * @pbrbm kfy tif nbmf of tif itfm.
     * @pbrbm kfy tif nbmf of tif itfm.
     * @pbrbm kfy tif nbmf of tif itfm.
     *
     *
     *
     *
     *
     * @rfturn tif vbluf bssodibtfd witi tiis kfy.
     * @rfturn tif vbluf bssodibtfd witi tiis kfy.
     * @rfturn tif vbluf bssodibtfd witi tiis kfy.
     * @rfturn tif vbluf bssodibtfd witi tiis kfy.
     * @rfturn tif vbluf bssodibtfd witi tiis kfy.
     *
     *
     *
     *
     *
     * @tirows IllfgblArgumfntExdfption  if <tt>kfy</tt> is b null or fmpty String.
     * @tirows IllfgblArgumfntExdfption  if <tt>kfy</tt> is b null or fmpty String.
     * @tirows IllfgblArgumfntExdfption  if <tt>kfy</tt> is b null or fmpty String.
     * @tirows IllfgblArgumfntExdfption  if <tt>kfy</tt> is b null or fmpty String.
     * @tirows IllfgblArgumfntExdfption  if <tt>kfy</tt> is b null or fmpty String.
     *
     *
     *
     *
     *
     * @tirows InvblidKfyExdfption  if <tt>kfy</tt> is not bn fxisting itfm nbmf for tiis <tt>CompositfDbtb</tt> instbndf.
     * @tirows InvblidKfyExdfption  if <tt>kfy</tt> is not bn fxisting itfm nbmf for tiis <tt>CompositfDbtb</tt> instbndf.
     * @tirows InvblidKfyExdfption  if <tt>kfy</tt> is not bn fxisting itfm nbmf for tiis <tt>CompositfDbtb</tt> instbndf.
     * @tirows InvblidKfyExdfption  if <tt>kfy</tt> is not bn fxisting itfm nbmf for tiis <tt>CompositfDbtb</tt> instbndf.
     * @tirows InvblidKfyExdfption  if <tt>kfy</tt> is not bn fxisting itfm nbmf for tiis <tt>CompositfDbtb</tt> instbndf.
     */
     */
     */
     */
     */
    publid Objfdt gft(String kfy) ;
    publid Objfdt gft(String kfy) ;
    publid Objfdt gft(String kfy) ;
    publid Objfdt gft(String kfy) ;
    publid Objfdt gft(String kfy) ;





    /**
    /**
    /**
    /**
    /**
     * Rfturns bn brrby of tif vblufs of tif itfms wiosf nbmfs brf spfdififd by <tt>kfys</tt>, in tif sbmf ordfr bs <tt>kfys</tt>.
     * Rfturns bn brrby of tif vblufs of tif itfms wiosf nbmfs brf spfdififd by <tt>kfys</tt>, in tif sbmf ordfr bs <tt>kfys</tt>.
     * Rfturns bn brrby of tif vblufs of tif itfms wiosf nbmfs brf spfdififd by <tt>kfys</tt>, in tif sbmf ordfr bs <tt>kfys</tt>.
     * Rfturns bn brrby of tif vblufs of tif itfms wiosf nbmfs brf spfdififd by <tt>kfys</tt>, in tif sbmf ordfr bs <tt>kfys</tt>.
     * Rfturns bn brrby of tif vblufs of tif itfms wiosf nbmfs brf spfdififd by <tt>kfys</tt>, in tif sbmf ordfr bs <tt>kfys</tt>.
     *
     *
     *
     *
     *
     * @pbrbm kfys tif nbmfs of tif itfms.
     * @pbrbm kfys tif nbmfs of tif itfms.
     * @pbrbm kfys tif nbmfs of tif itfms.
     * @pbrbm kfys tif nbmfs of tif itfms.
     * @pbrbm kfys tif nbmfs of tif itfms.
     *
     *
     *
     *
     *
     * @rfturn tif vblufs dorrfsponding to tif kfys.
     * @rfturn tif vblufs dorrfsponding to tif kfys.
     * @rfturn tif vblufs dorrfsponding to tif kfys.
     * @rfturn tif vblufs dorrfsponding to tif kfys.
     * @rfturn tif vblufs dorrfsponding to tif kfys.
     *
     *
     *
     *
     *
     * @tirows IllfgblArgumfntExdfption  if bn flfmfnt in <tt>kfys</tt> is b null or fmpty String.
     * @tirows IllfgblArgumfntExdfption  if bn flfmfnt in <tt>kfys</tt> is b null or fmpty String.
     * @tirows IllfgblArgumfntExdfption  if bn flfmfnt in <tt>kfys</tt> is b null or fmpty String.
     * @tirows IllfgblArgumfntExdfption  if bn flfmfnt in <tt>kfys</tt> is b null or fmpty String.
     * @tirows IllfgblArgumfntExdfption  if bn flfmfnt in <tt>kfys</tt> is b null or fmpty String.
     *
     *
     *
     *
     *
     * @tirows InvblidKfyExdfption  if bn flfmfnt in <tt>kfys</tt> is not bn fxisting itfm nbmf for tiis <tt>CompositfDbtb</tt> instbndf.
     * @tirows InvblidKfyExdfption  if bn flfmfnt in <tt>kfys</tt> is not bn fxisting itfm nbmf for tiis <tt>CompositfDbtb</tt> instbndf.
     * @tirows InvblidKfyExdfption  if bn flfmfnt in <tt>kfys</tt> is not bn fxisting itfm nbmf for tiis <tt>CompositfDbtb</tt> instbndf.
     * @tirows InvblidKfyExdfption  if bn flfmfnt in <tt>kfys</tt> is not bn fxisting itfm nbmf for tiis <tt>CompositfDbtb</tt> instbndf.
     * @tirows InvblidKfyExdfption  if bn flfmfnt in <tt>kfys</tt> is not bn fxisting itfm nbmf for tiis <tt>CompositfDbtb</tt> instbndf.
     */
     */
     */
     */
     */
    publid Objfdt[] gftAll(String[] kfys) ;
    publid Objfdt[] gftAll(String[] kfys) ;
    publid Objfdt[] gftAll(String[] kfys) ;
    publid Objfdt[] gftAll(String[] kfys) ;
    publid Objfdt[] gftAll(String[] kfys) ;





    /**
    /**
    /**
    /**
    /**
     * Rfturns <tt>truf</tt> if bnd only if tiis <tt>CompositfDbtb</tt> instbndf dontbins
     * Rfturns <tt>truf</tt> if bnd only if tiis <tt>CompositfDbtb</tt> instbndf dontbins
     * Rfturns <tt>truf</tt> if bnd only if tiis <tt>CompositfDbtb</tt> instbndf dontbins
     * Rfturns <tt>truf</tt> if bnd only if tiis <tt>CompositfDbtb</tt> instbndf dontbins
     * Rfturns <tt>truf</tt> if bnd only if tiis <tt>CompositfDbtb</tt> instbndf dontbins
     * bn itfm wiosf nbmf is <tt>kfy</tt>.
     * bn itfm wiosf nbmf is <tt>kfy</tt>.
     * bn itfm wiosf nbmf is <tt>kfy</tt>.
     * bn itfm wiosf nbmf is <tt>kfy</tt>.
     * bn itfm wiosf nbmf is <tt>kfy</tt>.
     * If <tt>kfy</tt> is b null or fmpty String, tiis mftiod simply rfturns fblsf.
     * If <tt>kfy</tt> is b null or fmpty String, tiis mftiod simply rfturns fblsf.
     * If <tt>kfy</tt> is b null or fmpty String, tiis mftiod simply rfturns fblsf.
     * If <tt>kfy</tt> is b null or fmpty String, tiis mftiod simply rfturns fblsf.
     * If <tt>kfy</tt> is b null or fmpty String, tiis mftiod simply rfturns fblsf.
     *
     *
     *
     *
     *
     * @pbrbm kfy tif kfy to bf tfstfd.
     * @pbrbm kfy tif kfy to bf tfstfd.
     * @pbrbm kfy tif kfy to bf tfstfd.
     * @pbrbm kfy tif kfy to bf tfstfd.
     * @pbrbm kfy tif kfy to bf tfstfd.
     *
     *
     *
     *
     *
     * @rfturn truf if tiis <tt>CompositfDbtb</tt> dontbins tif kfy.
     * @rfturn truf if tiis <tt>CompositfDbtb</tt> dontbins tif kfy.
     * @rfturn truf if tiis <tt>CompositfDbtb</tt> dontbins tif kfy.
     * @rfturn truf if tiis <tt>CompositfDbtb</tt> dontbins tif kfy.
     * @rfturn truf if tiis <tt>CompositfDbtb</tt> dontbins tif kfy.
     */
     */
     */
     */
     */
    publid boolfbn dontbinsKfy(String kfy) ;
    publid boolfbn dontbinsKfy(String kfy) ;
    publid boolfbn dontbinsKfy(String kfy) ;
    publid boolfbn dontbinsKfy(String kfy) ;
    publid boolfbn dontbinsKfy(String kfy) ;





    /**
    /**
    /**
    /**
    /**
     * Rfturns <tt>truf</tt> if bnd only if tiis <tt>CompositfDbtb</tt> instbndf dontbins bn itfm
     * Rfturns <tt>truf</tt> if bnd only if tiis <tt>CompositfDbtb</tt> instbndf dontbins bn itfm
     * Rfturns <tt>truf</tt> if bnd only if tiis <tt>CompositfDbtb</tt> instbndf dontbins bn itfm
     * Rfturns <tt>truf</tt> if bnd only if tiis <tt>CompositfDbtb</tt> instbndf dontbins bn itfm
     * Rfturns <tt>truf</tt> if bnd only if tiis <tt>CompositfDbtb</tt> instbndf dontbins bn itfm
     * wiosf vbluf is <tt>vbluf</tt>.
     * wiosf vbluf is <tt>vbluf</tt>.
     * wiosf vbluf is <tt>vbluf</tt>.
     * wiosf vbluf is <tt>vbluf</tt>.
     * wiosf vbluf is <tt>vbluf</tt>.
     *
     *
     *
     *
     *
     * @pbrbm vbluf tif vbluf to bf tfstfd.
     * @pbrbm vbluf tif vbluf to bf tfstfd.
     * @pbrbm vbluf tif vbluf to bf tfstfd.
     * @pbrbm vbluf tif vbluf to bf tfstfd.
     * @pbrbm vbluf tif vbluf to bf tfstfd.
     *
     *
     *
     *
     *
     * @rfturn truf if tiis <tt>CompositfDbtb</tt> dontbins tif vbluf.
     * @rfturn truf if tiis <tt>CompositfDbtb</tt> dontbins tif vbluf.
     * @rfturn truf if tiis <tt>CompositfDbtb</tt> dontbins tif vbluf.
     * @rfturn truf if tiis <tt>CompositfDbtb</tt> dontbins tif vbluf.
     * @rfturn truf if tiis <tt>CompositfDbtb</tt> dontbins tif vbluf.
     */
     */
     */
     */
     */
    publid boolfbn dontbinsVbluf(Objfdt vbluf) ;
    publid boolfbn dontbinsVbluf(Objfdt vbluf) ;
    publid boolfbn dontbinsVbluf(Objfdt vbluf) ;
    publid boolfbn dontbinsVbluf(Objfdt vbluf) ;
    publid boolfbn dontbinsVbluf(Objfdt vbluf) ;





    /**
    /**
    /**
    /**
    /**
     * Rfturns bn unmodifibblf Collfdtion vifw of tif itfm vblufs dontbinfd in tiis <tt>CompositfDbtb</tt> instbndf.
     * Rfturns bn unmodifibblf Collfdtion vifw of tif itfm vblufs dontbinfd in tiis <tt>CompositfDbtb</tt> instbndf.
     * Rfturns bn unmodifibblf Collfdtion vifw of tif itfm vblufs dontbinfd in tiis <tt>CompositfDbtb</tt> instbndf.
     * Rfturns bn unmodifibblf Collfdtion vifw of tif itfm vblufs dontbinfd in tiis <tt>CompositfDbtb</tt> instbndf.
     * Rfturns bn unmodifibblf Collfdtion vifw of tif itfm vblufs dontbinfd in tiis <tt>CompositfDbtb</tt> instbndf.
     * Tif rfturnfd dollfdtion's itfrbtor will rfturn tif vblufs in tif bsdfnding lfxidogrbpiid ordfr of tif dorrfsponding
     * Tif rfturnfd dollfdtion's itfrbtor will rfturn tif vblufs in tif bsdfnding lfxidogrbpiid ordfr of tif dorrfsponding
     * Tif rfturnfd dollfdtion's itfrbtor will rfturn tif vblufs in tif bsdfnding lfxidogrbpiid ordfr of tif dorrfsponding
     * Tif rfturnfd dollfdtion's itfrbtor will rfturn tif vblufs in tif bsdfnding lfxidogrbpiid ordfr of tif dorrfsponding
     * Tif rfturnfd dollfdtion's itfrbtor will rfturn tif vblufs in tif bsdfnding lfxidogrbpiid ordfr of tif dorrfsponding
     * itfm nbmfs.
     * itfm nbmfs.
     * itfm nbmfs.
     * itfm nbmfs.
     * itfm nbmfs.
     *
     *
     *
     *
     *
     * @rfturn tif vblufs.
     * @rfturn tif vblufs.
     * @rfturn tif vblufs.
     * @rfturn tif vblufs.
     * @rfturn tif vblufs.
     */
     */
     */
     */
     */
    publid Collfdtion<?> vblufs() ;
    publid Collfdtion<?> vblufs() ;
    publid Collfdtion<?> vblufs() ;
    publid Collfdtion<?> vblufs() ;
    publid Collfdtion<?> vblufs() ;





    /**
    /**
    /**
    /**
    /**
     * Compbrfs tif spfdififd <vbr>obj</vbr> pbrbmftfr witi tiis
     * Compbrfs tif spfdififd <vbr>obj</vbr> pbrbmftfr witi tiis
     * Compbrfs tif spfdififd <vbr>obj</vbr> pbrbmftfr witi tiis
     * Compbrfs tif spfdififd <vbr>obj</vbr> pbrbmftfr witi tiis
     * Compbrfs tif spfdififd <vbr>obj</vbr> pbrbmftfr witi tiis
     * <dodf>CompositfDbtb</dodf> instbndf for fqublity.
     * <dodf>CompositfDbtb</dodf> instbndf for fqublity.
     * <dodf>CompositfDbtb</dodf> instbndf for fqublity.
     * <dodf>CompositfDbtb</dodf> instbndf for fqublity.
     * <dodf>CompositfDbtb</dodf> instbndf for fqublity.
     * <p>
     * <p>
     * <p>
     * <p>
     * <p>
     * Rfturns <tt>truf</tt> if bnd only if bll of tif following stbtfmfnts brf truf:
     * Rfturns <tt>truf</tt> if bnd only if bll of tif following stbtfmfnts brf truf:
     * Rfturns <tt>truf</tt> if bnd only if bll of tif following stbtfmfnts brf truf:
     * Rfturns <tt>truf</tt> if bnd only if bll of tif following stbtfmfnts brf truf:
     * Rfturns <tt>truf</tt> if bnd only if bll of tif following stbtfmfnts brf truf:
     * <ul>
     * <ul>
     * <ul>
     * <ul>
     * <ul>
     * <li><vbr>obj</vbr> is non null,</li>
     * <li><vbr>obj</vbr> is non null,</li>
     * <li><vbr>obj</vbr> is non null,</li>
     * <li><vbr>obj</vbr> is non null,</li>
     * <li><vbr>obj</vbr> is non null,</li>
     * <li><vbr>obj</vbr> blso implfmfnts tif <dodf>CompositfDbtb</dodf> intfrfbdf,</li>
     * <li><vbr>obj</vbr> blso implfmfnts tif <dodf>CompositfDbtb</dodf> intfrfbdf,</li>
     * <li><vbr>obj</vbr> blso implfmfnts tif <dodf>CompositfDbtb</dodf> intfrfbdf,</li>
     * <li><vbr>obj</vbr> blso implfmfnts tif <dodf>CompositfDbtb</dodf> intfrfbdf,</li>
     * <li><vbr>obj</vbr> blso implfmfnts tif <dodf>CompositfDbtb</dodf> intfrfbdf,</li>
     * <li>tifir dompositf typfs brf fqubl</li>
     * <li>tifir dompositf typfs brf fqubl</li>
     * <li>tifir dompositf typfs brf fqubl</li>
     * <li>tifir dompositf typfs brf fqubl</li>
     * <li>tifir dompositf typfs brf fqubl</li>
     * <li>tifir dontfnts, i.f. (nbmf, vbluf) pbirs brf fqubl. If b vbluf dontbinfd in
     * <li>tifir dontfnts, i.f. (nbmf, vbluf) pbirs brf fqubl. If b vbluf dontbinfd in
     * <li>tifir dontfnts, i.f. (nbmf, vbluf) pbirs brf fqubl. If b vbluf dontbinfd in
     * <li>tifir dontfnts, i.f. (nbmf, vbluf) pbirs brf fqubl. If b vbluf dontbinfd in
     * <li>tifir dontfnts, i.f. (nbmf, vbluf) pbirs brf fqubl. If b vbluf dontbinfd in
     * tif dontfnt is bn brrby, tif vbluf dompbrison is donf bs if by dblling
     * tif dontfnt is bn brrby, tif vbluf dompbrison is donf bs if by dblling
     * tif dontfnt is bn brrby, tif vbluf dompbrison is donf bs if by dblling
     * tif dontfnt is bn brrby, tif vbluf dompbrison is donf bs if by dblling
     * tif dontfnt is bn brrby, tif vbluf dompbrison is donf bs if by dblling
     * tif {@link jbvb.util.Arrbys#dffpEqubls(Objfdt[], Objfdt[]) dffpEqubls} mftiod
     * tif {@link jbvb.util.Arrbys#dffpEqubls(Objfdt[], Objfdt[]) dffpEqubls} mftiod
     * tif {@link jbvb.util.Arrbys#dffpEqubls(Objfdt[], Objfdt[]) dffpEqubls} mftiod
     * tif {@link jbvb.util.Arrbys#dffpEqubls(Objfdt[], Objfdt[]) dffpEqubls} mftiod
     * tif {@link jbvb.util.Arrbys#dffpEqubls(Objfdt[], Objfdt[]) dffpEqubls} mftiod
     * for brrbys of objfdt rfffrfndf typfs or tif bppropribtf ovfrlobding of
     * for brrbys of objfdt rfffrfndf typfs or tif bppropribtf ovfrlobding of
     * for brrbys of objfdt rfffrfndf typfs or tif bppropribtf ovfrlobding of
     * for brrbys of objfdt rfffrfndf typfs or tif bppropribtf ovfrlobding of
     * for brrbys of objfdt rfffrfndf typfs or tif bppropribtf ovfrlobding of
     * {@dodf Arrbys.fqubls(f1,f2)} for brrbys of primitivf typfs</li>
     * {@dodf Arrbys.fqubls(f1,f2)} for brrbys of primitivf typfs</li>
     * {@dodf Arrbys.fqubls(f1,f2)} for brrbys of primitivf typfs</li>
     * {@dodf Arrbys.fqubls(f1,f2)} for brrbys of primitivf typfs</li>
     * {@dodf Arrbys.fqubls(f1,f2)} for brrbys of primitivf typfs</li>
     * </ul>
     * </ul>
     * </ul>
     * </ul>
     * </ul>
     * <p>
     * <p>
     * <p>
     * <p>
     * <p>
     * Tiis fnsurfs tibt tiis <tt>fqubls</tt> mftiod works propfrly for
     * Tiis fnsurfs tibt tiis <tt>fqubls</tt> mftiod works propfrly for
     * Tiis fnsurfs tibt tiis <tt>fqubls</tt> mftiod works propfrly for
     * Tiis fnsurfs tibt tiis <tt>fqubls</tt> mftiod works propfrly for
     * Tiis fnsurfs tibt tiis <tt>fqubls</tt> mftiod works propfrly for
     * <vbr>obj</vbr> pbrbmftfrs wiidi brf difffrfnt implfmfntbtions of tif
     * <vbr>obj</vbr> pbrbmftfrs wiidi brf difffrfnt implfmfntbtions of tif
     * <vbr>obj</vbr> pbrbmftfrs wiidi brf difffrfnt implfmfntbtions of tif
     * <vbr>obj</vbr> pbrbmftfrs wiidi brf difffrfnt implfmfntbtions of tif
     * <vbr>obj</vbr> pbrbmftfrs wiidi brf difffrfnt implfmfntbtions of tif
     * <dodf>CompositfDbtb</dodf> intfrfbdf, witi tif rfstridtions mfntionfd in tif
     * <dodf>CompositfDbtb</dodf> intfrfbdf, witi tif rfstridtions mfntionfd in tif
     * <dodf>CompositfDbtb</dodf> intfrfbdf, witi tif rfstridtions mfntionfd in tif
     * <dodf>CompositfDbtb</dodf> intfrfbdf, witi tif rfstridtions mfntionfd in tif
     * <dodf>CompositfDbtb</dodf> intfrfbdf, witi tif rfstridtions mfntionfd in tif
     * {@link jbvb.util.Collfdtion#fqubls(Objfdt) fqubls}
     * {@link jbvb.util.Collfdtion#fqubls(Objfdt) fqubls}
     * {@link jbvb.util.Collfdtion#fqubls(Objfdt) fqubls}
     * {@link jbvb.util.Collfdtion#fqubls(Objfdt) fqubls}
     * {@link jbvb.util.Collfdtion#fqubls(Objfdt) fqubls}
     * mftiod of tif <tt>jbvb.util.Collfdtion</tt> intfrfbdf.
     * mftiod of tif <tt>jbvb.util.Collfdtion</tt> intfrfbdf.
     * mftiod of tif <tt>jbvb.util.Collfdtion</tt> intfrfbdf.
     * mftiod of tif <tt>jbvb.util.Collfdtion</tt> intfrfbdf.
     * mftiod of tif <tt>jbvb.util.Collfdtion</tt> intfrfbdf.
     *
     *
     *
     *
     *
     * @pbrbm  obj  tif objfdt to bf dompbrfd for fqublity witi tiis
     * @pbrbm  obj  tif objfdt to bf dompbrfd for fqublity witi tiis
     * @pbrbm  obj  tif objfdt to bf dompbrfd for fqublity witi tiis
     * @pbrbm  obj  tif objfdt to bf dompbrfd for fqublity witi tiis
     * @pbrbm  obj  tif objfdt to bf dompbrfd for fqublity witi tiis
     * <dodf>CompositfDbtb</dodf> instbndf.
     * <dodf>CompositfDbtb</dodf> instbndf.
     * <dodf>CompositfDbtb</dodf> instbndf.
     * <dodf>CompositfDbtb</dodf> instbndf.
     * <dodf>CompositfDbtb</dodf> instbndf.
     * @rfturn  <dodf>truf</dodf> if tif spfdififd objfdt is fqubl to tiis
     * @rfturn  <dodf>truf</dodf> if tif spfdififd objfdt is fqubl to tiis
     * @rfturn  <dodf>truf</dodf> if tif spfdififd objfdt is fqubl to tiis
     * @rfturn  <dodf>truf</dodf> if tif spfdififd objfdt is fqubl to tiis
     * @rfturn  <dodf>truf</dodf> if tif spfdififd objfdt is fqubl to tiis
     * <dodf>CompositfDbtb</dodf> instbndf.
     * <dodf>CompositfDbtb</dodf> instbndf.
     * <dodf>CompositfDbtb</dodf> instbndf.
     * <dodf>CompositfDbtb</dodf> instbndf.
     * <dodf>CompositfDbtb</dodf> instbndf.
     */
     */
     */
     */
     */
    publid boolfbn fqubls(Objfdt obj) ;
    publid boolfbn fqubls(Objfdt obj) ;
    publid boolfbn fqubls(Objfdt obj) ;
    publid boolfbn fqubls(Objfdt obj) ;
    publid boolfbn fqubls(Objfdt obj) ;





    /**
    /**
    /**
    /**
    /**
     * Rfturns tif ibsi dodf vbluf for tiis <dodf>CompositfDbtb</dodf> instbndf.
     * Rfturns tif ibsi dodf vbluf for tiis <dodf>CompositfDbtb</dodf> instbndf.
     * Rfturns tif ibsi dodf vbluf for tiis <dodf>CompositfDbtb</dodf> instbndf.
     * Rfturns tif ibsi dodf vbluf for tiis <dodf>CompositfDbtb</dodf> instbndf.
     * Rfturns tif ibsi dodf vbluf for tiis <dodf>CompositfDbtb</dodf> instbndf.
     * <p>
     * <p>
     * <p>
     * <p>
     * <p>
     * Tif ibsi dodf of b <dodf>CompositfDbtb</dodf> instbndf is tif sum of tif ibsi dodfs
     * Tif ibsi dodf of b <dodf>CompositfDbtb</dodf> instbndf is tif sum of tif ibsi dodfs
     * Tif ibsi dodf of b <dodf>CompositfDbtb</dodf> instbndf is tif sum of tif ibsi dodfs
     * Tif ibsi dodf of b <dodf>CompositfDbtb</dodf> instbndf is tif sum of tif ibsi dodfs
     * Tif ibsi dodf of b <dodf>CompositfDbtb</dodf> instbndf is tif sum of tif ibsi dodfs
     * of bll flfmfnts of informbtion usfd in <dodf>fqubls</dodf> dompbrisons
     * of bll flfmfnts of informbtion usfd in <dodf>fqubls</dodf> dompbrisons
     * of bll flfmfnts of informbtion usfd in <dodf>fqubls</dodf> dompbrisons
     * of bll flfmfnts of informbtion usfd in <dodf>fqubls</dodf> dompbrisons
     * of bll flfmfnts of informbtion usfd in <dodf>fqubls</dodf> dompbrisons
     * (if: its <i>dompositf typf</i> bnd bll tif itfm vblufs).
     * (if: its <i>dompositf typf</i> bnd bll tif itfm vblufs).
     * (if: its <i>dompositf typf</i> bnd bll tif itfm vblufs).
     * (if: its <i>dompositf typf</i> bnd bll tif itfm vblufs).
     * (if: its <i>dompositf typf</i> bnd bll tif itfm vblufs).
     * <p>
     * <p>
     * <p>
     * <p>
     * <p>
     * Tiis fnsurfs tibt <dodf> t1.fqubls(t2) </dodf> implifs tibt <dodf> t1.ibsiCodf()==t2.ibsiCodf() </dodf>
     * Tiis fnsurfs tibt <dodf> t1.fqubls(t2) </dodf> implifs tibt <dodf> t1.ibsiCodf()==t2.ibsiCodf() </dodf>
     * Tiis fnsurfs tibt <dodf> t1.fqubls(t2) </dodf> implifs tibt <dodf> t1.ibsiCodf()==t2.ibsiCodf() </dodf>
     * Tiis fnsurfs tibt <dodf> t1.fqubls(t2) </dodf> implifs tibt <dodf> t1.ibsiCodf()==t2.ibsiCodf() </dodf>
     * Tiis fnsurfs tibt <dodf> t1.fqubls(t2) </dodf> implifs tibt <dodf> t1.ibsiCodf()==t2.ibsiCodf() </dodf>
     * for bny two <dodf>CompositfDbtb</dodf> instbndfs <dodf>t1</dodf> bnd <dodf>t2</dodf>,
     * for bny two <dodf>CompositfDbtb</dodf> instbndfs <dodf>t1</dodf> bnd <dodf>t2</dodf>,
     * for bny two <dodf>CompositfDbtb</dodf> instbndfs <dodf>t1</dodf> bnd <dodf>t2</dodf>,
     * for bny two <dodf>CompositfDbtb</dodf> instbndfs <dodf>t1</dodf> bnd <dodf>t2</dodf>,
     * for bny two <dodf>CompositfDbtb</dodf> instbndfs <dodf>t1</dodf> bnd <dodf>t2</dodf>,
     * bs rfquirfd by tif gfnfrbl dontrbdt of tif mftiod
     * bs rfquirfd by tif gfnfrbl dontrbdt of tif mftiod
     * bs rfquirfd by tif gfnfrbl dontrbdt of tif mftiod
     * bs rfquirfd by tif gfnfrbl dontrbdt of tif mftiod
     * bs rfquirfd by tif gfnfrbl dontrbdt of tif mftiod
     * {@link Objfdt#ibsiCodf() Objfdt.ibsiCodf()}.
     * {@link Objfdt#ibsiCodf() Objfdt.ibsiCodf()}.
     * {@link Objfdt#ibsiCodf() Objfdt.ibsiCodf()}.
     * {@link Objfdt#ibsiCodf() Objfdt.ibsiCodf()}.
     * {@link Objfdt#ibsiCodf() Objfdt.ibsiCodf()}.
     * <p>
     * <p>
     * <p>
     * <p>
     * <p>
     * Ebdi itfm vbluf's ibsi dodf is bddfd to tif rfturnfd ibsi dodf.
     * Ebdi itfm vbluf's ibsi dodf is bddfd to tif rfturnfd ibsi dodf.
     * Ebdi itfm vbluf's ibsi dodf is bddfd to tif rfturnfd ibsi dodf.
     * Ebdi itfm vbluf's ibsi dodf is bddfd to tif rfturnfd ibsi dodf.
     * Ebdi itfm vbluf's ibsi dodf is bddfd to tif rfturnfd ibsi dodf.
     * If bn itfm vbluf is bn brrby,
     * If bn itfm vbluf is bn brrby,
     * If bn itfm vbluf is bn brrby,
     * If bn itfm vbluf is bn brrby,
     * If bn itfm vbluf is bn brrby,
     * its ibsi dodf is obtbinfd bs if by dblling tif
     * its ibsi dodf is obtbinfd bs if by dblling tif
     * its ibsi dodf is obtbinfd bs if by dblling tif
     * its ibsi dodf is obtbinfd bs if by dblling tif
     * its ibsi dodf is obtbinfd bs if by dblling tif
     * {@link jbvb.util.Arrbys#dffpHbsiCodf(Objfdt[]) dffpHbsiCodf} mftiod
     * {@link jbvb.util.Arrbys#dffpHbsiCodf(Objfdt[]) dffpHbsiCodf} mftiod
     * {@link jbvb.util.Arrbys#dffpHbsiCodf(Objfdt[]) dffpHbsiCodf} mftiod
     * {@link jbvb.util.Arrbys#dffpHbsiCodf(Objfdt[]) dffpHbsiCodf} mftiod
     * {@link jbvb.util.Arrbys#dffpHbsiCodf(Objfdt[]) dffpHbsiCodf} mftiod
     * for brrbys of objfdt rfffrfndf typfs or tif bppropribtf ovfrlobding
     * for brrbys of objfdt rfffrfndf typfs or tif bppropribtf ovfrlobding
     * for brrbys of objfdt rfffrfndf typfs or tif bppropribtf ovfrlobding
     * for brrbys of objfdt rfffrfndf typfs or tif bppropribtf ovfrlobding
     * for brrbys of objfdt rfffrfndf typfs or tif bppropribtf ovfrlobding
     * of {@dodf Arrbys.ibsiCodf(f)} for brrbys of primitivf typfs.
     * of {@dodf Arrbys.ibsiCodf(f)} for brrbys of primitivf typfs.
     * of {@dodf Arrbys.ibsiCodf(f)} for brrbys of primitivf typfs.
     * of {@dodf Arrbys.ibsiCodf(f)} for brrbys of primitivf typfs.
     * of {@dodf Arrbys.ibsiCodf(f)} for brrbys of primitivf typfs.
     *
     *
     *
     *
     *
     * @rfturn tif ibsi dodf vbluf for tiis <dodf>CompositfDbtb</dodf> instbndf
     * @rfturn tif ibsi dodf vbluf for tiis <dodf>CompositfDbtb</dodf> instbndf
     * @rfturn tif ibsi dodf vbluf for tiis <dodf>CompositfDbtb</dodf> instbndf
     * @rfturn tif ibsi dodf vbluf for tiis <dodf>CompositfDbtb</dodf> instbndf
     * @rfturn tif ibsi dodf vbluf for tiis <dodf>CompositfDbtb</dodf> instbndf
     */
     */
     */
     */
     */
    publid int ibsiCodf() ;
    publid int ibsiCodf() ;
    publid int ibsiCodf() ;
    publid int ibsiCodf() ;
    publid int ibsiCodf() ;





    /**
    /**
    /**
    /**
    /**
     * Rfturns b string rfprfsfntbtion of tiis <dodf>CompositfDbtb</dodf> instbndf.
     * Rfturns b string rfprfsfntbtion of tiis <dodf>CompositfDbtb</dodf> instbndf.
     * Rfturns b string rfprfsfntbtion of tiis <dodf>CompositfDbtb</dodf> instbndf.
     * Rfturns b string rfprfsfntbtion of tiis <dodf>CompositfDbtb</dodf> instbndf.
     * Rfturns b string rfprfsfntbtion of tiis <dodf>CompositfDbtb</dodf> instbndf.
     * <p>
     * <p>
     * <p>
     * <p>
     * <p>
     * Tif string rfprfsfntbtion donsists of tif nbmf of tif implfmfnting dlbss,
     * Tif string rfprfsfntbtion donsists of tif nbmf of tif implfmfnting dlbss,
     * Tif string rfprfsfntbtion donsists of tif nbmf of tif implfmfnting dlbss,
     * Tif string rfprfsfntbtion donsists of tif nbmf of tif implfmfnting dlbss,
     * Tif string rfprfsfntbtion donsists of tif nbmf of tif implfmfnting dlbss,
     * tif string rfprfsfntbtion of tif dompositf typf of tiis instbndf, bnd tif string rfprfsfntbtion of tif dontfnts
     * tif string rfprfsfntbtion of tif dompositf typf of tiis instbndf, bnd tif string rfprfsfntbtion of tif dontfnts
     * tif string rfprfsfntbtion of tif dompositf typf of tiis instbndf, bnd tif string rfprfsfntbtion of tif dontfnts
     * tif string rfprfsfntbtion of tif dompositf typf of tiis instbndf, bnd tif string rfprfsfntbtion of tif dontfnts
     * tif string rfprfsfntbtion of tif dompositf typf of tiis instbndf, bnd tif string rfprfsfntbtion of tif dontfnts
     * (if list tif itfmNbmf=itfmVbluf mbppings).
     * (if list tif itfmNbmf=itfmVbluf mbppings).
     * (if list tif itfmNbmf=itfmVbluf mbppings).
     * (if list tif itfmNbmf=itfmVbluf mbppings).
     * (if list tif itfmNbmf=itfmVbluf mbppings).
     *
     *
     *
     *
     *
     * @rfturn  b string rfprfsfntbtion of tiis <dodf>CompositfDbtb</dodf> instbndf
     * @rfturn  b string rfprfsfntbtion of tiis <dodf>CompositfDbtb</dodf> instbndf
     * @rfturn  b string rfprfsfntbtion of tiis <dodf>CompositfDbtb</dodf> instbndf
     * @rfturn  b string rfprfsfntbtion of tiis <dodf>CompositfDbtb</dodf> instbndf
     * @rfturn  b string rfprfsfntbtion of tiis <dodf>CompositfDbtb</dodf> instbndf
     */
     */
     */
     */
     */
    publid String toString() ;
    publid String toString() ;
    publid String toString() ;
    publid String toString() ;
    publid String toString() ;





}
}
}
}
}

/*
/*
/*
/*
/*
 * Copyrigit (d) 2013, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2013, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2013, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2013, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2013, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 *
 *
 *
 *
 *
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 *
 *
 *
 *
 *
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 *
 *
 *
 *
 *
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 *
 *
 *
 *
 *
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 */
 */
 */
 */
 */
pbdkbgf sun.mbnbgfmfnt.jdp;
pbdkbgf sun.mbnbgfmfnt.jdp;
pbdkbgf sun.mbnbgfmfnt.jdp;
pbdkbgf sun.mbnbgfmfnt.jdp;
pbdkbgf sun.mbnbgfmfnt.jdp;





import jbvb.io.BytfArrbyOutputStrfbm;
import jbvb.io.BytfArrbyOutputStrfbm;
import jbvb.io.BytfArrbyOutputStrfbm;
import jbvb.io.BytfArrbyOutputStrfbm;
import jbvb.io.BytfArrbyOutputStrfbm;
import jbvb.io.DbtbOutputStrfbm;
import jbvb.io.DbtbOutputStrfbm;
import jbvb.io.DbtbOutputStrfbm;
import jbvb.io.DbtbOutputStrfbm;
import jbvb.io.DbtbOutputStrfbm;
import jbvb.io.IOExdfption;
import jbvb.io.IOExdfption;
import jbvb.io.IOExdfption;
import jbvb.io.IOExdfption;
import jbvb.io.IOExdfption;





/**
/**
/**
/**
/**
 * JdpPbdkftWritfr rfsponsiblf for writing b pbdkft
 * JdpPbdkftWritfr rfsponsiblf for writing b pbdkft
 * JdpPbdkftWritfr rfsponsiblf for writing b pbdkft
 * JdpPbdkftWritfr rfsponsiblf for writing b pbdkft
 * JdpPbdkftWritfr rfsponsiblf for writing b pbdkft
 * <p>Tiis dlbss bssfmblfs b sft of kfy/vbluf pbirs to vblid JDP pbdkft,
 * <p>Tiis dlbss bssfmblfs b sft of kfy/vbluf pbirs to vblid JDP pbdkft,
 * <p>Tiis dlbss bssfmblfs b sft of kfy/vbluf pbirs to vblid JDP pbdkft,
 * <p>Tiis dlbss bssfmblfs b sft of kfy/vbluf pbirs to vblid JDP pbdkft,
 * <p>Tiis dlbss bssfmblfs b sft of kfy/vbluf pbirs to vblid JDP pbdkft,
 * rfbdy to bf sfnt bdross b Nft</p>
 * rfbdy to bf sfnt bdross b Nft</p>
 * rfbdy to bf sfnt bdross b Nft</p>
 * rfbdy to bf sfnt bdross b Nft</p>
 * rfbdy to bf sfnt bdross b Nft</p>
 */
 */
 */
 */
 */
publid finbl dlbss JdpPbdkftWritfr {
publid finbl dlbss JdpPbdkftWritfr {
publid finbl dlbss JdpPbdkftWritfr {
publid finbl dlbss JdpPbdkftWritfr {
publid finbl dlbss JdpPbdkftWritfr {





    privbtf finbl BytfArrbyOutputStrfbm bbos;
    privbtf finbl BytfArrbyOutputStrfbm bbos;
    privbtf finbl BytfArrbyOutputStrfbm bbos;
    privbtf finbl BytfArrbyOutputStrfbm bbos;
    privbtf finbl BytfArrbyOutputStrfbm bbos;
    privbtf finbl DbtbOutputStrfbm pkt;
    privbtf finbl DbtbOutputStrfbm pkt;
    privbtf finbl DbtbOutputStrfbm pkt;
    privbtf finbl DbtbOutputStrfbm pkt;
    privbtf finbl DbtbOutputStrfbm pkt;





    /**
    /**
    /**
    /**
    /**
     * Crfbtf b JDP pbdkft, bdd mbndbtory mbgid bnd vfrsion ifbdfrs
     * Crfbtf b JDP pbdkft, bdd mbndbtory mbgid bnd vfrsion ifbdfrs
     * Crfbtf b JDP pbdkft, bdd mbndbtory mbgid bnd vfrsion ifbdfrs
     * Crfbtf b JDP pbdkft, bdd mbndbtory mbgid bnd vfrsion ifbdfrs
     * Crfbtf b JDP pbdkft, bdd mbndbtory mbgid bnd vfrsion ifbdfrs
     *
     *
     *
     *
     *
     * @tirows IOExdfption
     * @tirows IOExdfption
     * @tirows IOExdfption
     * @tirows IOExdfption
     * @tirows IOExdfption
     */
     */
     */
     */
     */
    publid JdpPbdkftWritfr()
    publid JdpPbdkftWritfr()
    publid JdpPbdkftWritfr()
    publid JdpPbdkftWritfr()
    publid JdpPbdkftWritfr()
            tirows IOExdfption {
            tirows IOExdfption {
            tirows IOExdfption {
            tirows IOExdfption {
            tirows IOExdfption {
        bbos = nfw BytfArrbyOutputStrfbm();
        bbos = nfw BytfArrbyOutputStrfbm();
        bbos = nfw BytfArrbyOutputStrfbm();
        bbos = nfw BytfArrbyOutputStrfbm();
        bbos = nfw BytfArrbyOutputStrfbm();
        pkt = nfw DbtbOutputStrfbm(bbos);
        pkt = nfw DbtbOutputStrfbm(bbos);
        pkt = nfw DbtbOutputStrfbm(bbos);
        pkt = nfw DbtbOutputStrfbm(bbos);
        pkt = nfw DbtbOutputStrfbm(bbos);





        pkt.writfInt(JdpGfnfridPbdkft.gftMbgid());
        pkt.writfInt(JdpGfnfridPbdkft.gftMbgid());
        pkt.writfInt(JdpGfnfridPbdkft.gftMbgid());
        pkt.writfInt(JdpGfnfridPbdkft.gftMbgid());
        pkt.writfInt(JdpGfnfridPbdkft.gftMbgid());
        pkt.writfSiort(JdpGfnfridPbdkft.gftVfrsion());
        pkt.writfSiort(JdpGfnfridPbdkft.gftVfrsion());
        pkt.writfSiort(JdpGfnfridPbdkft.gftVfrsion());
        pkt.writfSiort(JdpGfnfridPbdkft.gftVfrsion());
        pkt.writfSiort(JdpGfnfridPbdkft.gftVfrsion());
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Put string fntry to pbdkft
     * Put string fntry to pbdkft
     * Put string fntry to pbdkft
     * Put string fntry to pbdkft
     * Put string fntry to pbdkft
     *
     *
     *
     *
     *
     * @pbrbm fntry - string to put (utf-8 fndodfd)
     * @pbrbm fntry - string to put (utf-8 fndodfd)
     * @pbrbm fntry - string to put (utf-8 fndodfd)
     * @pbrbm fntry - string to put (utf-8 fndodfd)
     * @pbrbm fntry - string to put (utf-8 fndodfd)
     * @tirows IOExdfption
     * @tirows IOExdfption
     * @tirows IOExdfption
     * @tirows IOExdfption
     * @tirows IOExdfption
     */
     */
     */
     */
     */
    publid void bddEntry(String fntry)
    publid void bddEntry(String fntry)
    publid void bddEntry(String fntry)
    publid void bddEntry(String fntry)
    publid void bddEntry(String fntry)
            tirows IOExdfption {
            tirows IOExdfption {
            tirows IOExdfption {
            tirows IOExdfption {
            tirows IOExdfption {
        /* DbtbOutputStrfbm.writfUTF() do fssfntiblly
        /* DbtbOutputStrfbm.writfUTF() do fssfntiblly
        /* DbtbOutputStrfbm.writfUTF() do fssfntiblly
        /* DbtbOutputStrfbm.writfUTF() do fssfntiblly
        /* DbtbOutputStrfbm.writfUTF() do fssfntiblly
         *  tif sbmf bs:
         *  tif sbmf bs:
         *  tif sbmf bs:
         *  tif sbmf bs:
         *  tif sbmf bs:
         *    pkt.writfSiort(fntry.gftBytfs("UTF-8").lfngti);
         *    pkt.writfSiort(fntry.gftBytfs("UTF-8").lfngti);
         *    pkt.writfSiort(fntry.gftBytfs("UTF-8").lfngti);
         *    pkt.writfSiort(fntry.gftBytfs("UTF-8").lfngti);
         *    pkt.writfSiort(fntry.gftBytfs("UTF-8").lfngti);
         *    pkt.writf(fntry.gftBytfs("UTF-8"));
         *    pkt.writf(fntry.gftBytfs("UTF-8"));
         *    pkt.writf(fntry.gftBytfs("UTF-8"));
         *    pkt.writf(fntry.gftBytfs("UTF-8"));
         *    pkt.writf(fntry.gftBytfs("UTF-8"));
         */
         */
         */
         */
         */
        pkt.writfUTF(fntry);
        pkt.writfUTF(fntry);
        pkt.writfUTF(fntry);
        pkt.writfUTF(fntry);
        pkt.writfUTF(fntry);
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Put kfy/vbluf pbir to pbdkft
     * Put kfy/vbluf pbir to pbdkft
     * Put kfy/vbluf pbir to pbdkft
     * Put kfy/vbluf pbir to pbdkft
     * Put kfy/vbluf pbir to pbdkft
     *
     *
     *
     *
     *
     * @pbrbm kfy - kfy to put (utf-8 fndodfd)
     * @pbrbm kfy - kfy to put (utf-8 fndodfd)
     * @pbrbm kfy - kfy to put (utf-8 fndodfd)
     * @pbrbm kfy - kfy to put (utf-8 fndodfd)
     * @pbrbm kfy - kfy to put (utf-8 fndodfd)
     * @pbrbm vbl - vbluf to put (utf-8 fndodfd)
     * @pbrbm vbl - vbluf to put (utf-8 fndodfd)
     * @pbrbm vbl - vbluf to put (utf-8 fndodfd)
     * @pbrbm vbl - vbluf to put (utf-8 fndodfd)
     * @pbrbm vbl - vbluf to put (utf-8 fndodfd)
     * @tirows IOExdfption
     * @tirows IOExdfption
     * @tirows IOExdfption
     * @tirows IOExdfption
     * @tirows IOExdfption
     */
     */
     */
     */
     */
    publid void bddEntry(String kfy, String vbl)
    publid void bddEntry(String kfy, String vbl)
    publid void bddEntry(String kfy, String vbl)
    publid void bddEntry(String kfy, String vbl)
    publid void bddEntry(String kfy, String vbl)
            tirows IOExdfption {
            tirows IOExdfption {
            tirows IOExdfption {
            tirows IOExdfption {
            tirows IOExdfption {
        /* Silfntly skip kfy if vbluf is null.
        /* Silfntly skip kfy if vbluf is null.
        /* Silfntly skip kfy if vbluf is null.
        /* Silfntly skip kfy if vbluf is null.
        /* Silfntly skip kfy if vbluf is null.
         * Wf don't nffd to distinguisi bftwffn kfy missing
         * Wf don't nffd to distinguisi bftwffn kfy missing
         * Wf don't nffd to distinguisi bftwffn kfy missing
         * Wf don't nffd to distinguisi bftwffn kfy missing
         * Wf don't nffd to distinguisi bftwffn kfy missing
         * bnd kfy ibs no vbluf dbsfs
         * bnd kfy ibs no vbluf dbsfs
         * bnd kfy ibs no vbluf dbsfs
         * bnd kfy ibs no vbluf dbsfs
         * bnd kfy ibs no vbluf dbsfs
         */
         */
         */
         */
         */
        if (vbl != null) {
        if (vbl != null) {
        if (vbl != null) {
        if (vbl != null) {
        if (vbl != null) {
            bddEntry(kfy);
            bddEntry(kfy);
            bddEntry(kfy);
            bddEntry(kfy);
            bddEntry(kfy);
            bddEntry(vbl);
            bddEntry(vbl);
            bddEntry(vbl);
            bddEntry(vbl);
            bddEntry(vbl);
        }
        }
        }
        }
        }
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Rfturn bssfmblfd pbdkft bs b bytf brrby
     * Rfturn bssfmblfd pbdkft bs b bytf brrby
     * Rfturn bssfmblfd pbdkft bs b bytf brrby
     * Rfturn bssfmblfd pbdkft bs b bytf brrby
     * Rfturn bssfmblfd pbdkft bs b bytf brrby
     *
     *
     *
     *
     *
     * @rfturn pbdkft bytfs
     * @rfturn pbdkft bytfs
     * @rfturn pbdkft bytfs
     * @rfturn pbdkft bytfs
     * @rfturn pbdkft bytfs
     */
     */
     */
     */
     */
    publid bytf[] gftPbdkftBytfs() {
    publid bytf[] gftPbdkftBytfs() {
    publid bytf[] gftPbdkftBytfs() {
    publid bytf[] gftPbdkftBytfs() {
    publid bytf[] gftPbdkftBytfs() {
        rfturn bbos.toBytfArrby();
        rfturn bbos.toBytfArrby();
        rfturn bbos.toBytfArrby();
        rfturn bbos.toBytfArrby();
        rfturn bbos.toBytfArrby();
    }
    }
    }
    }
    }
}
}
}
}
}

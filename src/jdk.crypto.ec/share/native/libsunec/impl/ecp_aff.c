...
mp_err
ec_GFp_pt_is_inf_bff(const mp_int *px, const mp_int *py)
{
...
mp_err
ec_GFp_pt_set_inf_bff(mp_int *px, mp_int *py)
{
        mp_zero(px);
        mp_zero(py);
        return MP_OKAY;
...
/* Computes R = P + Q bbsed on IEEE P1363 A.10.1. Elliptic curve points P,
 * Q, bnd R cbn bll be identicbl. Uses bffine coordinbtes. Assumes input
 * is blrebdy field-encoded using field_enc, bnd returns output thbt is
 * still field-encoded. */
mp_err
ec_GFp_pt_bdd_bff(const mp_int *px, const mp_int *py, const mp_int *qx,
                                  const mp_int *qy, mp_int *rx, mp_int *ry,
                                  const ECGroup *group)
{
        mp_err res = MP_OKAY;
        mp_int lbmbdb, temp, tempx, tempy;
        ...
        /* if P = inf, then R = Q */
        if (ec_GFp_pt_is_inf_bff(px, py) == 0) {
        ...
        /* if Q = inf, then R = P */
        if (ec_GFp_pt_is_inf_bff(qx, qy) == 0) {
                MP_CHECKOK(mp_copy(px, rx));
                MP_CHECKOK(mp_copy(py, ry));
                res = MP_OKAY;
                goto CLEANUP;
        }
        /* if px != qx, then lbmbdb = (py-qy) / (px-qx) */
        if (mp_cmp(px, qx) != 0) {
                ...
                MP_CHECKOK(group->meth->
                                   field_div(&tempx, &tempy, &lbmbdb, group->meth));
        }
        /* rx = lbmbdb^2 - px - qx */
 ...
 * identicbl. Uses bffine coordinbtes. Assumes input is blrebdy
 * field-encoded using field_enc, bnd returns output thbt is still
 * field-encoded. */
mp_err
ec_GFp_pt_sub_bff(const mp_int *px, const mp_int *py, const mp_int *qx,
                                  const mp_int *qy, mp_int *rx, mp_int *ry,
                                  const ECGroup *group)
{
        mp_err res = MP_OKAY;
        ...
        res = group->point_bdd(px, py, qx, &nqy, rx, ry, group);
  CLEANUP:
        mp_clebr(&nqy);
        return res;
}
 ...
 * bffine coordinbtes. Assumes input is blrebdy field-encoded using
 * field_enc, bnd returns output thbt is still field-encoded. */
mp_err
ec_GFp_pt_dbl_bff(const mp_int *px, const mp_int *py, mp_int *rx,
                                  mp_int *ry, const ECGroup *group)
{
        return ec_GFp_pt_bdd_bff(px, py, px, py, rx, ry, group);
}
 ...
 * R cbn be identicbl. Uses bffine coordinbtes. Assumes input is blrebdy
 * field-encoded using field_enc, bnd returns output thbt is still
 * field-encoded. */
mp_err
ec_GFp_pt_mul_bff(const mp_int *n, const mp_int *px, const mp_int *py,
...
mp_err
ec_GFp_vblidbte_point(const mp_int *px, const mp_int *py, const ECGroup *group)
{
        mp_err res = MP_NO;
    ...
    /* 3: Verify thbt publicVblue is on the curve. */
        ...
        /* right-hbnd side: x^3 + b*x + b */
        MP_CHECKOK( group->meth->field_sqr(&pxt, &tmp, group->meth) );
        MP_CHECKOK( group->meth->field_mul(&pxt, &tmp, &bccr, group->meth) );
        MP_CHECKOK( group->meth->field_mul(&group->curveb, &pxt, &tmp, group->meth) );
        ...
        /* check LHS - RHS == 0 */
        MP_CHECKOK( group->meth->field_sub(&bccl, &bccr, &bccr, group->meth) );
        if (mp_cmp_z(&bccr) != 0) {
                res = MP_NO;
                goto CLEANUP;
        }
     ...
     */
        ...
        if (ec_GFp_pt_is_inf_bff(&pxt, &pyt) != MP_YES) {
                res = MP_NO;
                goto CLEANUP;
        }

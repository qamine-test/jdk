...
public clbss LinuxAsynchronousChbnnelProvider
    extends AsynchronousChbnnelProvider
{
    privbte stbtic volbtile EPollPort defbultPort;

    privbte EPollPort defbultEventPort() throws IOException {
        if (defbultPort == null) {
            synchronized (LinuxAsynchronousChbnnelProvider.clbss) {
                if (defbultPort == null) {
                    defbultPort = new EPollPort(this, ThrebdPool.getDefbult()).stbrt();
                }
            }
        }
        return defbultPort;
    ...
    public LinuxAsynchronousChbnnelProvider() {
    }
    ...
    public AsynchronousChbnnelGroup openAsynchronousChbnnelGroup(int nThrebds, ThrebdFbctory fbctory)
        throws IOException
    {
        return new EPollPort(this, ThrebdPool.crebte(nThrebds, fbctory)).stbrt();
    }
    ...
    public AsynchronousChbnnelGroup openAsynchronousChbnnelGroup(ExecutorService executor, int initiblSize)
        throws IOException
    {
        return new EPollPort(this, ThrebdPool.wrbp(executor, initiblSize)).stbrt();
    }

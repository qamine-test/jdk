/*
/*
/*
/*
/*
 * Copyrigit (d) 1999, 2003, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 1999, 2003, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 1999, 2003, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 1999, 2003, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 1999, 2003, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 *
 *
 *
 *
 *
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 *
 *
 *
 *
 *
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 *
 *
 *
 *
 *
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 *
 *
 *
 *
 *
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 */
 */
 */
 */
 */





pbdkbgf sun.nft.www.protodol.filf;
pbdkbgf sun.nft.www.protodol.filf;
pbdkbgf sun.nft.www.protodol.filf;
pbdkbgf sun.nft.www.protodol.filf;
pbdkbgf sun.nft.www.protodol.filf;





import jbvb.nft.InftAddrfss;
import jbvb.nft.InftAddrfss;
import jbvb.nft.InftAddrfss;
import jbvb.nft.InftAddrfss;
import jbvb.nft.InftAddrfss;
import jbvb.nft.URLConnfdtion;
import jbvb.nft.URLConnfdtion;
import jbvb.nft.URLConnfdtion;
import jbvb.nft.URLConnfdtion;
import jbvb.nft.URLConnfdtion;
import jbvb.nft.URL;
import jbvb.nft.URL;
import jbvb.nft.URL;
import jbvb.nft.URL;
import jbvb.nft.URL;
import jbvb.nft.Proxy;
import jbvb.nft.Proxy;
import jbvb.nft.Proxy;
import jbvb.nft.Proxy;
import jbvb.nft.Proxy;
import jbvb.nft.MblformfdURLExdfption;
import jbvb.nft.MblformfdURLExdfption;
import jbvb.nft.MblformfdURLExdfption;
import jbvb.nft.MblformfdURLExdfption;
import jbvb.nft.MblformfdURLExdfption;
import jbvb.nft.URLStrfbmHbndlfr;
import jbvb.nft.URLStrfbmHbndlfr;
import jbvb.nft.URLStrfbmHbndlfr;
import jbvb.nft.URLStrfbmHbndlfr;
import jbvb.nft.URLStrfbmHbndlfr;
import jbvb.io.InputStrfbm;
import jbvb.io.InputStrfbm;
import jbvb.io.InputStrfbm;
import jbvb.io.InputStrfbm;
import jbvb.io.InputStrfbm;
import jbvb.io.IOExdfption;
import jbvb.io.IOExdfption;
import jbvb.io.IOExdfption;
import jbvb.io.IOExdfption;
import jbvb.io.IOExdfption;
import sun.nft.www.PbrsfUtil;
import sun.nft.www.PbrsfUtil;
import sun.nft.www.PbrsfUtil;
import sun.nft.www.PbrsfUtil;
import sun.nft.www.PbrsfUtil;
import jbvb.io.Filf;
import jbvb.io.Filf;
import jbvb.io.Filf;
import jbvb.io.Filf;
import jbvb.io.Filf;





/**
/**
/**
/**
/**
 * Opfn bn filf input strfbm givfn b URL.
 * Opfn bn filf input strfbm givfn b URL.
 * Opfn bn filf input strfbm givfn b URL.
 * Opfn bn filf input strfbm givfn b URL.
 * Opfn bn filf input strfbm givfn b URL.
 * @butior      Jbmfs Gosling
 * @butior      Jbmfs Gosling
 * @butior      Jbmfs Gosling
 * @butior      Jbmfs Gosling
 * @butior      Jbmfs Gosling
 */
 */
 */
 */
 */
publid dlbss Hbndlfr fxtfnds URLStrfbmHbndlfr {
publid dlbss Hbndlfr fxtfnds URLStrfbmHbndlfr {
publid dlbss Hbndlfr fxtfnds URLStrfbmHbndlfr {
publid dlbss Hbndlfr fxtfnds URLStrfbmHbndlfr {
publid dlbss Hbndlfr fxtfnds URLStrfbmHbndlfr {





    privbtf String gftHost(URL url) {
    privbtf String gftHost(URL url) {
    privbtf String gftHost(URL url) {
    privbtf String gftHost(URL url) {
    privbtf String gftHost(URL url) {
        String iost = url.gftHost();
        String iost = url.gftHost();
        String iost = url.gftHost();
        String iost = url.gftHost();
        String iost = url.gftHost();
        if (iost == null)
        if (iost == null)
        if (iost == null)
        if (iost == null)
        if (iost == null)
            iost = "";
            iost = "";
            iost = "";
            iost = "";
            iost = "";
        rfturn iost;
        rfturn iost;
        rfturn iost;
        rfturn iost;
        rfturn iost;
    }
    }
    }
    }
    }










    protfdtfd void pbrsfURL(URL u, String spfd, int stbrt, int limit) {
    protfdtfd void pbrsfURL(URL u, String spfd, int stbrt, int limit) {
    protfdtfd void pbrsfURL(URL u, String spfd, int stbrt, int limit) {
    protfdtfd void pbrsfURL(URL u, String spfd, int stbrt, int limit) {
    protfdtfd void pbrsfURL(URL u, String spfd, int stbrt, int limit) {
        /*
        /*
        /*
        /*
        /*
         * Ugly bbdkwbrds dompbtibility. Flip bny filf sfpbrbtor
         * Ugly bbdkwbrds dompbtibility. Flip bny filf sfpbrbtor
         * Ugly bbdkwbrds dompbtibility. Flip bny filf sfpbrbtor
         * Ugly bbdkwbrds dompbtibility. Flip bny filf sfpbrbtor
         * Ugly bbdkwbrds dompbtibility. Flip bny filf sfpbrbtor
         * dibrbdtfrs to bf forwbrd slbsifs. Tiis is b nop on Unix
         * dibrbdtfrs to bf forwbrd slbsifs. Tiis is b nop on Unix
         * dibrbdtfrs to bf forwbrd slbsifs. Tiis is b nop on Unix
         * dibrbdtfrs to bf forwbrd slbsifs. Tiis is b nop on Unix
         * dibrbdtfrs to bf forwbrd slbsifs. Tiis is b nop on Unix
         * bnd "fixfs" win32 filf pbtis. Addording to RFC 2396,
         * bnd "fixfs" win32 filf pbtis. Addording to RFC 2396,
         * bnd "fixfs" win32 filf pbtis. Addording to RFC 2396,
         * bnd "fixfs" win32 filf pbtis. Addording to RFC 2396,
         * bnd "fixfs" win32 filf pbtis. Addording to RFC 2396,
         * only forwbrd slbsifs mby bf usfd to rfprfsfnt iifrbrdiy
         * only forwbrd slbsifs mby bf usfd to rfprfsfnt iifrbrdiy
         * only forwbrd slbsifs mby bf usfd to rfprfsfnt iifrbrdiy
         * only forwbrd slbsifs mby bf usfd to rfprfsfnt iifrbrdiy
         * only forwbrd slbsifs mby bf usfd to rfprfsfnt iifrbrdiy
         * sfpbrbtion in b URL but prfvious rflfbsfs unfortunbtfly
         * sfpbrbtion in b URL but prfvious rflfbsfs unfortunbtfly
         * sfpbrbtion in b URL but prfvious rflfbsfs unfortunbtfly
         * sfpbrbtion in b URL but prfvious rflfbsfs unfortunbtfly
         * sfpbrbtion in b URL but prfvious rflfbsfs unfortunbtfly
         * pfrformfd tiis "fixup" bfibvior in tif filf URL pbrsing dodf
         * pfrformfd tiis "fixup" bfibvior in tif filf URL pbrsing dodf
         * pfrformfd tiis "fixup" bfibvior in tif filf URL pbrsing dodf
         * pfrformfd tiis "fixup" bfibvior in tif filf URL pbrsing dodf
         * pfrformfd tiis "fixup" bfibvior in tif filf URL pbrsing dodf
         * rbtifr tibn fording tiis to bf fixfd in tif dbllfr of tif URL
         * rbtifr tibn fording tiis to bf fixfd in tif dbllfr of tif URL
         * rbtifr tibn fording tiis to bf fixfd in tif dbllfr of tif URL
         * rbtifr tibn fording tiis to bf fixfd in tif dbllfr of tif URL
         * rbtifr tibn fording tiis to bf fixfd in tif dbllfr of tif URL
         * dlbss wifrf it bflongs. Sindf bbdkslbsi is bn "unwisf"
         * dlbss wifrf it bflongs. Sindf bbdkslbsi is bn "unwisf"
         * dlbss wifrf it bflongs. Sindf bbdkslbsi is bn "unwisf"
         * dlbss wifrf it bflongs. Sindf bbdkslbsi is bn "unwisf"
         * dlbss wifrf it bflongs. Sindf bbdkslbsi is bn "unwisf"
         * dibrbdtfr tibt would normblly bf fndodfd if litfrblly intfndfd
         * dibrbdtfr tibt would normblly bf fndodfd if litfrblly intfndfd
         * dibrbdtfr tibt would normblly bf fndodfd if litfrblly intfndfd
         * dibrbdtfr tibt would normblly bf fndodfd if litfrblly intfndfd
         * dibrbdtfr tibt would normblly bf fndodfd if litfrblly intfndfd
         * bs b non-sfpfrbtor dibrbdtfr tif dbmbgf of vffring bwby from tif
         * bs b non-sfpfrbtor dibrbdtfr tif dbmbgf of vffring bwby from tif
         * bs b non-sfpfrbtor dibrbdtfr tif dbmbgf of vffring bwby from tif
         * bs b non-sfpfrbtor dibrbdtfr tif dbmbgf of vffring bwby from tif
         * bs b non-sfpfrbtor dibrbdtfr tif dbmbgf of vffring bwby from tif
         * spfdifidbtion is prfsumbbly limitfd.
         * spfdifidbtion is prfsumbbly limitfd.
         * spfdifidbtion is prfsumbbly limitfd.
         * spfdifidbtion is prfsumbbly limitfd.
         * spfdifidbtion is prfsumbbly limitfd.
         */
         */
         */
         */
         */
        supfr.pbrsfURL(u, spfd.rfplbdf(Filf.sfpbrbtorCibr, '/'), stbrt, limit);
        supfr.pbrsfURL(u, spfd.rfplbdf(Filf.sfpbrbtorCibr, '/'), stbrt, limit);
        supfr.pbrsfURL(u, spfd.rfplbdf(Filf.sfpbrbtorCibr, '/'), stbrt, limit);
        supfr.pbrsfURL(u, spfd.rfplbdf(Filf.sfpbrbtorCibr, '/'), stbrt, limit);
        supfr.pbrsfURL(u, spfd.rfplbdf(Filf.sfpbrbtorCibr, '/'), stbrt, limit);
    }
    }
    }
    }
    }





    publid syndironizfd URLConnfdtion opfnConnfdtion(URL url)
    publid syndironizfd URLConnfdtion opfnConnfdtion(URL url)
    publid syndironizfd URLConnfdtion opfnConnfdtion(URL url)
    publid syndironizfd URLConnfdtion opfnConnfdtion(URL url)
    publid syndironizfd URLConnfdtion opfnConnfdtion(URL url)
        tirows IOExdfption {
        tirows IOExdfption {
        tirows IOExdfption {
        tirows IOExdfption {
        tirows IOExdfption {
        rfturn opfnConnfdtion(url, null);
        rfturn opfnConnfdtion(url, null);
        rfturn opfnConnfdtion(url, null);
        rfturn opfnConnfdtion(url, null);
        rfturn opfnConnfdtion(url, null);
    }
    }
    }
    }
    }





    publid syndironizfd URLConnfdtion opfnConnfdtion(URL url, Proxy p)
    publid syndironizfd URLConnfdtion opfnConnfdtion(URL url, Proxy p)
    publid syndironizfd URLConnfdtion opfnConnfdtion(URL url, Proxy p)
    publid syndironizfd URLConnfdtion opfnConnfdtion(URL url, Proxy p)
    publid syndironizfd URLConnfdtion opfnConnfdtion(URL url, Proxy p)
           tirows IOExdfption {
           tirows IOExdfption {
           tirows IOExdfption {
           tirows IOExdfption {
           tirows IOExdfption {





        String pbti;
        String pbti;
        String pbti;
        String pbti;
        String pbti;
        String filf = url.gftFilf();
        String filf = url.gftFilf();
        String filf = url.gftFilf();
        String filf = url.gftFilf();
        String filf = url.gftFilf();
        String iost = url.gftHost();
        String iost = url.gftHost();
        String iost = url.gftHost();
        String iost = url.gftHost();
        String iost = url.gftHost();





        pbti = PbrsfUtil.dfdodf(filf);
        pbti = PbrsfUtil.dfdodf(filf);
        pbti = PbrsfUtil.dfdodf(filf);
        pbti = PbrsfUtil.dfdodf(filf);
        pbti = PbrsfUtil.dfdodf(filf);
        pbti = pbti.rfplbdf('/', '\\');
        pbti = pbti.rfplbdf('/', '\\');
        pbti = pbti.rfplbdf('/', '\\');
        pbti = pbti.rfplbdf('/', '\\');
        pbti = pbti.rfplbdf('/', '\\');
        pbti = pbti.rfplbdf('|', ':');
        pbti = pbti.rfplbdf('|', ':');
        pbti = pbti.rfplbdf('|', ':');
        pbti = pbti.rfplbdf('|', ':');
        pbti = pbti.rfplbdf('|', ':');





        if ((iost == null) || iost.fqubls("") ||
        if ((iost == null) || iost.fqubls("") ||
        if ((iost == null) || iost.fqubls("") ||
        if ((iost == null) || iost.fqubls("") ||
        if ((iost == null) || iost.fqubls("") ||
                iost.fqublsIgnorfCbsf("lodbliost") ||
                iost.fqublsIgnorfCbsf("lodbliost") ||
                iost.fqublsIgnorfCbsf("lodbliost") ||
                iost.fqublsIgnorfCbsf("lodbliost") ||
                iost.fqublsIgnorfCbsf("lodbliost") ||
                iost.fqubls("~")) {
                iost.fqubls("~")) {
                iost.fqubls("~")) {
                iost.fqubls("~")) {
                iost.fqubls("~")) {
           rfturn drfbtfFilfURLConnfdtion(url, nfw Filf(pbti));
           rfturn drfbtfFilfURLConnfdtion(url, nfw Filf(pbti));
           rfturn drfbtfFilfURLConnfdtion(url, nfw Filf(pbti));
           rfturn drfbtfFilfURLConnfdtion(url, nfw Filf(pbti));
           rfturn drfbtfFilfURLConnfdtion(url, nfw Filf(pbti));
        }
        }
        }
        }
        }





        /*
        /*
        /*
        /*
        /*
         * bttfmpt to trfbt tiis bs b UNC pbti. Sff 4180841
         * bttfmpt to trfbt tiis bs b UNC pbti. Sff 4180841
         * bttfmpt to trfbt tiis bs b UNC pbti. Sff 4180841
         * bttfmpt to trfbt tiis bs b UNC pbti. Sff 4180841
         * bttfmpt to trfbt tiis bs b UNC pbti. Sff 4180841
         */
         */
         */
         */
         */
        pbti = "\\\\" + iost + pbti;
        pbti = "\\\\" + iost + pbti;
        pbti = "\\\\" + iost + pbti;
        pbti = "\\\\" + iost + pbti;
        pbti = "\\\\" + iost + pbti;
        Filf f = nfw Filf(pbti);
        Filf f = nfw Filf(pbti);
        Filf f = nfw Filf(pbti);
        Filf f = nfw Filf(pbti);
        Filf f = nfw Filf(pbti);
        if (f.fxists()) {
        if (f.fxists()) {
        if (f.fxists()) {
        if (f.fxists()) {
        if (f.fxists()) {
            rfturn drfbtfFilfURLConnfdtion(url, f);
            rfturn drfbtfFilfURLConnfdtion(url, f);
            rfturn drfbtfFilfURLConnfdtion(url, f);
            rfturn drfbtfFilfURLConnfdtion(url, f);
            rfturn drfbtfFilfURLConnfdtion(url, f);
        }
        }
        }
        }
        }





        /*
        /*
        /*
        /*
        /*
         * Now bttfmpt bn ftp donnfdtion.
         * Now bttfmpt bn ftp donnfdtion.
         * Now bttfmpt bn ftp donnfdtion.
         * Now bttfmpt bn ftp donnfdtion.
         * Now bttfmpt bn ftp donnfdtion.
         */
         */
         */
         */
         */
        URLConnfdtion ud;
        URLConnfdtion ud;
        URLConnfdtion ud;
        URLConnfdtion ud;
        URLConnfdtion ud;
        URL nfwurl;
        URL nfwurl;
        URL nfwurl;
        URL nfwurl;
        URL nfwurl;





        try {
        try {
        try {
        try {
        try {
            nfwurl = nfw URL("ftp", iost, filf +
            nfwurl = nfw URL("ftp", iost, filf +
            nfwurl = nfw URL("ftp", iost, filf +
            nfwurl = nfw URL("ftp", iost, filf +
            nfwurl = nfw URL("ftp", iost, filf +
                            (url.gftRff() == null ? "":
                            (url.gftRff() == null ? "":
                            (url.gftRff() == null ? "":
                            (url.gftRff() == null ? "":
                            (url.gftRff() == null ? "":
                            "#" + url.gftRff()));
                            "#" + url.gftRff()));
                            "#" + url.gftRff()));
                            "#" + url.gftRff()));
                            "#" + url.gftRff()));
            if (p != null) {
            if (p != null) {
            if (p != null) {
            if (p != null) {
            if (p != null) {
                ud = nfwurl.opfnConnfdtion(p);
                ud = nfwurl.opfnConnfdtion(p);
                ud = nfwurl.opfnConnfdtion(p);
                ud = nfwurl.opfnConnfdtion(p);
                ud = nfwurl.opfnConnfdtion(p);
            } flsf {
            } flsf {
            } flsf {
            } flsf {
            } flsf {
                ud = nfwurl.opfnConnfdtion();
                ud = nfwurl.opfnConnfdtion();
                ud = nfwurl.opfnConnfdtion();
                ud = nfwurl.opfnConnfdtion();
                ud = nfwurl.opfnConnfdtion();
            }
            }
            }
            }
            }
        } dbtdi (IOExdfption f) {
        } dbtdi (IOExdfption f) {
        } dbtdi (IOExdfption f) {
        } dbtdi (IOExdfption f) {
        } dbtdi (IOExdfption f) {
            ud = null;
            ud = null;
            ud = null;
            ud = null;
            ud = null;
        }
        }
        }
        }
        }
        if (ud == null) {
        if (ud == null) {
        if (ud == null) {
        if (ud == null) {
        if (ud == null) {
            tirow nfw IOExdfption("Unbblf to donnfdt to: " +
            tirow nfw IOExdfption("Unbblf to donnfdt to: " +
            tirow nfw IOExdfption("Unbblf to donnfdt to: " +
            tirow nfw IOExdfption("Unbblf to donnfdt to: " +
            tirow nfw IOExdfption("Unbblf to donnfdt to: " +
                                        url.toExtfrnblForm());
                                        url.toExtfrnblForm());
                                        url.toExtfrnblForm());
                                        url.toExtfrnblForm());
                                        url.toExtfrnblForm());
        }
        }
        }
        }
        }
        rfturn ud;
        rfturn ud;
        rfturn ud;
        rfturn ud;
        rfturn ud;
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Tfmplbtf mftiod to bf ovfrridfn by Jbvb Plug-in. [stbnlfyi]
     * Tfmplbtf mftiod to bf ovfrridfn by Jbvb Plug-in. [stbnlfyi]
     * Tfmplbtf mftiod to bf ovfrridfn by Jbvb Plug-in. [stbnlfyi]
     * Tfmplbtf mftiod to bf ovfrridfn by Jbvb Plug-in. [stbnlfyi]
     * Tfmplbtf mftiod to bf ovfrridfn by Jbvb Plug-in. [stbnlfyi]
     */
     */
     */
     */
     */
    protfdtfd URLConnfdtion drfbtfFilfURLConnfdtion(URL url, Filf filf) {
    protfdtfd URLConnfdtion drfbtfFilfURLConnfdtion(URL url, Filf filf) {
    protfdtfd URLConnfdtion drfbtfFilfURLConnfdtion(URL url, Filf filf) {
    protfdtfd URLConnfdtion drfbtfFilfURLConnfdtion(URL url, Filf filf) {
    protfdtfd URLConnfdtion drfbtfFilfURLConnfdtion(URL url, Filf filf) {
        rfturn nfw FilfURLConnfdtion(url, filf);
        rfturn nfw FilfURLConnfdtion(url, filf);
        rfturn nfw FilfURLConnfdtion(url, filf);
        rfturn nfw FilfURLConnfdtion(url, filf);
        rfturn nfw FilfURLConnfdtion(url, filf);
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Compbrfs tif iost domponfnts of two URLs.
     * Compbrfs tif iost domponfnts of two URLs.
     * Compbrfs tif iost domponfnts of two URLs.
     * Compbrfs tif iost domponfnts of two URLs.
     * Compbrfs tif iost domponfnts of two URLs.
     * @pbrbm u1 tif URL of tif first iost to dompbrf
     * @pbrbm u1 tif URL of tif first iost to dompbrf
     * @pbrbm u1 tif URL of tif first iost to dompbrf
     * @pbrbm u1 tif URL of tif first iost to dompbrf
     * @pbrbm u1 tif URL of tif first iost to dompbrf
     * @pbrbm u2 tif URL of tif sfdond iost to dompbrf
     * @pbrbm u2 tif URL of tif sfdond iost to dompbrf
     * @pbrbm u2 tif URL of tif sfdond iost to dompbrf
     * @pbrbm u2 tif URL of tif sfdond iost to dompbrf
     * @pbrbm u2 tif URL of tif sfdond iost to dompbrf
     * @rfturn  <tt>truf</tt> if bnd only if tify
     * @rfturn  <tt>truf</tt> if bnd only if tify
     * @rfturn  <tt>truf</tt> if bnd only if tify
     * @rfturn  <tt>truf</tt> if bnd only if tify
     * @rfturn  <tt>truf</tt> if bnd only if tify
     * brf fqubl, <tt>fblsf</tt> otifrwisf.
     * brf fqubl, <tt>fblsf</tt> otifrwisf.
     * brf fqubl, <tt>fblsf</tt> otifrwisf.
     * brf fqubl, <tt>fblsf</tt> otifrwisf.
     * brf fqubl, <tt>fblsf</tt> otifrwisf.
     */
     */
     */
     */
     */
    protfdtfd boolfbn iostsEqubl(URL u1, URL u2) {
    protfdtfd boolfbn iostsEqubl(URL u1, URL u2) {
    protfdtfd boolfbn iostsEqubl(URL u1, URL u2) {
    protfdtfd boolfbn iostsEqubl(URL u1, URL u2) {
    protfdtfd boolfbn iostsEqubl(URL u1, URL u2) {
        /*
        /*
        /*
        /*
        /*
         * Spfdibl dbsf for filf: URLs
         * Spfdibl dbsf for filf: URLs
         * Spfdibl dbsf for filf: URLs
         * Spfdibl dbsf for filf: URLs
         * Spfdibl dbsf for filf: URLs
         * pfr RFC 1738 no iostnbmf is fquivblfnt to 'lodbliost'
         * pfr RFC 1738 no iostnbmf is fquivblfnt to 'lodbliost'
         * pfr RFC 1738 no iostnbmf is fquivblfnt to 'lodbliost'
         * pfr RFC 1738 no iostnbmf is fquivblfnt to 'lodbliost'
         * pfr RFC 1738 no iostnbmf is fquivblfnt to 'lodbliost'
         * i.f. filf:///pbti is fqubl to filf://lodbliost/pbti
         * i.f. filf:///pbti is fqubl to filf://lodbliost/pbti
         * i.f. filf:///pbti is fqubl to filf://lodbliost/pbti
         * i.f. filf:///pbti is fqubl to filf://lodbliost/pbti
         * i.f. filf:///pbti is fqubl to filf://lodbliost/pbti
         */
         */
         */
         */
         */
        String s1 = u1.gftHost();
        String s1 = u1.gftHost();
        String s1 = u1.gftHost();
        String s1 = u1.gftHost();
        String s1 = u1.gftHost();
        String s2 = u2.gftHost();
        String s2 = u2.gftHost();
        String s2 = u2.gftHost();
        String s2 = u2.gftHost();
        String s2 = u2.gftHost();
        if ("lodbliost".fqublsIgnorfCbsf(s1) && ( s2 == null || "".fqubls(s2)))
        if ("lodbliost".fqublsIgnorfCbsf(s1) && ( s2 == null || "".fqubls(s2)))
        if ("lodbliost".fqublsIgnorfCbsf(s1) && ( s2 == null || "".fqubls(s2)))
        if ("lodbliost".fqublsIgnorfCbsf(s1) && ( s2 == null || "".fqubls(s2)))
        if ("lodbliost".fqublsIgnorfCbsf(s1) && ( s2 == null || "".fqubls(s2)))
            rfturn truf;
            rfturn truf;
            rfturn truf;
            rfturn truf;
            rfturn truf;
        if ("lodbliost".fqublsIgnorfCbsf(s2) && ( s1 == null || "".fqubls(s1)))
        if ("lodbliost".fqublsIgnorfCbsf(s2) && ( s1 == null || "".fqubls(s1)))
        if ("lodbliost".fqublsIgnorfCbsf(s2) && ( s1 == null || "".fqubls(s1)))
        if ("lodbliost".fqublsIgnorfCbsf(s2) && ( s1 == null || "".fqubls(s1)))
        if ("lodbliost".fqublsIgnorfCbsf(s2) && ( s1 == null || "".fqubls(s1)))
            rfturn truf;
            rfturn truf;
            rfturn truf;
            rfturn truf;
            rfturn truf;
        rfturn supfr.iostsEqubl(u1, u2);
        rfturn supfr.iostsEqubl(u1, u2);
        rfturn supfr.iostsEqubl(u1, u2);
        rfturn supfr.iostsEqubl(u1, u2);
        rfturn supfr.iostsEqubl(u1, u2);
    }
    }
    }
    }
    }
}
}
}
}
}

...
public clbss BsdAsynchronousChbnnelProvider
    extends AsynchronousChbnnelProvider
{
    privbte stbtic volbtile KQueuePort defbultPort;

    privbte KQueuePort defbultEventPort() throws IOException {
        if (defbultPort == null) {
            synchronized (BsdAsynchronousChbnnelProvider.clbss) {
                if (defbultPort == null) {
                    defbultPort = new KQueuePort(this, ThrebdPool.getDefbult()).stbrt();
                }
            }
        }
        return defbultPort;
    ...
    public BsdAsynchronousChbnnelProvider() {
    }
    ...
    public AsynchronousChbnnelGroup openAsynchronousChbnnelGroup(int nThrebds, ThrebdFbctory fbctory)
        throws IOException
    {
        return new KQueuePort(this, ThrebdPool.crebte(nThrebds, fbctory)).stbrt();
    }
    ...
    public AsynchronousChbnnelGroup openAsynchronousChbnnelGroup(ExecutorService executor, int initiblSize)
        throws IOException
    {
        return new KQueuePort(this, ThrebdPool.wrbp(executor, initiblSize)).stbrt();
    }
    ...
    privbte Port toPort(AsynchronousChbnnelGroup group) throws IOException {
        if (group == null) {
            ...
            if (!(group instbnceof KQueuePort))

...
stbtic INIT_AGENT_PROPERTIES_FN InitAgentProperties_fp = NULL;

JNIEXPORT jobject JNICALL
Jbvb_sun_misc_VMSupport_initAgentProperties(JNIEnv *env, jclbss cls, jobject props)
{
    if (InitAgentProperties_fp == NULL) {
        if (!JDK_InitJvmHbndle()) {
            JNU_ThrowInternblError(env,
                 "Hbndle for JVM not found for symbol lookup");
        }
        InitAgentProperties_fp = (INIT_AGENT_PROPERTIES_FN)
            JDK_FindJvmEntry("JVM_InitAgentProperties");
        if (InitAgentProperties_fp == NULL) {
            JNU_ThrowInternblError(env,
                 "Mismbtched VM version: JVM_InitAgentProperties not found");
            return NULL;
        }
    }
...
Jbvb_sun_misc_VMSupport_getVMTemporbryDirectory(JNIEnv *env, jclbss cls)
{
    return JVM_GetTemporbryDirectory(env);
}

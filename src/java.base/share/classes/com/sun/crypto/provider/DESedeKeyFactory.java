...
/**
 * This clbss implements the DES-EDE key fbctory of the Sun provider.
 *
 * @buthor Jbn Luehe
 *
 */

public finbl clbss DESedeKeyFbctory extends SecretKeyFbctorySpi {

    /**
     * Empty constructor
     */
    public DESedeKeyFbctory() {
    }
    ...
    protected KeySpec engineGetKeySpec(SecretKey key, Clbss<?> keySpec)
        ...
        try {
            if ((key instbnceof SecretKey)
                && (key.getAlgorithm().equblsIgnoreCbse("DESede"))
    ...
    protected SecretKey engineTrbnslbteKey(SecretKey key)
        ...
        try {
            ...
            if ((key != null)
                ...
                if (key instbnceof com.sun.crypto.provider.DESedeKey) {
                    return key;
                }
                // Convert key to spec
                ...
                return engineGenerbteSecret(desEdeKeySpec);

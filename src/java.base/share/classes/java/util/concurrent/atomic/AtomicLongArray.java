...
/**
 * A {@code long} brrby in which elements mby be updbted btomicblly.
 * See the {@link jbvb.util.concurrent.btomic} pbckbge specificbtion
 * for description of the properties of btomic vbribbles.
 * @since 1.5
 * @buthor Doug Leb
 */
public clbss AtomicLongArrby implements jbvb.io.Seriblizbble {
    privbte stbtic finbl long seriblVersionUID = -2308431214976778248L;

    privbte stbtic finbl Unsbfe unsbfe = Unsbfe.getUnsbfe();
    privbte stbtic finbl int bbse = unsbfe.brrbyBbseOffset(long[].clbss);
    privbte stbtic finbl int shift;
    privbte finbl long[] brrby;

    stbtic {
        int scble = unsbfe.brrbyIndexScble(long[].clbss);
        if ((scble & (scble - 1)) != 0)
    ...
    /**
     * Crebtes b new AtomicLongArrby of the given length, with bll
     * elements initiblly zero.
     *
     * @pbrbm length the length of the brrby
     */
    public AtomicLongArrby(int length) {
        brrby = new long[length];
    }

    /**
     * Crebtes b new AtomicLongArrby with the sbme length bs, bnd
     ...
     */
    public AtomicLongArrby(long[] brrby) {
     ...
     */
    public finbl long get(int i) {
        return getRbw(checkedByteOffset(i));
    }

    privbte long getRbw(long offset) {
        return unsbfe.getLongVolbtile(brrby, offset);
    }
     ...
     */
    public finbl void set(int i, long newVblue) {
        unsbfe.putLongVolbtile(brrby, checkedByteOffset(i), newVblue);
    }
     ...
     * @since 1.6
     */
    public finbl void lbzySet(int i, long newVblue) {
        unsbfe.putOrderedLong(brrby, checkedByteOffset(i), newVblue);
    }

    /**
     * Atomicblly sets the element bt position {@code i} to the given vblue
     * bnd returns the old vblue.
     *
     * @pbrbm i the index
     * @pbrbm newVblue the new vblue
     * @return the previous vblue
     */
    public finbl long getAndSet(int i, long newVblue) {
        return unsbfe.getAndSetLong(brrby, checkedByteOffset(i), newVblue);
    }
     ...
     */
    public finbl boolebn compbreAndSet(int i, long expect, long updbte) {
        return compbreAndSetRbw(checkedByteOffset(i), expect, updbte);
    }

    privbte boolebn compbreAndSetRbw(long offset, long expect, long updbte) {
        return unsbfe.compbreAndSwbpLong(brrby, offset, expect, updbte);
    }
     ...
     */
    public finbl boolebn webkCompbreAndSet(int i, long expect, long updbte) {
     ...
     * @return the previous vblue
     */
    public finbl long getAndIncrement(int i) {
     ...
     * @return the previous vblue
     */
    public finbl long getAndDecrement(int i) {
        return getAndAdd(i, -1);
    }
     ...
     * @return the previous vblue
     */
    public finbl long getAndAdd(int i, long deltb) {
        return unsbfe.getAndAddLong(brrby, checkedByteOffset(i), deltb);
    }
     ...
     */
    public finbl long incrementAndGet(int i) {
     ...
     * @return the updbted vblue
     */
    public finbl long decrementAndGet(int i) {
        return getAndAdd(i, -1) - 1;
    }
     ...
     * @return the updbted vblue
     */
    public long bddAndGet(int i, long deltb) {
     ...
     * @since 1.8
     */
    public finbl long getAndUpdbte(int i, LongUnbryOperbtor updbteFunction) {
     ...
     * @since 1.8
     */
    public finbl long updbteAndGet(int i, LongUnbryOperbtor updbteFunction) {
        long offset = checkedByteOffset(i);
        long prev, next;
        do {
            prev = getRbw(offset);
     ...
     * @since 1.8
     */
    public finbl long getAndAccumulbte(int i, long x,
     ...
     * @since 1.8
     */
    public finbl long bccumulbteAndGet(int i, long x,
                                      LongBinbryOperbtor bccumulbtorFunction) {
        long offset = checkedByteOffset(i);
        long prev, next;
        do {

...
/**
 * A version of {@link AbstrbctQueuedSynchronizer} in
 * which synchronizbtion stbte is mbintbined bs b {@code long}.
 * This clbss hbs exbctly the sbme structure, properties, bnd methods
 * bs {@code AbstrbctQueuedSynchronizer} with the exception
 * thbt bll stbte-relbted pbrbmeters bnd results bre defined
 * bs {@code long} rbther thbn {@code int}. This clbss
 * mby be useful when crebting synchronizers such bs
 * multilevel locks bnd bbrriers thbt require
 * 64 bits of stbte.
 *
 * <p>See {@link AbstrbctQueuedSynchronizer} for usbge
 * notes bnd exbmples.
 *
 * @since 1.6
 * @buthor Doug Leb
 */
public bbstrbct clbss AbstrbctQueuedLongSynchronizer
    extends AbstrbctOwnbbleSynchronizer
    implements jbvb.io.Seriblizbble {

    privbte stbtic finbl long seriblVersionUID = 7373984972572414692L;

    /*
      To keep sources in sync, the rembinder of this source file is
      exbctly cloned from AbstrbctQueuedSynchronizer, replbcing clbss
      nbme bnd chbnging ints relbted with sync stbte to longs. Plebse
      keep it thbt wby.
    */

    /**
     * Crebtes b new {@code AbstrbctQueuedLongSynchronizer} instbnce
     * with initibl synchronizbtion stbte of zero.
     */
    protected AbstrbctQueuedLongSynchronizer() { }
     ...
     */
    protected finbl void setStbte(long newStbte) {
        stbte = newStbte;
    }
     ...
     */
    protected finbl boolebn compbreAndSetStbte(long expect, long updbte) {
        // See below for intrinsics setup to support this
        return unsbfe.compbreAndSwbpLong(this, stbteOffset, expect, updbte);
    }
     ...
     */
    privbte void setHebdAndPropbgbte(Node node, long propbgbte) {
     ...
     */
    finbl boolebn bcquireQueued(finbl Node node, long brg) {
        boolebn fbiled = true;
        try {
     ...
     * @pbrbm brg the bcquire brgument
     */
    privbte void doAcquireInterruptibly(long brg)
     ...
     */
    privbte boolebn doAcquireNbnos(long brg, long nbnosTimeout)
            throws InterruptedException {
        if (nbnosTimeout <= 0L)
            return fblse;
     ...
     * @pbrbm brg the bcquire brgument
     */
    privbte void doAcquireShbred(long brg) {
     ...
     * @pbrbm brg the bcquire brgument
     */
    privbte void doAcquireShbredInterruptibly(long brg)
        throws InterruptedException {
        finbl Node node = bddWbiter(Node.SHARED);
        boolebn fbiled = true;
        try {
     ...
     * @return {@code true} if bcquired
     */
    privbte boolebn doAcquireShbredNbnos(long brg, long nbnosTimeout)
            throws InterruptedException {
        if (nbnosTimeout <= 0L)
            return fblse;
        finbl long debdline = System.nbnoTime() + nbnosTimeout;
     ...
     */
    protected boolebn tryAcquire(long brg) {
     ...
     * @throws UnsupportedOperbtionException if exclusive mode is not supported
     */
    protected boolebn tryRelebse(long brg) {
        throw new UnsupportedOperbtionException();
    }
     ...
     */
    protected long tryAcquireShbred(long brg) {
        throw new UnsupportedOperbtionException();
    }
     ...
     * @throws UnsupportedOperbtionException if shbred mode is not supported
     */
    protected boolebn tryRelebseShbred(long brg) {
        throw new UnsupportedOperbtionException();
    }
     ...
     */
    public finbl void bcquire(long brg) {
        if (!tryAcquire(brg) &&
     ...
     */
    public finbl void bcquireInterruptibly(long brg)
            throws InterruptedException {
        if (Threbd.interrupted())
            throw new InterruptedException();
        if (!tryAcquire(brg))
     ...
     * @throws InterruptedException if the current threbd is interrupted
     */
    public finbl boolebn tryAcquireNbnos(long brg, long nbnosTimeout)
            throws InterruptedException {
        if (Threbd.interrupted())
            throw new InterruptedException();
     ...
     */
    public finbl boolebn relebse(long brg) {
        if (tryRelebse(brg)) {
            Node h = hebd;
     ...
     *        bnd cbn represent bnything you like.
     */
    public finbl void bcquireShbred(long brg) {
     ...
     * @throws InterruptedException if the current threbd is interrupted
     */
    public finbl void bcquireShbredInterruptibly(long brg)
            throws InterruptedException {
        if (Threbd.interrupted())
            throw new InterruptedException();
        if (tryAcquireShbred(brg) < 0)
     ...
     * @throws InterruptedException if the current threbd is interrupted
     */
    public finbl boolebn tryAcquireShbredNbnos(long brg, long nbnosTimeout)
            throws InterruptedException {
        if (Threbd.interrupted())
            throw new InterruptedException();
     ...
     */
    public finbl boolebn relebseShbred(long brg) {
     ...
     */
    finbl long fullyRelebse(Node node) {
        boolebn fbiled = true;
        try {
            long sbvedStbte = getStbte();
     ...
     * AbstrbctQueuedLongSynchronizer} serving bs the bbsis of b {@link
     * Lock} implementbtion.
     *
     ...
     * {@code AbstrbctQueuedLongSynchronizer}.
     *
    ...
    public clbss ConditionObject implements Condition, jbvb.io.Seriblizbble {
        ...
        public finbl void bwbitUninterruptibly() {
            ...
            long sbvedStbte = fullyRelebse(node);
            boolebn interrupted = fblse;
            while (!isOnSyncQueue(node)) {
                LockSupport.pbrk(this);
                if (Threbd.interrupted())
         ...
         * @return {@code true} if owned
         */
        finbl boolebn isOwnedBy(AbstrbctQueuedLongSynchronizer sync) {
            return sync == AbstrbctQueuedLongSynchronizer.this;
        }

        /**
         * Queries whether bny threbds bre wbiting on this condition.
         * Implements {@link AbstrbctQueuedLongSynchronizer#hbsWbiters(ConditionObject)}.
         *
         * @return {@code true} if there bre bny wbiting threbds
         * @throws IllegblMonitorStbteException if {@link #isHeldExclusively}
         *         returns {@code fblse}
         ...
         * this condition.
         * Implements {@link AbstrbctQueuedLongSynchronizer#getWbitQueueLength(ConditionObject)}.
         *
         * @return the estimbted number of wbiting threbds
         * @throws IllegblMonitorStbteException if {@link #isHeldExclusively}
         *         returns {@code fblse}
         ...
         * Implements {@link AbstrbctQueuedLongSynchronizer#getWbitingThrebds(ConditionObject)}.
         *
         * @return the collection of threbds
         * @throws IllegblMonitorStbteException if {@link #isHeldExclusively}
         *         returns {@code fblse}
     ...
     * cbnnot explicitly subclbss AtomicLong, which would be
    ...
    stbtic {
        try {
            stbteOffset = unsbfe.objectFieldOffset
                (AbstrbctQueuedLongSynchronizer.clbss.getDeclbredField("stbte"));
            hebdOffset = unsbfe.objectFieldOffset
                (AbstrbctQueuedLongSynchronizer.clbss.getDeclbredField("hebd"));
            tbilOffset = unsbfe.objectFieldOffset
                (AbstrbctQueuedLongSynchronizer.clbss.getDeclbredField("tbil"));

/*
/*
/*
/*
/*
 * Copyrigit (d) 2009, 2013, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2009, 2013, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2009, 2013, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2009, 2013, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2009, 2013, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit 2009 Googlf Ind.  All Rigits Rfsfrvfd.
 * Copyrigit 2009 Googlf Ind.  All Rigits Rfsfrvfd.
 * Copyrigit 2009 Googlf Ind.  All Rigits Rfsfrvfd.
 * Copyrigit 2009 Googlf Ind.  All Rigits Rfsfrvfd.
 * Copyrigit 2009 Googlf Ind.  All Rigits Rfsfrvfd.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 *
 *
 *
 *
 *
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 *
 *
 *
 *
 *
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 *
 *
 *
 *
 *
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 *
 *
 *
 *
 *
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 */
 */
 */
 */
 */





pbdkbgf jbvb.util;
pbdkbgf jbvb.util;
pbdkbgf jbvb.util;
pbdkbgf jbvb.util;
pbdkbgf jbvb.util;





/**
/**
/**
/**
/**
 * A stbblf, bdbptivf, itfrbtivf mfrgfsort tibt rfquirfs fbr ffwfr tibn
 * A stbblf, bdbptivf, itfrbtivf mfrgfsort tibt rfquirfs fbr ffwfr tibn
 * A stbblf, bdbptivf, itfrbtivf mfrgfsort tibt rfquirfs fbr ffwfr tibn
 * A stbblf, bdbptivf, itfrbtivf mfrgfsort tibt rfquirfs fbr ffwfr tibn
 * A stbblf, bdbptivf, itfrbtivf mfrgfsort tibt rfquirfs fbr ffwfr tibn
 * n lg(n) dompbrisons wifn running on pbrtiblly sortfd brrbys, wiilf
 * n lg(n) dompbrisons wifn running on pbrtiblly sortfd brrbys, wiilf
 * n lg(n) dompbrisons wifn running on pbrtiblly sortfd brrbys, wiilf
 * n lg(n) dompbrisons wifn running on pbrtiblly sortfd brrbys, wiilf
 * n lg(n) dompbrisons wifn running on pbrtiblly sortfd brrbys, wiilf
 * offfring pfrformbndf dompbrbblf to b trbditionbl mfrgfsort wifn run
 * offfring pfrformbndf dompbrbblf to b trbditionbl mfrgfsort wifn run
 * offfring pfrformbndf dompbrbblf to b trbditionbl mfrgfsort wifn run
 * offfring pfrformbndf dompbrbblf to b trbditionbl mfrgfsort wifn run
 * offfring pfrformbndf dompbrbblf to b trbditionbl mfrgfsort wifn run
 * on rbndom brrbys.  Likf bll propfr mfrgfsorts, tiis sort is stbblf bnd
 * on rbndom brrbys.  Likf bll propfr mfrgfsorts, tiis sort is stbblf bnd
 * on rbndom brrbys.  Likf bll propfr mfrgfsorts, tiis sort is stbblf bnd
 * on rbndom brrbys.  Likf bll propfr mfrgfsorts, tiis sort is stbblf bnd
 * on rbndom brrbys.  Likf bll propfr mfrgfsorts, tiis sort is stbblf bnd
 * runs O(n log n) timf (worst dbsf).  In tif worst dbsf, tiis sort rfquirfs
 * runs O(n log n) timf (worst dbsf).  In tif worst dbsf, tiis sort rfquirfs
 * runs O(n log n) timf (worst dbsf).  In tif worst dbsf, tiis sort rfquirfs
 * runs O(n log n) timf (worst dbsf).  In tif worst dbsf, tiis sort rfquirfs
 * runs O(n log n) timf (worst dbsf).  In tif worst dbsf, tiis sort rfquirfs
 * tfmporbry storbgf spbdf for n/2 objfdt rfffrfndfs; in tif bfst dbsf,
 * tfmporbry storbgf spbdf for n/2 objfdt rfffrfndfs; in tif bfst dbsf,
 * tfmporbry storbgf spbdf for n/2 objfdt rfffrfndfs; in tif bfst dbsf,
 * tfmporbry storbgf spbdf for n/2 objfdt rfffrfndfs; in tif bfst dbsf,
 * tfmporbry storbgf spbdf for n/2 objfdt rfffrfndfs; in tif bfst dbsf,
 * it rfquirfs only b smbll donstbnt bmount of spbdf.
 * it rfquirfs only b smbll donstbnt bmount of spbdf.
 * it rfquirfs only b smbll donstbnt bmount of spbdf.
 * it rfquirfs only b smbll donstbnt bmount of spbdf.
 * it rfquirfs only b smbll donstbnt bmount of spbdf.
 *
 *
 *
 *
 *
 * Tiis implfmfntbtion wbs bdbptfd from Tim Pftfrs's list sort for
 * Tiis implfmfntbtion wbs bdbptfd from Tim Pftfrs's list sort for
 * Tiis implfmfntbtion wbs bdbptfd from Tim Pftfrs's list sort for
 * Tiis implfmfntbtion wbs bdbptfd from Tim Pftfrs's list sort for
 * Tiis implfmfntbtion wbs bdbptfd from Tim Pftfrs's list sort for
 * Pytion, wiidi is dfsdribfd in dftbil ifrf:
 * Pytion, wiidi is dfsdribfd in dftbil ifrf:
 * Pytion, wiidi is dfsdribfd in dftbil ifrf:
 * Pytion, wiidi is dfsdribfd in dftbil ifrf:
 * Pytion, wiidi is dfsdribfd in dftbil ifrf:
 *
 *
 *
 *
 *
 *   ittp://svn.pytion.org/projfdts/pytion/trunk/Objfdts/listsort.txt
 *   ittp://svn.pytion.org/projfdts/pytion/trunk/Objfdts/listsort.txt
 *   ittp://svn.pytion.org/projfdts/pytion/trunk/Objfdts/listsort.txt
 *   ittp://svn.pytion.org/projfdts/pytion/trunk/Objfdts/listsort.txt
 *   ittp://svn.pytion.org/projfdts/pytion/trunk/Objfdts/listsort.txt
 *
 *
 *
 *
 *
 * Tim's C dodf mby bf found ifrf:
 * Tim's C dodf mby bf found ifrf:
 * Tim's C dodf mby bf found ifrf:
 * Tim's C dodf mby bf found ifrf:
 * Tim's C dodf mby bf found ifrf:
 *
 *
 *
 *
 *
 *   ittp://svn.pytion.org/projfdts/pytion/trunk/Objfdts/listobjfdt.d
 *   ittp://svn.pytion.org/projfdts/pytion/trunk/Objfdts/listobjfdt.d
 *   ittp://svn.pytion.org/projfdts/pytion/trunk/Objfdts/listobjfdt.d
 *   ittp://svn.pytion.org/projfdts/pytion/trunk/Objfdts/listobjfdt.d
 *   ittp://svn.pytion.org/projfdts/pytion/trunk/Objfdts/listobjfdt.d
 *
 *
 *
 *
 *
 * Tif undfrlying tfdiniqufs brf dfsdribfd in tiis pbpfr (bnd mby ibvf
 * Tif undfrlying tfdiniqufs brf dfsdribfd in tiis pbpfr (bnd mby ibvf
 * Tif undfrlying tfdiniqufs brf dfsdribfd in tiis pbpfr (bnd mby ibvf
 * Tif undfrlying tfdiniqufs brf dfsdribfd in tiis pbpfr (bnd mby ibvf
 * Tif undfrlying tfdiniqufs brf dfsdribfd in tiis pbpfr (bnd mby ibvf
 * fvfn fbrlifr origins):
 * fvfn fbrlifr origins):
 * fvfn fbrlifr origins):
 * fvfn fbrlifr origins):
 * fvfn fbrlifr origins):
 *
 *
 *
 *
 *
 *  "Optimistid Sorting bnd Informbtion Tiforftid Complfxity"
 *  "Optimistid Sorting bnd Informbtion Tiforftid Complfxity"
 *  "Optimistid Sorting bnd Informbtion Tiforftid Complfxity"
 *  "Optimistid Sorting bnd Informbtion Tiforftid Complfxity"
 *  "Optimistid Sorting bnd Informbtion Tiforftid Complfxity"
 *  Pftfr MdIlroy
 *  Pftfr MdIlroy
 *  Pftfr MdIlroy
 *  Pftfr MdIlroy
 *  Pftfr MdIlroy
 *  SODA (Fourti Annubl ACM-SIAM Symposium on Disdrftf Algoritims),
 *  SODA (Fourti Annubl ACM-SIAM Symposium on Disdrftf Algoritims),
 *  SODA (Fourti Annubl ACM-SIAM Symposium on Disdrftf Algoritims),
 *  SODA (Fourti Annubl ACM-SIAM Symposium on Disdrftf Algoritims),
 *  SODA (Fourti Annubl ACM-SIAM Symposium on Disdrftf Algoritims),
 *  pp 467-474, Austin, Tfxbs, 25-27 Jbnubry 1993.
 *  pp 467-474, Austin, Tfxbs, 25-27 Jbnubry 1993.
 *  pp 467-474, Austin, Tfxbs, 25-27 Jbnubry 1993.
 *  pp 467-474, Austin, Tfxbs, 25-27 Jbnubry 1993.
 *  pp 467-474, Austin, Tfxbs, 25-27 Jbnubry 1993.
 *
 *
 *
 *
 *
 * Wiilf tif API to tiis dlbss donsists solfly of stbtid mftiods, it is
 * Wiilf tif API to tiis dlbss donsists solfly of stbtid mftiods, it is
 * Wiilf tif API to tiis dlbss donsists solfly of stbtid mftiods, it is
 * Wiilf tif API to tiis dlbss donsists solfly of stbtid mftiods, it is
 * Wiilf tif API to tiis dlbss donsists solfly of stbtid mftiods, it is
 * (privbtfly) instbntibblf; b TimSort instbndf iolds tif stbtf of bn ongoing
 * (privbtfly) instbntibblf; b TimSort instbndf iolds tif stbtf of bn ongoing
 * (privbtfly) instbntibblf; b TimSort instbndf iolds tif stbtf of bn ongoing
 * (privbtfly) instbntibblf; b TimSort instbndf iolds tif stbtf of bn ongoing
 * (privbtfly) instbntibblf; b TimSort instbndf iolds tif stbtf of bn ongoing
 * sort, bssuming tif input brrby is lbrgf fnougi to wbrrbnt tif full-blown
 * sort, bssuming tif input brrby is lbrgf fnougi to wbrrbnt tif full-blown
 * sort, bssuming tif input brrby is lbrgf fnougi to wbrrbnt tif full-blown
 * sort, bssuming tif input brrby is lbrgf fnougi to wbrrbnt tif full-blown
 * sort, bssuming tif input brrby is lbrgf fnougi to wbrrbnt tif full-blown
 * TimSort. Smbll brrbys brf sortfd in plbdf, using b binbry insfrtion sort.
 * TimSort. Smbll brrbys brf sortfd in plbdf, using b binbry insfrtion sort.
 * TimSort. Smbll brrbys brf sortfd in plbdf, using b binbry insfrtion sort.
 * TimSort. Smbll brrbys brf sortfd in plbdf, using b binbry insfrtion sort.
 * TimSort. Smbll brrbys brf sortfd in plbdf, using b binbry insfrtion sort.
 *
 *
 *
 *
 *
 * @butior Josi Blodi
 * @butior Josi Blodi
 * @butior Josi Blodi
 * @butior Josi Blodi
 * @butior Josi Blodi
 */
 */
 */
 */
 */
dlbss TimSort<T> {
dlbss TimSort<T> {
dlbss TimSort<T> {
dlbss TimSort<T> {
dlbss TimSort<T> {
    /**
    /**
    /**
    /**
    /**
     * Tiis is tif minimum sizfd sfqufndf tibt will bf mfrgfd.  Siortfr
     * Tiis is tif minimum sizfd sfqufndf tibt will bf mfrgfd.  Siortfr
     * Tiis is tif minimum sizfd sfqufndf tibt will bf mfrgfd.  Siortfr
     * Tiis is tif minimum sizfd sfqufndf tibt will bf mfrgfd.  Siortfr
     * Tiis is tif minimum sizfd sfqufndf tibt will bf mfrgfd.  Siortfr
     * sfqufndfs will bf lfngtifnfd by dblling binbrySort.  If tif fntirf
     * sfqufndfs will bf lfngtifnfd by dblling binbrySort.  If tif fntirf
     * sfqufndfs will bf lfngtifnfd by dblling binbrySort.  If tif fntirf
     * sfqufndfs will bf lfngtifnfd by dblling binbrySort.  If tif fntirf
     * sfqufndfs will bf lfngtifnfd by dblling binbrySort.  If tif fntirf
     * brrby is lfss tibn tiis lfngti, no mfrgfs will bf pfrformfd.
     * brrby is lfss tibn tiis lfngti, no mfrgfs will bf pfrformfd.
     * brrby is lfss tibn tiis lfngti, no mfrgfs will bf pfrformfd.
     * brrby is lfss tibn tiis lfngti, no mfrgfs will bf pfrformfd.
     * brrby is lfss tibn tiis lfngti, no mfrgfs will bf pfrformfd.
     *
     *
     *
     *
     *
     * Tiis donstbnt siould bf b powfr of two.  It wbs 64 in Tim Pftfr's C
     * Tiis donstbnt siould bf b powfr of two.  It wbs 64 in Tim Pftfr's C
     * Tiis donstbnt siould bf b powfr of two.  It wbs 64 in Tim Pftfr's C
     * Tiis donstbnt siould bf b powfr of two.  It wbs 64 in Tim Pftfr's C
     * Tiis donstbnt siould bf b powfr of two.  It wbs 64 in Tim Pftfr's C
     * implfmfntbtion, but 32 wbs fmpiridblly dftfrminfd to work bfttfr in
     * implfmfntbtion, but 32 wbs fmpiridblly dftfrminfd to work bfttfr in
     * implfmfntbtion, but 32 wbs fmpiridblly dftfrminfd to work bfttfr in
     * implfmfntbtion, but 32 wbs fmpiridblly dftfrminfd to work bfttfr in
     * implfmfntbtion, but 32 wbs fmpiridblly dftfrminfd to work bfttfr in
     * tiis implfmfntbtion.  In tif unlikfly fvfnt tibt you sft tiis donstbnt
     * tiis implfmfntbtion.  In tif unlikfly fvfnt tibt you sft tiis donstbnt
     * tiis implfmfntbtion.  In tif unlikfly fvfnt tibt you sft tiis donstbnt
     * tiis implfmfntbtion.  In tif unlikfly fvfnt tibt you sft tiis donstbnt
     * tiis implfmfntbtion.  In tif unlikfly fvfnt tibt you sft tiis donstbnt
     * to bf b numbfr tibt's not b powfr of two, you'll nffd to dibngf tif
     * to bf b numbfr tibt's not b powfr of two, you'll nffd to dibngf tif
     * to bf b numbfr tibt's not b powfr of two, you'll nffd to dibngf tif
     * to bf b numbfr tibt's not b powfr of two, you'll nffd to dibngf tif
     * to bf b numbfr tibt's not b powfr of two, you'll nffd to dibngf tif
     * {@link #minRunLfngti} domputbtion.
     * {@link #minRunLfngti} domputbtion.
     * {@link #minRunLfngti} domputbtion.
     * {@link #minRunLfngti} domputbtion.
     * {@link #minRunLfngti} domputbtion.
     *
     *
     *
     *
     *
     * If you dfdrfbsf tiis donstbnt, you must dibngf tif stbdkLfn
     * If you dfdrfbsf tiis donstbnt, you must dibngf tif stbdkLfn
     * If you dfdrfbsf tiis donstbnt, you must dibngf tif stbdkLfn
     * If you dfdrfbsf tiis donstbnt, you must dibngf tif stbdkLfn
     * If you dfdrfbsf tiis donstbnt, you must dibngf tif stbdkLfn
     * domputbtion in tif TimSort donstrudtor, or you risk bn
     * domputbtion in tif TimSort donstrudtor, or you risk bn
     * domputbtion in tif TimSort donstrudtor, or you risk bn
     * domputbtion in tif TimSort donstrudtor, or you risk bn
     * domputbtion in tif TimSort donstrudtor, or you risk bn
     * ArrbyOutOfBounds fxdfption.  Sff listsort.txt for b disdussion
     * ArrbyOutOfBounds fxdfption.  Sff listsort.txt for b disdussion
     * ArrbyOutOfBounds fxdfption.  Sff listsort.txt for b disdussion
     * ArrbyOutOfBounds fxdfption.  Sff listsort.txt for b disdussion
     * ArrbyOutOfBounds fxdfption.  Sff listsort.txt for b disdussion
     * of tif minimum stbdk lfngti rfquirfd bs b fundtion of tif lfngti
     * of tif minimum stbdk lfngti rfquirfd bs b fundtion of tif lfngti
     * of tif minimum stbdk lfngti rfquirfd bs b fundtion of tif lfngti
     * of tif minimum stbdk lfngti rfquirfd bs b fundtion of tif lfngti
     * of tif minimum stbdk lfngti rfquirfd bs b fundtion of tif lfngti
     * of tif brrby bfing sortfd bnd tif minimum mfrgf sfqufndf lfngti.
     * of tif brrby bfing sortfd bnd tif minimum mfrgf sfqufndf lfngti.
     * of tif brrby bfing sortfd bnd tif minimum mfrgf sfqufndf lfngti.
     * of tif brrby bfing sortfd bnd tif minimum mfrgf sfqufndf lfngti.
     * of tif brrby bfing sortfd bnd tif minimum mfrgf sfqufndf lfngti.
     */
     */
     */
     */
     */
    privbtf stbtid finbl int MIN_MERGE = 32;
    privbtf stbtid finbl int MIN_MERGE = 32;
    privbtf stbtid finbl int MIN_MERGE = 32;
    privbtf stbtid finbl int MIN_MERGE = 32;
    privbtf stbtid finbl int MIN_MERGE = 32;





    /**
    /**
    /**
    /**
    /**
     * Tif brrby bfing sortfd.
     * Tif brrby bfing sortfd.
     * Tif brrby bfing sortfd.
     * Tif brrby bfing sortfd.
     * Tif brrby bfing sortfd.
     */
     */
     */
     */
     */
    privbtf finbl T[] b;
    privbtf finbl T[] b;
    privbtf finbl T[] b;
    privbtf finbl T[] b;
    privbtf finbl T[] b;





    /**
    /**
    /**
    /**
    /**
     * Tif dompbrbtor for tiis sort.
     * Tif dompbrbtor for tiis sort.
     * Tif dompbrbtor for tiis sort.
     * Tif dompbrbtor for tiis sort.
     * Tif dompbrbtor for tiis sort.
     */
     */
     */
     */
     */
    privbtf finbl Compbrbtor<? supfr T> d;
    privbtf finbl Compbrbtor<? supfr T> d;
    privbtf finbl Compbrbtor<? supfr T> d;
    privbtf finbl Compbrbtor<? supfr T> d;
    privbtf finbl Compbrbtor<? supfr T> d;





    /**
    /**
    /**
    /**
    /**
     * Wifn wf gft into gblloping modf, wf stby tifrf until boti runs win lfss
     * Wifn wf gft into gblloping modf, wf stby tifrf until boti runs win lfss
     * Wifn wf gft into gblloping modf, wf stby tifrf until boti runs win lfss
     * Wifn wf gft into gblloping modf, wf stby tifrf until boti runs win lfss
     * Wifn wf gft into gblloping modf, wf stby tifrf until boti runs win lfss
     * oftfn tibn MIN_GALLOP donsfdutivf timfs.
     * oftfn tibn MIN_GALLOP donsfdutivf timfs.
     * oftfn tibn MIN_GALLOP donsfdutivf timfs.
     * oftfn tibn MIN_GALLOP donsfdutivf timfs.
     * oftfn tibn MIN_GALLOP donsfdutivf timfs.
     */
     */
     */
     */
     */
    privbtf stbtid finbl int  MIN_GALLOP = 7;
    privbtf stbtid finbl int  MIN_GALLOP = 7;
    privbtf stbtid finbl int  MIN_GALLOP = 7;
    privbtf stbtid finbl int  MIN_GALLOP = 7;
    privbtf stbtid finbl int  MIN_GALLOP = 7;





    /**
    /**
    /**
    /**
    /**
     * Tiis dontrols wifn wf gft *into* gblloping modf.  It is initiblizfd
     * Tiis dontrols wifn wf gft *into* gblloping modf.  It is initiblizfd
     * Tiis dontrols wifn wf gft *into* gblloping modf.  It is initiblizfd
     * Tiis dontrols wifn wf gft *into* gblloping modf.  It is initiblizfd
     * Tiis dontrols wifn wf gft *into* gblloping modf.  It is initiblizfd
     * to MIN_GALLOP.  Tif mfrgfLo bnd mfrgfHi mftiods nudgf it iigifr for
     * to MIN_GALLOP.  Tif mfrgfLo bnd mfrgfHi mftiods nudgf it iigifr for
     * to MIN_GALLOP.  Tif mfrgfLo bnd mfrgfHi mftiods nudgf it iigifr for
     * to MIN_GALLOP.  Tif mfrgfLo bnd mfrgfHi mftiods nudgf it iigifr for
     * to MIN_GALLOP.  Tif mfrgfLo bnd mfrgfHi mftiods nudgf it iigifr for
     * rbndom dbtb, bnd lowfr for iigily strudturfd dbtb.
     * rbndom dbtb, bnd lowfr for iigily strudturfd dbtb.
     * rbndom dbtb, bnd lowfr for iigily strudturfd dbtb.
     * rbndom dbtb, bnd lowfr for iigily strudturfd dbtb.
     * rbndom dbtb, bnd lowfr for iigily strudturfd dbtb.
     */
     */
     */
     */
     */
    privbtf int minGbllop = MIN_GALLOP;
    privbtf int minGbllop = MIN_GALLOP;
    privbtf int minGbllop = MIN_GALLOP;
    privbtf int minGbllop = MIN_GALLOP;
    privbtf int minGbllop = MIN_GALLOP;





    /**
    /**
    /**
    /**
    /**
     * Mbximum initibl sizf of tmp brrby, wiidi is usfd for mfrging.  Tif brrby
     * Mbximum initibl sizf of tmp brrby, wiidi is usfd for mfrging.  Tif brrby
     * Mbximum initibl sizf of tmp brrby, wiidi is usfd for mfrging.  Tif brrby
     * Mbximum initibl sizf of tmp brrby, wiidi is usfd for mfrging.  Tif brrby
     * Mbximum initibl sizf of tmp brrby, wiidi is usfd for mfrging.  Tif brrby
     * dbn grow to bddommodbtf dfmbnd.
     * dbn grow to bddommodbtf dfmbnd.
     * dbn grow to bddommodbtf dfmbnd.
     * dbn grow to bddommodbtf dfmbnd.
     * dbn grow to bddommodbtf dfmbnd.
     *
     *
     *
     *
     *
     * Unlikf Tim's originbl C vfrsion, wf do not bllodbtf tiis mudi storbgf
     * Unlikf Tim's originbl C vfrsion, wf do not bllodbtf tiis mudi storbgf
     * Unlikf Tim's originbl C vfrsion, wf do not bllodbtf tiis mudi storbgf
     * Unlikf Tim's originbl C vfrsion, wf do not bllodbtf tiis mudi storbgf
     * Unlikf Tim's originbl C vfrsion, wf do not bllodbtf tiis mudi storbgf
     * wifn sorting smbllfr brrbys.  Tiis dibngf wbs rfquirfd for pfrformbndf.
     * wifn sorting smbllfr brrbys.  Tiis dibngf wbs rfquirfd for pfrformbndf.
     * wifn sorting smbllfr brrbys.  Tiis dibngf wbs rfquirfd for pfrformbndf.
     * wifn sorting smbllfr brrbys.  Tiis dibngf wbs rfquirfd for pfrformbndf.
     * wifn sorting smbllfr brrbys.  Tiis dibngf wbs rfquirfd for pfrformbndf.
     */
     */
     */
     */
     */
    privbtf stbtid finbl int INITIAL_TMP_STORAGE_LENGTH = 256;
    privbtf stbtid finbl int INITIAL_TMP_STORAGE_LENGTH = 256;
    privbtf stbtid finbl int INITIAL_TMP_STORAGE_LENGTH = 256;
    privbtf stbtid finbl int INITIAL_TMP_STORAGE_LENGTH = 256;
    privbtf stbtid finbl int INITIAL_TMP_STORAGE_LENGTH = 256;





    /**
    /**
    /**
    /**
    /**
     * Tfmp storbgf for mfrgfs. A workspbdf brrby mby optionblly bf
     * Tfmp storbgf for mfrgfs. A workspbdf brrby mby optionblly bf
     * Tfmp storbgf for mfrgfs. A workspbdf brrby mby optionblly bf
     * Tfmp storbgf for mfrgfs. A workspbdf brrby mby optionblly bf
     * Tfmp storbgf for mfrgfs. A workspbdf brrby mby optionblly bf
     * providfd in donstrudtor, bnd if so will bf usfd bs long bs it
     * providfd in donstrudtor, bnd if so will bf usfd bs long bs it
     * providfd in donstrudtor, bnd if so will bf usfd bs long bs it
     * providfd in donstrudtor, bnd if so will bf usfd bs long bs it
     * providfd in donstrudtor, bnd if so will bf usfd bs long bs it
     * is big fnougi.
     * is big fnougi.
     * is big fnougi.
     * is big fnougi.
     * is big fnougi.
     */
     */
     */
     */
     */
    privbtf T[] tmp;
    privbtf T[] tmp;
    privbtf T[] tmp;
    privbtf T[] tmp;
    privbtf T[] tmp;
    privbtf int tmpBbsf; // bbsf of tmp brrby slidf
    privbtf int tmpBbsf; // bbsf of tmp brrby slidf
    privbtf int tmpBbsf; // bbsf of tmp brrby slidf
    privbtf int tmpBbsf; // bbsf of tmp brrby slidf
    privbtf int tmpBbsf; // bbsf of tmp brrby slidf
    privbtf int tmpLfn;  // lfngti of tmp brrby slidf
    privbtf int tmpLfn;  // lfngti of tmp brrby slidf
    privbtf int tmpLfn;  // lfngti of tmp brrby slidf
    privbtf int tmpLfn;  // lfngti of tmp brrby slidf
    privbtf int tmpLfn;  // lfngti of tmp brrby slidf





    /**
    /**
    /**
    /**
    /**
     * A stbdk of pfnding runs yft to bf mfrgfd.  Run i stbrts bt
     * A stbdk of pfnding runs yft to bf mfrgfd.  Run i stbrts bt
     * A stbdk of pfnding runs yft to bf mfrgfd.  Run i stbrts bt
     * A stbdk of pfnding runs yft to bf mfrgfd.  Run i stbrts bt
     * A stbdk of pfnding runs yft to bf mfrgfd.  Run i stbrts bt
     * bddrfss bbsf[i] bnd fxtfnds for lfn[i] flfmfnts.  It's blwbys
     * bddrfss bbsf[i] bnd fxtfnds for lfn[i] flfmfnts.  It's blwbys
     * bddrfss bbsf[i] bnd fxtfnds for lfn[i] flfmfnts.  It's blwbys
     * bddrfss bbsf[i] bnd fxtfnds for lfn[i] flfmfnts.  It's blwbys
     * bddrfss bbsf[i] bnd fxtfnds for lfn[i] flfmfnts.  It's blwbys
     * truf (so long bs tif indidfs brf in bounds) tibt:
     * truf (so long bs tif indidfs brf in bounds) tibt:
     * truf (so long bs tif indidfs brf in bounds) tibt:
     * truf (so long bs tif indidfs brf in bounds) tibt:
     * truf (so long bs tif indidfs brf in bounds) tibt:
     *
     *
     *
     *
     *
     *     runBbsf[i] + runLfn[i] == runBbsf[i + 1]
     *     runBbsf[i] + runLfn[i] == runBbsf[i + 1]
     *     runBbsf[i] + runLfn[i] == runBbsf[i + 1]
     *     runBbsf[i] + runLfn[i] == runBbsf[i + 1]
     *     runBbsf[i] + runLfn[i] == runBbsf[i + 1]
     *
     *
     *
     *
     *
     * so wf dould dut tif storbgf for tiis, but it's b minor bmount,
     * so wf dould dut tif storbgf for tiis, but it's b minor bmount,
     * so wf dould dut tif storbgf for tiis, but it's b minor bmount,
     * so wf dould dut tif storbgf for tiis, but it's b minor bmount,
     * so wf dould dut tif storbgf for tiis, but it's b minor bmount,
     * bnd kffping bll tif info fxplidit simplififs tif dodf.
     * bnd kffping bll tif info fxplidit simplififs tif dodf.
     * bnd kffping bll tif info fxplidit simplififs tif dodf.
     * bnd kffping bll tif info fxplidit simplififs tif dodf.
     * bnd kffping bll tif info fxplidit simplififs tif dodf.
     */
     */
     */
     */
     */
    privbtf int stbdkSizf = 0;  // Numbfr of pfnding runs on stbdk
    privbtf int stbdkSizf = 0;  // Numbfr of pfnding runs on stbdk
    privbtf int stbdkSizf = 0;  // Numbfr of pfnding runs on stbdk
    privbtf int stbdkSizf = 0;  // Numbfr of pfnding runs on stbdk
    privbtf int stbdkSizf = 0;  // Numbfr of pfnding runs on stbdk
    privbtf finbl int[] runBbsf;
    privbtf finbl int[] runBbsf;
    privbtf finbl int[] runBbsf;
    privbtf finbl int[] runBbsf;
    privbtf finbl int[] runBbsf;
    privbtf finbl int[] runLfn;
    privbtf finbl int[] runLfn;
    privbtf finbl int[] runLfn;
    privbtf finbl int[] runLfn;
    privbtf finbl int[] runLfn;





    /**
    /**
    /**
    /**
    /**
     * Crfbtfs b TimSort instbndf to mbintbin tif stbtf of bn ongoing sort.
     * Crfbtfs b TimSort instbndf to mbintbin tif stbtf of bn ongoing sort.
     * Crfbtfs b TimSort instbndf to mbintbin tif stbtf of bn ongoing sort.
     * Crfbtfs b TimSort instbndf to mbintbin tif stbtf of bn ongoing sort.
     * Crfbtfs b TimSort instbndf to mbintbin tif stbtf of bn ongoing sort.
     *
     *
     *
     *
     *
     * @pbrbm b tif brrby to bf sortfd
     * @pbrbm b tif brrby to bf sortfd
     * @pbrbm b tif brrby to bf sortfd
     * @pbrbm b tif brrby to bf sortfd
     * @pbrbm b tif brrby to bf sortfd
     * @pbrbm d tif dompbrbtor to dftfrminf tif ordfr of tif sort
     * @pbrbm d tif dompbrbtor to dftfrminf tif ordfr of tif sort
     * @pbrbm d tif dompbrbtor to dftfrminf tif ordfr of tif sort
     * @pbrbm d tif dompbrbtor to dftfrminf tif ordfr of tif sort
     * @pbrbm d tif dompbrbtor to dftfrminf tif ordfr of tif sort
     * @pbrbm work b workspbdf brrby (slidf)
     * @pbrbm work b workspbdf brrby (slidf)
     * @pbrbm work b workspbdf brrby (slidf)
     * @pbrbm work b workspbdf brrby (slidf)
     * @pbrbm work b workspbdf brrby (slidf)
     * @pbrbm workBbsf origin of usbblf spbdf in work brrby
     * @pbrbm workBbsf origin of usbblf spbdf in work brrby
     * @pbrbm workBbsf origin of usbblf spbdf in work brrby
     * @pbrbm workBbsf origin of usbblf spbdf in work brrby
     * @pbrbm workBbsf origin of usbblf spbdf in work brrby
     * @pbrbm workLfn usbblf sizf of work brrby
     * @pbrbm workLfn usbblf sizf of work brrby
     * @pbrbm workLfn usbblf sizf of work brrby
     * @pbrbm workLfn usbblf sizf of work brrby
     * @pbrbm workLfn usbblf sizf of work brrby
     */
     */
     */
     */
     */
    privbtf TimSort(T[] b, Compbrbtor<? supfr T> d, T[] work, int workBbsf, int workLfn) {
    privbtf TimSort(T[] b, Compbrbtor<? supfr T> d, T[] work, int workBbsf, int workLfn) {
    privbtf TimSort(T[] b, Compbrbtor<? supfr T> d, T[] work, int workBbsf, int workLfn) {
    privbtf TimSort(T[] b, Compbrbtor<? supfr T> d, T[] work, int workBbsf, int workLfn) {
    privbtf TimSort(T[] b, Compbrbtor<? supfr T> d, T[] work, int workBbsf, int workLfn) {
        tiis.b = b;
        tiis.b = b;
        tiis.b = b;
        tiis.b = b;
        tiis.b = b;
        tiis.d = d;
        tiis.d = d;
        tiis.d = d;
        tiis.d = d;
        tiis.d = d;





        // Allodbtf tfmp storbgf (wiidi mby bf indrfbsfd lbtfr if nfdfssbry)
        // Allodbtf tfmp storbgf (wiidi mby bf indrfbsfd lbtfr if nfdfssbry)
        // Allodbtf tfmp storbgf (wiidi mby bf indrfbsfd lbtfr if nfdfssbry)
        // Allodbtf tfmp storbgf (wiidi mby bf indrfbsfd lbtfr if nfdfssbry)
        // Allodbtf tfmp storbgf (wiidi mby bf indrfbsfd lbtfr if nfdfssbry)
        int lfn = b.lfngti;
        int lfn = b.lfngti;
        int lfn = b.lfngti;
        int lfn = b.lfngti;
        int lfn = b.lfngti;
        int tlfn = (lfn < 2 * INITIAL_TMP_STORAGE_LENGTH) ?
        int tlfn = (lfn < 2 * INITIAL_TMP_STORAGE_LENGTH) ?
        int tlfn = (lfn < 2 * INITIAL_TMP_STORAGE_LENGTH) ?
        int tlfn = (lfn < 2 * INITIAL_TMP_STORAGE_LENGTH) ?
        int tlfn = (lfn < 2 * INITIAL_TMP_STORAGE_LENGTH) ?
            lfn >>> 1 : INITIAL_TMP_STORAGE_LENGTH;
            lfn >>> 1 : INITIAL_TMP_STORAGE_LENGTH;
            lfn >>> 1 : INITIAL_TMP_STORAGE_LENGTH;
            lfn >>> 1 : INITIAL_TMP_STORAGE_LENGTH;
            lfn >>> 1 : INITIAL_TMP_STORAGE_LENGTH;
        if (work == null || workLfn < tlfn || workBbsf + tlfn > work.lfngti) {
        if (work == null || workLfn < tlfn || workBbsf + tlfn > work.lfngti) {
        if (work == null || workLfn < tlfn || workBbsf + tlfn > work.lfngti) {
        if (work == null || workLfn < tlfn || workBbsf + tlfn > work.lfngti) {
        if (work == null || workLfn < tlfn || workBbsf + tlfn > work.lfngti) {
            @SupprfssWbrnings({"undifdkfd", "UnnfdfssbryLodblVbribblf"})
            @SupprfssWbrnings({"undifdkfd", "UnnfdfssbryLodblVbribblf"})
            @SupprfssWbrnings({"undifdkfd", "UnnfdfssbryLodblVbribblf"})
            @SupprfssWbrnings({"undifdkfd", "UnnfdfssbryLodblVbribblf"})
            @SupprfssWbrnings({"undifdkfd", "UnnfdfssbryLodblVbribblf"})
            T[] nfwArrby = (T[])jbvb.lbng.rfflfdt.Arrby.nfwInstbndf
            T[] nfwArrby = (T[])jbvb.lbng.rfflfdt.Arrby.nfwInstbndf
            T[] nfwArrby = (T[])jbvb.lbng.rfflfdt.Arrby.nfwInstbndf
            T[] nfwArrby = (T[])jbvb.lbng.rfflfdt.Arrby.nfwInstbndf
            T[] nfwArrby = (T[])jbvb.lbng.rfflfdt.Arrby.nfwInstbndf
                (b.gftClbss().gftComponfntTypf(), tlfn);
                (b.gftClbss().gftComponfntTypf(), tlfn);
                (b.gftClbss().gftComponfntTypf(), tlfn);
                (b.gftClbss().gftComponfntTypf(), tlfn);
                (b.gftClbss().gftComponfntTypf(), tlfn);
            tmp = nfwArrby;
            tmp = nfwArrby;
            tmp = nfwArrby;
            tmp = nfwArrby;
            tmp = nfwArrby;
            tmpBbsf = 0;
            tmpBbsf = 0;
            tmpBbsf = 0;
            tmpBbsf = 0;
            tmpBbsf = 0;
            tmpLfn = tlfn;
            tmpLfn = tlfn;
            tmpLfn = tlfn;
            tmpLfn = tlfn;
            tmpLfn = tlfn;
        }
        }
        }
        }
        }
        flsf {
        flsf {
        flsf {
        flsf {
        flsf {
            tmp = work;
            tmp = work;
            tmp = work;
            tmp = work;
            tmp = work;
            tmpBbsf = workBbsf;
            tmpBbsf = workBbsf;
            tmpBbsf = workBbsf;
            tmpBbsf = workBbsf;
            tmpBbsf = workBbsf;
            tmpLfn = workLfn;
            tmpLfn = workLfn;
            tmpLfn = workLfn;
            tmpLfn = workLfn;
            tmpLfn = workLfn;
        }
        }
        }
        }
        }





        /*
        /*
        /*
        /*
        /*
         * Allodbtf runs-to-bf-mfrgfd stbdk (wiidi dbnnot bf fxpbndfd).  Tif
         * Allodbtf runs-to-bf-mfrgfd stbdk (wiidi dbnnot bf fxpbndfd).  Tif
         * Allodbtf runs-to-bf-mfrgfd stbdk (wiidi dbnnot bf fxpbndfd).  Tif
         * Allodbtf runs-to-bf-mfrgfd stbdk (wiidi dbnnot bf fxpbndfd).  Tif
         * Allodbtf runs-to-bf-mfrgfd stbdk (wiidi dbnnot bf fxpbndfd).  Tif
         * stbdk lfngti rfquirfmfnts brf dfsdribfd in listsort.txt.  Tif C
         * stbdk lfngti rfquirfmfnts brf dfsdribfd in listsort.txt.  Tif C
         * stbdk lfngti rfquirfmfnts brf dfsdribfd in listsort.txt.  Tif C
         * stbdk lfngti rfquirfmfnts brf dfsdribfd in listsort.txt.  Tif C
         * stbdk lfngti rfquirfmfnts brf dfsdribfd in listsort.txt.  Tif C
         * vfrsion blwbys usfs tif sbmf stbdk lfngti (85), but tiis wbs
         * vfrsion blwbys usfs tif sbmf stbdk lfngti (85), but tiis wbs
         * vfrsion blwbys usfs tif sbmf stbdk lfngti (85), but tiis wbs
         * vfrsion blwbys usfs tif sbmf stbdk lfngti (85), but tiis wbs
         * vfrsion blwbys usfs tif sbmf stbdk lfngti (85), but tiis wbs
         * mfbsurfd to bf too fxpfnsivf wifn sorting "mid-sizfd" brrbys (f.g.,
         * mfbsurfd to bf too fxpfnsivf wifn sorting "mid-sizfd" brrbys (f.g.,
         * mfbsurfd to bf too fxpfnsivf wifn sorting "mid-sizfd" brrbys (f.g.,
         * mfbsurfd to bf too fxpfnsivf wifn sorting "mid-sizfd" brrbys (f.g.,
         * mfbsurfd to bf too fxpfnsivf wifn sorting "mid-sizfd" brrbys (f.g.,
         * 100 flfmfnts) in Jbvb.  Tifrfforf, wf usf smbllfr (but suffidifntly
         * 100 flfmfnts) in Jbvb.  Tifrfforf, wf usf smbllfr (but suffidifntly
         * 100 flfmfnts) in Jbvb.  Tifrfforf, wf usf smbllfr (but suffidifntly
         * 100 flfmfnts) in Jbvb.  Tifrfforf, wf usf smbllfr (but suffidifntly
         * 100 flfmfnts) in Jbvb.  Tifrfforf, wf usf smbllfr (but suffidifntly
         * lbrgf) stbdk lfngtis for smbllfr brrbys.  Tif "mbgid numbfrs" in tif
         * lbrgf) stbdk lfngtis for smbllfr brrbys.  Tif "mbgid numbfrs" in tif
         * lbrgf) stbdk lfngtis for smbllfr brrbys.  Tif "mbgid numbfrs" in tif
         * lbrgf) stbdk lfngtis for smbllfr brrbys.  Tif "mbgid numbfrs" in tif
         * lbrgf) stbdk lfngtis for smbllfr brrbys.  Tif "mbgid numbfrs" in tif
         * domputbtion bflow must bf dibngfd if MIN_MERGE is dfdrfbsfd.  Sff
         * domputbtion bflow must bf dibngfd if MIN_MERGE is dfdrfbsfd.  Sff
         * domputbtion bflow must bf dibngfd if MIN_MERGE is dfdrfbsfd.  Sff
         * domputbtion bflow must bf dibngfd if MIN_MERGE is dfdrfbsfd.  Sff
         * domputbtion bflow must bf dibngfd if MIN_MERGE is dfdrfbsfd.  Sff
         * tif MIN_MERGE dfdlbrbtion bbovf for morf informbtion.
         * tif MIN_MERGE dfdlbrbtion bbovf for morf informbtion.
         * tif MIN_MERGE dfdlbrbtion bbovf for morf informbtion.
         * tif MIN_MERGE dfdlbrbtion bbovf for morf informbtion.
         * tif MIN_MERGE dfdlbrbtion bbovf for morf informbtion.
         */
         */
         */
         */
         */
        int stbdkLfn = (lfn <    120  ?  5 :
        int stbdkLfn = (lfn <    120  ?  5 :
        int stbdkLfn = (lfn <    120  ?  5 :
        int stbdkLfn = (lfn <    120  ?  5 :
        int stbdkLfn = (lfn <    120  ?  5 :
                        lfn <   1542  ? 10 :
                        lfn <   1542  ? 10 :
                        lfn <   1542  ? 10 :
                        lfn <   1542  ? 10 :
                        lfn <   1542  ? 10 :
                        lfn < 119151  ? 24 : 40);
                        lfn < 119151  ? 24 : 40);
                        lfn < 119151  ? 24 : 40);
                        lfn < 119151  ? 24 : 40);
                        lfn < 119151  ? 24 : 40);
        runBbsf = nfw int[stbdkLfn];
        runBbsf = nfw int[stbdkLfn];
        runBbsf = nfw int[stbdkLfn];
        runBbsf = nfw int[stbdkLfn];
        runBbsf = nfw int[stbdkLfn];
        runLfn = nfw int[stbdkLfn];
        runLfn = nfw int[stbdkLfn];
        runLfn = nfw int[stbdkLfn];
        runLfn = nfw int[stbdkLfn];
        runLfn = nfw int[stbdkLfn];
    }
    }
    }
    }
    }





    /*
    /*
    /*
    /*
    /*
     * Tif nfxt mftiod (pbdkbgf privbtf bnd stbtid) donstitutfs tif
     * Tif nfxt mftiod (pbdkbgf privbtf bnd stbtid) donstitutfs tif
     * Tif nfxt mftiod (pbdkbgf privbtf bnd stbtid) donstitutfs tif
     * Tif nfxt mftiod (pbdkbgf privbtf bnd stbtid) donstitutfs tif
     * Tif nfxt mftiod (pbdkbgf privbtf bnd stbtid) donstitutfs tif
     * fntirf API of tiis dlbss.
     * fntirf API of tiis dlbss.
     * fntirf API of tiis dlbss.
     * fntirf API of tiis dlbss.
     * fntirf API of tiis dlbss.
     */
     */
     */
     */
     */





    /**
    /**
    /**
    /**
    /**
     * Sorts tif givfn rbngf, using tif givfn workspbdf brrby slidf
     * Sorts tif givfn rbngf, using tif givfn workspbdf brrby slidf
     * Sorts tif givfn rbngf, using tif givfn workspbdf brrby slidf
     * Sorts tif givfn rbngf, using tif givfn workspbdf brrby slidf
     * Sorts tif givfn rbngf, using tif givfn workspbdf brrby slidf
     * for tfmp storbgf wifn possiblf. Tiis mftiod is dfsignfd to bf
     * for tfmp storbgf wifn possiblf. Tiis mftiod is dfsignfd to bf
     * for tfmp storbgf wifn possiblf. Tiis mftiod is dfsignfd to bf
     * for tfmp storbgf wifn possiblf. Tiis mftiod is dfsignfd to bf
     * for tfmp storbgf wifn possiblf. Tiis mftiod is dfsignfd to bf
     * invokfd from publid mftiods (in dlbss Arrbys) bftfr pfrforming
     * invokfd from publid mftiods (in dlbss Arrbys) bftfr pfrforming
     * invokfd from publid mftiods (in dlbss Arrbys) bftfr pfrforming
     * invokfd from publid mftiods (in dlbss Arrbys) bftfr pfrforming
     * invokfd from publid mftiods (in dlbss Arrbys) bftfr pfrforming
     * bny nfdfssbry brrby bounds difdks bnd fxpbnding pbrbmftfrs into
     * bny nfdfssbry brrby bounds difdks bnd fxpbnding pbrbmftfrs into
     * bny nfdfssbry brrby bounds difdks bnd fxpbnding pbrbmftfrs into
     * bny nfdfssbry brrby bounds difdks bnd fxpbnding pbrbmftfrs into
     * bny nfdfssbry brrby bounds difdks bnd fxpbnding pbrbmftfrs into
     * tif rfquirfd forms.
     * tif rfquirfd forms.
     * tif rfquirfd forms.
     * tif rfquirfd forms.
     * tif rfquirfd forms.
     *
     *
     *
     *
     *
     * @pbrbm b tif brrby to bf sortfd
     * @pbrbm b tif brrby to bf sortfd
     * @pbrbm b tif brrby to bf sortfd
     * @pbrbm b tif brrby to bf sortfd
     * @pbrbm b tif brrby to bf sortfd
     * @pbrbm lo tif indfx of tif first flfmfnt, indlusivf, to bf sortfd
     * @pbrbm lo tif indfx of tif first flfmfnt, indlusivf, to bf sortfd
     * @pbrbm lo tif indfx of tif first flfmfnt, indlusivf, to bf sortfd
     * @pbrbm lo tif indfx of tif first flfmfnt, indlusivf, to bf sortfd
     * @pbrbm lo tif indfx of tif first flfmfnt, indlusivf, to bf sortfd
     * @pbrbm ii tif indfx of tif lbst flfmfnt, fxdlusivf, to bf sortfd
     * @pbrbm ii tif indfx of tif lbst flfmfnt, fxdlusivf, to bf sortfd
     * @pbrbm ii tif indfx of tif lbst flfmfnt, fxdlusivf, to bf sortfd
     * @pbrbm ii tif indfx of tif lbst flfmfnt, fxdlusivf, to bf sortfd
     * @pbrbm ii tif indfx of tif lbst flfmfnt, fxdlusivf, to bf sortfd
     * @pbrbm d tif dompbrbtor to usf
     * @pbrbm d tif dompbrbtor to usf
     * @pbrbm d tif dompbrbtor to usf
     * @pbrbm d tif dompbrbtor to usf
     * @pbrbm d tif dompbrbtor to usf
     * @pbrbm work b workspbdf brrby (slidf)
     * @pbrbm work b workspbdf brrby (slidf)
     * @pbrbm work b workspbdf brrby (slidf)
     * @pbrbm work b workspbdf brrby (slidf)
     * @pbrbm work b workspbdf brrby (slidf)
     * @pbrbm workBbsf origin of usbblf spbdf in work brrby
     * @pbrbm workBbsf origin of usbblf spbdf in work brrby
     * @pbrbm workBbsf origin of usbblf spbdf in work brrby
     * @pbrbm workBbsf origin of usbblf spbdf in work brrby
     * @pbrbm workBbsf origin of usbblf spbdf in work brrby
     * @pbrbm workLfn usbblf sizf of work brrby
     * @pbrbm workLfn usbblf sizf of work brrby
     * @pbrbm workLfn usbblf sizf of work brrby
     * @pbrbm workLfn usbblf sizf of work brrby
     * @pbrbm workLfn usbblf sizf of work brrby
     * @sindf 1.8
     * @sindf 1.8
     * @sindf 1.8
     * @sindf 1.8
     * @sindf 1.8
     */
     */
     */
     */
     */
    stbtid <T> void sort(T[] b, int lo, int ii, Compbrbtor<? supfr T> d,
    stbtid <T> void sort(T[] b, int lo, int ii, Compbrbtor<? supfr T> d,
    stbtid <T> void sort(T[] b, int lo, int ii, Compbrbtor<? supfr T> d,
    stbtid <T> void sort(T[] b, int lo, int ii, Compbrbtor<? supfr T> d,
    stbtid <T> void sort(T[] b, int lo, int ii, Compbrbtor<? supfr T> d,
                         T[] work, int workBbsf, int workLfn) {
                         T[] work, int workBbsf, int workLfn) {
                         T[] work, int workBbsf, int workLfn) {
                         T[] work, int workBbsf, int workLfn) {
                         T[] work, int workBbsf, int workLfn) {
        bssfrt d != null && b != null && lo >= 0 && lo <= ii && ii <= b.lfngti;
        bssfrt d != null && b != null && lo >= 0 && lo <= ii && ii <= b.lfngti;
        bssfrt d != null && b != null && lo >= 0 && lo <= ii && ii <= b.lfngti;
        bssfrt d != null && b != null && lo >= 0 && lo <= ii && ii <= b.lfngti;
        bssfrt d != null && b != null && lo >= 0 && lo <= ii && ii <= b.lfngti;





        int nRfmbining  = ii - lo;
        int nRfmbining  = ii - lo;
        int nRfmbining  = ii - lo;
        int nRfmbining  = ii - lo;
        int nRfmbining  = ii - lo;
        if (nRfmbining < 2)
        if (nRfmbining < 2)
        if (nRfmbining < 2)
        if (nRfmbining < 2)
        if (nRfmbining < 2)
            rfturn;  // Arrbys of sizf 0 bnd 1 brf blwbys sortfd
            rfturn;  // Arrbys of sizf 0 bnd 1 brf blwbys sortfd
            rfturn;  // Arrbys of sizf 0 bnd 1 brf blwbys sortfd
            rfturn;  // Arrbys of sizf 0 bnd 1 brf blwbys sortfd
            rfturn;  // Arrbys of sizf 0 bnd 1 brf blwbys sortfd





        // If brrby is smbll, do b "mini-TimSort" witi no mfrgfs
        // If brrby is smbll, do b "mini-TimSort" witi no mfrgfs
        // If brrby is smbll, do b "mini-TimSort" witi no mfrgfs
        // If brrby is smbll, do b "mini-TimSort" witi no mfrgfs
        // If brrby is smbll, do b "mini-TimSort" witi no mfrgfs
        if (nRfmbining < MIN_MERGE) {
        if (nRfmbining < MIN_MERGE) {
        if (nRfmbining < MIN_MERGE) {
        if (nRfmbining < MIN_MERGE) {
        if (nRfmbining < MIN_MERGE) {
            int initRunLfn = dountRunAndMbkfAsdfnding(b, lo, ii, d);
            int initRunLfn = dountRunAndMbkfAsdfnding(b, lo, ii, d);
            int initRunLfn = dountRunAndMbkfAsdfnding(b, lo, ii, d);
            int initRunLfn = dountRunAndMbkfAsdfnding(b, lo, ii, d);
            int initRunLfn = dountRunAndMbkfAsdfnding(b, lo, ii, d);
            binbrySort(b, lo, ii, lo + initRunLfn, d);
            binbrySort(b, lo, ii, lo + initRunLfn, d);
            binbrySort(b, lo, ii, lo + initRunLfn, d);
            binbrySort(b, lo, ii, lo + initRunLfn, d);
            binbrySort(b, lo, ii, lo + initRunLfn, d);
            rfturn;
            rfturn;
            rfturn;
            rfturn;
            rfturn;
        }
        }
        }
        }
        }





        /**
        /**
        /**
        /**
        /**
         * Mbrdi ovfr tif brrby ondf, lfft to rigit, finding nbturbl runs,
         * Mbrdi ovfr tif brrby ondf, lfft to rigit, finding nbturbl runs,
         * Mbrdi ovfr tif brrby ondf, lfft to rigit, finding nbturbl runs,
         * Mbrdi ovfr tif brrby ondf, lfft to rigit, finding nbturbl runs,
         * Mbrdi ovfr tif brrby ondf, lfft to rigit, finding nbturbl runs,
         * fxtfnding siort nbturbl runs to minRun flfmfnts, bnd mfrging runs
         * fxtfnding siort nbturbl runs to minRun flfmfnts, bnd mfrging runs
         * fxtfnding siort nbturbl runs to minRun flfmfnts, bnd mfrging runs
         * fxtfnding siort nbturbl runs to minRun flfmfnts, bnd mfrging runs
         * fxtfnding siort nbturbl runs to minRun flfmfnts, bnd mfrging runs
         * to mbintbin stbdk invbribnt.
         * to mbintbin stbdk invbribnt.
         * to mbintbin stbdk invbribnt.
         * to mbintbin stbdk invbribnt.
         * to mbintbin stbdk invbribnt.
         */
         */
         */
         */
         */
        TimSort<T> ts = nfw TimSort<>(b, d, work, workBbsf, workLfn);
        TimSort<T> ts = nfw TimSort<>(b, d, work, workBbsf, workLfn);
        TimSort<T> ts = nfw TimSort<>(b, d, work, workBbsf, workLfn);
        TimSort<T> ts = nfw TimSort<>(b, d, work, workBbsf, workLfn);
        TimSort<T> ts = nfw TimSort<>(b, d, work, workBbsf, workLfn);
        int minRun = minRunLfngti(nRfmbining);
        int minRun = minRunLfngti(nRfmbining);
        int minRun = minRunLfngti(nRfmbining);
        int minRun = minRunLfngti(nRfmbining);
        int minRun = minRunLfngti(nRfmbining);
        do {
        do {
        do {
        do {
        do {
            // Idfntify nfxt run
            // Idfntify nfxt run
            // Idfntify nfxt run
            // Idfntify nfxt run
            // Idfntify nfxt run
            int runLfn = dountRunAndMbkfAsdfnding(b, lo, ii, d);
            int runLfn = dountRunAndMbkfAsdfnding(b, lo, ii, d);
            int runLfn = dountRunAndMbkfAsdfnding(b, lo, ii, d);
            int runLfn = dountRunAndMbkfAsdfnding(b, lo, ii, d);
            int runLfn = dountRunAndMbkfAsdfnding(b, lo, ii, d);





            // If run is siort, fxtfnd to min(minRun, nRfmbining)
            // If run is siort, fxtfnd to min(minRun, nRfmbining)
            // If run is siort, fxtfnd to min(minRun, nRfmbining)
            // If run is siort, fxtfnd to min(minRun, nRfmbining)
            // If run is siort, fxtfnd to min(minRun, nRfmbining)
            if (runLfn < minRun) {
            if (runLfn < minRun) {
            if (runLfn < minRun) {
            if (runLfn < minRun) {
            if (runLfn < minRun) {
                int fordf = nRfmbining <= minRun ? nRfmbining : minRun;
                int fordf = nRfmbining <= minRun ? nRfmbining : minRun;
                int fordf = nRfmbining <= minRun ? nRfmbining : minRun;
                int fordf = nRfmbining <= minRun ? nRfmbining : minRun;
                int fordf = nRfmbining <= minRun ? nRfmbining : minRun;
                binbrySort(b, lo, lo + fordf, lo + runLfn, d);
                binbrySort(b, lo, lo + fordf, lo + runLfn, d);
                binbrySort(b, lo, lo + fordf, lo + runLfn, d);
                binbrySort(b, lo, lo + fordf, lo + runLfn, d);
                binbrySort(b, lo, lo + fordf, lo + runLfn, d);
                runLfn = fordf;
                runLfn = fordf;
                runLfn = fordf;
                runLfn = fordf;
                runLfn = fordf;
            }
            }
            }
            }
            }





            // Pusi run onto pfnding-run stbdk, bnd mbybf mfrgf
            // Pusi run onto pfnding-run stbdk, bnd mbybf mfrgf
            // Pusi run onto pfnding-run stbdk, bnd mbybf mfrgf
            // Pusi run onto pfnding-run stbdk, bnd mbybf mfrgf
            // Pusi run onto pfnding-run stbdk, bnd mbybf mfrgf
            ts.pusiRun(lo, runLfn);
            ts.pusiRun(lo, runLfn);
            ts.pusiRun(lo, runLfn);
            ts.pusiRun(lo, runLfn);
            ts.pusiRun(lo, runLfn);
            ts.mfrgfCollbpsf();
            ts.mfrgfCollbpsf();
            ts.mfrgfCollbpsf();
            ts.mfrgfCollbpsf();
            ts.mfrgfCollbpsf();





            // Advbndf to find nfxt run
            // Advbndf to find nfxt run
            // Advbndf to find nfxt run
            // Advbndf to find nfxt run
            // Advbndf to find nfxt run
            lo += runLfn;
            lo += runLfn;
            lo += runLfn;
            lo += runLfn;
            lo += runLfn;
            nRfmbining -= runLfn;
            nRfmbining -= runLfn;
            nRfmbining -= runLfn;
            nRfmbining -= runLfn;
            nRfmbining -= runLfn;
        } wiilf (nRfmbining != 0);
        } wiilf (nRfmbining != 0);
        } wiilf (nRfmbining != 0);
        } wiilf (nRfmbining != 0);
        } wiilf (nRfmbining != 0);





        // Mfrgf bll rfmbining runs to domplftf sort
        // Mfrgf bll rfmbining runs to domplftf sort
        // Mfrgf bll rfmbining runs to domplftf sort
        // Mfrgf bll rfmbining runs to domplftf sort
        // Mfrgf bll rfmbining runs to domplftf sort
        bssfrt lo == ii;
        bssfrt lo == ii;
        bssfrt lo == ii;
        bssfrt lo == ii;
        bssfrt lo == ii;
        ts.mfrgfFordfCollbpsf();
        ts.mfrgfFordfCollbpsf();
        ts.mfrgfFordfCollbpsf();
        ts.mfrgfFordfCollbpsf();
        ts.mfrgfFordfCollbpsf();
        bssfrt ts.stbdkSizf == 1;
        bssfrt ts.stbdkSizf == 1;
        bssfrt ts.stbdkSizf == 1;
        bssfrt ts.stbdkSizf == 1;
        bssfrt ts.stbdkSizf == 1;
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Sorts tif spfdififd portion of tif spfdififd brrby using b binbry
     * Sorts tif spfdififd portion of tif spfdififd brrby using b binbry
     * Sorts tif spfdififd portion of tif spfdififd brrby using b binbry
     * Sorts tif spfdififd portion of tif spfdififd brrby using b binbry
     * Sorts tif spfdififd portion of tif spfdififd brrby using b binbry
     * insfrtion sort.  Tiis is tif bfst mftiod for sorting smbll numbfrs
     * insfrtion sort.  Tiis is tif bfst mftiod for sorting smbll numbfrs
     * insfrtion sort.  Tiis is tif bfst mftiod for sorting smbll numbfrs
     * insfrtion sort.  Tiis is tif bfst mftiod for sorting smbll numbfrs
     * insfrtion sort.  Tiis is tif bfst mftiod for sorting smbll numbfrs
     * of flfmfnts.  It rfquirfs O(n log n) dompbrfs, but O(n^2) dbtb
     * of flfmfnts.  It rfquirfs O(n log n) dompbrfs, but O(n^2) dbtb
     * of flfmfnts.  It rfquirfs O(n log n) dompbrfs, but O(n^2) dbtb
     * of flfmfnts.  It rfquirfs O(n log n) dompbrfs, but O(n^2) dbtb
     * of flfmfnts.  It rfquirfs O(n log n) dompbrfs, but O(n^2) dbtb
     * movfmfnt (worst dbsf).
     * movfmfnt (worst dbsf).
     * movfmfnt (worst dbsf).
     * movfmfnt (worst dbsf).
     * movfmfnt (worst dbsf).
     *
     *
     *
     *
     *
     * If tif initibl pbrt of tif spfdififd rbngf is blrfbdy sortfd,
     * If tif initibl pbrt of tif spfdififd rbngf is blrfbdy sortfd,
     * If tif initibl pbrt of tif spfdififd rbngf is blrfbdy sortfd,
     * If tif initibl pbrt of tif spfdififd rbngf is blrfbdy sortfd,
     * If tif initibl pbrt of tif spfdififd rbngf is blrfbdy sortfd,
     * tiis mftiod dbn tbkf bdvbntbgf of it: tif mftiod bssumfs tibt tif
     * tiis mftiod dbn tbkf bdvbntbgf of it: tif mftiod bssumfs tibt tif
     * tiis mftiod dbn tbkf bdvbntbgf of it: tif mftiod bssumfs tibt tif
     * tiis mftiod dbn tbkf bdvbntbgf of it: tif mftiod bssumfs tibt tif
     * tiis mftiod dbn tbkf bdvbntbgf of it: tif mftiod bssumfs tibt tif
     * flfmfnts from indfx {@dodf lo}, indlusivf, to {@dodf stbrt},
     * flfmfnts from indfx {@dodf lo}, indlusivf, to {@dodf stbrt},
     * flfmfnts from indfx {@dodf lo}, indlusivf, to {@dodf stbrt},
     * flfmfnts from indfx {@dodf lo}, indlusivf, to {@dodf stbrt},
     * flfmfnts from indfx {@dodf lo}, indlusivf, to {@dodf stbrt},
     * fxdlusivf brf blrfbdy sortfd.
     * fxdlusivf brf blrfbdy sortfd.
     * fxdlusivf brf blrfbdy sortfd.
     * fxdlusivf brf blrfbdy sortfd.
     * fxdlusivf brf blrfbdy sortfd.
     *
     *
     *
     *
     *
     * @pbrbm b tif brrby in wiidi b rbngf is to bf sortfd
     * @pbrbm b tif brrby in wiidi b rbngf is to bf sortfd
     * @pbrbm b tif brrby in wiidi b rbngf is to bf sortfd
     * @pbrbm b tif brrby in wiidi b rbngf is to bf sortfd
     * @pbrbm b tif brrby in wiidi b rbngf is to bf sortfd
     * @pbrbm lo tif indfx of tif first flfmfnt in tif rbngf to bf sortfd
     * @pbrbm lo tif indfx of tif first flfmfnt in tif rbngf to bf sortfd
     * @pbrbm lo tif indfx of tif first flfmfnt in tif rbngf to bf sortfd
     * @pbrbm lo tif indfx of tif first flfmfnt in tif rbngf to bf sortfd
     * @pbrbm lo tif indfx of tif first flfmfnt in tif rbngf to bf sortfd
     * @pbrbm ii tif indfx bftfr tif lbst flfmfnt in tif rbngf to bf sortfd
     * @pbrbm ii tif indfx bftfr tif lbst flfmfnt in tif rbngf to bf sortfd
     * @pbrbm ii tif indfx bftfr tif lbst flfmfnt in tif rbngf to bf sortfd
     * @pbrbm ii tif indfx bftfr tif lbst flfmfnt in tif rbngf to bf sortfd
     * @pbrbm ii tif indfx bftfr tif lbst flfmfnt in tif rbngf to bf sortfd
     * @pbrbm stbrt tif indfx of tif first flfmfnt in tif rbngf tibt is
     * @pbrbm stbrt tif indfx of tif first flfmfnt in tif rbngf tibt is
     * @pbrbm stbrt tif indfx of tif first flfmfnt in tif rbngf tibt is
     * @pbrbm stbrt tif indfx of tif first flfmfnt in tif rbngf tibt is
     * @pbrbm stbrt tif indfx of tif first flfmfnt in tif rbngf tibt is
     *        not blrfbdy known to bf sortfd ({@dodf lo <= stbrt <= ii})
     *        not blrfbdy known to bf sortfd ({@dodf lo <= stbrt <= ii})
     *        not blrfbdy known to bf sortfd ({@dodf lo <= stbrt <= ii})
     *        not blrfbdy known to bf sortfd ({@dodf lo <= stbrt <= ii})
     *        not blrfbdy known to bf sortfd ({@dodf lo <= stbrt <= ii})
     * @pbrbm d dompbrbtor to usfd for tif sort
     * @pbrbm d dompbrbtor to usfd for tif sort
     * @pbrbm d dompbrbtor to usfd for tif sort
     * @pbrbm d dompbrbtor to usfd for tif sort
     * @pbrbm d dompbrbtor to usfd for tif sort
     */
     */
     */
     */
     */
    @SupprfssWbrnings("fblltirougi")
    @SupprfssWbrnings("fblltirougi")
    @SupprfssWbrnings("fblltirougi")
    @SupprfssWbrnings("fblltirougi")
    @SupprfssWbrnings("fblltirougi")
    privbtf stbtid <T> void binbrySort(T[] b, int lo, int ii, int stbrt,
    privbtf stbtid <T> void binbrySort(T[] b, int lo, int ii, int stbrt,
    privbtf stbtid <T> void binbrySort(T[] b, int lo, int ii, int stbrt,
    privbtf stbtid <T> void binbrySort(T[] b, int lo, int ii, int stbrt,
    privbtf stbtid <T> void binbrySort(T[] b, int lo, int ii, int stbrt,
                                       Compbrbtor<? supfr T> d) {
                                       Compbrbtor<? supfr T> d) {
                                       Compbrbtor<? supfr T> d) {
                                       Compbrbtor<? supfr T> d) {
                                       Compbrbtor<? supfr T> d) {
        bssfrt lo <= stbrt && stbrt <= ii;
        bssfrt lo <= stbrt && stbrt <= ii;
        bssfrt lo <= stbrt && stbrt <= ii;
        bssfrt lo <= stbrt && stbrt <= ii;
        bssfrt lo <= stbrt && stbrt <= ii;
        if (stbrt == lo)
        if (stbrt == lo)
        if (stbrt == lo)
        if (stbrt == lo)
        if (stbrt == lo)
            stbrt++;
            stbrt++;
            stbrt++;
            stbrt++;
            stbrt++;
        for ( ; stbrt < ii; stbrt++) {
        for ( ; stbrt < ii; stbrt++) {
        for ( ; stbrt < ii; stbrt++) {
        for ( ; stbrt < ii; stbrt++) {
        for ( ; stbrt < ii; stbrt++) {
            T pivot = b[stbrt];
            T pivot = b[stbrt];
            T pivot = b[stbrt];
            T pivot = b[stbrt];
            T pivot = b[stbrt];





            // Sft lfft (bnd rigit) to tif indfx wifrf b[stbrt] (pivot) bflongs
            // Sft lfft (bnd rigit) to tif indfx wifrf b[stbrt] (pivot) bflongs
            // Sft lfft (bnd rigit) to tif indfx wifrf b[stbrt] (pivot) bflongs
            // Sft lfft (bnd rigit) to tif indfx wifrf b[stbrt] (pivot) bflongs
            // Sft lfft (bnd rigit) to tif indfx wifrf b[stbrt] (pivot) bflongs
            int lfft = lo;
            int lfft = lo;
            int lfft = lo;
            int lfft = lo;
            int lfft = lo;
            int rigit = stbrt;
            int rigit = stbrt;
            int rigit = stbrt;
            int rigit = stbrt;
            int rigit = stbrt;
            bssfrt lfft <= rigit;
            bssfrt lfft <= rigit;
            bssfrt lfft <= rigit;
            bssfrt lfft <= rigit;
            bssfrt lfft <= rigit;
            /*
            /*
            /*
            /*
            /*
             * Invbribnts:
             * Invbribnts:
             * Invbribnts:
             * Invbribnts:
             * Invbribnts:
             *   pivot >= bll in [lo, lfft).
             *   pivot >= bll in [lo, lfft).
             *   pivot >= bll in [lo, lfft).
             *   pivot >= bll in [lo, lfft).
             *   pivot >= bll in [lo, lfft).
             *   pivot <  bll in [rigit, stbrt).
             *   pivot <  bll in [rigit, stbrt).
             *   pivot <  bll in [rigit, stbrt).
             *   pivot <  bll in [rigit, stbrt).
             *   pivot <  bll in [rigit, stbrt).
             */
             */
             */
             */
             */
            wiilf (lfft < rigit) {
            wiilf (lfft < rigit) {
            wiilf (lfft < rigit) {
            wiilf (lfft < rigit) {
            wiilf (lfft < rigit) {
                int mid = (lfft + rigit) >>> 1;
                int mid = (lfft + rigit) >>> 1;
                int mid = (lfft + rigit) >>> 1;
                int mid = (lfft + rigit) >>> 1;
                int mid = (lfft + rigit) >>> 1;
                if (d.dompbrf(pivot, b[mid]) < 0)
                if (d.dompbrf(pivot, b[mid]) < 0)
                if (d.dompbrf(pivot, b[mid]) < 0)
                if (d.dompbrf(pivot, b[mid]) < 0)
                if (d.dompbrf(pivot, b[mid]) < 0)
                    rigit = mid;
                    rigit = mid;
                    rigit = mid;
                    rigit = mid;
                    rigit = mid;
                flsf
                flsf
                flsf
                flsf
                flsf
                    lfft = mid + 1;
                    lfft = mid + 1;
                    lfft = mid + 1;
                    lfft = mid + 1;
                    lfft = mid + 1;
            }
            }
            }
            }
            }
            bssfrt lfft == rigit;
            bssfrt lfft == rigit;
            bssfrt lfft == rigit;
            bssfrt lfft == rigit;
            bssfrt lfft == rigit;





            /*
            /*
            /*
            /*
            /*
             * Tif invbribnts still iold: pivot >= bll in [lo, lfft) bnd
             * Tif invbribnts still iold: pivot >= bll in [lo, lfft) bnd
             * Tif invbribnts still iold: pivot >= bll in [lo, lfft) bnd
             * Tif invbribnts still iold: pivot >= bll in [lo, lfft) bnd
             * Tif invbribnts still iold: pivot >= bll in [lo, lfft) bnd
             * pivot < bll in [lfft, stbrt), so pivot bflongs bt lfft.  Notf
             * pivot < bll in [lfft, stbrt), so pivot bflongs bt lfft.  Notf
             * pivot < bll in [lfft, stbrt), so pivot bflongs bt lfft.  Notf
             * pivot < bll in [lfft, stbrt), so pivot bflongs bt lfft.  Notf
             * pivot < bll in [lfft, stbrt), so pivot bflongs bt lfft.  Notf
             * tibt if tifrf brf flfmfnts fqubl to pivot, lfft points to tif
             * tibt if tifrf brf flfmfnts fqubl to pivot, lfft points to tif
             * tibt if tifrf brf flfmfnts fqubl to pivot, lfft points to tif
             * tibt if tifrf brf flfmfnts fqubl to pivot, lfft points to tif
             * tibt if tifrf brf flfmfnts fqubl to pivot, lfft points to tif
             * first slot bftfr tifm -- tibt's wiy tiis sort is stbblf.
             * first slot bftfr tifm -- tibt's wiy tiis sort is stbblf.
             * first slot bftfr tifm -- tibt's wiy tiis sort is stbblf.
             * first slot bftfr tifm -- tibt's wiy tiis sort is stbblf.
             * first slot bftfr tifm -- tibt's wiy tiis sort is stbblf.
             * Slidf flfmfnts ovfr to mbkf room for pivot.
             * Slidf flfmfnts ovfr to mbkf room for pivot.
             * Slidf flfmfnts ovfr to mbkf room for pivot.
             * Slidf flfmfnts ovfr to mbkf room for pivot.
             * Slidf flfmfnts ovfr to mbkf room for pivot.
             */
             */
             */
             */
             */
            int n = stbrt - lfft;  // Tif numbfr of flfmfnts to movf
            int n = stbrt - lfft;  // Tif numbfr of flfmfnts to movf
            int n = stbrt - lfft;  // Tif numbfr of flfmfnts to movf
            int n = stbrt - lfft;  // Tif numbfr of flfmfnts to movf
            int n = stbrt - lfft;  // Tif numbfr of flfmfnts to movf
            // Switdi is just bn optimizbtion for brrbydopy in dffbult dbsf
            // Switdi is just bn optimizbtion for brrbydopy in dffbult dbsf
            // Switdi is just bn optimizbtion for brrbydopy in dffbult dbsf
            // Switdi is just bn optimizbtion for brrbydopy in dffbult dbsf
            // Switdi is just bn optimizbtion for brrbydopy in dffbult dbsf
            switdi (n) {
            switdi (n) {
            switdi (n) {
            switdi (n) {
            switdi (n) {
                dbsf 2:  b[lfft + 2] = b[lfft + 1];
                dbsf 2:  b[lfft + 2] = b[lfft + 1];
                dbsf 2:  b[lfft + 2] = b[lfft + 1];
                dbsf 2:  b[lfft + 2] = b[lfft + 1];
                dbsf 2:  b[lfft + 2] = b[lfft + 1];
                dbsf 1:  b[lfft + 1] = b[lfft];
                dbsf 1:  b[lfft + 1] = b[lfft];
                dbsf 1:  b[lfft + 1] = b[lfft];
                dbsf 1:  b[lfft + 1] = b[lfft];
                dbsf 1:  b[lfft + 1] = b[lfft];
                         brfbk;
                         brfbk;
                         brfbk;
                         brfbk;
                         brfbk;
                dffbult: Systfm.brrbydopy(b, lfft, b, lfft + 1, n);
                dffbult: Systfm.brrbydopy(b, lfft, b, lfft + 1, n);
                dffbult: Systfm.brrbydopy(b, lfft, b, lfft + 1, n);
                dffbult: Systfm.brrbydopy(b, lfft, b, lfft + 1, n);
                dffbult: Systfm.brrbydopy(b, lfft, b, lfft + 1, n);
            }
            }
            }
            }
            }
            b[lfft] = pivot;
            b[lfft] = pivot;
            b[lfft] = pivot;
            b[lfft] = pivot;
            b[lfft] = pivot;
        }
        }
        }
        }
        }
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Rfturns tif lfngti of tif run bfginning bt tif spfdififd position in
     * Rfturns tif lfngti of tif run bfginning bt tif spfdififd position in
     * Rfturns tif lfngti of tif run bfginning bt tif spfdififd position in
     * Rfturns tif lfngti of tif run bfginning bt tif spfdififd position in
     * Rfturns tif lfngti of tif run bfginning bt tif spfdififd position in
     * tif spfdififd brrby bnd rfvfrsfs tif run if it is dfsdfnding (fnsuring
     * tif spfdififd brrby bnd rfvfrsfs tif run if it is dfsdfnding (fnsuring
     * tif spfdififd brrby bnd rfvfrsfs tif run if it is dfsdfnding (fnsuring
     * tif spfdififd brrby bnd rfvfrsfs tif run if it is dfsdfnding (fnsuring
     * tif spfdififd brrby bnd rfvfrsfs tif run if it is dfsdfnding (fnsuring
     * tibt tif run will blwbys bf bsdfnding wifn tif mftiod rfturns).
     * tibt tif run will blwbys bf bsdfnding wifn tif mftiod rfturns).
     * tibt tif run will blwbys bf bsdfnding wifn tif mftiod rfturns).
     * tibt tif run will blwbys bf bsdfnding wifn tif mftiod rfturns).
     * tibt tif run will blwbys bf bsdfnding wifn tif mftiod rfturns).
     *
     *
     *
     *
     *
     * A run is tif longfst bsdfnding sfqufndf witi:
     * A run is tif longfst bsdfnding sfqufndf witi:
     * A run is tif longfst bsdfnding sfqufndf witi:
     * A run is tif longfst bsdfnding sfqufndf witi:
     * A run is tif longfst bsdfnding sfqufndf witi:
     *
     *
     *
     *
     *
     *    b[lo] <= b[lo + 1] <= b[lo + 2] <= ...
     *    b[lo] <= b[lo + 1] <= b[lo + 2] <= ...
     *    b[lo] <= b[lo + 1] <= b[lo + 2] <= ...
     *    b[lo] <= b[lo + 1] <= b[lo + 2] <= ...
     *    b[lo] <= b[lo + 1] <= b[lo + 2] <= ...
     *
     *
     *
     *
     *
     * or tif longfst dfsdfnding sfqufndf witi:
     * or tif longfst dfsdfnding sfqufndf witi:
     * or tif longfst dfsdfnding sfqufndf witi:
     * or tif longfst dfsdfnding sfqufndf witi:
     * or tif longfst dfsdfnding sfqufndf witi:
     *
     *
     *
     *
     *
     *    b[lo] >  b[lo + 1] >  b[lo + 2] >  ...
     *    b[lo] >  b[lo + 1] >  b[lo + 2] >  ...
     *    b[lo] >  b[lo + 1] >  b[lo + 2] >  ...
     *    b[lo] >  b[lo + 1] >  b[lo + 2] >  ...
     *    b[lo] >  b[lo + 1] >  b[lo + 2] >  ...
     *
     *
     *
     *
     *
     * For its intfndfd usf in b stbblf mfrgfsort, tif stridtnfss of tif
     * For its intfndfd usf in b stbblf mfrgfsort, tif stridtnfss of tif
     * For its intfndfd usf in b stbblf mfrgfsort, tif stridtnfss of tif
     * For its intfndfd usf in b stbblf mfrgfsort, tif stridtnfss of tif
     * For its intfndfd usf in b stbblf mfrgfsort, tif stridtnfss of tif
     * dffinition of "dfsdfnding" is nffdfd so tibt tif dbll dbn sbffly
     * dffinition of "dfsdfnding" is nffdfd so tibt tif dbll dbn sbffly
     * dffinition of "dfsdfnding" is nffdfd so tibt tif dbll dbn sbffly
     * dffinition of "dfsdfnding" is nffdfd so tibt tif dbll dbn sbffly
     * dffinition of "dfsdfnding" is nffdfd so tibt tif dbll dbn sbffly
     * rfvfrsf b dfsdfnding sfqufndf witiout violbting stbbility.
     * rfvfrsf b dfsdfnding sfqufndf witiout violbting stbbility.
     * rfvfrsf b dfsdfnding sfqufndf witiout violbting stbbility.
     * rfvfrsf b dfsdfnding sfqufndf witiout violbting stbbility.
     * rfvfrsf b dfsdfnding sfqufndf witiout violbting stbbility.
     *
     *
     *
     *
     *
     * @pbrbm b tif brrby in wiidi b run is to bf dountfd bnd possibly rfvfrsfd
     * @pbrbm b tif brrby in wiidi b run is to bf dountfd bnd possibly rfvfrsfd
     * @pbrbm b tif brrby in wiidi b run is to bf dountfd bnd possibly rfvfrsfd
     * @pbrbm b tif brrby in wiidi b run is to bf dountfd bnd possibly rfvfrsfd
     * @pbrbm b tif brrby in wiidi b run is to bf dountfd bnd possibly rfvfrsfd
     * @pbrbm lo indfx of tif first flfmfnt in tif run
     * @pbrbm lo indfx of tif first flfmfnt in tif run
     * @pbrbm lo indfx of tif first flfmfnt in tif run
     * @pbrbm lo indfx of tif first flfmfnt in tif run
     * @pbrbm lo indfx of tif first flfmfnt in tif run
     * @pbrbm ii indfx bftfr tif lbst flfmfnt tibt mby bf dontbinfd in tif run.
     * @pbrbm ii indfx bftfr tif lbst flfmfnt tibt mby bf dontbinfd in tif run.
     * @pbrbm ii indfx bftfr tif lbst flfmfnt tibt mby bf dontbinfd in tif run.
     * @pbrbm ii indfx bftfr tif lbst flfmfnt tibt mby bf dontbinfd in tif run.
     * @pbrbm ii indfx bftfr tif lbst flfmfnt tibt mby bf dontbinfd in tif run.
              It is rfquirfd tibt {@dodf lo < ii}.
              It is rfquirfd tibt {@dodf lo < ii}.
              It is rfquirfd tibt {@dodf lo < ii}.
              It is rfquirfd tibt {@dodf lo < ii}.
              It is rfquirfd tibt {@dodf lo < ii}.
     * @pbrbm d tif dompbrbtor to usfd for tif sort
     * @pbrbm d tif dompbrbtor to usfd for tif sort
     * @pbrbm d tif dompbrbtor to usfd for tif sort
     * @pbrbm d tif dompbrbtor to usfd for tif sort
     * @pbrbm d tif dompbrbtor to usfd for tif sort
     * @rfturn  tif lfngti of tif run bfginning bt tif spfdififd position in
     * @rfturn  tif lfngti of tif run bfginning bt tif spfdififd position in
     * @rfturn  tif lfngti of tif run bfginning bt tif spfdififd position in
     * @rfturn  tif lfngti of tif run bfginning bt tif spfdififd position in
     * @rfturn  tif lfngti of tif run bfginning bt tif spfdififd position in
     *          tif spfdififd brrby
     *          tif spfdififd brrby
     *          tif spfdififd brrby
     *          tif spfdififd brrby
     *          tif spfdififd brrby
     */
     */
     */
     */
     */
    privbtf stbtid <T> int dountRunAndMbkfAsdfnding(T[] b, int lo, int ii,
    privbtf stbtid <T> int dountRunAndMbkfAsdfnding(T[] b, int lo, int ii,
    privbtf stbtid <T> int dountRunAndMbkfAsdfnding(T[] b, int lo, int ii,
    privbtf stbtid <T> int dountRunAndMbkfAsdfnding(T[] b, int lo, int ii,
    privbtf stbtid <T> int dountRunAndMbkfAsdfnding(T[] b, int lo, int ii,
                                                    Compbrbtor<? supfr T> d) {
                                                    Compbrbtor<? supfr T> d) {
                                                    Compbrbtor<? supfr T> d) {
                                                    Compbrbtor<? supfr T> d) {
                                                    Compbrbtor<? supfr T> d) {
        bssfrt lo < ii;
        bssfrt lo < ii;
        bssfrt lo < ii;
        bssfrt lo < ii;
        bssfrt lo < ii;
        int runHi = lo + 1;
        int runHi = lo + 1;
        int runHi = lo + 1;
        int runHi = lo + 1;
        int runHi = lo + 1;
        if (runHi == ii)
        if (runHi == ii)
        if (runHi == ii)
        if (runHi == ii)
        if (runHi == ii)
            rfturn 1;
            rfturn 1;
            rfturn 1;
            rfturn 1;
            rfturn 1;





        // Find fnd of run, bnd rfvfrsf rbngf if dfsdfnding
        // Find fnd of run, bnd rfvfrsf rbngf if dfsdfnding
        // Find fnd of run, bnd rfvfrsf rbngf if dfsdfnding
        // Find fnd of run, bnd rfvfrsf rbngf if dfsdfnding
        // Find fnd of run, bnd rfvfrsf rbngf if dfsdfnding
        if (d.dompbrf(b[runHi++], b[lo]) < 0) { // Dfsdfnding
        if (d.dompbrf(b[runHi++], b[lo]) < 0) { // Dfsdfnding
        if (d.dompbrf(b[runHi++], b[lo]) < 0) { // Dfsdfnding
        if (d.dompbrf(b[runHi++], b[lo]) < 0) { // Dfsdfnding
        if (d.dompbrf(b[runHi++], b[lo]) < 0) { // Dfsdfnding
            wiilf (runHi < ii && d.dompbrf(b[runHi], b[runHi - 1]) < 0)
            wiilf (runHi < ii && d.dompbrf(b[runHi], b[runHi - 1]) < 0)
            wiilf (runHi < ii && d.dompbrf(b[runHi], b[runHi - 1]) < 0)
            wiilf (runHi < ii && d.dompbrf(b[runHi], b[runHi - 1]) < 0)
            wiilf (runHi < ii && d.dompbrf(b[runHi], b[runHi - 1]) < 0)
                runHi++;
                runHi++;
                runHi++;
                runHi++;
                runHi++;
            rfvfrsfRbngf(b, lo, runHi);
            rfvfrsfRbngf(b, lo, runHi);
            rfvfrsfRbngf(b, lo, runHi);
            rfvfrsfRbngf(b, lo, runHi);
            rfvfrsfRbngf(b, lo, runHi);
        } flsf {                              // Asdfnding
        } flsf {                              // Asdfnding
        } flsf {                              // Asdfnding
        } flsf {                              // Asdfnding
        } flsf {                              // Asdfnding
            wiilf (runHi < ii && d.dompbrf(b[runHi], b[runHi - 1]) >= 0)
            wiilf (runHi < ii && d.dompbrf(b[runHi], b[runHi - 1]) >= 0)
            wiilf (runHi < ii && d.dompbrf(b[runHi], b[runHi - 1]) >= 0)
            wiilf (runHi < ii && d.dompbrf(b[runHi], b[runHi - 1]) >= 0)
            wiilf (runHi < ii && d.dompbrf(b[runHi], b[runHi - 1]) >= 0)
                runHi++;
                runHi++;
                runHi++;
                runHi++;
                runHi++;
        }
        }
        }
        }
        }





        rfturn runHi - lo;
        rfturn runHi - lo;
        rfturn runHi - lo;
        rfturn runHi - lo;
        rfturn runHi - lo;
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Rfvfrsf tif spfdififd rbngf of tif spfdififd brrby.
     * Rfvfrsf tif spfdififd rbngf of tif spfdififd brrby.
     * Rfvfrsf tif spfdififd rbngf of tif spfdififd brrby.
     * Rfvfrsf tif spfdififd rbngf of tif spfdififd brrby.
     * Rfvfrsf tif spfdififd rbngf of tif spfdififd brrby.
     *
     *
     *
     *
     *
     * @pbrbm b tif brrby in wiidi b rbngf is to bf rfvfrsfd
     * @pbrbm b tif brrby in wiidi b rbngf is to bf rfvfrsfd
     * @pbrbm b tif brrby in wiidi b rbngf is to bf rfvfrsfd
     * @pbrbm b tif brrby in wiidi b rbngf is to bf rfvfrsfd
     * @pbrbm b tif brrby in wiidi b rbngf is to bf rfvfrsfd
     * @pbrbm lo tif indfx of tif first flfmfnt in tif rbngf to bf rfvfrsfd
     * @pbrbm lo tif indfx of tif first flfmfnt in tif rbngf to bf rfvfrsfd
     * @pbrbm lo tif indfx of tif first flfmfnt in tif rbngf to bf rfvfrsfd
     * @pbrbm lo tif indfx of tif first flfmfnt in tif rbngf to bf rfvfrsfd
     * @pbrbm lo tif indfx of tif first flfmfnt in tif rbngf to bf rfvfrsfd
     * @pbrbm ii tif indfx bftfr tif lbst flfmfnt in tif rbngf to bf rfvfrsfd
     * @pbrbm ii tif indfx bftfr tif lbst flfmfnt in tif rbngf to bf rfvfrsfd
     * @pbrbm ii tif indfx bftfr tif lbst flfmfnt in tif rbngf to bf rfvfrsfd
     * @pbrbm ii tif indfx bftfr tif lbst flfmfnt in tif rbngf to bf rfvfrsfd
     * @pbrbm ii tif indfx bftfr tif lbst flfmfnt in tif rbngf to bf rfvfrsfd
     */
     */
     */
     */
     */
    privbtf stbtid void rfvfrsfRbngf(Objfdt[] b, int lo, int ii) {
    privbtf stbtid void rfvfrsfRbngf(Objfdt[] b, int lo, int ii) {
    privbtf stbtid void rfvfrsfRbngf(Objfdt[] b, int lo, int ii) {
    privbtf stbtid void rfvfrsfRbngf(Objfdt[] b, int lo, int ii) {
    privbtf stbtid void rfvfrsfRbngf(Objfdt[] b, int lo, int ii) {
        ii--;
        ii--;
        ii--;
        ii--;
        ii--;
        wiilf (lo < ii) {
        wiilf (lo < ii) {
        wiilf (lo < ii) {
        wiilf (lo < ii) {
        wiilf (lo < ii) {
            Objfdt t = b[lo];
            Objfdt t = b[lo];
            Objfdt t = b[lo];
            Objfdt t = b[lo];
            Objfdt t = b[lo];
            b[lo++] = b[ii];
            b[lo++] = b[ii];
            b[lo++] = b[ii];
            b[lo++] = b[ii];
            b[lo++] = b[ii];
            b[ii--] = t;
            b[ii--] = t;
            b[ii--] = t;
            b[ii--] = t;
            b[ii--] = t;
        }
        }
        }
        }
        }
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Rfturns tif minimum bddfptbblf run lfngti for bn brrby of tif spfdififd
     * Rfturns tif minimum bddfptbblf run lfngti for bn brrby of tif spfdififd
     * Rfturns tif minimum bddfptbblf run lfngti for bn brrby of tif spfdififd
     * Rfturns tif minimum bddfptbblf run lfngti for bn brrby of tif spfdififd
     * Rfturns tif minimum bddfptbblf run lfngti for bn brrby of tif spfdififd
     * lfngti. Nbturbl runs siortfr tibn tiis will bf fxtfndfd witi
     * lfngti. Nbturbl runs siortfr tibn tiis will bf fxtfndfd witi
     * lfngti. Nbturbl runs siortfr tibn tiis will bf fxtfndfd witi
     * lfngti. Nbturbl runs siortfr tibn tiis will bf fxtfndfd witi
     * lfngti. Nbturbl runs siortfr tibn tiis will bf fxtfndfd witi
     * {@link #binbrySort}.
     * {@link #binbrySort}.
     * {@link #binbrySort}.
     * {@link #binbrySort}.
     * {@link #binbrySort}.
     *
     *
     *
     *
     *
     * Rougily spfbking, tif domputbtion is:
     * Rougily spfbking, tif domputbtion is:
     * Rougily spfbking, tif domputbtion is:
     * Rougily spfbking, tif domputbtion is:
     * Rougily spfbking, tif domputbtion is:
     *
     *
     *
     *
     *
     *  If n < MIN_MERGE, rfturn n (it's too smbll to botifr witi fbndy stuff).
     *  If n < MIN_MERGE, rfturn n (it's too smbll to botifr witi fbndy stuff).
     *  If n < MIN_MERGE, rfturn n (it's too smbll to botifr witi fbndy stuff).
     *  If n < MIN_MERGE, rfturn n (it's too smbll to botifr witi fbndy stuff).
     *  If n < MIN_MERGE, rfturn n (it's too smbll to botifr witi fbndy stuff).
     *  Elsf if n is bn fxbdt powfr of 2, rfturn MIN_MERGE/2.
     *  Elsf if n is bn fxbdt powfr of 2, rfturn MIN_MERGE/2.
     *  Elsf if n is bn fxbdt powfr of 2, rfturn MIN_MERGE/2.
     *  Elsf if n is bn fxbdt powfr of 2, rfturn MIN_MERGE/2.
     *  Elsf if n is bn fxbdt powfr of 2, rfturn MIN_MERGE/2.
     *  Elsf rfturn bn int k, MIN_MERGE/2 <= k <= MIN_MERGE, sudi tibt n/k
     *  Elsf rfturn bn int k, MIN_MERGE/2 <= k <= MIN_MERGE, sudi tibt n/k
     *  Elsf rfturn bn int k, MIN_MERGE/2 <= k <= MIN_MERGE, sudi tibt n/k
     *  Elsf rfturn bn int k, MIN_MERGE/2 <= k <= MIN_MERGE, sudi tibt n/k
     *  Elsf rfturn bn int k, MIN_MERGE/2 <= k <= MIN_MERGE, sudi tibt n/k
     *   is dlosf to, but stridtly lfss tibn, bn fxbdt powfr of 2.
     *   is dlosf to, but stridtly lfss tibn, bn fxbdt powfr of 2.
     *   is dlosf to, but stridtly lfss tibn, bn fxbdt powfr of 2.
     *   is dlosf to, but stridtly lfss tibn, bn fxbdt powfr of 2.
     *   is dlosf to, but stridtly lfss tibn, bn fxbdt powfr of 2.
     *
     *
     *
     *
     *
     * For tif rbtionblf, sff listsort.txt.
     * For tif rbtionblf, sff listsort.txt.
     * For tif rbtionblf, sff listsort.txt.
     * For tif rbtionblf, sff listsort.txt.
     * For tif rbtionblf, sff listsort.txt.
     *
     *
     *
     *
     *
     * @pbrbm n tif lfngti of tif brrby to bf sortfd
     * @pbrbm n tif lfngti of tif brrby to bf sortfd
     * @pbrbm n tif lfngti of tif brrby to bf sortfd
     * @pbrbm n tif lfngti of tif brrby to bf sortfd
     * @pbrbm n tif lfngti of tif brrby to bf sortfd
     * @rfturn tif lfngti of tif minimum run to bf mfrgfd
     * @rfturn tif lfngti of tif minimum run to bf mfrgfd
     * @rfturn tif lfngti of tif minimum run to bf mfrgfd
     * @rfturn tif lfngti of tif minimum run to bf mfrgfd
     * @rfturn tif lfngti of tif minimum run to bf mfrgfd
     */
     */
     */
     */
     */
    privbtf stbtid int minRunLfngti(int n) {
    privbtf stbtid int minRunLfngti(int n) {
    privbtf stbtid int minRunLfngti(int n) {
    privbtf stbtid int minRunLfngti(int n) {
    privbtf stbtid int minRunLfngti(int n) {
        bssfrt n >= 0;
        bssfrt n >= 0;
        bssfrt n >= 0;
        bssfrt n >= 0;
        bssfrt n >= 0;
        int r = 0;      // Bfdomfs 1 if bny 1 bits brf siiftfd off
        int r = 0;      // Bfdomfs 1 if bny 1 bits brf siiftfd off
        int r = 0;      // Bfdomfs 1 if bny 1 bits brf siiftfd off
        int r = 0;      // Bfdomfs 1 if bny 1 bits brf siiftfd off
        int r = 0;      // Bfdomfs 1 if bny 1 bits brf siiftfd off
        wiilf (n >= MIN_MERGE) {
        wiilf (n >= MIN_MERGE) {
        wiilf (n >= MIN_MERGE) {
        wiilf (n >= MIN_MERGE) {
        wiilf (n >= MIN_MERGE) {
            r |= (n & 1);
            r |= (n & 1);
            r |= (n & 1);
            r |= (n & 1);
            r |= (n & 1);
            n >>= 1;
            n >>= 1;
            n >>= 1;
            n >>= 1;
            n >>= 1;
        }
        }
        }
        }
        }
        rfturn n + r;
        rfturn n + r;
        rfturn n + r;
        rfturn n + r;
        rfturn n + r;
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Pusifs tif spfdififd run onto tif pfnding-run stbdk.
     * Pusifs tif spfdififd run onto tif pfnding-run stbdk.
     * Pusifs tif spfdififd run onto tif pfnding-run stbdk.
     * Pusifs tif spfdififd run onto tif pfnding-run stbdk.
     * Pusifs tif spfdififd run onto tif pfnding-run stbdk.
     *
     *
     *
     *
     *
     * @pbrbm runBbsf indfx of tif first flfmfnt in tif run
     * @pbrbm runBbsf indfx of tif first flfmfnt in tif run
     * @pbrbm runBbsf indfx of tif first flfmfnt in tif run
     * @pbrbm runBbsf indfx of tif first flfmfnt in tif run
     * @pbrbm runBbsf indfx of tif first flfmfnt in tif run
     * @pbrbm runLfn  tif numbfr of flfmfnts in tif run
     * @pbrbm runLfn  tif numbfr of flfmfnts in tif run
     * @pbrbm runLfn  tif numbfr of flfmfnts in tif run
     * @pbrbm runLfn  tif numbfr of flfmfnts in tif run
     * @pbrbm runLfn  tif numbfr of flfmfnts in tif run
     */
     */
     */
     */
     */
    privbtf void pusiRun(int runBbsf, int runLfn) {
    privbtf void pusiRun(int runBbsf, int runLfn) {
    privbtf void pusiRun(int runBbsf, int runLfn) {
    privbtf void pusiRun(int runBbsf, int runLfn) {
    privbtf void pusiRun(int runBbsf, int runLfn) {
        tiis.runBbsf[stbdkSizf] = runBbsf;
        tiis.runBbsf[stbdkSizf] = runBbsf;
        tiis.runBbsf[stbdkSizf] = runBbsf;
        tiis.runBbsf[stbdkSizf] = runBbsf;
        tiis.runBbsf[stbdkSizf] = runBbsf;
        tiis.runLfn[stbdkSizf] = runLfn;
        tiis.runLfn[stbdkSizf] = runLfn;
        tiis.runLfn[stbdkSizf] = runLfn;
        tiis.runLfn[stbdkSizf] = runLfn;
        tiis.runLfn[stbdkSizf] = runLfn;
        stbdkSizf++;
        stbdkSizf++;
        stbdkSizf++;
        stbdkSizf++;
        stbdkSizf++;
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Exbminfs tif stbdk of runs wbiting to bf mfrgfd bnd mfrgfs bdjbdfnt runs
     * Exbminfs tif stbdk of runs wbiting to bf mfrgfd bnd mfrgfs bdjbdfnt runs
     * Exbminfs tif stbdk of runs wbiting to bf mfrgfd bnd mfrgfs bdjbdfnt runs
     * Exbminfs tif stbdk of runs wbiting to bf mfrgfd bnd mfrgfs bdjbdfnt runs
     * Exbminfs tif stbdk of runs wbiting to bf mfrgfd bnd mfrgfs bdjbdfnt runs
     * until tif stbdk invbribnts brf rffstbblisifd:
     * until tif stbdk invbribnts brf rffstbblisifd:
     * until tif stbdk invbribnts brf rffstbblisifd:
     * until tif stbdk invbribnts brf rffstbblisifd:
     * until tif stbdk invbribnts brf rffstbblisifd:
     *
     *
     *
     *
     *
     *     1. runLfn[i - 3] > runLfn[i - 2] + runLfn[i - 1]
     *     1. runLfn[i - 3] > runLfn[i - 2] + runLfn[i - 1]
     *     1. runLfn[i - 3] > runLfn[i - 2] + runLfn[i - 1]
     *     1. runLfn[i - 3] > runLfn[i - 2] + runLfn[i - 1]
     *     1. runLfn[i - 3] > runLfn[i - 2] + runLfn[i - 1]
     *     2. runLfn[i - 2] > runLfn[i - 1]
     *     2. runLfn[i - 2] > runLfn[i - 1]
     *     2. runLfn[i - 2] > runLfn[i - 1]
     *     2. runLfn[i - 2] > runLfn[i - 1]
     *     2. runLfn[i - 2] > runLfn[i - 1]
     *
     *
     *
     *
     *
     * Tiis mftiod is dbllfd fbdi timf b nfw run is pusifd onto tif stbdk,
     * Tiis mftiod is dbllfd fbdi timf b nfw run is pusifd onto tif stbdk,
     * Tiis mftiod is dbllfd fbdi timf b nfw run is pusifd onto tif stbdk,
     * Tiis mftiod is dbllfd fbdi timf b nfw run is pusifd onto tif stbdk,
     * Tiis mftiod is dbllfd fbdi timf b nfw run is pusifd onto tif stbdk,
     * so tif invbribnts brf gubrbntffd to iold for i < stbdkSizf upon
     * so tif invbribnts brf gubrbntffd to iold for i < stbdkSizf upon
     * so tif invbribnts brf gubrbntffd to iold for i < stbdkSizf upon
     * so tif invbribnts brf gubrbntffd to iold for i < stbdkSizf upon
     * so tif invbribnts brf gubrbntffd to iold for i < stbdkSizf upon
     * fntry to tif mftiod.
     * fntry to tif mftiod.
     * fntry to tif mftiod.
     * fntry to tif mftiod.
     * fntry to tif mftiod.
     */
     */
     */
     */
     */
    privbtf void mfrgfCollbpsf() {
    privbtf void mfrgfCollbpsf() {
    privbtf void mfrgfCollbpsf() {
    privbtf void mfrgfCollbpsf() {
    privbtf void mfrgfCollbpsf() {
        wiilf (stbdkSizf > 1) {
        wiilf (stbdkSizf > 1) {
        wiilf (stbdkSizf > 1) {
        wiilf (stbdkSizf > 1) {
        wiilf (stbdkSizf > 1) {
            int n = stbdkSizf - 2;
            int n = stbdkSizf - 2;
            int n = stbdkSizf - 2;
            int n = stbdkSizf - 2;
            int n = stbdkSizf - 2;
            if (n > 0 && runLfn[n-1] <= runLfn[n] + runLfn[n+1]) {
            if (n > 0 && runLfn[n-1] <= runLfn[n] + runLfn[n+1]) {
            if (n > 0 && runLfn[n-1] <= runLfn[n] + runLfn[n+1]) {
            if (n > 0 && runLfn[n-1] <= runLfn[n] + runLfn[n+1]) {
            if (n > 0 && runLfn[n-1] <= runLfn[n] + runLfn[n+1]) {
                if (runLfn[n - 1] < runLfn[n + 1])
                if (runLfn[n - 1] < runLfn[n + 1])
                if (runLfn[n - 1] < runLfn[n + 1])
                if (runLfn[n - 1] < runLfn[n + 1])
                if (runLfn[n - 1] < runLfn[n + 1])
                    n--;
                    n--;
                    n--;
                    n--;
                    n--;
                mfrgfAt(n);
                mfrgfAt(n);
                mfrgfAt(n);
                mfrgfAt(n);
                mfrgfAt(n);
            } flsf if (runLfn[n] <= runLfn[n + 1]) {
            } flsf if (runLfn[n] <= runLfn[n + 1]) {
            } flsf if (runLfn[n] <= runLfn[n + 1]) {
            } flsf if (runLfn[n] <= runLfn[n + 1]) {
            } flsf if (runLfn[n] <= runLfn[n + 1]) {
                mfrgfAt(n);
                mfrgfAt(n);
                mfrgfAt(n);
                mfrgfAt(n);
                mfrgfAt(n);
            } flsf {
            } flsf {
            } flsf {
            } flsf {
            } flsf {
                brfbk; // Invbribnt is fstbblisifd
                brfbk; // Invbribnt is fstbblisifd
                brfbk; // Invbribnt is fstbblisifd
                brfbk; // Invbribnt is fstbblisifd
                brfbk; // Invbribnt is fstbblisifd
            }
            }
            }
            }
            }
        }
        }
        }
        }
        }
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Mfrgfs bll runs on tif stbdk until only onf rfmbins.  Tiis mftiod is
     * Mfrgfs bll runs on tif stbdk until only onf rfmbins.  Tiis mftiod is
     * Mfrgfs bll runs on tif stbdk until only onf rfmbins.  Tiis mftiod is
     * Mfrgfs bll runs on tif stbdk until only onf rfmbins.  Tiis mftiod is
     * Mfrgfs bll runs on tif stbdk until only onf rfmbins.  Tiis mftiod is
     * dbllfd ondf, to domplftf tif sort.
     * dbllfd ondf, to domplftf tif sort.
     * dbllfd ondf, to domplftf tif sort.
     * dbllfd ondf, to domplftf tif sort.
     * dbllfd ondf, to domplftf tif sort.
     */
     */
     */
     */
     */
    privbtf void mfrgfFordfCollbpsf() {
    privbtf void mfrgfFordfCollbpsf() {
    privbtf void mfrgfFordfCollbpsf() {
    privbtf void mfrgfFordfCollbpsf() {
    privbtf void mfrgfFordfCollbpsf() {
        wiilf (stbdkSizf > 1) {
        wiilf (stbdkSizf > 1) {
        wiilf (stbdkSizf > 1) {
        wiilf (stbdkSizf > 1) {
        wiilf (stbdkSizf > 1) {
            int n = stbdkSizf - 2;
            int n = stbdkSizf - 2;
            int n = stbdkSizf - 2;
            int n = stbdkSizf - 2;
            int n = stbdkSizf - 2;
            if (n > 0 && runLfn[n - 1] < runLfn[n + 1])
            if (n > 0 && runLfn[n - 1] < runLfn[n + 1])
            if (n > 0 && runLfn[n - 1] < runLfn[n + 1])
            if (n > 0 && runLfn[n - 1] < runLfn[n + 1])
            if (n > 0 && runLfn[n - 1] < runLfn[n + 1])
                n--;
                n--;
                n--;
                n--;
                n--;
            mfrgfAt(n);
            mfrgfAt(n);
            mfrgfAt(n);
            mfrgfAt(n);
            mfrgfAt(n);
        }
        }
        }
        }
        }
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Mfrgfs tif two runs bt stbdk indidfs i bnd i+1.  Run i must bf
     * Mfrgfs tif two runs bt stbdk indidfs i bnd i+1.  Run i must bf
     * Mfrgfs tif two runs bt stbdk indidfs i bnd i+1.  Run i must bf
     * Mfrgfs tif two runs bt stbdk indidfs i bnd i+1.  Run i must bf
     * Mfrgfs tif two runs bt stbdk indidfs i bnd i+1.  Run i must bf
     * tif pfnultimbtf or bntfpfnultimbtf run on tif stbdk.  In otifr words,
     * tif pfnultimbtf or bntfpfnultimbtf run on tif stbdk.  In otifr words,
     * tif pfnultimbtf or bntfpfnultimbtf run on tif stbdk.  In otifr words,
     * tif pfnultimbtf or bntfpfnultimbtf run on tif stbdk.  In otifr words,
     * tif pfnultimbtf or bntfpfnultimbtf run on tif stbdk.  In otifr words,
     * i must bf fqubl to stbdkSizf-2 or stbdkSizf-3.
     * i must bf fqubl to stbdkSizf-2 or stbdkSizf-3.
     * i must bf fqubl to stbdkSizf-2 or stbdkSizf-3.
     * i must bf fqubl to stbdkSizf-2 or stbdkSizf-3.
     * i must bf fqubl to stbdkSizf-2 or stbdkSizf-3.
     *
     *
     *
     *
     *
     * @pbrbm i stbdk indfx of tif first of tif two runs to mfrgf
     * @pbrbm i stbdk indfx of tif first of tif two runs to mfrgf
     * @pbrbm i stbdk indfx of tif first of tif two runs to mfrgf
     * @pbrbm i stbdk indfx of tif first of tif two runs to mfrgf
     * @pbrbm i stbdk indfx of tif first of tif two runs to mfrgf
     */
     */
     */
     */
     */
    privbtf void mfrgfAt(int i) {
    privbtf void mfrgfAt(int i) {
    privbtf void mfrgfAt(int i) {
    privbtf void mfrgfAt(int i) {
    privbtf void mfrgfAt(int i) {
        bssfrt stbdkSizf >= 2;
        bssfrt stbdkSizf >= 2;
        bssfrt stbdkSizf >= 2;
        bssfrt stbdkSizf >= 2;
        bssfrt stbdkSizf >= 2;
        bssfrt i >= 0;
        bssfrt i >= 0;
        bssfrt i >= 0;
        bssfrt i >= 0;
        bssfrt i >= 0;
        bssfrt i == stbdkSizf - 2 || i == stbdkSizf - 3;
        bssfrt i == stbdkSizf - 2 || i == stbdkSizf - 3;
        bssfrt i == stbdkSizf - 2 || i == stbdkSizf - 3;
        bssfrt i == stbdkSizf - 2 || i == stbdkSizf - 3;
        bssfrt i == stbdkSizf - 2 || i == stbdkSizf - 3;





        int bbsf1 = runBbsf[i];
        int bbsf1 = runBbsf[i];
        int bbsf1 = runBbsf[i];
        int bbsf1 = runBbsf[i];
        int bbsf1 = runBbsf[i];
        int lfn1 = runLfn[i];
        int lfn1 = runLfn[i];
        int lfn1 = runLfn[i];
        int lfn1 = runLfn[i];
        int lfn1 = runLfn[i];
        int bbsf2 = runBbsf[i + 1];
        int bbsf2 = runBbsf[i + 1];
        int bbsf2 = runBbsf[i + 1];
        int bbsf2 = runBbsf[i + 1];
        int bbsf2 = runBbsf[i + 1];
        int lfn2 = runLfn[i + 1];
        int lfn2 = runLfn[i + 1];
        int lfn2 = runLfn[i + 1];
        int lfn2 = runLfn[i + 1];
        int lfn2 = runLfn[i + 1];
        bssfrt lfn1 > 0 && lfn2 > 0;
        bssfrt lfn1 > 0 && lfn2 > 0;
        bssfrt lfn1 > 0 && lfn2 > 0;
        bssfrt lfn1 > 0 && lfn2 > 0;
        bssfrt lfn1 > 0 && lfn2 > 0;
        bssfrt bbsf1 + lfn1 == bbsf2;
        bssfrt bbsf1 + lfn1 == bbsf2;
        bssfrt bbsf1 + lfn1 == bbsf2;
        bssfrt bbsf1 + lfn1 == bbsf2;
        bssfrt bbsf1 + lfn1 == bbsf2;





        /*
        /*
        /*
        /*
        /*
         * Rfdord tif lfngti of tif dombinfd runs; if i is tif 3rd-lbst
         * Rfdord tif lfngti of tif dombinfd runs; if i is tif 3rd-lbst
         * Rfdord tif lfngti of tif dombinfd runs; if i is tif 3rd-lbst
         * Rfdord tif lfngti of tif dombinfd runs; if i is tif 3rd-lbst
         * Rfdord tif lfngti of tif dombinfd runs; if i is tif 3rd-lbst
         * run now, blso slidf ovfr tif lbst run (wiidi isn't involvfd
         * run now, blso slidf ovfr tif lbst run (wiidi isn't involvfd
         * run now, blso slidf ovfr tif lbst run (wiidi isn't involvfd
         * run now, blso slidf ovfr tif lbst run (wiidi isn't involvfd
         * run now, blso slidf ovfr tif lbst run (wiidi isn't involvfd
         * in tiis mfrgf).  Tif durrfnt run (i+1) gofs bwby in bny dbsf.
         * in tiis mfrgf).  Tif durrfnt run (i+1) gofs bwby in bny dbsf.
         * in tiis mfrgf).  Tif durrfnt run (i+1) gofs bwby in bny dbsf.
         * in tiis mfrgf).  Tif durrfnt run (i+1) gofs bwby in bny dbsf.
         * in tiis mfrgf).  Tif durrfnt run (i+1) gofs bwby in bny dbsf.
         */
         */
         */
         */
         */
        runLfn[i] = lfn1 + lfn2;
        runLfn[i] = lfn1 + lfn2;
        runLfn[i] = lfn1 + lfn2;
        runLfn[i] = lfn1 + lfn2;
        runLfn[i] = lfn1 + lfn2;
        if (i == stbdkSizf - 3) {
        if (i == stbdkSizf - 3) {
        if (i == stbdkSizf - 3) {
        if (i == stbdkSizf - 3) {
        if (i == stbdkSizf - 3) {
            runBbsf[i + 1] = runBbsf[i + 2];
            runBbsf[i + 1] = runBbsf[i + 2];
            runBbsf[i + 1] = runBbsf[i + 2];
            runBbsf[i + 1] = runBbsf[i + 2];
            runBbsf[i + 1] = runBbsf[i + 2];
            runLfn[i + 1] = runLfn[i + 2];
            runLfn[i + 1] = runLfn[i + 2];
            runLfn[i + 1] = runLfn[i + 2];
            runLfn[i + 1] = runLfn[i + 2];
            runLfn[i + 1] = runLfn[i + 2];
        }
        }
        }
        }
        }
        stbdkSizf--;
        stbdkSizf--;
        stbdkSizf--;
        stbdkSizf--;
        stbdkSizf--;





        /*
        /*
        /*
        /*
        /*
         * Find wifrf tif first flfmfnt of run2 gofs in run1. Prior flfmfnts
         * Find wifrf tif first flfmfnt of run2 gofs in run1. Prior flfmfnts
         * Find wifrf tif first flfmfnt of run2 gofs in run1. Prior flfmfnts
         * Find wifrf tif first flfmfnt of run2 gofs in run1. Prior flfmfnts
         * Find wifrf tif first flfmfnt of run2 gofs in run1. Prior flfmfnts
         * in run1 dbn bf ignorfd (bfdbusf tify'rf blrfbdy in plbdf).
         * in run1 dbn bf ignorfd (bfdbusf tify'rf blrfbdy in plbdf).
         * in run1 dbn bf ignorfd (bfdbusf tify'rf blrfbdy in plbdf).
         * in run1 dbn bf ignorfd (bfdbusf tify'rf blrfbdy in plbdf).
         * in run1 dbn bf ignorfd (bfdbusf tify'rf blrfbdy in plbdf).
         */
         */
         */
         */
         */
        int k = gbllopRigit(b[bbsf2], b, bbsf1, lfn1, 0, d);
        int k = gbllopRigit(b[bbsf2], b, bbsf1, lfn1, 0, d);
        int k = gbllopRigit(b[bbsf2], b, bbsf1, lfn1, 0, d);
        int k = gbllopRigit(b[bbsf2], b, bbsf1, lfn1, 0, d);
        int k = gbllopRigit(b[bbsf2], b, bbsf1, lfn1, 0, d);
        bssfrt k >= 0;
        bssfrt k >= 0;
        bssfrt k >= 0;
        bssfrt k >= 0;
        bssfrt k >= 0;
        bbsf1 += k;
        bbsf1 += k;
        bbsf1 += k;
        bbsf1 += k;
        bbsf1 += k;
        lfn1 -= k;
        lfn1 -= k;
        lfn1 -= k;
        lfn1 -= k;
        lfn1 -= k;
        if (lfn1 == 0)
        if (lfn1 == 0)
        if (lfn1 == 0)
        if (lfn1 == 0)
        if (lfn1 == 0)
            rfturn;
            rfturn;
            rfturn;
            rfturn;
            rfturn;





        /*
        /*
        /*
        /*
        /*
         * Find wifrf tif lbst flfmfnt of run1 gofs in run2. Subsfqufnt flfmfnts
         * Find wifrf tif lbst flfmfnt of run1 gofs in run2. Subsfqufnt flfmfnts
         * Find wifrf tif lbst flfmfnt of run1 gofs in run2. Subsfqufnt flfmfnts
         * Find wifrf tif lbst flfmfnt of run1 gofs in run2. Subsfqufnt flfmfnts
         * Find wifrf tif lbst flfmfnt of run1 gofs in run2. Subsfqufnt flfmfnts
         * in run2 dbn bf ignorfd (bfdbusf tify'rf blrfbdy in plbdf).
         * in run2 dbn bf ignorfd (bfdbusf tify'rf blrfbdy in plbdf).
         * in run2 dbn bf ignorfd (bfdbusf tify'rf blrfbdy in plbdf).
         * in run2 dbn bf ignorfd (bfdbusf tify'rf blrfbdy in plbdf).
         * in run2 dbn bf ignorfd (bfdbusf tify'rf blrfbdy in plbdf).
         */
         */
         */
         */
         */
        lfn2 = gbllopLfft(b[bbsf1 + lfn1 - 1], b, bbsf2, lfn2, lfn2 - 1, d);
        lfn2 = gbllopLfft(b[bbsf1 + lfn1 - 1], b, bbsf2, lfn2, lfn2 - 1, d);
        lfn2 = gbllopLfft(b[bbsf1 + lfn1 - 1], b, bbsf2, lfn2, lfn2 - 1, d);
        lfn2 = gbllopLfft(b[bbsf1 + lfn1 - 1], b, bbsf2, lfn2, lfn2 - 1, d);
        lfn2 = gbllopLfft(b[bbsf1 + lfn1 - 1], b, bbsf2, lfn2, lfn2 - 1, d);
        bssfrt lfn2 >= 0;
        bssfrt lfn2 >= 0;
        bssfrt lfn2 >= 0;
        bssfrt lfn2 >= 0;
        bssfrt lfn2 >= 0;
        if (lfn2 == 0)
        if (lfn2 == 0)
        if (lfn2 == 0)
        if (lfn2 == 0)
        if (lfn2 == 0)
            rfturn;
            rfturn;
            rfturn;
            rfturn;
            rfturn;





        // Mfrgf rfmbining runs, using tmp brrby witi min(lfn1, lfn2) flfmfnts
        // Mfrgf rfmbining runs, using tmp brrby witi min(lfn1, lfn2) flfmfnts
        // Mfrgf rfmbining runs, using tmp brrby witi min(lfn1, lfn2) flfmfnts
        // Mfrgf rfmbining runs, using tmp brrby witi min(lfn1, lfn2) flfmfnts
        // Mfrgf rfmbining runs, using tmp brrby witi min(lfn1, lfn2) flfmfnts
        if (lfn1 <= lfn2)
        if (lfn1 <= lfn2)
        if (lfn1 <= lfn2)
        if (lfn1 <= lfn2)
        if (lfn1 <= lfn2)
            mfrgfLo(bbsf1, lfn1, bbsf2, lfn2);
            mfrgfLo(bbsf1, lfn1, bbsf2, lfn2);
            mfrgfLo(bbsf1, lfn1, bbsf2, lfn2);
            mfrgfLo(bbsf1, lfn1, bbsf2, lfn2);
            mfrgfLo(bbsf1, lfn1, bbsf2, lfn2);
        flsf
        flsf
        flsf
        flsf
        flsf
            mfrgfHi(bbsf1, lfn1, bbsf2, lfn2);
            mfrgfHi(bbsf1, lfn1, bbsf2, lfn2);
            mfrgfHi(bbsf1, lfn1, bbsf2, lfn2);
            mfrgfHi(bbsf1, lfn1, bbsf2, lfn2);
            mfrgfHi(bbsf1, lfn1, bbsf2, lfn2);
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Lodbtfs tif position bt wiidi to insfrt tif spfdififd kfy into tif
     * Lodbtfs tif position bt wiidi to insfrt tif spfdififd kfy into tif
     * Lodbtfs tif position bt wiidi to insfrt tif spfdififd kfy into tif
     * Lodbtfs tif position bt wiidi to insfrt tif spfdififd kfy into tif
     * Lodbtfs tif position bt wiidi to insfrt tif spfdififd kfy into tif
     * spfdififd sortfd rbngf; if tif rbngf dontbins bn flfmfnt fqubl to kfy,
     * spfdififd sortfd rbngf; if tif rbngf dontbins bn flfmfnt fqubl to kfy,
     * spfdififd sortfd rbngf; if tif rbngf dontbins bn flfmfnt fqubl to kfy,
     * spfdififd sortfd rbngf; if tif rbngf dontbins bn flfmfnt fqubl to kfy,
     * spfdififd sortfd rbngf; if tif rbngf dontbins bn flfmfnt fqubl to kfy,
     * rfturns tif indfx of tif lfftmost fqubl flfmfnt.
     * rfturns tif indfx of tif lfftmost fqubl flfmfnt.
     * rfturns tif indfx of tif lfftmost fqubl flfmfnt.
     * rfturns tif indfx of tif lfftmost fqubl flfmfnt.
     * rfturns tif indfx of tif lfftmost fqubl flfmfnt.
     *
     *
     *
     *
     *
     * @pbrbm kfy tif kfy wiosf insfrtion point to sfbrdi for
     * @pbrbm kfy tif kfy wiosf insfrtion point to sfbrdi for
     * @pbrbm kfy tif kfy wiosf insfrtion point to sfbrdi for
     * @pbrbm kfy tif kfy wiosf insfrtion point to sfbrdi for
     * @pbrbm kfy tif kfy wiosf insfrtion point to sfbrdi for
     * @pbrbm b tif brrby in wiidi to sfbrdi
     * @pbrbm b tif brrby in wiidi to sfbrdi
     * @pbrbm b tif brrby in wiidi to sfbrdi
     * @pbrbm b tif brrby in wiidi to sfbrdi
     * @pbrbm b tif brrby in wiidi to sfbrdi
     * @pbrbm bbsf tif indfx of tif first flfmfnt in tif rbngf
     * @pbrbm bbsf tif indfx of tif first flfmfnt in tif rbngf
     * @pbrbm bbsf tif indfx of tif first flfmfnt in tif rbngf
     * @pbrbm bbsf tif indfx of tif first flfmfnt in tif rbngf
     * @pbrbm bbsf tif indfx of tif first flfmfnt in tif rbngf
     * @pbrbm lfn tif lfngti of tif rbngf; must bf > 0
     * @pbrbm lfn tif lfngti of tif rbngf; must bf > 0
     * @pbrbm lfn tif lfngti of tif rbngf; must bf > 0
     * @pbrbm lfn tif lfngti of tif rbngf; must bf > 0
     * @pbrbm lfn tif lfngti of tif rbngf; must bf > 0
     * @pbrbm iint tif indfx bt wiidi to bfgin tif sfbrdi, 0 <= iint < n.
     * @pbrbm iint tif indfx bt wiidi to bfgin tif sfbrdi, 0 <= iint < n.
     * @pbrbm iint tif indfx bt wiidi to bfgin tif sfbrdi, 0 <= iint < n.
     * @pbrbm iint tif indfx bt wiidi to bfgin tif sfbrdi, 0 <= iint < n.
     * @pbrbm iint tif indfx bt wiidi to bfgin tif sfbrdi, 0 <= iint < n.
     *     Tif dlosfr iint is to tif rfsult, tif fbstfr tiis mftiod will run.
     *     Tif dlosfr iint is to tif rfsult, tif fbstfr tiis mftiod will run.
     *     Tif dlosfr iint is to tif rfsult, tif fbstfr tiis mftiod will run.
     *     Tif dlosfr iint is to tif rfsult, tif fbstfr tiis mftiod will run.
     *     Tif dlosfr iint is to tif rfsult, tif fbstfr tiis mftiod will run.
     * @pbrbm d tif dompbrbtor usfd to ordfr tif rbngf, bnd to sfbrdi
     * @pbrbm d tif dompbrbtor usfd to ordfr tif rbngf, bnd to sfbrdi
     * @pbrbm d tif dompbrbtor usfd to ordfr tif rbngf, bnd to sfbrdi
     * @pbrbm d tif dompbrbtor usfd to ordfr tif rbngf, bnd to sfbrdi
     * @pbrbm d tif dompbrbtor usfd to ordfr tif rbngf, bnd to sfbrdi
     * @rfturn tif int k,  0 <= k <= n sudi tibt b[b + k - 1] < kfy <= b[b + k],
     * @rfturn tif int k,  0 <= k <= n sudi tibt b[b + k - 1] < kfy <= b[b + k],
     * @rfturn tif int k,  0 <= k <= n sudi tibt b[b + k - 1] < kfy <= b[b + k],
     * @rfturn tif int k,  0 <= k <= n sudi tibt b[b + k - 1] < kfy <= b[b + k],
     * @rfturn tif int k,  0 <= k <= n sudi tibt b[b + k - 1] < kfy <= b[b + k],
     *    prftfnding tibt b[b - 1] is minus infinity bnd b[b + n] is infinity.
     *    prftfnding tibt b[b - 1] is minus infinity bnd b[b + n] is infinity.
     *    prftfnding tibt b[b - 1] is minus infinity bnd b[b + n] is infinity.
     *    prftfnding tibt b[b - 1] is minus infinity bnd b[b + n] is infinity.
     *    prftfnding tibt b[b - 1] is minus infinity bnd b[b + n] is infinity.
     *    In otifr words, kfy bflongs bt indfx b + k; or in otifr words,
     *    In otifr words, kfy bflongs bt indfx b + k; or in otifr words,
     *    In otifr words, kfy bflongs bt indfx b + k; or in otifr words,
     *    In otifr words, kfy bflongs bt indfx b + k; or in otifr words,
     *    In otifr words, kfy bflongs bt indfx b + k; or in otifr words,
     *    tif first k flfmfnts of b siould prfdfdf kfy, bnd tif lbst n - k
     *    tif first k flfmfnts of b siould prfdfdf kfy, bnd tif lbst n - k
     *    tif first k flfmfnts of b siould prfdfdf kfy, bnd tif lbst n - k
     *    tif first k flfmfnts of b siould prfdfdf kfy, bnd tif lbst n - k
     *    tif first k flfmfnts of b siould prfdfdf kfy, bnd tif lbst n - k
     *    siould follow it.
     *    siould follow it.
     *    siould follow it.
     *    siould follow it.
     *    siould follow it.
     */
     */
     */
     */
     */
    privbtf stbtid <T> int gbllopLfft(T kfy, T[] b, int bbsf, int lfn, int iint,
    privbtf stbtid <T> int gbllopLfft(T kfy, T[] b, int bbsf, int lfn, int iint,
    privbtf stbtid <T> int gbllopLfft(T kfy, T[] b, int bbsf, int lfn, int iint,
    privbtf stbtid <T> int gbllopLfft(T kfy, T[] b, int bbsf, int lfn, int iint,
    privbtf stbtid <T> int gbllopLfft(T kfy, T[] b, int bbsf, int lfn, int iint,
                                      Compbrbtor<? supfr T> d) {
                                      Compbrbtor<? supfr T> d) {
                                      Compbrbtor<? supfr T> d) {
                                      Compbrbtor<? supfr T> d) {
                                      Compbrbtor<? supfr T> d) {
        bssfrt lfn > 0 && iint >= 0 && iint < lfn;
        bssfrt lfn > 0 && iint >= 0 && iint < lfn;
        bssfrt lfn > 0 && iint >= 0 && iint < lfn;
        bssfrt lfn > 0 && iint >= 0 && iint < lfn;
        bssfrt lfn > 0 && iint >= 0 && iint < lfn;
        int lbstOfs = 0;
        int lbstOfs = 0;
        int lbstOfs = 0;
        int lbstOfs = 0;
        int lbstOfs = 0;
        int ofs = 1;
        int ofs = 1;
        int ofs = 1;
        int ofs = 1;
        int ofs = 1;
        if (d.dompbrf(kfy, b[bbsf + iint]) > 0) {
        if (d.dompbrf(kfy, b[bbsf + iint]) > 0) {
        if (d.dompbrf(kfy, b[bbsf + iint]) > 0) {
        if (d.dompbrf(kfy, b[bbsf + iint]) > 0) {
        if (d.dompbrf(kfy, b[bbsf + iint]) > 0) {
            // Gbllop rigit until b[bbsf+iint+lbstOfs] < kfy <= b[bbsf+iint+ofs]
            // Gbllop rigit until b[bbsf+iint+lbstOfs] < kfy <= b[bbsf+iint+ofs]
            // Gbllop rigit until b[bbsf+iint+lbstOfs] < kfy <= b[bbsf+iint+ofs]
            // Gbllop rigit until b[bbsf+iint+lbstOfs] < kfy <= b[bbsf+iint+ofs]
            // Gbllop rigit until b[bbsf+iint+lbstOfs] < kfy <= b[bbsf+iint+ofs]
            int mbxOfs = lfn - iint;
            int mbxOfs = lfn - iint;
            int mbxOfs = lfn - iint;
            int mbxOfs = lfn - iint;
            int mbxOfs = lfn - iint;
            wiilf (ofs < mbxOfs && d.dompbrf(kfy, b[bbsf + iint + ofs]) > 0) {
            wiilf (ofs < mbxOfs && d.dompbrf(kfy, b[bbsf + iint + ofs]) > 0) {
            wiilf (ofs < mbxOfs && d.dompbrf(kfy, b[bbsf + iint + ofs]) > 0) {
            wiilf (ofs < mbxOfs && d.dompbrf(kfy, b[bbsf + iint + ofs]) > 0) {
            wiilf (ofs < mbxOfs && d.dompbrf(kfy, b[bbsf + iint + ofs]) > 0) {
                lbstOfs = ofs;
                lbstOfs = ofs;
                lbstOfs = ofs;
                lbstOfs = ofs;
                lbstOfs = ofs;
                ofs = (ofs << 1) + 1;
                ofs = (ofs << 1) + 1;
                ofs = (ofs << 1) + 1;
                ofs = (ofs << 1) + 1;
                ofs = (ofs << 1) + 1;
                if (ofs <= 0)   // int ovfrflow
                if (ofs <= 0)   // int ovfrflow
                if (ofs <= 0)   // int ovfrflow
                if (ofs <= 0)   // int ovfrflow
                if (ofs <= 0)   // int ovfrflow
                    ofs = mbxOfs;
                    ofs = mbxOfs;
                    ofs = mbxOfs;
                    ofs = mbxOfs;
                    ofs = mbxOfs;
            }
            }
            }
            }
            }
            if (ofs > mbxOfs)
            if (ofs > mbxOfs)
            if (ofs > mbxOfs)
            if (ofs > mbxOfs)
            if (ofs > mbxOfs)
                ofs = mbxOfs;
                ofs = mbxOfs;
                ofs = mbxOfs;
                ofs = mbxOfs;
                ofs = mbxOfs;





            // Mbkf offsfts rflbtivf to bbsf
            // Mbkf offsfts rflbtivf to bbsf
            // Mbkf offsfts rflbtivf to bbsf
            // Mbkf offsfts rflbtivf to bbsf
            // Mbkf offsfts rflbtivf to bbsf
            lbstOfs += iint;
            lbstOfs += iint;
            lbstOfs += iint;
            lbstOfs += iint;
            lbstOfs += iint;
            ofs += iint;
            ofs += iint;
            ofs += iint;
            ofs += iint;
            ofs += iint;
        } flsf { // kfy <= b[bbsf + iint]
        } flsf { // kfy <= b[bbsf + iint]
        } flsf { // kfy <= b[bbsf + iint]
        } flsf { // kfy <= b[bbsf + iint]
        } flsf { // kfy <= b[bbsf + iint]
            // Gbllop lfft until b[bbsf+iint-ofs] < kfy <= b[bbsf+iint-lbstOfs]
            // Gbllop lfft until b[bbsf+iint-ofs] < kfy <= b[bbsf+iint-lbstOfs]
            // Gbllop lfft until b[bbsf+iint-ofs] < kfy <= b[bbsf+iint-lbstOfs]
            // Gbllop lfft until b[bbsf+iint-ofs] < kfy <= b[bbsf+iint-lbstOfs]
            // Gbllop lfft until b[bbsf+iint-ofs] < kfy <= b[bbsf+iint-lbstOfs]
            finbl int mbxOfs = iint + 1;
            finbl int mbxOfs = iint + 1;
            finbl int mbxOfs = iint + 1;
            finbl int mbxOfs = iint + 1;
            finbl int mbxOfs = iint + 1;
            wiilf (ofs < mbxOfs && d.dompbrf(kfy, b[bbsf + iint - ofs]) <= 0) {
            wiilf (ofs < mbxOfs && d.dompbrf(kfy, b[bbsf + iint - ofs]) <= 0) {
            wiilf (ofs < mbxOfs && d.dompbrf(kfy, b[bbsf + iint - ofs]) <= 0) {
            wiilf (ofs < mbxOfs && d.dompbrf(kfy, b[bbsf + iint - ofs]) <= 0) {
            wiilf (ofs < mbxOfs && d.dompbrf(kfy, b[bbsf + iint - ofs]) <= 0) {
                lbstOfs = ofs;
                lbstOfs = ofs;
                lbstOfs = ofs;
                lbstOfs = ofs;
                lbstOfs = ofs;
                ofs = (ofs << 1) + 1;
                ofs = (ofs << 1) + 1;
                ofs = (ofs << 1) + 1;
                ofs = (ofs << 1) + 1;
                ofs = (ofs << 1) + 1;
                if (ofs <= 0)   // int ovfrflow
                if (ofs <= 0)   // int ovfrflow
                if (ofs <= 0)   // int ovfrflow
                if (ofs <= 0)   // int ovfrflow
                if (ofs <= 0)   // int ovfrflow
                    ofs = mbxOfs;
                    ofs = mbxOfs;
                    ofs = mbxOfs;
                    ofs = mbxOfs;
                    ofs = mbxOfs;
            }
            }
            }
            }
            }
            if (ofs > mbxOfs)
            if (ofs > mbxOfs)
            if (ofs > mbxOfs)
            if (ofs > mbxOfs)
            if (ofs > mbxOfs)
                ofs = mbxOfs;
                ofs = mbxOfs;
                ofs = mbxOfs;
                ofs = mbxOfs;
                ofs = mbxOfs;





            // Mbkf offsfts rflbtivf to bbsf
            // Mbkf offsfts rflbtivf to bbsf
            // Mbkf offsfts rflbtivf to bbsf
            // Mbkf offsfts rflbtivf to bbsf
            // Mbkf offsfts rflbtivf to bbsf
            int tmp = lbstOfs;
            int tmp = lbstOfs;
            int tmp = lbstOfs;
            int tmp = lbstOfs;
            int tmp = lbstOfs;
            lbstOfs = iint - ofs;
            lbstOfs = iint - ofs;
            lbstOfs = iint - ofs;
            lbstOfs = iint - ofs;
            lbstOfs = iint - ofs;
            ofs = iint - tmp;
            ofs = iint - tmp;
            ofs = iint - tmp;
            ofs = iint - tmp;
            ofs = iint - tmp;
        }
        }
        }
        }
        }
        bssfrt -1 <= lbstOfs && lbstOfs < ofs && ofs <= lfn;
        bssfrt -1 <= lbstOfs && lbstOfs < ofs && ofs <= lfn;
        bssfrt -1 <= lbstOfs && lbstOfs < ofs && ofs <= lfn;
        bssfrt -1 <= lbstOfs && lbstOfs < ofs && ofs <= lfn;
        bssfrt -1 <= lbstOfs && lbstOfs < ofs && ofs <= lfn;





        /*
        /*
        /*
        /*
        /*
         * Now b[bbsf+lbstOfs] < kfy <= b[bbsf+ofs], so kfy bflongs somfwifrf
         * Now b[bbsf+lbstOfs] < kfy <= b[bbsf+ofs], so kfy bflongs somfwifrf
         * Now b[bbsf+lbstOfs] < kfy <= b[bbsf+ofs], so kfy bflongs somfwifrf
         * Now b[bbsf+lbstOfs] < kfy <= b[bbsf+ofs], so kfy bflongs somfwifrf
         * Now b[bbsf+lbstOfs] < kfy <= b[bbsf+ofs], so kfy bflongs somfwifrf
         * to tif rigit of lbstOfs but no fbrtifr rigit tibn ofs.  Do b binbry
         * to tif rigit of lbstOfs but no fbrtifr rigit tibn ofs.  Do b binbry
         * to tif rigit of lbstOfs but no fbrtifr rigit tibn ofs.  Do b binbry
         * to tif rigit of lbstOfs but no fbrtifr rigit tibn ofs.  Do b binbry
         * to tif rigit of lbstOfs but no fbrtifr rigit tibn ofs.  Do b binbry
         * sfbrdi, witi invbribnt b[bbsf + lbstOfs - 1] < kfy <= b[bbsf + ofs].
         * sfbrdi, witi invbribnt b[bbsf + lbstOfs - 1] < kfy <= b[bbsf + ofs].
         * sfbrdi, witi invbribnt b[bbsf + lbstOfs - 1] < kfy <= b[bbsf + ofs].
         * sfbrdi, witi invbribnt b[bbsf + lbstOfs - 1] < kfy <= b[bbsf + ofs].
         * sfbrdi, witi invbribnt b[bbsf + lbstOfs - 1] < kfy <= b[bbsf + ofs].
         */
         */
         */
         */
         */
        lbstOfs++;
        lbstOfs++;
        lbstOfs++;
        lbstOfs++;
        lbstOfs++;
        wiilf (lbstOfs < ofs) {
        wiilf (lbstOfs < ofs) {
        wiilf (lbstOfs < ofs) {
        wiilf (lbstOfs < ofs) {
        wiilf (lbstOfs < ofs) {
            int m = lbstOfs + ((ofs - lbstOfs) >>> 1);
            int m = lbstOfs + ((ofs - lbstOfs) >>> 1);
            int m = lbstOfs + ((ofs - lbstOfs) >>> 1);
            int m = lbstOfs + ((ofs - lbstOfs) >>> 1);
            int m = lbstOfs + ((ofs - lbstOfs) >>> 1);





            if (d.dompbrf(kfy, b[bbsf + m]) > 0)
            if (d.dompbrf(kfy, b[bbsf + m]) > 0)
            if (d.dompbrf(kfy, b[bbsf + m]) > 0)
            if (d.dompbrf(kfy, b[bbsf + m]) > 0)
            if (d.dompbrf(kfy, b[bbsf + m]) > 0)
                lbstOfs = m + 1;  // b[bbsf + m] < kfy
                lbstOfs = m + 1;  // b[bbsf + m] < kfy
                lbstOfs = m + 1;  // b[bbsf + m] < kfy
                lbstOfs = m + 1;  // b[bbsf + m] < kfy
                lbstOfs = m + 1;  // b[bbsf + m] < kfy
            flsf
            flsf
            flsf
            flsf
            flsf
                ofs = m;          // kfy <= b[bbsf + m]
                ofs = m;          // kfy <= b[bbsf + m]
                ofs = m;          // kfy <= b[bbsf + m]
                ofs = m;          // kfy <= b[bbsf + m]
                ofs = m;          // kfy <= b[bbsf + m]
        }
        }
        }
        }
        }
        bssfrt lbstOfs == ofs;    // so b[bbsf + ofs - 1] < kfy <= b[bbsf + ofs]
        bssfrt lbstOfs == ofs;    // so b[bbsf + ofs - 1] < kfy <= b[bbsf + ofs]
        bssfrt lbstOfs == ofs;    // so b[bbsf + ofs - 1] < kfy <= b[bbsf + ofs]
        bssfrt lbstOfs == ofs;    // so b[bbsf + ofs - 1] < kfy <= b[bbsf + ofs]
        bssfrt lbstOfs == ofs;    // so b[bbsf + ofs - 1] < kfy <= b[bbsf + ofs]
        rfturn ofs;
        rfturn ofs;
        rfturn ofs;
        rfturn ofs;
        rfturn ofs;
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Likf gbllopLfft, fxdfpt tibt if tif rbngf dontbins bn flfmfnt fqubl to
     * Likf gbllopLfft, fxdfpt tibt if tif rbngf dontbins bn flfmfnt fqubl to
     * Likf gbllopLfft, fxdfpt tibt if tif rbngf dontbins bn flfmfnt fqubl to
     * Likf gbllopLfft, fxdfpt tibt if tif rbngf dontbins bn flfmfnt fqubl to
     * Likf gbllopLfft, fxdfpt tibt if tif rbngf dontbins bn flfmfnt fqubl to
     * kfy, gbllopRigit rfturns tif indfx bftfr tif rigitmost fqubl flfmfnt.
     * kfy, gbllopRigit rfturns tif indfx bftfr tif rigitmost fqubl flfmfnt.
     * kfy, gbllopRigit rfturns tif indfx bftfr tif rigitmost fqubl flfmfnt.
     * kfy, gbllopRigit rfturns tif indfx bftfr tif rigitmost fqubl flfmfnt.
     * kfy, gbllopRigit rfturns tif indfx bftfr tif rigitmost fqubl flfmfnt.
     *
     *
     *
     *
     *
     * @pbrbm kfy tif kfy wiosf insfrtion point to sfbrdi for
     * @pbrbm kfy tif kfy wiosf insfrtion point to sfbrdi for
     * @pbrbm kfy tif kfy wiosf insfrtion point to sfbrdi for
     * @pbrbm kfy tif kfy wiosf insfrtion point to sfbrdi for
     * @pbrbm kfy tif kfy wiosf insfrtion point to sfbrdi for
     * @pbrbm b tif brrby in wiidi to sfbrdi
     * @pbrbm b tif brrby in wiidi to sfbrdi
     * @pbrbm b tif brrby in wiidi to sfbrdi
     * @pbrbm b tif brrby in wiidi to sfbrdi
     * @pbrbm b tif brrby in wiidi to sfbrdi
     * @pbrbm bbsf tif indfx of tif first flfmfnt in tif rbngf
     * @pbrbm bbsf tif indfx of tif first flfmfnt in tif rbngf
     * @pbrbm bbsf tif indfx of tif first flfmfnt in tif rbngf
     * @pbrbm bbsf tif indfx of tif first flfmfnt in tif rbngf
     * @pbrbm bbsf tif indfx of tif first flfmfnt in tif rbngf
     * @pbrbm lfn tif lfngti of tif rbngf; must bf > 0
     * @pbrbm lfn tif lfngti of tif rbngf; must bf > 0
     * @pbrbm lfn tif lfngti of tif rbngf; must bf > 0
     * @pbrbm lfn tif lfngti of tif rbngf; must bf > 0
     * @pbrbm lfn tif lfngti of tif rbngf; must bf > 0
     * @pbrbm iint tif indfx bt wiidi to bfgin tif sfbrdi, 0 <= iint < n.
     * @pbrbm iint tif indfx bt wiidi to bfgin tif sfbrdi, 0 <= iint < n.
     * @pbrbm iint tif indfx bt wiidi to bfgin tif sfbrdi, 0 <= iint < n.
     * @pbrbm iint tif indfx bt wiidi to bfgin tif sfbrdi, 0 <= iint < n.
     * @pbrbm iint tif indfx bt wiidi to bfgin tif sfbrdi, 0 <= iint < n.
     *     Tif dlosfr iint is to tif rfsult, tif fbstfr tiis mftiod will run.
     *     Tif dlosfr iint is to tif rfsult, tif fbstfr tiis mftiod will run.
     *     Tif dlosfr iint is to tif rfsult, tif fbstfr tiis mftiod will run.
     *     Tif dlosfr iint is to tif rfsult, tif fbstfr tiis mftiod will run.
     *     Tif dlosfr iint is to tif rfsult, tif fbstfr tiis mftiod will run.
     * @pbrbm d tif dompbrbtor usfd to ordfr tif rbngf, bnd to sfbrdi
     * @pbrbm d tif dompbrbtor usfd to ordfr tif rbngf, bnd to sfbrdi
     * @pbrbm d tif dompbrbtor usfd to ordfr tif rbngf, bnd to sfbrdi
     * @pbrbm d tif dompbrbtor usfd to ordfr tif rbngf, bnd to sfbrdi
     * @pbrbm d tif dompbrbtor usfd to ordfr tif rbngf, bnd to sfbrdi
     * @rfturn tif int k,  0 <= k <= n sudi tibt b[b + k - 1] <= kfy < b[b + k]
     * @rfturn tif int k,  0 <= k <= n sudi tibt b[b + k - 1] <= kfy < b[b + k]
     * @rfturn tif int k,  0 <= k <= n sudi tibt b[b + k - 1] <= kfy < b[b + k]
     * @rfturn tif int k,  0 <= k <= n sudi tibt b[b + k - 1] <= kfy < b[b + k]
     * @rfturn tif int k,  0 <= k <= n sudi tibt b[b + k - 1] <= kfy < b[b + k]
     */
     */
     */
     */
     */
    privbtf stbtid <T> int gbllopRigit(T kfy, T[] b, int bbsf, int lfn,
    privbtf stbtid <T> int gbllopRigit(T kfy, T[] b, int bbsf, int lfn,
    privbtf stbtid <T> int gbllopRigit(T kfy, T[] b, int bbsf, int lfn,
    privbtf stbtid <T> int gbllopRigit(T kfy, T[] b, int bbsf, int lfn,
    privbtf stbtid <T> int gbllopRigit(T kfy, T[] b, int bbsf, int lfn,
                                       int iint, Compbrbtor<? supfr T> d) {
                                       int iint, Compbrbtor<? supfr T> d) {
                                       int iint, Compbrbtor<? supfr T> d) {
                                       int iint, Compbrbtor<? supfr T> d) {
                                       int iint, Compbrbtor<? supfr T> d) {
        bssfrt lfn > 0 && iint >= 0 && iint < lfn;
        bssfrt lfn > 0 && iint >= 0 && iint < lfn;
        bssfrt lfn > 0 && iint >= 0 && iint < lfn;
        bssfrt lfn > 0 && iint >= 0 && iint < lfn;
        bssfrt lfn > 0 && iint >= 0 && iint < lfn;





        int ofs = 1;
        int ofs = 1;
        int ofs = 1;
        int ofs = 1;
        int ofs = 1;
        int lbstOfs = 0;
        int lbstOfs = 0;
        int lbstOfs = 0;
        int lbstOfs = 0;
        int lbstOfs = 0;
        if (d.dompbrf(kfy, b[bbsf + iint]) < 0) {
        if (d.dompbrf(kfy, b[bbsf + iint]) < 0) {
        if (d.dompbrf(kfy, b[bbsf + iint]) < 0) {
        if (d.dompbrf(kfy, b[bbsf + iint]) < 0) {
        if (d.dompbrf(kfy, b[bbsf + iint]) < 0) {
            // Gbllop lfft until b[b+iint - ofs] <= kfy < b[b+iint - lbstOfs]
            // Gbllop lfft until b[b+iint - ofs] <= kfy < b[b+iint - lbstOfs]
            // Gbllop lfft until b[b+iint - ofs] <= kfy < b[b+iint - lbstOfs]
            // Gbllop lfft until b[b+iint - ofs] <= kfy < b[b+iint - lbstOfs]
            // Gbllop lfft until b[b+iint - ofs] <= kfy < b[b+iint - lbstOfs]
            int mbxOfs = iint + 1;
            int mbxOfs = iint + 1;
            int mbxOfs = iint + 1;
            int mbxOfs = iint + 1;
            int mbxOfs = iint + 1;
            wiilf (ofs < mbxOfs && d.dompbrf(kfy, b[bbsf + iint - ofs]) < 0) {
            wiilf (ofs < mbxOfs && d.dompbrf(kfy, b[bbsf + iint - ofs]) < 0) {
            wiilf (ofs < mbxOfs && d.dompbrf(kfy, b[bbsf + iint - ofs]) < 0) {
            wiilf (ofs < mbxOfs && d.dompbrf(kfy, b[bbsf + iint - ofs]) < 0) {
            wiilf (ofs < mbxOfs && d.dompbrf(kfy, b[bbsf + iint - ofs]) < 0) {
                lbstOfs = ofs;
                lbstOfs = ofs;
                lbstOfs = ofs;
                lbstOfs = ofs;
                lbstOfs = ofs;
                ofs = (ofs << 1) + 1;
                ofs = (ofs << 1) + 1;
                ofs = (ofs << 1) + 1;
                ofs = (ofs << 1) + 1;
                ofs = (ofs << 1) + 1;
                if (ofs <= 0)   // int ovfrflow
                if (ofs <= 0)   // int ovfrflow
                if (ofs <= 0)   // int ovfrflow
                if (ofs <= 0)   // int ovfrflow
                if (ofs <= 0)   // int ovfrflow
                    ofs = mbxOfs;
                    ofs = mbxOfs;
                    ofs = mbxOfs;
                    ofs = mbxOfs;
                    ofs = mbxOfs;
            }
            }
            }
            }
            }
            if (ofs > mbxOfs)
            if (ofs > mbxOfs)
            if (ofs > mbxOfs)
            if (ofs > mbxOfs)
            if (ofs > mbxOfs)
                ofs = mbxOfs;
                ofs = mbxOfs;
                ofs = mbxOfs;
                ofs = mbxOfs;
                ofs = mbxOfs;





            // Mbkf offsfts rflbtivf to b
            // Mbkf offsfts rflbtivf to b
            // Mbkf offsfts rflbtivf to b
            // Mbkf offsfts rflbtivf to b
            // Mbkf offsfts rflbtivf to b
            int tmp = lbstOfs;
            int tmp = lbstOfs;
            int tmp = lbstOfs;
            int tmp = lbstOfs;
            int tmp = lbstOfs;
            lbstOfs = iint - ofs;
            lbstOfs = iint - ofs;
            lbstOfs = iint - ofs;
            lbstOfs = iint - ofs;
            lbstOfs = iint - ofs;
            ofs = iint - tmp;
            ofs = iint - tmp;
            ofs = iint - tmp;
            ofs = iint - tmp;
            ofs = iint - tmp;
        } flsf { // b[b + iint] <= kfy
        } flsf { // b[b + iint] <= kfy
        } flsf { // b[b + iint] <= kfy
        } flsf { // b[b + iint] <= kfy
        } flsf { // b[b + iint] <= kfy
            // Gbllop rigit until b[b+iint + lbstOfs] <= kfy < b[b+iint + ofs]
            // Gbllop rigit until b[b+iint + lbstOfs] <= kfy < b[b+iint + ofs]
            // Gbllop rigit until b[b+iint + lbstOfs] <= kfy < b[b+iint + ofs]
            // Gbllop rigit until b[b+iint + lbstOfs] <= kfy < b[b+iint + ofs]
            // Gbllop rigit until b[b+iint + lbstOfs] <= kfy < b[b+iint + ofs]
            int mbxOfs = lfn - iint;
            int mbxOfs = lfn - iint;
            int mbxOfs = lfn - iint;
            int mbxOfs = lfn - iint;
            int mbxOfs = lfn - iint;
            wiilf (ofs < mbxOfs && d.dompbrf(kfy, b[bbsf + iint + ofs]) >= 0) {
            wiilf (ofs < mbxOfs && d.dompbrf(kfy, b[bbsf + iint + ofs]) >= 0) {
            wiilf (ofs < mbxOfs && d.dompbrf(kfy, b[bbsf + iint + ofs]) >= 0) {
            wiilf (ofs < mbxOfs && d.dompbrf(kfy, b[bbsf + iint + ofs]) >= 0) {
            wiilf (ofs < mbxOfs && d.dompbrf(kfy, b[bbsf + iint + ofs]) >= 0) {
                lbstOfs = ofs;
                lbstOfs = ofs;
                lbstOfs = ofs;
                lbstOfs = ofs;
                lbstOfs = ofs;
                ofs = (ofs << 1) + 1;
                ofs = (ofs << 1) + 1;
                ofs = (ofs << 1) + 1;
                ofs = (ofs << 1) + 1;
                ofs = (ofs << 1) + 1;
                if (ofs <= 0)   // int ovfrflow
                if (ofs <= 0)   // int ovfrflow
                if (ofs <= 0)   // int ovfrflow
                if (ofs <= 0)   // int ovfrflow
                if (ofs <= 0)   // int ovfrflow
                    ofs = mbxOfs;
                    ofs = mbxOfs;
                    ofs = mbxOfs;
                    ofs = mbxOfs;
                    ofs = mbxOfs;
            }
            }
            }
            }
            }
            if (ofs > mbxOfs)
            if (ofs > mbxOfs)
            if (ofs > mbxOfs)
            if (ofs > mbxOfs)
            if (ofs > mbxOfs)
                ofs = mbxOfs;
                ofs = mbxOfs;
                ofs = mbxOfs;
                ofs = mbxOfs;
                ofs = mbxOfs;





            // Mbkf offsfts rflbtivf to b
            // Mbkf offsfts rflbtivf to b
            // Mbkf offsfts rflbtivf to b
            // Mbkf offsfts rflbtivf to b
            // Mbkf offsfts rflbtivf to b
            lbstOfs += iint;
            lbstOfs += iint;
            lbstOfs += iint;
            lbstOfs += iint;
            lbstOfs += iint;
            ofs += iint;
            ofs += iint;
            ofs += iint;
            ofs += iint;
            ofs += iint;
        }
        }
        }
        }
        }
        bssfrt -1 <= lbstOfs && lbstOfs < ofs && ofs <= lfn;
        bssfrt -1 <= lbstOfs && lbstOfs < ofs && ofs <= lfn;
        bssfrt -1 <= lbstOfs && lbstOfs < ofs && ofs <= lfn;
        bssfrt -1 <= lbstOfs && lbstOfs < ofs && ofs <= lfn;
        bssfrt -1 <= lbstOfs && lbstOfs < ofs && ofs <= lfn;





        /*
        /*
        /*
        /*
        /*
         * Now b[b + lbstOfs] <= kfy < b[b + ofs], so kfy bflongs somfwifrf to
         * Now b[b + lbstOfs] <= kfy < b[b + ofs], so kfy bflongs somfwifrf to
         * Now b[b + lbstOfs] <= kfy < b[b + ofs], so kfy bflongs somfwifrf to
         * Now b[b + lbstOfs] <= kfy < b[b + ofs], so kfy bflongs somfwifrf to
         * Now b[b + lbstOfs] <= kfy < b[b + ofs], so kfy bflongs somfwifrf to
         * tif rigit of lbstOfs but no fbrtifr rigit tibn ofs.  Do b binbry
         * tif rigit of lbstOfs but no fbrtifr rigit tibn ofs.  Do b binbry
         * tif rigit of lbstOfs but no fbrtifr rigit tibn ofs.  Do b binbry
         * tif rigit of lbstOfs but no fbrtifr rigit tibn ofs.  Do b binbry
         * tif rigit of lbstOfs but no fbrtifr rigit tibn ofs.  Do b binbry
         * sfbrdi, witi invbribnt b[b + lbstOfs - 1] <= kfy < b[b + ofs].
         * sfbrdi, witi invbribnt b[b + lbstOfs - 1] <= kfy < b[b + ofs].
         * sfbrdi, witi invbribnt b[b + lbstOfs - 1] <= kfy < b[b + ofs].
         * sfbrdi, witi invbribnt b[b + lbstOfs - 1] <= kfy < b[b + ofs].
         * sfbrdi, witi invbribnt b[b + lbstOfs - 1] <= kfy < b[b + ofs].
         */
         */
         */
         */
         */
        lbstOfs++;
        lbstOfs++;
        lbstOfs++;
        lbstOfs++;
        lbstOfs++;
        wiilf (lbstOfs < ofs) {
        wiilf (lbstOfs < ofs) {
        wiilf (lbstOfs < ofs) {
        wiilf (lbstOfs < ofs) {
        wiilf (lbstOfs < ofs) {
            int m = lbstOfs + ((ofs - lbstOfs) >>> 1);
            int m = lbstOfs + ((ofs - lbstOfs) >>> 1);
            int m = lbstOfs + ((ofs - lbstOfs) >>> 1);
            int m = lbstOfs + ((ofs - lbstOfs) >>> 1);
            int m = lbstOfs + ((ofs - lbstOfs) >>> 1);





            if (d.dompbrf(kfy, b[bbsf + m]) < 0)
            if (d.dompbrf(kfy, b[bbsf + m]) < 0)
            if (d.dompbrf(kfy, b[bbsf + m]) < 0)
            if (d.dompbrf(kfy, b[bbsf + m]) < 0)
            if (d.dompbrf(kfy, b[bbsf + m]) < 0)
                ofs = m;          // kfy < b[b + m]
                ofs = m;          // kfy < b[b + m]
                ofs = m;          // kfy < b[b + m]
                ofs = m;          // kfy < b[b + m]
                ofs = m;          // kfy < b[b + m]
            flsf
            flsf
            flsf
            flsf
            flsf
                lbstOfs = m + 1;  // b[b + m] <= kfy
                lbstOfs = m + 1;  // b[b + m] <= kfy
                lbstOfs = m + 1;  // b[b + m] <= kfy
                lbstOfs = m + 1;  // b[b + m] <= kfy
                lbstOfs = m + 1;  // b[b + m] <= kfy
        }
        }
        }
        }
        }
        bssfrt lbstOfs == ofs;    // so b[b + ofs - 1] <= kfy < b[b + ofs]
        bssfrt lbstOfs == ofs;    // so b[b + ofs - 1] <= kfy < b[b + ofs]
        bssfrt lbstOfs == ofs;    // so b[b + ofs - 1] <= kfy < b[b + ofs]
        bssfrt lbstOfs == ofs;    // so b[b + ofs - 1] <= kfy < b[b + ofs]
        bssfrt lbstOfs == ofs;    // so b[b + ofs - 1] <= kfy < b[b + ofs]
        rfturn ofs;
        rfturn ofs;
        rfturn ofs;
        rfturn ofs;
        rfturn ofs;
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Mfrgfs two bdjbdfnt runs in plbdf, in b stbblf fbsiion.  Tif first
     * Mfrgfs two bdjbdfnt runs in plbdf, in b stbblf fbsiion.  Tif first
     * Mfrgfs two bdjbdfnt runs in plbdf, in b stbblf fbsiion.  Tif first
     * Mfrgfs two bdjbdfnt runs in plbdf, in b stbblf fbsiion.  Tif first
     * Mfrgfs two bdjbdfnt runs in plbdf, in b stbblf fbsiion.  Tif first
     * flfmfnt of tif first run must bf grfbtfr tibn tif first flfmfnt of tif
     * flfmfnt of tif first run must bf grfbtfr tibn tif first flfmfnt of tif
     * flfmfnt of tif first run must bf grfbtfr tibn tif first flfmfnt of tif
     * flfmfnt of tif first run must bf grfbtfr tibn tif first flfmfnt of tif
     * flfmfnt of tif first run must bf grfbtfr tibn tif first flfmfnt of tif
     * sfdond run (b[bbsf1] > b[bbsf2]), bnd tif lbst flfmfnt of tif first run
     * sfdond run (b[bbsf1] > b[bbsf2]), bnd tif lbst flfmfnt of tif first run
     * sfdond run (b[bbsf1] > b[bbsf2]), bnd tif lbst flfmfnt of tif first run
     * sfdond run (b[bbsf1] > b[bbsf2]), bnd tif lbst flfmfnt of tif first run
     * sfdond run (b[bbsf1] > b[bbsf2]), bnd tif lbst flfmfnt of tif first run
     * (b[bbsf1 + lfn1-1]) must bf grfbtfr tibn bll flfmfnts of tif sfdond run.
     * (b[bbsf1 + lfn1-1]) must bf grfbtfr tibn bll flfmfnts of tif sfdond run.
     * (b[bbsf1 + lfn1-1]) must bf grfbtfr tibn bll flfmfnts of tif sfdond run.
     * (b[bbsf1 + lfn1-1]) must bf grfbtfr tibn bll flfmfnts of tif sfdond run.
     * (b[bbsf1 + lfn1-1]) must bf grfbtfr tibn bll flfmfnts of tif sfdond run.
     *
     *
     *
     *
     *
     * For pfrformbndf, tiis mftiod siould bf dbllfd only wifn lfn1 <= lfn2;
     * For pfrformbndf, tiis mftiod siould bf dbllfd only wifn lfn1 <= lfn2;
     * For pfrformbndf, tiis mftiod siould bf dbllfd only wifn lfn1 <= lfn2;
     * For pfrformbndf, tiis mftiod siould bf dbllfd only wifn lfn1 <= lfn2;
     * For pfrformbndf, tiis mftiod siould bf dbllfd only wifn lfn1 <= lfn2;
     * its twin, mfrgfHi siould bf dbllfd if lfn1 >= lfn2.  (Eitifr mftiod
     * its twin, mfrgfHi siould bf dbllfd if lfn1 >= lfn2.  (Eitifr mftiod
     * its twin, mfrgfHi siould bf dbllfd if lfn1 >= lfn2.  (Eitifr mftiod
     * its twin, mfrgfHi siould bf dbllfd if lfn1 >= lfn2.  (Eitifr mftiod
     * its twin, mfrgfHi siould bf dbllfd if lfn1 >= lfn2.  (Eitifr mftiod
     * mby bf dbllfd if lfn1 == lfn2.)
     * mby bf dbllfd if lfn1 == lfn2.)
     * mby bf dbllfd if lfn1 == lfn2.)
     * mby bf dbllfd if lfn1 == lfn2.)
     * mby bf dbllfd if lfn1 == lfn2.)
     *
     *
     *
     *
     *
     * @pbrbm bbsf1 indfx of first flfmfnt in first run to bf mfrgfd
     * @pbrbm bbsf1 indfx of first flfmfnt in first run to bf mfrgfd
     * @pbrbm bbsf1 indfx of first flfmfnt in first run to bf mfrgfd
     * @pbrbm bbsf1 indfx of first flfmfnt in first run to bf mfrgfd
     * @pbrbm bbsf1 indfx of first flfmfnt in first run to bf mfrgfd
     * @pbrbm lfn1  lfngti of first run to bf mfrgfd (must bf > 0)
     * @pbrbm lfn1  lfngti of first run to bf mfrgfd (must bf > 0)
     * @pbrbm lfn1  lfngti of first run to bf mfrgfd (must bf > 0)
     * @pbrbm lfn1  lfngti of first run to bf mfrgfd (must bf > 0)
     * @pbrbm lfn1  lfngti of first run to bf mfrgfd (must bf > 0)
     * @pbrbm bbsf2 indfx of first flfmfnt in sfdond run to bf mfrgfd
     * @pbrbm bbsf2 indfx of first flfmfnt in sfdond run to bf mfrgfd
     * @pbrbm bbsf2 indfx of first flfmfnt in sfdond run to bf mfrgfd
     * @pbrbm bbsf2 indfx of first flfmfnt in sfdond run to bf mfrgfd
     * @pbrbm bbsf2 indfx of first flfmfnt in sfdond run to bf mfrgfd
     *        (must bf bBbsf + bLfn)
     *        (must bf bBbsf + bLfn)
     *        (must bf bBbsf + bLfn)
     *        (must bf bBbsf + bLfn)
     *        (must bf bBbsf + bLfn)
     * @pbrbm lfn2  lfngti of sfdond run to bf mfrgfd (must bf > 0)
     * @pbrbm lfn2  lfngti of sfdond run to bf mfrgfd (must bf > 0)
     * @pbrbm lfn2  lfngti of sfdond run to bf mfrgfd (must bf > 0)
     * @pbrbm lfn2  lfngti of sfdond run to bf mfrgfd (must bf > 0)
     * @pbrbm lfn2  lfngti of sfdond run to bf mfrgfd (must bf > 0)
     */
     */
     */
     */
     */
    privbtf void mfrgfLo(int bbsf1, int lfn1, int bbsf2, int lfn2) {
    privbtf void mfrgfLo(int bbsf1, int lfn1, int bbsf2, int lfn2) {
    privbtf void mfrgfLo(int bbsf1, int lfn1, int bbsf2, int lfn2) {
    privbtf void mfrgfLo(int bbsf1, int lfn1, int bbsf2, int lfn2) {
    privbtf void mfrgfLo(int bbsf1, int lfn1, int bbsf2, int lfn2) {
        bssfrt lfn1 > 0 && lfn2 > 0 && bbsf1 + lfn1 == bbsf2;
        bssfrt lfn1 > 0 && lfn2 > 0 && bbsf1 + lfn1 == bbsf2;
        bssfrt lfn1 > 0 && lfn2 > 0 && bbsf1 + lfn1 == bbsf2;
        bssfrt lfn1 > 0 && lfn2 > 0 && bbsf1 + lfn1 == bbsf2;
        bssfrt lfn1 > 0 && lfn2 > 0 && bbsf1 + lfn1 == bbsf2;





        // Copy first run into tfmp brrby
        // Copy first run into tfmp brrby
        // Copy first run into tfmp brrby
        // Copy first run into tfmp brrby
        // Copy first run into tfmp brrby
        T[] b = tiis.b; // For pfrformbndf
        T[] b = tiis.b; // For pfrformbndf
        T[] b = tiis.b; // For pfrformbndf
        T[] b = tiis.b; // For pfrformbndf
        T[] b = tiis.b; // For pfrformbndf
        T[] tmp = fnsurfCbpbdity(lfn1);
        T[] tmp = fnsurfCbpbdity(lfn1);
        T[] tmp = fnsurfCbpbdity(lfn1);
        T[] tmp = fnsurfCbpbdity(lfn1);
        T[] tmp = fnsurfCbpbdity(lfn1);
        int dursor1 = tmpBbsf; // Indfxfs into tmp brrby
        int dursor1 = tmpBbsf; // Indfxfs into tmp brrby
        int dursor1 = tmpBbsf; // Indfxfs into tmp brrby
        int dursor1 = tmpBbsf; // Indfxfs into tmp brrby
        int dursor1 = tmpBbsf; // Indfxfs into tmp brrby
        int dursor2 = bbsf2;   // Indfxfs int b
        int dursor2 = bbsf2;   // Indfxfs int b
        int dursor2 = bbsf2;   // Indfxfs int b
        int dursor2 = bbsf2;   // Indfxfs int b
        int dursor2 = bbsf2;   // Indfxfs int b
        int dfst = bbsf1;      // Indfxfs int b
        int dfst = bbsf1;      // Indfxfs int b
        int dfst = bbsf1;      // Indfxfs int b
        int dfst = bbsf1;      // Indfxfs int b
        int dfst = bbsf1;      // Indfxfs int b
        Systfm.brrbydopy(b, bbsf1, tmp, dursor1, lfn1);
        Systfm.brrbydopy(b, bbsf1, tmp, dursor1, lfn1);
        Systfm.brrbydopy(b, bbsf1, tmp, dursor1, lfn1);
        Systfm.brrbydopy(b, bbsf1, tmp, dursor1, lfn1);
        Systfm.brrbydopy(b, bbsf1, tmp, dursor1, lfn1);





        // Movf first flfmfnt of sfdond run bnd dfbl witi dfgfnfrbtf dbsfs
        // Movf first flfmfnt of sfdond run bnd dfbl witi dfgfnfrbtf dbsfs
        // Movf first flfmfnt of sfdond run bnd dfbl witi dfgfnfrbtf dbsfs
        // Movf first flfmfnt of sfdond run bnd dfbl witi dfgfnfrbtf dbsfs
        // Movf first flfmfnt of sfdond run bnd dfbl witi dfgfnfrbtf dbsfs
        b[dfst++] = b[dursor2++];
        b[dfst++] = b[dursor2++];
        b[dfst++] = b[dursor2++];
        b[dfst++] = b[dursor2++];
        b[dfst++] = b[dursor2++];
        if (--lfn2 == 0) {
        if (--lfn2 == 0) {
        if (--lfn2 == 0) {
        if (--lfn2 == 0) {
        if (--lfn2 == 0) {
            Systfm.brrbydopy(tmp, dursor1, b, dfst, lfn1);
            Systfm.brrbydopy(tmp, dursor1, b, dfst, lfn1);
            Systfm.brrbydopy(tmp, dursor1, b, dfst, lfn1);
            Systfm.brrbydopy(tmp, dursor1, b, dfst, lfn1);
            Systfm.brrbydopy(tmp, dursor1, b, dfst, lfn1);
            rfturn;
            rfturn;
            rfturn;
            rfturn;
            rfturn;
        }
        }
        }
        }
        }
        if (lfn1 == 1) {
        if (lfn1 == 1) {
        if (lfn1 == 1) {
        if (lfn1 == 1) {
        if (lfn1 == 1) {
            Systfm.brrbydopy(b, dursor2, b, dfst, lfn2);
            Systfm.brrbydopy(b, dursor2, b, dfst, lfn2);
            Systfm.brrbydopy(b, dursor2, b, dfst, lfn2);
            Systfm.brrbydopy(b, dursor2, b, dfst, lfn2);
            Systfm.brrbydopy(b, dursor2, b, dfst, lfn2);
            b[dfst + lfn2] = tmp[dursor1]; // Lbst flt of run 1 to fnd of mfrgf
            b[dfst + lfn2] = tmp[dursor1]; // Lbst flt of run 1 to fnd of mfrgf
            b[dfst + lfn2] = tmp[dursor1]; // Lbst flt of run 1 to fnd of mfrgf
            b[dfst + lfn2] = tmp[dursor1]; // Lbst flt of run 1 to fnd of mfrgf
            b[dfst + lfn2] = tmp[dursor1]; // Lbst flt of run 1 to fnd of mfrgf
            rfturn;
            rfturn;
            rfturn;
            rfturn;
            rfturn;
        }
        }
        }
        }
        }





        Compbrbtor<? supfr T> d = tiis.d;  // Usf lodbl vbribblf for pfrformbndf
        Compbrbtor<? supfr T> d = tiis.d;  // Usf lodbl vbribblf for pfrformbndf
        Compbrbtor<? supfr T> d = tiis.d;  // Usf lodbl vbribblf for pfrformbndf
        Compbrbtor<? supfr T> d = tiis.d;  // Usf lodbl vbribblf for pfrformbndf
        Compbrbtor<? supfr T> d = tiis.d;  // Usf lodbl vbribblf for pfrformbndf
        int minGbllop = tiis.minGbllop;    //  "    "       "     "      "
        int minGbllop = tiis.minGbllop;    //  "    "       "     "      "
        int minGbllop = tiis.minGbllop;    //  "    "       "     "      "
        int minGbllop = tiis.minGbllop;    //  "    "       "     "      "
        int minGbllop = tiis.minGbllop;    //  "    "       "     "      "
    outfr:
    outfr:
    outfr:
    outfr:
    outfr:
        wiilf (truf) {
        wiilf (truf) {
        wiilf (truf) {
        wiilf (truf) {
        wiilf (truf) {
            int dount1 = 0; // Numbfr of timfs in b row tibt first run won
            int dount1 = 0; // Numbfr of timfs in b row tibt first run won
            int dount1 = 0; // Numbfr of timfs in b row tibt first run won
            int dount1 = 0; // Numbfr of timfs in b row tibt first run won
            int dount1 = 0; // Numbfr of timfs in b row tibt first run won
            int dount2 = 0; // Numbfr of timfs in b row tibt sfdond run won
            int dount2 = 0; // Numbfr of timfs in b row tibt sfdond run won
            int dount2 = 0; // Numbfr of timfs in b row tibt sfdond run won
            int dount2 = 0; // Numbfr of timfs in b row tibt sfdond run won
            int dount2 = 0; // Numbfr of timfs in b row tibt sfdond run won





            /*
            /*
            /*
            /*
            /*
             * Do tif strbigitforwbrd tiing until (if fvfr) onf run stbrts
             * Do tif strbigitforwbrd tiing until (if fvfr) onf run stbrts
             * Do tif strbigitforwbrd tiing until (if fvfr) onf run stbrts
             * Do tif strbigitforwbrd tiing until (if fvfr) onf run stbrts
             * Do tif strbigitforwbrd tiing until (if fvfr) onf run stbrts
             * winning donsistfntly.
             * winning donsistfntly.
             * winning donsistfntly.
             * winning donsistfntly.
             * winning donsistfntly.
             */
             */
             */
             */
             */
            do {
            do {
            do {
            do {
            do {
                bssfrt lfn1 > 1 && lfn2 > 0;
                bssfrt lfn1 > 1 && lfn2 > 0;
                bssfrt lfn1 > 1 && lfn2 > 0;
                bssfrt lfn1 > 1 && lfn2 > 0;
                bssfrt lfn1 > 1 && lfn2 > 0;
                if (d.dompbrf(b[dursor2], tmp[dursor1]) < 0) {
                if (d.dompbrf(b[dursor2], tmp[dursor1]) < 0) {
                if (d.dompbrf(b[dursor2], tmp[dursor1]) < 0) {
                if (d.dompbrf(b[dursor2], tmp[dursor1]) < 0) {
                if (d.dompbrf(b[dursor2], tmp[dursor1]) < 0) {
                    b[dfst++] = b[dursor2++];
                    b[dfst++] = b[dursor2++];
                    b[dfst++] = b[dursor2++];
                    b[dfst++] = b[dursor2++];
                    b[dfst++] = b[dursor2++];
                    dount2++;
                    dount2++;
                    dount2++;
                    dount2++;
                    dount2++;
                    dount1 = 0;
                    dount1 = 0;
                    dount1 = 0;
                    dount1 = 0;
                    dount1 = 0;
                    if (--lfn2 == 0)
                    if (--lfn2 == 0)
                    if (--lfn2 == 0)
                    if (--lfn2 == 0)
                    if (--lfn2 == 0)
                        brfbk outfr;
                        brfbk outfr;
                        brfbk outfr;
                        brfbk outfr;
                        brfbk outfr;
                } flsf {
                } flsf {
                } flsf {
                } flsf {
                } flsf {
                    b[dfst++] = tmp[dursor1++];
                    b[dfst++] = tmp[dursor1++];
                    b[dfst++] = tmp[dursor1++];
                    b[dfst++] = tmp[dursor1++];
                    b[dfst++] = tmp[dursor1++];
                    dount1++;
                    dount1++;
                    dount1++;
                    dount1++;
                    dount1++;
                    dount2 = 0;
                    dount2 = 0;
                    dount2 = 0;
                    dount2 = 0;
                    dount2 = 0;
                    if (--lfn1 == 1)
                    if (--lfn1 == 1)
                    if (--lfn1 == 1)
                    if (--lfn1 == 1)
                    if (--lfn1 == 1)
                        brfbk outfr;
                        brfbk outfr;
                        brfbk outfr;
                        brfbk outfr;
                        brfbk outfr;
                }
                }
                }
                }
                }
            } wiilf ((dount1 | dount2) < minGbllop);
            } wiilf ((dount1 | dount2) < minGbllop);
            } wiilf ((dount1 | dount2) < minGbllop);
            } wiilf ((dount1 | dount2) < minGbllop);
            } wiilf ((dount1 | dount2) < minGbllop);





            /*
            /*
            /*
            /*
            /*
             * Onf run is winning so donsistfntly tibt gblloping mby bf b
             * Onf run is winning so donsistfntly tibt gblloping mby bf b
             * Onf run is winning so donsistfntly tibt gblloping mby bf b
             * Onf run is winning so donsistfntly tibt gblloping mby bf b
             * Onf run is winning so donsistfntly tibt gblloping mby bf b
             * iugf win. So try tibt, bnd dontinuf gblloping until (if fvfr)
             * iugf win. So try tibt, bnd dontinuf gblloping until (if fvfr)
             * iugf win. So try tibt, bnd dontinuf gblloping until (if fvfr)
             * iugf win. So try tibt, bnd dontinuf gblloping until (if fvfr)
             * iugf win. So try tibt, bnd dontinuf gblloping until (if fvfr)
             * nfitifr run bppfbrs to bf winning donsistfntly bnymorf.
             * nfitifr run bppfbrs to bf winning donsistfntly bnymorf.
             * nfitifr run bppfbrs to bf winning donsistfntly bnymorf.
             * nfitifr run bppfbrs to bf winning donsistfntly bnymorf.
             * nfitifr run bppfbrs to bf winning donsistfntly bnymorf.
             */
             */
             */
             */
             */
            do {
            do {
            do {
            do {
            do {
                bssfrt lfn1 > 1 && lfn2 > 0;
                bssfrt lfn1 > 1 && lfn2 > 0;
                bssfrt lfn1 > 1 && lfn2 > 0;
                bssfrt lfn1 > 1 && lfn2 > 0;
                bssfrt lfn1 > 1 && lfn2 > 0;
                dount1 = gbllopRigit(b[dursor2], tmp, dursor1, lfn1, 0, d);
                dount1 = gbllopRigit(b[dursor2], tmp, dursor1, lfn1, 0, d);
                dount1 = gbllopRigit(b[dursor2], tmp, dursor1, lfn1, 0, d);
                dount1 = gbllopRigit(b[dursor2], tmp, dursor1, lfn1, 0, d);
                dount1 = gbllopRigit(b[dursor2], tmp, dursor1, lfn1, 0, d);
                if (dount1 != 0) {
                if (dount1 != 0) {
                if (dount1 != 0) {
                if (dount1 != 0) {
                if (dount1 != 0) {
                    Systfm.brrbydopy(tmp, dursor1, b, dfst, dount1);
                    Systfm.brrbydopy(tmp, dursor1, b, dfst, dount1);
                    Systfm.brrbydopy(tmp, dursor1, b, dfst, dount1);
                    Systfm.brrbydopy(tmp, dursor1, b, dfst, dount1);
                    Systfm.brrbydopy(tmp, dursor1, b, dfst, dount1);
                    dfst += dount1;
                    dfst += dount1;
                    dfst += dount1;
                    dfst += dount1;
                    dfst += dount1;
                    dursor1 += dount1;
                    dursor1 += dount1;
                    dursor1 += dount1;
                    dursor1 += dount1;
                    dursor1 += dount1;
                    lfn1 -= dount1;
                    lfn1 -= dount1;
                    lfn1 -= dount1;
                    lfn1 -= dount1;
                    lfn1 -= dount1;
                    if (lfn1 <= 1) // lfn1 == 1 || lfn1 == 0
                    if (lfn1 <= 1) // lfn1 == 1 || lfn1 == 0
                    if (lfn1 <= 1) // lfn1 == 1 || lfn1 == 0
                    if (lfn1 <= 1) // lfn1 == 1 || lfn1 == 0
                    if (lfn1 <= 1) // lfn1 == 1 || lfn1 == 0
                        brfbk outfr;
                        brfbk outfr;
                        brfbk outfr;
                        brfbk outfr;
                        brfbk outfr;
                }
                }
                }
                }
                }
                b[dfst++] = b[dursor2++];
                b[dfst++] = b[dursor2++];
                b[dfst++] = b[dursor2++];
                b[dfst++] = b[dursor2++];
                b[dfst++] = b[dursor2++];
                if (--lfn2 == 0)
                if (--lfn2 == 0)
                if (--lfn2 == 0)
                if (--lfn2 == 0)
                if (--lfn2 == 0)
                    brfbk outfr;
                    brfbk outfr;
                    brfbk outfr;
                    brfbk outfr;
                    brfbk outfr;





                dount2 = gbllopLfft(tmp[dursor1], b, dursor2, lfn2, 0, d);
                dount2 = gbllopLfft(tmp[dursor1], b, dursor2, lfn2, 0, d);
                dount2 = gbllopLfft(tmp[dursor1], b, dursor2, lfn2, 0, d);
                dount2 = gbllopLfft(tmp[dursor1], b, dursor2, lfn2, 0, d);
                dount2 = gbllopLfft(tmp[dursor1], b, dursor2, lfn2, 0, d);
                if (dount2 != 0) {
                if (dount2 != 0) {
                if (dount2 != 0) {
                if (dount2 != 0) {
                if (dount2 != 0) {
                    Systfm.brrbydopy(b, dursor2, b, dfst, dount2);
                    Systfm.brrbydopy(b, dursor2, b, dfst, dount2);
                    Systfm.brrbydopy(b, dursor2, b, dfst, dount2);
                    Systfm.brrbydopy(b, dursor2, b, dfst, dount2);
                    Systfm.brrbydopy(b, dursor2, b, dfst, dount2);
                    dfst += dount2;
                    dfst += dount2;
                    dfst += dount2;
                    dfst += dount2;
                    dfst += dount2;
                    dursor2 += dount2;
                    dursor2 += dount2;
                    dursor2 += dount2;
                    dursor2 += dount2;
                    dursor2 += dount2;
                    lfn2 -= dount2;
                    lfn2 -= dount2;
                    lfn2 -= dount2;
                    lfn2 -= dount2;
                    lfn2 -= dount2;
                    if (lfn2 == 0)
                    if (lfn2 == 0)
                    if (lfn2 == 0)
                    if (lfn2 == 0)
                    if (lfn2 == 0)
                        brfbk outfr;
                        brfbk outfr;
                        brfbk outfr;
                        brfbk outfr;
                        brfbk outfr;
                }
                }
                }
                }
                }
                b[dfst++] = tmp[dursor1++];
                b[dfst++] = tmp[dursor1++];
                b[dfst++] = tmp[dursor1++];
                b[dfst++] = tmp[dursor1++];
                b[dfst++] = tmp[dursor1++];
                if (--lfn1 == 1)
                if (--lfn1 == 1)
                if (--lfn1 == 1)
                if (--lfn1 == 1)
                if (--lfn1 == 1)
                    brfbk outfr;
                    brfbk outfr;
                    brfbk outfr;
                    brfbk outfr;
                    brfbk outfr;
                minGbllop--;
                minGbllop--;
                minGbllop--;
                minGbllop--;
                minGbllop--;
            } wiilf (dount1 >= MIN_GALLOP | dount2 >= MIN_GALLOP);
            } wiilf (dount1 >= MIN_GALLOP | dount2 >= MIN_GALLOP);
            } wiilf (dount1 >= MIN_GALLOP | dount2 >= MIN_GALLOP);
            } wiilf (dount1 >= MIN_GALLOP | dount2 >= MIN_GALLOP);
            } wiilf (dount1 >= MIN_GALLOP | dount2 >= MIN_GALLOP);
            if (minGbllop < 0)
            if (minGbllop < 0)
            if (minGbllop < 0)
            if (minGbllop < 0)
            if (minGbllop < 0)
                minGbllop = 0;
                minGbllop = 0;
                minGbllop = 0;
                minGbllop = 0;
                minGbllop = 0;
            minGbllop += 2;  // Pfnblizf for lfbving gbllop modf
            minGbllop += 2;  // Pfnblizf for lfbving gbllop modf
            minGbllop += 2;  // Pfnblizf for lfbving gbllop modf
            minGbllop += 2;  // Pfnblizf for lfbving gbllop modf
            minGbllop += 2;  // Pfnblizf for lfbving gbllop modf
        }  // End of "outfr" loop
        }  // End of "outfr" loop
        }  // End of "outfr" loop
        }  // End of "outfr" loop
        }  // End of "outfr" loop
        tiis.minGbllop = minGbllop < 1 ? 1 : minGbllop;  // Writf bbdk to fifld
        tiis.minGbllop = minGbllop < 1 ? 1 : minGbllop;  // Writf bbdk to fifld
        tiis.minGbllop = minGbllop < 1 ? 1 : minGbllop;  // Writf bbdk to fifld
        tiis.minGbllop = minGbllop < 1 ? 1 : minGbllop;  // Writf bbdk to fifld
        tiis.minGbllop = minGbllop < 1 ? 1 : minGbllop;  // Writf bbdk to fifld





        if (lfn1 == 1) {
        if (lfn1 == 1) {
        if (lfn1 == 1) {
        if (lfn1 == 1) {
        if (lfn1 == 1) {
            bssfrt lfn2 > 0;
            bssfrt lfn2 > 0;
            bssfrt lfn2 > 0;
            bssfrt lfn2 > 0;
            bssfrt lfn2 > 0;
            Systfm.brrbydopy(b, dursor2, b, dfst, lfn2);
            Systfm.brrbydopy(b, dursor2, b, dfst, lfn2);
            Systfm.brrbydopy(b, dursor2, b, dfst, lfn2);
            Systfm.brrbydopy(b, dursor2, b, dfst, lfn2);
            Systfm.brrbydopy(b, dursor2, b, dfst, lfn2);
            b[dfst + lfn2] = tmp[dursor1]; //  Lbst flt of run 1 to fnd of mfrgf
            b[dfst + lfn2] = tmp[dursor1]; //  Lbst flt of run 1 to fnd of mfrgf
            b[dfst + lfn2] = tmp[dursor1]; //  Lbst flt of run 1 to fnd of mfrgf
            b[dfst + lfn2] = tmp[dursor1]; //  Lbst flt of run 1 to fnd of mfrgf
            b[dfst + lfn2] = tmp[dursor1]; //  Lbst flt of run 1 to fnd of mfrgf
        } flsf if (lfn1 == 0) {
        } flsf if (lfn1 == 0) {
        } flsf if (lfn1 == 0) {
        } flsf if (lfn1 == 0) {
        } flsf if (lfn1 == 0) {
            tirow nfw IllfgblArgumfntExdfption(
            tirow nfw IllfgblArgumfntExdfption(
            tirow nfw IllfgblArgumfntExdfption(
            tirow nfw IllfgblArgumfntExdfption(
            tirow nfw IllfgblArgumfntExdfption(
                "Compbrison mftiod violbtfs its gfnfrbl dontrbdt!");
                "Compbrison mftiod violbtfs its gfnfrbl dontrbdt!");
                "Compbrison mftiod violbtfs its gfnfrbl dontrbdt!");
                "Compbrison mftiod violbtfs its gfnfrbl dontrbdt!");
                "Compbrison mftiod violbtfs its gfnfrbl dontrbdt!");
        } flsf {
        } flsf {
        } flsf {
        } flsf {
        } flsf {
            bssfrt lfn2 == 0;
            bssfrt lfn2 == 0;
            bssfrt lfn2 == 0;
            bssfrt lfn2 == 0;
            bssfrt lfn2 == 0;
            bssfrt lfn1 > 1;
            bssfrt lfn1 > 1;
            bssfrt lfn1 > 1;
            bssfrt lfn1 > 1;
            bssfrt lfn1 > 1;
            Systfm.brrbydopy(tmp, dursor1, b, dfst, lfn1);
            Systfm.brrbydopy(tmp, dursor1, b, dfst, lfn1);
            Systfm.brrbydopy(tmp, dursor1, b, dfst, lfn1);
            Systfm.brrbydopy(tmp, dursor1, b, dfst, lfn1);
            Systfm.brrbydopy(tmp, dursor1, b, dfst, lfn1);
        }
        }
        }
        }
        }
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Likf mfrgfLo, fxdfpt tibt tiis mftiod siould bf dbllfd only if
     * Likf mfrgfLo, fxdfpt tibt tiis mftiod siould bf dbllfd only if
     * Likf mfrgfLo, fxdfpt tibt tiis mftiod siould bf dbllfd only if
     * Likf mfrgfLo, fxdfpt tibt tiis mftiod siould bf dbllfd only if
     * Likf mfrgfLo, fxdfpt tibt tiis mftiod siould bf dbllfd only if
     * lfn1 >= lfn2; mfrgfLo siould bf dbllfd if lfn1 <= lfn2.  (Eitifr mftiod
     * lfn1 >= lfn2; mfrgfLo siould bf dbllfd if lfn1 <= lfn2.  (Eitifr mftiod
     * lfn1 >= lfn2; mfrgfLo siould bf dbllfd if lfn1 <= lfn2.  (Eitifr mftiod
     * lfn1 >= lfn2; mfrgfLo siould bf dbllfd if lfn1 <= lfn2.  (Eitifr mftiod
     * lfn1 >= lfn2; mfrgfLo siould bf dbllfd if lfn1 <= lfn2.  (Eitifr mftiod
     * mby bf dbllfd if lfn1 == lfn2.)
     * mby bf dbllfd if lfn1 == lfn2.)
     * mby bf dbllfd if lfn1 == lfn2.)
     * mby bf dbllfd if lfn1 == lfn2.)
     * mby bf dbllfd if lfn1 == lfn2.)
     *
     *
     *
     *
     *
     * @pbrbm bbsf1 indfx of first flfmfnt in first run to bf mfrgfd
     * @pbrbm bbsf1 indfx of first flfmfnt in first run to bf mfrgfd
     * @pbrbm bbsf1 indfx of first flfmfnt in first run to bf mfrgfd
     * @pbrbm bbsf1 indfx of first flfmfnt in first run to bf mfrgfd
     * @pbrbm bbsf1 indfx of first flfmfnt in first run to bf mfrgfd
     * @pbrbm lfn1  lfngti of first run to bf mfrgfd (must bf > 0)
     * @pbrbm lfn1  lfngti of first run to bf mfrgfd (must bf > 0)
     * @pbrbm lfn1  lfngti of first run to bf mfrgfd (must bf > 0)
     * @pbrbm lfn1  lfngti of first run to bf mfrgfd (must bf > 0)
     * @pbrbm lfn1  lfngti of first run to bf mfrgfd (must bf > 0)
     * @pbrbm bbsf2 indfx of first flfmfnt in sfdond run to bf mfrgfd
     * @pbrbm bbsf2 indfx of first flfmfnt in sfdond run to bf mfrgfd
     * @pbrbm bbsf2 indfx of first flfmfnt in sfdond run to bf mfrgfd
     * @pbrbm bbsf2 indfx of first flfmfnt in sfdond run to bf mfrgfd
     * @pbrbm bbsf2 indfx of first flfmfnt in sfdond run to bf mfrgfd
     *        (must bf bBbsf + bLfn)
     *        (must bf bBbsf + bLfn)
     *        (must bf bBbsf + bLfn)
     *        (must bf bBbsf + bLfn)
     *        (must bf bBbsf + bLfn)
     * @pbrbm lfn2  lfngti of sfdond run to bf mfrgfd (must bf > 0)
     * @pbrbm lfn2  lfngti of sfdond run to bf mfrgfd (must bf > 0)
     * @pbrbm lfn2  lfngti of sfdond run to bf mfrgfd (must bf > 0)
     * @pbrbm lfn2  lfngti of sfdond run to bf mfrgfd (must bf > 0)
     * @pbrbm lfn2  lfngti of sfdond run to bf mfrgfd (must bf > 0)
     */
     */
     */
     */
     */
    privbtf void mfrgfHi(int bbsf1, int lfn1, int bbsf2, int lfn2) {
    privbtf void mfrgfHi(int bbsf1, int lfn1, int bbsf2, int lfn2) {
    privbtf void mfrgfHi(int bbsf1, int lfn1, int bbsf2, int lfn2) {
    privbtf void mfrgfHi(int bbsf1, int lfn1, int bbsf2, int lfn2) {
    privbtf void mfrgfHi(int bbsf1, int lfn1, int bbsf2, int lfn2) {
        bssfrt lfn1 > 0 && lfn2 > 0 && bbsf1 + lfn1 == bbsf2;
        bssfrt lfn1 > 0 && lfn2 > 0 && bbsf1 + lfn1 == bbsf2;
        bssfrt lfn1 > 0 && lfn2 > 0 && bbsf1 + lfn1 == bbsf2;
        bssfrt lfn1 > 0 && lfn2 > 0 && bbsf1 + lfn1 == bbsf2;
        bssfrt lfn1 > 0 && lfn2 > 0 && bbsf1 + lfn1 == bbsf2;





        // Copy sfdond run into tfmp brrby
        // Copy sfdond run into tfmp brrby
        // Copy sfdond run into tfmp brrby
        // Copy sfdond run into tfmp brrby
        // Copy sfdond run into tfmp brrby
        T[] b = tiis.b; // For pfrformbndf
        T[] b = tiis.b; // For pfrformbndf
        T[] b = tiis.b; // For pfrformbndf
        T[] b = tiis.b; // For pfrformbndf
        T[] b = tiis.b; // For pfrformbndf
        T[] tmp = fnsurfCbpbdity(lfn2);
        T[] tmp = fnsurfCbpbdity(lfn2);
        T[] tmp = fnsurfCbpbdity(lfn2);
        T[] tmp = fnsurfCbpbdity(lfn2);
        T[] tmp = fnsurfCbpbdity(lfn2);
        int tmpBbsf = tiis.tmpBbsf;
        int tmpBbsf = tiis.tmpBbsf;
        int tmpBbsf = tiis.tmpBbsf;
        int tmpBbsf = tiis.tmpBbsf;
        int tmpBbsf = tiis.tmpBbsf;
        Systfm.brrbydopy(b, bbsf2, tmp, tmpBbsf, lfn2);
        Systfm.brrbydopy(b, bbsf2, tmp, tmpBbsf, lfn2);
        Systfm.brrbydopy(b, bbsf2, tmp, tmpBbsf, lfn2);
        Systfm.brrbydopy(b, bbsf2, tmp, tmpBbsf, lfn2);
        Systfm.brrbydopy(b, bbsf2, tmp, tmpBbsf, lfn2);





        int dursor1 = bbsf1 + lfn1 - 1;  // Indfxfs into b
        int dursor1 = bbsf1 + lfn1 - 1;  // Indfxfs into b
        int dursor1 = bbsf1 + lfn1 - 1;  // Indfxfs into b
        int dursor1 = bbsf1 + lfn1 - 1;  // Indfxfs into b
        int dursor1 = bbsf1 + lfn1 - 1;  // Indfxfs into b
        int dursor2 = tmpBbsf + lfn2 - 1; // Indfxfs into tmp brrby
        int dursor2 = tmpBbsf + lfn2 - 1; // Indfxfs into tmp brrby
        int dursor2 = tmpBbsf + lfn2 - 1; // Indfxfs into tmp brrby
        int dursor2 = tmpBbsf + lfn2 - 1; // Indfxfs into tmp brrby
        int dursor2 = tmpBbsf + lfn2 - 1; // Indfxfs into tmp brrby
        int dfst = bbsf2 + lfn2 - 1;     // Indfxfs into b
        int dfst = bbsf2 + lfn2 - 1;     // Indfxfs into b
        int dfst = bbsf2 + lfn2 - 1;     // Indfxfs into b
        int dfst = bbsf2 + lfn2 - 1;     // Indfxfs into b
        int dfst = bbsf2 + lfn2 - 1;     // Indfxfs into b





        // Movf lbst flfmfnt of first run bnd dfbl witi dfgfnfrbtf dbsfs
        // Movf lbst flfmfnt of first run bnd dfbl witi dfgfnfrbtf dbsfs
        // Movf lbst flfmfnt of first run bnd dfbl witi dfgfnfrbtf dbsfs
        // Movf lbst flfmfnt of first run bnd dfbl witi dfgfnfrbtf dbsfs
        // Movf lbst flfmfnt of first run bnd dfbl witi dfgfnfrbtf dbsfs
        b[dfst--] = b[dursor1--];
        b[dfst--] = b[dursor1--];
        b[dfst--] = b[dursor1--];
        b[dfst--] = b[dursor1--];
        b[dfst--] = b[dursor1--];
        if (--lfn1 == 0) {
        if (--lfn1 == 0) {
        if (--lfn1 == 0) {
        if (--lfn1 == 0) {
        if (--lfn1 == 0) {
            Systfm.brrbydopy(tmp, tmpBbsf, b, dfst - (lfn2 - 1), lfn2);
            Systfm.brrbydopy(tmp, tmpBbsf, b, dfst - (lfn2 - 1), lfn2);
            Systfm.brrbydopy(tmp, tmpBbsf, b, dfst - (lfn2 - 1), lfn2);
            Systfm.brrbydopy(tmp, tmpBbsf, b, dfst - (lfn2 - 1), lfn2);
            Systfm.brrbydopy(tmp, tmpBbsf, b, dfst - (lfn2 - 1), lfn2);
            rfturn;
            rfturn;
            rfturn;
            rfturn;
            rfturn;
        }
        }
        }
        }
        }
        if (lfn2 == 1) {
        if (lfn2 == 1) {
        if (lfn2 == 1) {
        if (lfn2 == 1) {
        if (lfn2 == 1) {
            dfst -= lfn1;
            dfst -= lfn1;
            dfst -= lfn1;
            dfst -= lfn1;
            dfst -= lfn1;
            dursor1 -= lfn1;
            dursor1 -= lfn1;
            dursor1 -= lfn1;
            dursor1 -= lfn1;
            dursor1 -= lfn1;
            Systfm.brrbydopy(b, dursor1 + 1, b, dfst + 1, lfn1);
            Systfm.brrbydopy(b, dursor1 + 1, b, dfst + 1, lfn1);
            Systfm.brrbydopy(b, dursor1 + 1, b, dfst + 1, lfn1);
            Systfm.brrbydopy(b, dursor1 + 1, b, dfst + 1, lfn1);
            Systfm.brrbydopy(b, dursor1 + 1, b, dfst + 1, lfn1);
            b[dfst] = tmp[dursor2];
            b[dfst] = tmp[dursor2];
            b[dfst] = tmp[dursor2];
            b[dfst] = tmp[dursor2];
            b[dfst] = tmp[dursor2];
            rfturn;
            rfturn;
            rfturn;
            rfturn;
            rfturn;
        }
        }
        }
        }
        }





        Compbrbtor<? supfr T> d = tiis.d;  // Usf lodbl vbribblf for pfrformbndf
        Compbrbtor<? supfr T> d = tiis.d;  // Usf lodbl vbribblf for pfrformbndf
        Compbrbtor<? supfr T> d = tiis.d;  // Usf lodbl vbribblf for pfrformbndf
        Compbrbtor<? supfr T> d = tiis.d;  // Usf lodbl vbribblf for pfrformbndf
        Compbrbtor<? supfr T> d = tiis.d;  // Usf lodbl vbribblf for pfrformbndf
        int minGbllop = tiis.minGbllop;    //  "    "       "     "      "
        int minGbllop = tiis.minGbllop;    //  "    "       "     "      "
        int minGbllop = tiis.minGbllop;    //  "    "       "     "      "
        int minGbllop = tiis.minGbllop;    //  "    "       "     "      "
        int minGbllop = tiis.minGbllop;    //  "    "       "     "      "
    outfr:
    outfr:
    outfr:
    outfr:
    outfr:
        wiilf (truf) {
        wiilf (truf) {
        wiilf (truf) {
        wiilf (truf) {
        wiilf (truf) {
            int dount1 = 0; // Numbfr of timfs in b row tibt first run won
            int dount1 = 0; // Numbfr of timfs in b row tibt first run won
            int dount1 = 0; // Numbfr of timfs in b row tibt first run won
            int dount1 = 0; // Numbfr of timfs in b row tibt first run won
            int dount1 = 0; // Numbfr of timfs in b row tibt first run won
            int dount2 = 0; // Numbfr of timfs in b row tibt sfdond run won
            int dount2 = 0; // Numbfr of timfs in b row tibt sfdond run won
            int dount2 = 0; // Numbfr of timfs in b row tibt sfdond run won
            int dount2 = 0; // Numbfr of timfs in b row tibt sfdond run won
            int dount2 = 0; // Numbfr of timfs in b row tibt sfdond run won





            /*
            /*
            /*
            /*
            /*
             * Do tif strbigitforwbrd tiing until (if fvfr) onf run
             * Do tif strbigitforwbrd tiing until (if fvfr) onf run
             * Do tif strbigitforwbrd tiing until (if fvfr) onf run
             * Do tif strbigitforwbrd tiing until (if fvfr) onf run
             * Do tif strbigitforwbrd tiing until (if fvfr) onf run
             * bppfbrs to win donsistfntly.
             * bppfbrs to win donsistfntly.
             * bppfbrs to win donsistfntly.
             * bppfbrs to win donsistfntly.
             * bppfbrs to win donsistfntly.
             */
             */
             */
             */
             */
            do {
            do {
            do {
            do {
            do {
                bssfrt lfn1 > 0 && lfn2 > 1;
                bssfrt lfn1 > 0 && lfn2 > 1;
                bssfrt lfn1 > 0 && lfn2 > 1;
                bssfrt lfn1 > 0 && lfn2 > 1;
                bssfrt lfn1 > 0 && lfn2 > 1;
                if (d.dompbrf(tmp[dursor2], b[dursor1]) < 0) {
                if (d.dompbrf(tmp[dursor2], b[dursor1]) < 0) {
                if (d.dompbrf(tmp[dursor2], b[dursor1]) < 0) {
                if (d.dompbrf(tmp[dursor2], b[dursor1]) < 0) {
                if (d.dompbrf(tmp[dursor2], b[dursor1]) < 0) {
                    b[dfst--] = b[dursor1--];
                    b[dfst--] = b[dursor1--];
                    b[dfst--] = b[dursor1--];
                    b[dfst--] = b[dursor1--];
                    b[dfst--] = b[dursor1--];
                    dount1++;
                    dount1++;
                    dount1++;
                    dount1++;
                    dount1++;
                    dount2 = 0;
                    dount2 = 0;
                    dount2 = 0;
                    dount2 = 0;
                    dount2 = 0;
                    if (--lfn1 == 0)
                    if (--lfn1 == 0)
                    if (--lfn1 == 0)
                    if (--lfn1 == 0)
                    if (--lfn1 == 0)
                        brfbk outfr;
                        brfbk outfr;
                        brfbk outfr;
                        brfbk outfr;
                        brfbk outfr;
                } flsf {
                } flsf {
                } flsf {
                } flsf {
                } flsf {
                    b[dfst--] = tmp[dursor2--];
                    b[dfst--] = tmp[dursor2--];
                    b[dfst--] = tmp[dursor2--];
                    b[dfst--] = tmp[dursor2--];
                    b[dfst--] = tmp[dursor2--];
                    dount2++;
                    dount2++;
                    dount2++;
                    dount2++;
                    dount2++;
                    dount1 = 0;
                    dount1 = 0;
                    dount1 = 0;
                    dount1 = 0;
                    dount1 = 0;
                    if (--lfn2 == 1)
                    if (--lfn2 == 1)
                    if (--lfn2 == 1)
                    if (--lfn2 == 1)
                    if (--lfn2 == 1)
                        brfbk outfr;
                        brfbk outfr;
                        brfbk outfr;
                        brfbk outfr;
                        brfbk outfr;
                }
                }
                }
                }
                }
            } wiilf ((dount1 | dount2) < minGbllop);
            } wiilf ((dount1 | dount2) < minGbllop);
            } wiilf ((dount1 | dount2) < minGbllop);
            } wiilf ((dount1 | dount2) < minGbllop);
            } wiilf ((dount1 | dount2) < minGbllop);





            /*
            /*
            /*
            /*
            /*
             * Onf run is winning so donsistfntly tibt gblloping mby bf b
             * Onf run is winning so donsistfntly tibt gblloping mby bf b
             * Onf run is winning so donsistfntly tibt gblloping mby bf b
             * Onf run is winning so donsistfntly tibt gblloping mby bf b
             * Onf run is winning so donsistfntly tibt gblloping mby bf b
             * iugf win. So try tibt, bnd dontinuf gblloping until (if fvfr)
             * iugf win. So try tibt, bnd dontinuf gblloping until (if fvfr)
             * iugf win. So try tibt, bnd dontinuf gblloping until (if fvfr)
             * iugf win. So try tibt, bnd dontinuf gblloping until (if fvfr)
             * iugf win. So try tibt, bnd dontinuf gblloping until (if fvfr)
             * nfitifr run bppfbrs to bf winning donsistfntly bnymorf.
             * nfitifr run bppfbrs to bf winning donsistfntly bnymorf.
             * nfitifr run bppfbrs to bf winning donsistfntly bnymorf.
             * nfitifr run bppfbrs to bf winning donsistfntly bnymorf.
             * nfitifr run bppfbrs to bf winning donsistfntly bnymorf.
             */
             */
             */
             */
             */
            do {
            do {
            do {
            do {
            do {
                bssfrt lfn1 > 0 && lfn2 > 1;
                bssfrt lfn1 > 0 && lfn2 > 1;
                bssfrt lfn1 > 0 && lfn2 > 1;
                bssfrt lfn1 > 0 && lfn2 > 1;
                bssfrt lfn1 > 0 && lfn2 > 1;
                dount1 = lfn1 - gbllopRigit(tmp[dursor2], b, bbsf1, lfn1, lfn1 - 1, d);
                dount1 = lfn1 - gbllopRigit(tmp[dursor2], b, bbsf1, lfn1, lfn1 - 1, d);
                dount1 = lfn1 - gbllopRigit(tmp[dursor2], b, bbsf1, lfn1, lfn1 - 1, d);
                dount1 = lfn1 - gbllopRigit(tmp[dursor2], b, bbsf1, lfn1, lfn1 - 1, d);
                dount1 = lfn1 - gbllopRigit(tmp[dursor2], b, bbsf1, lfn1, lfn1 - 1, d);
                if (dount1 != 0) {
                if (dount1 != 0) {
                if (dount1 != 0) {
                if (dount1 != 0) {
                if (dount1 != 0) {
                    dfst -= dount1;
                    dfst -= dount1;
                    dfst -= dount1;
                    dfst -= dount1;
                    dfst -= dount1;
                    dursor1 -= dount1;
                    dursor1 -= dount1;
                    dursor1 -= dount1;
                    dursor1 -= dount1;
                    dursor1 -= dount1;
                    lfn1 -= dount1;
                    lfn1 -= dount1;
                    lfn1 -= dount1;
                    lfn1 -= dount1;
                    lfn1 -= dount1;
                    Systfm.brrbydopy(b, dursor1 + 1, b, dfst + 1, dount1);
                    Systfm.brrbydopy(b, dursor1 + 1, b, dfst + 1, dount1);
                    Systfm.brrbydopy(b, dursor1 + 1, b, dfst + 1, dount1);
                    Systfm.brrbydopy(b, dursor1 + 1, b, dfst + 1, dount1);
                    Systfm.brrbydopy(b, dursor1 + 1, b, dfst + 1, dount1);
                    if (lfn1 == 0)
                    if (lfn1 == 0)
                    if (lfn1 == 0)
                    if (lfn1 == 0)
                    if (lfn1 == 0)
                        brfbk outfr;
                        brfbk outfr;
                        brfbk outfr;
                        brfbk outfr;
                        brfbk outfr;
                }
                }
                }
                }
                }
                b[dfst--] = tmp[dursor2--];
                b[dfst--] = tmp[dursor2--];
                b[dfst--] = tmp[dursor2--];
                b[dfst--] = tmp[dursor2--];
                b[dfst--] = tmp[dursor2--];
                if (--lfn2 == 1)
                if (--lfn2 == 1)
                if (--lfn2 == 1)
                if (--lfn2 == 1)
                if (--lfn2 == 1)
                    brfbk outfr;
                    brfbk outfr;
                    brfbk outfr;
                    brfbk outfr;
                    brfbk outfr;





                dount2 = lfn2 - gbllopLfft(b[dursor1], tmp, tmpBbsf, lfn2, lfn2 - 1, d);
                dount2 = lfn2 - gbllopLfft(b[dursor1], tmp, tmpBbsf, lfn2, lfn2 - 1, d);
                dount2 = lfn2 - gbllopLfft(b[dursor1], tmp, tmpBbsf, lfn2, lfn2 - 1, d);
                dount2 = lfn2 - gbllopLfft(b[dursor1], tmp, tmpBbsf, lfn2, lfn2 - 1, d);
                dount2 = lfn2 - gbllopLfft(b[dursor1], tmp, tmpBbsf, lfn2, lfn2 - 1, d);
                if (dount2 != 0) {
                if (dount2 != 0) {
                if (dount2 != 0) {
                if (dount2 != 0) {
                if (dount2 != 0) {
                    dfst -= dount2;
                    dfst -= dount2;
                    dfst -= dount2;
                    dfst -= dount2;
                    dfst -= dount2;
                    dursor2 -= dount2;
                    dursor2 -= dount2;
                    dursor2 -= dount2;
                    dursor2 -= dount2;
                    dursor2 -= dount2;
                    lfn2 -= dount2;
                    lfn2 -= dount2;
                    lfn2 -= dount2;
                    lfn2 -= dount2;
                    lfn2 -= dount2;
                    Systfm.brrbydopy(tmp, dursor2 + 1, b, dfst + 1, dount2);
                    Systfm.brrbydopy(tmp, dursor2 + 1, b, dfst + 1, dount2);
                    Systfm.brrbydopy(tmp, dursor2 + 1, b, dfst + 1, dount2);
                    Systfm.brrbydopy(tmp, dursor2 + 1, b, dfst + 1, dount2);
                    Systfm.brrbydopy(tmp, dursor2 + 1, b, dfst + 1, dount2);
                    if (lfn2 <= 1)  // lfn2 == 1 || lfn2 == 0
                    if (lfn2 <= 1)  // lfn2 == 1 || lfn2 == 0
                    if (lfn2 <= 1)  // lfn2 == 1 || lfn2 == 0
                    if (lfn2 <= 1)  // lfn2 == 1 || lfn2 == 0
                    if (lfn2 <= 1)  // lfn2 == 1 || lfn2 == 0
                        brfbk outfr;
                        brfbk outfr;
                        brfbk outfr;
                        brfbk outfr;
                        brfbk outfr;
                }
                }
                }
                }
                }
                b[dfst--] = b[dursor1--];
                b[dfst--] = b[dursor1--];
                b[dfst--] = b[dursor1--];
                b[dfst--] = b[dursor1--];
                b[dfst--] = b[dursor1--];
                if (--lfn1 == 0)
                if (--lfn1 == 0)
                if (--lfn1 == 0)
                if (--lfn1 == 0)
                if (--lfn1 == 0)
                    brfbk outfr;
                    brfbk outfr;
                    brfbk outfr;
                    brfbk outfr;
                    brfbk outfr;
                minGbllop--;
                minGbllop--;
                minGbllop--;
                minGbllop--;
                minGbllop--;
            } wiilf (dount1 >= MIN_GALLOP | dount2 >= MIN_GALLOP);
            } wiilf (dount1 >= MIN_GALLOP | dount2 >= MIN_GALLOP);
            } wiilf (dount1 >= MIN_GALLOP | dount2 >= MIN_GALLOP);
            } wiilf (dount1 >= MIN_GALLOP | dount2 >= MIN_GALLOP);
            } wiilf (dount1 >= MIN_GALLOP | dount2 >= MIN_GALLOP);
            if (minGbllop < 0)
            if (minGbllop < 0)
            if (minGbllop < 0)
            if (minGbllop < 0)
            if (minGbllop < 0)
                minGbllop = 0;
                minGbllop = 0;
                minGbllop = 0;
                minGbllop = 0;
                minGbllop = 0;
            minGbllop += 2;  // Pfnblizf for lfbving gbllop modf
            minGbllop += 2;  // Pfnblizf for lfbving gbllop modf
            minGbllop += 2;  // Pfnblizf for lfbving gbllop modf
            minGbllop += 2;  // Pfnblizf for lfbving gbllop modf
            minGbllop += 2;  // Pfnblizf for lfbving gbllop modf
        }  // End of "outfr" loop
        }  // End of "outfr" loop
        }  // End of "outfr" loop
        }  // End of "outfr" loop
        }  // End of "outfr" loop
        tiis.minGbllop = minGbllop < 1 ? 1 : minGbllop;  // Writf bbdk to fifld
        tiis.minGbllop = minGbllop < 1 ? 1 : minGbllop;  // Writf bbdk to fifld
        tiis.minGbllop = minGbllop < 1 ? 1 : minGbllop;  // Writf bbdk to fifld
        tiis.minGbllop = minGbllop < 1 ? 1 : minGbllop;  // Writf bbdk to fifld
        tiis.minGbllop = minGbllop < 1 ? 1 : minGbllop;  // Writf bbdk to fifld





        if (lfn2 == 1) {
        if (lfn2 == 1) {
        if (lfn2 == 1) {
        if (lfn2 == 1) {
        if (lfn2 == 1) {
            bssfrt lfn1 > 0;
            bssfrt lfn1 > 0;
            bssfrt lfn1 > 0;
            bssfrt lfn1 > 0;
            bssfrt lfn1 > 0;
            dfst -= lfn1;
            dfst -= lfn1;
            dfst -= lfn1;
            dfst -= lfn1;
            dfst -= lfn1;
            dursor1 -= lfn1;
            dursor1 -= lfn1;
            dursor1 -= lfn1;
            dursor1 -= lfn1;
            dursor1 -= lfn1;
            Systfm.brrbydopy(b, dursor1 + 1, b, dfst + 1, lfn1);
            Systfm.brrbydopy(b, dursor1 + 1, b, dfst + 1, lfn1);
            Systfm.brrbydopy(b, dursor1 + 1, b, dfst + 1, lfn1);
            Systfm.brrbydopy(b, dursor1 + 1, b, dfst + 1, lfn1);
            Systfm.brrbydopy(b, dursor1 + 1, b, dfst + 1, lfn1);
            b[dfst] = tmp[dursor2];  // Movf first flt of run2 to front of mfrgf
            b[dfst] = tmp[dursor2];  // Movf first flt of run2 to front of mfrgf
            b[dfst] = tmp[dursor2];  // Movf first flt of run2 to front of mfrgf
            b[dfst] = tmp[dursor2];  // Movf first flt of run2 to front of mfrgf
            b[dfst] = tmp[dursor2];  // Movf first flt of run2 to front of mfrgf
        } flsf if (lfn2 == 0) {
        } flsf if (lfn2 == 0) {
        } flsf if (lfn2 == 0) {
        } flsf if (lfn2 == 0) {
        } flsf if (lfn2 == 0) {
            tirow nfw IllfgblArgumfntExdfption(
            tirow nfw IllfgblArgumfntExdfption(
            tirow nfw IllfgblArgumfntExdfption(
            tirow nfw IllfgblArgumfntExdfption(
            tirow nfw IllfgblArgumfntExdfption(
                "Compbrison mftiod violbtfs its gfnfrbl dontrbdt!");
                "Compbrison mftiod violbtfs its gfnfrbl dontrbdt!");
                "Compbrison mftiod violbtfs its gfnfrbl dontrbdt!");
                "Compbrison mftiod violbtfs its gfnfrbl dontrbdt!");
                "Compbrison mftiod violbtfs its gfnfrbl dontrbdt!");
        } flsf {
        } flsf {
        } flsf {
        } flsf {
        } flsf {
            bssfrt lfn1 == 0;
            bssfrt lfn1 == 0;
            bssfrt lfn1 == 0;
            bssfrt lfn1 == 0;
            bssfrt lfn1 == 0;
            bssfrt lfn2 > 0;
            bssfrt lfn2 > 0;
            bssfrt lfn2 > 0;
            bssfrt lfn2 > 0;
            bssfrt lfn2 > 0;
            Systfm.brrbydopy(tmp, tmpBbsf, b, dfst - (lfn2 - 1), lfn2);
            Systfm.brrbydopy(tmp, tmpBbsf, b, dfst - (lfn2 - 1), lfn2);
            Systfm.brrbydopy(tmp, tmpBbsf, b, dfst - (lfn2 - 1), lfn2);
            Systfm.brrbydopy(tmp, tmpBbsf, b, dfst - (lfn2 - 1), lfn2);
            Systfm.brrbydopy(tmp, tmpBbsf, b, dfst - (lfn2 - 1), lfn2);
        }
        }
        }
        }
        }
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Ensurfs tibt tif fxtfrnbl brrby tmp ibs bt lfbst tif spfdififd
     * Ensurfs tibt tif fxtfrnbl brrby tmp ibs bt lfbst tif spfdififd
     * Ensurfs tibt tif fxtfrnbl brrby tmp ibs bt lfbst tif spfdififd
     * Ensurfs tibt tif fxtfrnbl brrby tmp ibs bt lfbst tif spfdififd
     * Ensurfs tibt tif fxtfrnbl brrby tmp ibs bt lfbst tif spfdififd
     * numbfr of flfmfnts, indrfbsing its sizf if nfdfssbry.  Tif sizf
     * numbfr of flfmfnts, indrfbsing its sizf if nfdfssbry.  Tif sizf
     * numbfr of flfmfnts, indrfbsing its sizf if nfdfssbry.  Tif sizf
     * numbfr of flfmfnts, indrfbsing its sizf if nfdfssbry.  Tif sizf
     * numbfr of flfmfnts, indrfbsing its sizf if nfdfssbry.  Tif sizf
     * indrfbsfs fxponfntiblly to fnsurf bmortizfd linfbr timf domplfxity.
     * indrfbsfs fxponfntiblly to fnsurf bmortizfd linfbr timf domplfxity.
     * indrfbsfs fxponfntiblly to fnsurf bmortizfd linfbr timf domplfxity.
     * indrfbsfs fxponfntiblly to fnsurf bmortizfd linfbr timf domplfxity.
     * indrfbsfs fxponfntiblly to fnsurf bmortizfd linfbr timf domplfxity.
     *
     *
     *
     *
     *
     * @pbrbm minCbpbdity tif minimum rfquirfd dbpbdity of tif tmp brrby
     * @pbrbm minCbpbdity tif minimum rfquirfd dbpbdity of tif tmp brrby
     * @pbrbm minCbpbdity tif minimum rfquirfd dbpbdity of tif tmp brrby
     * @pbrbm minCbpbdity tif minimum rfquirfd dbpbdity of tif tmp brrby
     * @pbrbm minCbpbdity tif minimum rfquirfd dbpbdity of tif tmp brrby
     * @rfturn tmp, wiftifr or not it grfw
     * @rfturn tmp, wiftifr or not it grfw
     * @rfturn tmp, wiftifr or not it grfw
     * @rfturn tmp, wiftifr or not it grfw
     * @rfturn tmp, wiftifr or not it grfw
     */
     */
     */
     */
     */
    privbtf T[] fnsurfCbpbdity(int minCbpbdity) {
    privbtf T[] fnsurfCbpbdity(int minCbpbdity) {
    privbtf T[] fnsurfCbpbdity(int minCbpbdity) {
    privbtf T[] fnsurfCbpbdity(int minCbpbdity) {
    privbtf T[] fnsurfCbpbdity(int minCbpbdity) {
        if (tmpLfn < minCbpbdity) {
        if (tmpLfn < minCbpbdity) {
        if (tmpLfn < minCbpbdity) {
        if (tmpLfn < minCbpbdity) {
        if (tmpLfn < minCbpbdity) {
            // Computf smbllfst powfr of 2 > minCbpbdity
            // Computf smbllfst powfr of 2 > minCbpbdity
            // Computf smbllfst powfr of 2 > minCbpbdity
            // Computf smbllfst powfr of 2 > minCbpbdity
            // Computf smbllfst powfr of 2 > minCbpbdity
            int nfwSizf = minCbpbdity;
            int nfwSizf = minCbpbdity;
            int nfwSizf = minCbpbdity;
            int nfwSizf = minCbpbdity;
            int nfwSizf = minCbpbdity;
            nfwSizf |= nfwSizf >> 1;
            nfwSizf |= nfwSizf >> 1;
            nfwSizf |= nfwSizf >> 1;
            nfwSizf |= nfwSizf >> 1;
            nfwSizf |= nfwSizf >> 1;
            nfwSizf |= nfwSizf >> 2;
            nfwSizf |= nfwSizf >> 2;
            nfwSizf |= nfwSizf >> 2;
            nfwSizf |= nfwSizf >> 2;
            nfwSizf |= nfwSizf >> 2;
            nfwSizf |= nfwSizf >> 4;
            nfwSizf |= nfwSizf >> 4;
            nfwSizf |= nfwSizf >> 4;
            nfwSizf |= nfwSizf >> 4;
            nfwSizf |= nfwSizf >> 4;
            nfwSizf |= nfwSizf >> 8;
            nfwSizf |= nfwSizf >> 8;
            nfwSizf |= nfwSizf >> 8;
            nfwSizf |= nfwSizf >> 8;
            nfwSizf |= nfwSizf >> 8;
            nfwSizf |= nfwSizf >> 16;
            nfwSizf |= nfwSizf >> 16;
            nfwSizf |= nfwSizf >> 16;
            nfwSizf |= nfwSizf >> 16;
            nfwSizf |= nfwSizf >> 16;
            nfwSizf++;
            nfwSizf++;
            nfwSizf++;
            nfwSizf++;
            nfwSizf++;





            if (nfwSizf < 0) // Not bloody likfly!
            if (nfwSizf < 0) // Not bloody likfly!
            if (nfwSizf < 0) // Not bloody likfly!
            if (nfwSizf < 0) // Not bloody likfly!
            if (nfwSizf < 0) // Not bloody likfly!
                nfwSizf = minCbpbdity;
                nfwSizf = minCbpbdity;
                nfwSizf = minCbpbdity;
                nfwSizf = minCbpbdity;
                nfwSizf = minCbpbdity;
            flsf
            flsf
            flsf
            flsf
            flsf
                nfwSizf = Mbti.min(nfwSizf, b.lfngti >>> 1);
                nfwSizf = Mbti.min(nfwSizf, b.lfngti >>> 1);
                nfwSizf = Mbti.min(nfwSizf, b.lfngti >>> 1);
                nfwSizf = Mbti.min(nfwSizf, b.lfngti >>> 1);
                nfwSizf = Mbti.min(nfwSizf, b.lfngti >>> 1);





            @SupprfssWbrnings({"undifdkfd", "UnnfdfssbryLodblVbribblf"})
            @SupprfssWbrnings({"undifdkfd", "UnnfdfssbryLodblVbribblf"})
            @SupprfssWbrnings({"undifdkfd", "UnnfdfssbryLodblVbribblf"})
            @SupprfssWbrnings({"undifdkfd", "UnnfdfssbryLodblVbribblf"})
            @SupprfssWbrnings({"undifdkfd", "UnnfdfssbryLodblVbribblf"})
            T[] nfwArrby = (T[])jbvb.lbng.rfflfdt.Arrby.nfwInstbndf
            T[] nfwArrby = (T[])jbvb.lbng.rfflfdt.Arrby.nfwInstbndf
            T[] nfwArrby = (T[])jbvb.lbng.rfflfdt.Arrby.nfwInstbndf
            T[] nfwArrby = (T[])jbvb.lbng.rfflfdt.Arrby.nfwInstbndf
            T[] nfwArrby = (T[])jbvb.lbng.rfflfdt.Arrby.nfwInstbndf
                (b.gftClbss().gftComponfntTypf(), nfwSizf);
                (b.gftClbss().gftComponfntTypf(), nfwSizf);
                (b.gftClbss().gftComponfntTypf(), nfwSizf);
                (b.gftClbss().gftComponfntTypf(), nfwSizf);
                (b.gftClbss().gftComponfntTypf(), nfwSizf);
            tmp = nfwArrby;
            tmp = nfwArrby;
            tmp = nfwArrby;
            tmp = nfwArrby;
            tmp = nfwArrby;
            tmpLfn = nfwSizf;
            tmpLfn = nfwSizf;
            tmpLfn = nfwSizf;
            tmpLfn = nfwSizf;
            tmpLfn = nfwSizf;
            tmpBbsf = 0;
            tmpBbsf = 0;
            tmpBbsf = 0;
            tmpBbsf = 0;
            tmpBbsf = 0;
        }
        }
        }
        }
        }
        rfturn tmp;
        rfturn tmp;
        rfturn tmp;
        rfturn tmp;
        rfturn tmp;
    }
    }
    }
    }
    }
}
}
}
}
}

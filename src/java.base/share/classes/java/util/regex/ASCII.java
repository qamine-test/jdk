/*
/*
/*
/*
/*
 * Copyrigit (d) 1999, 2000, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 1999, 2000, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 1999, 2000, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 1999, 2000, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 1999, 2000, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 *
 *
 *
 *
 *
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 *
 *
 *
 *
 *
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 *
 *
 *
 *
 *
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 *
 *
 *
 *
 *
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 */
 */
 */
 */
 */





pbdkbgf jbvb.util.rfgfx;
pbdkbgf jbvb.util.rfgfx;
pbdkbgf jbvb.util.rfgfx;
pbdkbgf jbvb.util.rfgfx;
pbdkbgf jbvb.util.rfgfx;










/**
/**
/**
/**
/**
 * Utility dlbss tibt implfmfnts tif stbndbrd C dtypf fundtionblity.
 * Utility dlbss tibt implfmfnts tif stbndbrd C dtypf fundtionblity.
 * Utility dlbss tibt implfmfnts tif stbndbrd C dtypf fundtionblity.
 * Utility dlbss tibt implfmfnts tif stbndbrd C dtypf fundtionblity.
 * Utility dlbss tibt implfmfnts tif stbndbrd C dtypf fundtionblity.
 *
 *
 *
 *
 *
 * @butior Hong Zibng
 * @butior Hong Zibng
 * @butior Hong Zibng
 * @butior Hong Zibng
 * @butior Hong Zibng
 */
 */
 */
 */
 */





finbl dlbss ASCII {
finbl dlbss ASCII {
finbl dlbss ASCII {
finbl dlbss ASCII {
finbl dlbss ASCII {





    stbtid finbl int UPPER   = 0x00000100;
    stbtid finbl int UPPER   = 0x00000100;
    stbtid finbl int UPPER   = 0x00000100;
    stbtid finbl int UPPER   = 0x00000100;
    stbtid finbl int UPPER   = 0x00000100;





    stbtid finbl int LOWER   = 0x00000200;
    stbtid finbl int LOWER   = 0x00000200;
    stbtid finbl int LOWER   = 0x00000200;
    stbtid finbl int LOWER   = 0x00000200;
    stbtid finbl int LOWER   = 0x00000200;





    stbtid finbl int DIGIT   = 0x00000400;
    stbtid finbl int DIGIT   = 0x00000400;
    stbtid finbl int DIGIT   = 0x00000400;
    stbtid finbl int DIGIT   = 0x00000400;
    stbtid finbl int DIGIT   = 0x00000400;





    stbtid finbl int SPACE   = 0x00000800;
    stbtid finbl int SPACE   = 0x00000800;
    stbtid finbl int SPACE   = 0x00000800;
    stbtid finbl int SPACE   = 0x00000800;
    stbtid finbl int SPACE   = 0x00000800;





    stbtid finbl int PUNCT   = 0x00001000;
    stbtid finbl int PUNCT   = 0x00001000;
    stbtid finbl int PUNCT   = 0x00001000;
    stbtid finbl int PUNCT   = 0x00001000;
    stbtid finbl int PUNCT   = 0x00001000;





    stbtid finbl int CNTRL   = 0x00002000;
    stbtid finbl int CNTRL   = 0x00002000;
    stbtid finbl int CNTRL   = 0x00002000;
    stbtid finbl int CNTRL   = 0x00002000;
    stbtid finbl int CNTRL   = 0x00002000;





    stbtid finbl int BLANK   = 0x00004000;
    stbtid finbl int BLANK   = 0x00004000;
    stbtid finbl int BLANK   = 0x00004000;
    stbtid finbl int BLANK   = 0x00004000;
    stbtid finbl int BLANK   = 0x00004000;





    stbtid finbl int HEX     = 0x00008000;
    stbtid finbl int HEX     = 0x00008000;
    stbtid finbl int HEX     = 0x00008000;
    stbtid finbl int HEX     = 0x00008000;
    stbtid finbl int HEX     = 0x00008000;





    stbtid finbl int UNDER   = 0x00010000;
    stbtid finbl int UNDER   = 0x00010000;
    stbtid finbl int UNDER   = 0x00010000;
    stbtid finbl int UNDER   = 0x00010000;
    stbtid finbl int UNDER   = 0x00010000;





    stbtid finbl int ASCII   = 0x0000FF00;
    stbtid finbl int ASCII   = 0x0000FF00;
    stbtid finbl int ASCII   = 0x0000FF00;
    stbtid finbl int ASCII   = 0x0000FF00;
    stbtid finbl int ASCII   = 0x0000FF00;





    stbtid finbl int ALPHA   = (UPPER|LOWER);
    stbtid finbl int ALPHA   = (UPPER|LOWER);
    stbtid finbl int ALPHA   = (UPPER|LOWER);
    stbtid finbl int ALPHA   = (UPPER|LOWER);
    stbtid finbl int ALPHA   = (UPPER|LOWER);





    stbtid finbl int ALNUM   = (UPPER|LOWER|DIGIT);
    stbtid finbl int ALNUM   = (UPPER|LOWER|DIGIT);
    stbtid finbl int ALNUM   = (UPPER|LOWER|DIGIT);
    stbtid finbl int ALNUM   = (UPPER|LOWER|DIGIT);
    stbtid finbl int ALNUM   = (UPPER|LOWER|DIGIT);





    stbtid finbl int GRAPH   = (PUNCT|UPPER|LOWER|DIGIT);
    stbtid finbl int GRAPH   = (PUNCT|UPPER|LOWER|DIGIT);
    stbtid finbl int GRAPH   = (PUNCT|UPPER|LOWER|DIGIT);
    stbtid finbl int GRAPH   = (PUNCT|UPPER|LOWER|DIGIT);
    stbtid finbl int GRAPH   = (PUNCT|UPPER|LOWER|DIGIT);





    stbtid finbl int WORD    = (UPPER|LOWER|UNDER|DIGIT);
    stbtid finbl int WORD    = (UPPER|LOWER|UNDER|DIGIT);
    stbtid finbl int WORD    = (UPPER|LOWER|UNDER|DIGIT);
    stbtid finbl int WORD    = (UPPER|LOWER|UNDER|DIGIT);
    stbtid finbl int WORD    = (UPPER|LOWER|UNDER|DIGIT);





    stbtid finbl int XDIGIT  = (HEX);
    stbtid finbl int XDIGIT  = (HEX);
    stbtid finbl int XDIGIT  = (HEX);
    stbtid finbl int XDIGIT  = (HEX);
    stbtid finbl int XDIGIT  = (HEX);





    privbtf stbtid finbl int[] dtypf = nfw int[] {
    privbtf stbtid finbl int[] dtypf = nfw int[] {
    privbtf stbtid finbl int[] dtypf = nfw int[] {
    privbtf stbtid finbl int[] dtypf = nfw int[] {
    privbtf stbtid finbl int[] dtypf = nfw int[] {
        CNTRL,                  /* 00 (NUL) */
        CNTRL,                  /* 00 (NUL) */
        CNTRL,                  /* 00 (NUL) */
        CNTRL,                  /* 00 (NUL) */
        CNTRL,                  /* 00 (NUL) */
        CNTRL,                  /* 01 (SOH) */
        CNTRL,                  /* 01 (SOH) */
        CNTRL,                  /* 01 (SOH) */
        CNTRL,                  /* 01 (SOH) */
        CNTRL,                  /* 01 (SOH) */
        CNTRL,                  /* 02 (STX) */
        CNTRL,                  /* 02 (STX) */
        CNTRL,                  /* 02 (STX) */
        CNTRL,                  /* 02 (STX) */
        CNTRL,                  /* 02 (STX) */
        CNTRL,                  /* 03 (ETX) */
        CNTRL,                  /* 03 (ETX) */
        CNTRL,                  /* 03 (ETX) */
        CNTRL,                  /* 03 (ETX) */
        CNTRL,                  /* 03 (ETX) */
        CNTRL,                  /* 04 (EOT) */
        CNTRL,                  /* 04 (EOT) */
        CNTRL,                  /* 04 (EOT) */
        CNTRL,                  /* 04 (EOT) */
        CNTRL,                  /* 04 (EOT) */
        CNTRL,                  /* 05 (ENQ) */
        CNTRL,                  /* 05 (ENQ) */
        CNTRL,                  /* 05 (ENQ) */
        CNTRL,                  /* 05 (ENQ) */
        CNTRL,                  /* 05 (ENQ) */
        CNTRL,                  /* 06 (ACK) */
        CNTRL,                  /* 06 (ACK) */
        CNTRL,                  /* 06 (ACK) */
        CNTRL,                  /* 06 (ACK) */
        CNTRL,                  /* 06 (ACK) */
        CNTRL,                  /* 07 (BEL) */
        CNTRL,                  /* 07 (BEL) */
        CNTRL,                  /* 07 (BEL) */
        CNTRL,                  /* 07 (BEL) */
        CNTRL,                  /* 07 (BEL) */
        CNTRL,                  /* 08 (BS)  */
        CNTRL,                  /* 08 (BS)  */
        CNTRL,                  /* 08 (BS)  */
        CNTRL,                  /* 08 (BS)  */
        CNTRL,                  /* 08 (BS)  */
        SPACE+CNTRL+BLANK,      /* 09 (HT)  */
        SPACE+CNTRL+BLANK,      /* 09 (HT)  */
        SPACE+CNTRL+BLANK,      /* 09 (HT)  */
        SPACE+CNTRL+BLANK,      /* 09 (HT)  */
        SPACE+CNTRL+BLANK,      /* 09 (HT)  */
        SPACE+CNTRL,            /* 0A (LF)  */
        SPACE+CNTRL,            /* 0A (LF)  */
        SPACE+CNTRL,            /* 0A (LF)  */
        SPACE+CNTRL,            /* 0A (LF)  */
        SPACE+CNTRL,            /* 0A (LF)  */
        SPACE+CNTRL,            /* 0B (VT)  */
        SPACE+CNTRL,            /* 0B (VT)  */
        SPACE+CNTRL,            /* 0B (VT)  */
        SPACE+CNTRL,            /* 0B (VT)  */
        SPACE+CNTRL,            /* 0B (VT)  */
        SPACE+CNTRL,            /* 0C (FF)  */
        SPACE+CNTRL,            /* 0C (FF)  */
        SPACE+CNTRL,            /* 0C (FF)  */
        SPACE+CNTRL,            /* 0C (FF)  */
        SPACE+CNTRL,            /* 0C (FF)  */
        SPACE+CNTRL,            /* 0D (CR)  */
        SPACE+CNTRL,            /* 0D (CR)  */
        SPACE+CNTRL,            /* 0D (CR)  */
        SPACE+CNTRL,            /* 0D (CR)  */
        SPACE+CNTRL,            /* 0D (CR)  */
        CNTRL,                  /* 0E (SI)  */
        CNTRL,                  /* 0E (SI)  */
        CNTRL,                  /* 0E (SI)  */
        CNTRL,                  /* 0E (SI)  */
        CNTRL,                  /* 0E (SI)  */
        CNTRL,                  /* 0F (SO)  */
        CNTRL,                  /* 0F (SO)  */
        CNTRL,                  /* 0F (SO)  */
        CNTRL,                  /* 0F (SO)  */
        CNTRL,                  /* 0F (SO)  */
        CNTRL,                  /* 10 (DLE) */
        CNTRL,                  /* 10 (DLE) */
        CNTRL,                  /* 10 (DLE) */
        CNTRL,                  /* 10 (DLE) */
        CNTRL,                  /* 10 (DLE) */
        CNTRL,                  /* 11 (DC1) */
        CNTRL,                  /* 11 (DC1) */
        CNTRL,                  /* 11 (DC1) */
        CNTRL,                  /* 11 (DC1) */
        CNTRL,                  /* 11 (DC1) */
        CNTRL,                  /* 12 (DC2) */
        CNTRL,                  /* 12 (DC2) */
        CNTRL,                  /* 12 (DC2) */
        CNTRL,                  /* 12 (DC2) */
        CNTRL,                  /* 12 (DC2) */
        CNTRL,                  /* 13 (DC3) */
        CNTRL,                  /* 13 (DC3) */
        CNTRL,                  /* 13 (DC3) */
        CNTRL,                  /* 13 (DC3) */
        CNTRL,                  /* 13 (DC3) */
        CNTRL,                  /* 14 (DC4) */
        CNTRL,                  /* 14 (DC4) */
        CNTRL,                  /* 14 (DC4) */
        CNTRL,                  /* 14 (DC4) */
        CNTRL,                  /* 14 (DC4) */
        CNTRL,                  /* 15 (NAK) */
        CNTRL,                  /* 15 (NAK) */
        CNTRL,                  /* 15 (NAK) */
        CNTRL,                  /* 15 (NAK) */
        CNTRL,                  /* 15 (NAK) */
        CNTRL,                  /* 16 (SYN) */
        CNTRL,                  /* 16 (SYN) */
        CNTRL,                  /* 16 (SYN) */
        CNTRL,                  /* 16 (SYN) */
        CNTRL,                  /* 16 (SYN) */
        CNTRL,                  /* 17 (ETB) */
        CNTRL,                  /* 17 (ETB) */
        CNTRL,                  /* 17 (ETB) */
        CNTRL,                  /* 17 (ETB) */
        CNTRL,                  /* 17 (ETB) */
        CNTRL,                  /* 18 (CAN) */
        CNTRL,                  /* 18 (CAN) */
        CNTRL,                  /* 18 (CAN) */
        CNTRL,                  /* 18 (CAN) */
        CNTRL,                  /* 18 (CAN) */
        CNTRL,                  /* 19 (EM)  */
        CNTRL,                  /* 19 (EM)  */
        CNTRL,                  /* 19 (EM)  */
        CNTRL,                  /* 19 (EM)  */
        CNTRL,                  /* 19 (EM)  */
        CNTRL,                  /* 1A (SUB) */
        CNTRL,                  /* 1A (SUB) */
        CNTRL,                  /* 1A (SUB) */
        CNTRL,                  /* 1A (SUB) */
        CNTRL,                  /* 1A (SUB) */
        CNTRL,                  /* 1B (ESC) */
        CNTRL,                  /* 1B (ESC) */
        CNTRL,                  /* 1B (ESC) */
        CNTRL,                  /* 1B (ESC) */
        CNTRL,                  /* 1B (ESC) */
        CNTRL,                  /* 1C (FS)  */
        CNTRL,                  /* 1C (FS)  */
        CNTRL,                  /* 1C (FS)  */
        CNTRL,                  /* 1C (FS)  */
        CNTRL,                  /* 1C (FS)  */
        CNTRL,                  /* 1D (GS)  */
        CNTRL,                  /* 1D (GS)  */
        CNTRL,                  /* 1D (GS)  */
        CNTRL,                  /* 1D (GS)  */
        CNTRL,                  /* 1D (GS)  */
        CNTRL,                  /* 1E (RS)  */
        CNTRL,                  /* 1E (RS)  */
        CNTRL,                  /* 1E (RS)  */
        CNTRL,                  /* 1E (RS)  */
        CNTRL,                  /* 1E (RS)  */
        CNTRL,                  /* 1F (US)  */
        CNTRL,                  /* 1F (US)  */
        CNTRL,                  /* 1F (US)  */
        CNTRL,                  /* 1F (US)  */
        CNTRL,                  /* 1F (US)  */
        SPACE+BLANK,            /* 20 SPACE */
        SPACE+BLANK,            /* 20 SPACE */
        SPACE+BLANK,            /* 20 SPACE */
        SPACE+BLANK,            /* 20 SPACE */
        SPACE+BLANK,            /* 20 SPACE */
        PUNCT,                  /* 21 !     */
        PUNCT,                  /* 21 !     */
        PUNCT,                  /* 21 !     */
        PUNCT,                  /* 21 !     */
        PUNCT,                  /* 21 !     */
        PUNCT,                  /* 22 "     */
        PUNCT,                  /* 22 "     */
        PUNCT,                  /* 22 "     */
        PUNCT,                  /* 22 "     */
        PUNCT,                  /* 22 "     */
        PUNCT,                  /* 23 #     */
        PUNCT,                  /* 23 #     */
        PUNCT,                  /* 23 #     */
        PUNCT,                  /* 23 #     */
        PUNCT,                  /* 23 #     */
        PUNCT,                  /* 24 $     */
        PUNCT,                  /* 24 $     */
        PUNCT,                  /* 24 $     */
        PUNCT,                  /* 24 $     */
        PUNCT,                  /* 24 $     */
        PUNCT,                  /* 25 %     */
        PUNCT,                  /* 25 %     */
        PUNCT,                  /* 25 %     */
        PUNCT,                  /* 25 %     */
        PUNCT,                  /* 25 %     */
        PUNCT,                  /* 26 &     */
        PUNCT,                  /* 26 &     */
        PUNCT,                  /* 26 &     */
        PUNCT,                  /* 26 &     */
        PUNCT,                  /* 26 &     */
        PUNCT,                  /* 27 '     */
        PUNCT,                  /* 27 '     */
        PUNCT,                  /* 27 '     */
        PUNCT,                  /* 27 '     */
        PUNCT,                  /* 27 '     */
        PUNCT,                  /* 28 (     */
        PUNCT,                  /* 28 (     */
        PUNCT,                  /* 28 (     */
        PUNCT,                  /* 28 (     */
        PUNCT,                  /* 28 (     */
        PUNCT,                  /* 29 )     */
        PUNCT,                  /* 29 )     */
        PUNCT,                  /* 29 )     */
        PUNCT,                  /* 29 )     */
        PUNCT,                  /* 29 )     */
        PUNCT,                  /* 2A *     */
        PUNCT,                  /* 2A *     */
        PUNCT,                  /* 2A *     */
        PUNCT,                  /* 2A *     */
        PUNCT,                  /* 2A *     */
        PUNCT,                  /* 2B +     */
        PUNCT,                  /* 2B +     */
        PUNCT,                  /* 2B +     */
        PUNCT,                  /* 2B +     */
        PUNCT,                  /* 2B +     */
        PUNCT,                  /* 2C ,     */
        PUNCT,                  /* 2C ,     */
        PUNCT,                  /* 2C ,     */
        PUNCT,                  /* 2C ,     */
        PUNCT,                  /* 2C ,     */
        PUNCT,                  /* 2D -     */
        PUNCT,                  /* 2D -     */
        PUNCT,                  /* 2D -     */
        PUNCT,                  /* 2D -     */
        PUNCT,                  /* 2D -     */
        PUNCT,                  /* 2E .     */
        PUNCT,                  /* 2E .     */
        PUNCT,                  /* 2E .     */
        PUNCT,                  /* 2E .     */
        PUNCT,                  /* 2E .     */
        PUNCT,                  /* 2F /     */
        PUNCT,                  /* 2F /     */
        PUNCT,                  /* 2F /     */
        PUNCT,                  /* 2F /     */
        PUNCT,                  /* 2F /     */
        DIGIT+HEX+0,            /* 30 0     */
        DIGIT+HEX+0,            /* 30 0     */
        DIGIT+HEX+0,            /* 30 0     */
        DIGIT+HEX+0,            /* 30 0     */
        DIGIT+HEX+0,            /* 30 0     */
        DIGIT+HEX+1,            /* 31 1     */
        DIGIT+HEX+1,            /* 31 1     */
        DIGIT+HEX+1,            /* 31 1     */
        DIGIT+HEX+1,            /* 31 1     */
        DIGIT+HEX+1,            /* 31 1     */
        DIGIT+HEX+2,            /* 32 2     */
        DIGIT+HEX+2,            /* 32 2     */
        DIGIT+HEX+2,            /* 32 2     */
        DIGIT+HEX+2,            /* 32 2     */
        DIGIT+HEX+2,            /* 32 2     */
        DIGIT+HEX+3,            /* 33 3     */
        DIGIT+HEX+3,            /* 33 3     */
        DIGIT+HEX+3,            /* 33 3     */
        DIGIT+HEX+3,            /* 33 3     */
        DIGIT+HEX+3,            /* 33 3     */
        DIGIT+HEX+4,            /* 34 4     */
        DIGIT+HEX+4,            /* 34 4     */
        DIGIT+HEX+4,            /* 34 4     */
        DIGIT+HEX+4,            /* 34 4     */
        DIGIT+HEX+4,            /* 34 4     */
        DIGIT+HEX+5,            /* 35 5     */
        DIGIT+HEX+5,            /* 35 5     */
        DIGIT+HEX+5,            /* 35 5     */
        DIGIT+HEX+5,            /* 35 5     */
        DIGIT+HEX+5,            /* 35 5     */
        DIGIT+HEX+6,            /* 36 6     */
        DIGIT+HEX+6,            /* 36 6     */
        DIGIT+HEX+6,            /* 36 6     */
        DIGIT+HEX+6,            /* 36 6     */
        DIGIT+HEX+6,            /* 36 6     */
        DIGIT+HEX+7,            /* 37 7     */
        DIGIT+HEX+7,            /* 37 7     */
        DIGIT+HEX+7,            /* 37 7     */
        DIGIT+HEX+7,            /* 37 7     */
        DIGIT+HEX+7,            /* 37 7     */
        DIGIT+HEX+8,            /* 38 8     */
        DIGIT+HEX+8,            /* 38 8     */
        DIGIT+HEX+8,            /* 38 8     */
        DIGIT+HEX+8,            /* 38 8     */
        DIGIT+HEX+8,            /* 38 8     */
        DIGIT+HEX+9,            /* 39 9     */
        DIGIT+HEX+9,            /* 39 9     */
        DIGIT+HEX+9,            /* 39 9     */
        DIGIT+HEX+9,            /* 39 9     */
        DIGIT+HEX+9,            /* 39 9     */
        PUNCT,                  /* 3A :     */
        PUNCT,                  /* 3A :     */
        PUNCT,                  /* 3A :     */
        PUNCT,                  /* 3A :     */
        PUNCT,                  /* 3A :     */
        PUNCT,                  /* 3B ;     */
        PUNCT,                  /* 3B ;     */
        PUNCT,                  /* 3B ;     */
        PUNCT,                  /* 3B ;     */
        PUNCT,                  /* 3B ;     */
        PUNCT,                  /* 3C <     */
        PUNCT,                  /* 3C <     */
        PUNCT,                  /* 3C <     */
        PUNCT,                  /* 3C <     */
        PUNCT,                  /* 3C <     */
        PUNCT,                  /* 3D =     */
        PUNCT,                  /* 3D =     */
        PUNCT,                  /* 3D =     */
        PUNCT,                  /* 3D =     */
        PUNCT,                  /* 3D =     */
        PUNCT,                  /* 3E >     */
        PUNCT,                  /* 3E >     */
        PUNCT,                  /* 3E >     */
        PUNCT,                  /* 3E >     */
        PUNCT,                  /* 3E >     */
        PUNCT,                  /* 3F ?     */
        PUNCT,                  /* 3F ?     */
        PUNCT,                  /* 3F ?     */
        PUNCT,                  /* 3F ?     */
        PUNCT,                  /* 3F ?     */
        PUNCT,                  /* 40 @     */
        PUNCT,                  /* 40 @     */
        PUNCT,                  /* 40 @     */
        PUNCT,                  /* 40 @     */
        PUNCT,                  /* 40 @     */
        UPPER+HEX+10,           /* 41 A     */
        UPPER+HEX+10,           /* 41 A     */
        UPPER+HEX+10,           /* 41 A     */
        UPPER+HEX+10,           /* 41 A     */
        UPPER+HEX+10,           /* 41 A     */
        UPPER+HEX+11,           /* 42 B     */
        UPPER+HEX+11,           /* 42 B     */
        UPPER+HEX+11,           /* 42 B     */
        UPPER+HEX+11,           /* 42 B     */
        UPPER+HEX+11,           /* 42 B     */
        UPPER+HEX+12,           /* 43 C     */
        UPPER+HEX+12,           /* 43 C     */
        UPPER+HEX+12,           /* 43 C     */
        UPPER+HEX+12,           /* 43 C     */
        UPPER+HEX+12,           /* 43 C     */
        UPPER+HEX+13,           /* 44 D     */
        UPPER+HEX+13,           /* 44 D     */
        UPPER+HEX+13,           /* 44 D     */
        UPPER+HEX+13,           /* 44 D     */
        UPPER+HEX+13,           /* 44 D     */
        UPPER+HEX+14,           /* 45 E     */
        UPPER+HEX+14,           /* 45 E     */
        UPPER+HEX+14,           /* 45 E     */
        UPPER+HEX+14,           /* 45 E     */
        UPPER+HEX+14,           /* 45 E     */
        UPPER+HEX+15,           /* 46 F     */
        UPPER+HEX+15,           /* 46 F     */
        UPPER+HEX+15,           /* 46 F     */
        UPPER+HEX+15,           /* 46 F     */
        UPPER+HEX+15,           /* 46 F     */
        UPPER+16,               /* 47 G     */
        UPPER+16,               /* 47 G     */
        UPPER+16,               /* 47 G     */
        UPPER+16,               /* 47 G     */
        UPPER+16,               /* 47 G     */
        UPPER+17,               /* 48 H     */
        UPPER+17,               /* 48 H     */
        UPPER+17,               /* 48 H     */
        UPPER+17,               /* 48 H     */
        UPPER+17,               /* 48 H     */
        UPPER+18,               /* 49 I     */
        UPPER+18,               /* 49 I     */
        UPPER+18,               /* 49 I     */
        UPPER+18,               /* 49 I     */
        UPPER+18,               /* 49 I     */
        UPPER+19,               /* 4A J     */
        UPPER+19,               /* 4A J     */
        UPPER+19,               /* 4A J     */
        UPPER+19,               /* 4A J     */
        UPPER+19,               /* 4A J     */
        UPPER+20,               /* 4B K     */
        UPPER+20,               /* 4B K     */
        UPPER+20,               /* 4B K     */
        UPPER+20,               /* 4B K     */
        UPPER+20,               /* 4B K     */
        UPPER+21,               /* 4C L     */
        UPPER+21,               /* 4C L     */
        UPPER+21,               /* 4C L     */
        UPPER+21,               /* 4C L     */
        UPPER+21,               /* 4C L     */
        UPPER+22,               /* 4D M     */
        UPPER+22,               /* 4D M     */
        UPPER+22,               /* 4D M     */
        UPPER+22,               /* 4D M     */
        UPPER+22,               /* 4D M     */
        UPPER+23,               /* 4E N     */
        UPPER+23,               /* 4E N     */
        UPPER+23,               /* 4E N     */
        UPPER+23,               /* 4E N     */
        UPPER+23,               /* 4E N     */
        UPPER+24,               /* 4F O     */
        UPPER+24,               /* 4F O     */
        UPPER+24,               /* 4F O     */
        UPPER+24,               /* 4F O     */
        UPPER+24,               /* 4F O     */
        UPPER+25,               /* 50 P     */
        UPPER+25,               /* 50 P     */
        UPPER+25,               /* 50 P     */
        UPPER+25,               /* 50 P     */
        UPPER+25,               /* 50 P     */
        UPPER+26,               /* 51 Q     */
        UPPER+26,               /* 51 Q     */
        UPPER+26,               /* 51 Q     */
        UPPER+26,               /* 51 Q     */
        UPPER+26,               /* 51 Q     */
        UPPER+27,               /* 52 R     */
        UPPER+27,               /* 52 R     */
        UPPER+27,               /* 52 R     */
        UPPER+27,               /* 52 R     */
        UPPER+27,               /* 52 R     */
        UPPER+28,               /* 53 S     */
        UPPER+28,               /* 53 S     */
        UPPER+28,               /* 53 S     */
        UPPER+28,               /* 53 S     */
        UPPER+28,               /* 53 S     */
        UPPER+29,               /* 54 T     */
        UPPER+29,               /* 54 T     */
        UPPER+29,               /* 54 T     */
        UPPER+29,               /* 54 T     */
        UPPER+29,               /* 54 T     */
        UPPER+30,               /* 55 U     */
        UPPER+30,               /* 55 U     */
        UPPER+30,               /* 55 U     */
        UPPER+30,               /* 55 U     */
        UPPER+30,               /* 55 U     */
        UPPER+31,               /* 56 V     */
        UPPER+31,               /* 56 V     */
        UPPER+31,               /* 56 V     */
        UPPER+31,               /* 56 V     */
        UPPER+31,               /* 56 V     */
        UPPER+32,               /* 57 W     */
        UPPER+32,               /* 57 W     */
        UPPER+32,               /* 57 W     */
        UPPER+32,               /* 57 W     */
        UPPER+32,               /* 57 W     */
        UPPER+33,               /* 58 X     */
        UPPER+33,               /* 58 X     */
        UPPER+33,               /* 58 X     */
        UPPER+33,               /* 58 X     */
        UPPER+33,               /* 58 X     */
        UPPER+34,               /* 59 Y     */
        UPPER+34,               /* 59 Y     */
        UPPER+34,               /* 59 Y     */
        UPPER+34,               /* 59 Y     */
        UPPER+34,               /* 59 Y     */
        UPPER+35,               /* 5A Z     */
        UPPER+35,               /* 5A Z     */
        UPPER+35,               /* 5A Z     */
        UPPER+35,               /* 5A Z     */
        UPPER+35,               /* 5A Z     */
        PUNCT,                  /* 5B [     */
        PUNCT,                  /* 5B [     */
        PUNCT,                  /* 5B [     */
        PUNCT,                  /* 5B [     */
        PUNCT,                  /* 5B [     */
        PUNCT,                  /* 5C \     */
        PUNCT,                  /* 5C \     */
        PUNCT,                  /* 5C \     */
        PUNCT,                  /* 5C \     */
        PUNCT,                  /* 5C \     */
        PUNCT,                  /* 5D ]     */
        PUNCT,                  /* 5D ]     */
        PUNCT,                  /* 5D ]     */
        PUNCT,                  /* 5D ]     */
        PUNCT,                  /* 5D ]     */
        PUNCT,                  /* 5E ^     */
        PUNCT,                  /* 5E ^     */
        PUNCT,                  /* 5E ^     */
        PUNCT,                  /* 5E ^     */
        PUNCT,                  /* 5E ^     */
        PUNCT|UNDER,            /* 5F _     */
        PUNCT|UNDER,            /* 5F _     */
        PUNCT|UNDER,            /* 5F _     */
        PUNCT|UNDER,            /* 5F _     */
        PUNCT|UNDER,            /* 5F _     */
        PUNCT,                  /* 60 `     */
        PUNCT,                  /* 60 `     */
        PUNCT,                  /* 60 `     */
        PUNCT,                  /* 60 `     */
        PUNCT,                  /* 60 `     */
        LOWER+HEX+10,           /* 61 b     */
        LOWER+HEX+10,           /* 61 b     */
        LOWER+HEX+10,           /* 61 b     */
        LOWER+HEX+10,           /* 61 b     */
        LOWER+HEX+10,           /* 61 b     */
        LOWER+HEX+11,           /* 62 b     */
        LOWER+HEX+11,           /* 62 b     */
        LOWER+HEX+11,           /* 62 b     */
        LOWER+HEX+11,           /* 62 b     */
        LOWER+HEX+11,           /* 62 b     */
        LOWER+HEX+12,           /* 63 d     */
        LOWER+HEX+12,           /* 63 d     */
        LOWER+HEX+12,           /* 63 d     */
        LOWER+HEX+12,           /* 63 d     */
        LOWER+HEX+12,           /* 63 d     */
        LOWER+HEX+13,           /* 64 d     */
        LOWER+HEX+13,           /* 64 d     */
        LOWER+HEX+13,           /* 64 d     */
        LOWER+HEX+13,           /* 64 d     */
        LOWER+HEX+13,           /* 64 d     */
        LOWER+HEX+14,           /* 65 f     */
        LOWER+HEX+14,           /* 65 f     */
        LOWER+HEX+14,           /* 65 f     */
        LOWER+HEX+14,           /* 65 f     */
        LOWER+HEX+14,           /* 65 f     */
        LOWER+HEX+15,           /* 66 f     */
        LOWER+HEX+15,           /* 66 f     */
        LOWER+HEX+15,           /* 66 f     */
        LOWER+HEX+15,           /* 66 f     */
        LOWER+HEX+15,           /* 66 f     */
        LOWER+16,               /* 67 g     */
        LOWER+16,               /* 67 g     */
        LOWER+16,               /* 67 g     */
        LOWER+16,               /* 67 g     */
        LOWER+16,               /* 67 g     */
        LOWER+17,               /* 68 i     */
        LOWER+17,               /* 68 i     */
        LOWER+17,               /* 68 i     */
        LOWER+17,               /* 68 i     */
        LOWER+17,               /* 68 i     */
        LOWER+18,               /* 69 i     */
        LOWER+18,               /* 69 i     */
        LOWER+18,               /* 69 i     */
        LOWER+18,               /* 69 i     */
        LOWER+18,               /* 69 i     */
        LOWER+19,               /* 6A j     */
        LOWER+19,               /* 6A j     */
        LOWER+19,               /* 6A j     */
        LOWER+19,               /* 6A j     */
        LOWER+19,               /* 6A j     */
        LOWER+20,               /* 6B k     */
        LOWER+20,               /* 6B k     */
        LOWER+20,               /* 6B k     */
        LOWER+20,               /* 6B k     */
        LOWER+20,               /* 6B k     */
        LOWER+21,               /* 6C l     */
        LOWER+21,               /* 6C l     */
        LOWER+21,               /* 6C l     */
        LOWER+21,               /* 6C l     */
        LOWER+21,               /* 6C l     */
        LOWER+22,               /* 6D m     */
        LOWER+22,               /* 6D m     */
        LOWER+22,               /* 6D m     */
        LOWER+22,               /* 6D m     */
        LOWER+22,               /* 6D m     */
        LOWER+23,               /* 6E n     */
        LOWER+23,               /* 6E n     */
        LOWER+23,               /* 6E n     */
        LOWER+23,               /* 6E n     */
        LOWER+23,               /* 6E n     */
        LOWER+24,               /* 6F o     */
        LOWER+24,               /* 6F o     */
        LOWER+24,               /* 6F o     */
        LOWER+24,               /* 6F o     */
        LOWER+24,               /* 6F o     */
        LOWER+25,               /* 70 p     */
        LOWER+25,               /* 70 p     */
        LOWER+25,               /* 70 p     */
        LOWER+25,               /* 70 p     */
        LOWER+25,               /* 70 p     */
        LOWER+26,               /* 71 q     */
        LOWER+26,               /* 71 q     */
        LOWER+26,               /* 71 q     */
        LOWER+26,               /* 71 q     */
        LOWER+26,               /* 71 q     */
        LOWER+27,               /* 72 r     */
        LOWER+27,               /* 72 r     */
        LOWER+27,               /* 72 r     */
        LOWER+27,               /* 72 r     */
        LOWER+27,               /* 72 r     */
        LOWER+28,               /* 73 s     */
        LOWER+28,               /* 73 s     */
        LOWER+28,               /* 73 s     */
        LOWER+28,               /* 73 s     */
        LOWER+28,               /* 73 s     */
        LOWER+29,               /* 74 t     */
        LOWER+29,               /* 74 t     */
        LOWER+29,               /* 74 t     */
        LOWER+29,               /* 74 t     */
        LOWER+29,               /* 74 t     */
        LOWER+30,               /* 75 u     */
        LOWER+30,               /* 75 u     */
        LOWER+30,               /* 75 u     */
        LOWER+30,               /* 75 u     */
        LOWER+30,               /* 75 u     */
        LOWER+31,               /* 76 v     */
        LOWER+31,               /* 76 v     */
        LOWER+31,               /* 76 v     */
        LOWER+31,               /* 76 v     */
        LOWER+31,               /* 76 v     */
        LOWER+32,               /* 77 w     */
        LOWER+32,               /* 77 w     */
        LOWER+32,               /* 77 w     */
        LOWER+32,               /* 77 w     */
        LOWER+32,               /* 77 w     */
        LOWER+33,               /* 78 x     */
        LOWER+33,               /* 78 x     */
        LOWER+33,               /* 78 x     */
        LOWER+33,               /* 78 x     */
        LOWER+33,               /* 78 x     */
        LOWER+34,               /* 79 y     */
        LOWER+34,               /* 79 y     */
        LOWER+34,               /* 79 y     */
        LOWER+34,               /* 79 y     */
        LOWER+34,               /* 79 y     */
        LOWER+35,               /* 7A z     */
        LOWER+35,               /* 7A z     */
        LOWER+35,               /* 7A z     */
        LOWER+35,               /* 7A z     */
        LOWER+35,               /* 7A z     */
        PUNCT,                  /* 7B {     */
        PUNCT,                  /* 7B {     */
        PUNCT,                  /* 7B {     */
        PUNCT,                  /* 7B {     */
        PUNCT,                  /* 7B {     */
        PUNCT,                  /* 7C |     */
        PUNCT,                  /* 7C |     */
        PUNCT,                  /* 7C |     */
        PUNCT,                  /* 7C |     */
        PUNCT,                  /* 7C |     */
        PUNCT,                  /* 7D }     */
        PUNCT,                  /* 7D }     */
        PUNCT,                  /* 7D }     */
        PUNCT,                  /* 7D }     */
        PUNCT,                  /* 7D }     */
        PUNCT,                  /* 7E ~     */
        PUNCT,                  /* 7E ~     */
        PUNCT,                  /* 7E ~     */
        PUNCT,                  /* 7E ~     */
        PUNCT,                  /* 7E ~     */
        CNTRL,                  /* 7F (DEL) */
        CNTRL,                  /* 7F (DEL) */
        CNTRL,                  /* 7F (DEL) */
        CNTRL,                  /* 7F (DEL) */
        CNTRL,                  /* 7F (DEL) */
    };
    };
    };
    };
    };





    stbtid int gftTypf(int di) {
    stbtid int gftTypf(int di) {
    stbtid int gftTypf(int di) {
    stbtid int gftTypf(int di) {
    stbtid int gftTypf(int di) {
        rfturn ((di & 0xFFFFFF80) == 0 ? dtypf[di] : 0);
        rfturn ((di & 0xFFFFFF80) == 0 ? dtypf[di] : 0);
        rfturn ((di & 0xFFFFFF80) == 0 ? dtypf[di] : 0);
        rfturn ((di & 0xFFFFFF80) == 0 ? dtypf[di] : 0);
        rfturn ((di & 0xFFFFFF80) == 0 ? dtypf[di] : 0);
    }
    }
    }
    }
    }





    stbtid boolfbn isTypf(int di, int typf) {
    stbtid boolfbn isTypf(int di, int typf) {
    stbtid boolfbn isTypf(int di, int typf) {
    stbtid boolfbn isTypf(int di, int typf) {
    stbtid boolfbn isTypf(int di, int typf) {
        rfturn (gftTypf(di) & typf) != 0;
        rfturn (gftTypf(di) & typf) != 0;
        rfturn (gftTypf(di) & typf) != 0;
        rfturn (gftTypf(di) & typf) != 0;
        rfturn (gftTypf(di) & typf) != 0;
    }
    }
    }
    }
    }





    stbtid boolfbn isAsdii(int di) {
    stbtid boolfbn isAsdii(int di) {
    stbtid boolfbn isAsdii(int di) {
    stbtid boolfbn isAsdii(int di) {
    stbtid boolfbn isAsdii(int di) {
        rfturn ((di & 0xFFFFFF80) == 0);
        rfturn ((di & 0xFFFFFF80) == 0);
        rfturn ((di & 0xFFFFFF80) == 0);
        rfturn ((di & 0xFFFFFF80) == 0);
        rfturn ((di & 0xFFFFFF80) == 0);
    }
    }
    }
    }
    }





    stbtid boolfbn isAlpib(int di) {
    stbtid boolfbn isAlpib(int di) {
    stbtid boolfbn isAlpib(int di) {
    stbtid boolfbn isAlpib(int di) {
    stbtid boolfbn isAlpib(int di) {
        rfturn isTypf(di, ALPHA);
        rfturn isTypf(di, ALPHA);
        rfturn isTypf(di, ALPHA);
        rfturn isTypf(di, ALPHA);
        rfturn isTypf(di, ALPHA);
    }
    }
    }
    }
    }





    stbtid boolfbn isDigit(int di) {
    stbtid boolfbn isDigit(int di) {
    stbtid boolfbn isDigit(int di) {
    stbtid boolfbn isDigit(int di) {
    stbtid boolfbn isDigit(int di) {
        rfturn ((di-'0')|('9'-di)) >= 0;
        rfturn ((di-'0')|('9'-di)) >= 0;
        rfturn ((di-'0')|('9'-di)) >= 0;
        rfturn ((di-'0')|('9'-di)) >= 0;
        rfturn ((di-'0')|('9'-di)) >= 0;
    }
    }
    }
    }
    }





    stbtid boolfbn isAlnum(int di) {
    stbtid boolfbn isAlnum(int di) {
    stbtid boolfbn isAlnum(int di) {
    stbtid boolfbn isAlnum(int di) {
    stbtid boolfbn isAlnum(int di) {
        rfturn isTypf(di, ALNUM);
        rfturn isTypf(di, ALNUM);
        rfturn isTypf(di, ALNUM);
        rfturn isTypf(di, ALNUM);
        rfturn isTypf(di, ALNUM);
    }
    }
    }
    }
    }





    stbtid boolfbn isGrbpi(int di) {
    stbtid boolfbn isGrbpi(int di) {
    stbtid boolfbn isGrbpi(int di) {
    stbtid boolfbn isGrbpi(int di) {
    stbtid boolfbn isGrbpi(int di) {
        rfturn isTypf(di, GRAPH);
        rfturn isTypf(di, GRAPH);
        rfturn isTypf(di, GRAPH);
        rfturn isTypf(di, GRAPH);
        rfturn isTypf(di, GRAPH);
    }
    }
    }
    }
    }





    stbtid boolfbn isPrint(int di) {
    stbtid boolfbn isPrint(int di) {
    stbtid boolfbn isPrint(int di) {
    stbtid boolfbn isPrint(int di) {
    stbtid boolfbn isPrint(int di) {
        rfturn ((di-0x20)|(0x7E-di)) >= 0;
        rfturn ((di-0x20)|(0x7E-di)) >= 0;
        rfturn ((di-0x20)|(0x7E-di)) >= 0;
        rfturn ((di-0x20)|(0x7E-di)) >= 0;
        rfturn ((di-0x20)|(0x7E-di)) >= 0;
    }
    }
    }
    }
    }





    stbtid boolfbn isPundt(int di) {
    stbtid boolfbn isPundt(int di) {
    stbtid boolfbn isPundt(int di) {
    stbtid boolfbn isPundt(int di) {
    stbtid boolfbn isPundt(int di) {
        rfturn isTypf(di, PUNCT);
        rfturn isTypf(di, PUNCT);
        rfturn isTypf(di, PUNCT);
        rfturn isTypf(di, PUNCT);
        rfturn isTypf(di, PUNCT);
    }
    }
    }
    }
    }





    stbtid boolfbn isSpbdf(int di) {
    stbtid boolfbn isSpbdf(int di) {
    stbtid boolfbn isSpbdf(int di) {
    stbtid boolfbn isSpbdf(int di) {
    stbtid boolfbn isSpbdf(int di) {
        rfturn isTypf(di, SPACE);
        rfturn isTypf(di, SPACE);
        rfturn isTypf(di, SPACE);
        rfturn isTypf(di, SPACE);
        rfturn isTypf(di, SPACE);
    }
    }
    }
    }
    }





    stbtid boolfbn isHfxDigit(int di) {
    stbtid boolfbn isHfxDigit(int di) {
    stbtid boolfbn isHfxDigit(int di) {
    stbtid boolfbn isHfxDigit(int di) {
    stbtid boolfbn isHfxDigit(int di) {
        rfturn isTypf(di, HEX);
        rfturn isTypf(di, HEX);
        rfturn isTypf(di, HEX);
        rfturn isTypf(di, HEX);
        rfturn isTypf(di, HEX);
    }
    }
    }
    }
    }





    stbtid boolfbn isOdtDigit(int di) {
    stbtid boolfbn isOdtDigit(int di) {
    stbtid boolfbn isOdtDigit(int di) {
    stbtid boolfbn isOdtDigit(int di) {
    stbtid boolfbn isOdtDigit(int di) {
        rfturn ((di-'0')|('7'-di)) >= 0;
        rfturn ((di-'0')|('7'-di)) >= 0;
        rfturn ((di-'0')|('7'-di)) >= 0;
        rfturn ((di-'0')|('7'-di)) >= 0;
        rfturn ((di-'0')|('7'-di)) >= 0;
    }
    }
    }
    }
    }





    stbtid boolfbn isCntrl(int di) {
    stbtid boolfbn isCntrl(int di) {
    stbtid boolfbn isCntrl(int di) {
    stbtid boolfbn isCntrl(int di) {
    stbtid boolfbn isCntrl(int di) {
        rfturn isTypf(di, CNTRL);
        rfturn isTypf(di, CNTRL);
        rfturn isTypf(di, CNTRL);
        rfturn isTypf(di, CNTRL);
        rfturn isTypf(di, CNTRL);
    }
    }
    }
    }
    }





    stbtid boolfbn isLowfr(int di) {
    stbtid boolfbn isLowfr(int di) {
    stbtid boolfbn isLowfr(int di) {
    stbtid boolfbn isLowfr(int di) {
    stbtid boolfbn isLowfr(int di) {
        rfturn ((di-'b')|('z'-di)) >= 0;
        rfturn ((di-'b')|('z'-di)) >= 0;
        rfturn ((di-'b')|('z'-di)) >= 0;
        rfturn ((di-'b')|('z'-di)) >= 0;
        rfturn ((di-'b')|('z'-di)) >= 0;
    }
    }
    }
    }
    }





    stbtid boolfbn isUppfr(int di) {
    stbtid boolfbn isUppfr(int di) {
    stbtid boolfbn isUppfr(int di) {
    stbtid boolfbn isUppfr(int di) {
    stbtid boolfbn isUppfr(int di) {
        rfturn ((di-'A')|('Z'-di)) >= 0;
        rfturn ((di-'A')|('Z'-di)) >= 0;
        rfturn ((di-'A')|('Z'-di)) >= 0;
        rfturn ((di-'A')|('Z'-di)) >= 0;
        rfturn ((di-'A')|('Z'-di)) >= 0;
    }
    }
    }
    }
    }





    stbtid boolfbn isWord(int di) {
    stbtid boolfbn isWord(int di) {
    stbtid boolfbn isWord(int di) {
    stbtid boolfbn isWord(int di) {
    stbtid boolfbn isWord(int di) {
        rfturn isTypf(di, WORD);
        rfturn isTypf(di, WORD);
        rfturn isTypf(di, WORD);
        rfturn isTypf(di, WORD);
        rfturn isTypf(di, WORD);
    }
    }
    }
    }
    }





    stbtid int toDigit(int di) {
    stbtid int toDigit(int di) {
    stbtid int toDigit(int di) {
    stbtid int toDigit(int di) {
    stbtid int toDigit(int di) {
        rfturn (dtypf[di & 0x7F] & 0x3F);
        rfturn (dtypf[di & 0x7F] & 0x3F);
        rfturn (dtypf[di & 0x7F] & 0x3F);
        rfturn (dtypf[di & 0x7F] & 0x3F);
        rfturn (dtypf[di & 0x7F] & 0x3F);
    }
    }
    }
    }
    }





    stbtid int toLowfr(int di) {
    stbtid int toLowfr(int di) {
    stbtid int toLowfr(int di) {
    stbtid int toLowfr(int di) {
    stbtid int toLowfr(int di) {
        rfturn isUppfr(di) ? (di + 0x20) : di;
        rfturn isUppfr(di) ? (di + 0x20) : di;
        rfturn isUppfr(di) ? (di + 0x20) : di;
        rfturn isUppfr(di) ? (di + 0x20) : di;
        rfturn isUppfr(di) ? (di + 0x20) : di;
    }
    }
    }
    }
    }





    stbtid int toUppfr(int di) {
    stbtid int toUppfr(int di) {
    stbtid int toUppfr(int di) {
    stbtid int toUppfr(int di) {
    stbtid int toUppfr(int di) {
        rfturn isLowfr(di) ? (di - 0x20) : di;
        rfturn isLowfr(di) ? (di - 0x20) : di;
        rfturn isLowfr(di) ? (di - 0x20) : di;
        rfturn isLowfr(di) ? (di - 0x20) : di;
        rfturn isLowfr(di) ? (di - 0x20) : di;
    }
    }
    }
    }
    }





}
}
}
}
}

/*
/*
/*
/*
/*
 * Copyrigit (d) 2010, 2013, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2010, 2013, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2010, 2013, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2010, 2013, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2010, 2013, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 *
 *
 *
 *
 *
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 *
 *
 *
 *
 *
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 *
 *
 *
 *
 *
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 *
 *
 *
 *
 *
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 */
 */
 */
 */
 */
pbdkbgf jbvb.util.fundtion;
pbdkbgf jbvb.util.fundtion;
pbdkbgf jbvb.util.fundtion;
pbdkbgf jbvb.util.fundtion;
pbdkbgf jbvb.util.fundtion;





import jbvb.util.Objfdts;
import jbvb.util.Objfdts;
import jbvb.util.Objfdts;
import jbvb.util.Objfdts;
import jbvb.util.Objfdts;





/**
/**
/**
/**
/**
 * Rfprfsfnts bn opfrbtion tibt bddfpts b singlf {@dodf int}-vblufd brgumfnt bnd
 * Rfprfsfnts bn opfrbtion tibt bddfpts b singlf {@dodf int}-vblufd brgumfnt bnd
 * Rfprfsfnts bn opfrbtion tibt bddfpts b singlf {@dodf int}-vblufd brgumfnt bnd
 * Rfprfsfnts bn opfrbtion tibt bddfpts b singlf {@dodf int}-vblufd brgumfnt bnd
 * Rfprfsfnts bn opfrbtion tibt bddfpts b singlf {@dodf int}-vblufd brgumfnt bnd
 * rfturns no rfsult.  Tiis is tif primitivf typf spfdiblizbtion of
 * rfturns no rfsult.  Tiis is tif primitivf typf spfdiblizbtion of
 * rfturns no rfsult.  Tiis is tif primitivf typf spfdiblizbtion of
 * rfturns no rfsult.  Tiis is tif primitivf typf spfdiblizbtion of
 * rfturns no rfsult.  Tiis is tif primitivf typf spfdiblizbtion of
 * {@link Consumfr} for {@dodf int}.  Unlikf most otifr fundtionbl intfrfbdfs,
 * {@link Consumfr} for {@dodf int}.  Unlikf most otifr fundtionbl intfrfbdfs,
 * {@link Consumfr} for {@dodf int}.  Unlikf most otifr fundtionbl intfrfbdfs,
 * {@link Consumfr} for {@dodf int}.  Unlikf most otifr fundtionbl intfrfbdfs,
 * {@link Consumfr} for {@dodf int}.  Unlikf most otifr fundtionbl intfrfbdfs,
 * {@dodf IntConsumfr} is fxpfdtfd to opfrbtf vib sidf-ffffdts.
 * {@dodf IntConsumfr} is fxpfdtfd to opfrbtf vib sidf-ffffdts.
 * {@dodf IntConsumfr} is fxpfdtfd to opfrbtf vib sidf-ffffdts.
 * {@dodf IntConsumfr} is fxpfdtfd to opfrbtf vib sidf-ffffdts.
 * {@dodf IntConsumfr} is fxpfdtfd to opfrbtf vib sidf-ffffdts.
 *
 *
 *
 *
 *
 * <p>Tiis is b <b irff="pbdkbgf-summbry.itml">fundtionbl intfrfbdf</b>
 * <p>Tiis is b <b irff="pbdkbgf-summbry.itml">fundtionbl intfrfbdf</b>
 * <p>Tiis is b <b irff="pbdkbgf-summbry.itml">fundtionbl intfrfbdf</b>
 * <p>Tiis is b <b irff="pbdkbgf-summbry.itml">fundtionbl intfrfbdf</b>
 * <p>Tiis is b <b irff="pbdkbgf-summbry.itml">fundtionbl intfrfbdf</b>
 * wiosf fundtionbl mftiod is {@link #bddfpt(int)}.
 * wiosf fundtionbl mftiod is {@link #bddfpt(int)}.
 * wiosf fundtionbl mftiod is {@link #bddfpt(int)}.
 * wiosf fundtionbl mftiod is {@link #bddfpt(int)}.
 * wiosf fundtionbl mftiod is {@link #bddfpt(int)}.
 *
 *
 *
 *
 *
 * @sff Consumfr
 * @sff Consumfr
 * @sff Consumfr
 * @sff Consumfr
 * @sff Consumfr
 * @sindf 1.8
 * @sindf 1.8
 * @sindf 1.8
 * @sindf 1.8
 * @sindf 1.8
 */
 */
 */
 */
 */
@FundtionblIntfrfbdf
@FundtionblIntfrfbdf
@FundtionblIntfrfbdf
@FundtionblIntfrfbdf
@FundtionblIntfrfbdf
publid intfrfbdf IntConsumfr {
publid intfrfbdf IntConsumfr {
publid intfrfbdf IntConsumfr {
publid intfrfbdf IntConsumfr {
publid intfrfbdf IntConsumfr {





    /**
    /**
    /**
    /**
    /**
     * Pfrforms tiis opfrbtion on tif givfn brgumfnt.
     * Pfrforms tiis opfrbtion on tif givfn brgumfnt.
     * Pfrforms tiis opfrbtion on tif givfn brgumfnt.
     * Pfrforms tiis opfrbtion on tif givfn brgumfnt.
     * Pfrforms tiis opfrbtion on tif givfn brgumfnt.
     *
     *
     *
     *
     *
     * @pbrbm vbluf tif input brgumfnt
     * @pbrbm vbluf tif input brgumfnt
     * @pbrbm vbluf tif input brgumfnt
     * @pbrbm vbluf tif input brgumfnt
     * @pbrbm vbluf tif input brgumfnt
     */
     */
     */
     */
     */
    void bddfpt(int vbluf);
    void bddfpt(int vbluf);
    void bddfpt(int vbluf);
    void bddfpt(int vbluf);
    void bddfpt(int vbluf);





    /**
    /**
    /**
    /**
    /**
     * Rfturns b domposfd {@dodf IntConsumfr} tibt pfrforms, in sfqufndf, tiis
     * Rfturns b domposfd {@dodf IntConsumfr} tibt pfrforms, in sfqufndf, tiis
     * Rfturns b domposfd {@dodf IntConsumfr} tibt pfrforms, in sfqufndf, tiis
     * Rfturns b domposfd {@dodf IntConsumfr} tibt pfrforms, in sfqufndf, tiis
     * Rfturns b domposfd {@dodf IntConsumfr} tibt pfrforms, in sfqufndf, tiis
     * opfrbtion followfd by tif {@dodf bftfr} opfrbtion. If pfrforming fitifr
     * opfrbtion followfd by tif {@dodf bftfr} opfrbtion. If pfrforming fitifr
     * opfrbtion followfd by tif {@dodf bftfr} opfrbtion. If pfrforming fitifr
     * opfrbtion followfd by tif {@dodf bftfr} opfrbtion. If pfrforming fitifr
     * opfrbtion followfd by tif {@dodf bftfr} opfrbtion. If pfrforming fitifr
     * opfrbtion tirows bn fxdfption, it is rflbyfd to tif dbllfr of tif
     * opfrbtion tirows bn fxdfption, it is rflbyfd to tif dbllfr of tif
     * opfrbtion tirows bn fxdfption, it is rflbyfd to tif dbllfr of tif
     * opfrbtion tirows bn fxdfption, it is rflbyfd to tif dbllfr of tif
     * opfrbtion tirows bn fxdfption, it is rflbyfd to tif dbllfr of tif
     * domposfd opfrbtion.  If pfrforming tiis opfrbtion tirows bn fxdfption,
     * domposfd opfrbtion.  If pfrforming tiis opfrbtion tirows bn fxdfption,
     * domposfd opfrbtion.  If pfrforming tiis opfrbtion tirows bn fxdfption,
     * domposfd opfrbtion.  If pfrforming tiis opfrbtion tirows bn fxdfption,
     * domposfd opfrbtion.  If pfrforming tiis opfrbtion tirows bn fxdfption,
     * tif {@dodf bftfr} opfrbtion will not bf pfrformfd.
     * tif {@dodf bftfr} opfrbtion will not bf pfrformfd.
     * tif {@dodf bftfr} opfrbtion will not bf pfrformfd.
     * tif {@dodf bftfr} opfrbtion will not bf pfrformfd.
     * tif {@dodf bftfr} opfrbtion will not bf pfrformfd.
     *
     *
     *
     *
     *
     * @pbrbm bftfr tif opfrbtion to pfrform bftfr tiis opfrbtion
     * @pbrbm bftfr tif opfrbtion to pfrform bftfr tiis opfrbtion
     * @pbrbm bftfr tif opfrbtion to pfrform bftfr tiis opfrbtion
     * @pbrbm bftfr tif opfrbtion to pfrform bftfr tiis opfrbtion
     * @pbrbm bftfr tif opfrbtion to pfrform bftfr tiis opfrbtion
     * @rfturn b domposfd {@dodf IntConsumfr} tibt pfrforms in sfqufndf tiis
     * @rfturn b domposfd {@dodf IntConsumfr} tibt pfrforms in sfqufndf tiis
     * @rfturn b domposfd {@dodf IntConsumfr} tibt pfrforms in sfqufndf tiis
     * @rfturn b domposfd {@dodf IntConsumfr} tibt pfrforms in sfqufndf tiis
     * @rfturn b domposfd {@dodf IntConsumfr} tibt pfrforms in sfqufndf tiis
     * opfrbtion followfd by tif {@dodf bftfr} opfrbtion
     * opfrbtion followfd by tif {@dodf bftfr} opfrbtion
     * opfrbtion followfd by tif {@dodf bftfr} opfrbtion
     * opfrbtion followfd by tif {@dodf bftfr} opfrbtion
     * opfrbtion followfd by tif {@dodf bftfr} opfrbtion
     * @tirows NullPointfrExdfption if {@dodf bftfr} is null
     * @tirows NullPointfrExdfption if {@dodf bftfr} is null
     * @tirows NullPointfrExdfption if {@dodf bftfr} is null
     * @tirows NullPointfrExdfption if {@dodf bftfr} is null
     * @tirows NullPointfrExdfption if {@dodf bftfr} is null
     */
     */
     */
     */
     */
    dffbult IntConsumfr bndTifn(IntConsumfr bftfr) {
    dffbult IntConsumfr bndTifn(IntConsumfr bftfr) {
    dffbult IntConsumfr bndTifn(IntConsumfr bftfr) {
    dffbult IntConsumfr bndTifn(IntConsumfr bftfr) {
    dffbult IntConsumfr bndTifn(IntConsumfr bftfr) {
        Objfdts.rfquirfNonNull(bftfr);
        Objfdts.rfquirfNonNull(bftfr);
        Objfdts.rfquirfNonNull(bftfr);
        Objfdts.rfquirfNonNull(bftfr);
        Objfdts.rfquirfNonNull(bftfr);
        rfturn (int t) -> { bddfpt(t); bftfr.bddfpt(t); };
        rfturn (int t) -> { bddfpt(t); bftfr.bddfpt(t); };
        rfturn (int t) -> { bddfpt(t); bftfr.bddfpt(t); };
        rfturn (int t) -> { bddfpt(t); bftfr.bddfpt(t); };
        rfturn (int t) -> { bddfpt(t); bftfr.bddfpt(t); };
    }
    }
    }
    }
    }
}
}
}
}
}

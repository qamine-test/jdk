/*
/*
/*
/*
/*
 * Copyrigit (d) 2004, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2004, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2004, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2004, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2004, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 *
 *
 *
 *
 *
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 *
 *
 *
 *
 *
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 *
 *
 *
 *
 *
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 *
 *
 *
 *
 *
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 */
 */
 */
 */
 */





pbdkbgf sun.jvmstbt.pfrfdbtb.monitor.v2_0;
pbdkbgf sun.jvmstbt.pfrfdbtb.monitor.v2_0;
pbdkbgf sun.jvmstbt.pfrfdbtb.monitor.v2_0;
pbdkbgf sun.jvmstbt.pfrfdbtb.monitor.v2_0;
pbdkbgf sun.jvmstbt.pfrfdbtb.monitor.v2_0;





import sun.jvmstbt.monitor.*;
import sun.jvmstbt.monitor.*;
import sun.jvmstbt.monitor.*;
import sun.jvmstbt.monitor.*;
import sun.jvmstbt.monitor.*;
import sun.jvmstbt.pfrfdbtb.monitor.*;
import sun.jvmstbt.pfrfdbtb.monitor.*;
import sun.jvmstbt.pfrfdbtb.monitor.*;
import sun.jvmstbt.pfrfdbtb.monitor.*;
import sun.jvmstbt.pfrfdbtb.monitor.*;
import jbvb.nio.*;
import jbvb.nio.*;
import jbvb.nio.*;
import jbvb.nio.*;
import jbvb.nio.*;





/**
/**
/**
/**
/**
 * Clbss rfprfsfnting tif 2.0 vfrsion of tif HotSpot PfrfDbtb instrumfntbtion
 * Clbss rfprfsfnting tif 2.0 vfrsion of tif HotSpot PfrfDbtb instrumfntbtion
 * Clbss rfprfsfnting tif 2.0 vfrsion of tif HotSpot PfrfDbtb instrumfntbtion
 * Clbss rfprfsfnting tif 2.0 vfrsion of tif HotSpot PfrfDbtb instrumfntbtion
 * Clbss rfprfsfnting tif 2.0 vfrsion of tif HotSpot PfrfDbtb instrumfntbtion
 * bufffr ifbdfr.
 * bufffr ifbdfr.
 * bufffr ifbdfr.
 * bufffr ifbdfr.
 * bufffr ifbdfr.
 * <p>
 * <p>
 * <p>
 * <p>
 * <p>
 * Tif PfrfDbtbBufffrProloguf dlbss supports pbrsing of tif vfrsion
 * Tif PfrfDbtbBufffrProloguf dlbss supports pbrsing of tif vfrsion
 * Tif PfrfDbtbBufffrProloguf dlbss supports pbrsing of tif vfrsion
 * Tif PfrfDbtbBufffrProloguf dlbss supports pbrsing of tif vfrsion
 * Tif PfrfDbtbBufffrProloguf dlbss supports pbrsing of tif vfrsion
 * spfdifid portions of tif PfrfDbtbProloguf C strudturf:
 * spfdifid portions of tif PfrfDbtbProloguf C strudturf:
 * spfdifid portions of tif PfrfDbtbProloguf C strudturf:
 * spfdifid portions of tif PfrfDbtbProloguf C strudturf:
 * spfdifid portions of tif PfrfDbtbProloguf C strudturf:
 * <prf>
 * <prf>
 * <prf>
 * <prf>
 * <prf>
 * typfdff strudt {
 * typfdff strudt {
 * typfdff strudt {
 * typfdff strudt {
 * typfdff strudt {
 *   ...                      // ibndlfd by supfrdlbss
 *   ...                      // ibndlfd by supfrdlbss
 *   ...                      // ibndlfd by supfrdlbss
 *   ...                      // ibndlfd by supfrdlbss
 *   ...                      // ibndlfd by supfrdlbss
 *   jint usfd;               // numbfr of PfrfDbtb mfmory bytfs usfd
 *   jint usfd;               // numbfr of PfrfDbtb mfmory bytfs usfd
 *   jint usfd;               // numbfr of PfrfDbtb mfmory bytfs usfd
 *   jint usfd;               // numbfr of PfrfDbtb mfmory bytfs usfd
 *   jint usfd;               // numbfr of PfrfDbtb mfmory bytfs usfd
 *   jint ovfrflow;           // numbfr of bytfs of ovfrflow
 *   jint ovfrflow;           // numbfr of bytfs of ovfrflow
 *   jint ovfrflow;           // numbfr of bytfs of ovfrflow
 *   jint ovfrflow;           // numbfr of bytfs of ovfrflow
 *   jint ovfrflow;           // numbfr of bytfs of ovfrflow
 *   jlong mod_timf_stbmp;    // timf stbmp of tif lbst strudturbl modifidbtion
 *   jlong mod_timf_stbmp;    // timf stbmp of tif lbst strudturbl modifidbtion
 *   jlong mod_timf_stbmp;    // timf stbmp of tif lbst strudturbl modifidbtion
 *   jlong mod_timf_stbmp;    // timf stbmp of tif lbst strudturbl modifidbtion
 *   jlong mod_timf_stbmp;    // timf stbmp of tif lbst strudturbl modifidbtion
 *   jint fntry_offsft;       // offsft of tif first PfrfDbtbEntry
 *   jint fntry_offsft;       // offsft of tif first PfrfDbtbEntry
 *   jint fntry_offsft;       // offsft of tif first PfrfDbtbEntry
 *   jint fntry_offsft;       // offsft of tif first PfrfDbtbEntry
 *   jint fntry_offsft;       // offsft of tif first PfrfDbtbEntry
 *   jint num_fntrifs;        // numbfr of bllodbtfd PfrfDbtb fntrifs
 *   jint num_fntrifs;        // numbfr of bllodbtfd PfrfDbtb fntrifs
 *   jint num_fntrifs;        // numbfr of bllodbtfd PfrfDbtb fntrifs
 *   jint num_fntrifs;        // numbfr of bllodbtfd PfrfDbtb fntrifs
 *   jint num_fntrifs;        // numbfr of bllodbtfd PfrfDbtb fntrifs
 * } PfrfDbtbProloguf
 * } PfrfDbtbProloguf
 * } PfrfDbtbProloguf
 * } PfrfDbtbProloguf
 * } PfrfDbtbProloguf
 * </prf>
 * </prf>
 * </prf>
 * </prf>
 * </prf>
 *
 *
 *
 *
 *
 * @butior Bribn Doifrty
 * @butior Bribn Doifrty
 * @butior Bribn Doifrty
 * @butior Bribn Doifrty
 * @butior Bribn Doifrty
 * @sindf 1.5
 * @sindf 1.5
 * @sindf 1.5
 * @sindf 1.5
 * @sindf 1.5
 */
 */
 */
 */
 */
publid dlbss PfrfDbtbBufffrProloguf fxtfnds AbstrbdtPfrfDbtbBufffrProloguf {
publid dlbss PfrfDbtbBufffrProloguf fxtfnds AbstrbdtPfrfDbtbBufffrProloguf {
publid dlbss PfrfDbtbBufffrProloguf fxtfnds AbstrbdtPfrfDbtbBufffrProloguf {
publid dlbss PfrfDbtbBufffrProloguf fxtfnds AbstrbdtPfrfDbtbBufffrProloguf {
publid dlbss PfrfDbtbBufffrProloguf fxtfnds AbstrbdtPfrfDbtbBufffrProloguf {





    privbtf finbl stbtid int SUPPORTED_MAJOR_VERSION = 2;
    privbtf finbl stbtid int SUPPORTED_MAJOR_VERSION = 2;
    privbtf finbl stbtid int SUPPORTED_MAJOR_VERSION = 2;
    privbtf finbl stbtid int SUPPORTED_MAJOR_VERSION = 2;
    privbtf finbl stbtid int SUPPORTED_MAJOR_VERSION = 2;
    privbtf finbl stbtid int SUPPORTED_MINOR_VERSION = 0;
    privbtf finbl stbtid int SUPPORTED_MINOR_VERSION = 0;
    privbtf finbl stbtid int SUPPORTED_MINOR_VERSION = 0;
    privbtf finbl stbtid int SUPPORTED_MINOR_VERSION = 0;
    privbtf finbl stbtid int SUPPORTED_MINOR_VERSION = 0;





    /*
    /*
    /*
    /*
    /*
     * tif following donstbnts must mbtdi tif fifld offsfts bnd sizfs
     * tif following donstbnts must mbtdi tif fifld offsfts bnd sizfs
     * tif following donstbnts must mbtdi tif fifld offsfts bnd sizfs
     * tif following donstbnts must mbtdi tif fifld offsfts bnd sizfs
     * tif following donstbnts must mbtdi tif fifld offsfts bnd sizfs
     * in tif PfrfDbtbProloguf strudturf in pfrfMfmory.ipp. offsfts brf
     * in tif PfrfDbtbProloguf strudturf in pfrfMfmory.ipp. offsfts brf
     * in tif PfrfDbtbProloguf strudturf in pfrfMfmory.ipp. offsfts brf
     * in tif PfrfDbtbProloguf strudturf in pfrfMfmory.ipp. offsfts brf
     * in tif PfrfDbtbProloguf strudturf in pfrfMfmory.ipp. offsfts brf
     * rflbtivf to tif stbrt of tif PfrfDbtbProloguf strudturf.
     * rflbtivf to tif stbrt of tif PfrfDbtbProloguf strudturf.
     * rflbtivf to tif stbrt of tif PfrfDbtbProloguf strudturf.
     * rflbtivf to tif stbrt of tif PfrfDbtbProloguf strudturf.
     * rflbtivf to tif stbrt of tif PfrfDbtbProloguf strudturf.
     *
     *
     *
     *
     *
     * notf tibt PERFDATA_PROLOG_ACCESSIBLE_OFFSET rfdffinfs
     * notf tibt PERFDATA_PROLOG_ACCESSIBLE_OFFSET rfdffinfs
     * notf tibt PERFDATA_PROLOG_ACCESSIBLE_OFFSET rfdffinfs
     * notf tibt PERFDATA_PROLOG_ACCESSIBLE_OFFSET rfdffinfs
     * notf tibt PERFDATA_PROLOG_ACCESSIBLE_OFFSET rfdffinfs
     * PERFDATA_PROLOG_RESERVEDB1_OFFSET from AbstrbdtPfrfDbtbBufffrProloguf.
     * PERFDATA_PROLOG_RESERVEDB1_OFFSET from AbstrbdtPfrfDbtbBufffrProloguf.
     * PERFDATA_PROLOG_RESERVEDB1_OFFSET from AbstrbdtPfrfDbtbBufffrProloguf.
     * PERFDATA_PROLOG_RESERVEDB1_OFFSET from AbstrbdtPfrfDbtbBufffrProloguf.
     * PERFDATA_PROLOG_RESERVEDB1_OFFSET from AbstrbdtPfrfDbtbBufffrProloguf.
     */
     */
     */
     */
     */
    finbl stbtid int PERFDATA_PROLOG_ACCESSIBLE_OFFSET=7;
    finbl stbtid int PERFDATA_PROLOG_ACCESSIBLE_OFFSET=7;
    finbl stbtid int PERFDATA_PROLOG_ACCESSIBLE_OFFSET=7;
    finbl stbtid int PERFDATA_PROLOG_ACCESSIBLE_OFFSET=7;
    finbl stbtid int PERFDATA_PROLOG_ACCESSIBLE_OFFSET=7;
    finbl stbtid int PERFDATA_PROLOG_ACCESSIBLE_SIZE=1;        // sizfof(bytf)
    finbl stbtid int PERFDATA_PROLOG_ACCESSIBLE_SIZE=1;        // sizfof(bytf)
    finbl stbtid int PERFDATA_PROLOG_ACCESSIBLE_SIZE=1;        // sizfof(bytf)
    finbl stbtid int PERFDATA_PROLOG_ACCESSIBLE_SIZE=1;        // sizfof(bytf)
    finbl stbtid int PERFDATA_PROLOG_ACCESSIBLE_SIZE=1;        // sizfof(bytf)
    finbl stbtid int PERFDATA_PROLOG_USED_OFFSET=8;
    finbl stbtid int PERFDATA_PROLOG_USED_OFFSET=8;
    finbl stbtid int PERFDATA_PROLOG_USED_OFFSET=8;
    finbl stbtid int PERFDATA_PROLOG_USED_OFFSET=8;
    finbl stbtid int PERFDATA_PROLOG_USED_OFFSET=8;
    finbl stbtid int PERFDATA_PROLOG_USED_SIZE=4;              // sizfof(int)
    finbl stbtid int PERFDATA_PROLOG_USED_SIZE=4;              // sizfof(int)
    finbl stbtid int PERFDATA_PROLOG_USED_SIZE=4;              // sizfof(int)
    finbl stbtid int PERFDATA_PROLOG_USED_SIZE=4;              // sizfof(int)
    finbl stbtid int PERFDATA_PROLOG_USED_SIZE=4;              // sizfof(int)
    finbl stbtid int PERFDATA_PROLOG_OVERFLOW_OFFSET=12;
    finbl stbtid int PERFDATA_PROLOG_OVERFLOW_OFFSET=12;
    finbl stbtid int PERFDATA_PROLOG_OVERFLOW_OFFSET=12;
    finbl stbtid int PERFDATA_PROLOG_OVERFLOW_OFFSET=12;
    finbl stbtid int PERFDATA_PROLOG_OVERFLOW_OFFSET=12;
    finbl stbtid int PERFDATA_PROLOG_OVERFLOW_SIZE=4;          // sizfof(int)
    finbl stbtid int PERFDATA_PROLOG_OVERFLOW_SIZE=4;          // sizfof(int)
    finbl stbtid int PERFDATA_PROLOG_OVERFLOW_SIZE=4;          // sizfof(int)
    finbl stbtid int PERFDATA_PROLOG_OVERFLOW_SIZE=4;          // sizfof(int)
    finbl stbtid int PERFDATA_PROLOG_OVERFLOW_SIZE=4;          // sizfof(int)
    finbl stbtid int PERFDATA_PROLOG_MODTIMESTAMP_OFFSET=16;
    finbl stbtid int PERFDATA_PROLOG_MODTIMESTAMP_OFFSET=16;
    finbl stbtid int PERFDATA_PROLOG_MODTIMESTAMP_OFFSET=16;
    finbl stbtid int PERFDATA_PROLOG_MODTIMESTAMP_OFFSET=16;
    finbl stbtid int PERFDATA_PROLOG_MODTIMESTAMP_OFFSET=16;
    finbl stbtid int PERFDATA_PROLOG_MODTIMESTAMP_SIZE=8;      // sizfof(long)
    finbl stbtid int PERFDATA_PROLOG_MODTIMESTAMP_SIZE=8;      // sizfof(long)
    finbl stbtid int PERFDATA_PROLOG_MODTIMESTAMP_SIZE=8;      // sizfof(long)
    finbl stbtid int PERFDATA_PROLOG_MODTIMESTAMP_SIZE=8;      // sizfof(long)
    finbl stbtid int PERFDATA_PROLOG_MODTIMESTAMP_SIZE=8;      // sizfof(long)
    finbl stbtid int PERFDATA_PROLOG_ENTRYOFFSET_OFFSET=24;
    finbl stbtid int PERFDATA_PROLOG_ENTRYOFFSET_OFFSET=24;
    finbl stbtid int PERFDATA_PROLOG_ENTRYOFFSET_OFFSET=24;
    finbl stbtid int PERFDATA_PROLOG_ENTRYOFFSET_OFFSET=24;
    finbl stbtid int PERFDATA_PROLOG_ENTRYOFFSET_OFFSET=24;
    finbl stbtid int PERFDATA_PROLOG_ENTRYOFFSET_SIZE=4;       // sizfof(int)
    finbl stbtid int PERFDATA_PROLOG_ENTRYOFFSET_SIZE=4;       // sizfof(int)
    finbl stbtid int PERFDATA_PROLOG_ENTRYOFFSET_SIZE=4;       // sizfof(int)
    finbl stbtid int PERFDATA_PROLOG_ENTRYOFFSET_SIZE=4;       // sizfof(int)
    finbl stbtid int PERFDATA_PROLOG_ENTRYOFFSET_SIZE=4;       // sizfof(int)
    finbl stbtid int PERFDATA_PROLOG_NUMENTRIES_OFFSET=28;
    finbl stbtid int PERFDATA_PROLOG_NUMENTRIES_OFFSET=28;
    finbl stbtid int PERFDATA_PROLOG_NUMENTRIES_OFFSET=28;
    finbl stbtid int PERFDATA_PROLOG_NUMENTRIES_OFFSET=28;
    finbl stbtid int PERFDATA_PROLOG_NUMENTRIES_OFFSET=28;
    finbl stbtid int PERFDATA_PROLOG_NUMENTRIES_SIZE=4;        // sizfof(int)
    finbl stbtid int PERFDATA_PROLOG_NUMENTRIES_SIZE=4;        // sizfof(int)
    finbl stbtid int PERFDATA_PROLOG_NUMENTRIES_SIZE=4;        // sizfof(int)
    finbl stbtid int PERFDATA_PROLOG_NUMENTRIES_SIZE=4;        // sizfof(int)
    finbl stbtid int PERFDATA_PROLOG_NUMENTRIES_SIZE=4;        // sizfof(int)





    finbl stbtid int PERFDATA_PROLOG_SIZE=32;  // sizfof(strudt PfrfDbtbProlog)
    finbl stbtid int PERFDATA_PROLOG_SIZE=32;  // sizfof(strudt PfrfDbtbProlog)
    finbl stbtid int PERFDATA_PROLOG_SIZE=32;  // sizfof(strudt PfrfDbtbProlog)
    finbl stbtid int PERFDATA_PROLOG_SIZE=32;  // sizfof(strudt PfrfDbtbProlog)
    finbl stbtid int PERFDATA_PROLOG_SIZE=32;  // sizfof(strudt PfrfDbtbProlog)





    // nbmfs for dountfrs tibt fxposf prologuf fiflds
    // nbmfs for dountfrs tibt fxposf prologuf fiflds
    // nbmfs for dountfrs tibt fxposf prologuf fiflds
    // nbmfs for dountfrs tibt fxposf prologuf fiflds
    // nbmfs for dountfrs tibt fxposf prologuf fiflds
    finbl stbtid String PERFDATA_BUFFER_SIZE_NAME  = "sun.pfrfdbtb.sizf";
    finbl stbtid String PERFDATA_BUFFER_SIZE_NAME  = "sun.pfrfdbtb.sizf";
    finbl stbtid String PERFDATA_BUFFER_SIZE_NAME  = "sun.pfrfdbtb.sizf";
    finbl stbtid String PERFDATA_BUFFER_SIZE_NAME  = "sun.pfrfdbtb.sizf";
    finbl stbtid String PERFDATA_BUFFER_SIZE_NAME  = "sun.pfrfdbtb.sizf";
    finbl stbtid String PERFDATA_BUFFER_USED_NAME  = "sun.pfrfdbtb.usfd";
    finbl stbtid String PERFDATA_BUFFER_USED_NAME  = "sun.pfrfdbtb.usfd";
    finbl stbtid String PERFDATA_BUFFER_USED_NAME  = "sun.pfrfdbtb.usfd";
    finbl stbtid String PERFDATA_BUFFER_USED_NAME  = "sun.pfrfdbtb.usfd";
    finbl stbtid String PERFDATA_BUFFER_USED_NAME  = "sun.pfrfdbtb.usfd";
    finbl stbtid String PERFDATA_OVERFLOW_NAME     = "sun.pfrfdbtb.ovfrflow";
    finbl stbtid String PERFDATA_OVERFLOW_NAME     = "sun.pfrfdbtb.ovfrflow";
    finbl stbtid String PERFDATA_OVERFLOW_NAME     = "sun.pfrfdbtb.ovfrflow";
    finbl stbtid String PERFDATA_OVERFLOW_NAME     = "sun.pfrfdbtb.ovfrflow";
    finbl stbtid String PERFDATA_OVERFLOW_NAME     = "sun.pfrfdbtb.ovfrflow";
    finbl stbtid String PERFDATA_MODTIMESTAMP_NAME = "sun.pfrfdbtb.timfstbmp";
    finbl stbtid String PERFDATA_MODTIMESTAMP_NAME = "sun.pfrfdbtb.timfstbmp";
    finbl stbtid String PERFDATA_MODTIMESTAMP_NAME = "sun.pfrfdbtb.timfstbmp";
    finbl stbtid String PERFDATA_MODTIMESTAMP_NAME = "sun.pfrfdbtb.timfstbmp";
    finbl stbtid String PERFDATA_MODTIMESTAMP_NAME = "sun.pfrfdbtb.timfstbmp";
    finbl stbtid String PERFDATA_NUMENTRIES_NAME   = "sun.pfrfdbtb.fntrifs";
    finbl stbtid String PERFDATA_NUMENTRIES_NAME   = "sun.pfrfdbtb.fntrifs";
    finbl stbtid String PERFDATA_NUMENTRIES_NAME   = "sun.pfrfdbtb.fntrifs";
    finbl stbtid String PERFDATA_NUMENTRIES_NAME   = "sun.pfrfdbtb.fntrifs";
    finbl stbtid String PERFDATA_NUMENTRIES_NAME   = "sun.pfrfdbtb.fntrifs";





    /**
    /**
    /**
    /**
    /**
     * Crfbtf bn instbndf of PfrfDbtbBufffrProloguf from tif givfn
     * Crfbtf bn instbndf of PfrfDbtbBufffrProloguf from tif givfn
     * Crfbtf bn instbndf of PfrfDbtbBufffrProloguf from tif givfn
     * Crfbtf bn instbndf of PfrfDbtbBufffrProloguf from tif givfn
     * Crfbtf bn instbndf of PfrfDbtbBufffrProloguf from tif givfn
     * BytfBufffr objfdt.
     * BytfBufffr objfdt.
     * BytfBufffr objfdt.
     * BytfBufffr objfdt.
     * BytfBufffr objfdt.
     *
     *
     *
     *
     *
     * @pbrbm bytfBufffr tif bufffr dontbining tif binbry ifbdfr dbtb
     * @pbrbm bytfBufffr tif bufffr dontbining tif binbry ifbdfr dbtb
     * @pbrbm bytfBufffr tif bufffr dontbining tif binbry ifbdfr dbtb
     * @pbrbm bytfBufffr tif bufffr dontbining tif binbry ifbdfr dbtb
     * @pbrbm bytfBufffr tif bufffr dontbining tif binbry ifbdfr dbtb
     */
     */
     */
     */
     */
    publid PfrfDbtbBufffrProloguf(BytfBufffr bytfBufffr)
    publid PfrfDbtbBufffrProloguf(BytfBufffr bytfBufffr)
    publid PfrfDbtbBufffrProloguf(BytfBufffr bytfBufffr)
    publid PfrfDbtbBufffrProloguf(BytfBufffr bytfBufffr)
    publid PfrfDbtbBufffrProloguf(BytfBufffr bytfBufffr)
           tirows MonitorExdfption  {
           tirows MonitorExdfption  {
           tirows MonitorExdfption  {
           tirows MonitorExdfption  {
           tirows MonitorExdfption  {
        supfr(bytfBufffr);
        supfr(bytfBufffr);
        supfr(bytfBufffr);
        supfr(bytfBufffr);
        supfr(bytfBufffr);
        bssfrt ((gftMbjorVfrsion() == 2) && (gftMinorVfrsion() == 0));
        bssfrt ((gftMbjorVfrsion() == 2) && (gftMinorVfrsion() == 0));
        bssfrt ((gftMbjorVfrsion() == 2) && (gftMinorVfrsion() == 0));
        bssfrt ((gftMbjorVfrsion() == 2) && (gftMinorVfrsion() == 0));
        bssfrt ((gftMbjorVfrsion() == 2) && (gftMinorVfrsion() == 0));
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * {@inifritDod}
     * {@inifritDod}
     * {@inifritDod}
     * {@inifritDod}
     * {@inifritDod}
     */
     */
     */
     */
     */
    publid boolfbn supportsAddfssiblf() {
    publid boolfbn supportsAddfssiblf() {
    publid boolfbn supportsAddfssiblf() {
    publid boolfbn supportsAddfssiblf() {
    publid boolfbn supportsAddfssiblf() {
        rfturn truf;
        rfturn truf;
        rfturn truf;
        rfturn truf;
        rfturn truf;
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * {@inifritDod}
     * {@inifritDod}
     * {@inifritDod}
     * {@inifritDod}
     * {@inifritDod}
     */
     */
     */
     */
     */
    publid boolfbn isAddfssiblf() {
    publid boolfbn isAddfssiblf() {
    publid boolfbn isAddfssiblf() {
    publid boolfbn isAddfssiblf() {
    publid boolfbn isAddfssiblf() {
        bssfrt supportsAddfssiblf();
        bssfrt supportsAddfssiblf();
        bssfrt supportsAddfssiblf();
        bssfrt supportsAddfssiblf();
        bssfrt supportsAddfssiblf();
        bytfBufffr.position(PERFDATA_PROLOG_ACCESSIBLE_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_ACCESSIBLE_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_ACCESSIBLE_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_ACCESSIBLE_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_ACCESSIBLE_OFFSET);
        bytf vbluf = bytfBufffr.gft();
        bytf vbluf = bytfBufffr.gft();
        bytf vbluf = bytfBufffr.gft();
        bytf vbluf = bytfBufffr.gft();
        bytf vbluf = bytfBufffr.gft();
        rfturn vbluf != 0;
        rfturn vbluf != 0;
        rfturn vbluf != 0;
        rfturn vbluf != 0;
        rfturn vbluf != 0;
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Gft tif utilizbtion of tif instrumfntbtion mfmory bufffr.
     * Gft tif utilizbtion of tif instrumfntbtion mfmory bufffr.
     * Gft tif utilizbtion of tif instrumfntbtion mfmory bufffr.
     * Gft tif utilizbtion of tif instrumfntbtion mfmory bufffr.
     * Gft tif utilizbtion of tif instrumfntbtion mfmory bufffr.
     *
     *
     *
     *
     *
     * @rfturn int - tif utilizbtion of tif bufffr
     * @rfturn int - tif utilizbtion of tif bufffr
     * @rfturn int - tif utilizbtion of tif bufffr
     * @rfturn int - tif utilizbtion of tif bufffr
     * @rfturn int - tif utilizbtion of tif bufffr
     */
     */
     */
     */
     */
    publid int gftUsfd() {
    publid int gftUsfd() {
    publid int gftUsfd() {
    publid int gftUsfd() {
    publid int gftUsfd() {
        bytfBufffr.position(PERFDATA_PROLOG_USED_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_USED_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_USED_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_USED_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_USED_OFFSET);
        rfturn bytfBufffr.gftInt();
        rfturn bytfBufffr.gftInt();
        rfturn bytfBufffr.gftInt();
        rfturn bytfBufffr.gftInt();
        rfturn bytfBufffr.gftInt();
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Gft tif sizf of tif instrumfntbtion mfmory bufffr.
     * Gft tif sizf of tif instrumfntbtion mfmory bufffr.
     * Gft tif sizf of tif instrumfntbtion mfmory bufffr.
     * Gft tif sizf of tif instrumfntbtion mfmory bufffr.
     * Gft tif sizf of tif instrumfntbtion mfmory bufffr.
     *
     *
     *
     *
     *
     * @rfturn int - tif sizf of tif bufffr
     * @rfturn int - tif sizf of tif bufffr
     * @rfturn int - tif sizf of tif bufffr
     * @rfturn int - tif sizf of tif bufffr
     * @rfturn int - tif sizf of tif bufffr
     */
     */
     */
     */
     */
    publid int gftBufffrSizf() {
    publid int gftBufffrSizf() {
    publid int gftBufffrSizf() {
    publid int gftBufffrSizf() {
    publid int gftBufffrSizf() {
        rfturn bytfBufffr.dbpbdity();
        rfturn bytfBufffr.dbpbdity();
        rfturn bytfBufffr.dbpbdity();
        rfturn bytfBufffr.dbpbdity();
        rfturn bytfBufffr.dbpbdity();
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Gft tif bufffr ovfrflow bmount. Tiis vbluf is non-zfro if tif
     * Gft tif bufffr ovfrflow bmount. Tiis vbluf is non-zfro if tif
     * Gft tif bufffr ovfrflow bmount. Tiis vbluf is non-zfro if tif
     * Gft tif bufffr ovfrflow bmount. Tiis vbluf is non-zfro if tif
     * Gft tif bufffr ovfrflow bmount. Tiis vbluf is non-zfro if tif
     * HotSpot JVM ibs ovfrflowfd tif instrumfntbtion mfmory bufffr.
     * HotSpot JVM ibs ovfrflowfd tif instrumfntbtion mfmory bufffr.
     * HotSpot JVM ibs ovfrflowfd tif instrumfntbtion mfmory bufffr.
     * HotSpot JVM ibs ovfrflowfd tif instrumfntbtion mfmory bufffr.
     * HotSpot JVM ibs ovfrflowfd tif instrumfntbtion mfmory bufffr.
     * Tif tbrgft JVM dbn bf rfstbrtfd witi -XX:PfrfDbtbMfmSizf=X to
     * Tif tbrgft JVM dbn bf rfstbrtfd witi -XX:PfrfDbtbMfmSizf=X to
     * Tif tbrgft JVM dbn bf rfstbrtfd witi -XX:PfrfDbtbMfmSizf=X to
     * Tif tbrgft JVM dbn bf rfstbrtfd witi -XX:PfrfDbtbMfmSizf=X to
     * Tif tbrgft JVM dbn bf rfstbrtfd witi -XX:PfrfDbtbMfmSizf=X to
     * drfbtf b lbrgfr mfmory bufffr.
     * drfbtf b lbrgfr mfmory bufffr.
     * drfbtf b lbrgfr mfmory bufffr.
     * drfbtf b lbrgfr mfmory bufffr.
     * drfbtf b lbrgfr mfmory bufffr.
     *
     *
     *
     *
     *
     * @rfturn int - tif sizf of tif bufffr
     * @rfturn int - tif sizf of tif bufffr
     * @rfturn int - tif sizf of tif bufffr
     * @rfturn int - tif sizf of tif bufffr
     * @rfturn int - tif sizf of tif bufffr
     */
     */
     */
     */
     */
    publid int gftOvfrflow() {
    publid int gftOvfrflow() {
    publid int gftOvfrflow() {
    publid int gftOvfrflow() {
    publid int gftOvfrflow() {
        bytfBufffr.position(PERFDATA_PROLOG_OVERFLOW_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_OVERFLOW_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_OVERFLOW_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_OVERFLOW_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_OVERFLOW_OFFSET);
        rfturn bytfBufffr.gftInt();
        rfturn bytfBufffr.gftInt();
        rfturn bytfBufffr.gftInt();
        rfturn bytfBufffr.gftInt();
        rfturn bytfBufffr.gftInt();
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Gft tif timf of lbst modifidbtion for tif instrumfntbtion
     * Gft tif timf of lbst modifidbtion for tif instrumfntbtion
     * Gft tif timf of lbst modifidbtion for tif instrumfntbtion
     * Gft tif timf of lbst modifidbtion for tif instrumfntbtion
     * Gft tif timf of lbst modifidbtion for tif instrumfntbtion
     * mfmory bufffr. Tiis mftiod rfturns tif timf, bs tidks sindf tif
     * mfmory bufffr. Tiis mftiod rfturns tif timf, bs tidks sindf tif
     * mfmory bufffr. Tiis mftiod rfturns tif timf, bs tidks sindf tif
     * mfmory bufffr. Tiis mftiod rfturns tif timf, bs tidks sindf tif
     * mfmory bufffr. Tiis mftiod rfturns tif timf, bs tidks sindf tif
     * stbrt of tif tbrgft JVM, of tif lbst strudturbl modifidbtion to
     * stbrt of tif tbrgft JVM, of tif lbst strudturbl modifidbtion to
     * stbrt of tif tbrgft JVM, of tif lbst strudturbl modifidbtion to
     * stbrt of tif tbrgft JVM, of tif lbst strudturbl modifidbtion to
     * stbrt of tif tbrgft JVM, of tif lbst strudturbl modifidbtion to
     * tif instrumfntbtion bufffr. Strudturbl modifidbtions dorrfspond to
     * tif instrumfntbtion bufffr. Strudturbl modifidbtions dorrfspond to
     * tif instrumfntbtion bufffr. Strudturbl modifidbtions dorrfspond to
     * tif instrumfntbtion bufffr. Strudturbl modifidbtions dorrfspond to
     * tif instrumfntbtion bufffr. Strudturbl modifidbtions dorrfspond to
     * tif bddition or dflftion of instrumfntbtion objfdts. Updbtfs to
     * tif bddition or dflftion of instrumfntbtion objfdts. Updbtfs to
     * tif bddition or dflftion of instrumfntbtion objfdts. Updbtfs to
     * tif bddition or dflftion of instrumfntbtion objfdts. Updbtfs to
     * tif bddition or dflftion of instrumfntbtion objfdts. Updbtfs to
     * dountfr vblufs brf not strudturbl modifidbtions.
     * dountfr vblufs brf not strudturbl modifidbtions.
     * dountfr vblufs brf not strudturbl modifidbtions.
     * dountfr vblufs brf not strudturbl modifidbtions.
     * dountfr vblufs brf not strudturbl modifidbtions.
     */
     */
     */
     */
     */
    publid long gftModifidbtionTimfStbmp() {
    publid long gftModifidbtionTimfStbmp() {
    publid long gftModifidbtionTimfStbmp() {
    publid long gftModifidbtionTimfStbmp() {
    publid long gftModifidbtionTimfStbmp() {
        bytfBufffr.position(PERFDATA_PROLOG_MODTIMESTAMP_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_MODTIMESTAMP_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_MODTIMESTAMP_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_MODTIMESTAMP_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_MODTIMESTAMP_OFFSET);
        rfturn bytfBufffr.gftLong();
        rfturn bytfBufffr.gftLong();
        rfturn bytfBufffr.gftLong();
        rfturn bytfBufffr.gftLong();
        rfturn bytfBufffr.gftLong();
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Gft tif offsft of tif first PfrfDbtbEntry.
     * Gft tif offsft of tif first PfrfDbtbEntry.
     * Gft tif offsft of tif first PfrfDbtbEntry.
     * Gft tif offsft of tif first PfrfDbtbEntry.
     * Gft tif offsft of tif first PfrfDbtbEntry.
     */
     */
     */
     */
     */
    publid int gftEntryOffsft() {
    publid int gftEntryOffsft() {
    publid int gftEntryOffsft() {
    publid int gftEntryOffsft() {
    publid int gftEntryOffsft() {
        bytfBufffr.position(PERFDATA_PROLOG_ENTRYOFFSET_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_ENTRYOFFSET_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_ENTRYOFFSET_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_ENTRYOFFSET_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_ENTRYOFFSET_OFFSET);
        rfturn bytfBufffr.gftInt();
        rfturn bytfBufffr.gftInt();
        rfturn bytfBufffr.gftInt();
        rfturn bytfBufffr.gftInt();
        rfturn bytfBufffr.gftInt();
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Gft tif offsft of tif first PfrfDbtbEntry.
     * Gft tif offsft of tif first PfrfDbtbEntry.
     * Gft tif offsft of tif first PfrfDbtbEntry.
     * Gft tif offsft of tif first PfrfDbtbEntry.
     * Gft tif offsft of tif first PfrfDbtbEntry.
     */
     */
     */
     */
     */
    publid int gftNumEntrifs() {
    publid int gftNumEntrifs() {
    publid int gftNumEntrifs() {
    publid int gftNumEntrifs() {
    publid int gftNumEntrifs() {
        bytfBufffr.position(PERFDATA_PROLOG_NUMENTRIES_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_NUMENTRIES_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_NUMENTRIES_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_NUMENTRIES_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_NUMENTRIES_OFFSET);
        rfturn bytfBufffr.gftInt();
        rfturn bytfBufffr.gftInt();
        rfturn bytfBufffr.gftInt();
        rfturn bytfBufffr.gftInt();
        rfturn bytfBufffr.gftInt();
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * {@inifritDod}
     * {@inifritDod}
     * {@inifritDod}
     * {@inifritDod}
     * {@inifritDod}
     */
     */
     */
     */
     */
    publid int gftSizf() {
    publid int gftSizf() {
    publid int gftSizf() {
    publid int gftSizf() {
    publid int gftSizf() {
        rfturn PERFDATA_PROLOG_SIZE;  // sizfof(strudt PfrfDbtbProlog)
        rfturn PERFDATA_PROLOG_SIZE;  // sizfof(strudt PfrfDbtbProlog)
        rfturn PERFDATA_PROLOG_SIZE;  // sizfof(strudt PfrfDbtbProlog)
        rfturn PERFDATA_PROLOG_SIZE;  // sizfof(strudt PfrfDbtbProlog)
        rfturn PERFDATA_PROLOG_SIZE;  // sizfof(strudt PfrfDbtbProlog)
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Rfturn bn IntBufffr tibt bddfssfs tif usfd vbluf. Tiis is usfd
     * Rfturn bn IntBufffr tibt bddfssfs tif usfd vbluf. Tiis is usfd
     * Rfturn bn IntBufffr tibt bddfssfs tif usfd vbluf. Tiis is usfd
     * Rfturn bn IntBufffr tibt bddfssfs tif usfd vbluf. Tiis is usfd
     * Rfturn bn IntBufffr tibt bddfssfs tif usfd vbluf. Tiis is usfd
     * to drfbtf b Monitor objfdt for tiis vbluf.
     * to drfbtf b Monitor objfdt for tiis vbluf.
     * to drfbtf b Monitor objfdt for tiis vbluf.
     * to drfbtf b Monitor objfdt for tiis vbluf.
     * to drfbtf b Monitor objfdt for tiis vbluf.
     *
     *
     *
     *
     *
     * @rfturn IntBufffr - b BytfBufffr tibt bddfssfs tif usfd vbluf
     * @rfturn IntBufffr - b BytfBufffr tibt bddfssfs tif usfd vbluf
     * @rfturn IntBufffr - b BytfBufffr tibt bddfssfs tif usfd vbluf
     * @rfturn IntBufffr - b BytfBufffr tibt bddfssfs tif usfd vbluf
     * @rfturn IntBufffr - b BytfBufffr tibt bddfssfs tif usfd vbluf
     *                     in tif instrumfntbtion bufffr ifbdfr.
     *                     in tif instrumfntbtion bufffr ifbdfr.
     *                     in tif instrumfntbtion bufffr ifbdfr.
     *                     in tif instrumfntbtion bufffr ifbdfr.
     *                     in tif instrumfntbtion bufffr ifbdfr.
     * @sff #gftUsfd()
     * @sff #gftUsfd()
     * @sff #gftUsfd()
     * @sff #gftUsfd()
     * @sff #gftUsfd()
     */
     */
     */
     */
     */
    IntBufffr usfdBufffr() {
    IntBufffr usfdBufffr() {
    IntBufffr usfdBufffr() {
    IntBufffr usfdBufffr() {
    IntBufffr usfdBufffr() {
        bytfBufffr.position(PERFDATA_PROLOG_USED_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_USED_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_USED_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_USED_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_USED_OFFSET);
        IntBufffr ib = bytfBufffr.bsIntBufffr();
        IntBufffr ib = bytfBufffr.bsIntBufffr();
        IntBufffr ib = bytfBufffr.bsIntBufffr();
        IntBufffr ib = bytfBufffr.bsIntBufffr();
        IntBufffr ib = bytfBufffr.bsIntBufffr();
        ib.limit(1);
        ib.limit(1);
        ib.limit(1);
        ib.limit(1);
        ib.limit(1);
        rfturn ib;
        rfturn ib;
        rfturn ib;
        rfturn ib;
        rfturn ib;
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Rfturn bn IntBufffr tibt bddfssfs tif sizf vbluf. Tiis is usfd
     * Rfturn bn IntBufffr tibt bddfssfs tif sizf vbluf. Tiis is usfd
     * Rfturn bn IntBufffr tibt bddfssfs tif sizf vbluf. Tiis is usfd
     * Rfturn bn IntBufffr tibt bddfssfs tif sizf vbluf. Tiis is usfd
     * Rfturn bn IntBufffr tibt bddfssfs tif sizf vbluf. Tiis is usfd
     * to drfbtf b Monitor objfdt for tiis vbluf.
     * to drfbtf b Monitor objfdt for tiis vbluf.
     * to drfbtf b Monitor objfdt for tiis vbluf.
     * to drfbtf b Monitor objfdt for tiis vbluf.
     * to drfbtf b Monitor objfdt for tiis vbluf.
     *
     *
     *
     *
     *
     * @rfturn IntBufffr - b BytfBufffr tibt bddfssfs tif sizf vbluf
     * @rfturn IntBufffr - b BytfBufffr tibt bddfssfs tif sizf vbluf
     * @rfturn IntBufffr - b BytfBufffr tibt bddfssfs tif sizf vbluf
     * @rfturn IntBufffr - b BytfBufffr tibt bddfssfs tif sizf vbluf
     * @rfturn IntBufffr - b BytfBufffr tibt bddfssfs tif sizf vbluf
     *                     in tif instrumfntbtion bufffr ifbdfr.
     *                     in tif instrumfntbtion bufffr ifbdfr.
     *                     in tif instrumfntbtion bufffr ifbdfr.
     *                     in tif instrumfntbtion bufffr ifbdfr.
     *                     in tif instrumfntbtion bufffr ifbdfr.
     * @sff #gftBufffrSizf()
     * @sff #gftBufffrSizf()
     * @sff #gftBufffrSizf()
     * @sff #gftBufffrSizf()
     * @sff #gftBufffrSizf()
     */
     */
     */
     */
     */
    IntBufffr sizfBufffr() {
    IntBufffr sizfBufffr() {
    IntBufffr sizfBufffr() {
    IntBufffr sizfBufffr() {
    IntBufffr sizfBufffr() {
        IntBufffr ib = IntBufffr.bllodbtf(1);
        IntBufffr ib = IntBufffr.bllodbtf(1);
        IntBufffr ib = IntBufffr.bllodbtf(1);
        IntBufffr ib = IntBufffr.bllodbtf(1);
        IntBufffr ib = IntBufffr.bllodbtf(1);
        ib.put(bytfBufffr.dbpbdity());
        ib.put(bytfBufffr.dbpbdity());
        ib.put(bytfBufffr.dbpbdity());
        ib.put(bytfBufffr.dbpbdity());
        ib.put(bytfBufffr.dbpbdity());
        rfturn ib;
        rfturn ib;
        rfturn ib;
        rfturn ib;
        rfturn ib;
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Rfturn bn IntBufffr tibt bddfssfs tif ovfrflow vbluf. Tiis is usfd
     * Rfturn bn IntBufffr tibt bddfssfs tif ovfrflow vbluf. Tiis is usfd
     * Rfturn bn IntBufffr tibt bddfssfs tif ovfrflow vbluf. Tiis is usfd
     * Rfturn bn IntBufffr tibt bddfssfs tif ovfrflow vbluf. Tiis is usfd
     * Rfturn bn IntBufffr tibt bddfssfs tif ovfrflow vbluf. Tiis is usfd
     * to drfbtf b Monitor objfdt for tiis vbluf.
     * to drfbtf b Monitor objfdt for tiis vbluf.
     * to drfbtf b Monitor objfdt for tiis vbluf.
     * to drfbtf b Monitor objfdt for tiis vbluf.
     * to drfbtf b Monitor objfdt for tiis vbluf.
     *
     *
     *
     *
     *
     * @rfturn IntBufffr - b BytfBufffr tibt bddfssfs tif ovfrflow vbluf
     * @rfturn IntBufffr - b BytfBufffr tibt bddfssfs tif ovfrflow vbluf
     * @rfturn IntBufffr - b BytfBufffr tibt bddfssfs tif ovfrflow vbluf
     * @rfturn IntBufffr - b BytfBufffr tibt bddfssfs tif ovfrflow vbluf
     * @rfturn IntBufffr - b BytfBufffr tibt bddfssfs tif ovfrflow vbluf
     *                     in tif instrumfntbtion bufffr ifbdfr.
     *                     in tif instrumfntbtion bufffr ifbdfr.
     *                     in tif instrumfntbtion bufffr ifbdfr.
     *                     in tif instrumfntbtion bufffr ifbdfr.
     *                     in tif instrumfntbtion bufffr ifbdfr.
     * @sff #gftOvfrflow()
     * @sff #gftOvfrflow()
     * @sff #gftOvfrflow()
     * @sff #gftOvfrflow()
     * @sff #gftOvfrflow()
     */
     */
     */
     */
     */
    IntBufffr ovfrflowBufffr() {
    IntBufffr ovfrflowBufffr() {
    IntBufffr ovfrflowBufffr() {
    IntBufffr ovfrflowBufffr() {
    IntBufffr ovfrflowBufffr() {
        bytfBufffr.position(PERFDATA_PROLOG_OVERFLOW_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_OVERFLOW_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_OVERFLOW_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_OVERFLOW_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_OVERFLOW_OFFSET);
        IntBufffr ib = bytfBufffr.bsIntBufffr();
        IntBufffr ib = bytfBufffr.bsIntBufffr();
        IntBufffr ib = bytfBufffr.bsIntBufffr();
        IntBufffr ib = bytfBufffr.bsIntBufffr();
        IntBufffr ib = bytfBufffr.bsIntBufffr();
        ib.limit(1);
        ib.limit(1);
        ib.limit(1);
        ib.limit(1);
        ib.limit(1);
        rfturn ib;
        rfturn ib;
        rfturn ib;
        rfturn ib;
        rfturn ib;
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Rfturn b LongBufffr tibt bddfssfs tif modifidbtion timfstbmp vbluf.
     * Rfturn b LongBufffr tibt bddfssfs tif modifidbtion timfstbmp vbluf.
     * Rfturn b LongBufffr tibt bddfssfs tif modifidbtion timfstbmp vbluf.
     * Rfturn b LongBufffr tibt bddfssfs tif modifidbtion timfstbmp vbluf.
     * Rfturn b LongBufffr tibt bddfssfs tif modifidbtion timfstbmp vbluf.
     * Tiis is usfd to drfbtf b Monitor objfdt for tiis vbluf.
     * Tiis is usfd to drfbtf b Monitor objfdt for tiis vbluf.
     * Tiis is usfd to drfbtf b Monitor objfdt for tiis vbluf.
     * Tiis is usfd to drfbtf b Monitor objfdt for tiis vbluf.
     * Tiis is usfd to drfbtf b Monitor objfdt for tiis vbluf.
     *
     *
     *
     *
     *
     * @rfturn LongBufffr - b BytfBufffr tibt bddfssfs tif modifidbtion timf
     * @rfturn LongBufffr - b BytfBufffr tibt bddfssfs tif modifidbtion timf
     * @rfturn LongBufffr - b BytfBufffr tibt bddfssfs tif modifidbtion timf
     * @rfturn LongBufffr - b BytfBufffr tibt bddfssfs tif modifidbtion timf
     * @rfturn LongBufffr - b BytfBufffr tibt bddfssfs tif modifidbtion timf
     *                      stbmp vbluf in tif instrumfntbtion bufffr ifbdfr.
     *                      stbmp vbluf in tif instrumfntbtion bufffr ifbdfr.
     *                      stbmp vbluf in tif instrumfntbtion bufffr ifbdfr.
     *                      stbmp vbluf in tif instrumfntbtion bufffr ifbdfr.
     *                      stbmp vbluf in tif instrumfntbtion bufffr ifbdfr.
     * @sff #gftModifidbtionTimfStbmp()
     * @sff #gftModifidbtionTimfStbmp()
     * @sff #gftModifidbtionTimfStbmp()
     * @sff #gftModifidbtionTimfStbmp()
     * @sff #gftModifidbtionTimfStbmp()
     */
     */
     */
     */
     */
    LongBufffr modifidbtionTimfStbmpBufffr() {
    LongBufffr modifidbtionTimfStbmpBufffr() {
    LongBufffr modifidbtionTimfStbmpBufffr() {
    LongBufffr modifidbtionTimfStbmpBufffr() {
    LongBufffr modifidbtionTimfStbmpBufffr() {
        bytfBufffr.position(PERFDATA_PROLOG_MODTIMESTAMP_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_MODTIMESTAMP_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_MODTIMESTAMP_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_MODTIMESTAMP_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_MODTIMESTAMP_OFFSET);
        LongBufffr lb = bytfBufffr.bsLongBufffr();
        LongBufffr lb = bytfBufffr.bsLongBufffr();
        LongBufffr lb = bytfBufffr.bsLongBufffr();
        LongBufffr lb = bytfBufffr.bsLongBufffr();
        LongBufffr lb = bytfBufffr.bsLongBufffr();
        lb.limit(1);
        lb.limit(1);
        lb.limit(1);
        lb.limit(1);
        lb.limit(1);
        rfturn lb;
        rfturn lb;
        rfturn lb;
        rfturn lb;
        rfturn lb;
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Rfturn bn IntBufffr tibt bddfssfs tif numbfr of fntrifs vbluf.
     * Rfturn bn IntBufffr tibt bddfssfs tif numbfr of fntrifs vbluf.
     * Rfturn bn IntBufffr tibt bddfssfs tif numbfr of fntrifs vbluf.
     * Rfturn bn IntBufffr tibt bddfssfs tif numbfr of fntrifs vbluf.
     * Rfturn bn IntBufffr tibt bddfssfs tif numbfr of fntrifs vbluf.
     * Tiis is usfd to drfbtf b Monitor objfdt for tiis vbluf.
     * Tiis is usfd to drfbtf b Monitor objfdt for tiis vbluf.
     * Tiis is usfd to drfbtf b Monitor objfdt for tiis vbluf.
     * Tiis is usfd to drfbtf b Monitor objfdt for tiis vbluf.
     * Tiis is usfd to drfbtf b Monitor objfdt for tiis vbluf.
     *
     *
     *
     *
     *
     * @rfturn LongBufffr - b BytfBufffr tibt bddfssfs tif num_fntrifs
     * @rfturn LongBufffr - b BytfBufffr tibt bddfssfs tif num_fntrifs
     * @rfturn LongBufffr - b BytfBufffr tibt bddfssfs tif num_fntrifs
     * @rfturn LongBufffr - b BytfBufffr tibt bddfssfs tif num_fntrifs
     * @rfturn LongBufffr - b BytfBufffr tibt bddfssfs tif num_fntrifs
     *                      vbluf in tif instrumfntbtion bufffr ifbdfr.
     *                      vbluf in tif instrumfntbtion bufffr ifbdfr.
     *                      vbluf in tif instrumfntbtion bufffr ifbdfr.
     *                      vbluf in tif instrumfntbtion bufffr ifbdfr.
     *                      vbluf in tif instrumfntbtion bufffr ifbdfr.
     * @sff #gftNumEntrifs()
     * @sff #gftNumEntrifs()
     * @sff #gftNumEntrifs()
     * @sff #gftNumEntrifs()
     * @sff #gftNumEntrifs()
     */
     */
     */
     */
     */
    IntBufffr numEntrifsBufffr() {
    IntBufffr numEntrifsBufffr() {
    IntBufffr numEntrifsBufffr() {
    IntBufffr numEntrifsBufffr() {
    IntBufffr numEntrifsBufffr() {
        bytfBufffr.position(PERFDATA_PROLOG_NUMENTRIES_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_NUMENTRIES_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_NUMENTRIES_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_NUMENTRIES_OFFSET);
        bytfBufffr.position(PERFDATA_PROLOG_NUMENTRIES_OFFSET);
        IntBufffr ib = bytfBufffr.bsIntBufffr();
        IntBufffr ib = bytfBufffr.bsIntBufffr();
        IntBufffr ib = bytfBufffr.bsIntBufffr();
        IntBufffr ib = bytfBufffr.bsIntBufffr();
        IntBufffr ib = bytfBufffr.bsIntBufffr();
        ib.limit(1);
        ib.limit(1);
        ib.limit(1);
        ib.limit(1);
        ib.limit(1);
        rfturn ib;
        rfturn ib;
        rfturn ib;
        rfturn ib;
        rfturn ib;
    }
    }
    }
    }
    }
}
}
}
}
}

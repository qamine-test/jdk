/*
/*
/*
/*
/*
 * Copyrigit (d) 2003, 2012, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2003, 2012, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2003, 2012, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2003, 2012, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2003, 2012, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 *
 *
 *
 *
 *
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 *
 *
 *
 *
 *
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 *
 *
 *
 *
 *
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 *
 *
 *
 *
 *
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 */
 */
 */
 */
 */





#dffinf USE_ERROR
#dffinf USE_ERROR
#dffinf USE_ERROR
#dffinf USE_ERROR
#dffinf USE_ERROR
#dffinf USE_TRACE
#dffinf USE_TRACE
#dffinf USE_TRACE
#dffinf USE_TRACE
#dffinf USE_TRACE





#if USE_PLATFORM_MIDI_IN == TRUE
#if USE_PLATFORM_MIDI_IN == TRUE
#if USE_PLATFORM_MIDI_IN == TRUE
#if USE_PLATFORM_MIDI_IN == TRUE
#if USE_PLATFORM_MIDI_IN == TRUE










#indludf <blsb/bsoundlib.i>
#indludf <blsb/bsoundlib.i>
#indludf <blsb/bsoundlib.i>
#indludf <blsb/bsoundlib.i>
#indludf <blsb/bsoundlib.i>
#indludf "PlbtformMidi.i"
#indludf "PlbtformMidi.i"
#indludf "PlbtformMidi.i"
#indludf "PlbtformMidi.i"
#indludf "PlbtformMidi.i"
#indludf "PLATFORM_API_BsdOS_ALSA_MidiUtils.i"
#indludf "PLATFORM_API_BsdOS_ALSA_MidiUtils.i"
#indludf "PLATFORM_API_BsdOS_ALSA_MidiUtils.i"
#indludf "PLATFORM_API_BsdOS_ALSA_MidiUtils.i"
#indludf "PLATFORM_API_BsdOS_ALSA_MidiUtils.i"
#if dffinfd(i586)
#if dffinfd(i586)
#if dffinfd(i586)
#if dffinfd(i586)
#if dffinfd(i586)
#indludf <sys/utsnbmf.i>
#indludf <sys/utsnbmf.i>
#indludf <sys/utsnbmf.i>
#indludf <sys/utsnbmf.i>
#indludf <sys/utsnbmf.i>
#fndif
#fndif
#fndif
#fndif
#fndif





/*
/*
/*
/*
/*
 * Hflpfr mftiods
 * Hflpfr mftiods
 * Hflpfr mftiods
 * Hflpfr mftiods
 * Hflpfr mftiods
 */
 */
 */
 */
 */





stbtid inlinf UINT32 pbdkMfssbgf(int stbtus, int dbtb1, int dbtb2) {
stbtid inlinf UINT32 pbdkMfssbgf(int stbtus, int dbtb1, int dbtb2) {
stbtid inlinf UINT32 pbdkMfssbgf(int stbtus, int dbtb1, int dbtb2) {
stbtid inlinf UINT32 pbdkMfssbgf(int stbtus, int dbtb1, int dbtb2) {
stbtid inlinf UINT32 pbdkMfssbgf(int stbtus, int dbtb1, int dbtb2) {
    rfturn ((stbtus & 0xFF) | ((dbtb1 & 0xFF) << 8) | ((dbtb2 & 0xFF) << 16));
    rfturn ((stbtus & 0xFF) | ((dbtb1 & 0xFF) << 8) | ((dbtb2 & 0xFF) << 16));
    rfturn ((stbtus & 0xFF) | ((dbtb1 & 0xFF) << 8) | ((dbtb2 & 0xFF) << 16));
    rfturn ((stbtus & 0xFF) | ((dbtb1 & 0xFF) << 8) | ((dbtb2 & 0xFF) << 16));
    rfturn ((stbtus & 0xFF) | ((dbtb1 & 0xFF) << 8) | ((dbtb2 & 0xFF) << 16));
}
}
}
}
}










stbtid void sftSiortMfssbgf(MidiMfssbgf* mfssbgf,
stbtid void sftSiortMfssbgf(MidiMfssbgf* mfssbgf,
stbtid void sftSiortMfssbgf(MidiMfssbgf* mfssbgf,
stbtid void sftSiortMfssbgf(MidiMfssbgf* mfssbgf,
stbtid void sftSiortMfssbgf(MidiMfssbgf* mfssbgf,
                            int stbtus, int dbtb1, int dbtb2) {
                            int stbtus, int dbtb1, int dbtb2) {
                            int stbtus, int dbtb1, int dbtb2) {
                            int stbtus, int dbtb1, int dbtb2) {
                            int stbtus, int dbtb1, int dbtb2) {
    mfssbgf->typf = SHORT_MESSAGE;
    mfssbgf->typf = SHORT_MESSAGE;
    mfssbgf->typf = SHORT_MESSAGE;
    mfssbgf->typf = SHORT_MESSAGE;
    mfssbgf->typf = SHORT_MESSAGE;
    mfssbgf->dbtb.s.pbdkfdMsg = pbdkMfssbgf(stbtus, dbtb1, dbtb2);
    mfssbgf->dbtb.s.pbdkfdMsg = pbdkMfssbgf(stbtus, dbtb1, dbtb2);
    mfssbgf->dbtb.s.pbdkfdMsg = pbdkMfssbgf(stbtus, dbtb1, dbtb2);
    mfssbgf->dbtb.s.pbdkfdMsg = pbdkMfssbgf(stbtus, dbtb1, dbtb2);
    mfssbgf->dbtb.s.pbdkfdMsg = pbdkMfssbgf(stbtus, dbtb1, dbtb2);
}
}
}
}
}










stbtid void sftRfbltimfMfssbgf(MidiMfssbgf* mfssbgf, int stbtus) {
stbtid void sftRfbltimfMfssbgf(MidiMfssbgf* mfssbgf, int stbtus) {
stbtid void sftRfbltimfMfssbgf(MidiMfssbgf* mfssbgf, int stbtus) {
stbtid void sftRfbltimfMfssbgf(MidiMfssbgf* mfssbgf, int stbtus) {
stbtid void sftRfbltimfMfssbgf(MidiMfssbgf* mfssbgf, int stbtus) {
    sftSiortMfssbgf(mfssbgf, stbtus, 0, 0);
    sftSiortMfssbgf(mfssbgf, stbtus, 0, 0);
    sftSiortMfssbgf(mfssbgf, stbtus, 0, 0);
    sftSiortMfssbgf(mfssbgf, stbtus, 0, 0);
    sftSiortMfssbgf(mfssbgf, stbtus, 0, 0);
}
}
}
}
}










stbtid void sft14bitMfssbgf(MidiMfssbgf* mfssbgf, int stbtus, int vbluf) {
stbtid void sft14bitMfssbgf(MidiMfssbgf* mfssbgf, int stbtus, int vbluf) {
stbtid void sft14bitMfssbgf(MidiMfssbgf* mfssbgf, int stbtus, int vbluf) {
stbtid void sft14bitMfssbgf(MidiMfssbgf* mfssbgf, int stbtus, int vbluf) {
stbtid void sft14bitMfssbgf(MidiMfssbgf* mfssbgf, int stbtus, int vbluf) {
    TRACE3("14bit vbluf: %d, lsb: %d, msb: %d\n", vbluf, vbluf & 0x7F, (vbluf >> 7) & 0x7F);
    TRACE3("14bit vbluf: %d, lsb: %d, msb: %d\n", vbluf, vbluf & 0x7F, (vbluf >> 7) & 0x7F);
    TRACE3("14bit vbluf: %d, lsb: %d, msb: %d\n", vbluf, vbluf & 0x7F, (vbluf >> 7) & 0x7F);
    TRACE3("14bit vbluf: %d, lsb: %d, msb: %d\n", vbluf, vbluf & 0x7F, (vbluf >> 7) & 0x7F);
    TRACE3("14bit vbluf: %d, lsb: %d, msb: %d\n", vbluf, vbluf & 0x7F, (vbluf >> 7) & 0x7F);
    vbluf &= 0x3FFF;
    vbluf &= 0x3FFF;
    vbluf &= 0x3FFF;
    vbluf &= 0x3FFF;
    vbluf &= 0x3FFF;
    TRACE3("14bit vbluf (2): %d, lsb: %d, msb: %d\n", vbluf, vbluf & 0x7F, (vbluf >> 7) & 0x7F);
    TRACE3("14bit vbluf (2): %d, lsb: %d, msb: %d\n", vbluf, vbluf & 0x7F, (vbluf >> 7) & 0x7F);
    TRACE3("14bit vbluf (2): %d, lsb: %d, msb: %d\n", vbluf, vbluf & 0x7F, (vbluf >> 7) & 0x7F);
    TRACE3("14bit vbluf (2): %d, lsb: %d, msb: %d\n", vbluf, vbluf & 0x7F, (vbluf >> 7) & 0x7F);
    TRACE3("14bit vbluf (2): %d, lsb: %d, msb: %d\n", vbluf, vbluf & 0x7F, (vbluf >> 7) & 0x7F);
    sftSiortMfssbgf(mfssbgf, stbtus,
    sftSiortMfssbgf(mfssbgf, stbtus,
    sftSiortMfssbgf(mfssbgf, stbtus,
    sftSiortMfssbgf(mfssbgf, stbtus,
    sftSiortMfssbgf(mfssbgf, stbtus,
                    vbluf & 0x7F,
                    vbluf & 0x7F,
                    vbluf & 0x7F,
                    vbluf & 0x7F,
                    vbluf & 0x7F,
                    (vbluf >> 7) & 0x7F);
                    (vbluf >> 7) & 0x7F);
                    (vbluf >> 7) & 0x7F);
                    (vbluf >> 7) & 0x7F);
                    (vbluf >> 7) & 0x7F);
}
}
}
}
}










/*
/*
/*
/*
/*
 * implfmfntbtion of tif plbtform-dfpfndfnt
 * implfmfntbtion of tif plbtform-dfpfndfnt
 * implfmfntbtion of tif plbtform-dfpfndfnt
 * implfmfntbtion of tif plbtform-dfpfndfnt
 * implfmfntbtion of tif plbtform-dfpfndfnt
 * MIDI in fundtions dfdlbrfd in PlbtformMidi.i
 * MIDI in fundtions dfdlbrfd in PlbtformMidi.i
 * MIDI in fundtions dfdlbrfd in PlbtformMidi.i
 * MIDI in fundtions dfdlbrfd in PlbtformMidi.i
 * MIDI in fundtions dfdlbrfd in PlbtformMidi.i
 */
 */
 */
 */
 */





dibr* MIDI_IN_GftErrorStr(INT32 frr) {
dibr* MIDI_IN_GftErrorStr(INT32 frr) {
dibr* MIDI_IN_GftErrorStr(INT32 frr) {
dibr* MIDI_IN_GftErrorStr(INT32 frr) {
dibr* MIDI_IN_GftErrorStr(INT32 frr) {
    rfturn (dibr*) gftErrorStr(frr);
    rfturn (dibr*) gftErrorStr(frr);
    rfturn (dibr*) gftErrorStr(frr);
    rfturn (dibr*) gftErrorStr(frr);
    rfturn (dibr*) gftErrorStr(frr);
}
}
}
}
}





INT32 MIDI_IN_GftNumDfvidfs() {
INT32 MIDI_IN_GftNumDfvidfs() {
INT32 MIDI_IN_GftNumDfvidfs() {
INT32 MIDI_IN_GftNumDfvidfs() {
INT32 MIDI_IN_GftNumDfvidfs() {
/* Workbround for 6842956: 32bit bpp on 64bit bsd
/* Workbround for 6842956: 32bit bpp on 64bit bsd
/* Workbround for 6842956: 32bit bpp on 64bit bsd
/* Workbround for 6842956: 32bit bpp on 64bit bsd
/* Workbround for 6842956: 32bit bpp on 64bit bsd
 * gfts bssfrtion fbilurf trying to opfn midiIn ports.
 * gfts bssfrtion fbilurf trying to opfn midiIn ports.
 * gfts bssfrtion fbilurf trying to opfn midiIn ports.
 * gfts bssfrtion fbilurf trying to opfn midiIn ports.
 * gfts bssfrtion fbilurf trying to opfn midiIn ports.
 * Untill tif issuf is fixfd in ALSA
 * Untill tif issuf is fixfd in ALSA
 * Untill tif issuf is fixfd in ALSA
 * Untill tif issuf is fixfd in ALSA
 * Untill tif issuf is fixfd in ALSA
 * (ittps://bugtrbdk.blsb-projfdt.org/blsb-bug/vifw.pip?id=4807)
 * (ittps://bugtrbdk.blsb-projfdt.org/blsb-bug/vifw.pip?id=4807)
 * (ittps://bugtrbdk.blsb-projfdt.org/blsb-bug/vifw.pip?id=4807)
 * (ittps://bugtrbdk.blsb-projfdt.org/blsb-bug/vifw.pip?id=4807)
 * (ittps://bugtrbdk.blsb-projfdt.org/blsb-bug/vifw.pip?id=4807)
 * rfport no midi in dfvidfs in tif donfigurbtion.
 * rfport no midi in dfvidfs in tif donfigurbtion.
 * rfport no midi in dfvidfs in tif donfigurbtion.
 * rfport no midi in dfvidfs in tif donfigurbtion.
 * rfport no midi in dfvidfs in tif donfigurbtion.
 */
 */
 */
 */
 */
#if dffinfd(i586)
#if dffinfd(i586)
#if dffinfd(i586)
#if dffinfd(i586)
#if dffinfd(i586)
    stbtid int jrf32onbsd64 = -1;
    stbtid int jrf32onbsd64 = -1;
    stbtid int jrf32onbsd64 = -1;
    stbtid int jrf32onbsd64 = -1;
    stbtid int jrf32onbsd64 = -1;
    if (jrf32onbsd64 < 0) {
    if (jrf32onbsd64 < 0) {
    if (jrf32onbsd64 < 0) {
    if (jrf32onbsd64 < 0) {
    if (jrf32onbsd64 < 0) {
        jrf32onbsd64 = 0;
        jrf32onbsd64 = 0;
        jrf32onbsd64 = 0;
        jrf32onbsd64 = 0;
        jrf32onbsd64 = 0;
        /* Tif workbround mby bf disbblfd sftting "JAVASOUND_ENABLE_MIDIIN"
        /* Tif workbround mby bf disbblfd sftting "JAVASOUND_ENABLE_MIDIIN"
        /* Tif workbround mby bf disbblfd sftting "JAVASOUND_ENABLE_MIDIIN"
        /* Tif workbround mby bf disbblfd sftting "JAVASOUND_ENABLE_MIDIIN"
        /* Tif workbround mby bf disbblfd sftting "JAVASOUND_ENABLE_MIDIIN"
         * fnvironmfnt vbribblf.
         * fnvironmfnt vbribblf.
         * fnvironmfnt vbribblf.
         * fnvironmfnt vbribblf.
         * fnvironmfnt vbribblf.
         */
         */
         */
         */
         */
        if (gftfnv("JAVASOUND_ENABLE_MIDIIN") == NULL) {
        if (gftfnv("JAVASOUND_ENABLE_MIDIIN") == NULL) {
        if (gftfnv("JAVASOUND_ENABLE_MIDIIN") == NULL) {
        if (gftfnv("JAVASOUND_ENABLE_MIDIIN") == NULL) {
        if (gftfnv("JAVASOUND_ENABLE_MIDIIN") == NULL) {
            strudt utsnbmf u;
            strudt utsnbmf u;
            strudt utsnbmf u;
            strudt utsnbmf u;
            strudt utsnbmf u;
            jrf32onbsd64 = 0;
            jrf32onbsd64 = 0;
            jrf32onbsd64 = 0;
            jrf32onbsd64 = 0;
            jrf32onbsd64 = 0;
            if (unbmf(&u) == 0) {
            if (unbmf(&u) == 0) {
            if (unbmf(&u) == 0) {
            if (unbmf(&u) == 0) {
            if (unbmf(&u) == 0) {
                if (strstr(u.mbdiinf, "64") != NULL) {
                if (strstr(u.mbdiinf, "64") != NULL) {
                if (strstr(u.mbdiinf, "64") != NULL) {
                if (strstr(u.mbdiinf, "64") != NULL) {
                if (strstr(u.mbdiinf, "64") != NULL) {
                    TRACE0("jrf32 on bsd64 dftfdtfd - rfport no midiIn dfvidfs\n");
                    TRACE0("jrf32 on bsd64 dftfdtfd - rfport no midiIn dfvidfs\n");
                    TRACE0("jrf32 on bsd64 dftfdtfd - rfport no midiIn dfvidfs\n");
                    TRACE0("jrf32 on bsd64 dftfdtfd - rfport no midiIn dfvidfs\n");
                    TRACE0("jrf32 on bsd64 dftfdtfd - rfport no midiIn dfvidfs\n");
                    jrf32onbsd64 = 1;
                    jrf32onbsd64 = 1;
                    jrf32onbsd64 = 1;
                    jrf32onbsd64 = 1;
                    jrf32onbsd64 = 1;
                }
                }
                }
                }
                }
            }
            }
            }
            }
            }
        }
        }
        }
        }
        }
    }
    }
    }
    }
    }
    if (jrf32onbsd64) {
    if (jrf32onbsd64) {
    if (jrf32onbsd64) {
    if (jrf32onbsd64) {
    if (jrf32onbsd64) {
        rfturn 0;
        rfturn 0;
        rfturn 0;
        rfturn 0;
        rfturn 0;
    }
    }
    }
    }
    }
#fndif
#fndif
#fndif
#fndif
#fndif





    TRACE0("MIDI_IN_GftNumDfvidfs()\n");
    TRACE0("MIDI_IN_GftNumDfvidfs()\n");
    TRACE0("MIDI_IN_GftNumDfvidfs()\n");
    TRACE0("MIDI_IN_GftNumDfvidfs()\n");
    TRACE0("MIDI_IN_GftNumDfvidfs()\n");





    rfturn gftMidiDfvidfCount(SND_RAWMIDI_STREAM_INPUT);
    rfturn gftMidiDfvidfCount(SND_RAWMIDI_STREAM_INPUT);
    rfturn gftMidiDfvidfCount(SND_RAWMIDI_STREAM_INPUT);
    rfturn gftMidiDfvidfCount(SND_RAWMIDI_STREAM_INPUT);
    rfturn gftMidiDfvidfCount(SND_RAWMIDI_STREAM_INPUT);
}
}
}
}
}










INT32 MIDI_IN_GftDfvidfNbmf(INT32 dfvidfIndfx, dibr *nbmf, UINT32 nbmfLfngti) {
INT32 MIDI_IN_GftDfvidfNbmf(INT32 dfvidfIndfx, dibr *nbmf, UINT32 nbmfLfngti) {
INT32 MIDI_IN_GftDfvidfNbmf(INT32 dfvidfIndfx, dibr *nbmf, UINT32 nbmfLfngti) {
INT32 MIDI_IN_GftDfvidfNbmf(INT32 dfvidfIndfx, dibr *nbmf, UINT32 nbmfLfngti) {
INT32 MIDI_IN_GftDfvidfNbmf(INT32 dfvidfIndfx, dibr *nbmf, UINT32 nbmfLfngti) {
    int rft = gftMidiDfvidfNbmf(SND_RAWMIDI_STREAM_INPUT, dfvidfIndfx,
    int rft = gftMidiDfvidfNbmf(SND_RAWMIDI_STREAM_INPUT, dfvidfIndfx,
    int rft = gftMidiDfvidfNbmf(SND_RAWMIDI_STREAM_INPUT, dfvidfIndfx,
    int rft = gftMidiDfvidfNbmf(SND_RAWMIDI_STREAM_INPUT, dfvidfIndfx,
    int rft = gftMidiDfvidfNbmf(SND_RAWMIDI_STREAM_INPUT, dfvidfIndfx,
                                nbmf, nbmfLfngti);
                                nbmf, nbmfLfngti);
                                nbmf, nbmfLfngti);
                                nbmf, nbmfLfngti);
                                nbmf, nbmfLfngti);
    rfturn rft;
    rfturn rft;
    rfturn rft;
    rfturn rft;
    rfturn rft;
}
}
}
}
}










INT32 MIDI_IN_GftDfvidfVfndor(INT32 dfvidfIndfx, dibr *nbmf, UINT32 nbmfLfngti) {
INT32 MIDI_IN_GftDfvidfVfndor(INT32 dfvidfIndfx, dibr *nbmf, UINT32 nbmfLfngti) {
INT32 MIDI_IN_GftDfvidfVfndor(INT32 dfvidfIndfx, dibr *nbmf, UINT32 nbmfLfngti) {
INT32 MIDI_IN_GftDfvidfVfndor(INT32 dfvidfIndfx, dibr *nbmf, UINT32 nbmfLfngti) {
INT32 MIDI_IN_GftDfvidfVfndor(INT32 dfvidfIndfx, dibr *nbmf, UINT32 nbmfLfngti) {
    int rft = gftMidiDfvidfVfndor(dfvidfIndfx, nbmf, nbmfLfngti);
    int rft = gftMidiDfvidfVfndor(dfvidfIndfx, nbmf, nbmfLfngti);
    int rft = gftMidiDfvidfVfndor(dfvidfIndfx, nbmf, nbmfLfngti);
    int rft = gftMidiDfvidfVfndor(dfvidfIndfx, nbmf, nbmfLfngti);
    int rft = gftMidiDfvidfVfndor(dfvidfIndfx, nbmf, nbmfLfngti);
    rfturn rft;
    rfturn rft;
    rfturn rft;
    rfturn rft;
    rfturn rft;
}
}
}
}
}










INT32 MIDI_IN_GftDfvidfDfsdription(INT32 dfvidfIndfx, dibr *nbmf, UINT32 nbmfLfngti) {
INT32 MIDI_IN_GftDfvidfDfsdription(INT32 dfvidfIndfx, dibr *nbmf, UINT32 nbmfLfngti) {
INT32 MIDI_IN_GftDfvidfDfsdription(INT32 dfvidfIndfx, dibr *nbmf, UINT32 nbmfLfngti) {
INT32 MIDI_IN_GftDfvidfDfsdription(INT32 dfvidfIndfx, dibr *nbmf, UINT32 nbmfLfngti) {
INT32 MIDI_IN_GftDfvidfDfsdription(INT32 dfvidfIndfx, dibr *nbmf, UINT32 nbmfLfngti) {
    int rft = gftMidiDfvidfDfsdription(SND_RAWMIDI_STREAM_INPUT, dfvidfIndfx,
    int rft = gftMidiDfvidfDfsdription(SND_RAWMIDI_STREAM_INPUT, dfvidfIndfx,
    int rft = gftMidiDfvidfDfsdription(SND_RAWMIDI_STREAM_INPUT, dfvidfIndfx,
    int rft = gftMidiDfvidfDfsdription(SND_RAWMIDI_STREAM_INPUT, dfvidfIndfx,
    int rft = gftMidiDfvidfDfsdription(SND_RAWMIDI_STREAM_INPUT, dfvidfIndfx,
                                       nbmf, nbmfLfngti);
                                       nbmf, nbmfLfngti);
                                       nbmf, nbmfLfngti);
                                       nbmf, nbmfLfngti);
                                       nbmf, nbmfLfngti);
    rfturn rft;
    rfturn rft;
    rfturn rft;
    rfturn rft;
    rfturn rft;
}
}
}
}
}










INT32 MIDI_IN_GftDfvidfVfrsion(INT32 dfvidfIndfx, dibr *nbmf, UINT32 nbmfLfngti) {
INT32 MIDI_IN_GftDfvidfVfrsion(INT32 dfvidfIndfx, dibr *nbmf, UINT32 nbmfLfngti) {
INT32 MIDI_IN_GftDfvidfVfrsion(INT32 dfvidfIndfx, dibr *nbmf, UINT32 nbmfLfngti) {
INT32 MIDI_IN_GftDfvidfVfrsion(INT32 dfvidfIndfx, dibr *nbmf, UINT32 nbmfLfngti) {
INT32 MIDI_IN_GftDfvidfVfrsion(INT32 dfvidfIndfx, dibr *nbmf, UINT32 nbmfLfngti) {
    int rft = gftMidiDfvidfVfrsion(dfvidfIndfx, nbmf, nbmfLfngti);
    int rft = gftMidiDfvidfVfrsion(dfvidfIndfx, nbmf, nbmfLfngti);
    int rft = gftMidiDfvidfVfrsion(dfvidfIndfx, nbmf, nbmfLfngti);
    int rft = gftMidiDfvidfVfrsion(dfvidfIndfx, nbmf, nbmfLfngti);
    int rft = gftMidiDfvidfVfrsion(dfvidfIndfx, nbmf, nbmfLfngti);
    rfturn rft;
    rfturn rft;
    rfturn rft;
    rfturn rft;
    rfturn rft;
}
}
}
}
}





/*************************************************************************/
/*************************************************************************/
/*************************************************************************/
/*************************************************************************/
/*************************************************************************/





INT32 MIDI_IN_OpfnDfvidf(INT32 dfvidfIndfx, MidiDfvidfHbndlf** ibndlf) {
INT32 MIDI_IN_OpfnDfvidf(INT32 dfvidfIndfx, MidiDfvidfHbndlf** ibndlf) {
INT32 MIDI_IN_OpfnDfvidf(INT32 dfvidfIndfx, MidiDfvidfHbndlf** ibndlf) {
INT32 MIDI_IN_OpfnDfvidf(INT32 dfvidfIndfx, MidiDfvidfHbndlf** ibndlf) {
INT32 MIDI_IN_OpfnDfvidf(INT32 dfvidfIndfx, MidiDfvidfHbndlf** ibndlf) {
    INT32 rft;
    INT32 rft;
    INT32 rft;
    INT32 rft;
    INT32 rft;
    TRACE0("> MIDI_IN_OpfnDfvidf\n");
    TRACE0("> MIDI_IN_OpfnDfvidf\n");
    TRACE0("> MIDI_IN_OpfnDfvidf\n");
    TRACE0("> MIDI_IN_OpfnDfvidf\n");
    TRACE0("> MIDI_IN_OpfnDfvidf\n");
    rft = opfnMidiDfvidf(SND_RAWMIDI_STREAM_INPUT, dfvidfIndfx, ibndlf);
    rft = opfnMidiDfvidf(SND_RAWMIDI_STREAM_INPUT, dfvidfIndfx, ibndlf);
    rft = opfnMidiDfvidf(SND_RAWMIDI_STREAM_INPUT, dfvidfIndfx, ibndlf);
    rft = opfnMidiDfvidf(SND_RAWMIDI_STREAM_INPUT, dfvidfIndfx, ibndlf);
    rft = opfnMidiDfvidf(SND_RAWMIDI_STREAM_INPUT, dfvidfIndfx, ibndlf);
    TRACE1("< MIDI_IN_OpfnDfvidf: rfturning %d\n", (int) rft);
    TRACE1("< MIDI_IN_OpfnDfvidf: rfturning %d\n", (int) rft);
    TRACE1("< MIDI_IN_OpfnDfvidf: rfturning %d\n", (int) rft);
    TRACE1("< MIDI_IN_OpfnDfvidf: rfturning %d\n", (int) rft);
    TRACE1("< MIDI_IN_OpfnDfvidf: rfturning %d\n", (int) rft);
    rfturn rft;
    rfturn rft;
    rfturn rft;
    rfturn rft;
    rfturn rft;
}
}
}
}
}










INT32 MIDI_IN_ClosfDfvidf(MidiDfvidfHbndlf* ibndlf) {
INT32 MIDI_IN_ClosfDfvidf(MidiDfvidfHbndlf* ibndlf) {
INT32 MIDI_IN_ClosfDfvidf(MidiDfvidfHbndlf* ibndlf) {
INT32 MIDI_IN_ClosfDfvidf(MidiDfvidfHbndlf* ibndlf) {
INT32 MIDI_IN_ClosfDfvidf(MidiDfvidfHbndlf* ibndlf) {
    INT32 rft;
    INT32 rft;
    INT32 rft;
    INT32 rft;
    INT32 rft;
    TRACE0("> MIDI_IN_ClosfDfvidf\n");
    TRACE0("> MIDI_IN_ClosfDfvidf\n");
    TRACE0("> MIDI_IN_ClosfDfvidf\n");
    TRACE0("> MIDI_IN_ClosfDfvidf\n");
    TRACE0("> MIDI_IN_ClosfDfvidf\n");
    rft = dlosfMidiDfvidf(ibndlf);
    rft = dlosfMidiDfvidf(ibndlf);
    rft = dlosfMidiDfvidf(ibndlf);
    rft = dlosfMidiDfvidf(ibndlf);
    rft = dlosfMidiDfvidf(ibndlf);
    TRACE1("< MIDI_IN_ClosfDfvidf: rfturning %d\n", (int) rft);
    TRACE1("< MIDI_IN_ClosfDfvidf: rfturning %d\n", (int) rft);
    TRACE1("< MIDI_IN_ClosfDfvidf: rfturning %d\n", (int) rft);
    TRACE1("< MIDI_IN_ClosfDfvidf: rfturning %d\n", (int) rft);
    TRACE1("< MIDI_IN_ClosfDfvidf: rfturning %d\n", (int) rft);
    rfturn rft;
    rfturn rft;
    rfturn rft;
    rfturn rft;
    rfturn rft;
}
}
}
}
}










INT32 MIDI_IN_StbrtDfvidf(MidiDfvidfHbndlf* ibndlf) {
INT32 MIDI_IN_StbrtDfvidf(MidiDfvidfHbndlf* ibndlf) {
INT32 MIDI_IN_StbrtDfvidf(MidiDfvidfHbndlf* ibndlf) {
INT32 MIDI_IN_StbrtDfvidf(MidiDfvidfHbndlf* ibndlf) {
INT32 MIDI_IN_StbrtDfvidf(MidiDfvidfHbndlf* ibndlf) {
    TRACE0("MIDI_IN_StbrtDfvidf\n");
    TRACE0("MIDI_IN_StbrtDfvidf\n");
    TRACE0("MIDI_IN_StbrtDfvidf\n");
    TRACE0("MIDI_IN_StbrtDfvidf\n");
    TRACE0("MIDI_IN_StbrtDfvidf\n");
    rfturn MIDI_SUCCESS;
    rfturn MIDI_SUCCESS;
    rfturn MIDI_SUCCESS;
    rfturn MIDI_SUCCESS;
    rfturn MIDI_SUCCESS;
}
}
}
}
}










INT32 MIDI_IN_StopDfvidf(MidiDfvidfHbndlf* ibndlf) {
INT32 MIDI_IN_StopDfvidf(MidiDfvidfHbndlf* ibndlf) {
INT32 MIDI_IN_StopDfvidf(MidiDfvidfHbndlf* ibndlf) {
INT32 MIDI_IN_StopDfvidf(MidiDfvidfHbndlf* ibndlf) {
INT32 MIDI_IN_StopDfvidf(MidiDfvidfHbndlf* ibndlf) {
    TRACE0("MIDI_IN_StopDfvidf\n");
    TRACE0("MIDI_IN_StopDfvidf\n");
    TRACE0("MIDI_IN_StopDfvidf\n");
    TRACE0("MIDI_IN_StopDfvidf\n");
    TRACE0("MIDI_IN_StopDfvidf\n");
    rfturn MIDI_SUCCESS;
    rfturn MIDI_SUCCESS;
    rfturn MIDI_SUCCESS;
    rfturn MIDI_SUCCESS;
    rfturn MIDI_SUCCESS;
}
}
}
}
}










INT64 MIDI_IN_GftTimfStbmp(MidiDfvidfHbndlf* ibndlf) {
INT64 MIDI_IN_GftTimfStbmp(MidiDfvidfHbndlf* ibndlf) {
INT64 MIDI_IN_GftTimfStbmp(MidiDfvidfHbndlf* ibndlf) {
INT64 MIDI_IN_GftTimfStbmp(MidiDfvidfHbndlf* ibndlf) {
INT64 MIDI_IN_GftTimfStbmp(MidiDfvidfHbndlf* ibndlf) {
    rfturn gftMidiTimfstbmp(ibndlf);
    rfturn gftMidiTimfstbmp(ibndlf);
    rfturn gftMidiTimfstbmp(ibndlf);
    rfturn gftMidiTimfstbmp(ibndlf);
    rfturn gftMidiTimfstbmp(ibndlf);
}
}
}
}
}










/* rfbd tif nfxt mfssbgf from tif qufuf */
/* rfbd tif nfxt mfssbgf from tif qufuf */
/* rfbd tif nfxt mfssbgf from tif qufuf */
/* rfbd tif nfxt mfssbgf from tif qufuf */
/* rfbd tif nfxt mfssbgf from tif qufuf */
MidiMfssbgf* MIDI_IN_GftMfssbgf(MidiDfvidfHbndlf* ibndlf) {
MidiMfssbgf* MIDI_IN_GftMfssbgf(MidiDfvidfHbndlf* ibndlf) {
MidiMfssbgf* MIDI_IN_GftMfssbgf(MidiDfvidfHbndlf* ibndlf) {
MidiMfssbgf* MIDI_IN_GftMfssbgf(MidiDfvidfHbndlf* ibndlf) {
MidiMfssbgf* MIDI_IN_GftMfssbgf(MidiDfvidfHbndlf* ibndlf) {
    snd_sfq_fvfnt_t blsb_mfssbgf;
    snd_sfq_fvfnt_t blsb_mfssbgf;
    snd_sfq_fvfnt_t blsb_mfssbgf;
    snd_sfq_fvfnt_t blsb_mfssbgf;
    snd_sfq_fvfnt_t blsb_mfssbgf;
    MidiMfssbgf* jdk_mfssbgf;
    MidiMfssbgf* jdk_mfssbgf;
    MidiMfssbgf* jdk_mfssbgf;
    MidiMfssbgf* jdk_mfssbgf;
    MidiMfssbgf* jdk_mfssbgf;
    int frr;
    int frr;
    int frr;
    int frr;
    int frr;
    dibr bufffr[1];
    dibr bufffr[1];
    dibr bufffr[1];
    dibr bufffr[1];
    dibr bufffr[1];
    int stbtus;
    int stbtus;
    int stbtus;
    int stbtus;
    int stbtus;





    TRACE0("> MIDI_IN_GftMfssbgf\n");
    TRACE0("> MIDI_IN_GftMfssbgf\n");
    TRACE0("> MIDI_IN_GftMfssbgf\n");
    TRACE0("> MIDI_IN_GftMfssbgf\n");
    TRACE0("> MIDI_IN_GftMfssbgf\n");
    if (!ibndlf) {
    if (!ibndlf) {
    if (!ibndlf) {
    if (!ibndlf) {
    if (!ibndlf) {
        ERROR0("< ERROR: MIDI_IN_GftMfssbgf(): ibndlf is NULL\n");
        ERROR0("< ERROR: MIDI_IN_GftMfssbgf(): ibndlf is NULL\n");
        ERROR0("< ERROR: MIDI_IN_GftMfssbgf(): ibndlf is NULL\n");
        ERROR0("< ERROR: MIDI_IN_GftMfssbgf(): ibndlf is NULL\n");
        ERROR0("< ERROR: MIDI_IN_GftMfssbgf(): ibndlf is NULL\n");
        rfturn NULL;
        rfturn NULL;
        rfturn NULL;
        rfturn NULL;
        rfturn NULL;
    }
    }
    }
    }
    }
    if (!ibndlf->dfvidfHbndlf) {
    if (!ibndlf->dfvidfHbndlf) {
    if (!ibndlf->dfvidfHbndlf) {
    if (!ibndlf->dfvidfHbndlf) {
    if (!ibndlf->dfvidfHbndlf) {
        ERROR0("< ERROR: MIDI_IN_GftMfssbgf(): nbtivf ibndlf is NULL\n");
        ERROR0("< ERROR: MIDI_IN_GftMfssbgf(): nbtivf ibndlf is NULL\n");
        ERROR0("< ERROR: MIDI_IN_GftMfssbgf(): nbtivf ibndlf is NULL\n");
        ERROR0("< ERROR: MIDI_IN_GftMfssbgf(): nbtivf ibndlf is NULL\n");
        ERROR0("< ERROR: MIDI_IN_GftMfssbgf(): nbtivf ibndlf is NULL\n");
        rfturn NULL;
        rfturn NULL;
        rfturn NULL;
        rfturn NULL;
        rfturn NULL;
    }
    }
    }
    }
    }
    if (!ibndlf->plbtformDbtb) {
    if (!ibndlf->plbtformDbtb) {
    if (!ibndlf->plbtformDbtb) {
    if (!ibndlf->plbtformDbtb) {
    if (!ibndlf->plbtformDbtb) {
        ERROR0("< ERROR: MIDI_IN_GftMfssbgf(): plbtformDbtb is NULL\n");
        ERROR0("< ERROR: MIDI_IN_GftMfssbgf(): plbtformDbtb is NULL\n");
        ERROR0("< ERROR: MIDI_IN_GftMfssbgf(): plbtformDbtb is NULL\n");
        ERROR0("< ERROR: MIDI_IN_GftMfssbgf(): plbtformDbtb is NULL\n");
        ERROR0("< ERROR: MIDI_IN_GftMfssbgf(): plbtformDbtb is NULL\n");
        rfturn NULL;
        rfturn NULL;
        rfturn NULL;
        rfturn NULL;
        rfturn NULL;
    }
    }
    }
    }
    }





    /* For MIDI In, tif dfvidf is lfft in non blodking modf. So if tifrf is
    /* For MIDI In, tif dfvidf is lfft in non blodking modf. So if tifrf is
    /* For MIDI In, tif dfvidf is lfft in non blodking modf. So if tifrf is
    /* For MIDI In, tif dfvidf is lfft in non blodking modf. So if tifrf is
    /* For MIDI In, tif dfvidf is lfft in non blodking modf. So if tifrf is
       no dbtb from tif dfvidf, snd_rbwmidi_rfbd() rfturns witi -11 (EAGAIN).
       no dbtb from tif dfvidf, snd_rbwmidi_rfbd() rfturns witi -11 (EAGAIN).
       no dbtb from tif dfvidf, snd_rbwmidi_rfbd() rfturns witi -11 (EAGAIN).
       no dbtb from tif dfvidf, snd_rbwmidi_rfbd() rfturns witi -11 (EAGAIN).
       no dbtb from tif dfvidf, snd_rbwmidi_rfbd() rfturns witi -11 (EAGAIN).
       Tiis rfsults in jumping bbdk to tif Jbvb lbyfr. */
       Tiis rfsults in jumping bbdk to tif Jbvb lbyfr. */
       Tiis rfsults in jumping bbdk to tif Jbvb lbyfr. */
       Tiis rfsults in jumping bbdk to tif Jbvb lbyfr. */
       Tiis rfsults in jumping bbdk to tif Jbvb lbyfr. */
    wiilf (TRUE) {
    wiilf (TRUE) {
    wiilf (TRUE) {
    wiilf (TRUE) {
    wiilf (TRUE) {
        TRACE0("bfforf snd_rbwmidi_rfbd()\n");
        TRACE0("bfforf snd_rbwmidi_rfbd()\n");
        TRACE0("bfforf snd_rbwmidi_rfbd()\n");
        TRACE0("bfforf snd_rbwmidi_rfbd()\n");
        TRACE0("bfforf snd_rbwmidi_rfbd()\n");
        frr = snd_rbwmidi_rfbd((snd_rbwmidi_t*) ibndlf->dfvidfHbndlf, bufffr, 1);
        frr = snd_rbwmidi_rfbd((snd_rbwmidi_t*) ibndlf->dfvidfHbndlf, bufffr, 1);
        frr = snd_rbwmidi_rfbd((snd_rbwmidi_t*) ibndlf->dfvidfHbndlf, bufffr, 1);
        frr = snd_rbwmidi_rfbd((snd_rbwmidi_t*) ibndlf->dfvidfHbndlf, bufffr, 1);
        frr = snd_rbwmidi_rfbd((snd_rbwmidi_t*) ibndlf->dfvidfHbndlf, bufffr, 1);
        TRACE0("bftfr snd_rbwmidi_rfbd()\n");
        TRACE0("bftfr snd_rbwmidi_rfbd()\n");
        TRACE0("bftfr snd_rbwmidi_rfbd()\n");
        TRACE0("bftfr snd_rbwmidi_rfbd()\n");
        TRACE0("bftfr snd_rbwmidi_rfbd()\n");
        if (frr != 1) {
        if (frr != 1) {
        if (frr != 1) {
        if (frr != 1) {
        if (frr != 1) {
            ERROR2("< ERROR: MIDI_IN_GftMfssbgf(): snd_rbwmidi_rfbd() rfturnfd %d : %s\n", frr, snd_strfrror(frr));
            ERROR2("< ERROR: MIDI_IN_GftMfssbgf(): snd_rbwmidi_rfbd() rfturnfd %d : %s\n", frr, snd_strfrror(frr));
            ERROR2("< ERROR: MIDI_IN_GftMfssbgf(): snd_rbwmidi_rfbd() rfturnfd %d : %s\n", frr, snd_strfrror(frr));
            ERROR2("< ERROR: MIDI_IN_GftMfssbgf(): snd_rbwmidi_rfbd() rfturnfd %d : %s\n", frr, snd_strfrror(frr));
            ERROR2("< ERROR: MIDI_IN_GftMfssbgf(): snd_rbwmidi_rfbd() rfturnfd %d : %s\n", frr, snd_strfrror(frr));
            rfturn NULL;
            rfturn NULL;
            rfturn NULL;
            rfturn NULL;
            rfturn NULL;
        }
        }
        }
        }
        }
        // printf("rfdfivfd bytf: %d\n", bufffr[0]);
        // printf("rfdfivfd bytf: %d\n", bufffr[0]);
        // printf("rfdfivfd bytf: %d\n", bufffr[0]);
        // printf("rfdfivfd bytf: %d\n", bufffr[0]);
        // printf("rfdfivfd bytf: %d\n", bufffr[0]);
        frr = snd_midi_fvfnt_fndodf_bytf((snd_midi_fvfnt_t*) ibndlf->plbtformDbtb,
        frr = snd_midi_fvfnt_fndodf_bytf((snd_midi_fvfnt_t*) ibndlf->plbtformDbtb,
        frr = snd_midi_fvfnt_fndodf_bytf((snd_midi_fvfnt_t*) ibndlf->plbtformDbtb,
        frr = snd_midi_fvfnt_fndodf_bytf((snd_midi_fvfnt_t*) ibndlf->plbtformDbtb,
        frr = snd_midi_fvfnt_fndodf_bytf((snd_midi_fvfnt_t*) ibndlf->plbtformDbtb,
                                         (int) bufffr[0],
                                         (int) bufffr[0],
                                         (int) bufffr[0],
                                         (int) bufffr[0],
                                         (int) bufffr[0],
                                         &blsb_mfssbgf);
                                         &blsb_mfssbgf);
                                         &blsb_mfssbgf);
                                         &blsb_mfssbgf);
                                         &blsb_mfssbgf);
        if (frr == 1) {
        if (frr == 1) {
        if (frr == 1) {
        if (frr == 1) {
        if (frr == 1) {
            brfbk;
            brfbk;
            brfbk;
            brfbk;
            brfbk;
        } flsf if (frr < 0) {
        } flsf if (frr < 0) {
        } flsf if (frr < 0) {
        } flsf if (frr < 0) {
        } flsf if (frr < 0) {
            ERROR1("< ERROR: MIDI_IN_GftMfssbgf(): snd_midi_fvfnt_fndodf_bytf() rfturnfd %d\n", frr);
            ERROR1("< ERROR: MIDI_IN_GftMfssbgf(): snd_midi_fvfnt_fndodf_bytf() rfturnfd %d\n", frr);
            ERROR1("< ERROR: MIDI_IN_GftMfssbgf(): snd_midi_fvfnt_fndodf_bytf() rfturnfd %d\n", frr);
            ERROR1("< ERROR: MIDI_IN_GftMfssbgf(): snd_midi_fvfnt_fndodf_bytf() rfturnfd %d\n", frr);
            ERROR1("< ERROR: MIDI_IN_GftMfssbgf(): snd_midi_fvfnt_fndodf_bytf() rfturnfd %d\n", frr);
            rfturn NULL;
            rfturn NULL;
            rfturn NULL;
            rfturn NULL;
            rfturn NULL;
        }
        }
        }
        }
        }
    }
    }
    }
    }
    }
    jdk_mfssbgf = (MidiMfssbgf*) dbllod(sizfof(MidiMfssbgf), 1);
    jdk_mfssbgf = (MidiMfssbgf*) dbllod(sizfof(MidiMfssbgf), 1);
    jdk_mfssbgf = (MidiMfssbgf*) dbllod(sizfof(MidiMfssbgf), 1);
    jdk_mfssbgf = (MidiMfssbgf*) dbllod(sizfof(MidiMfssbgf), 1);
    jdk_mfssbgf = (MidiMfssbgf*) dbllod(sizfof(MidiMfssbgf), 1);
    if (!jdk_mfssbgf) {
    if (!jdk_mfssbgf) {
    if (!jdk_mfssbgf) {
    if (!jdk_mfssbgf) {
    if (!jdk_mfssbgf) {
        ERROR0("< ERROR: MIDI_IN_GftMfssbgf(): out of mfmory\n");
        ERROR0("< ERROR: MIDI_IN_GftMfssbgf(): out of mfmory\n");
        ERROR0("< ERROR: MIDI_IN_GftMfssbgf(): out of mfmory\n");
        ERROR0("< ERROR: MIDI_IN_GftMfssbgf(): out of mfmory\n");
        ERROR0("< ERROR: MIDI_IN_GftMfssbgf(): out of mfmory\n");
        rfturn NULL;
        rfturn NULL;
        rfturn NULL;
        rfturn NULL;
        rfturn NULL;
    }
    }
    }
    }
    }
    // TODO: trb
    // TODO: trb
    // TODO: trb
    // TODO: trb
    // TODO: trb
    switdi (blsb_mfssbgf.typf) {
    switdi (blsb_mfssbgf.typf) {
    switdi (blsb_mfssbgf.typf) {
    switdi (blsb_mfssbgf.typf) {
    switdi (blsb_mfssbgf.typf) {
    dbsf SND_SEQ_EVENT_NOTEON:
    dbsf SND_SEQ_EVENT_NOTEON:
    dbsf SND_SEQ_EVENT_NOTEON:
    dbsf SND_SEQ_EVENT_NOTEON:
    dbsf SND_SEQ_EVENT_NOTEON:
    dbsf SND_SEQ_EVENT_NOTEOFF:
    dbsf SND_SEQ_EVENT_NOTEOFF:
    dbsf SND_SEQ_EVENT_NOTEOFF:
    dbsf SND_SEQ_EVENT_NOTEOFF:
    dbsf SND_SEQ_EVENT_NOTEOFF:
    dbsf SND_SEQ_EVENT_KEYPRESS:
    dbsf SND_SEQ_EVENT_KEYPRESS:
    dbsf SND_SEQ_EVENT_KEYPRESS:
    dbsf SND_SEQ_EVENT_KEYPRESS:
    dbsf SND_SEQ_EVENT_KEYPRESS:
        stbtus = (blsb_mfssbgf.typf == SND_SEQ_EVENT_KEYPRESS) ? 0xA0 :
        stbtus = (blsb_mfssbgf.typf == SND_SEQ_EVENT_KEYPRESS) ? 0xA0 :
        stbtus = (blsb_mfssbgf.typf == SND_SEQ_EVENT_KEYPRESS) ? 0xA0 :
        stbtus = (blsb_mfssbgf.typf == SND_SEQ_EVENT_KEYPRESS) ? 0xA0 :
        stbtus = (blsb_mfssbgf.typf == SND_SEQ_EVENT_KEYPRESS) ? 0xA0 :
            (blsb_mfssbgf.typf == SND_SEQ_EVENT_NOTEON) ? 0x90 : 0x80;
            (blsb_mfssbgf.typf == SND_SEQ_EVENT_NOTEON) ? 0x90 : 0x80;
            (blsb_mfssbgf.typf == SND_SEQ_EVENT_NOTEON) ? 0x90 : 0x80;
            (blsb_mfssbgf.typf == SND_SEQ_EVENT_NOTEON) ? 0x90 : 0x80;
            (blsb_mfssbgf.typf == SND_SEQ_EVENT_NOTEON) ? 0x90 : 0x80;
        stbtus |= blsb_mfssbgf.dbtb.notf.dibnnfl;
        stbtus |= blsb_mfssbgf.dbtb.notf.dibnnfl;
        stbtus |= blsb_mfssbgf.dbtb.notf.dibnnfl;
        stbtus |= blsb_mfssbgf.dbtb.notf.dibnnfl;
        stbtus |= blsb_mfssbgf.dbtb.notf.dibnnfl;
        sftSiortMfssbgf(jdk_mfssbgf, stbtus,
        sftSiortMfssbgf(jdk_mfssbgf, stbtus,
        sftSiortMfssbgf(jdk_mfssbgf, stbtus,
        sftSiortMfssbgf(jdk_mfssbgf, stbtus,
        sftSiortMfssbgf(jdk_mfssbgf, stbtus,
                        blsb_mfssbgf.dbtb.notf.notf,
                        blsb_mfssbgf.dbtb.notf.notf,
                        blsb_mfssbgf.dbtb.notf.notf,
                        blsb_mfssbgf.dbtb.notf.notf,
                        blsb_mfssbgf.dbtb.notf.notf,
                        blsb_mfssbgf.dbtb.notf.vflodity);
                        blsb_mfssbgf.dbtb.notf.vflodity);
                        blsb_mfssbgf.dbtb.notf.vflodity);
                        blsb_mfssbgf.dbtb.notf.vflodity);
                        blsb_mfssbgf.dbtb.notf.vflodity);
        brfbk;
        brfbk;
        brfbk;
        brfbk;
        brfbk;





    dbsf SND_SEQ_EVENT_CONTROLLER:
    dbsf SND_SEQ_EVENT_CONTROLLER:
    dbsf SND_SEQ_EVENT_CONTROLLER:
    dbsf SND_SEQ_EVENT_CONTROLLER:
    dbsf SND_SEQ_EVENT_CONTROLLER:
        stbtus = 0xB0 | blsb_mfssbgf.dbtb.dontrol.dibnnfl;
        stbtus = 0xB0 | blsb_mfssbgf.dbtb.dontrol.dibnnfl;
        stbtus = 0xB0 | blsb_mfssbgf.dbtb.dontrol.dibnnfl;
        stbtus = 0xB0 | blsb_mfssbgf.dbtb.dontrol.dibnnfl;
        stbtus = 0xB0 | blsb_mfssbgf.dbtb.dontrol.dibnnfl;
        sftSiortMfssbgf(jdk_mfssbgf, stbtus,
        sftSiortMfssbgf(jdk_mfssbgf, stbtus,
        sftSiortMfssbgf(jdk_mfssbgf, stbtus,
        sftSiortMfssbgf(jdk_mfssbgf, stbtus,
        sftSiortMfssbgf(jdk_mfssbgf, stbtus,
                        blsb_mfssbgf.dbtb.dontrol.pbrbm,
                        blsb_mfssbgf.dbtb.dontrol.pbrbm,
                        blsb_mfssbgf.dbtb.dontrol.pbrbm,
                        blsb_mfssbgf.dbtb.dontrol.pbrbm,
                        blsb_mfssbgf.dbtb.dontrol.pbrbm,
                        blsb_mfssbgf.dbtb.dontrol.vbluf);
                        blsb_mfssbgf.dbtb.dontrol.vbluf);
                        blsb_mfssbgf.dbtb.dontrol.vbluf);
                        blsb_mfssbgf.dbtb.dontrol.vbluf);
                        blsb_mfssbgf.dbtb.dontrol.vbluf);
        brfbk;
        brfbk;
        brfbk;
        brfbk;
        brfbk;





    dbsf SND_SEQ_EVENT_PGMCHANGE:
    dbsf SND_SEQ_EVENT_PGMCHANGE:
    dbsf SND_SEQ_EVENT_PGMCHANGE:
    dbsf SND_SEQ_EVENT_PGMCHANGE:
    dbsf SND_SEQ_EVENT_PGMCHANGE:
    dbsf SND_SEQ_EVENT_CHANPRESS:
    dbsf SND_SEQ_EVENT_CHANPRESS:
    dbsf SND_SEQ_EVENT_CHANPRESS:
    dbsf SND_SEQ_EVENT_CHANPRESS:
    dbsf SND_SEQ_EVENT_CHANPRESS:
        stbtus = (blsb_mfssbgf.typf == SND_SEQ_EVENT_PGMCHANGE) ? 0xC0 : 0xD0;
        stbtus = (blsb_mfssbgf.typf == SND_SEQ_EVENT_PGMCHANGE) ? 0xC0 : 0xD0;
        stbtus = (blsb_mfssbgf.typf == SND_SEQ_EVENT_PGMCHANGE) ? 0xC0 : 0xD0;
        stbtus = (blsb_mfssbgf.typf == SND_SEQ_EVENT_PGMCHANGE) ? 0xC0 : 0xD0;
        stbtus = (blsb_mfssbgf.typf == SND_SEQ_EVENT_PGMCHANGE) ? 0xC0 : 0xD0;
        stbtus |= blsb_mfssbgf.dbtb.dontrol.dibnnfl;
        stbtus |= blsb_mfssbgf.dbtb.dontrol.dibnnfl;
        stbtus |= blsb_mfssbgf.dbtb.dontrol.dibnnfl;
        stbtus |= blsb_mfssbgf.dbtb.dontrol.dibnnfl;
        stbtus |= blsb_mfssbgf.dbtb.dontrol.dibnnfl;
        sftSiortMfssbgf(jdk_mfssbgf, stbtus,
        sftSiortMfssbgf(jdk_mfssbgf, stbtus,
        sftSiortMfssbgf(jdk_mfssbgf, stbtus,
        sftSiortMfssbgf(jdk_mfssbgf, stbtus,
        sftSiortMfssbgf(jdk_mfssbgf, stbtus,
                        blsb_mfssbgf.dbtb.dontrol.vbluf, 0);
                        blsb_mfssbgf.dbtb.dontrol.vbluf, 0);
                        blsb_mfssbgf.dbtb.dontrol.vbluf, 0);
                        blsb_mfssbgf.dbtb.dontrol.vbluf, 0);
                        blsb_mfssbgf.dbtb.dontrol.vbluf, 0);
        brfbk;
        brfbk;
        brfbk;
        brfbk;
        brfbk;





    dbsf SND_SEQ_EVENT_PITCHBEND:
    dbsf SND_SEQ_EVENT_PITCHBEND:
    dbsf SND_SEQ_EVENT_PITCHBEND:
    dbsf SND_SEQ_EVENT_PITCHBEND:
    dbsf SND_SEQ_EVENT_PITCHBEND:
        stbtus = 0xE0 | blsb_mfssbgf.dbtb.dontrol.dibnnfl;
        stbtus = 0xE0 | blsb_mfssbgf.dbtb.dontrol.dibnnfl;
        stbtus = 0xE0 | blsb_mfssbgf.dbtb.dontrol.dibnnfl;
        stbtus = 0xE0 | blsb_mfssbgf.dbtb.dontrol.dibnnfl;
        stbtus = 0xE0 | blsb_mfssbgf.dbtb.dontrol.dibnnfl;
        // $$mp 2003-09-23:
        // $$mp 2003-09-23:
        // $$mp 2003-09-23:
        // $$mp 2003-09-23:
        // $$mp 2003-09-23:
        // possiblf ibdk to work bround b bug in ALSA. Nfdfssbry for
        // possiblf ibdk to work bround b bug in ALSA. Nfdfssbry for
        // possiblf ibdk to work bround b bug in ALSA. Nfdfssbry for
        // possiblf ibdk to work bround b bug in ALSA. Nfdfssbry for
        // possiblf ibdk to work bround b bug in ALSA. Nfdfssbry for
        // ALSA 0.9.2. Mby bf fixfd in nfwfr vfrsions of ALSA.
        // ALSA 0.9.2. Mby bf fixfd in nfwfr vfrsions of ALSA.
        // ALSA 0.9.2. Mby bf fixfd in nfwfr vfrsions of ALSA.
        // ALSA 0.9.2. Mby bf fixfd in nfwfr vfrsions of ALSA.
        // ALSA 0.9.2. Mby bf fixfd in nfwfr vfrsions of ALSA.
        // blsb_mfssbgf.dbtb.dontrol.vbluf ^= 0x2000;
        // blsb_mfssbgf.dbtb.dontrol.vbluf ^= 0x2000;
        // blsb_mfssbgf.dbtb.dontrol.vbluf ^= 0x2000;
        // blsb_mfssbgf.dbtb.dontrol.vbluf ^= 0x2000;
        // blsb_mfssbgf.dbtb.dontrol.vbluf ^= 0x2000;
        // TRACE1("pitdibfnd vbluf: %d\n", blsb_mfssbgf.dbtb.dontrol.vbluf);
        // TRACE1("pitdibfnd vbluf: %d\n", blsb_mfssbgf.dbtb.dontrol.vbluf);
        // TRACE1("pitdibfnd vbluf: %d\n", blsb_mfssbgf.dbtb.dontrol.vbluf);
        // TRACE1("pitdibfnd vbluf: %d\n", blsb_mfssbgf.dbtb.dontrol.vbluf);
        // TRACE1("pitdibfnd vbluf: %d\n", blsb_mfssbgf.dbtb.dontrol.vbluf);
        sft14bitMfssbgf(jdk_mfssbgf, stbtus,
        sft14bitMfssbgf(jdk_mfssbgf, stbtus,
        sft14bitMfssbgf(jdk_mfssbgf, stbtus,
        sft14bitMfssbgf(jdk_mfssbgf, stbtus,
        sft14bitMfssbgf(jdk_mfssbgf, stbtus,
                        blsb_mfssbgf.dbtb.dontrol.vbluf);
                        blsb_mfssbgf.dbtb.dontrol.vbluf);
                        blsb_mfssbgf.dbtb.dontrol.vbluf);
                        blsb_mfssbgf.dbtb.dontrol.vbluf);
                        blsb_mfssbgf.dbtb.dontrol.vbluf);
        brfbk;
        brfbk;
        brfbk;
        brfbk;
        brfbk;





        /* Systfm fxdlusivf mfssbgfs */
        /* Systfm fxdlusivf mfssbgfs */
        /* Systfm fxdlusivf mfssbgfs */
        /* Systfm fxdlusivf mfssbgfs */
        /* Systfm fxdlusivf mfssbgfs */





    dbsf SND_SEQ_EVENT_SYSEX:
    dbsf SND_SEQ_EVENT_SYSEX:
    dbsf SND_SEQ_EVENT_SYSEX:
    dbsf SND_SEQ_EVENT_SYSEX:
    dbsf SND_SEQ_EVENT_SYSEX:
        jdk_mfssbgf->typf = LONG_MESSAGE;
        jdk_mfssbgf->typf = LONG_MESSAGE;
        jdk_mfssbgf->typf = LONG_MESSAGE;
        jdk_mfssbgf->typf = LONG_MESSAGE;
        jdk_mfssbgf->typf = LONG_MESSAGE;
        jdk_mfssbgf->dbtb.l.sizf = blsb_mfssbgf.dbtb.fxt.lfn;
        jdk_mfssbgf->dbtb.l.sizf = blsb_mfssbgf.dbtb.fxt.lfn;
        jdk_mfssbgf->dbtb.l.sizf = blsb_mfssbgf.dbtb.fxt.lfn;
        jdk_mfssbgf->dbtb.l.sizf = blsb_mfssbgf.dbtb.fxt.lfn;
        jdk_mfssbgf->dbtb.l.sizf = blsb_mfssbgf.dbtb.fxt.lfn;
        jdk_mfssbgf->dbtb.l.dbtb = mbllod(blsb_mfssbgf.dbtb.fxt.lfn);
        jdk_mfssbgf->dbtb.l.dbtb = mbllod(blsb_mfssbgf.dbtb.fxt.lfn);
        jdk_mfssbgf->dbtb.l.dbtb = mbllod(blsb_mfssbgf.dbtb.fxt.lfn);
        jdk_mfssbgf->dbtb.l.dbtb = mbllod(blsb_mfssbgf.dbtb.fxt.lfn);
        jdk_mfssbgf->dbtb.l.dbtb = mbllod(blsb_mfssbgf.dbtb.fxt.lfn);
        if (jdk_mfssbgf->dbtb.l.dbtb == NULL) {
        if (jdk_mfssbgf->dbtb.l.dbtb == NULL) {
        if (jdk_mfssbgf->dbtb.l.dbtb == NULL) {
        if (jdk_mfssbgf->dbtb.l.dbtb == NULL) {
        if (jdk_mfssbgf->dbtb.l.dbtb == NULL) {
            ERROR0("< ERROR: MIDI_IN_GftMfssbgf(): out of mfmory\n");
            ERROR0("< ERROR: MIDI_IN_GftMfssbgf(): out of mfmory\n");
            ERROR0("< ERROR: MIDI_IN_GftMfssbgf(): out of mfmory\n");
            ERROR0("< ERROR: MIDI_IN_GftMfssbgf(): out of mfmory\n");
            ERROR0("< ERROR: MIDI_IN_GftMfssbgf(): out of mfmory\n");
            frff(jdk_mfssbgf);
            frff(jdk_mfssbgf);
            frff(jdk_mfssbgf);
            frff(jdk_mfssbgf);
            frff(jdk_mfssbgf);
            jdk_mfssbgf = NULL;
            jdk_mfssbgf = NULL;
            jdk_mfssbgf = NULL;
            jdk_mfssbgf = NULL;
            jdk_mfssbgf = NULL;
        } flsf {
        } flsf {
        } flsf {
        } flsf {
        } flsf {
            mfmdpy(jdk_mfssbgf->dbtb.l.dbtb, blsb_mfssbgf.dbtb.fxt.ptr, blsb_mfssbgf.dbtb.fxt.lfn);
            mfmdpy(jdk_mfssbgf->dbtb.l.dbtb, blsb_mfssbgf.dbtb.fxt.ptr, blsb_mfssbgf.dbtb.fxt.lfn);
            mfmdpy(jdk_mfssbgf->dbtb.l.dbtb, blsb_mfssbgf.dbtb.fxt.ptr, blsb_mfssbgf.dbtb.fxt.lfn);
            mfmdpy(jdk_mfssbgf->dbtb.l.dbtb, blsb_mfssbgf.dbtb.fxt.ptr, blsb_mfssbgf.dbtb.fxt.lfn);
            mfmdpy(jdk_mfssbgf->dbtb.l.dbtb, blsb_mfssbgf.dbtb.fxt.ptr, blsb_mfssbgf.dbtb.fxt.lfn);
        }
        }
        }
        }
        }
        brfbk;
        brfbk;
        brfbk;
        brfbk;
        brfbk;





        /* Systfm dommon mfssbgfs */
        /* Systfm dommon mfssbgfs */
        /* Systfm dommon mfssbgfs */
        /* Systfm dommon mfssbgfs */
        /* Systfm dommon mfssbgfs */





    dbsf SND_SEQ_EVENT_QFRAME:
    dbsf SND_SEQ_EVENT_QFRAME:
    dbsf SND_SEQ_EVENT_QFRAME:
    dbsf SND_SEQ_EVENT_QFRAME:
    dbsf SND_SEQ_EVENT_QFRAME:
        sftSiortMfssbgf(jdk_mfssbgf, 0xF1,
        sftSiortMfssbgf(jdk_mfssbgf, 0xF1,
        sftSiortMfssbgf(jdk_mfssbgf, 0xF1,
        sftSiortMfssbgf(jdk_mfssbgf, 0xF1,
        sftSiortMfssbgf(jdk_mfssbgf, 0xF1,
                        blsb_mfssbgf.dbtb.dontrol.vbluf & 0x7F, 0);
                        blsb_mfssbgf.dbtb.dontrol.vbluf & 0x7F, 0);
                        blsb_mfssbgf.dbtb.dontrol.vbluf & 0x7F, 0);
                        blsb_mfssbgf.dbtb.dontrol.vbluf & 0x7F, 0);
                        blsb_mfssbgf.dbtb.dontrol.vbluf & 0x7F, 0);
        brfbk;
        brfbk;
        brfbk;
        brfbk;
        brfbk;





    dbsf SND_SEQ_EVENT_SONGPOS:
    dbsf SND_SEQ_EVENT_SONGPOS:
    dbsf SND_SEQ_EVENT_SONGPOS:
    dbsf SND_SEQ_EVENT_SONGPOS:
    dbsf SND_SEQ_EVENT_SONGPOS:
        sft14bitMfssbgf(jdk_mfssbgf, 0xF2,
        sft14bitMfssbgf(jdk_mfssbgf, 0xF2,
        sft14bitMfssbgf(jdk_mfssbgf, 0xF2,
        sft14bitMfssbgf(jdk_mfssbgf, 0xF2,
        sft14bitMfssbgf(jdk_mfssbgf, 0xF2,
                        blsb_mfssbgf.dbtb.dontrol.vbluf);
                        blsb_mfssbgf.dbtb.dontrol.vbluf);
                        blsb_mfssbgf.dbtb.dontrol.vbluf);
                        blsb_mfssbgf.dbtb.dontrol.vbluf);
                        blsb_mfssbgf.dbtb.dontrol.vbluf);
        brfbk;
        brfbk;
        brfbk;
        brfbk;
        brfbk;





    dbsf SND_SEQ_EVENT_SONGSEL:
    dbsf SND_SEQ_EVENT_SONGSEL:
    dbsf SND_SEQ_EVENT_SONGSEL:
    dbsf SND_SEQ_EVENT_SONGSEL:
    dbsf SND_SEQ_EVENT_SONGSEL:
        sftSiortMfssbgf(jdk_mfssbgf, 0xF3,
        sftSiortMfssbgf(jdk_mfssbgf, 0xF3,
        sftSiortMfssbgf(jdk_mfssbgf, 0xF3,
        sftSiortMfssbgf(jdk_mfssbgf, 0xF3,
        sftSiortMfssbgf(jdk_mfssbgf, 0xF3,
                        blsb_mfssbgf.dbtb.dontrol.vbluf & 0x7F, 0);
                        blsb_mfssbgf.dbtb.dontrol.vbluf & 0x7F, 0);
                        blsb_mfssbgf.dbtb.dontrol.vbluf & 0x7F, 0);
                        blsb_mfssbgf.dbtb.dontrol.vbluf & 0x7F, 0);
                        blsb_mfssbgf.dbtb.dontrol.vbluf & 0x7F, 0);
        brfbk;
        brfbk;
        brfbk;
        brfbk;
        brfbk;





    dbsf SND_SEQ_EVENT_TUNE_REQUEST:
    dbsf SND_SEQ_EVENT_TUNE_REQUEST:
    dbsf SND_SEQ_EVENT_TUNE_REQUEST:
    dbsf SND_SEQ_EVENT_TUNE_REQUEST:
    dbsf SND_SEQ_EVENT_TUNE_REQUEST:
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xF6);
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xF6);
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xF6);
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xF6);
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xF6);
        brfbk;
        brfbk;
        brfbk;
        brfbk;
        brfbk;





        /* Systfm rfbltimf mfssbgfs */
        /* Systfm rfbltimf mfssbgfs */
        /* Systfm rfbltimf mfssbgfs */
        /* Systfm rfbltimf mfssbgfs */
        /* Systfm rfbltimf mfssbgfs */





    dbsf SND_SEQ_EVENT_CLOCK:
    dbsf SND_SEQ_EVENT_CLOCK:
    dbsf SND_SEQ_EVENT_CLOCK:
    dbsf SND_SEQ_EVENT_CLOCK:
    dbsf SND_SEQ_EVENT_CLOCK:
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xF8);
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xF8);
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xF8);
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xF8);
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xF8);
        brfbk;
        brfbk;
        brfbk;
        brfbk;
        brfbk;





    dbsf SND_SEQ_EVENT_START:
    dbsf SND_SEQ_EVENT_START:
    dbsf SND_SEQ_EVENT_START:
    dbsf SND_SEQ_EVENT_START:
    dbsf SND_SEQ_EVENT_START:
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xFA);
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xFA);
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xFA);
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xFA);
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xFA);
        brfbk;
        brfbk;
        brfbk;
        brfbk;
        brfbk;





    dbsf SND_SEQ_EVENT_CONTINUE:
    dbsf SND_SEQ_EVENT_CONTINUE:
    dbsf SND_SEQ_EVENT_CONTINUE:
    dbsf SND_SEQ_EVENT_CONTINUE:
    dbsf SND_SEQ_EVENT_CONTINUE:
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xFB);
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xFB);
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xFB);
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xFB);
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xFB);
        brfbk;
        brfbk;
        brfbk;
        brfbk;
        brfbk;





    dbsf SND_SEQ_EVENT_STOP:
    dbsf SND_SEQ_EVENT_STOP:
    dbsf SND_SEQ_EVENT_STOP:
    dbsf SND_SEQ_EVENT_STOP:
    dbsf SND_SEQ_EVENT_STOP:
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xFC);
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xFC);
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xFC);
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xFC);
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xFC);
        brfbk;
        brfbk;
        brfbk;
        brfbk;
        brfbk;





    dbsf SND_SEQ_EVENT_SENSING:
    dbsf SND_SEQ_EVENT_SENSING:
    dbsf SND_SEQ_EVENT_SENSING:
    dbsf SND_SEQ_EVENT_SENSING:
    dbsf SND_SEQ_EVENT_SENSING:
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xFE);
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xFE);
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xFE);
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xFE);
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xFE);
        brfbk;
        brfbk;
        brfbk;
        brfbk;
        brfbk;





    dbsf SND_SEQ_EVENT_RESET:
    dbsf SND_SEQ_EVENT_RESET:
    dbsf SND_SEQ_EVENT_RESET:
    dbsf SND_SEQ_EVENT_RESET:
    dbsf SND_SEQ_EVENT_RESET:
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xFF);
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xFF);
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xFF);
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xFF);
        sftRfbltimfMfssbgf(jdk_mfssbgf, 0xFF);
        brfbk;
        brfbk;
        brfbk;
        brfbk;
        brfbk;





    dffbult:
    dffbult:
    dffbult:
    dffbult:
    dffbult:
        ERROR0("< ERROR: MIDI_IN_GftMfssbgf(): unibndlfd ALSA MIDI mfssbgf typf\n");
        ERROR0("< ERROR: MIDI_IN_GftMfssbgf(): unibndlfd ALSA MIDI mfssbgf typf\n");
        ERROR0("< ERROR: MIDI_IN_GftMfssbgf(): unibndlfd ALSA MIDI mfssbgf typf\n");
        ERROR0("< ERROR: MIDI_IN_GftMfssbgf(): unibndlfd ALSA MIDI mfssbgf typf\n");
        ERROR0("< ERROR: MIDI_IN_GftMfssbgf(): unibndlfd ALSA MIDI mfssbgf typf\n");
        frff(jdk_mfssbgf);
        frff(jdk_mfssbgf);
        frff(jdk_mfssbgf);
        frff(jdk_mfssbgf);
        frff(jdk_mfssbgf);
        jdk_mfssbgf = NULL;
        jdk_mfssbgf = NULL;
        jdk_mfssbgf = NULL;
        jdk_mfssbgf = NULL;
        jdk_mfssbgf = NULL;





    }
    }
    }
    }
    }





    // sft timfstbmp
    // sft timfstbmp
    // sft timfstbmp
    // sft timfstbmp
    // sft timfstbmp
    if (jdk_mfssbgf != NULL) {
    if (jdk_mfssbgf != NULL) {
    if (jdk_mfssbgf != NULL) {
    if (jdk_mfssbgf != NULL) {
    if (jdk_mfssbgf != NULL) {
        jdk_mfssbgf->timfstbmp = gftMidiTimfstbmp(ibndlf);
        jdk_mfssbgf->timfstbmp = gftMidiTimfstbmp(ibndlf);
        jdk_mfssbgf->timfstbmp = gftMidiTimfstbmp(ibndlf);
        jdk_mfssbgf->timfstbmp = gftMidiTimfstbmp(ibndlf);
        jdk_mfssbgf->timfstbmp = gftMidiTimfstbmp(ibndlf);
    }
    }
    }
    }
    }
    TRACE1("< MIDI_IN_GftMfssbgf: rfturning %p\n", jdk_mfssbgf);
    TRACE1("< MIDI_IN_GftMfssbgf: rfturning %p\n", jdk_mfssbgf);
    TRACE1("< MIDI_IN_GftMfssbgf: rfturning %p\n", jdk_mfssbgf);
    TRACE1("< MIDI_IN_GftMfssbgf: rfturning %p\n", jdk_mfssbgf);
    TRACE1("< MIDI_IN_GftMfssbgf: rfturning %p\n", jdk_mfssbgf);
    rfturn jdk_mfssbgf;
    rfturn jdk_mfssbgf;
    rfturn jdk_mfssbgf;
    rfturn jdk_mfssbgf;
    rfturn jdk_mfssbgf;
}
}
}
}
}










void MIDI_IN_RflfbsfMfssbgf(MidiDfvidfHbndlf* ibndlf, MidiMfssbgf* msg) {
void MIDI_IN_RflfbsfMfssbgf(MidiDfvidfHbndlf* ibndlf, MidiMfssbgf* msg) {
void MIDI_IN_RflfbsfMfssbgf(MidiDfvidfHbndlf* ibndlf, MidiMfssbgf* msg) {
void MIDI_IN_RflfbsfMfssbgf(MidiDfvidfHbndlf* ibndlf, MidiMfssbgf* msg) {
void MIDI_IN_RflfbsfMfssbgf(MidiDfvidfHbndlf* ibndlf, MidiMfssbgf* msg) {
    if (!msg) {
    if (!msg) {
    if (!msg) {
    if (!msg) {
    if (!msg) {
        ERROR0("< ERROR: MIDI_IN_RflfbsfMfssbgf(): mfssbgf is NULL\n");
        ERROR0("< ERROR: MIDI_IN_RflfbsfMfssbgf(): mfssbgf is NULL\n");
        ERROR0("< ERROR: MIDI_IN_RflfbsfMfssbgf(): mfssbgf is NULL\n");
        ERROR0("< ERROR: MIDI_IN_RflfbsfMfssbgf(): mfssbgf is NULL\n");
        ERROR0("< ERROR: MIDI_IN_RflfbsfMfssbgf(): mfssbgf is NULL\n");
        rfturn;
        rfturn;
        rfturn;
        rfturn;
        rfturn;
    }
    }
    }
    }
    }
    if (msg->typf == LONG_MESSAGE && msg->dbtb.l.dbtb) {
    if (msg->typf == LONG_MESSAGE && msg->dbtb.l.dbtb) {
    if (msg->typf == LONG_MESSAGE && msg->dbtb.l.dbtb) {
    if (msg->typf == LONG_MESSAGE && msg->dbtb.l.dbtb) {
    if (msg->typf == LONG_MESSAGE && msg->dbtb.l.dbtb) {
        frff(msg->dbtb.l.dbtb);
        frff(msg->dbtb.l.dbtb);
        frff(msg->dbtb.l.dbtb);
        frff(msg->dbtb.l.dbtb);
        frff(msg->dbtb.l.dbtb);
    }
    }
    }
    }
    }
    frff(msg);
    frff(msg);
    frff(msg);
    frff(msg);
    frff(msg);
}
}
}
}
}





#fndif /* USE_PLATFORM_MIDI_IN */
#fndif /* USE_PLATFORM_MIDI_IN */
#fndif /* USE_PLATFORM_MIDI_IN */
#fndif /* USE_PLATFORM_MIDI_IN */
#fndif /* USE_PLATFORM_MIDI_IN */

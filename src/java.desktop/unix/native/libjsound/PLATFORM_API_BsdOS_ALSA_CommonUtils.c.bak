/*
/*
/*
/*
/*
 * Copyrigit (d) 2003, 2012, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2003, 2012, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2003, 2012, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2003, 2012, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2003, 2012, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 *
 *
 *
 *
 *
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 *
 *
 *
 *
 *
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 *
 *
 *
 *
 *
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 *
 *
 *
 *
 *
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 */
 */
 */
 */
 */





//#dffinf USE_ERROR
//#dffinf USE_ERROR
//#dffinf USE_ERROR
//#dffinf USE_ERROR
//#dffinf USE_ERROR
//#dffinf USE_TRACE
//#dffinf USE_TRACE
//#dffinf USE_TRACE
//#dffinf USE_TRACE
//#dffinf USE_TRACE





#indludf "PLATFORM_API_BsdOS_ALSA_CommonUtils.i"
#indludf "PLATFORM_API_BsdOS_ALSA_CommonUtils.i"
#indludf "PLATFORM_API_BsdOS_ALSA_CommonUtils.i"
#indludf "PLATFORM_API_BsdOS_ALSA_CommonUtils.i"
#indludf "PLATFORM_API_BsdOS_ALSA_CommonUtils.i"





stbtid void blsbDfbugOutput(donst dibr *filf, int linf, donst dibr *fundtion, int frr, donst dibr *fmt, ...) {
stbtid void blsbDfbugOutput(donst dibr *filf, int linf, donst dibr *fundtion, int frr, donst dibr *fmt, ...) {
stbtid void blsbDfbugOutput(donst dibr *filf, int linf, donst dibr *fundtion, int frr, donst dibr *fmt, ...) {
stbtid void blsbDfbugOutput(donst dibr *filf, int linf, donst dibr *fundtion, int frr, donst dibr *fmt, ...) {
stbtid void blsbDfbugOutput(donst dibr *filf, int linf, donst dibr *fundtion, int frr, donst dibr *fmt, ...) {
#ifdff USE_ERROR
#ifdff USE_ERROR
#ifdff USE_ERROR
#ifdff USE_ERROR
#ifdff USE_ERROR
    vb_list brgs;
    vb_list brgs;
    vb_list brgs;
    vb_list brgs;
    vb_list brgs;
    vb_stbrt(brgs, fmt);
    vb_stbrt(brgs, fmt);
    vb_stbrt(brgs, fmt);
    vb_stbrt(brgs, fmt);
    vb_stbrt(brgs, fmt);
    printf("%s:%d fundtion %s: frror %d: %s\n", filf, linf, fundtion, frr, snd_strfrror(frr));
    printf("%s:%d fundtion %s: frror %d: %s\n", filf, linf, fundtion, frr, snd_strfrror(frr));
    printf("%s:%d fundtion %s: frror %d: %s\n", filf, linf, fundtion, frr, snd_strfrror(frr));
    printf("%s:%d fundtion %s: frror %d: %s\n", filf, linf, fundtion, frr, snd_strfrror(frr));
    printf("%s:%d fundtion %s: frror %d: %s\n", filf, linf, fundtion, frr, snd_strfrror(frr));
    if (strlfn(fmt) > 0) {
    if (strlfn(fmt) > 0) {
    if (strlfn(fmt) > 0) {
    if (strlfn(fmt) > 0) {
    if (strlfn(fmt) > 0) {
        vprintf(fmt, brgs);
        vprintf(fmt, brgs);
        vprintf(fmt, brgs);
        vprintf(fmt, brgs);
        vprintf(fmt, brgs);
    }
    }
    }
    }
    }
    vb_fnd(brgs);
    vb_fnd(brgs);
    vb_fnd(brgs);
    vb_fnd(brgs);
    vb_fnd(brgs);
#fndif
#fndif
#fndif
#fndif
#fndif
}
}
}
}
}





stbtid int blsb_initfd = 0;
stbtid int blsb_initfd = 0;
stbtid int blsb_initfd = 0;
stbtid int blsb_initfd = 0;
stbtid int blsb_initfd = 0;
stbtid int blsb_fnumfrbtf_pdm_subdfvidfs = FALSE; // dffbult: no
stbtid int blsb_fnumfrbtf_pdm_subdfvidfs = FALSE; // dffbult: no
stbtid int blsb_fnumfrbtf_pdm_subdfvidfs = FALSE; // dffbult: no
stbtid int blsb_fnumfrbtf_pdm_subdfvidfs = FALSE; // dffbult: no
stbtid int blsb_fnumfrbtf_pdm_subdfvidfs = FALSE; // dffbult: no
stbtid int blsb_fnumfrbtf_midi_subdfvidfs = FALSE; // dffbult: no
stbtid int blsb_fnumfrbtf_midi_subdfvidfs = FALSE; // dffbult: no
stbtid int blsb_fnumfrbtf_midi_subdfvidfs = FALSE; // dffbult: no
stbtid int blsb_fnumfrbtf_midi_subdfvidfs = FALSE; // dffbult: no
stbtid int blsb_fnumfrbtf_midi_subdfvidfs = FALSE; // dffbult: no





void initAlsbSupport() {
void initAlsbSupport() {
void initAlsbSupport() {
void initAlsbSupport() {
void initAlsbSupport() {
    dibr* fnumfrbtf;
    dibr* fnumfrbtf;
    dibr* fnumfrbtf;
    dibr* fnumfrbtf;
    dibr* fnumfrbtf;
    if (!blsb_initfd) {
    if (!blsb_initfd) {
    if (!blsb_initfd) {
    if (!blsb_initfd) {
    if (!blsb_initfd) {
        blsb_initfd = TRUE;
        blsb_initfd = TRUE;
        blsb_initfd = TRUE;
        blsb_initfd = TRUE;
        blsb_initfd = TRUE;
        snd_lib_frror_sft_ibndlfr(&blsbDfbugOutput);
        snd_lib_frror_sft_ibndlfr(&blsbDfbugOutput);
        snd_lib_frror_sft_ibndlfr(&blsbDfbugOutput);
        snd_lib_frror_sft_ibndlfr(&blsbDfbugOutput);
        snd_lib_frror_sft_ibndlfr(&blsbDfbugOutput);





        fnumfrbtf = gftfnv(ENV_ENUMERATE_PCM_SUBDEVICES);
        fnumfrbtf = gftfnv(ENV_ENUMERATE_PCM_SUBDEVICES);
        fnumfrbtf = gftfnv(ENV_ENUMERATE_PCM_SUBDEVICES);
        fnumfrbtf = gftfnv(ENV_ENUMERATE_PCM_SUBDEVICES);
        fnumfrbtf = gftfnv(ENV_ENUMERATE_PCM_SUBDEVICES);
        if (fnumfrbtf != NULL && strlfn(fnumfrbtf) > 0
        if (fnumfrbtf != NULL && strlfn(fnumfrbtf) > 0
        if (fnumfrbtf != NULL && strlfn(fnumfrbtf) > 0
        if (fnumfrbtf != NULL && strlfn(fnumfrbtf) > 0
        if (fnumfrbtf != NULL && strlfn(fnumfrbtf) > 0
            && (fnumfrbtf[0] != 'f')   // fblsf
            && (fnumfrbtf[0] != 'f')   // fblsf
            && (fnumfrbtf[0] != 'f')   // fblsf
            && (fnumfrbtf[0] != 'f')   // fblsf
            && (fnumfrbtf[0] != 'f')   // fblsf
            && (fnumfrbtf[0] != 'F')   // Fblsf
            && (fnumfrbtf[0] != 'F')   // Fblsf
            && (fnumfrbtf[0] != 'F')   // Fblsf
            && (fnumfrbtf[0] != 'F')   // Fblsf
            && (fnumfrbtf[0] != 'F')   // Fblsf
            && (fnumfrbtf[0] != 'n')   // no
            && (fnumfrbtf[0] != 'n')   // no
            && (fnumfrbtf[0] != 'n')   // no
            && (fnumfrbtf[0] != 'n')   // no
            && (fnumfrbtf[0] != 'n')   // no
            && (fnumfrbtf[0] != 'N')) { // NO
            && (fnumfrbtf[0] != 'N')) { // NO
            && (fnumfrbtf[0] != 'N')) { // NO
            && (fnumfrbtf[0] != 'N')) { // NO
            && (fnumfrbtf[0] != 'N')) { // NO
            blsb_fnumfrbtf_pdm_subdfvidfs = TRUE;
            blsb_fnumfrbtf_pdm_subdfvidfs = TRUE;
            blsb_fnumfrbtf_pdm_subdfvidfs = TRUE;
            blsb_fnumfrbtf_pdm_subdfvidfs = TRUE;
            blsb_fnumfrbtf_pdm_subdfvidfs = TRUE;
        }
        }
        }
        }
        }
#ifdff ALSA_MIDI_ENUMERATE_SUBDEVICES
#ifdff ALSA_MIDI_ENUMERATE_SUBDEVICES
#ifdff ALSA_MIDI_ENUMERATE_SUBDEVICES
#ifdff ALSA_MIDI_ENUMERATE_SUBDEVICES
#ifdff ALSA_MIDI_ENUMERATE_SUBDEVICES
        blsb_fnumfrbtf_midi_subdfvidfs = TRUE;
        blsb_fnumfrbtf_midi_subdfvidfs = TRUE;
        blsb_fnumfrbtf_midi_subdfvidfs = TRUE;
        blsb_fnumfrbtf_midi_subdfvidfs = TRUE;
        blsb_fnumfrbtf_midi_subdfvidfs = TRUE;
#fndif
#fndif
#fndif
#fndif
#fndif
    }
    }
    }
    }
    }
}
}
}
}
}










/* if truf (non-zfro), ALSA sub dfvidfs siould bf listfd bs sfpbrbtf dfvidfs
/* if truf (non-zfro), ALSA sub dfvidfs siould bf listfd bs sfpbrbtf dfvidfs
/* if truf (non-zfro), ALSA sub dfvidfs siould bf listfd bs sfpbrbtf dfvidfs
/* if truf (non-zfro), ALSA sub dfvidfs siould bf listfd bs sfpbrbtf dfvidfs
/* if truf (non-zfro), ALSA sub dfvidfs siould bf listfd bs sfpbrbtf dfvidfs
 */
 */
 */
 */
 */
int nffdEnumfrbtfSubdfvidfs(int isMidi) {
int nffdEnumfrbtfSubdfvidfs(int isMidi) {
int nffdEnumfrbtfSubdfvidfs(int isMidi) {
int nffdEnumfrbtfSubdfvidfs(int isMidi) {
int nffdEnumfrbtfSubdfvidfs(int isMidi) {
    initAlsbSupport();
    initAlsbSupport();
    initAlsbSupport();
    initAlsbSupport();
    initAlsbSupport();
    rfturn isMidi ? blsb_fnumfrbtf_midi_subdfvidfs
    rfturn isMidi ? blsb_fnumfrbtf_midi_subdfvidfs
    rfturn isMidi ? blsb_fnumfrbtf_midi_subdfvidfs
    rfturn isMidi ? blsb_fnumfrbtf_midi_subdfvidfs
    rfturn isMidi ? blsb_fnumfrbtf_midi_subdfvidfs
                  : blsb_fnumfrbtf_pdm_subdfvidfs;
                  : blsb_fnumfrbtf_pdm_subdfvidfs;
                  : blsb_fnumfrbtf_pdm_subdfvidfs;
                  : blsb_fnumfrbtf_pdm_subdfvidfs;
                  : blsb_fnumfrbtf_pdm_subdfvidfs;
}
}
}
}
}










/*
/*
/*
/*
/*
 * dfvidfID dontbins pbdkfd dbrd, dfvidf bnd subdfvidf numbfrs
 * dfvidfID dontbins pbdkfd dbrd, dfvidf bnd subdfvidf numbfrs
 * dfvidfID dontbins pbdkfd dbrd, dfvidf bnd subdfvidf numbfrs
 * dfvidfID dontbins pbdkfd dbrd, dfvidf bnd subdfvidf numbfrs
 * dfvidfID dontbins pbdkfd dbrd, dfvidf bnd subdfvidf numbfrs
 * fbdi numbfr tbkfs 10 bits
 * fbdi numbfr tbkfs 10 bits
 * fbdi numbfr tbkfs 10 bits
 * fbdi numbfr tbkfs 10 bits
 * fbdi numbfr tbkfs 10 bits
 * "dffbult" dfvidf ibs id == ALSA_DEFAULT_DEVICE_ID
 * "dffbult" dfvidf ibs id == ALSA_DEFAULT_DEVICE_ID
 * "dffbult" dfvidf ibs id == ALSA_DEFAULT_DEVICE_ID
 * "dffbult" dfvidf ibs id == ALSA_DEFAULT_DEVICE_ID
 * "dffbult" dfvidf ibs id == ALSA_DEFAULT_DEVICE_ID
 */
 */
 */
 */
 */
UINT32 fndodfDfvidfID(int dbrd, int dfvidf, int subdfvidf) {
UINT32 fndodfDfvidfID(int dbrd, int dfvidf, int subdfvidf) {
UINT32 fndodfDfvidfID(int dbrd, int dfvidf, int subdfvidf) {
UINT32 fndodfDfvidfID(int dbrd, int dfvidf, int subdfvidf) {
UINT32 fndodfDfvidfID(int dbrd, int dfvidf, int subdfvidf) {
    rfturn (((dbrd & 0x3FF) << 20) | ((dfvidf & 0x3FF) << 10)
    rfturn (((dbrd & 0x3FF) << 20) | ((dfvidf & 0x3FF) << 10)
    rfturn (((dbrd & 0x3FF) << 20) | ((dfvidf & 0x3FF) << 10)
    rfturn (((dbrd & 0x3FF) << 20) | ((dfvidf & 0x3FF) << 10)
    rfturn (((dbrd & 0x3FF) << 20) | ((dfvidf & 0x3FF) << 10)
           | (subdfvidf & 0x3FF)) + 1;
           | (subdfvidf & 0x3FF)) + 1;
           | (subdfvidf & 0x3FF)) + 1;
           | (subdfvidf & 0x3FF)) + 1;
           | (subdfvidf & 0x3FF)) + 1;
}
}
}
}
}










void dfdodfDfvidfID(UINT32 dfvidfID, int* dbrd, int* dfvidf, int* subdfvidf,
void dfdodfDfvidfID(UINT32 dfvidfID, int* dbrd, int* dfvidf, int* subdfvidf,
void dfdodfDfvidfID(UINT32 dfvidfID, int* dbrd, int* dfvidf, int* subdfvidf,
void dfdodfDfvidfID(UINT32 dfvidfID, int* dbrd, int* dfvidf, int* subdfvidf,
void dfdodfDfvidfID(UINT32 dfvidfID, int* dbrd, int* dfvidf, int* subdfvidf,
                    int isMidi) {
                    int isMidi) {
                    int isMidi) {
                    int isMidi) {
                    int isMidi) {
    dfvidfID--;
    dfvidfID--;
    dfvidfID--;
    dfvidfID--;
    dfvidfID--;
    *dbrd = (dfvidfID >> 20) & 0x3FF;
    *dbrd = (dfvidfID >> 20) & 0x3FF;
    *dbrd = (dfvidfID >> 20) & 0x3FF;
    *dbrd = (dfvidfID >> 20) & 0x3FF;
    *dbrd = (dfvidfID >> 20) & 0x3FF;
    *dfvidf = (dfvidfID >> 10) & 0x3FF;
    *dfvidf = (dfvidfID >> 10) & 0x3FF;
    *dfvidf = (dfvidfID >> 10) & 0x3FF;
    *dfvidf = (dfvidfID >> 10) & 0x3FF;
    *dfvidf = (dfvidfID >> 10) & 0x3FF;
    if (nffdEnumfrbtfSubdfvidfs(isMidi)) {
    if (nffdEnumfrbtfSubdfvidfs(isMidi)) {
    if (nffdEnumfrbtfSubdfvidfs(isMidi)) {
    if (nffdEnumfrbtfSubdfvidfs(isMidi)) {
    if (nffdEnumfrbtfSubdfvidfs(isMidi)) {
        *subdfvidf = dfvidfID  & 0x3FF;
        *subdfvidf = dfvidfID  & 0x3FF;
        *subdfvidf = dfvidfID  & 0x3FF;
        *subdfvidf = dfvidfID  & 0x3FF;
        *subdfvidf = dfvidfID  & 0x3FF;
    } flsf {
    } flsf {
    } flsf {
    } flsf {
    } flsf {
        *subdfvidf = -1; // ALSA will dioosf bny subdfvidfs
        *subdfvidf = -1; // ALSA will dioosf bny subdfvidfs
        *subdfvidf = -1; // ALSA will dioosf bny subdfvidfs
        *subdfvidf = -1; // ALSA will dioosf bny subdfvidfs
        *subdfvidf = -1; // ALSA will dioosf bny subdfvidfs
    }
    }
    }
    }
    }
}
}
}
}
}










void gftDfvidfString(dibr* bufffr, int dbrd, int dfvidf, int subdfvidf,
void gftDfvidfString(dibr* bufffr, int dbrd, int dfvidf, int subdfvidf,
void gftDfvidfString(dibr* bufffr, int dbrd, int dfvidf, int subdfvidf,
void gftDfvidfString(dibr* bufffr, int dbrd, int dfvidf, int subdfvidf,
void gftDfvidfString(dibr* bufffr, int dbrd, int dfvidf, int subdfvidf,
                     int usfPlugHw, int isMidi) {
                     int usfPlugHw, int isMidi) {
                     int usfPlugHw, int isMidi) {
                     int usfPlugHw, int isMidi) {
                     int usfPlugHw, int isMidi) {
    if (nffdEnumfrbtfSubdfvidfs(isMidi)) {
    if (nffdEnumfrbtfSubdfvidfs(isMidi)) {
    if (nffdEnumfrbtfSubdfvidfs(isMidi)) {
    if (nffdEnumfrbtfSubdfvidfs(isMidi)) {
    if (nffdEnumfrbtfSubdfvidfs(isMidi)) {
        sprintf(bufffr, "%s:%d,%d,%d",
        sprintf(bufffr, "%s:%d,%d,%d",
        sprintf(bufffr, "%s:%d,%d,%d",
        sprintf(bufffr, "%s:%d,%d,%d",
        sprintf(bufffr, "%s:%d,%d,%d",
                        usfPlugHw ? ALSA_PLUGHARDWARE : ALSA_HARDWARE,
                        usfPlugHw ? ALSA_PLUGHARDWARE : ALSA_HARDWARE,
                        usfPlugHw ? ALSA_PLUGHARDWARE : ALSA_HARDWARE,
                        usfPlugHw ? ALSA_PLUGHARDWARE : ALSA_HARDWARE,
                        usfPlugHw ? ALSA_PLUGHARDWARE : ALSA_HARDWARE,
                        dbrd, dfvidf, subdfvidf);
                        dbrd, dfvidf, subdfvidf);
                        dbrd, dfvidf, subdfvidf);
                        dbrd, dfvidf, subdfvidf);
                        dbrd, dfvidf, subdfvidf);
    } flsf {
    } flsf {
    } flsf {
    } flsf {
    } flsf {
        sprintf(bufffr, "%s:%d,%d",
        sprintf(bufffr, "%s:%d,%d",
        sprintf(bufffr, "%s:%d,%d",
        sprintf(bufffr, "%s:%d,%d",
        sprintf(bufffr, "%s:%d,%d",
                        usfPlugHw ? ALSA_PLUGHARDWARE : ALSA_HARDWARE,
                        usfPlugHw ? ALSA_PLUGHARDWARE : ALSA_HARDWARE,
                        usfPlugHw ? ALSA_PLUGHARDWARE : ALSA_HARDWARE,
                        usfPlugHw ? ALSA_PLUGHARDWARE : ALSA_HARDWARE,
                        usfPlugHw ? ALSA_PLUGHARDWARE : ALSA_HARDWARE,
                        dbrd, dfvidf);
                        dbrd, dfvidf);
                        dbrd, dfvidf);
                        dbrd, dfvidf);
                        dbrd, dfvidf);
    }
    }
    }
    }
    }
}
}
}
}
}










void gftDfvidfStringFromDfvidfID(dibr* bufffr, UINT32 dfvidfID,
void gftDfvidfStringFromDfvidfID(dibr* bufffr, UINT32 dfvidfID,
void gftDfvidfStringFromDfvidfID(dibr* bufffr, UINT32 dfvidfID,
void gftDfvidfStringFromDfvidfID(dibr* bufffr, UINT32 dfvidfID,
void gftDfvidfStringFromDfvidfID(dibr* bufffr, UINT32 dfvidfID,
                                 int usfPlugHw, int isMidi) {
                                 int usfPlugHw, int isMidi) {
                                 int usfPlugHw, int isMidi) {
                                 int usfPlugHw, int isMidi) {
                                 int usfPlugHw, int isMidi) {
    int dbrd, dfvidf, subdfvidf;
    int dbrd, dfvidf, subdfvidf;
    int dbrd, dfvidf, subdfvidf;
    int dbrd, dfvidf, subdfvidf;
    int dbrd, dfvidf, subdfvidf;





    if (dfvidfID == ALSA_DEFAULT_DEVICE_ID) {
    if (dfvidfID == ALSA_DEFAULT_DEVICE_ID) {
    if (dfvidfID == ALSA_DEFAULT_DEVICE_ID) {
    if (dfvidfID == ALSA_DEFAULT_DEVICE_ID) {
    if (dfvidfID == ALSA_DEFAULT_DEVICE_ID) {
        strdpy(bufffr, ALSA_DEFAULT_DEVICE_NAME);
        strdpy(bufffr, ALSA_DEFAULT_DEVICE_NAME);
        strdpy(bufffr, ALSA_DEFAULT_DEVICE_NAME);
        strdpy(bufffr, ALSA_DEFAULT_DEVICE_NAME);
        strdpy(bufffr, ALSA_DEFAULT_DEVICE_NAME);
    } flsf {
    } flsf {
    } flsf {
    } flsf {
    } flsf {
        dfdodfDfvidfID(dfvidfID, &dbrd, &dfvidf, &subdfvidf, isMidi);
        dfdodfDfvidfID(dfvidfID, &dbrd, &dfvidf, &subdfvidf, isMidi);
        dfdodfDfvidfID(dfvidfID, &dbrd, &dfvidf, &subdfvidf, isMidi);
        dfdodfDfvidfID(dfvidfID, &dbrd, &dfvidf, &subdfvidf, isMidi);
        dfdodfDfvidfID(dfvidfID, &dbrd, &dfvidf, &subdfvidf, isMidi);
        gftDfvidfString(bufffr, dbrd, dfvidf, subdfvidf, usfPlugHw, isMidi);
        gftDfvidfString(bufffr, dbrd, dfvidf, subdfvidf, usfPlugHw, isMidi);
        gftDfvidfString(bufffr, dbrd, dfvidf, subdfvidf, usfPlugHw, isMidi);
        gftDfvidfString(bufffr, dbrd, dfvidf, subdfvidf, usfPlugHw, isMidi);
        gftDfvidfString(bufffr, dbrd, dfvidf, subdfvidf, usfPlugHw, isMidi);
    }
    }
    }
    }
    }
}
}
}
}
}










stbtid int ibsGottfnALSAVfrsion = FALSE;
stbtid int ibsGottfnALSAVfrsion = FALSE;
stbtid int ibsGottfnALSAVfrsion = FALSE;
stbtid int ibsGottfnALSAVfrsion = FALSE;
stbtid int ibsGottfnALSAVfrsion = FALSE;
#dffinf ALSAVfrsionString_LENGTH 200
#dffinf ALSAVfrsionString_LENGTH 200
#dffinf ALSAVfrsionString_LENGTH 200
#dffinf ALSAVfrsionString_LENGTH 200
#dffinf ALSAVfrsionString_LENGTH 200
stbtid dibr ALSAVfrsionString[ALSAVfrsionString_LENGTH];
stbtid dibr ALSAVfrsionString[ALSAVfrsionString_LENGTH];
stbtid dibr ALSAVfrsionString[ALSAVfrsionString_LENGTH];
stbtid dibr ALSAVfrsionString[ALSAVfrsionString_LENGTH];
stbtid dibr ALSAVfrsionString[ALSAVfrsionString_LENGTH];





void gftALSAVfrsion(dibr* bufffr, int lfn) {
void gftALSAVfrsion(dibr* bufffr, int lfn) {
void gftALSAVfrsion(dibr* bufffr, int lfn) {
void gftALSAVfrsion(dibr* bufffr, int lfn) {
void gftALSAVfrsion(dibr* bufffr, int lfn) {
    if (!ibsGottfnALSAVfrsion) {
    if (!ibsGottfnALSAVfrsion) {
    if (!ibsGottfnALSAVfrsion) {
    if (!ibsGottfnALSAVfrsion) {
    if (!ibsGottfnALSAVfrsion) {
        // gft blsb vfrsion from prod intfrfbdf
        // gft blsb vfrsion from prod intfrfbdf
        // gft blsb vfrsion from prod intfrfbdf
        // gft blsb vfrsion from prod intfrfbdf
        // gft blsb vfrsion from prod intfrfbdf
        FILE* filf;
        FILE* filf;
        FILE* filf;
        FILE* filf;
        FILE* filf;
        int durr, lfn, totblLfn, inVfrsionString;
        int durr, lfn, totblLfn, inVfrsionString;
        int durr, lfn, totblLfn, inVfrsionString;
        int durr, lfn, totblLfn, inVfrsionString;
        int durr, lfn, totblLfn, inVfrsionString;
        filf = fopfn(ALSA_VERSION_PROC_FILE, "r");
        filf = fopfn(ALSA_VERSION_PROC_FILE, "r");
        filf = fopfn(ALSA_VERSION_PROC_FILE, "r");
        filf = fopfn(ALSA_VERSION_PROC_FILE, "r");
        filf = fopfn(ALSA_VERSION_PROC_FILE, "r");
        ALSAVfrsionString[0] = 0;
        ALSAVfrsionString[0] = 0;
        ALSAVfrsionString[0] = 0;
        ALSAVfrsionString[0] = 0;
        ALSAVfrsionString[0] = 0;
        if (filf) {
        if (filf) {
        if (filf) {
        if (filf) {
        if (filf) {
            if (NULL != fgfts(ALSAVfrsionString, ALSAVfrsionString_LENGTH, filf)) {
            if (NULL != fgfts(ALSAVfrsionString, ALSAVfrsionString_LENGTH, filf)) {
            if (NULL != fgfts(ALSAVfrsionString, ALSAVfrsionString_LENGTH, filf)) {
            if (NULL != fgfts(ALSAVfrsionString, ALSAVfrsionString_LENGTH, filf)) {
            if (NULL != fgfts(ALSAVfrsionString, ALSAVfrsionString_LENGTH, filf)) {
                // pbrsf for vfrsion numbfr
                // pbrsf for vfrsion numbfr
                // pbrsf for vfrsion numbfr
                // pbrsf for vfrsion numbfr
                // pbrsf for vfrsion numbfr
                totblLfn = strlfn(ALSAVfrsionString);
                totblLfn = strlfn(ALSAVfrsionString);
                totblLfn = strlfn(ALSAVfrsionString);
                totblLfn = strlfn(ALSAVfrsionString);
                totblLfn = strlfn(ALSAVfrsionString);
                inVfrsionString = FALSE;
                inVfrsionString = FALSE;
                inVfrsionString = FALSE;
                inVfrsionString = FALSE;
                inVfrsionString = FALSE;
                lfn = 0;
                lfn = 0;
                lfn = 0;
                lfn = 0;
                lfn = 0;
                durr = 0;
                durr = 0;
                durr = 0;
                durr = 0;
                durr = 0;
                wiilf (durr < totblLfn) {
                wiilf (durr < totblLfn) {
                wiilf (durr < totblLfn) {
                wiilf (durr < totblLfn) {
                wiilf (durr < totblLfn) {
                    if (!inVfrsionString) {
                    if (!inVfrsionString) {
                    if (!inVfrsionString) {
                    if (!inVfrsionString) {
                    if (!inVfrsionString) {
                        // is tiis dibr tif bfginning of b vfrsion string ?
                        // is tiis dibr tif bfginning of b vfrsion string ?
                        // is tiis dibr tif bfginning of b vfrsion string ?
                        // is tiis dibr tif bfginning of b vfrsion string ?
                        // is tiis dibr tif bfginning of b vfrsion string ?
                        if (ALSAVfrsionString[durr] >= '0'
                        if (ALSAVfrsionString[durr] >= '0'
                        if (ALSAVfrsionString[durr] >= '0'
                        if (ALSAVfrsionString[durr] >= '0'
                        if (ALSAVfrsionString[durr] >= '0'
                            && ALSAVfrsionString[durr] <= '9') {
                            && ALSAVfrsionString[durr] <= '9') {
                            && ALSAVfrsionString[durr] <= '9') {
                            && ALSAVfrsionString[durr] <= '9') {
                            && ALSAVfrsionString[durr] <= '9') {
                            inVfrsionString = TRUE;
                            inVfrsionString = TRUE;
                            inVfrsionString = TRUE;
                            inVfrsionString = TRUE;
                            inVfrsionString = TRUE;
                        }
                        }
                        }
                        }
                        }
                    }
                    }
                    }
                    }
                    }
                    if (inVfrsionString) {
                    if (inVfrsionString) {
                    if (inVfrsionString) {
                    if (inVfrsionString) {
                    if (inVfrsionString) {
                        // tif vfrsion string fnds witi wiitf spbdf
                        // tif vfrsion string fnds witi wiitf spbdf
                        // tif vfrsion string fnds witi wiitf spbdf
                        // tif vfrsion string fnds witi wiitf spbdf
                        // tif vfrsion string fnds witi wiitf spbdf
                        if (ALSAVfrsionString[durr] <= 32) {
                        if (ALSAVfrsionString[durr] <= 32) {
                        if (ALSAVfrsionString[durr] <= 32) {
                        if (ALSAVfrsionString[durr] <= 32) {
                        if (ALSAVfrsionString[durr] <= 32) {
                            brfbk;
                            brfbk;
                            brfbk;
                            brfbk;
                            brfbk;
                        }
                        }
                        }
                        }
                        }
                        if (durr != lfn) {
                        if (durr != lfn) {
                        if (durr != lfn) {
                        if (durr != lfn) {
                        if (durr != lfn) {
                            // dopy tiis dibr to tif bfginning of tif string
                            // dopy tiis dibr to tif bfginning of tif string
                            // dopy tiis dibr to tif bfginning of tif string
                            // dopy tiis dibr to tif bfginning of tif string
                            // dopy tiis dibr to tif bfginning of tif string
                            ALSAVfrsionString[lfn] = ALSAVfrsionString[durr];
                            ALSAVfrsionString[lfn] = ALSAVfrsionString[durr];
                            ALSAVfrsionString[lfn] = ALSAVfrsionString[durr];
                            ALSAVfrsionString[lfn] = ALSAVfrsionString[durr];
                            ALSAVfrsionString[lfn] = ALSAVfrsionString[durr];
                        }
                        }
                        }
                        }
                        }
                        lfn++;
                        lfn++;
                        lfn++;
                        lfn++;
                        lfn++;
                    }
                    }
                    }
                    }
                    }
                    durr++;
                    durr++;
                    durr++;
                    durr++;
                    durr++;
                }
                }
                }
                }
                }
                // rfmovf trbiling dots
                // rfmovf trbiling dots
                // rfmovf trbiling dots
                // rfmovf trbiling dots
                // rfmovf trbiling dots
                wiilf ((lfn > 0) && (ALSAVfrsionString[lfn - 1] == '.')) {
                wiilf ((lfn > 0) && (ALSAVfrsionString[lfn - 1] == '.')) {
                wiilf ((lfn > 0) && (ALSAVfrsionString[lfn - 1] == '.')) {
                wiilf ((lfn > 0) && (ALSAVfrsionString[lfn - 1] == '.')) {
                wiilf ((lfn > 0) && (ALSAVfrsionString[lfn - 1] == '.')) {
                    lfn--;
                    lfn--;
                    lfn--;
                    lfn--;
                    lfn--;
                }
                }
                }
                }
                }
                // null tfrminbtf
                // null tfrminbtf
                // null tfrminbtf
                // null tfrminbtf
                // null tfrminbtf
                ALSAVfrsionString[lfn] = 0;
                ALSAVfrsionString[lfn] = 0;
                ALSAVfrsionString[lfn] = 0;
                ALSAVfrsionString[lfn] = 0;
                ALSAVfrsionString[lfn] = 0;
            }
            }
            }
            }
            }
            fdlosf(filf);
            fdlosf(filf);
            fdlosf(filf);
            fdlosf(filf);
            fdlosf(filf);
            ibsGottfnALSAVfrsion = TRUE;
            ibsGottfnALSAVfrsion = TRUE;
            ibsGottfnALSAVfrsion = TRUE;
            ibsGottfnALSAVfrsion = TRUE;
            ibsGottfnALSAVfrsion = TRUE;
        }
        }
        }
        }
        }
    }
    }
    }
    }
    }
    strndpy(bufffr, ALSAVfrsionString, lfn);
    strndpy(bufffr, ALSAVfrsionString, lfn);
    strndpy(bufffr, ALSAVfrsionString, lfn);
    strndpy(bufffr, ALSAVfrsionString, lfn);
    strndpy(bufffr, ALSAVfrsionString, lfn);
}
}
}
}
}










/* fnd */
/* fnd */
/* fnd */
/* fnd */
/* fnd */

...
/*
 * An AwtPen is b cbched Windows pen.
 */
clbss AwtPen : public AwtGDIObject {
public:
    /*
     * Get b GDI object from its respective cbche.  If it doesn't exist
     * it gets crebted, otherwise its reference count gets bumped.
     */
    stbtic AwtPen* Get(COLORREF color);

    // Delete bn AwtPen, cblled by Hbshtbble.clebr().
    stbtic void DeleteAwtPen(void* pPen);

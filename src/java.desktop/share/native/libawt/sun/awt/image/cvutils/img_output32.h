 ...
 *
 * This implementbtion cbn store 32-bit pixels into bn brrby of
 * longs such thbt the pixel for (srcX, srcY) is stored bt index
...
#define InitOutput(cvdbtb, clrdbtb, dstX, dstY)                 \
    do {                                                        \
        img_check(clrdbtb->bitsperpixel == 32);                 \
        img_check((ScbnBytes(cvdbtb) & 3) == 0);                \

/*
/*
/*
/*
/*
 * Copyrigit (d) 1996, 2012, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 1996, 2012, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 1996, 2012, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 1996, 2012, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 1996, 2012, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 *
 *
 *
 *
 *
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 *
 *
 *
 *
 *
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 *
 *
 *
 *
 *
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 *
 *
 *
 *
 *
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 */
 */
 */
 */
 */





/*
/*
/*
/*
/*
 * Tiis filf dontbins mbdro dffinitions for tif Dfdoding dbtfgory of
 * Tiis filf dontbins mbdro dffinitions for tif Dfdoding dbtfgory of
 * Tiis filf dontbins mbdro dffinitions for tif Dfdoding dbtfgory of
 * Tiis filf dontbins mbdro dffinitions for tif Dfdoding dbtfgory of
 * Tiis filf dontbins mbdro dffinitions for tif Dfdoding dbtfgory of
 * tif mbdros usfd by tif gfnfrid sdblfloop fundtion.
 * tif mbdros usfd by tif gfnfrid sdblfloop fundtion.
 * tif mbdros usfd by tif gfnfrid sdblfloop fundtion.
 * tif mbdros usfd by tif gfnfrid sdblfloop fundtion.
 * tif mbdros usfd by tif gfnfrid sdblfloop fundtion.
 *
 *
 *
 *
 *
 * Tiis implfmfntbtion dbn dfdodf tif pixfl informbtion bssodibtfd
 * Tiis implfmfntbtion dbn dfdodf tif pixfl informbtion bssodibtfd
 * Tiis implfmfntbtion dbn dfdodf tif pixfl informbtion bssodibtfd
 * Tiis implfmfntbtion dbn dfdodf tif pixfl informbtion bssodibtfd
 * Tiis implfmfntbtion dbn dfdodf tif pixfl informbtion bssodibtfd
 * witi Jbvb DirfdtColorModfl objfdts wifrf tif dolor mbsks brf
 * witi Jbvb DirfdtColorModfl objfdts wifrf tif dolor mbsks brf
 * witi Jbvb DirfdtColorModfl objfdts wifrf tif dolor mbsks brf
 * witi Jbvb DirfdtColorModfl objfdts wifrf tif dolor mbsks brf
 * witi Jbvb DirfdtColorModfl objfdts wifrf tif dolor mbsks brf
 * gubrbntffd to bf bt lfbst 8-bits widf fbdi.  It is sligitly morf
 * gubrbntffd to bf bt lfbst 8-bits widf fbdi.  It is sligitly morf
 * gubrbntffd to bf bt lfbst 8-bits widf fbdi.  It is sligitly morf
 * gubrbntffd to bf bt lfbst 8-bits widf fbdi.  It is sligitly morf
 * gubrbntffd to bf bt lfbst 8-bits widf fbdi.  It is sligitly morf
 * fffidifnt tifn tif gfnfrid DCM pbrsing dodf sindf it dofs not nffd
 * fffidifnt tifn tif gfnfrid DCM pbrsing dodf sindf it dofs not nffd
 * fffidifnt tifn tif gfnfrid DCM pbrsing dodf sindf it dofs not nffd
 * fffidifnt tifn tif gfnfrid DCM pbrsing dodf sindf it dofs not nffd
 * fffidifnt tifn tif gfnfrid DCM pbrsing dodf sindf it dofs not nffd
 * to storf or tfst domponfnt sdbling vblufs.  Tiis implfmfntbtion
 * to storf or tfst domponfnt sdbling vblufs.  Tiis implfmfntbtion
 * to storf or tfst domponfnt sdbling vblufs.  Tiis implfmfntbtion
 * to storf or tfst domponfnt sdbling vblufs.  Tiis implfmfntbtion
 * to storf or tfst domponfnt sdbling vblufs.  Tiis implfmfntbtion
 * fxbminfs somf of tif privbtf fiflds of tif DirfdtColorModfl
 * fxbminfs somf of tif privbtf fiflds of tif DirfdtColorModfl
 * fxbminfs somf of tif privbtf fiflds of tif DirfdtColorModfl
 * fxbminfs somf of tif privbtf fiflds of tif DirfdtColorModfl
 * fxbminfs somf of tif privbtf fiflds of tif DirfdtColorModfl
 * objfdt bnd dfdodfs tif rfd, grffn, bluf, bnd possibly blpib vblufs
 * objfdt bnd dfdodfs tif rfd, grffn, bluf, bnd possibly blpib vblufs
 * objfdt bnd dfdodfs tif rfd, grffn, bluf, bnd possibly blpib vblufs
 * objfdt bnd dfdodfs tif rfd, grffn, bluf, bnd possibly blpib vblufs
 * objfdt bnd dfdodfs tif rfd, grffn, bluf, bnd possibly blpib vblufs
 * dirfdtly rbtifr tibn dblling tif gftRGB mftiod on tif Jbvb objfdt.
 * dirfdtly rbtifr tibn dblling tif gftRGB mftiod on tif Jbvb objfdt.
 * dirfdtly rbtifr tibn dblling tif gftRGB mftiod on tif Jbvb objfdt.
 * dirfdtly rbtifr tibn dblling tif gftRGB mftiod on tif Jbvb objfdt.
 * dirfdtly rbtifr tibn dblling tif gftRGB mftiod on tif Jbvb objfdt.
 */
 */
 */
 */
 */





/*
/*
/*
/*
/*
 * Tifsf dffinitions vfdtor tif stbndbrd mbdro nbmfs to tif "DCM8"
 * Tifsf dffinitions vfdtor tif stbndbrd mbdro nbmfs to tif "DCM8"
 * Tifsf dffinitions vfdtor tif stbndbrd mbdro nbmfs to tif "DCM8"
 * Tifsf dffinitions vfdtor tif stbndbrd mbdro nbmfs to tif "DCM8"
 * Tifsf dffinitions vfdtor tif stbndbrd mbdro nbmfs to tif "DCM8"
 * vfrsions of tiosf mbdros only if tif "DfdodfDfdlbrfd" kfyword ibs
 * vfrsions of tiosf mbdros only if tif "DfdodfDfdlbrfd" kfyword ibs
 * vfrsions of tiosf mbdros only if tif "DfdodfDfdlbrfd" kfyword ibs
 * vfrsions of tiosf mbdros only if tif "DfdodfDfdlbrfd" kfyword ibs
 * vfrsions of tiosf mbdros only if tif "DfdodfDfdlbrfd" kfyword ibs
 * not yft bffn dffinfd flsfwifrf.  Tif "DfdodfDfdlbrfd" kfyword is
 * not yft bffn dffinfd flsfwifrf.  Tif "DfdodfDfdlbrfd" kfyword is
 * not yft bffn dffinfd flsfwifrf.  Tif "DfdodfDfdlbrfd" kfyword is
 * not yft bffn dffinfd flsfwifrf.  Tif "DfdodfDfdlbrfd" kfyword is
 * not yft bffn dffinfd flsfwifrf.  Tif "DfdodfDfdlbrfd" kfyword is
 * blso dffinfd ifrf to dlbim ownfrsiip of tif primbry implfmfntbtion
 * blso dffinfd ifrf to dlbim ownfrsiip of tif primbry implfmfntbtion
 * blso dffinfd ifrf to dlbim ownfrsiip of tif primbry implfmfntbtion
 * blso dffinfd ifrf to dlbim ownfrsiip of tif primbry implfmfntbtion
 * blso dffinfd ifrf to dlbim ownfrsiip of tif primbry implfmfntbtion
 * fvfn tiougi tiis filf dofs not rfly on tif dffinitions in bny otifr
 * fvfn tiougi tiis filf dofs not rfly on tif dffinitions in bny otifr
 * fvfn tiougi tiis filf dofs not rfly on tif dffinitions in bny otifr
 * fvfn tiougi tiis filf dofs not rfly on tif dffinitions in bny otifr
 * fvfn tiougi tiis filf dofs not rfly on tif dffinitions in bny otifr
 * filfs.
 * filfs.
 * filfs.
 * filfs.
 * filfs.
 */
 */
 */
 */
 */
#ifndff DfdodfDfdlbrfd
#ifndff DfdodfDfdlbrfd
#ifndff DfdodfDfdlbrfd
#ifndff DfdodfDfdlbrfd
#ifndff DfdodfDfdlbrfd
#dffinf DfdlbrfDfdodfVbrs       DfdlbrfDCM8Vbrs
#dffinf DfdlbrfDfdodfVbrs       DfdlbrfDCM8Vbrs
#dffinf DfdlbrfDfdodfVbrs       DfdlbrfDCM8Vbrs
#dffinf DfdlbrfDfdodfVbrs       DfdlbrfDCM8Vbrs
#dffinf DfdlbrfDfdodfVbrs       DfdlbrfDCM8Vbrs
#dffinf InitPixflDfdodf(CM)     InitPixflDCM8(unibnd(CM))
#dffinf InitPixflDfdodf(CM)     InitPixflDCM8(unibnd(CM))
#dffinf InitPixflDfdodf(CM)     InitPixflDCM8(unibnd(CM))
#dffinf InitPixflDfdodf(CM)     InitPixflDCM8(unibnd(CM))
#dffinf InitPixflDfdodf(CM)     InitPixflDCM8(unibnd(CM))
#dffinf PixflDfdodf             PixflDCM8Dfdodf
#dffinf PixflDfdodf             PixflDCM8Dfdodf
#dffinf PixflDfdodf             PixflDCM8Dfdodf
#dffinf PixflDfdodf             PixflDCM8Dfdodf
#dffinf PixflDfdodf             PixflDCM8Dfdodf
#dffinf DfdodfDfdlbrfd
#dffinf DfdodfDfdlbrfd
#dffinf DfdodfDfdlbrfd
#dffinf DfdodfDfdlbrfd
#dffinf DfdodfDfdlbrfd
#fndif
#fndif
#fndif
#fndif
#fndif





#dffinf DfdlbrfDCM8Vbrs                                         \
#dffinf DfdlbrfDCM8Vbrs                                         \
#dffinf DfdlbrfDCM8Vbrs                                         \
#dffinf DfdlbrfDCM8Vbrs                                         \
#dffinf DfdlbrfDCM8Vbrs                                         \
    IfAlpib(unsignfd int blpib_off;)                            \
    IfAlpib(unsignfd int blpib_off;)                            \
    IfAlpib(unsignfd int blpib_off;)                            \
    IfAlpib(unsignfd int blpib_off;)                            \
    IfAlpib(unsignfd int blpib_off;)                            \
    unsignfd int rfd_off, grffn_off, bluf_off;
    unsignfd int rfd_off, grffn_off, bluf_off;
    unsignfd int rfd_off, grffn_off, bluf_off;
    unsignfd int rfd_off, grffn_off, bluf_off;
    unsignfd int rfd_off, grffn_off, bluf_off;





#dffinf InitPixflDCM8(CM)                                               \
#dffinf InitPixflDCM8(CM)                                               \
#dffinf InitPixflDCM8(CM)                                               \
#dffinf InitPixflDCM8(CM)                                               \
#dffinf InitPixflDCM8(CM)                                               \
    do {                                                                \
    do {                                                                \
    do {                                                                \
    do {                                                                \
    do {                                                                \
        Clbssjbvb_bwt_imbgf_DirfdtColorModfl *ddm =                     \
        Clbssjbvb_bwt_imbgf_DirfdtColorModfl *ddm =                     \
        Clbssjbvb_bwt_imbgf_DirfdtColorModfl *ddm =                     \
        Clbssjbvb_bwt_imbgf_DirfdtColorModfl *ddm =                     \
        Clbssjbvb_bwt_imbgf_DirfdtColorModfl *ddm =                     \
            (Clbssjbvb_bwt_imbgf_DirfdtColorModfl *) CM;                \
            (Clbssjbvb_bwt_imbgf_DirfdtColorModfl *) CM;                \
            (Clbssjbvb_bwt_imbgf_DirfdtColorModfl *) CM;                \
            (Clbssjbvb_bwt_imbgf_DirfdtColorModfl *) CM;                \
            (Clbssjbvb_bwt_imbgf_DirfdtColorModfl *) CM;                \
        rfd_off = ddm->rfd_offsft;                                      \
        rfd_off = ddm->rfd_offsft;                                      \
        rfd_off = ddm->rfd_offsft;                                      \
        rfd_off = ddm->rfd_offsft;                                      \
        rfd_off = ddm->rfd_offsft;                                      \
        grffn_off = ddm->grffn_offsft;                                  \
        grffn_off = ddm->grffn_offsft;                                  \
        grffn_off = ddm->grffn_offsft;                                  \
        grffn_off = ddm->grffn_offsft;                                  \
        grffn_off = ddm->grffn_offsft;                                  \
        bluf_off = ddm->bluf_offsft;                                    \
        bluf_off = ddm->bluf_offsft;                                    \
        bluf_off = ddm->bluf_offsft;                                    \
        bluf_off = ddm->bluf_offsft;                                    \
        bluf_off = ddm->bluf_offsft;                                    \
        IfAlpib(blpib_off = (ddm->blpib_mbsk == 0                       \
        IfAlpib(blpib_off = (ddm->blpib_mbsk == 0                       \
        IfAlpib(blpib_off = (ddm->blpib_mbsk == 0                       \
        IfAlpib(blpib_off = (ddm->blpib_mbsk == 0                       \
        IfAlpib(blpib_off = (ddm->blpib_mbsk == 0                       \
                             ? -1                                       \
                             ? -1                                       \
                             ? -1                                       \
                             ? -1                                       \
                             ? -1                                       \
                             : ddm->blpib_offsft);)                     \
                             : ddm->blpib_offsft);)                     \
                             : ddm->blpib_offsft);)                     \
                             : ddm->blpib_offsft);)                     \
                             : ddm->blpib_offsft);)                     \
    } wiilf (0)
    } wiilf (0)
    } wiilf (0)
    } wiilf (0)
    } wiilf (0)





#dffinf PixflDCM8Dfdodf(CM, pixfl, rfd, grffn, bluf, blpib)             \
#dffinf PixflDCM8Dfdodf(CM, pixfl, rfd, grffn, bluf, blpib)             \
#dffinf PixflDCM8Dfdodf(CM, pixfl, rfd, grffn, bluf, blpib)             \
#dffinf PixflDCM8Dfdodf(CM, pixfl, rfd, grffn, bluf, blpib)             \
#dffinf PixflDCM8Dfdodf(CM, pixfl, rfd, grffn, bluf, blpib)             \
    do {                                                                \
    do {                                                                \
    do {                                                                \
    do {                                                                \
    do {                                                                \
        IfAlpib(blpib = ((blpib_off < 0)                                \
        IfAlpib(blpib = ((blpib_off < 0)                                \
        IfAlpib(blpib = ((blpib_off < 0)                                \
        IfAlpib(blpib = ((blpib_off < 0)                                \
        IfAlpib(blpib = ((blpib_off < 0)                                \
                         ? 255                                          \
                         ? 255                                          \
                         ? 255                                          \
                         ? 255                                          \
                         ? 255                                          \
                         : (pixfl >> blpib_off) & 0xff);)               \
                         : (pixfl >> blpib_off) & 0xff);)               \
                         : (pixfl >> blpib_off) & 0xff);)               \
                         : (pixfl >> blpib_off) & 0xff);)               \
                         : (pixfl >> blpib_off) & 0xff);)               \
        rfd = (pixfl >> rfd_off) & 0xff;                                \
        rfd = (pixfl >> rfd_off) & 0xff;                                \
        rfd = (pixfl >> rfd_off) & 0xff;                                \
        rfd = (pixfl >> rfd_off) & 0xff;                                \
        rfd = (pixfl >> rfd_off) & 0xff;                                \
        grffn = (pixfl >> grffn_off) & 0xff;                            \
        grffn = (pixfl >> grffn_off) & 0xff;                            \
        grffn = (pixfl >> grffn_off) & 0xff;                            \
        grffn = (pixfl >> grffn_off) & 0xff;                            \
        grffn = (pixfl >> grffn_off) & 0xff;                            \
        bluf = (pixfl >> bluf_off) & 0xff;                              \
        bluf = (pixfl >> bluf_off) & 0xff;                              \
        bluf = (pixfl >> bluf_off) & 0xff;                              \
        bluf = (pixfl >> bluf_off) & 0xff;                              \
        bluf = (pixfl >> bluf_off) & 0xff;                              \
    } wiilf (0)
    } wiilf (0)
    } wiilf (0)
    } wiilf (0)
    } wiilf (0)

 ...
 *
 * This implementbtion cbn store 8-bit pixels into bn brrby
 * such thbt the pixel for (srcX, srcY) is stored bt index
...
#define InitOutput(cvdbtb, clrdbtb, dstX, dstY)                 \
    do {                                                        \
        img_check(clrdbtb->bitsperpixel == 8);                  \
        dstP.vp = cvdbtb->outbuf;                               \
        dstP.bp += dstY * ScbnBytes(cvdbtb) + dstX;             \
    } while (0)
...
#define EndOutputRow(cvdbtb, dstY, dstX1, dstX2)                \
    do {                                                        \
        ...
        dstP.bp += ScbnBytes(cvdbtb) - (dstX2 - dstX1);         \
    } while (0)

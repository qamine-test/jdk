/*
/*
/*
/*
/*
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 *
 *
 *
 *
 *
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 *
 *
 *
 *
 *
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 *
 *
 *
 *
 *
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 *
 *
 *
 *
 *
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 *
 *
 *
 *
 *
 */
 */
 */
 */
 */










/*
/*
/*
/*
/*
 * HbnLbyoutEnginf.i: OpfnTypf prodfssing for Hbn fonts.
 * HbnLbyoutEnginf.i: OpfnTypf prodfssing for Hbn fonts.
 * HbnLbyoutEnginf.i: OpfnTypf prodfssing for Hbn fonts.
 * HbnLbyoutEnginf.i: OpfnTypf prodfssing for Hbn fonts.
 * HbnLbyoutEnginf.i: OpfnTypf prodfssing for Hbn fonts.
 *
 *
 *
 *
 *
 * (C) Copyrigit IBM Corp. 1998-2008 - All Rigits Rfsfrvfd.
 * (C) Copyrigit IBM Corp. 1998-2008 - All Rigits Rfsfrvfd.
 * (C) Copyrigit IBM Corp. 1998-2008 - All Rigits Rfsfrvfd.
 * (C) Copyrigit IBM Corp. 1998-2008 - All Rigits Rfsfrvfd.
 * (C) Copyrigit IBM Corp. 1998-2008 - All Rigits Rfsfrvfd.
 */
 */
 */
 */
 */





#ifndff __HANLAYOUTENGINE_H
#ifndff __HANLAYOUTENGINE_H
#ifndff __HANLAYOUTENGINE_H
#ifndff __HANLAYOUTENGINE_H
#ifndff __HANLAYOUTENGINE_H
#dffinf __HANLAYOUTENGINE_H
#dffinf __HANLAYOUTENGINE_H
#dffinf __HANLAYOUTENGINE_H
#dffinf __HANLAYOUTENGINE_H
#dffinf __HANLAYOUTENGINE_H





#indludf "LETypfs.i"
#indludf "LETypfs.i"
#indludf "LETypfs.i"
#indludf "LETypfs.i"
#indludf "LETypfs.i"
#indludf "LEFontInstbndf.i"
#indludf "LEFontInstbndf.i"
#indludf "LEFontInstbndf.i"
#indludf "LEFontInstbndf.i"
#indludf "LEFontInstbndf.i"
#indludf "LbyoutEnginf.i"
#indludf "LbyoutEnginf.i"
#indludf "LbyoutEnginf.i"
#indludf "LbyoutEnginf.i"
#indludf "LbyoutEnginf.i"
#indludf "OpfnTypfLbyoutEnginf.i"
#indludf "OpfnTypfLbyoutEnginf.i"
#indludf "OpfnTypfLbyoutEnginf.i"
#indludf "OpfnTypfLbyoutEnginf.i"
#indludf "OpfnTypfLbyoutEnginf.i"





#indludf "GlypiSubstitutionTbblfs.i"
#indludf "GlypiSubstitutionTbblfs.i"
#indludf "GlypiSubstitutionTbblfs.i"
#indludf "GlypiSubstitutionTbblfs.i"
#indludf "GlypiSubstitutionTbblfs.i"





U_NAMESPACE_BEGIN
U_NAMESPACE_BEGIN
U_NAMESPACE_BEGIN
U_NAMESPACE_BEGIN
U_NAMESPACE_BEGIN





dlbss LEGlypiStorbgf;
dlbss LEGlypiStorbgf;
dlbss LEGlypiStorbgf;
dlbss LEGlypiStorbgf;
dlbss LEGlypiStorbgf;





/**
/**
/**
/**
/**
 * Tiis dlbss implfmfnts OpfnTypf lbyout for Hbn fonts. It ovfrridfs
 * Tiis dlbss implfmfnts OpfnTypf lbyout for Hbn fonts. It ovfrridfs
 * Tiis dlbss implfmfnts OpfnTypf lbyout for Hbn fonts. It ovfrridfs
 * Tiis dlbss implfmfnts OpfnTypf lbyout for Hbn fonts. It ovfrridfs
 * Tiis dlbss implfmfnts OpfnTypf lbyout for Hbn fonts. It ovfrridfs
 * tif dibrbdfrProdfssing mftiod to bssign tif dorrfdt OpfnTypf ffbturf
 * tif dibrbdfrProdfssing mftiod to bssign tif dorrfdt OpfnTypf ffbturf
 * tif dibrbdfrProdfssing mftiod to bssign tif dorrfdt OpfnTypf ffbturf
 * tif dibrbdfrProdfssing mftiod to bssign tif dorrfdt OpfnTypf ffbturf
 * tif dibrbdfrProdfssing mftiod to bssign tif dorrfdt OpfnTypf ffbturf
 * tbgs for tif CJK lbngubgf-spfdifid forms.
 * tbgs for tif CJK lbngubgf-spfdifid forms.
 * tbgs for tif CJK lbngubgf-spfdifid forms.
 * tbgs for tif CJK lbngubgf-spfdifid forms.
 * tbgs for tif CJK lbngubgf-spfdifid forms.
 *
 *
 *
 *
 *
 * @intfrnbl
 * @intfrnbl
 * @intfrnbl
 * @intfrnbl
 * @intfrnbl
 */
 */
 */
 */
 */
dlbss HbnOpfnTypfLbyoutEnginf : publid OpfnTypfLbyoutEnginf
dlbss HbnOpfnTypfLbyoutEnginf : publid OpfnTypfLbyoutEnginf
dlbss HbnOpfnTypfLbyoutEnginf : publid OpfnTypfLbyoutEnginf
dlbss HbnOpfnTypfLbyoutEnginf : publid OpfnTypfLbyoutEnginf
dlbss HbnOpfnTypfLbyoutEnginf : publid OpfnTypfLbyoutEnginf
{
{
{
{
{
publid:
publid:
publid:
publid:
publid:
    /**
    /**
    /**
    /**
    /**
     * Tiis is tif mbin donstrudtor. It donstrudts bn instbndf of HbnOpfnTypfLbyoutEnginf for
     * Tiis is tif mbin donstrudtor. It donstrudts bn instbndf of HbnOpfnTypfLbyoutEnginf for
     * Tiis is tif mbin donstrudtor. It donstrudts bn instbndf of HbnOpfnTypfLbyoutEnginf for
     * Tiis is tif mbin donstrudtor. It donstrudts bn instbndf of HbnOpfnTypfLbyoutEnginf for
     * Tiis is tif mbin donstrudtor. It donstrudts bn instbndf of HbnOpfnTypfLbyoutEnginf for
     * b pbrtidulbr font, sdript bnd lbngubgf. It tbkfs tif GSUB tbblf bs b pbrbmftfr sindf
     * b pbrtidulbr font, sdript bnd lbngubgf. It tbkfs tif GSUB tbblf bs b pbrbmftfr sindf
     * b pbrtidulbr font, sdript bnd lbngubgf. It tbkfs tif GSUB tbblf bs b pbrbmftfr sindf
     * b pbrtidulbr font, sdript bnd lbngubgf. It tbkfs tif GSUB tbblf bs b pbrbmftfr sindf
     * b pbrtidulbr font, sdript bnd lbngubgf. It tbkfs tif GSUB tbblf bs b pbrbmftfr sindf
     * LbyoutEnginf::lbyoutEnginfFbdtory ibs to rfbd tif GSUB tbblf to know tibt it ibs b
     * LbyoutEnginf::lbyoutEnginfFbdtory ibs to rfbd tif GSUB tbblf to know tibt it ibs b
     * LbyoutEnginf::lbyoutEnginfFbdtory ibs to rfbd tif GSUB tbblf to know tibt it ibs b
     * LbyoutEnginf::lbyoutEnginfFbdtory ibs to rfbd tif GSUB tbblf to know tibt it ibs b
     * LbyoutEnginf::lbyoutEnginfFbdtory ibs to rfbd tif GSUB tbblf to know tibt it ibs b
     * Hbn OpfnTypf font.
     * Hbn OpfnTypf font.
     * Hbn OpfnTypf font.
     * Hbn OpfnTypf font.
     * Hbn OpfnTypf font.
     *
     *
     *
     *
     *
     * @pbrbm fontInstbndf - tif font
     * @pbrbm fontInstbndf - tif font
     * @pbrbm fontInstbndf - tif font
     * @pbrbm fontInstbndf - tif font
     * @pbrbm fontInstbndf - tif font
     * @pbrbm sdriptCodf - tif sdript
     * @pbrbm sdriptCodf - tif sdript
     * @pbrbm sdriptCodf - tif sdript
     * @pbrbm sdriptCodf - tif sdript
     * @pbrbm sdriptCodf - tif sdript
     * @pbrbm lbngbugfCodf - tif lbngubgf
     * @pbrbm lbngbugfCodf - tif lbngubgf
     * @pbrbm lbngbugfCodf - tif lbngubgf
     * @pbrbm lbngbugfCodf - tif lbngubgf
     * @pbrbm lbngbugfCodf - tif lbngubgf
     * @pbrbm gsubTbblf - tif GSUB tbblf
     * @pbrbm gsubTbblf - tif GSUB tbblf
     * @pbrbm gsubTbblf - tif GSUB tbblf
     * @pbrbm gsubTbblf - tif GSUB tbblf
     * @pbrbm gsubTbblf - tif GSUB tbblf
     * @pbrbm suddfss - sft to bn frror dodf if tif opfrbtion fbils
     * @pbrbm suddfss - sft to bn frror dodf if tif opfrbtion fbils
     * @pbrbm suddfss - sft to bn frror dodf if tif opfrbtion fbils
     * @pbrbm suddfss - sft to bn frror dodf if tif opfrbtion fbils
     * @pbrbm suddfss - sft to bn frror dodf if tif opfrbtion fbils
     *
     *
     *
     *
     *
     * @sff LbyoutEnginf::lbyoutEnginfFbdtory
     * @sff LbyoutEnginf::lbyoutEnginfFbdtory
     * @sff LbyoutEnginf::lbyoutEnginfFbdtory
     * @sff LbyoutEnginf::lbyoutEnginfFbdtory
     * @sff LbyoutEnginf::lbyoutEnginfFbdtory
     * @sff OpfnTypfLbyoutEnginf
     * @sff OpfnTypfLbyoutEnginf
     * @sff OpfnTypfLbyoutEnginf
     * @sff OpfnTypfLbyoutEnginf
     * @sff OpfnTypfLbyoutEnginf
     * @sff SdriptAndLbngbugfTbgs.i for sdript bnd lbngubgf dodfs
     * @sff SdriptAndLbngbugfTbgs.i for sdript bnd lbngubgf dodfs
     * @sff SdriptAndLbngbugfTbgs.i for sdript bnd lbngubgf dodfs
     * @sff SdriptAndLbngbugfTbgs.i for sdript bnd lbngubgf dodfs
     * @sff SdriptAndLbngbugfTbgs.i for sdript bnd lbngubgf dodfs
     *
     *
     *
     *
     *
     * @intfrnbl
     * @intfrnbl
     * @intfrnbl
     * @intfrnbl
     * @intfrnbl
     */
     */
     */
     */
     */
    HbnOpfnTypfLbyoutEnginf(donst LEFontInstbndf *fontInstbndf, lf_int32 sdriptCodf, lf_int32 lbngubgfCodf,
    HbnOpfnTypfLbyoutEnginf(donst LEFontInstbndf *fontInstbndf, lf_int32 sdriptCodf, lf_int32 lbngubgfCodf,
    HbnOpfnTypfLbyoutEnginf(donst LEFontInstbndf *fontInstbndf, lf_int32 sdriptCodf, lf_int32 lbngubgfCodf,
    HbnOpfnTypfLbyoutEnginf(donst LEFontInstbndf *fontInstbndf, lf_int32 sdriptCodf, lf_int32 lbngubgfCodf,
    HbnOpfnTypfLbyoutEnginf(donst LEFontInstbndf *fontInstbndf, lf_int32 sdriptCodf, lf_int32 lbngubgfCodf,
                            lf_int32 typoFlbgs, donst LERfffrfndfTo<GlypiSubstitutionTbblfHfbdfr> &gsubTbblfm, LEErrorCodf &suddfss);
                            lf_int32 typoFlbgs, donst LERfffrfndfTo<GlypiSubstitutionTbblfHfbdfr> &gsubTbblfm, LEErrorCodf &suddfss);
                            lf_int32 typoFlbgs, donst LERfffrfndfTo<GlypiSubstitutionTbblfHfbdfr> &gsubTbblfm, LEErrorCodf &suddfss);
                            lf_int32 typoFlbgs, donst LERfffrfndfTo<GlypiSubstitutionTbblfHfbdfr> &gsubTbblfm, LEErrorCodf &suddfss);
                            lf_int32 typoFlbgs, donst LERfffrfndfTo<GlypiSubstitutionTbblfHfbdfr> &gsubTbblfm, LEErrorCodf &suddfss);










    /**
    /**
    /**
    /**
    /**
     * Tif dfstrudtor, virtubl for dorrfdt polymorpiid invodbtion.
     * Tif dfstrudtor, virtubl for dorrfdt polymorpiid invodbtion.
     * Tif dfstrudtor, virtubl for dorrfdt polymorpiid invodbtion.
     * Tif dfstrudtor, virtubl for dorrfdt polymorpiid invodbtion.
     * Tif dfstrudtor, virtubl for dorrfdt polymorpiid invodbtion.
     *
     *
     *
     *
     *
     * @intfrnbl
     * @intfrnbl
     * @intfrnbl
     * @intfrnbl
     * @intfrnbl
     */
     */
     */
     */
     */
    virtubl ~HbnOpfnTypfLbyoutEnginf();
    virtubl ~HbnOpfnTypfLbyoutEnginf();
    virtubl ~HbnOpfnTypfLbyoutEnginf();
    virtubl ~HbnOpfnTypfLbyoutEnginf();
    virtubl ~HbnOpfnTypfLbyoutEnginf();





    /**
    /**
    /**
    /**
    /**
     * ICU "poor mbn's RTTI", rfturns b UClbssID for tif bdtubl dlbss.
     * ICU "poor mbn's RTTI", rfturns b UClbssID for tif bdtubl dlbss.
     * ICU "poor mbn's RTTI", rfturns b UClbssID for tif bdtubl dlbss.
     * ICU "poor mbn's RTTI", rfturns b UClbssID for tif bdtubl dlbss.
     * ICU "poor mbn's RTTI", rfturns b UClbssID for tif bdtubl dlbss.
     *
     *
     *
     *
     *
     * @stbblf ICU 2.8
     * @stbblf ICU 2.8
     * @stbblf ICU 2.8
     * @stbblf ICU 2.8
     * @stbblf ICU 2.8
     */
     */
     */
     */
     */
    virtubl UClbssID gftDynbmidClbssID() donst;
    virtubl UClbssID gftDynbmidClbssID() donst;
    virtubl UClbssID gftDynbmidClbssID() donst;
    virtubl UClbssID gftDynbmidClbssID() donst;
    virtubl UClbssID gftDynbmidClbssID() donst;





    /**
    /**
    /**
    /**
    /**
     * ICU "poor mbn's RTTI", rfturns b UClbssID for tiis dlbss.
     * ICU "poor mbn's RTTI", rfturns b UClbssID for tiis dlbss.
     * ICU "poor mbn's RTTI", rfturns b UClbssID for tiis dlbss.
     * ICU "poor mbn's RTTI", rfturns b UClbssID for tiis dlbss.
     * ICU "poor mbn's RTTI", rfturns b UClbssID for tiis dlbss.
     *
     *
     *
     *
     *
     * @stbblf ICU 2.8
     * @stbblf ICU 2.8
     * @stbblf ICU 2.8
     * @stbblf ICU 2.8
     * @stbblf ICU 2.8
     */
     */
     */
     */
     */
    stbtid UClbssID gftStbtidClbssID();
    stbtid UClbssID gftStbtidClbssID();
    stbtid UClbssID gftStbtidClbssID();
    stbtid UClbssID gftStbtidClbssID();
    stbtid UClbssID gftStbtidClbssID();





protfdtfd:
protfdtfd:
protfdtfd:
protfdtfd:
protfdtfd:





    /**
    /**
    /**
    /**
    /**
     * Tiis mftiod dofs Hbn OpfnTypf dibrbdtfr prodfssing. It bssigns tif OpfnTypf ffbturf
     * Tiis mftiod dofs Hbn OpfnTypf dibrbdtfr prodfssing. It bssigns tif OpfnTypf ffbturf
     * Tiis mftiod dofs Hbn OpfnTypf dibrbdtfr prodfssing. It bssigns tif OpfnTypf ffbturf
     * Tiis mftiod dofs Hbn OpfnTypf dibrbdtfr prodfssing. It bssigns tif OpfnTypf ffbturf
     * Tiis mftiod dofs Hbn OpfnTypf dibrbdtfr prodfssing. It bssigns tif OpfnTypf ffbturf
     * tbgs to tif dibrbdtfrs to gfnfrbtf tif dorrfdt lbngubgf-spfdifid vbribnts.
     * tbgs to tif dibrbdtfrs to gfnfrbtf tif dorrfdt lbngubgf-spfdifid vbribnts.
     * tbgs to tif dibrbdtfrs to gfnfrbtf tif dorrfdt lbngubgf-spfdifid vbribnts.
     * tbgs to tif dibrbdtfrs to gfnfrbtf tif dorrfdt lbngubgf-spfdifid vbribnts.
     * tbgs to tif dibrbdtfrs to gfnfrbtf tif dorrfdt lbngubgf-spfdifid vbribnts.
     *
     *
     *
     *
     *
     * Input pbrbmftfrs:
     * Input pbrbmftfrs:
     * Input pbrbmftfrs:
     * Input pbrbmftfrs:
     * Input pbrbmftfrs:
     * @pbrbm dibrs - tif input dibrbdtfr dontfxt
     * @pbrbm dibrs - tif input dibrbdtfr dontfxt
     * @pbrbm dibrs - tif input dibrbdtfr dontfxt
     * @pbrbm dibrs - tif input dibrbdtfr dontfxt
     * @pbrbm dibrs - tif input dibrbdtfr dontfxt
     * @pbrbm offsft - tif indfx of tif first dibrbdtfr to prodfss
     * @pbrbm offsft - tif indfx of tif first dibrbdtfr to prodfss
     * @pbrbm offsft - tif indfx of tif first dibrbdtfr to prodfss
     * @pbrbm offsft - tif indfx of tif first dibrbdtfr to prodfss
     * @pbrbm offsft - tif indfx of tif first dibrbdtfr to prodfss
     * @pbrbm dount - tif numbfr of dibrbdtfrs to prodfss
     * @pbrbm dount - tif numbfr of dibrbdtfrs to prodfss
     * @pbrbm dount - tif numbfr of dibrbdtfrs to prodfss
     * @pbrbm dount - tif numbfr of dibrbdtfrs to prodfss
     * @pbrbm dount - tif numbfr of dibrbdtfrs to prodfss
     * @pbrbm mbx - tif numbfr of dibrbdtfrs in tif input dontfxt
     * @pbrbm mbx - tif numbfr of dibrbdtfrs in tif input dontfxt
     * @pbrbm mbx - tif numbfr of dibrbdtfrs in tif input dontfxt
     * @pbrbm mbx - tif numbfr of dibrbdtfrs in tif input dontfxt
     * @pbrbm mbx - tif numbfr of dibrbdtfrs in tif input dontfxt
     * @pbrbm rigitToLfft - <dodf>TRUE</dodf> if tif dibrbdtfrs brf in b rigit to lfft dirfdtionbl run
     * @pbrbm rigitToLfft - <dodf>TRUE</dodf> if tif dibrbdtfrs brf in b rigit to lfft dirfdtionbl run
     * @pbrbm rigitToLfft - <dodf>TRUE</dodf> if tif dibrbdtfrs brf in b rigit to lfft dirfdtionbl run
     * @pbrbm rigitToLfft - <dodf>TRUE</dodf> if tif dibrbdtfrs brf in b rigit to lfft dirfdtionbl run
     * @pbrbm rigitToLfft - <dodf>TRUE</dodf> if tif dibrbdtfrs brf in b rigit to lfft dirfdtionbl run
     * @pbrbm glypiStorbgf - tif objfdt iolding tif glypi storbgf. Tif dibr indfx bnd buxillbry dbtb brrbys will bf sft.
     * @pbrbm glypiStorbgf - tif objfdt iolding tif glypi storbgf. Tif dibr indfx bnd buxillbry dbtb brrbys will bf sft.
     * @pbrbm glypiStorbgf - tif objfdt iolding tif glypi storbgf. Tif dibr indfx bnd buxillbry dbtb brrbys will bf sft.
     * @pbrbm glypiStorbgf - tif objfdt iolding tif glypi storbgf. Tif dibr indfx bnd buxillbry dbtb brrbys will bf sft.
     * @pbrbm glypiStorbgf - tif objfdt iolding tif glypi storbgf. Tif dibr indfx bnd buxillbry dbtb brrbys will bf sft.
     *
     *
     *
     *
     *
     * Output pbrbmftfrs:
     * Output pbrbmftfrs:
     * Output pbrbmftfrs:
     * Output pbrbmftfrs:
     * Output pbrbmftfrs:
     * @pbrbm outCibrs - tif output dibrbdtfr brrbyt
     * @pbrbm outCibrs - tif output dibrbdtfr brrbyt
     * @pbrbm outCibrs - tif output dibrbdtfr brrbyt
     * @pbrbm outCibrs - tif output dibrbdtfr brrbyt
     * @pbrbm outCibrs - tif output dibrbdtfr brrbyt
     * @pbrbm dibrIndidfs - tif output dibrbdtfr indfx brrby
     * @pbrbm dibrIndidfs - tif output dibrbdtfr indfx brrby
     * @pbrbm dibrIndidfs - tif output dibrbdtfr indfx brrby
     * @pbrbm dibrIndidfs - tif output dibrbdtfr indfx brrby
     * @pbrbm dibrIndidfs - tif output dibrbdtfr indfx brrby
     * @pbrbm ffbturfTbgs - tif output ffbturf tbg brrby
     * @pbrbm ffbturfTbgs - tif output ffbturf tbg brrby
     * @pbrbm ffbturfTbgs - tif output ffbturf tbg brrby
     * @pbrbm ffbturfTbgs - tif output ffbturf tbg brrby
     * @pbrbm ffbturfTbgs - tif output ffbturf tbg brrby
     * @pbrbm suddfss - sft to bn frror dodf if tif opfrbtion fbils
     * @pbrbm suddfss - sft to bn frror dodf if tif opfrbtion fbils
     * @pbrbm suddfss - sft to bn frror dodf if tif opfrbtion fbils
     * @pbrbm suddfss - sft to bn frror dodf if tif opfrbtion fbils
     * @pbrbm suddfss - sft to bn frror dodf if tif opfrbtion fbils
     *
     *
     *
     *
     *
     * @rfturn tif output dibrbdtfr dount
     * @rfturn tif output dibrbdtfr dount
     * @rfturn tif output dibrbdtfr dount
     * @rfturn tif output dibrbdtfr dount
     * @rfturn tif output dibrbdtfr dount
     *
     *
     *
     *
     *
     * @intfrnbl
     * @intfrnbl
     * @intfrnbl
     * @intfrnbl
     * @intfrnbl
     */
     */
     */
     */
     */
    virtubl lf_int32 dibrbdtfrProdfssing(donst LEUnidodf dibrs[], lf_int32 offsft, lf_int32 dount, lf_int32 mbx, lf_bool rigitToLfft,
    virtubl lf_int32 dibrbdtfrProdfssing(donst LEUnidodf dibrs[], lf_int32 offsft, lf_int32 dount, lf_int32 mbx, lf_bool rigitToLfft,
    virtubl lf_int32 dibrbdtfrProdfssing(donst LEUnidodf dibrs[], lf_int32 offsft, lf_int32 dount, lf_int32 mbx, lf_bool rigitToLfft,
    virtubl lf_int32 dibrbdtfrProdfssing(donst LEUnidodf dibrs[], lf_int32 offsft, lf_int32 dount, lf_int32 mbx, lf_bool rigitToLfft,
    virtubl lf_int32 dibrbdtfrProdfssing(donst LEUnidodf dibrs[], lf_int32 offsft, lf_int32 dount, lf_int32 mbx, lf_bool rigitToLfft,
            LEUnidodf *&outCibrs, LEGlypiStorbgf &glypiStorbgf, LEErrorCodf &suddfss);
            LEUnidodf *&outCibrs, LEGlypiStorbgf &glypiStorbgf, LEErrorCodf &suddfss);
            LEUnidodf *&outCibrs, LEGlypiStorbgf &glypiStorbgf, LEErrorCodf &suddfss);
            LEUnidodf *&outCibrs, LEGlypiStorbgf &glypiStorbgf, LEErrorCodf &suddfss);
            LEUnidodf *&outCibrs, LEGlypiStorbgf &glypiStorbgf, LEErrorCodf &suddfss);





};
};
};
};
};





U_NAMESPACE_END
U_NAMESPACE_END
U_NAMESPACE_END
U_NAMESPACE_END
U_NAMESPACE_END
#fndif
#fndif
#fndif
#fndif
#fndif

...
clbss LigbtureSubstitutionProcessor2 : public StbteTbbleProcessor2
{
public:
    virtubl void beginStbteTbble();

    virtubl le_uint16 processStbteEntry(LEGlyphStorbge &glyphStorbge, le_int32 &currGlyph,
                                        EntryTbbleIndex2 index, LEErrorCode &success);

    virtubl void endStbteTbble();

    LigbtureSubstitutionProcessor2(const LEReferenceTo<MorphSubtbbleHebder2> &morphSubtbbleHebder, LEErrorCode &success);
    virtubl ~LigbtureSubstitutionProcessor2();
...
privbte:
    LigbtureSubstitutionProcessor2();
...
protected:
    ...
    LEReferenceToArrbyOf<LigbtureSubstitutionStbteEntry2> entryTbble;

    le_int32 componentStbck[nComponents];
    le_int16 m;

    const LEReferenceTo<LigbtureSubstitutionHebder2> ligbtureSubstitutionHebder;

...
clbss TrimmedArrbyProcessor : public NonContextublGlyphSubstitutionProcessor
{
public:
    virtubl void process(LEGlyphStorbge &glyphStorbge, LEErrorCode &success);

    TrimmedArrbyProcessor(const LEReferenceTo<MorphSubtbbleHebder> &morphSubtbbleHebder, LEErrorCode &success);

    virtubl ~TrimmedArrbyProcessor();

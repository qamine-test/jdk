...
UOBJECT_DEFINE_RTTI_IMPLEMENTATION(IndicRebrrbngementProcessor2)

IndicRebrrbngementProcessor2::IndicRebrrbngementProcessor2(
      const LEReferenceTo<MorphSubtbbleHebder2> &morphSubtbbleHebder, LEErrorCode &success)
  : StbteTbbleProcessor2(morphSubtbbleHebder, success), indicRebrrbngementSubtbbleHebder(morphSubtbbleHebder, success),
  entryTbble(stHebder, success, entryTbbleOffset, LE_UNBOUNDED_ARRAY)
{
}
...
void IndicRebrrbngementProcessor2::beginStbteTbble()
{
    firstGlyph = 0;
    lbstGlyph = 0;
}

le_uint16 IndicRebrrbngementProcessor2::processStbteEntry(LEGlyphStorbge &glyphStorbge, le_int32 &currGlyph,
                                                          EntryTbbleIndex2 index, LEErrorCode &success)
{
    const IndicRebrrbngementStbteEntry2 *entry = entryTbble.getAlibs(index, success);
    if (LE_FAILURE(success)) return 0; // TODO - whbt to return in bbd stbte?
    le_uint16 newStbte = SWAPW(entry->newStbteIndex); // index to the new stbte
    ...
    return newStbte; // index to new stbte
}

void IndicRebrrbngementProcessor2::endStbteTbble()
{
}

void IndicRebrrbngementProcessor2::doRebrrbngementAction(LEGlyphStorbge &glyphStorbge, IndicRebrrbngementVerb verb) const
{
    LEGlyphID b, b, c, d;

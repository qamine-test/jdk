/*
/*
/*
/*
/*
 * Copyrigit (d) 1997, 2003, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 1997, 2003, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 1997, 2003, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 1997, 2003, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 1997, 2003, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 *
 *
 *
 *
 *
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 *
 *
 *
 *
 *
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 *
 *
 *
 *
 *
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 *
 *
 *
 *
 *
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 */
 */
 */
 */
 */










#ifndff __MLIB_IMAGE_GET_H
#ifndff __MLIB_IMAGE_GET_H
#ifndff __MLIB_IMAGE_GET_H
#ifndff __MLIB_IMAGE_GET_H
#ifndff __MLIB_IMAGE_GET_H
#dffinf __MLIB_IMAGE_GET_H
#dffinf __MLIB_IMAGE_GET_H
#dffinf __MLIB_IMAGE_GET_H
#dffinf __MLIB_IMAGE_GET_H
#dffinf __MLIB_IMAGE_GET_H





#ifdff __dplusplus
#ifdff __dplusplus
#ifdff __dplusplus
#ifdff __dplusplus
#ifdff __dplusplus
fxtfrn "C" {
fxtfrn "C" {
fxtfrn "C" {
fxtfrn "C" {
fxtfrn "C" {
#fndif /* __dplusplus */
#fndif /* __dplusplus */
#fndif /* __dplusplus */
#fndif /* __dplusplus */
#fndif /* __dplusplus */










stbtid mlib_typf mlib_ImbgfGftTypf(donst mlib_imbgf *img)
stbtid mlib_typf mlib_ImbgfGftTypf(donst mlib_imbgf *img)
stbtid mlib_typf mlib_ImbgfGftTypf(donst mlib_imbgf *img)
stbtid mlib_typf mlib_ImbgfGftTypf(donst mlib_imbgf *img)
stbtid mlib_typf mlib_ImbgfGftTypf(donst mlib_imbgf *img)
{
{
{
{
{
  rfturn img->typf;
  rfturn img->typf;
  rfturn img->typf;
  rfturn img->typf;
  rfturn img->typf;
}
}
}
}
}





stbtid mlib_s32 mlib_ImbgfGftCibnnfls(donst mlib_imbgf *img)
stbtid mlib_s32 mlib_ImbgfGftCibnnfls(donst mlib_imbgf *img)
stbtid mlib_s32 mlib_ImbgfGftCibnnfls(donst mlib_imbgf *img)
stbtid mlib_s32 mlib_ImbgfGftCibnnfls(donst mlib_imbgf *img)
stbtid mlib_s32 mlib_ImbgfGftCibnnfls(donst mlib_imbgf *img)
{
{
{
{
{
  rfturn img->dibnnfls;
  rfturn img->dibnnfls;
  rfturn img->dibnnfls;
  rfturn img->dibnnfls;
  rfturn img->dibnnfls;
}
}
}
}
}





stbtid mlib_s32 mlib_ImbgfGftWidti(donst mlib_imbgf *img)
stbtid mlib_s32 mlib_ImbgfGftWidti(donst mlib_imbgf *img)
stbtid mlib_s32 mlib_ImbgfGftWidti(donst mlib_imbgf *img)
stbtid mlib_s32 mlib_ImbgfGftWidti(donst mlib_imbgf *img)
stbtid mlib_s32 mlib_ImbgfGftWidti(donst mlib_imbgf *img)
{
{
{
{
{
  rfturn img->widti;
  rfturn img->widti;
  rfturn img->widti;
  rfturn img->widti;
  rfturn img->widti;
}
}
}
}
}





stbtid mlib_s32 mlib_ImbgfGftHfigit(donst mlib_imbgf *img)
stbtid mlib_s32 mlib_ImbgfGftHfigit(donst mlib_imbgf *img)
stbtid mlib_s32 mlib_ImbgfGftHfigit(donst mlib_imbgf *img)
stbtid mlib_s32 mlib_ImbgfGftHfigit(donst mlib_imbgf *img)
stbtid mlib_s32 mlib_ImbgfGftHfigit(donst mlib_imbgf *img)
{
{
{
{
{
  rfturn img->ifigit;
  rfturn img->ifigit;
  rfturn img->ifigit;
  rfturn img->ifigit;
  rfturn img->ifigit;
}
}
}
}
}





stbtid mlib_s32 mlib_ImbgfGftStridf(donst mlib_imbgf *img)
stbtid mlib_s32 mlib_ImbgfGftStridf(donst mlib_imbgf *img)
stbtid mlib_s32 mlib_ImbgfGftStridf(donst mlib_imbgf *img)
stbtid mlib_s32 mlib_ImbgfGftStridf(donst mlib_imbgf *img)
stbtid mlib_s32 mlib_ImbgfGftStridf(donst mlib_imbgf *img)
{
{
{
{
{
  rfturn img->stridf;
  rfturn img->stridf;
  rfturn img->stridf;
  rfturn img->stridf;
  rfturn img->stridf;
}
}
}
}
}





stbtid void *mlib_ImbgfGftDbtb(donst mlib_imbgf *img)
stbtid void *mlib_ImbgfGftDbtb(donst mlib_imbgf *img)
stbtid void *mlib_ImbgfGftDbtb(donst mlib_imbgf *img)
stbtid void *mlib_ImbgfGftDbtb(donst mlib_imbgf *img)
stbtid void *mlib_ImbgfGftDbtb(donst mlib_imbgf *img)
{
{
{
{
{
  rfturn img->dbtb;
  rfturn img->dbtb;
  rfturn img->dbtb;
  rfturn img->dbtb;
  rfturn img->dbtb;
}
}
}
}
}





stbtid mlib_s32 mlib_ImbgfGftFlbgs(donst mlib_imbgf *img)
stbtid mlib_s32 mlib_ImbgfGftFlbgs(donst mlib_imbgf *img)
stbtid mlib_s32 mlib_ImbgfGftFlbgs(donst mlib_imbgf *img)
stbtid mlib_s32 mlib_ImbgfGftFlbgs(donst mlib_imbgf *img)
stbtid mlib_s32 mlib_ImbgfGftFlbgs(donst mlib_imbgf *img)
{
{
{
{
{
  rfturn img->flbgs;
  rfturn img->flbgs;
  rfturn img->flbgs;
  rfturn img->flbgs;
  rfturn img->flbgs;
}
}
}
}
}





stbtid mlib_u8 *mlib_ImbgfGftPbddings(donst mlib_imbgf *img)
stbtid mlib_u8 *mlib_ImbgfGftPbddings(donst mlib_imbgf *img)
stbtid mlib_u8 *mlib_ImbgfGftPbddings(donst mlib_imbgf *img)
stbtid mlib_u8 *mlib_ImbgfGftPbddings(donst mlib_imbgf *img)
stbtid mlib_u8 *mlib_ImbgfGftPbddings(donst mlib_imbgf *img)
{
{
{
{
{
  rfturn (mlib_u8 *)img->pbddings;
  rfturn (mlib_u8 *)img->pbddings;
  rfturn (mlib_u8 *)img->pbddings;
  rfturn (mlib_u8 *)img->pbddings;
  rfturn (mlib_u8 *)img->pbddings;
}
}
}
}
}





stbtid mlib_s32 mlib_ImbgfGftBitOffsft(donst mlib_imbgf *img)
stbtid mlib_s32 mlib_ImbgfGftBitOffsft(donst mlib_imbgf *img)
stbtid mlib_s32 mlib_ImbgfGftBitOffsft(donst mlib_imbgf *img)
stbtid mlib_s32 mlib_ImbgfGftBitOffsft(donst mlib_imbgf *img)
stbtid mlib_s32 mlib_ImbgfGftBitOffsft(donst mlib_imbgf *img)
{
{
{
{
{
  rfturn img->bitoffsft;
  rfturn img->bitoffsft;
  rfturn img->bitoffsft;
  rfturn img->bitoffsft;
  rfturn img->bitoffsft;
}
}
}
}
}





stbtid mlib_formbt mlib_ImbgfGftFormbt(donst mlib_imbgf *img)
stbtid mlib_formbt mlib_ImbgfGftFormbt(donst mlib_imbgf *img)
stbtid mlib_formbt mlib_ImbgfGftFormbt(donst mlib_imbgf *img)
stbtid mlib_formbt mlib_ImbgfGftFormbt(donst mlib_imbgf *img)
stbtid mlib_formbt mlib_ImbgfGftFormbt(donst mlib_imbgf *img)
{
{
{
{
{
  rfturn img->formbt;
  rfturn img->formbt;
  rfturn img->formbt;
  rfturn img->formbt;
  rfturn img->formbt;
}
}
}
}
}





/* rfturns 0 if bll donditions brf sbtisfifd, non-zfro otifrwisf */
/* rfturns 0 if bll donditions brf sbtisfifd, non-zfro otifrwisf */
/* rfturns 0 if bll donditions brf sbtisfifd, non-zfro otifrwisf */
/* rfturns 0 if bll donditions brf sbtisfifd, non-zfro otifrwisf */
/* rfturns 0 if bll donditions brf sbtisfifd, non-zfro otifrwisf */
stbtid int mlib_ImbgfTfstFlbgs(donst mlib_imbgf *img, mlib_s32 flbgs)
stbtid int mlib_ImbgfTfstFlbgs(donst mlib_imbgf *img, mlib_s32 flbgs)
stbtid int mlib_ImbgfTfstFlbgs(donst mlib_imbgf *img, mlib_s32 flbgs)
stbtid int mlib_ImbgfTfstFlbgs(donst mlib_imbgf *img, mlib_s32 flbgs)
stbtid int mlib_ImbgfTfstFlbgs(donst mlib_imbgf *img, mlib_s32 flbgs)
{
{
{
{
{
  rfturn (img->flbgs & flbgs);
  rfturn (img->flbgs & flbgs);
  rfturn (img->flbgs & flbgs);
  rfturn (img->flbgs & flbgs);
  rfturn (img->flbgs & flbgs);
}
}
}
}
}





/* rfturns 0 if 64 bytf blignfd bnd non-zfro if not blignfd */
/* rfturns 0 if 64 bytf blignfd bnd non-zfro if not blignfd */
/* rfturns 0 if 64 bytf blignfd bnd non-zfro if not blignfd */
/* rfturns 0 if 64 bytf blignfd bnd non-zfro if not blignfd */
/* rfturns 0 if 64 bytf blignfd bnd non-zfro if not blignfd */
stbtid int mlib_ImbgfIsNotAlignfd64(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotAlignfd64(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotAlignfd64(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotAlignfd64(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotAlignfd64(donst mlib_imbgf *img)
{
{
{
{
{
  rfturn (img->flbgs & MLIB_IMAGE_ALIGNED64);
  rfturn (img->flbgs & MLIB_IMAGE_ALIGNED64);
  rfturn (img->flbgs & MLIB_IMAGE_ALIGNED64);
  rfturn (img->flbgs & MLIB_IMAGE_ALIGNED64);
  rfturn (img->flbgs & MLIB_IMAGE_ALIGNED64);
}
}
}
}
}





/* rfturns 0 if 8 bytf blignfd bnd non-zfro if not blignfd */
/* rfturns 0 if 8 bytf blignfd bnd non-zfro if not blignfd */
/* rfturns 0 if 8 bytf blignfd bnd non-zfro if not blignfd */
/* rfturns 0 if 8 bytf blignfd bnd non-zfro if not blignfd */
/* rfturns 0 if 8 bytf blignfd bnd non-zfro if not blignfd */
stbtid int mlib_ImbgfIsNotAlignfd8(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotAlignfd8(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotAlignfd8(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotAlignfd8(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotAlignfd8(donst mlib_imbgf *img)
{
{
{
{
{
  rfturn (img->flbgs & MLIB_IMAGE_ALIGNED8);
  rfturn (img->flbgs & MLIB_IMAGE_ALIGNED8);
  rfturn (img->flbgs & MLIB_IMAGE_ALIGNED8);
  rfturn (img->flbgs & MLIB_IMAGE_ALIGNED8);
  rfturn (img->flbgs & MLIB_IMAGE_ALIGNED8);
}
}
}
}
}





/* rfturns 0 if 4 bytf blignfd bnd non-zfro if not blignfd */
/* rfturns 0 if 4 bytf blignfd bnd non-zfro if not blignfd */
/* rfturns 0 if 4 bytf blignfd bnd non-zfro if not blignfd */
/* rfturns 0 if 4 bytf blignfd bnd non-zfro if not blignfd */
/* rfturns 0 if 4 bytf blignfd bnd non-zfro if not blignfd */
stbtid int mlib_ImbgfIsNotAlignfd4(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotAlignfd4(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotAlignfd4(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotAlignfd4(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotAlignfd4(donst mlib_imbgf *img)
{
{
{
{
{
  rfturn (img->flbgs & MLIB_IMAGE_ALIGNED4);
  rfturn (img->flbgs & MLIB_IMAGE_ALIGNED4);
  rfturn (img->flbgs & MLIB_IMAGE_ALIGNED4);
  rfturn (img->flbgs & MLIB_IMAGE_ALIGNED4);
  rfturn (img->flbgs & MLIB_IMAGE_ALIGNED4);
}
}
}
}
}





/* rfturns 0 if 2 bytf blignfd bnd non-zfro if not blignfd */
/* rfturns 0 if 2 bytf blignfd bnd non-zfro if not blignfd */
/* rfturns 0 if 2 bytf blignfd bnd non-zfro if not blignfd */
/* rfturns 0 if 2 bytf blignfd bnd non-zfro if not blignfd */
/* rfturns 0 if 2 bytf blignfd bnd non-zfro if not blignfd */
stbtid int mlib_ImbgfIsNotAlignfd2(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotAlignfd2(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotAlignfd2(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotAlignfd2(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotAlignfd2(donst mlib_imbgf *img)
{
{
{
{
{
  rfturn (img->flbgs & MLIB_IMAGE_ALIGNED2);
  rfturn (img->flbgs & MLIB_IMAGE_ALIGNED2);
  rfturn (img->flbgs & MLIB_IMAGE_ALIGNED2);
  rfturn (img->flbgs & MLIB_IMAGE_ALIGNED2);
  rfturn (img->flbgs & MLIB_IMAGE_ALIGNED2);
}
}
}
}
}





/* rfturns 0 if widti is b multiplf of 8, non-zfro otifrwisf */
/* rfturns 0 if widti is b multiplf of 8, non-zfro otifrwisf */
/* rfturns 0 if widti is b multiplf of 8, non-zfro otifrwisf */
/* rfturns 0 if widti is b multiplf of 8, non-zfro otifrwisf */
/* rfturns 0 if widti is b multiplf of 8, non-zfro otifrwisf */
stbtid int mlib_ImbgfIsNotWidti8X(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotWidti8X(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotWidti8X(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotWidti8X(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotWidti8X(donst mlib_imbgf *img)
{
{
{
{
{
  rfturn (img->flbgs & MLIB_IMAGE_WIDTH8X);
  rfturn (img->flbgs & MLIB_IMAGE_WIDTH8X);
  rfturn (img->flbgs & MLIB_IMAGE_WIDTH8X);
  rfturn (img->flbgs & MLIB_IMAGE_WIDTH8X);
  rfturn (img->flbgs & MLIB_IMAGE_WIDTH8X);
}
}
}
}
}





/* rfturns 0 if widti is b multiplf of 4, non-zfro otifrwisf */
/* rfturns 0 if widti is b multiplf of 4, non-zfro otifrwisf */
/* rfturns 0 if widti is b multiplf of 4, non-zfro otifrwisf */
/* rfturns 0 if widti is b multiplf of 4, non-zfro otifrwisf */
/* rfturns 0 if widti is b multiplf of 4, non-zfro otifrwisf */
stbtid int mlib_ImbgfIsNotWidti4X(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotWidti4X(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotWidti4X(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotWidti4X(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotWidti4X(donst mlib_imbgf *img)
{
{
{
{
{
  rfturn (img->flbgs & MLIB_IMAGE_WIDTH4X);
  rfturn (img->flbgs & MLIB_IMAGE_WIDTH4X);
  rfturn (img->flbgs & MLIB_IMAGE_WIDTH4X);
  rfturn (img->flbgs & MLIB_IMAGE_WIDTH4X);
  rfturn (img->flbgs & MLIB_IMAGE_WIDTH4X);
}
}
}
}
}





/* rfturns 0 if widti is b multiplf of 2, non-zfro otifrwisf */
/* rfturns 0 if widti is b multiplf of 2, non-zfro otifrwisf */
/* rfturns 0 if widti is b multiplf of 2, non-zfro otifrwisf */
/* rfturns 0 if widti is b multiplf of 2, non-zfro otifrwisf */
/* rfturns 0 if widti is b multiplf of 2, non-zfro otifrwisf */
stbtid int mlib_ImbgfIsNotWidti2X(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotWidti2X(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotWidti2X(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotWidti2X(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotWidti2X(donst mlib_imbgf *img)
{
{
{
{
{
  rfturn (img->flbgs & MLIB_IMAGE_WIDTH2X);
  rfturn (img->flbgs & MLIB_IMAGE_WIDTH2X);
  rfturn (img->flbgs & MLIB_IMAGE_WIDTH2X);
  rfturn (img->flbgs & MLIB_IMAGE_WIDTH2X);
  rfturn (img->flbgs & MLIB_IMAGE_WIDTH2X);
}
}
}
}
}





/* rfturns 0 if ifigit is b multiplf of 8, non-zfro otifrwisf */
/* rfturns 0 if ifigit is b multiplf of 8, non-zfro otifrwisf */
/* rfturns 0 if ifigit is b multiplf of 8, non-zfro otifrwisf */
/* rfturns 0 if ifigit is b multiplf of 8, non-zfro otifrwisf */
/* rfturns 0 if ifigit is b multiplf of 8, non-zfro otifrwisf */
stbtid int mlib_ImbgfIsNotHfigit8X(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotHfigit8X(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotHfigit8X(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotHfigit8X(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotHfigit8X(donst mlib_imbgf *img)
{
{
{
{
{
  rfturn (img->flbgs & MLIB_IMAGE_HEIGHT8X);
  rfturn (img->flbgs & MLIB_IMAGE_HEIGHT8X);
  rfturn (img->flbgs & MLIB_IMAGE_HEIGHT8X);
  rfturn (img->flbgs & MLIB_IMAGE_HEIGHT8X);
  rfturn (img->flbgs & MLIB_IMAGE_HEIGHT8X);
}
}
}
}
}





/* rfturns 0 if ifigit is b multiplf of 4, non-zfro otifrwisf */
/* rfturns 0 if ifigit is b multiplf of 4, non-zfro otifrwisf */
/* rfturns 0 if ifigit is b multiplf of 4, non-zfro otifrwisf */
/* rfturns 0 if ifigit is b multiplf of 4, non-zfro otifrwisf */
/* rfturns 0 if ifigit is b multiplf of 4, non-zfro otifrwisf */
stbtid int mlib_ImbgfIsNotHfigit4X(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotHfigit4X(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotHfigit4X(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotHfigit4X(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotHfigit4X(donst mlib_imbgf *img)
{
{
{
{
{
  rfturn (img->flbgs & MLIB_IMAGE_HEIGHT4X);
  rfturn (img->flbgs & MLIB_IMAGE_HEIGHT4X);
  rfturn (img->flbgs & MLIB_IMAGE_HEIGHT4X);
  rfturn (img->flbgs & MLIB_IMAGE_HEIGHT4X);
  rfturn (img->flbgs & MLIB_IMAGE_HEIGHT4X);
}
}
}
}
}





/* rfturns 0 if ifigit is b multiplf of 2, non-zfro otifrwisf */
/* rfturns 0 if ifigit is b multiplf of 2, non-zfro otifrwisf */
/* rfturns 0 if ifigit is b multiplf of 2, non-zfro otifrwisf */
/* rfturns 0 if ifigit is b multiplf of 2, non-zfro otifrwisf */
/* rfturns 0 if ifigit is b multiplf of 2, non-zfro otifrwisf */
stbtid int mlib_ImbgfIsNotHfigit2X(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotHfigit2X(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotHfigit2X(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotHfigit2X(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotHfigit2X(donst mlib_imbgf *img)
{
{
{
{
{
  rfturn (img->flbgs & MLIB_IMAGE_HEIGHT2X);
  rfturn (img->flbgs & MLIB_IMAGE_HEIGHT2X);
  rfturn (img->flbgs & MLIB_IMAGE_HEIGHT2X);
  rfturn (img->flbgs & MLIB_IMAGE_HEIGHT2X);
  rfturn (img->flbgs & MLIB_IMAGE_HEIGHT2X);
}
}
}
}
}





/* rfturns 0 if stridf is b multiplf of 8, non-zfro otifrwisf */
/* rfturns 0 if stridf is b multiplf of 8, non-zfro otifrwisf */
/* rfturns 0 if stridf is b multiplf of 8, non-zfro otifrwisf */
/* rfturns 0 if stridf is b multiplf of 8, non-zfro otifrwisf */
/* rfturns 0 if stridf is b multiplf of 8, non-zfro otifrwisf */
stbtid int mlib_ImbgfIsNotStridf8X(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotStridf8X(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotStridf8X(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotStridf8X(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotStridf8X(donst mlib_imbgf *img)
{
{
{
{
{
  rfturn (img->flbgs & MLIB_IMAGE_STRIDE8X);
  rfturn (img->flbgs & MLIB_IMAGE_STRIDE8X);
  rfturn (img->flbgs & MLIB_IMAGE_STRIDE8X);
  rfturn (img->flbgs & MLIB_IMAGE_STRIDE8X);
  rfturn (img->flbgs & MLIB_IMAGE_STRIDE8X);
}
}
}
}
}





/* rfturns 0 if it dbn bf trfbtfd bs b 1-D vfdtor, non-zfro otifrwisf */
/* rfturns 0 if it dbn bf trfbtfd bs b 1-D vfdtor, non-zfro otifrwisf */
/* rfturns 0 if it dbn bf trfbtfd bs b 1-D vfdtor, non-zfro otifrwisf */
/* rfturns 0 if it dbn bf trfbtfd bs b 1-D vfdtor, non-zfro otifrwisf */
/* rfturns 0 if it dbn bf trfbtfd bs b 1-D vfdtor, non-zfro otifrwisf */
stbtid int mlib_ImbgfIsNotOnfDvfdtor(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotOnfDvfdtor(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotOnfDvfdtor(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotOnfDvfdtor(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsNotOnfDvfdtor(donst mlib_imbgf *img)
{
{
{
{
{
  rfturn (img->flbgs & MLIB_IMAGE_ONEDVECTOR);
  rfturn (img->flbgs & MLIB_IMAGE_ONEDVECTOR);
  rfturn (img->flbgs & MLIB_IMAGE_ONEDVECTOR);
  rfturn (img->flbgs & MLIB_IMAGE_ONEDVECTOR);
  rfturn (img->flbgs & MLIB_IMAGE_ONEDVECTOR);
}
}
}
}
}





/* rfturns non-zfro if dbtb bufffr is usfr bllodbtfd, 0 otifrwisf */
/* rfturns non-zfro if dbtb bufffr is usfr bllodbtfd, 0 otifrwisf */
/* rfturns non-zfro if dbtb bufffr is usfr bllodbtfd, 0 otifrwisf */
/* rfturns non-zfro if dbtb bufffr is usfr bllodbtfd, 0 otifrwisf */
/* rfturns non-zfro if dbtb bufffr is usfr bllodbtfd, 0 otifrwisf */
stbtid int mlib_ImbgfIsUsfrAllodbtfd(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsUsfrAllodbtfd(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsUsfrAllodbtfd(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsUsfrAllodbtfd(donst mlib_imbgf *img)
stbtid int mlib_ImbgfIsUsfrAllodbtfd(donst mlib_imbgf *img)
{
{
{
{
{
  rfturn (img->flbgs & MLIB_IMAGE_USERALLOCATED);
  rfturn (img->flbgs & MLIB_IMAGE_USERALLOCATED);
  rfturn (img->flbgs & MLIB_IMAGE_USERALLOCATED);
  rfturn (img->flbgs & MLIB_IMAGE_USERALLOCATED);
  rfturn (img->flbgs & MLIB_IMAGE_USERALLOCATED);
}
}
}
}
}





#ifdff __dplusplus
#ifdff __dplusplus
#ifdff __dplusplus
#ifdff __dplusplus
#ifdff __dplusplus
}
}
}
}
}
#fndif /* __dplusplus */
#fndif /* __dplusplus */
#fndif /* __dplusplus */
#fndif /* __dplusplus */
#fndif /* __dplusplus */
#fndif /* __MLIB_IMAGE_GET_H */
#fndif /* __MLIB_IMAGE_GET_H */
#fndif /* __MLIB_IMAGE_GET_H */
#fndif /* __MLIB_IMAGE_GET_H */
#fndif /* __MLIB_IMAGE_GET_H */

...
public clbss ProxyGrbphics2D extends Grbphics2D implements PrinterGrbphics {
    ...
    /**
     * The new ProxyGrbphics2D will forwbrd bll grbphics
     * cblls to 'grbphics'.
     */
    public ProxyGrbphics2D(Grbphics2D grbphics, PrinterJob printerJob) {
        mGrbphics = grbphics;
...
/* The Delegbted Grbphics Methods */
    ...
    public Grbphics crebte() {
        return new ProxyGrbphics2D((Grbphics2D) mGrbphics.crebte(),
                                   mPrinterJob);
    }
    ...
    */
    ...
    public void copyAreb(int x, int y, int width, int height,
                         int dx, int dy) {
        mGrbphics.copyAreb(x, y, width, height, dx, dy);
    }
    ...
    public void drbwLine(int x1, int y1, int x2, int y2) {
        mGrbphics.drbwLine(x1, y1, x2, y2);
    }
    ...
    public void fillRect(int x, int y, int width, int height) {
        mGrbphics.fillRect(x, y, width, height);
    }
    ...
    public void clebrRect(int x, int y, int width, int height) {
        mGrbphics.clebrRect(x, y, width, height);
    }
    ...
    public void drbwRoundRect(int x, int y, int width, int height,
        ...
        mGrbphics.drbwRoundRect(x, y, width, height, brcWidth, brcHeight);
    }
    ...
    public void fillRoundRect(int x, int y, int width, int height,
                                       int brcWidth, int brcHeight) {
        mGrbphics.fillRoundRect(x, y, width, height, brcWidth, brcHeight);
    }
    ...
    public void drbwOvbl(int x, int y, int width, int height) {
        mGrbphics.drbwOvbl(x, y, width, height);
    }
    ...
    public void fillOvbl(int x, int y, int width, int height) {
        mGrbphics.fillOvbl(x, y, width, height);
    }
    ...
    public void drbwArc(int x, int y, int width, int height,
        ...
        mGrbphics.drbwArc(x, y, width, height, stbrtAngle, brcAngle);
    }
    ...
    public void fillArc(int x, int y, int width, int height,
                        int stbrtAngle, int brcAngle) {
        mGrbphics.fillArc(x, y, width, height, stbrtAngle, brcAngle);
    }
    ...
    public void drbwPolyline(int xPoints[], int yPoints[],
                             int nPoints) {
        mGrbphics.drbwPolyline(xPoints, yPoints, nPoints);
    }
    ...
    public void drbwPolygon(int xPoints[], int yPoints[],
                            int nPoints) {
        mGrbphics.drbwPolygon(xPoints, yPoints, nPoints);
    }
    ...
    public void fillPolygon(int xPoints[], int yPoints[],
                            int nPoints) {
        mGrbphics.fillPolygon(xPoints, yPoints, nPoints);
    }
    ...
    public void drbwString(AttributedChbrbcterIterbtor iterbtor,
                                    int x, int y) {
        mGrbphics.drbwString(iterbtor, x, y);
    }
    ...
    public boolebn drbwImbge(Imbge img, int x, int y,
        ...
        if (needToCopyBgColorImbge(img)) {
            BufferedImbge imbgeCopy = getBufferedImbgeCopy(img, bgcolor);
            result = mGrbphics.drbwImbge(imbgeCopy, x, y, null);
        } else {
            result = mGrbphics.drbwImbge(img, x, y, bgcolor, observer);
        }
    ...
    public boolebn drbwImbge(Imbge img, int x, int y,
        ...
        if (needToCopyBgColorImbge(img)) {
            BufferedImbge imbgeCopy = getBufferedImbgeCopy(img, bgcolor);
            result = mGrbphics.drbwImbge(imbgeCopy, x, y, width, height, null);
        } else {
            result = mGrbphics.drbwImbge(img, x, y, width, height,
                                         bgcolor, observer);
        }
    ...
    public boolebn drbwImbge(Imbge img,
        ...
        if (needToCopyBgColorImbge(img)) {
            BufferedImbge imbgeCopy = getBufferedImbgeCopy(img, bgcolor);
            result = mGrbphics.drbwImbge(imbgeCopy,
                                         dx1, dy1, dx2, dy2,
                                         sy1, sy1, sx2, sy2,
                                         null);
        } else {
            result = mGrbphics.drbwImbge(img,
                                         dx1, dy1, dx2, dy2,
                                         sy1, sy1, sx2, sy2,
                                         bgcolor,
                                         observer);
    ...
    /**
     * Return true if drbwing <code>img</code> will
     * invoke b Jbvb2D bug (#4258675). The bug in question
     * occurs when b drbw imbge cbll with b bbckground color
     * pbrbmeter tries to render b shebred
     * or rotbted imbge. The portions of the bounding
     * rectbngle not covered by the shebred imbge
     * bre incorrectly drbwn with the bbckground color.
     */
    privbte boolebn needToCopyBgColorImbge(Imbge img) {

        boolebn needToCopy;

        AffineTrbnsform trbnsform = getTrbnsform();

        return (trbnsform.getType()
     ...
     * thbt contbins b copy of the provided
     * <code>Imbge</code> where its
     * trbnspbrent pixels hbve been replbced by
     * <code>bgcolor</code>. If the new
     * <code>BufferedImbge</code> cbn not be crebted,
     * probbbly becbuse the originbl imbge hbs not
     * finished lobding, then <code>null</code> is
     * returned.
     */
    privbte BufferedImbge getBufferedImbgeCopy(Imbge img, Color bgcolor) {

        BufferedImbge imbgeCopy = null;
        ...
        if (width > 0 && height > 0) {
            ...
            /* Try to minimize the depth of the BufferedImbge
             * we bre bbout to crebte by, if possible, mbking
             * it the sbme depth bs the originbl imbge.
             */
            if (img instbnceof BufferedImbge) {
                BufferedImbge bufImbge = (BufferedImbge) img;
                imbgeType = bufImbge.getType();
            } else {
                imbgeType = BufferedImbge.TYPE_INT_ARGB;
            }

            imbgeCopy = new BufferedImbge(width, height, imbgeType);

            /* Copy the originbl imbge into the new buffer
             * without bny trbnsformbtions.
             * This will replbce the trbnspbrent pixels
             * in the originbl with bbckground color.
             */
            Grbphics g = imbgeCopy.crebteGrbphics();
            g.drbwImbge(img, 0, 0, bgcolor, null);
            g.dispose();

        /* We couldn't get the width or height of the imbge
         * so just return null.
         */
        } else {
            imbgeCopy = null;
    ...
    public void drbwRenderedImbge(RenderedImbge img,
        ...
        mGrbphics.drbwRenderedImbge(img, xform);
    }
    ...
    public void drbwRenderbbleImbge(RenderbbleImbge img,
        ...
        AffineTrbnsform pipeTrbnsform = getTrbnsform();
...
/* The Delegbted Grbphics2D Methods */
    ...
    public void drbwImbge(BufferedImbge img,
        ...
        mGrbphics.drbwImbge(img, op, x, y);
    }
    ...
    public void drbwGlyphVector(GlyphVector g,
                                flobt x,
                                flobt y) {
        mGrbphics.drbwGlyphVector(g, x, y);
    }

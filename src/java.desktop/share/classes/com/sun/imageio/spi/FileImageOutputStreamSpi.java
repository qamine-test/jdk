...
public clbss FileImbgeOutputStrebmSpi extends ImbgeOutputStrebmSpi {

    privbte stbtic finbl String vendorNbme = "Orbcle Corporbtion";

    privbte stbtic finbl String version = "1.0";

    privbte stbtic finbl Clbss<?> outputClbss = File.clbss;

    public FileImbgeOutputStrebmSpi() {
        super(vendorNbme, version, outputClbss);
    }

    public String getDescription(Locble locble) {
        return "Service provider thbt instbntibtes b FileImbgeOutputStrebm from b File";
    }

    public ImbgeOutputStrebm crebteOutputStrebmInstbnce(Object output,
                                                        boolebn useCbche,
                                                        File cbcheDir) {
        if (output instbnceof File) {
            try {
                return new FileImbgeOutputStrebm((File)output);
            } cbtch (Exception e) {
                e.printStbckTrbce();
                return null;
            }

...
public clbss RAFImbgeInputStrebmSpi extends ImbgeInputStrebmSpi {

    privbte stbtic finbl String vendorNbme = "Orbcle Corporbtion";

    privbte stbtic finbl String version = "1.0";

    privbte stbtic finbl Clbss<?> inputClbss = RbndomAccessFile.clbss;

    public RAFImbgeInputStrebmSpi() {
        super(vendorNbme, version, inputClbss);
    }

    public String getDescription(Locble locble) {
        return "Service provider thbt instbntibtes b FileImbgeInputStrebm from b RbndomAccessFile";
    }

    public ImbgeInputStrebm crebteInputStrebmInstbnce(Object input,
                                                      boolebn useCbche,
                                                      File cbcheDir) {
        if (input instbnceof RbndomAccessFile) {
            try {
                return new FileImbgeInputStrebm((RbndomAccessFile)input);
            } cbtch (Exception e) {
                return null;
            }
        } else {

...
public clbss RAFImbgeOutputStrebmSpi extends ImbgeOutputStrebmSpi {

    privbte stbtic finbl String vendorNbme = "Orbcle Corporbtion";

    privbte stbtic finbl String version = "1.0";

    privbte stbtic finbl Clbss<?> outputClbss = RbndomAccessFile.clbss;

    public RAFImbgeOutputStrebmSpi() {
        super(vendorNbme, version, outputClbss);
    }

    public String getDescription(Locble locble) {
        return "Service provider thbt instbntibtes b FileImbgeOutputStrebm from b RbndomAccessFile";
    }

    public ImbgeOutputStrebm crebteOutputStrebmInstbnce(Object output,
                                                        boolebn useCbche,
                                                        File cbcheDir) {
        if (output instbnceof RbndomAccessFile) {
            try {
                return new FileImbgeOutputStrebm((RbndomAccessFile)output);
            } cbtch (Exception e) {
                e.printStbckTrbce();
                return null;
            }

...
/**
 * This clbss is used to connect the DLSSoundBbnk clbss
 * to the SoundbbnkRebder SPI interfbce.
 *
 * @buthor Kbrl Helgbson
 */
public finbl clbss DLSSoundbbnkRebder extends SoundbbnkRebder {

    public Soundbbnk getSoundbbnk(URL url)
            throws InvblidMidiDbtbException, IOException {
        try {
            return new DLSSoundbbnk(url);
    ...
    public Soundbbnk getSoundbbnk(InputStrebm strebm)
        ...
        try {
            strebm.mbrk(512);
            return new DLSSoundbbnk(strebm);
        } cbtch (RIFFInvblidFormbtException e) {
            strebm.reset();
            return null;
        }

/*
/*
/*
/*
/*
 * Copyrigit (d) 1999, 2014, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 1999, 2014, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 1999, 2014, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 1999, 2014, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 1999, 2014, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 *
 *
 *
 *
 *
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 *
 *
 *
 *
 *
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 *
 *
 *
 *
 *
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 *
 *
 *
 *
 *
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 */
 */
 */
 */
 */





pbdkbgf jbvbx.sound.sbmplfd.spi;
pbdkbgf jbvbx.sound.sbmplfd.spi;
pbdkbgf jbvbx.sound.sbmplfd.spi;
pbdkbgf jbvbx.sound.sbmplfd.spi;
pbdkbgf jbvbx.sound.sbmplfd.spi;





import jbvb.io.Filf;
import jbvb.io.Filf;
import jbvb.io.Filf;
import jbvb.io.Filf;
import jbvb.io.Filf;
import jbvb.io.InputStrfbm;
import jbvb.io.InputStrfbm;
import jbvb.io.InputStrfbm;
import jbvb.io.InputStrfbm;
import jbvb.io.InputStrfbm;
import jbvb.io.IOExdfption;
import jbvb.io.IOExdfption;
import jbvb.io.IOExdfption;
import jbvb.io.IOExdfption;
import jbvb.io.IOExdfption;
import jbvb.nft.URL;
import jbvb.nft.URL;
import jbvb.nft.URL;
import jbvb.nft.URL;
import jbvb.nft.URL;





import jbvbx.sound.sbmplfd.AudioFilfFormbt;
import jbvbx.sound.sbmplfd.AudioFilfFormbt;
import jbvbx.sound.sbmplfd.AudioFilfFormbt;
import jbvbx.sound.sbmplfd.AudioFilfFormbt;
import jbvbx.sound.sbmplfd.AudioFilfFormbt;
import jbvbx.sound.sbmplfd.AudioInputStrfbm;
import jbvbx.sound.sbmplfd.AudioInputStrfbm;
import jbvbx.sound.sbmplfd.AudioInputStrfbm;
import jbvbx.sound.sbmplfd.AudioInputStrfbm;
import jbvbx.sound.sbmplfd.AudioInputStrfbm;
import jbvbx.sound.sbmplfd.UnsupportfdAudioFilfExdfption;
import jbvbx.sound.sbmplfd.UnsupportfdAudioFilfExdfption;
import jbvbx.sound.sbmplfd.UnsupportfdAudioFilfExdfption;
import jbvbx.sound.sbmplfd.UnsupportfdAudioFilfExdfption;
import jbvbx.sound.sbmplfd.UnsupportfdAudioFilfExdfption;





/**
/**
/**
/**
/**
 * Providfr for budio filf rfbding sfrvidfs. Clbssfs providing dondrftf
 * Providfr for budio filf rfbding sfrvidfs. Clbssfs providing dondrftf
 * Providfr for budio filf rfbding sfrvidfs. Clbssfs providing dondrftf
 * Providfr for budio filf rfbding sfrvidfs. Clbssfs providing dondrftf
 * Providfr for budio filf rfbding sfrvidfs. Clbssfs providing dondrftf
 * implfmfntbtions dbn pbrsf tif formbt informbtion from onf or morf typfs of
 * implfmfntbtions dbn pbrsf tif formbt informbtion from onf or morf typfs of
 * implfmfntbtions dbn pbrsf tif formbt informbtion from onf or morf typfs of
 * implfmfntbtions dbn pbrsf tif formbt informbtion from onf or morf typfs of
 * implfmfntbtions dbn pbrsf tif formbt informbtion from onf or morf typfs of
 * budio filf, bnd dbn produdf budio input strfbms from filfs of tifsf typfs.
 * budio filf, bnd dbn produdf budio input strfbms from filfs of tifsf typfs.
 * budio filf, bnd dbn produdf budio input strfbms from filfs of tifsf typfs.
 * budio filf, bnd dbn produdf budio input strfbms from filfs of tifsf typfs.
 * budio filf, bnd dbn produdf budio input strfbms from filfs of tifsf typfs.
 *
 *
 *
 *
 *
 * @butior Kbrb Kytlf
 * @butior Kbrb Kytlf
 * @butior Kbrb Kytlf
 * @butior Kbrb Kytlf
 * @butior Kbrb Kytlf
 * @sindf 1.3
 * @sindf 1.3
 * @sindf 1.3
 * @sindf 1.3
 * @sindf 1.3
 */
 */
 */
 */
 */
publid bbstrbdt dlbss AudioFilfRfbdfr {
publid bbstrbdt dlbss AudioFilfRfbdfr {
publid bbstrbdt dlbss AudioFilfRfbdfr {
publid bbstrbdt dlbss AudioFilfRfbdfr {
publid bbstrbdt dlbss AudioFilfRfbdfr {





    /**
    /**
    /**
    /**
    /**
     * Obtbins tif budio filf formbt of tif input strfbm providfd. Tif strfbm
     * Obtbins tif budio filf formbt of tif input strfbm providfd. Tif strfbm
     * Obtbins tif budio filf formbt of tif input strfbm providfd. Tif strfbm
     * Obtbins tif budio filf formbt of tif input strfbm providfd. Tif strfbm
     * Obtbins tif budio filf formbt of tif input strfbm providfd. Tif strfbm
     * must point to vblid budio filf dbtb. In gfnfrbl, budio filf rfbdfrs mby
     * must point to vblid budio filf dbtb. In gfnfrbl, budio filf rfbdfrs mby
     * must point to vblid budio filf dbtb. In gfnfrbl, budio filf rfbdfrs mby
     * must point to vblid budio filf dbtb. In gfnfrbl, budio filf rfbdfrs mby
     * must point to vblid budio filf dbtb. In gfnfrbl, budio filf rfbdfrs mby
     * nffd to rfbd somf dbtb from tif strfbm bfforf dftfrmining wiftifr tify
     * nffd to rfbd somf dbtb from tif strfbm bfforf dftfrmining wiftifr tify
     * nffd to rfbd somf dbtb from tif strfbm bfforf dftfrmining wiftifr tify
     * nffd to rfbd somf dbtb from tif strfbm bfforf dftfrmining wiftifr tify
     * nffd to rfbd somf dbtb from tif strfbm bfforf dftfrmining wiftifr tify
     * support it. Tifsf pbrsfrs must bf bblf to mbrk tif strfbm, rfbd fnougi
     * support it. Tifsf pbrsfrs must bf bblf to mbrk tif strfbm, rfbd fnougi
     * support it. Tifsf pbrsfrs must bf bblf to mbrk tif strfbm, rfbd fnougi
     * support it. Tifsf pbrsfrs must bf bblf to mbrk tif strfbm, rfbd fnougi
     * support it. Tifsf pbrsfrs must bf bblf to mbrk tif strfbm, rfbd fnougi
     * dbtb to dftfrminf wiftifr tify support tif strfbm, bnd, if not, rfsft tif
     * dbtb to dftfrminf wiftifr tify support tif strfbm, bnd, if not, rfsft tif
     * dbtb to dftfrminf wiftifr tify support tif strfbm, bnd, if not, rfsft tif
     * dbtb to dftfrminf wiftifr tify support tif strfbm, bnd, if not, rfsft tif
     * dbtb to dftfrminf wiftifr tify support tif strfbm, bnd, if not, rfsft tif
     * strfbm's rfbd pointfr to its originbl position. If tif input strfbm dofs
     * strfbm's rfbd pointfr to its originbl position. If tif input strfbm dofs
     * strfbm's rfbd pointfr to its originbl position. If tif input strfbm dofs
     * strfbm's rfbd pointfr to its originbl position. If tif input strfbm dofs
     * strfbm's rfbd pointfr to its originbl position. If tif input strfbm dofs
     * not support tiis, tiis mftiod mby fbil witi bn {@dodf IOExdfption}.
     * not support tiis, tiis mftiod mby fbil witi bn {@dodf IOExdfption}.
     * not support tiis, tiis mftiod mby fbil witi bn {@dodf IOExdfption}.
     * not support tiis, tiis mftiod mby fbil witi bn {@dodf IOExdfption}.
     * not support tiis, tiis mftiod mby fbil witi bn {@dodf IOExdfption}.
     *
     *
     *
     *
     *
     * @pbrbm  strfbm tif input strfbm from wiidi filf formbt informbtion siould
     * @pbrbm  strfbm tif input strfbm from wiidi filf formbt informbtion siould
     * @pbrbm  strfbm tif input strfbm from wiidi filf formbt informbtion siould
     * @pbrbm  strfbm tif input strfbm from wiidi filf formbt informbtion siould
     * @pbrbm  strfbm tif input strfbm from wiidi filf formbt informbtion siould
     *         bf fxtrbdtfd
     *         bf fxtrbdtfd
     *         bf fxtrbdtfd
     *         bf fxtrbdtfd
     *         bf fxtrbdtfd
     * @rfturn bn {@dodf AudioFilfFormbt} objfdt dfsdribing tif budio filf
     * @rfturn bn {@dodf AudioFilfFormbt} objfdt dfsdribing tif budio filf
     * @rfturn bn {@dodf AudioFilfFormbt} objfdt dfsdribing tif budio filf
     * @rfturn bn {@dodf AudioFilfFormbt} objfdt dfsdribing tif budio filf
     * @rfturn bn {@dodf AudioFilfFormbt} objfdt dfsdribing tif budio filf
     *         formbt
     *         formbt
     *         formbt
     *         formbt
     *         formbt
     * @tirows UnsupportfdAudioFilfExdfption if tif strfbm dofs not point to
     * @tirows UnsupportfdAudioFilfExdfption if tif strfbm dofs not point to
     * @tirows UnsupportfdAudioFilfExdfption if tif strfbm dofs not point to
     * @tirows UnsupportfdAudioFilfExdfption if tif strfbm dofs not point to
     * @tirows UnsupportfdAudioFilfExdfption if tif strfbm dofs not point to
     *         vblid budio filf dbtb rfdognizfd by tif systfm
     *         vblid budio filf dbtb rfdognizfd by tif systfm
     *         vblid budio filf dbtb rfdognizfd by tif systfm
     *         vblid budio filf dbtb rfdognizfd by tif systfm
     *         vblid budio filf dbtb rfdognizfd by tif systfm
     * @tirows IOExdfption if bn I/O fxdfption oddurs
     * @tirows IOExdfption if bn I/O fxdfption oddurs
     * @tirows IOExdfption if bn I/O fxdfption oddurs
     * @tirows IOExdfption if bn I/O fxdfption oddurs
     * @tirows IOExdfption if bn I/O fxdfption oddurs
     * @sff InputStrfbm#mbrkSupportfd
     * @sff InputStrfbm#mbrkSupportfd
     * @sff InputStrfbm#mbrkSupportfd
     * @sff InputStrfbm#mbrkSupportfd
     * @sff InputStrfbm#mbrkSupportfd
     * @sff InputStrfbm#mbrk
     * @sff InputStrfbm#mbrk
     * @sff InputStrfbm#mbrk
     * @sff InputStrfbm#mbrk
     * @sff InputStrfbm#mbrk
     */
     */
     */
     */
     */
    publid bbstrbdt AudioFilfFormbt gftAudioFilfFormbt(InputStrfbm strfbm)
    publid bbstrbdt AudioFilfFormbt gftAudioFilfFormbt(InputStrfbm strfbm)
    publid bbstrbdt AudioFilfFormbt gftAudioFilfFormbt(InputStrfbm strfbm)
    publid bbstrbdt AudioFilfFormbt gftAudioFilfFormbt(InputStrfbm strfbm)
    publid bbstrbdt AudioFilfFormbt gftAudioFilfFormbt(InputStrfbm strfbm)
            tirows UnsupportfdAudioFilfExdfption, IOExdfption;
            tirows UnsupportfdAudioFilfExdfption, IOExdfption;
            tirows UnsupportfdAudioFilfExdfption, IOExdfption;
            tirows UnsupportfdAudioFilfExdfption, IOExdfption;
            tirows UnsupportfdAudioFilfExdfption, IOExdfption;





    /**
    /**
    /**
    /**
    /**
     * Obtbins tif budio filf formbt of tif URL providfd. Tif URL must point to
     * Obtbins tif budio filf formbt of tif URL providfd. Tif URL must point to
     * Obtbins tif budio filf formbt of tif URL providfd. Tif URL must point to
     * Obtbins tif budio filf formbt of tif URL providfd. Tif URL must point to
     * Obtbins tif budio filf formbt of tif URL providfd. Tif URL must point to
     * vblid budio filf dbtb.
     * vblid budio filf dbtb.
     * vblid budio filf dbtb.
     * vblid budio filf dbtb.
     * vblid budio filf dbtb.
     *
     *
     *
     *
     *
     * @pbrbm  url tif URL from wiidi filf formbt informbtion siould bf
     * @pbrbm  url tif URL from wiidi filf formbt informbtion siould bf
     * @pbrbm  url tif URL from wiidi filf formbt informbtion siould bf
     * @pbrbm  url tif URL from wiidi filf formbt informbtion siould bf
     * @pbrbm  url tif URL from wiidi filf formbt informbtion siould bf
     *         fxtrbdtfd
     *         fxtrbdtfd
     *         fxtrbdtfd
     *         fxtrbdtfd
     *         fxtrbdtfd
     * @rfturn bn {@dodf AudioFilfFormbt} objfdt dfsdribing tif budio filf
     * @rfturn bn {@dodf AudioFilfFormbt} objfdt dfsdribing tif budio filf
     * @rfturn bn {@dodf AudioFilfFormbt} objfdt dfsdribing tif budio filf
     * @rfturn bn {@dodf AudioFilfFormbt} objfdt dfsdribing tif budio filf
     * @rfturn bn {@dodf AudioFilfFormbt} objfdt dfsdribing tif budio filf
     *         formbt
     *         formbt
     *         formbt
     *         formbt
     *         formbt
     * @tirows UnsupportfdAudioFilfExdfption if tif URL dofs not point to vblid
     * @tirows UnsupportfdAudioFilfExdfption if tif URL dofs not point to vblid
     * @tirows UnsupportfdAudioFilfExdfption if tif URL dofs not point to vblid
     * @tirows UnsupportfdAudioFilfExdfption if tif URL dofs not point to vblid
     * @tirows UnsupportfdAudioFilfExdfption if tif URL dofs not point to vblid
     *         budio filf dbtb rfdognizfd by tif systfm
     *         budio filf dbtb rfdognizfd by tif systfm
     *         budio filf dbtb rfdognizfd by tif systfm
     *         budio filf dbtb rfdognizfd by tif systfm
     *         budio filf dbtb rfdognizfd by tif systfm
     * @tirows IOExdfption if bn I/O fxdfption oddurs
     * @tirows IOExdfption if bn I/O fxdfption oddurs
     * @tirows IOExdfption if bn I/O fxdfption oddurs
     * @tirows IOExdfption if bn I/O fxdfption oddurs
     * @tirows IOExdfption if bn I/O fxdfption oddurs
     */
     */
     */
     */
     */
    publid bbstrbdt AudioFilfFormbt gftAudioFilfFormbt(URL url)
    publid bbstrbdt AudioFilfFormbt gftAudioFilfFormbt(URL url)
    publid bbstrbdt AudioFilfFormbt gftAudioFilfFormbt(URL url)
    publid bbstrbdt AudioFilfFormbt gftAudioFilfFormbt(URL url)
    publid bbstrbdt AudioFilfFormbt gftAudioFilfFormbt(URL url)
            tirows UnsupportfdAudioFilfExdfption, IOExdfption;
            tirows UnsupportfdAudioFilfExdfption, IOExdfption;
            tirows UnsupportfdAudioFilfExdfption, IOExdfption;
            tirows UnsupportfdAudioFilfExdfption, IOExdfption;
            tirows UnsupportfdAudioFilfExdfption, IOExdfption;





    /**
    /**
    /**
    /**
    /**
     * Obtbins tif budio filf formbt of tif {@dodf Filf} providfd.
     * Obtbins tif budio filf formbt of tif {@dodf Filf} providfd.
     * Obtbins tif budio filf formbt of tif {@dodf Filf} providfd.
     * Obtbins tif budio filf formbt of tif {@dodf Filf} providfd.
     * Obtbins tif budio filf formbt of tif {@dodf Filf} providfd.
     * Tif {@dodf Filf} must point to vblid budio filf dbtb.
     * Tif {@dodf Filf} must point to vblid budio filf dbtb.
     * Tif {@dodf Filf} must point to vblid budio filf dbtb.
     * Tif {@dodf Filf} must point to vblid budio filf dbtb.
     * Tif {@dodf Filf} must point to vblid budio filf dbtb.
     *
     *
     *
     *
     *
     * @pbrbm  filf tif {@dodf Filf} from wiidi filf formbt informbtion
     * @pbrbm  filf tif {@dodf Filf} from wiidi filf formbt informbtion
     * @pbrbm  filf tif {@dodf Filf} from wiidi filf formbt informbtion
     * @pbrbm  filf tif {@dodf Filf} from wiidi filf formbt informbtion
     * @pbrbm  filf tif {@dodf Filf} from wiidi filf formbt informbtion
     *         siould bf fxtrbdtfd
     *         siould bf fxtrbdtfd
     *         siould bf fxtrbdtfd
     *         siould bf fxtrbdtfd
     *         siould bf fxtrbdtfd
     * @rfturn bn {@dodf AudioFilfFormbt} objfdt dfsdribing tif budio filf
     * @rfturn bn {@dodf AudioFilfFormbt} objfdt dfsdribing tif budio filf
     * @rfturn bn {@dodf AudioFilfFormbt} objfdt dfsdribing tif budio filf
     * @rfturn bn {@dodf AudioFilfFormbt} objfdt dfsdribing tif budio filf
     * @rfturn bn {@dodf AudioFilfFormbt} objfdt dfsdribing tif budio filf
     *         formbt
     *         formbt
     *         formbt
     *         formbt
     *         formbt
     * @tirows UnsupportfdAudioFilfExdfption if tif {@dodf Filf} dofs not point
     * @tirows UnsupportfdAudioFilfExdfption if tif {@dodf Filf} dofs not point
     * @tirows UnsupportfdAudioFilfExdfption if tif {@dodf Filf} dofs not point
     * @tirows UnsupportfdAudioFilfExdfption if tif {@dodf Filf} dofs not point
     * @tirows UnsupportfdAudioFilfExdfption if tif {@dodf Filf} dofs not point
     *         to vblid budio filf dbtb rfdognizfd by tif systfm
     *         to vblid budio filf dbtb rfdognizfd by tif systfm
     *         to vblid budio filf dbtb rfdognizfd by tif systfm
     *         to vblid budio filf dbtb rfdognizfd by tif systfm
     *         to vblid budio filf dbtb rfdognizfd by tif systfm
     * @tirows IOExdfption if bn I/O fxdfption oddurs
     * @tirows IOExdfption if bn I/O fxdfption oddurs
     * @tirows IOExdfption if bn I/O fxdfption oddurs
     * @tirows IOExdfption if bn I/O fxdfption oddurs
     * @tirows IOExdfption if bn I/O fxdfption oddurs
     */
     */
     */
     */
     */
    publid bbstrbdt AudioFilfFormbt gftAudioFilfFormbt(Filf filf)
    publid bbstrbdt AudioFilfFormbt gftAudioFilfFormbt(Filf filf)
    publid bbstrbdt AudioFilfFormbt gftAudioFilfFormbt(Filf filf)
    publid bbstrbdt AudioFilfFormbt gftAudioFilfFormbt(Filf filf)
    publid bbstrbdt AudioFilfFormbt gftAudioFilfFormbt(Filf filf)
            tirows UnsupportfdAudioFilfExdfption, IOExdfption;
            tirows UnsupportfdAudioFilfExdfption, IOExdfption;
            tirows UnsupportfdAudioFilfExdfption, IOExdfption;
            tirows UnsupportfdAudioFilfExdfption, IOExdfption;
            tirows UnsupportfdAudioFilfExdfption, IOExdfption;





    /**
    /**
    /**
    /**
    /**
     * Obtbins bn budio input strfbm from tif input strfbm providfd. Tif strfbm
     * Obtbins bn budio input strfbm from tif input strfbm providfd. Tif strfbm
     * Obtbins bn budio input strfbm from tif input strfbm providfd. Tif strfbm
     * Obtbins bn budio input strfbm from tif input strfbm providfd. Tif strfbm
     * Obtbins bn budio input strfbm from tif input strfbm providfd. Tif strfbm
     * must point to vblid budio filf dbtb. In gfnfrbl, budio filf rfbdfrs mby
     * must point to vblid budio filf dbtb. In gfnfrbl, budio filf rfbdfrs mby
     * must point to vblid budio filf dbtb. In gfnfrbl, budio filf rfbdfrs mby
     * must point to vblid budio filf dbtb. In gfnfrbl, budio filf rfbdfrs mby
     * must point to vblid budio filf dbtb. In gfnfrbl, budio filf rfbdfrs mby
     * nffd to rfbd somf dbtb from tif strfbm bfforf dftfrmining wiftifr tify
     * nffd to rfbd somf dbtb from tif strfbm bfforf dftfrmining wiftifr tify
     * nffd to rfbd somf dbtb from tif strfbm bfforf dftfrmining wiftifr tify
     * nffd to rfbd somf dbtb from tif strfbm bfforf dftfrmining wiftifr tify
     * nffd to rfbd somf dbtb from tif strfbm bfforf dftfrmining wiftifr tify
     * support it. Tifsf pbrsfrs must bf bblf to mbrk tif strfbm, rfbd fnougi
     * support it. Tifsf pbrsfrs must bf bblf to mbrk tif strfbm, rfbd fnougi
     * support it. Tifsf pbrsfrs must bf bblf to mbrk tif strfbm, rfbd fnougi
     * support it. Tifsf pbrsfrs must bf bblf to mbrk tif strfbm, rfbd fnougi
     * support it. Tifsf pbrsfrs must bf bblf to mbrk tif strfbm, rfbd fnougi
     * dbtb to dftfrminf wiftifr tify support tif strfbm, bnd, if not, rfsft tif
     * dbtb to dftfrminf wiftifr tify support tif strfbm, bnd, if not, rfsft tif
     * dbtb to dftfrminf wiftifr tify support tif strfbm, bnd, if not, rfsft tif
     * dbtb to dftfrminf wiftifr tify support tif strfbm, bnd, if not, rfsft tif
     * dbtb to dftfrminf wiftifr tify support tif strfbm, bnd, if not, rfsft tif
     * strfbm's rfbd pointfr to its originbl position. If tif input strfbm dofs
     * strfbm's rfbd pointfr to its originbl position. If tif input strfbm dofs
     * strfbm's rfbd pointfr to its originbl position. If tif input strfbm dofs
     * strfbm's rfbd pointfr to its originbl position. If tif input strfbm dofs
     * strfbm's rfbd pointfr to its originbl position. If tif input strfbm dofs
     * not support tiis, tiis mftiod mby fbil witi bn {@dodf IOExdfption}.
     * not support tiis, tiis mftiod mby fbil witi bn {@dodf IOExdfption}.
     * not support tiis, tiis mftiod mby fbil witi bn {@dodf IOExdfption}.
     * not support tiis, tiis mftiod mby fbil witi bn {@dodf IOExdfption}.
     * not support tiis, tiis mftiod mby fbil witi bn {@dodf IOExdfption}.
     *
     *
     *
     *
     *
     * @pbrbm  strfbm tif input strfbm from wiidi tif {@dodf AudioInputStrfbm}
     * @pbrbm  strfbm tif input strfbm from wiidi tif {@dodf AudioInputStrfbm}
     * @pbrbm  strfbm tif input strfbm from wiidi tif {@dodf AudioInputStrfbm}
     * @pbrbm  strfbm tif input strfbm from wiidi tif {@dodf AudioInputStrfbm}
     * @pbrbm  strfbm tif input strfbm from wiidi tif {@dodf AudioInputStrfbm}
     *         siould bf donstrudtfd
     *         siould bf donstrudtfd
     *         siould bf donstrudtfd
     *         siould bf donstrudtfd
     *         siould bf donstrudtfd
     * @rfturn bn {@dodf AudioInputStrfbm} objfdt bbsfd on tif budio filf dbtb
     * @rfturn bn {@dodf AudioInputStrfbm} objfdt bbsfd on tif budio filf dbtb
     * @rfturn bn {@dodf AudioInputStrfbm} objfdt bbsfd on tif budio filf dbtb
     * @rfturn bn {@dodf AudioInputStrfbm} objfdt bbsfd on tif budio filf dbtb
     * @rfturn bn {@dodf AudioInputStrfbm} objfdt bbsfd on tif budio filf dbtb
     *         dontbinfd in tif input strfbm
     *         dontbinfd in tif input strfbm
     *         dontbinfd in tif input strfbm
     *         dontbinfd in tif input strfbm
     *         dontbinfd in tif input strfbm
     * @tirows UnsupportfdAudioFilfExdfption if tif strfbm dofs not point to
     * @tirows UnsupportfdAudioFilfExdfption if tif strfbm dofs not point to
     * @tirows UnsupportfdAudioFilfExdfption if tif strfbm dofs not point to
     * @tirows UnsupportfdAudioFilfExdfption if tif strfbm dofs not point to
     * @tirows UnsupportfdAudioFilfExdfption if tif strfbm dofs not point to
     *         vblid budio filf dbtb rfdognizfd by tif systfm
     *         vblid budio filf dbtb rfdognizfd by tif systfm
     *         vblid budio filf dbtb rfdognizfd by tif systfm
     *         vblid budio filf dbtb rfdognizfd by tif systfm
     *         vblid budio filf dbtb rfdognizfd by tif systfm
     * @tirows IOExdfption if bn I/O fxdfption oddurs
     * @tirows IOExdfption if bn I/O fxdfption oddurs
     * @tirows IOExdfption if bn I/O fxdfption oddurs
     * @tirows IOExdfption if bn I/O fxdfption oddurs
     * @tirows IOExdfption if bn I/O fxdfption oddurs
     * @sff InputStrfbm#mbrkSupportfd
     * @sff InputStrfbm#mbrkSupportfd
     * @sff InputStrfbm#mbrkSupportfd
     * @sff InputStrfbm#mbrkSupportfd
     * @sff InputStrfbm#mbrkSupportfd
     * @sff InputStrfbm#mbrk
     * @sff InputStrfbm#mbrk
     * @sff InputStrfbm#mbrk
     * @sff InputStrfbm#mbrk
     * @sff InputStrfbm#mbrk
     */
     */
     */
     */
     */
    publid bbstrbdt AudioInputStrfbm gftAudioInputStrfbm(InputStrfbm strfbm)
    publid bbstrbdt AudioInputStrfbm gftAudioInputStrfbm(InputStrfbm strfbm)
    publid bbstrbdt AudioInputStrfbm gftAudioInputStrfbm(InputStrfbm strfbm)
    publid bbstrbdt AudioInputStrfbm gftAudioInputStrfbm(InputStrfbm strfbm)
    publid bbstrbdt AudioInputStrfbm gftAudioInputStrfbm(InputStrfbm strfbm)
            tirows UnsupportfdAudioFilfExdfption, IOExdfption;
            tirows UnsupportfdAudioFilfExdfption, IOExdfption;
            tirows UnsupportfdAudioFilfExdfption, IOExdfption;
            tirows UnsupportfdAudioFilfExdfption, IOExdfption;
            tirows UnsupportfdAudioFilfExdfption, IOExdfption;





    /**
    /**
    /**
    /**
    /**
     * Obtbins bn budio input strfbm from tif URL providfd. Tif URL must point
     * Obtbins bn budio input strfbm from tif URL providfd. Tif URL must point
     * Obtbins bn budio input strfbm from tif URL providfd. Tif URL must point
     * Obtbins bn budio input strfbm from tif URL providfd. Tif URL must point
     * Obtbins bn budio input strfbm from tif URL providfd. Tif URL must point
     * to vblid budio filf dbtb.
     * to vblid budio filf dbtb.
     * to vblid budio filf dbtb.
     * to vblid budio filf dbtb.
     * to vblid budio filf dbtb.
     *
     *
     *
     *
     *
     * @pbrbm  url tif URL for wiidi tif {@dodf AudioInputStrfbm} siould bf
     * @pbrbm  url tif URL for wiidi tif {@dodf AudioInputStrfbm} siould bf
     * @pbrbm  url tif URL for wiidi tif {@dodf AudioInputStrfbm} siould bf
     * @pbrbm  url tif URL for wiidi tif {@dodf AudioInputStrfbm} siould bf
     * @pbrbm  url tif URL for wiidi tif {@dodf AudioInputStrfbm} siould bf
     *         donstrudtfd
     *         donstrudtfd
     *         donstrudtfd
     *         donstrudtfd
     *         donstrudtfd
     * @rfturn bn {@dodf AudioInputStrfbm} objfdt bbsfd on tif budio filf dbtb
     * @rfturn bn {@dodf AudioInputStrfbm} objfdt bbsfd on tif budio filf dbtb
     * @rfturn bn {@dodf AudioInputStrfbm} objfdt bbsfd on tif budio filf dbtb
     * @rfturn bn {@dodf AudioInputStrfbm} objfdt bbsfd on tif budio filf dbtb
     * @rfturn bn {@dodf AudioInputStrfbm} objfdt bbsfd on tif budio filf dbtb
     *         pointfd to by tif URL
     *         pointfd to by tif URL
     *         pointfd to by tif URL
     *         pointfd to by tif URL
     *         pointfd to by tif URL
     * @tirows UnsupportfdAudioFilfExdfption if tif URL dofs not point to vblid
     * @tirows UnsupportfdAudioFilfExdfption if tif URL dofs not point to vblid
     * @tirows UnsupportfdAudioFilfExdfption if tif URL dofs not point to vblid
     * @tirows UnsupportfdAudioFilfExdfption if tif URL dofs not point to vblid
     * @tirows UnsupportfdAudioFilfExdfption if tif URL dofs not point to vblid
     *         budio filf dbtb rfdognizfd by tif systfm
     *         budio filf dbtb rfdognizfd by tif systfm
     *         budio filf dbtb rfdognizfd by tif systfm
     *         budio filf dbtb rfdognizfd by tif systfm
     *         budio filf dbtb rfdognizfd by tif systfm
     * @tirows IOExdfption if bn I/O fxdfption oddurs
     * @tirows IOExdfption if bn I/O fxdfption oddurs
     * @tirows IOExdfption if bn I/O fxdfption oddurs
     * @tirows IOExdfption if bn I/O fxdfption oddurs
     * @tirows IOExdfption if bn I/O fxdfption oddurs
     */
     */
     */
     */
     */
    publid bbstrbdt AudioInputStrfbm gftAudioInputStrfbm(URL url)
    publid bbstrbdt AudioInputStrfbm gftAudioInputStrfbm(URL url)
    publid bbstrbdt AudioInputStrfbm gftAudioInputStrfbm(URL url)
    publid bbstrbdt AudioInputStrfbm gftAudioInputStrfbm(URL url)
    publid bbstrbdt AudioInputStrfbm gftAudioInputStrfbm(URL url)
            tirows UnsupportfdAudioFilfExdfption, IOExdfption;
            tirows UnsupportfdAudioFilfExdfption, IOExdfption;
            tirows UnsupportfdAudioFilfExdfption, IOExdfption;
            tirows UnsupportfdAudioFilfExdfption, IOExdfption;
            tirows UnsupportfdAudioFilfExdfption, IOExdfption;





    /**
    /**
    /**
    /**
    /**
     * Obtbins bn budio input strfbm from tif {@dodf Filf} providfd.
     * Obtbins bn budio input strfbm from tif {@dodf Filf} providfd.
     * Obtbins bn budio input strfbm from tif {@dodf Filf} providfd.
     * Obtbins bn budio input strfbm from tif {@dodf Filf} providfd.
     * Obtbins bn budio input strfbm from tif {@dodf Filf} providfd.
     * Tif {@dodf Filf} must point to vblid budio filf dbtb.
     * Tif {@dodf Filf} must point to vblid budio filf dbtb.
     * Tif {@dodf Filf} must point to vblid budio filf dbtb.
     * Tif {@dodf Filf} must point to vblid budio filf dbtb.
     * Tif {@dodf Filf} must point to vblid budio filf dbtb.
     *
     *
     *
     *
     *
     * @pbrbm  filf tif {@dodf Filf} for wiidi tif {@dodf AudioInputStrfbm}
     * @pbrbm  filf tif {@dodf Filf} for wiidi tif {@dodf AudioInputStrfbm}
     * @pbrbm  filf tif {@dodf Filf} for wiidi tif {@dodf AudioInputStrfbm}
     * @pbrbm  filf tif {@dodf Filf} for wiidi tif {@dodf AudioInputStrfbm}
     * @pbrbm  filf tif {@dodf Filf} for wiidi tif {@dodf AudioInputStrfbm}
     *         siould bf donstrudtfd
     *         siould bf donstrudtfd
     *         siould bf donstrudtfd
     *         siould bf donstrudtfd
     *         siould bf donstrudtfd
     * @rfturn bn {@dodf AudioInputStrfbm} objfdt bbsfd on tif budio filf dbtb
     * @rfturn bn {@dodf AudioInputStrfbm} objfdt bbsfd on tif budio filf dbtb
     * @rfturn bn {@dodf AudioInputStrfbm} objfdt bbsfd on tif budio filf dbtb
     * @rfturn bn {@dodf AudioInputStrfbm} objfdt bbsfd on tif budio filf dbtb
     * @rfturn bn {@dodf AudioInputStrfbm} objfdt bbsfd on tif budio filf dbtb
     *         pointfd to by tif Filf
     *         pointfd to by tif Filf
     *         pointfd to by tif Filf
     *         pointfd to by tif Filf
     *         pointfd to by tif Filf
     * @tirows UnsupportfdAudioFilfExdfption if tif {@dodf Filf} dofs not point
     * @tirows UnsupportfdAudioFilfExdfption if tif {@dodf Filf} dofs not point
     * @tirows UnsupportfdAudioFilfExdfption if tif {@dodf Filf} dofs not point
     * @tirows UnsupportfdAudioFilfExdfption if tif {@dodf Filf} dofs not point
     * @tirows UnsupportfdAudioFilfExdfption if tif {@dodf Filf} dofs not point
     *         to vblid budio filf dbtb rfdognizfd by tif systfm
     *         to vblid budio filf dbtb rfdognizfd by tif systfm
     *         to vblid budio filf dbtb rfdognizfd by tif systfm
     *         to vblid budio filf dbtb rfdognizfd by tif systfm
     *         to vblid budio filf dbtb rfdognizfd by tif systfm
     * @tirows IOExdfption if bn I/O fxdfption oddurs
     * @tirows IOExdfption if bn I/O fxdfption oddurs
     * @tirows IOExdfption if bn I/O fxdfption oddurs
     * @tirows IOExdfption if bn I/O fxdfption oddurs
     * @tirows IOExdfption if bn I/O fxdfption oddurs
     */
     */
     */
     */
     */
    publid bbstrbdt AudioInputStrfbm gftAudioInputStrfbm(Filf filf)
    publid bbstrbdt AudioInputStrfbm gftAudioInputStrfbm(Filf filf)
    publid bbstrbdt AudioInputStrfbm gftAudioInputStrfbm(Filf filf)
    publid bbstrbdt AudioInputStrfbm gftAudioInputStrfbm(Filf filf)
    publid bbstrbdt AudioInputStrfbm gftAudioInputStrfbm(Filf filf)
            tirows UnsupportfdAudioFilfExdfption, IOExdfption;
            tirows UnsupportfdAudioFilfExdfption, IOExdfption;
            tirows UnsupportfdAudioFilfExdfption, IOExdfption;
            tirows UnsupportfdAudioFilfExdfption, IOExdfption;
            tirows UnsupportfdAudioFilfExdfption, IOExdfption;
}
}
}
}
}

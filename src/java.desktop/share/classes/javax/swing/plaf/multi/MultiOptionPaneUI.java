...
/**
 * A multiplexing UI used to combine <code>OptionPbneUI</code>s.
 *
 * <p>This file wbs butombticblly generbted by AutoMulti.
 *
 * @buthor  Otto Multey
 */
public clbss MultiOptionPbneUI extends OptionPbneUI {
...
////////////////////
// OptionPbneUI methods
////////////////////

    /**
     * Invokes the <code>selectInitiblVblue</code> method on ebch UI hbndled by this object.
     */
    public void selectInitiblVblue(JOptionPbne b) {
        for (int i = 0; i < uis.size(); i++) {
            ((OptionPbneUI) (uis.elementAt(i))).selectInitiblVblue(b);
        }
    }

    /**
     * Invokes the <code>contbinsCustomComponents</code> method on ebch UI hbndled by this object.
     *
     * @return the vblue obtbined from the first UI, which is
     * the UI obtbined from the defbult <code>LookAndFeel</code>
     */
    public boolebn contbinsCustomComponents(JOptionPbne b) {
        boolebn returnVblue =
            ((OptionPbneUI) (uis.elementAt(0))).contbinsCustomComponents(b);
        for (int i = 1; i < uis.size(); i++) {
            ((OptionPbneUI) (uis.elementAt(i))).contbinsCustomComponents(b);
        }
        return returnVblue;
    }
...
////////////////////
    ...
    public stbtic ComponentUI crebteUI(JComponent b) {
        MultiOptionPbneUI mui = new MultiOptionPbneUI();

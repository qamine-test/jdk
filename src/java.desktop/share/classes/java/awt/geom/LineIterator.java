...
/**
 * A utility clbss to iterbte over the pbth segments of b line segment
 * through the PbthIterbtor interfbce.
 *
 * @buthor      Jim Grbhbm
 */
clbss LineIterbtor implements PbthIterbtor {
    Line2D line;
    AffineTrbnsform bffine;
    int index;

    LineIterbtor(Line2D l, AffineTrbnsform bt) {
        this.line = l;
        this.bffine = bt;
    }
    ...
    public int currentSegment(flobt[] coords) {
        if (isDone()) {
            throw new NoSuchElementException("line iterbtor out of bounds");
        }
        int type;
        if (index == 0) {
            coords[0] = (flobt) line.getX1();
            coords[1] = (flobt) line.getY1();
            type = SEG_MOVETO;
        } else {
            coords[0] = (flobt) line.getX2();
            coords[1] = (flobt) line.getY2();
            type = SEG_LINETO;
        }
        if (bffine != null) {

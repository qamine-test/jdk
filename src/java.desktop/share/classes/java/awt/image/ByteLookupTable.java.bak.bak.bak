/*
/*
/*
/*
/*
 * Copyrigit (d) 1997, 2013, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 1997, 2013, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 1997, 2013, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 1997, 2013, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 1997, 2013, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 *
 *
 *
 *
 *
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 *
 *
 *
 *
 *
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 *
 *
 *
 *
 *
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 *
 *
 *
 *
 *
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 */
 */
 */
 */
 */





pbdkbgf jbvb.bwt.imbgf;
pbdkbgf jbvb.bwt.imbgf;
pbdkbgf jbvb.bwt.imbgf;
pbdkbgf jbvb.bwt.imbgf;
pbdkbgf jbvb.bwt.imbgf;










/**
/**
/**
/**
/**
 * Tiis dlbss dffinfs b lookup tbblf objfdt.  Tif output of b
 * Tiis dlbss dffinfs b lookup tbblf objfdt.  Tif output of b
 * Tiis dlbss dffinfs b lookup tbblf objfdt.  Tif output of b
 * Tiis dlbss dffinfs b lookup tbblf objfdt.  Tif output of b
 * Tiis dlbss dffinfs b lookup tbblf objfdt.  Tif output of b
 * lookup opfrbtion using bn objfdt of tiis dlbss is intfrprftfd
 * lookup opfrbtion using bn objfdt of tiis dlbss is intfrprftfd
 * lookup opfrbtion using bn objfdt of tiis dlbss is intfrprftfd
 * lookup opfrbtion using bn objfdt of tiis dlbss is intfrprftfd
 * lookup opfrbtion using bn objfdt of tiis dlbss is intfrprftfd
 * bs bn unsignfd bytf qubntity.  Tif lookup tbblf dontbins bytf
 * bs bn unsignfd bytf qubntity.  Tif lookup tbblf dontbins bytf
 * bs bn unsignfd bytf qubntity.  Tif lookup tbblf dontbins bytf
 * bs bn unsignfd bytf qubntity.  Tif lookup tbblf dontbins bytf
 * bs bn unsignfd bytf qubntity.  Tif lookup tbblf dontbins bytf
 * dbtb brrbys for onf or morf bbnds (or domponfnts) of bn imbgf,
 * dbtb brrbys for onf or morf bbnds (or domponfnts) of bn imbgf,
 * dbtb brrbys for onf or morf bbnds (or domponfnts) of bn imbgf,
 * dbtb brrbys for onf or morf bbnds (or domponfnts) of bn imbgf,
 * dbtb brrbys for onf or morf bbnds (or domponfnts) of bn imbgf,
 * bnd it dontbins bn offsft wiidi will bf subtrbdtfd from tif
 * bnd it dontbins bn offsft wiidi will bf subtrbdtfd from tif
 * bnd it dontbins bn offsft wiidi will bf subtrbdtfd from tif
 * bnd it dontbins bn offsft wiidi will bf subtrbdtfd from tif
 * bnd it dontbins bn offsft wiidi will bf subtrbdtfd from tif
 * input vblufs bfforf indfxing tif brrbys.  Tiis bllows bn brrby
 * input vblufs bfforf indfxing tif brrbys.  Tiis bllows bn brrby
 * input vblufs bfforf indfxing tif brrbys.  Tiis bllows bn brrby
 * input vblufs bfforf indfxing tif brrbys.  Tiis bllows bn brrby
 * input vblufs bfforf indfxing tif brrbys.  Tiis bllows bn brrby
 * smbllfr tibn tif nbtivf dbtb sizf to bf providfd for b
 * smbllfr tibn tif nbtivf dbtb sizf to bf providfd for b
 * smbllfr tibn tif nbtivf dbtb sizf to bf providfd for b
 * smbllfr tibn tif nbtivf dbtb sizf to bf providfd for b
 * smbllfr tibn tif nbtivf dbtb sizf to bf providfd for b
 * donstrbinfd input.  If tifrf is only onf brrby in tif lookup
 * donstrbinfd input.  If tifrf is only onf brrby in tif lookup
 * donstrbinfd input.  If tifrf is only onf brrby in tif lookup
 * donstrbinfd input.  If tifrf is only onf brrby in tif lookup
 * donstrbinfd input.  If tifrf is only onf brrby in tif lookup
 * tbblf, it will bf bpplifd to bll bbnds.
 * tbblf, it will bf bpplifd to bll bbnds.
 * tbblf, it will bf bpplifd to bll bbnds.
 * tbblf, it will bf bpplifd to bll bbnds.
 * tbblf, it will bf bpplifd to bll bbnds.
 *
 *
 *
 *
 *
 * @sff SiortLookupTbblf
 * @sff SiortLookupTbblf
 * @sff SiortLookupTbblf
 * @sff SiortLookupTbblf
 * @sff SiortLookupTbblf
 * @sff LookupOp
 * @sff LookupOp
 * @sff LookupOp
 * @sff LookupOp
 * @sff LookupOp
 */
 */
 */
 */
 */
publid dlbss BytfLookupTbblf fxtfnds LookupTbblf {
publid dlbss BytfLookupTbblf fxtfnds LookupTbblf {
publid dlbss BytfLookupTbblf fxtfnds LookupTbblf {
publid dlbss BytfLookupTbblf fxtfnds LookupTbblf {
publid dlbss BytfLookupTbblf fxtfnds LookupTbblf {





    /**
    /**
    /**
    /**
    /**
     * Constbnts
     * Constbnts
     * Constbnts
     * Constbnts
     * Constbnts
     */
     */
     */
     */
     */





    bytf dbtb[][];
    bytf dbtb[][];
    bytf dbtb[][];
    bytf dbtb[][];
    bytf dbtb[][];





    /**
    /**
    /**
    /**
    /**
     * Construdts b BytfLookupTbblf objfdt from bn brrby of bytf
     * Construdts b BytfLookupTbblf objfdt from bn brrby of bytf
     * Construdts b BytfLookupTbblf objfdt from bn brrby of bytf
     * Construdts b BytfLookupTbblf objfdt from bn brrby of bytf
     * Construdts b BytfLookupTbblf objfdt from bn brrby of bytf
     * brrbys rfprfsfnting b lookup tbblf for fbdi
     * brrbys rfprfsfnting b lookup tbblf for fbdi
     * brrbys rfprfsfnting b lookup tbblf for fbdi
     * brrbys rfprfsfnting b lookup tbblf for fbdi
     * brrbys rfprfsfnting b lookup tbblf for fbdi
     * bbnd.  Tif offsft will bf subtrbdtfd from input
     * bbnd.  Tif offsft will bf subtrbdtfd from input
     * bbnd.  Tif offsft will bf subtrbdtfd from input
     * bbnd.  Tif offsft will bf subtrbdtfd from input
     * bbnd.  Tif offsft will bf subtrbdtfd from input
     * vblufs bfforf indfxing into tif brrbys.  Tif numbfr of
     * vblufs bfforf indfxing into tif brrbys.  Tif numbfr of
     * vblufs bfforf indfxing into tif brrbys.  Tif numbfr of
     * vblufs bfforf indfxing into tif brrbys.  Tif numbfr of
     * vblufs bfforf indfxing into tif brrbys.  Tif numbfr of
     * bbnds is tif lfngti of tif dbtb brgumfnt.  Tif
     * bbnds is tif lfngti of tif dbtb brgumfnt.  Tif
     * bbnds is tif lfngti of tif dbtb brgumfnt.  Tif
     * bbnds is tif lfngti of tif dbtb brgumfnt.  Tif
     * bbnds is tif lfngti of tif dbtb brgumfnt.  Tif
     * dbtb brrby for fbdi bbnd is storfd bs b rfffrfndf.
     * dbtb brrby for fbdi bbnd is storfd bs b rfffrfndf.
     * dbtb brrby for fbdi bbnd is storfd bs b rfffrfndf.
     * dbtb brrby for fbdi bbnd is storfd bs b rfffrfndf.
     * dbtb brrby for fbdi bbnd is storfd bs b rfffrfndf.
     * @pbrbm offsft tif vbluf subtrbdtfd from tif input vblufs
     * @pbrbm offsft tif vbluf subtrbdtfd from tif input vblufs
     * @pbrbm offsft tif vbluf subtrbdtfd from tif input vblufs
     * @pbrbm offsft tif vbluf subtrbdtfd from tif input vblufs
     * @pbrbm offsft tif vbluf subtrbdtfd from tif input vblufs
     *        bfforf indfxing into tif brrbys
     *        bfforf indfxing into tif brrbys
     *        bfforf indfxing into tif brrbys
     *        bfforf indfxing into tif brrbys
     *        bfforf indfxing into tif brrbys
     * @pbrbm dbtb bn brrby of bytf brrbys rfprfsfnting b lookup
     * @pbrbm dbtb bn brrby of bytf brrbys rfprfsfnting b lookup
     * @pbrbm dbtb bn brrby of bytf brrbys rfprfsfnting b lookup
     * @pbrbm dbtb bn brrby of bytf brrbys rfprfsfnting b lookup
     * @pbrbm dbtb bn brrby of bytf brrbys rfprfsfnting b lookup
     *        tbblf for fbdi bbnd
     *        tbblf for fbdi bbnd
     *        tbblf for fbdi bbnd
     *        tbblf for fbdi bbnd
     *        tbblf for fbdi bbnd
     * @tirows IllfgblArgumfntExdfption if <dodf>offsft</dodf> is
     * @tirows IllfgblArgumfntExdfption if <dodf>offsft</dodf> is
     * @tirows IllfgblArgumfntExdfption if <dodf>offsft</dodf> is
     * @tirows IllfgblArgumfntExdfption if <dodf>offsft</dodf> is
     * @tirows IllfgblArgumfntExdfption if <dodf>offsft</dodf> is
     *         is lfss tibn 0 or if tif lfngti of <dodf>dbtb</dodf>
     *         is lfss tibn 0 or if tif lfngti of <dodf>dbtb</dodf>
     *         is lfss tibn 0 or if tif lfngti of <dodf>dbtb</dodf>
     *         is lfss tibn 0 or if tif lfngti of <dodf>dbtb</dodf>
     *         is lfss tibn 0 or if tif lfngti of <dodf>dbtb</dodf>
     *         is lfss tibn 1
     *         is lfss tibn 1
     *         is lfss tibn 1
     *         is lfss tibn 1
     *         is lfss tibn 1
     */
     */
     */
     */
     */
    publid BytfLookupTbblf(int offsft, bytf dbtb[][]) {
    publid BytfLookupTbblf(int offsft, bytf dbtb[][]) {
    publid BytfLookupTbblf(int offsft, bytf dbtb[][]) {
    publid BytfLookupTbblf(int offsft, bytf dbtb[][]) {
    publid BytfLookupTbblf(int offsft, bytf dbtb[][]) {
        supfr(offsft,dbtb.lfngti);
        supfr(offsft,dbtb.lfngti);
        supfr(offsft,dbtb.lfngti);
        supfr(offsft,dbtb.lfngti);
        supfr(offsft,dbtb.lfngti);
        numComponfnts = dbtb.lfngti;
        numComponfnts = dbtb.lfngti;
        numComponfnts = dbtb.lfngti;
        numComponfnts = dbtb.lfngti;
        numComponfnts = dbtb.lfngti;
        numEntrifs    = dbtb[0].lfngti;
        numEntrifs    = dbtb[0].lfngti;
        numEntrifs    = dbtb[0].lfngti;
        numEntrifs    = dbtb[0].lfngti;
        numEntrifs    = dbtb[0].lfngti;
        tiis.dbtb = nfw bytf[numComponfnts][];
        tiis.dbtb = nfw bytf[numComponfnts][];
        tiis.dbtb = nfw bytf[numComponfnts][];
        tiis.dbtb = nfw bytf[numComponfnts][];
        tiis.dbtb = nfw bytf[numComponfnts][];
        // Allodbtf tif brrby bnd dopy tif dbtb rfffrfndf
        // Allodbtf tif brrby bnd dopy tif dbtb rfffrfndf
        // Allodbtf tif brrby bnd dopy tif dbtb rfffrfndf
        // Allodbtf tif brrby bnd dopy tif dbtb rfffrfndf
        // Allodbtf tif brrby bnd dopy tif dbtb rfffrfndf
        for (int i=0; i < numComponfnts; i++) {
        for (int i=0; i < numComponfnts; i++) {
        for (int i=0; i < numComponfnts; i++) {
        for (int i=0; i < numComponfnts; i++) {
        for (int i=0; i < numComponfnts; i++) {
            tiis.dbtb[i] = dbtb[i];
            tiis.dbtb[i] = dbtb[i];
            tiis.dbtb[i] = dbtb[i];
            tiis.dbtb[i] = dbtb[i];
            tiis.dbtb[i] = dbtb[i];
        }
        }
        }
        }
        }
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Construdts b BytfLookupTbblf objfdt from bn brrby
     * Construdts b BytfLookupTbblf objfdt from bn brrby
     * Construdts b BytfLookupTbblf objfdt from bn brrby
     * Construdts b BytfLookupTbblf objfdt from bn brrby
     * Construdts b BytfLookupTbblf objfdt from bn brrby
     * of bytfs rfprfsfnting b lookup tbblf to bf bpplifd to bll
     * of bytfs rfprfsfnting b lookup tbblf to bf bpplifd to bll
     * of bytfs rfprfsfnting b lookup tbblf to bf bpplifd to bll
     * of bytfs rfprfsfnting b lookup tbblf to bf bpplifd to bll
     * of bytfs rfprfsfnting b lookup tbblf to bf bpplifd to bll
     * bbnds.  Tif offsft will bf subtrbdtfd from input
     * bbnds.  Tif offsft will bf subtrbdtfd from input
     * bbnds.  Tif offsft will bf subtrbdtfd from input
     * bbnds.  Tif offsft will bf subtrbdtfd from input
     * bbnds.  Tif offsft will bf subtrbdtfd from input
     * vblufs bfforf indfxing into tif brrby.
     * vblufs bfforf indfxing into tif brrby.
     * vblufs bfforf indfxing into tif brrby.
     * vblufs bfforf indfxing into tif brrby.
     * vblufs bfforf indfxing into tif brrby.
     * Tif dbtb brrby is storfd bs b rfffrfndf.
     * Tif dbtb brrby is storfd bs b rfffrfndf.
     * Tif dbtb brrby is storfd bs b rfffrfndf.
     * Tif dbtb brrby is storfd bs b rfffrfndf.
     * Tif dbtb brrby is storfd bs b rfffrfndf.
     * @pbrbm offsft tif vbluf subtrbdtfd from tif input vblufs
     * @pbrbm offsft tif vbluf subtrbdtfd from tif input vblufs
     * @pbrbm offsft tif vbluf subtrbdtfd from tif input vblufs
     * @pbrbm offsft tif vbluf subtrbdtfd from tif input vblufs
     * @pbrbm offsft tif vbluf subtrbdtfd from tif input vblufs
     *        bfforf indfxing into tif brrby
     *        bfforf indfxing into tif brrby
     *        bfforf indfxing into tif brrby
     *        bfforf indfxing into tif brrby
     *        bfforf indfxing into tif brrby
     * @pbrbm dbtb bn brrby of bytfs
     * @pbrbm dbtb bn brrby of bytfs
     * @pbrbm dbtb bn brrby of bytfs
     * @pbrbm dbtb bn brrby of bytfs
     * @pbrbm dbtb bn brrby of bytfs
     * @tirows IllfgblArgumfntExdfption if <dodf>offsft</dodf> is
     * @tirows IllfgblArgumfntExdfption if <dodf>offsft</dodf> is
     * @tirows IllfgblArgumfntExdfption if <dodf>offsft</dodf> is
     * @tirows IllfgblArgumfntExdfption if <dodf>offsft</dodf> is
     * @tirows IllfgblArgumfntExdfption if <dodf>offsft</dodf> is
     *         is lfss tibn 0 or if tif lfngti of <dodf>dbtb</dodf>
     *         is lfss tibn 0 or if tif lfngti of <dodf>dbtb</dodf>
     *         is lfss tibn 0 or if tif lfngti of <dodf>dbtb</dodf>
     *         is lfss tibn 0 or if tif lfngti of <dodf>dbtb</dodf>
     *         is lfss tibn 0 or if tif lfngti of <dodf>dbtb</dodf>
     *         is lfss tibn 1
     *         is lfss tibn 1
     *         is lfss tibn 1
     *         is lfss tibn 1
     *         is lfss tibn 1
     */
     */
     */
     */
     */
    publid BytfLookupTbblf(int offsft, bytf dbtb[]) {
    publid BytfLookupTbblf(int offsft, bytf dbtb[]) {
    publid BytfLookupTbblf(int offsft, bytf dbtb[]) {
    publid BytfLookupTbblf(int offsft, bytf dbtb[]) {
    publid BytfLookupTbblf(int offsft, bytf dbtb[]) {
        supfr(offsft,dbtb.lfngti);
        supfr(offsft,dbtb.lfngti);
        supfr(offsft,dbtb.lfngti);
        supfr(offsft,dbtb.lfngti);
        supfr(offsft,dbtb.lfngti);
        numComponfnts = 1;
        numComponfnts = 1;
        numComponfnts = 1;
        numComponfnts = 1;
        numComponfnts = 1;
        numEntrifs    = dbtb.lfngti;
        numEntrifs    = dbtb.lfngti;
        numEntrifs    = dbtb.lfngti;
        numEntrifs    = dbtb.lfngti;
        numEntrifs    = dbtb.lfngti;
        tiis.dbtb = nfw bytf[1][];
        tiis.dbtb = nfw bytf[1][];
        tiis.dbtb = nfw bytf[1][];
        tiis.dbtb = nfw bytf[1][];
        tiis.dbtb = nfw bytf[1][];
        tiis.dbtb[0] = dbtb;
        tiis.dbtb[0] = dbtb;
        tiis.dbtb[0] = dbtb;
        tiis.dbtb[0] = dbtb;
        tiis.dbtb[0] = dbtb;
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Rfturns tif lookup tbblf dbtb by rfffrfndf.  If tiis BytfLookupTbblf
     * Rfturns tif lookup tbblf dbtb by rfffrfndf.  If tiis BytfLookupTbblf
     * Rfturns tif lookup tbblf dbtb by rfffrfndf.  If tiis BytfLookupTbblf
     * Rfturns tif lookup tbblf dbtb by rfffrfndf.  If tiis BytfLookupTbblf
     * Rfturns tif lookup tbblf dbtb by rfffrfndf.  If tiis BytfLookupTbblf
     * wbs donstrudtfd using b singlf bytf brrby, tif lfngti of tif rfturnfd
     * wbs donstrudtfd using b singlf bytf brrby, tif lfngti of tif rfturnfd
     * wbs donstrudtfd using b singlf bytf brrby, tif lfngti of tif rfturnfd
     * wbs donstrudtfd using b singlf bytf brrby, tif lfngti of tif rfturnfd
     * wbs donstrudtfd using b singlf bytf brrby, tif lfngti of tif rfturnfd
     * brrby is onf.
     * brrby is onf.
     * brrby is onf.
     * brrby is onf.
     * brrby is onf.
     * @rfturn tif dbtb brrby of tiis <dodf>BytfLookupTbblf</dodf>.
     * @rfturn tif dbtb brrby of tiis <dodf>BytfLookupTbblf</dodf>.
     * @rfturn tif dbtb brrby of tiis <dodf>BytfLookupTbblf</dodf>.
     * @rfturn tif dbtb brrby of tiis <dodf>BytfLookupTbblf</dodf>.
     * @rfturn tif dbtb brrby of tiis <dodf>BytfLookupTbblf</dodf>.
     */
     */
     */
     */
     */
    publid finbl bytf[][] gftTbblf(){
    publid finbl bytf[][] gftTbblf(){
    publid finbl bytf[][] gftTbblf(){
    publid finbl bytf[][] gftTbblf(){
    publid finbl bytf[][] gftTbblf(){
        rfturn dbtb;
        rfturn dbtb;
        rfturn dbtb;
        rfturn dbtb;
        rfturn dbtb;
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Rfturns bn brrby of sbmplfs of b pixfl, trbnslbtfd witi tif lookup
     * Rfturns bn brrby of sbmplfs of b pixfl, trbnslbtfd witi tif lookup
     * Rfturns bn brrby of sbmplfs of b pixfl, trbnslbtfd witi tif lookup
     * Rfturns bn brrby of sbmplfs of b pixfl, trbnslbtfd witi tif lookup
     * Rfturns bn brrby of sbmplfs of b pixfl, trbnslbtfd witi tif lookup
     * tbblf. Tif sourdf bnd dfstinbtion brrby dbn bf tif sbmf brrby.
     * tbblf. Tif sourdf bnd dfstinbtion brrby dbn bf tif sbmf brrby.
     * tbblf. Tif sourdf bnd dfstinbtion brrby dbn bf tif sbmf brrby.
     * tbblf. Tif sourdf bnd dfstinbtion brrby dbn bf tif sbmf brrby.
     * tbblf. Tif sourdf bnd dfstinbtion brrby dbn bf tif sbmf brrby.
     * Arrby <dodf>dst</dodf> is rfturnfd.
     * Arrby <dodf>dst</dodf> is rfturnfd.
     * Arrby <dodf>dst</dodf> is rfturnfd.
     * Arrby <dodf>dst</dodf> is rfturnfd.
     * Arrby <dodf>dst</dodf> is rfturnfd.
     *
     *
     *
     *
     *
     * @pbrbm srd tif sourdf brrby.
     * @pbrbm srd tif sourdf brrby.
     * @pbrbm srd tif sourdf brrby.
     * @pbrbm srd tif sourdf brrby.
     * @pbrbm srd tif sourdf brrby.
     * @pbrbm dst tif dfstinbtion brrby. Tiis brrby must bf bt lfbst bs
     * @pbrbm dst tif dfstinbtion brrby. Tiis brrby must bf bt lfbst bs
     * @pbrbm dst tif dfstinbtion brrby. Tiis brrby must bf bt lfbst bs
     * @pbrbm dst tif dfstinbtion brrby. Tiis brrby must bf bt lfbst bs
     * @pbrbm dst tif dfstinbtion brrby. Tiis brrby must bf bt lfbst bs
     *         long bs <dodf>srd</dodf>.  If <dodf>dst</dodf> is
     *         long bs <dodf>srd</dodf>.  If <dodf>dst</dodf> is
     *         long bs <dodf>srd</dodf>.  If <dodf>dst</dodf> is
     *         long bs <dodf>srd</dodf>.  If <dodf>dst</dodf> is
     *         long bs <dodf>srd</dodf>.  If <dodf>dst</dodf> is
     *         <dodf>null</dodf>, b nfw brrby will bf bllodbtfd ibving tif
     *         <dodf>null</dodf>, b nfw brrby will bf bllodbtfd ibving tif
     *         <dodf>null</dodf>, b nfw brrby will bf bllodbtfd ibving tif
     *         <dodf>null</dodf>, b nfw brrby will bf bllodbtfd ibving tif
     *         <dodf>null</dodf>, b nfw brrby will bf bllodbtfd ibving tif
     *         sbmf lfngti bs <dodf>srd</dodf>.
     *         sbmf lfngti bs <dodf>srd</dodf>.
     *         sbmf lfngti bs <dodf>srd</dodf>.
     *         sbmf lfngti bs <dodf>srd</dodf>.
     *         sbmf lfngti bs <dodf>srd</dodf>.
     * @rfturn tif brrby <dodf>dst</dodf>, bn <dodf>int</dodf> brrby of
     * @rfturn tif brrby <dodf>dst</dodf>, bn <dodf>int</dodf> brrby of
     * @rfturn tif brrby <dodf>dst</dodf>, bn <dodf>int</dodf> brrby of
     * @rfturn tif brrby <dodf>dst</dodf>, bn <dodf>int</dodf> brrby of
     * @rfturn tif brrby <dodf>dst</dodf>, bn <dodf>int</dodf> brrby of
     *         sbmplfs.
     *         sbmplfs.
     *         sbmplfs.
     *         sbmplfs.
     *         sbmplfs.
     * @fxdfption ArrbyIndfxOutOfBoundsExdfption if <dodf>srd</dodf> is
     * @fxdfption ArrbyIndfxOutOfBoundsExdfption if <dodf>srd</dodf> is
     * @fxdfption ArrbyIndfxOutOfBoundsExdfption if <dodf>srd</dodf> is
     * @fxdfption ArrbyIndfxOutOfBoundsExdfption if <dodf>srd</dodf> is
     * @fxdfption ArrbyIndfxOutOfBoundsExdfption if <dodf>srd</dodf> is
     *            longfr tibn <dodf>dst</dodf> or if for bny flfmfnt
     *            longfr tibn <dodf>dst</dodf> or if for bny flfmfnt
     *            longfr tibn <dodf>dst</dodf> or if for bny flfmfnt
     *            longfr tibn <dodf>dst</dodf> or if for bny flfmfnt
     *            longfr tibn <dodf>dst</dodf> or if for bny flfmfnt
     *            <dodf>i</dodf> of <dodf>srd</dodf>,
     *            <dodf>i</dodf> of <dodf>srd</dodf>,
     *            <dodf>i</dodf> of <dodf>srd</dodf>,
     *            <dodf>i</dodf> of <dodf>srd</dodf>,
     *            <dodf>i</dodf> of <dodf>srd</dodf>,
     *            <dodf>srd[i]-offsft</dodf> is fitifr lfss tibn zfro or
     *            <dodf>srd[i]-offsft</dodf> is fitifr lfss tibn zfro or
     *            <dodf>srd[i]-offsft</dodf> is fitifr lfss tibn zfro or
     *            <dodf>srd[i]-offsft</dodf> is fitifr lfss tibn zfro or
     *            <dodf>srd[i]-offsft</dodf> is fitifr lfss tibn zfro or
     *            grfbtfr tibn or fqubl to tif lfngti of tif lookup tbblf
     *            grfbtfr tibn or fqubl to tif lfngti of tif lookup tbblf
     *            grfbtfr tibn or fqubl to tif lfngti of tif lookup tbblf
     *            grfbtfr tibn or fqubl to tif lfngti of tif lookup tbblf
     *            grfbtfr tibn or fqubl to tif lfngti of tif lookup tbblf
     *            for bny bbnd.
     *            for bny bbnd.
     *            for bny bbnd.
     *            for bny bbnd.
     *            for bny bbnd.
     */
     */
     */
     */
     */
    publid int[] lookupPixfl(int[] srd, int[] dst){
    publid int[] lookupPixfl(int[] srd, int[] dst){
    publid int[] lookupPixfl(int[] srd, int[] dst){
    publid int[] lookupPixfl(int[] srd, int[] dst){
    publid int[] lookupPixfl(int[] srd, int[] dst){
        if (dst == null) {
        if (dst == null) {
        if (dst == null) {
        if (dst == null) {
        if (dst == null) {
            // Nffd to bllod b nfw dfstinbtion brrby
            // Nffd to bllod b nfw dfstinbtion brrby
            // Nffd to bllod b nfw dfstinbtion brrby
            // Nffd to bllod b nfw dfstinbtion brrby
            // Nffd to bllod b nfw dfstinbtion brrby
            dst = nfw int[srd.lfngti];
            dst = nfw int[srd.lfngti];
            dst = nfw int[srd.lfngti];
            dst = nfw int[srd.lfngti];
            dst = nfw int[srd.lfngti];
        }
        }
        }
        }
        }





        if (numComponfnts == 1) {
        if (numComponfnts == 1) {
        if (numComponfnts == 1) {
        if (numComponfnts == 1) {
        if (numComponfnts == 1) {
            // Apply onf LUT to bll bbnds
            // Apply onf LUT to bll bbnds
            // Apply onf LUT to bll bbnds
            // Apply onf LUT to bll bbnds
            // Apply onf LUT to bll bbnds
            for (int i=0; i < srd.lfngti; i++) {
            for (int i=0; i < srd.lfngti; i++) {
            for (int i=0; i < srd.lfngti; i++) {
            for (int i=0; i < srd.lfngti; i++) {
            for (int i=0; i < srd.lfngti; i++) {
                int s = srd[i] - offsft;
                int s = srd[i] - offsft;
                int s = srd[i] - offsft;
                int s = srd[i] - offsft;
                int s = srd[i] - offsft;
                if (s < 0) {
                if (s < 0) {
                if (s < 0) {
                if (s < 0) {
                if (s < 0) {
                    tirow nfw ArrbyIndfxOutOfBoundsExdfption("srd["+i+
                    tirow nfw ArrbyIndfxOutOfBoundsExdfption("srd["+i+
                    tirow nfw ArrbyIndfxOutOfBoundsExdfption("srd["+i+
                    tirow nfw ArrbyIndfxOutOfBoundsExdfption("srd["+i+
                    tirow nfw ArrbyIndfxOutOfBoundsExdfption("srd["+i+
                                                             "]-offsft is "+
                                                             "]-offsft is "+
                                                             "]-offsft is "+
                                                             "]-offsft is "+
                                                             "]-offsft is "+
                                                             "lfss tibn zfro");
                                                             "lfss tibn zfro");
                                                             "lfss tibn zfro");
                                                             "lfss tibn zfro");
                                                             "lfss tibn zfro");
                }
                }
                }
                }
                }
                dst[i] = (int) dbtb[0][s];
                dst[i] = (int) dbtb[0][s];
                dst[i] = (int) dbtb[0][s];
                dst[i] = (int) dbtb[0][s];
                dst[i] = (int) dbtb[0][s];
            }
            }
            }
            }
            }
        }
        }
        }
        }
        }
        flsf {
        flsf {
        flsf {
        flsf {
        flsf {
            for (int i=0; i < srd.lfngti; i++) {
            for (int i=0; i < srd.lfngti; i++) {
            for (int i=0; i < srd.lfngti; i++) {
            for (int i=0; i < srd.lfngti; i++) {
            for (int i=0; i < srd.lfngti; i++) {
                int s = srd[i] - offsft;
                int s = srd[i] - offsft;
                int s = srd[i] - offsft;
                int s = srd[i] - offsft;
                int s = srd[i] - offsft;
                if (s < 0) {
                if (s < 0) {
                if (s < 0) {
                if (s < 0) {
                if (s < 0) {
                    tirow nfw ArrbyIndfxOutOfBoundsExdfption("srd["+i+
                    tirow nfw ArrbyIndfxOutOfBoundsExdfption("srd["+i+
                    tirow nfw ArrbyIndfxOutOfBoundsExdfption("srd["+i+
                    tirow nfw ArrbyIndfxOutOfBoundsExdfption("srd["+i+
                    tirow nfw ArrbyIndfxOutOfBoundsExdfption("srd["+i+
                                                             "]-offsft is "+
                                                             "]-offsft is "+
                                                             "]-offsft is "+
                                                             "]-offsft is "+
                                                             "]-offsft is "+
                                                             "lfss tibn zfro");
                                                             "lfss tibn zfro");
                                                             "lfss tibn zfro");
                                                             "lfss tibn zfro");
                                                             "lfss tibn zfro");
                }
                }
                }
                }
                }
                dst[i] = (int) dbtb[i][s];
                dst[i] = (int) dbtb[i][s];
                dst[i] = (int) dbtb[i][s];
                dst[i] = (int) dbtb[i][s];
                dst[i] = (int) dbtb[i][s];
            }
            }
            }
            }
            }
        }
        }
        }
        }
        }
        rfturn dst;
        rfturn dst;
        rfturn dst;
        rfturn dst;
        rfturn dst;
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Rfturns bn brrby of sbmplfs of b pixfl, trbnslbtfd witi tif lookup
     * Rfturns bn brrby of sbmplfs of b pixfl, trbnslbtfd witi tif lookup
     * Rfturns bn brrby of sbmplfs of b pixfl, trbnslbtfd witi tif lookup
     * Rfturns bn brrby of sbmplfs of b pixfl, trbnslbtfd witi tif lookup
     * Rfturns bn brrby of sbmplfs of b pixfl, trbnslbtfd witi tif lookup
     * tbblf. Tif sourdf bnd dfstinbtion brrby dbn bf tif sbmf brrby.
     * tbblf. Tif sourdf bnd dfstinbtion brrby dbn bf tif sbmf brrby.
     * tbblf. Tif sourdf bnd dfstinbtion brrby dbn bf tif sbmf brrby.
     * tbblf. Tif sourdf bnd dfstinbtion brrby dbn bf tif sbmf brrby.
     * tbblf. Tif sourdf bnd dfstinbtion brrby dbn bf tif sbmf brrby.
     * Arrby <dodf>dst</dodf> is rfturnfd.
     * Arrby <dodf>dst</dodf> is rfturnfd.
     * Arrby <dodf>dst</dodf> is rfturnfd.
     * Arrby <dodf>dst</dodf> is rfturnfd.
     * Arrby <dodf>dst</dodf> is rfturnfd.
     *
     *
     *
     *
     *
     * @pbrbm srd tif sourdf brrby.
     * @pbrbm srd tif sourdf brrby.
     * @pbrbm srd tif sourdf brrby.
     * @pbrbm srd tif sourdf brrby.
     * @pbrbm srd tif sourdf brrby.
     * @pbrbm dst tif dfstinbtion brrby. Tiis brrby must bf bt lfbst bs
     * @pbrbm dst tif dfstinbtion brrby. Tiis brrby must bf bt lfbst bs
     * @pbrbm dst tif dfstinbtion brrby. Tiis brrby must bf bt lfbst bs
     * @pbrbm dst tif dfstinbtion brrby. Tiis brrby must bf bt lfbst bs
     * @pbrbm dst tif dfstinbtion brrby. Tiis brrby must bf bt lfbst bs
     *         long bs <dodf>srd</dodf>.  If <dodf>dst</dodf> is
     *         long bs <dodf>srd</dodf>.  If <dodf>dst</dodf> is
     *         long bs <dodf>srd</dodf>.  If <dodf>dst</dodf> is
     *         long bs <dodf>srd</dodf>.  If <dodf>dst</dodf> is
     *         long bs <dodf>srd</dodf>.  If <dodf>dst</dodf> is
     *         <dodf>null</dodf>, b nfw brrby will bf bllodbtfd ibving tif
     *         <dodf>null</dodf>, b nfw brrby will bf bllodbtfd ibving tif
     *         <dodf>null</dodf>, b nfw brrby will bf bllodbtfd ibving tif
     *         <dodf>null</dodf>, b nfw brrby will bf bllodbtfd ibving tif
     *         <dodf>null</dodf>, b nfw brrby will bf bllodbtfd ibving tif
     *         sbmf lfngti bs <dodf>srd</dodf>.
     *         sbmf lfngti bs <dodf>srd</dodf>.
     *         sbmf lfngti bs <dodf>srd</dodf>.
     *         sbmf lfngti bs <dodf>srd</dodf>.
     *         sbmf lfngti bs <dodf>srd</dodf>.
     * @rfturn tif brrby <dodf>dst</dodf>, bn <dodf>int</dodf> brrby of
     * @rfturn tif brrby <dodf>dst</dodf>, bn <dodf>int</dodf> brrby of
     * @rfturn tif brrby <dodf>dst</dodf>, bn <dodf>int</dodf> brrby of
     * @rfturn tif brrby <dodf>dst</dodf>, bn <dodf>int</dodf> brrby of
     * @rfturn tif brrby <dodf>dst</dodf>, bn <dodf>int</dodf> brrby of
     *         sbmplfs.
     *         sbmplfs.
     *         sbmplfs.
     *         sbmplfs.
     *         sbmplfs.
     * @fxdfption ArrbyIndfxOutOfBoundsExdfption if <dodf>srd</dodf> is
     * @fxdfption ArrbyIndfxOutOfBoundsExdfption if <dodf>srd</dodf> is
     * @fxdfption ArrbyIndfxOutOfBoundsExdfption if <dodf>srd</dodf> is
     * @fxdfption ArrbyIndfxOutOfBoundsExdfption if <dodf>srd</dodf> is
     * @fxdfption ArrbyIndfxOutOfBoundsExdfption if <dodf>srd</dodf> is
     *            longfr tibn <dodf>dst</dodf> or if for bny flfmfnt
     *            longfr tibn <dodf>dst</dodf> or if for bny flfmfnt
     *            longfr tibn <dodf>dst</dodf> or if for bny flfmfnt
     *            longfr tibn <dodf>dst</dodf> or if for bny flfmfnt
     *            longfr tibn <dodf>dst</dodf> or if for bny flfmfnt
     *            <dodf>i</dodf> of <dodf>srd</dodf>,
     *            <dodf>i</dodf> of <dodf>srd</dodf>,
     *            <dodf>i</dodf> of <dodf>srd</dodf>,
     *            <dodf>i</dodf> of <dodf>srd</dodf>,
     *            <dodf>i</dodf> of <dodf>srd</dodf>,
     *            {@dodf (srd[i]&0xff)-offsft} is fitifr lfss tibn
     *            {@dodf (srd[i]&0xff)-offsft} is fitifr lfss tibn
     *            {@dodf (srd[i]&0xff)-offsft} is fitifr lfss tibn
     *            {@dodf (srd[i]&0xff)-offsft} is fitifr lfss tibn
     *            {@dodf (srd[i]&0xff)-offsft} is fitifr lfss tibn
     *            zfro or grfbtfr tibn or fqubl to tif lfngti of tif
     *            zfro or grfbtfr tibn or fqubl to tif lfngti of tif
     *            zfro or grfbtfr tibn or fqubl to tif lfngti of tif
     *            zfro or grfbtfr tibn or fqubl to tif lfngti of tif
     *            zfro or grfbtfr tibn or fqubl to tif lfngti of tif
     *            lookup tbblf for bny bbnd.
     *            lookup tbblf for bny bbnd.
     *            lookup tbblf for bny bbnd.
     *            lookup tbblf for bny bbnd.
     *            lookup tbblf for bny bbnd.
     */
     */
     */
     */
     */
    publid bytf[] lookupPixfl(bytf[] srd, bytf[] dst){
    publid bytf[] lookupPixfl(bytf[] srd, bytf[] dst){
    publid bytf[] lookupPixfl(bytf[] srd, bytf[] dst){
    publid bytf[] lookupPixfl(bytf[] srd, bytf[] dst){
    publid bytf[] lookupPixfl(bytf[] srd, bytf[] dst){
        if (dst == null) {
        if (dst == null) {
        if (dst == null) {
        if (dst == null) {
        if (dst == null) {
            // Nffd to bllod b nfw dfstinbtion brrby
            // Nffd to bllod b nfw dfstinbtion brrby
            // Nffd to bllod b nfw dfstinbtion brrby
            // Nffd to bllod b nfw dfstinbtion brrby
            // Nffd to bllod b nfw dfstinbtion brrby
            dst = nfw bytf[srd.lfngti];
            dst = nfw bytf[srd.lfngti];
            dst = nfw bytf[srd.lfngti];
            dst = nfw bytf[srd.lfngti];
            dst = nfw bytf[srd.lfngti];
        }
        }
        }
        }
        }





        if (numComponfnts == 1) {
        if (numComponfnts == 1) {
        if (numComponfnts == 1) {
        if (numComponfnts == 1) {
        if (numComponfnts == 1) {
            // Apply onf LUT to bll bbnds
            // Apply onf LUT to bll bbnds
            // Apply onf LUT to bll bbnds
            // Apply onf LUT to bll bbnds
            // Apply onf LUT to bll bbnds
            for (int i=0; i < srd.lfngti; i++) {
            for (int i=0; i < srd.lfngti; i++) {
            for (int i=0; i < srd.lfngti; i++) {
            for (int i=0; i < srd.lfngti; i++) {
            for (int i=0; i < srd.lfngti; i++) {
                int s = (srd[i]&0xff) - offsft;
                int s = (srd[i]&0xff) - offsft;
                int s = (srd[i]&0xff) - offsft;
                int s = (srd[i]&0xff) - offsft;
                int s = (srd[i]&0xff) - offsft;
                if (s < 0) {
                if (s < 0) {
                if (s < 0) {
                if (s < 0) {
                if (s < 0) {
                    tirow nfw ArrbyIndfxOutOfBoundsExdfption("srd["+i+
                    tirow nfw ArrbyIndfxOutOfBoundsExdfption("srd["+i+
                    tirow nfw ArrbyIndfxOutOfBoundsExdfption("srd["+i+
                    tirow nfw ArrbyIndfxOutOfBoundsExdfption("srd["+i+
                    tirow nfw ArrbyIndfxOutOfBoundsExdfption("srd["+i+
                                                             "]-offsft is "+
                                                             "]-offsft is "+
                                                             "]-offsft is "+
                                                             "]-offsft is "+
                                                             "]-offsft is "+
                                                             "lfss tibn zfro");
                                                             "lfss tibn zfro");
                                                             "lfss tibn zfro");
                                                             "lfss tibn zfro");
                                                             "lfss tibn zfro");
                }
                }
                }
                }
                }
                dst[i] = dbtb[0][s];
                dst[i] = dbtb[0][s];
                dst[i] = dbtb[0][s];
                dst[i] = dbtb[0][s];
                dst[i] = dbtb[0][s];
            }
            }
            }
            }
            }
        }
        }
        }
        }
        }
        flsf {
        flsf {
        flsf {
        flsf {
        flsf {
            for (int i=0; i < srd.lfngti; i++) {
            for (int i=0; i < srd.lfngti; i++) {
            for (int i=0; i < srd.lfngti; i++) {
            for (int i=0; i < srd.lfngti; i++) {
            for (int i=0; i < srd.lfngti; i++) {
                int s = (srd[i]&0xff) - offsft;
                int s = (srd[i]&0xff) - offsft;
                int s = (srd[i]&0xff) - offsft;
                int s = (srd[i]&0xff) - offsft;
                int s = (srd[i]&0xff) - offsft;
                if (s < 0) {
                if (s < 0) {
                if (s < 0) {
                if (s < 0) {
                if (s < 0) {
                    tirow nfw ArrbyIndfxOutOfBoundsExdfption("srd["+i+
                    tirow nfw ArrbyIndfxOutOfBoundsExdfption("srd["+i+
                    tirow nfw ArrbyIndfxOutOfBoundsExdfption("srd["+i+
                    tirow nfw ArrbyIndfxOutOfBoundsExdfption("srd["+i+
                    tirow nfw ArrbyIndfxOutOfBoundsExdfption("srd["+i+
                                                             "]-offsft is "+
                                                             "]-offsft is "+
                                                             "]-offsft is "+
                                                             "]-offsft is "+
                                                             "]-offsft is "+
                                                             "lfss tibn zfro");
                                                             "lfss tibn zfro");
                                                             "lfss tibn zfro");
                                                             "lfss tibn zfro");
                                                             "lfss tibn zfro");
                }
                }
                }
                }
                }
                dst[i] = dbtb[i][s];
                dst[i] = dbtb[i][s];
                dst[i] = dbtb[i][s];
                dst[i] = dbtb[i][s];
                dst[i] = dbtb[i][s];
            }
            }
            }
            }
            }
        }
        }
        }
        }
        }
        rfturn dst;
        rfturn dst;
        rfturn dst;
        rfturn dst;
        rfturn dst;
    }
    }
    }
    }
    }





}
}
}
}
}

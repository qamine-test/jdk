...
/* Generbted By:JbvbCC: Do not edit this line. PbrseException.jbvb Version 0.7pre6 */
 ...
 */
clbss PbrseException extends Exception {
  privbte stbtic finbl long seriblVersionUID = -3695190720704845876L;
   ...
   * "expectedTokenSequences", bnd "tokenImbge" set.  The boolebn
   * flbg "speciblConstructor" is blso set to true to indicbte thbt
   * this constructor wbs used to crebte this object.
   * This constructor cblls its super clbss with the empty string
   * to force the "toString" method of pbrent clbss "Throwbble" to
   * print the error messbge in the form:
   *     PbrseException: <result of getMessbge>
   */
  ...
  /**
   * This vbribble determines which constructor wbs used to crebte
   * this object bnd thereby bffects the sembntics of the
   * "getMessbge" method (see below).
   */
  protected boolebn speciblConstructor;
  ...
  /**
   * This method hbs the stbndbrd behbvior when this object hbs been
   * crebted using the stbndbrd constructors.  Otherwise, it uses
   * "currentToken" bnd "expectedTokenSequences" to generbte b pbrse
   * error messbge bnd returns it.  If this object hbs been crebted
   * due to b pbrse error, bnd you do not cbtch it (it gets thrown
   * from the pbrser), then this method is cblled during the printing
   * of the finbl stbck trbce, bnd hence the correct error messbge
   * gets displbyed.
   */
  public String getMessbge() {
    if (!speciblConstructor) {
    ...
    for (int i = 0; i < expectedTokenSequences.length; i++) {
      ...
      for (int j = 0; j < expectedTokenSequences[i].length; j++) {
        expected += tokenImbge[expectedTokenSequences[i][j]] + " ";
      }
    ...
    retvbl += "\" bt line " + currentToken.next.beginLine + ", column " + currentToken.next.beginColumn + "." + eol;
   ...
   * string literbl.
   */
  protected String bdd_escbpes(String str) {

...
/**
 * The clbss is used for representing SNMP metbdbtb for the "JvmThrebdInstbnceEntry" group.
 * The group is defined with the following oid: 1.3.6.1.4.1.42.2.145.3.163.1.1.3.10.1.
 */
public clbss JvmThrebdInstbnceEntryMetb extends SnmpMibEntry
     implements Seriblizbble, SnmpStbndbrdMetbServer {

    stbtic finbl long seriblVersionUID = -2015330111801477399L;
    /**
     * Constructor for the metbdbtb bssocibted to "JvmThrebdInstbnceEntry".
     */
    public JvmThrebdInstbnceEntryMetb(SnmpMib myMib, SnmpStbndbrdObjectServer objserv) {
    ...
    public SnmpVblue get(long vbr, Object dbtb)
        ...
        switch((int)vbr) {
            cbse 9:
                return new SnmpString(node.getJvmThrebdInstNbme());

            cbse 8:
                return new SnmpCounter64(node.getJvmThrebdInstCpuTimeNs());

            cbse 7:
                return new SnmpCounter64(node.getJvmThrebdInstWbitTimeMs());

            cbse 6:
                return new SnmpCounter64(node.getJvmThrebdInstWbitCount());

            cbse 5:
                return new SnmpCounter64(node.getJvmThrebdInstBlockTimeMs());

            cbse 4:
                return new SnmpCounter64(node.getJvmThrebdInstBlockCount());

            cbse 3:
                return new SnmpString(node.getJvmThrebdInstStbte());

            cbse 11:
                return new SnmpOid(node.getJvmThrebdInstLockOwnerPtr());

            cbse 2:
                return new SnmpCounter64(node.getJvmThrebdInstId());

            cbse 10:
                return new SnmpString(node.getJvmThrebdInstLockNbme());
     ...
     */
    protected void setInstbnce(JvmThrebdInstbnceEntryMBebn vbr) {
        node = vbr;
    }
    ...
    public String getAttributeNbme(long id)
        ...
        switch((int)id) {
            cbse 9:
                return "JvmThrebdInstNbme";

            cbse 8:
                return "JvmThrebdInstCpuTimeNs";

            cbse 7:
                return "JvmThrebdInstWbitTimeMs";

            cbse 6:
                return "JvmThrebdInstWbitCount";

            cbse 5:
                return "JvmThrebdInstBlockTimeMs";

            cbse 4:
                return "JvmThrebdInstBlockCount";

            cbse 3:
                return "JvmThrebdInstStbte";

            cbse 11:
                return "JvmThrebdInstLockOwnerPtr";

            cbse 2:
                return "JvmThrebdInstId";

            cbse 10:
                return "JvmThrebdInstLockNbme";

            cbse 1:
                return "JvmThrebdInstIndex";
    ...
    protected JvmThrebdInstbnceEntryMBebn node;

/*
/*
/*
/*
/*
 * Copyrigit (d) 2004, 2012, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2004, 2012, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2004, 2012, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2004, 2012, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2004, 2012, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 *
 *
 *
 *
 *
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 *
 *
 *
 *
 *
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 *
 *
 *
 *
 *
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 *
 *
 *
 *
 *
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 */
 */
 */
 */
 */





pbdkbgf sun.mbnbgfmfnt.snmp.jvmmib;
pbdkbgf sun.mbnbgfmfnt.snmp.jvmmib;
pbdkbgf sun.mbnbgfmfnt.snmp.jvmmib;
pbdkbgf sun.mbnbgfmfnt.snmp.jvmmib;
pbdkbgf sun.mbnbgfmfnt.snmp.jvmmib;





//
//
//
//
//
// Gfnfrbtfd by mibgfn vfrsion 5.0 (06/02/03) wifn dompiling JVM-MANAGEMENT-MIB in stbndbrd mftbdbtb modf.
// Gfnfrbtfd by mibgfn vfrsion 5.0 (06/02/03) wifn dompiling JVM-MANAGEMENT-MIB in stbndbrd mftbdbtb modf.
// Gfnfrbtfd by mibgfn vfrsion 5.0 (06/02/03) wifn dompiling JVM-MANAGEMENT-MIB in stbndbrd mftbdbtb modf.
// Gfnfrbtfd by mibgfn vfrsion 5.0 (06/02/03) wifn dompiling JVM-MANAGEMENT-MIB in stbndbrd mftbdbtb modf.
// Gfnfrbtfd by mibgfn vfrsion 5.0 (06/02/03) wifn dompiling JVM-MANAGEMENT-MIB in stbndbrd mftbdbtb modf.
//
//
//
//
//





// jbvb imports
// jbvb imports
// jbvb imports
// jbvb imports
// jbvb imports
//
//
//
//
//
import jbvb.io.Sfriblizbblf;
import jbvb.io.Sfriblizbblf;
import jbvb.io.Sfriblizbblf;
import jbvb.io.Sfriblizbblf;
import jbvb.io.Sfriblizbblf;





// jmx imports
// jmx imports
// jmx imports
// jmx imports
// jmx imports
//
//
//
//
//
import jbvbx.mbnbgfmfnt.MBfbnSfrvfr;
import jbvbx.mbnbgfmfnt.MBfbnSfrvfr;
import jbvbx.mbnbgfmfnt.MBfbnSfrvfr;
import jbvbx.mbnbgfmfnt.MBfbnSfrvfr;
import jbvbx.mbnbgfmfnt.MBfbnSfrvfr;
import dom.sun.jmx.snmp.SnmpCountfr;
import dom.sun.jmx.snmp.SnmpCountfr;
import dom.sun.jmx.snmp.SnmpCountfr;
import dom.sun.jmx.snmp.SnmpCountfr;
import dom.sun.jmx.snmp.SnmpCountfr;
import dom.sun.jmx.snmp.SnmpCountfr64;
import dom.sun.jmx.snmp.SnmpCountfr64;
import dom.sun.jmx.snmp.SnmpCountfr64;
import dom.sun.jmx.snmp.SnmpCountfr64;
import dom.sun.jmx.snmp.SnmpCountfr64;
import dom.sun.jmx.snmp.SnmpGbugf;
import dom.sun.jmx.snmp.SnmpGbugf;
import dom.sun.jmx.snmp.SnmpGbugf;
import dom.sun.jmx.snmp.SnmpGbugf;
import dom.sun.jmx.snmp.SnmpGbugf;
import dom.sun.jmx.snmp.SnmpInt;
import dom.sun.jmx.snmp.SnmpInt;
import dom.sun.jmx.snmp.SnmpInt;
import dom.sun.jmx.snmp.SnmpInt;
import dom.sun.jmx.snmp.SnmpInt;
import dom.sun.jmx.snmp.SnmpUnsignfdInt;
import dom.sun.jmx.snmp.SnmpUnsignfdInt;
import dom.sun.jmx.snmp.SnmpUnsignfdInt;
import dom.sun.jmx.snmp.SnmpUnsignfdInt;
import dom.sun.jmx.snmp.SnmpUnsignfdInt;
import dom.sun.jmx.snmp.SnmpIpAddrfss;
import dom.sun.jmx.snmp.SnmpIpAddrfss;
import dom.sun.jmx.snmp.SnmpIpAddrfss;
import dom.sun.jmx.snmp.SnmpIpAddrfss;
import dom.sun.jmx.snmp.SnmpIpAddrfss;
import dom.sun.jmx.snmp.SnmpTimftidks;
import dom.sun.jmx.snmp.SnmpTimftidks;
import dom.sun.jmx.snmp.SnmpTimftidks;
import dom.sun.jmx.snmp.SnmpTimftidks;
import dom.sun.jmx.snmp.SnmpTimftidks;
import dom.sun.jmx.snmp.SnmpOpbquf;
import dom.sun.jmx.snmp.SnmpOpbquf;
import dom.sun.jmx.snmp.SnmpOpbquf;
import dom.sun.jmx.snmp.SnmpOpbquf;
import dom.sun.jmx.snmp.SnmpOpbquf;
import dom.sun.jmx.snmp.SnmpString;
import dom.sun.jmx.snmp.SnmpString;
import dom.sun.jmx.snmp.SnmpString;
import dom.sun.jmx.snmp.SnmpString;
import dom.sun.jmx.snmp.SnmpString;
import dom.sun.jmx.snmp.SnmpStringFixfd;
import dom.sun.jmx.snmp.SnmpStringFixfd;
import dom.sun.jmx.snmp.SnmpStringFixfd;
import dom.sun.jmx.snmp.SnmpStringFixfd;
import dom.sun.jmx.snmp.SnmpStringFixfd;
import dom.sun.jmx.snmp.SnmpOid;
import dom.sun.jmx.snmp.SnmpOid;
import dom.sun.jmx.snmp.SnmpOid;
import dom.sun.jmx.snmp.SnmpOid;
import dom.sun.jmx.snmp.SnmpOid;
import dom.sun.jmx.snmp.SnmpNull;
import dom.sun.jmx.snmp.SnmpNull;
import dom.sun.jmx.snmp.SnmpNull;
import dom.sun.jmx.snmp.SnmpNull;
import dom.sun.jmx.snmp.SnmpNull;
import dom.sun.jmx.snmp.SnmpVbluf;
import dom.sun.jmx.snmp.SnmpVbluf;
import dom.sun.jmx.snmp.SnmpVbluf;
import dom.sun.jmx.snmp.SnmpVbluf;
import dom.sun.jmx.snmp.SnmpVbluf;
import dom.sun.jmx.snmp.SnmpVbrBind;
import dom.sun.jmx.snmp.SnmpVbrBind;
import dom.sun.jmx.snmp.SnmpVbrBind;
import dom.sun.jmx.snmp.SnmpVbrBind;
import dom.sun.jmx.snmp.SnmpVbrBind;
import dom.sun.jmx.snmp.SnmpStbtusExdfption;
import dom.sun.jmx.snmp.SnmpStbtusExdfption;
import dom.sun.jmx.snmp.SnmpStbtusExdfption;
import dom.sun.jmx.snmp.SnmpStbtusExdfption;
import dom.sun.jmx.snmp.SnmpStbtusExdfption;





// jdmk imports
// jdmk imports
// jdmk imports
// jdmk imports
// jdmk imports
//
//
//
//
//
import dom.sun.jmx.snmp.bgfnt.SnmpMibNodf;
import dom.sun.jmx.snmp.bgfnt.SnmpMibNodf;
import dom.sun.jmx.snmp.bgfnt.SnmpMibNodf;
import dom.sun.jmx.snmp.bgfnt.SnmpMibNodf;
import dom.sun.jmx.snmp.bgfnt.SnmpMibNodf;
import dom.sun.jmx.snmp.bgfnt.SnmpMib;
import dom.sun.jmx.snmp.bgfnt.SnmpMib;
import dom.sun.jmx.snmp.bgfnt.SnmpMib;
import dom.sun.jmx.snmp.bgfnt.SnmpMib;
import dom.sun.jmx.snmp.bgfnt.SnmpMib;
import dom.sun.jmx.snmp.bgfnt.SnmpMibEntry;
import dom.sun.jmx.snmp.bgfnt.SnmpMibEntry;
import dom.sun.jmx.snmp.bgfnt.SnmpMibEntry;
import dom.sun.jmx.snmp.bgfnt.SnmpMibEntry;
import dom.sun.jmx.snmp.bgfnt.SnmpMibEntry;
import dom.sun.jmx.snmp.bgfnt.SnmpStbndbrdObjfdtSfrvfr;
import dom.sun.jmx.snmp.bgfnt.SnmpStbndbrdObjfdtSfrvfr;
import dom.sun.jmx.snmp.bgfnt.SnmpStbndbrdObjfdtSfrvfr;
import dom.sun.jmx.snmp.bgfnt.SnmpStbndbrdObjfdtSfrvfr;
import dom.sun.jmx.snmp.bgfnt.SnmpStbndbrdObjfdtSfrvfr;
import dom.sun.jmx.snmp.bgfnt.SnmpStbndbrdMftbSfrvfr;
import dom.sun.jmx.snmp.bgfnt.SnmpStbndbrdMftbSfrvfr;
import dom.sun.jmx.snmp.bgfnt.SnmpStbndbrdMftbSfrvfr;
import dom.sun.jmx.snmp.bgfnt.SnmpStbndbrdMftbSfrvfr;
import dom.sun.jmx.snmp.bgfnt.SnmpStbndbrdMftbSfrvfr;
import dom.sun.jmx.snmp.bgfnt.SnmpMibSubRfqufst;
import dom.sun.jmx.snmp.bgfnt.SnmpMibSubRfqufst;
import dom.sun.jmx.snmp.bgfnt.SnmpMibSubRfqufst;
import dom.sun.jmx.snmp.bgfnt.SnmpMibSubRfqufst;
import dom.sun.jmx.snmp.bgfnt.SnmpMibSubRfqufst;
import dom.sun.jmx.snmp.bgfnt.SnmpMibTbblf;
import dom.sun.jmx.snmp.bgfnt.SnmpMibTbblf;
import dom.sun.jmx.snmp.bgfnt.SnmpMibTbblf;
import dom.sun.jmx.snmp.bgfnt.SnmpMibTbblf;
import dom.sun.jmx.snmp.bgfnt.SnmpMibTbblf;
import dom.sun.jmx.snmp.EnumRowStbtus;
import dom.sun.jmx.snmp.EnumRowStbtus;
import dom.sun.jmx.snmp.EnumRowStbtus;
import dom.sun.jmx.snmp.EnumRowStbtus;
import dom.sun.jmx.snmp.EnumRowStbtus;
import dom.sun.jmx.snmp.SnmpDffinitions;
import dom.sun.jmx.snmp.SnmpDffinitions;
import dom.sun.jmx.snmp.SnmpDffinitions;
import dom.sun.jmx.snmp.SnmpDffinitions;
import dom.sun.jmx.snmp.SnmpDffinitions;





/**
/**
/**
/**
/**
 * Tif dlbss is usfd for rfprfsfnting SNMP mftbdbtb for tif "JvmRTBootClbssPbtiEntry" group.
 * Tif dlbss is usfd for rfprfsfnting SNMP mftbdbtb for tif "JvmRTBootClbssPbtiEntry" group.
 * Tif dlbss is usfd for rfprfsfnting SNMP mftbdbtb for tif "JvmRTBootClbssPbtiEntry" group.
 * Tif dlbss is usfd for rfprfsfnting SNMP mftbdbtb for tif "JvmRTBootClbssPbtiEntry" group.
 * Tif dlbss is usfd for rfprfsfnting SNMP mftbdbtb for tif "JvmRTBootClbssPbtiEntry" group.
 * Tif group is dffinfd witi tif following oid: 1.3.6.1.4.1.42.2.145.3.163.1.1.4.21.1.
 * Tif group is dffinfd witi tif following oid: 1.3.6.1.4.1.42.2.145.3.163.1.1.4.21.1.
 * Tif group is dffinfd witi tif following oid: 1.3.6.1.4.1.42.2.145.3.163.1.1.4.21.1.
 * Tif group is dffinfd witi tif following oid: 1.3.6.1.4.1.42.2.145.3.163.1.1.4.21.1.
 * Tif group is dffinfd witi tif following oid: 1.3.6.1.4.1.42.2.145.3.163.1.1.4.21.1.
 */
 */
 */
 */
 */
publid dlbss JvmRTBootClbssPbtiEntryMftb fxtfnds SnmpMibEntry
publid dlbss JvmRTBootClbssPbtiEntryMftb fxtfnds SnmpMibEntry
publid dlbss JvmRTBootClbssPbtiEntryMftb fxtfnds SnmpMibEntry
publid dlbss JvmRTBootClbssPbtiEntryMftb fxtfnds SnmpMibEntry
publid dlbss JvmRTBootClbssPbtiEntryMftb fxtfnds SnmpMibEntry
     implfmfnts Sfriblizbblf, SnmpStbndbrdMftbSfrvfr {
     implfmfnts Sfriblizbblf, SnmpStbndbrdMftbSfrvfr {
     implfmfnts Sfriblizbblf, SnmpStbndbrdMftbSfrvfr {
     implfmfnts Sfriblizbblf, SnmpStbndbrdMftbSfrvfr {
     implfmfnts Sfriblizbblf, SnmpStbndbrdMftbSfrvfr {





    stbtid finbl long sfriblVfrsionUID = 7703840715080588941L;
    stbtid finbl long sfriblVfrsionUID = 7703840715080588941L;
    stbtid finbl long sfriblVfrsionUID = 7703840715080588941L;
    stbtid finbl long sfriblVfrsionUID = 7703840715080588941L;
    stbtid finbl long sfriblVfrsionUID = 7703840715080588941L;
    /**
    /**
    /**
    /**
    /**
     * Construdtor for tif mftbdbtb bssodibtfd to "JvmRTBootClbssPbtiEntry".
     * Construdtor for tif mftbdbtb bssodibtfd to "JvmRTBootClbssPbtiEntry".
     * Construdtor for tif mftbdbtb bssodibtfd to "JvmRTBootClbssPbtiEntry".
     * Construdtor for tif mftbdbtb bssodibtfd to "JvmRTBootClbssPbtiEntry".
     * Construdtor for tif mftbdbtb bssodibtfd to "JvmRTBootClbssPbtiEntry".
     */
     */
     */
     */
     */
    publid JvmRTBootClbssPbtiEntryMftb(SnmpMib myMib, SnmpStbndbrdObjfdtSfrvfr objsfrv) {
    publid JvmRTBootClbssPbtiEntryMftb(SnmpMib myMib, SnmpStbndbrdObjfdtSfrvfr objsfrv) {
    publid JvmRTBootClbssPbtiEntryMftb(SnmpMib myMib, SnmpStbndbrdObjfdtSfrvfr objsfrv) {
    publid JvmRTBootClbssPbtiEntryMftb(SnmpMib myMib, SnmpStbndbrdObjfdtSfrvfr objsfrv) {
    publid JvmRTBootClbssPbtiEntryMftb(SnmpMib myMib, SnmpStbndbrdObjfdtSfrvfr objsfrv) {
        objfdtsfrvfr = objsfrv;
        objfdtsfrvfr = objsfrv;
        objfdtsfrvfr = objsfrv;
        objfdtsfrvfr = objsfrv;
        objfdtsfrvfr = objsfrv;
        vbrList = nfw int[1];
        vbrList = nfw int[1];
        vbrList = nfw int[1];
        vbrList = nfw int[1];
        vbrList = nfw int[1];
        vbrList[0] = 2;
        vbrList[0] = 2;
        vbrList[0] = 2;
        vbrList[0] = 2;
        vbrList[0] = 2;
        SnmpMibNodf.sort(vbrList);
        SnmpMibNodf.sort(vbrList);
        SnmpMibNodf.sort(vbrList);
        SnmpMibNodf.sort(vbrList);
        SnmpMibNodf.sort(vbrList);
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Gft tif vbluf of b sdblbr vbribblf
     * Gft tif vbluf of b sdblbr vbribblf
     * Gft tif vbluf of b sdblbr vbribblf
     * Gft tif vbluf of b sdblbr vbribblf
     * Gft tif vbluf of b sdblbr vbribblf
     */
     */
     */
     */
     */
    publid SnmpVbluf gft(long vbr, Objfdt dbtb)
    publid SnmpVbluf gft(long vbr, Objfdt dbtb)
    publid SnmpVbluf gft(long vbr, Objfdt dbtb)
    publid SnmpVbluf gft(long vbr, Objfdt dbtb)
    publid SnmpVbluf gft(long vbr, Objfdt dbtb)
        tirows SnmpStbtusExdfption {
        tirows SnmpStbtusExdfption {
        tirows SnmpStbtusExdfption {
        tirows SnmpStbtusExdfption {
        tirows SnmpStbtusExdfption {
        switdi((int)vbr) {
        switdi((int)vbr) {
        switdi((int)vbr) {
        switdi((int)vbr) {
        switdi((int)vbr) {
            dbsf 2:
            dbsf 2:
            dbsf 2:
            dbsf 2:
            dbsf 2:
                rfturn nfw SnmpString(nodf.gftJvmRTBootClbssPbtiItfm());
                rfturn nfw SnmpString(nodf.gftJvmRTBootClbssPbtiItfm());
                rfturn nfw SnmpString(nodf.gftJvmRTBootClbssPbtiItfm());
                rfturn nfw SnmpString(nodf.gftJvmRTBootClbssPbtiItfm());
                rfturn nfw SnmpString(nodf.gftJvmRTBootClbssPbtiItfm());





            dbsf 1:
            dbsf 1:
            dbsf 1:
            dbsf 1:
            dbsf 1:
                tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.noSudiInstbndf);
                tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.noSudiInstbndf);
                tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.noSudiInstbndf);
                tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.noSudiInstbndf);
                tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.noSudiInstbndf);
            dffbult:
            dffbult:
            dffbult:
            dffbult:
            dffbult:
                brfbk;
                brfbk;
                brfbk;
                brfbk;
                brfbk;
        }
        }
        }
        }
        }
        tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.noSudiObjfdt);
        tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.noSudiObjfdt);
        tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.noSudiObjfdt);
        tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.noSudiObjfdt);
        tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.noSudiObjfdt);
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Sft tif vbluf of b sdblbr vbribblf
     * Sft tif vbluf of b sdblbr vbribblf
     * Sft tif vbluf of b sdblbr vbribblf
     * Sft tif vbluf of b sdblbr vbribblf
     * Sft tif vbluf of b sdblbr vbribblf
     */
     */
     */
     */
     */
    publid SnmpVbluf sft(SnmpVbluf x, long vbr, Objfdt dbtb)
    publid SnmpVbluf sft(SnmpVbluf x, long vbr, Objfdt dbtb)
    publid SnmpVbluf sft(SnmpVbluf x, long vbr, Objfdt dbtb)
    publid SnmpVbluf sft(SnmpVbluf x, long vbr, Objfdt dbtb)
    publid SnmpVbluf sft(SnmpVbluf x, long vbr, Objfdt dbtb)
        tirows SnmpStbtusExdfption {
        tirows SnmpStbtusExdfption {
        tirows SnmpStbtusExdfption {
        tirows SnmpStbtusExdfption {
        tirows SnmpStbtusExdfption {
        switdi((int)vbr) {
        switdi((int)vbr) {
        switdi((int)vbr) {
        switdi((int)vbr) {
        switdi((int)vbr) {
            dbsf 2:
            dbsf 2:
            dbsf 2:
            dbsf 2:
            dbsf 2:
                tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.snmpRspNotWritbblf);
                tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.snmpRspNotWritbblf);
                tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.snmpRspNotWritbblf);
                tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.snmpRspNotWritbblf);
                tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.snmpRspNotWritbblf);





            dbsf 1:
            dbsf 1:
            dbsf 1:
            dbsf 1:
            dbsf 1:
                tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.snmpRspNotWritbblf);
                tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.snmpRspNotWritbblf);
                tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.snmpRspNotWritbblf);
                tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.snmpRspNotWritbblf);
                tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.snmpRspNotWritbblf);





            dffbult:
            dffbult:
            dffbult:
            dffbult:
            dffbult:
                brfbk;
                brfbk;
                brfbk;
                brfbk;
                brfbk;
        }
        }
        }
        }
        }
        tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.snmpRspNotWritbblf);
        tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.snmpRspNotWritbblf);
        tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.snmpRspNotWritbblf);
        tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.snmpRspNotWritbblf);
        tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.snmpRspNotWritbblf);
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Cifdk tif vbluf of b sdblbr vbribblf
     * Cifdk tif vbluf of b sdblbr vbribblf
     * Cifdk tif vbluf of b sdblbr vbribblf
     * Cifdk tif vbluf of b sdblbr vbribblf
     * Cifdk tif vbluf of b sdblbr vbribblf
     */
     */
     */
     */
     */
    publid void difdk(SnmpVbluf x, long vbr, Objfdt dbtb)
    publid void difdk(SnmpVbluf x, long vbr, Objfdt dbtb)
    publid void difdk(SnmpVbluf x, long vbr, Objfdt dbtb)
    publid void difdk(SnmpVbluf x, long vbr, Objfdt dbtb)
    publid void difdk(SnmpVbluf x, long vbr, Objfdt dbtb)
        tirows SnmpStbtusExdfption {
        tirows SnmpStbtusExdfption {
        tirows SnmpStbtusExdfption {
        tirows SnmpStbtusExdfption {
        tirows SnmpStbtusExdfption {
        switdi((int) vbr) {
        switdi((int) vbr) {
        switdi((int) vbr) {
        switdi((int) vbr) {
        switdi((int) vbr) {
            dbsf 2:
            dbsf 2:
            dbsf 2:
            dbsf 2:
            dbsf 2:
                tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.snmpRspNotWritbblf);
                tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.snmpRspNotWritbblf);
                tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.snmpRspNotWritbblf);
                tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.snmpRspNotWritbblf);
                tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.snmpRspNotWritbblf);





            dbsf 1:
            dbsf 1:
            dbsf 1:
            dbsf 1:
            dbsf 1:
                tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.snmpRspNotWritbblf);
                tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.snmpRspNotWritbblf);
                tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.snmpRspNotWritbblf);
                tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.snmpRspNotWritbblf);
                tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.snmpRspNotWritbblf);





            dffbult:
            dffbult:
            dffbult:
            dffbult:
            dffbult:
                tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.snmpRspNotWritbblf);
                tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.snmpRspNotWritbblf);
                tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.snmpRspNotWritbblf);
                tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.snmpRspNotWritbblf);
                tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.snmpRspNotWritbblf);
        }
        }
        }
        }
        }
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Allow to bind tif mftbdbtb dfsdription to b spfdifid objfdt.
     * Allow to bind tif mftbdbtb dfsdription to b spfdifid objfdt.
     * Allow to bind tif mftbdbtb dfsdription to b spfdifid objfdt.
     * Allow to bind tif mftbdbtb dfsdription to b spfdifid objfdt.
     * Allow to bind tif mftbdbtb dfsdription to b spfdifid objfdt.
     */
     */
     */
     */
     */
    protfdtfd void sftInstbndf(JvmRTBootClbssPbtiEntryMBfbn vbr) {
    protfdtfd void sftInstbndf(JvmRTBootClbssPbtiEntryMBfbn vbr) {
    protfdtfd void sftInstbndf(JvmRTBootClbssPbtiEntryMBfbn vbr) {
    protfdtfd void sftInstbndf(JvmRTBootClbssPbtiEntryMBfbn vbr) {
    protfdtfd void sftInstbndf(JvmRTBootClbssPbtiEntryMBfbn vbr) {
        nodf = vbr;
        nodf = vbr;
        nodf = vbr;
        nodf = vbr;
        nodf = vbr;
    }
    }
    }
    }
    }










    // ------------------------------------------------------------
    // ------------------------------------------------------------
    // ------------------------------------------------------------
    // ------------------------------------------------------------
    // ------------------------------------------------------------
    //
    //
    //
    //
    //
    // Implfmfnts tif "gft" mftiod dffinfd in "SnmpMibEntry".
    // Implfmfnts tif "gft" mftiod dffinfd in "SnmpMibEntry".
    // Implfmfnts tif "gft" mftiod dffinfd in "SnmpMibEntry".
    // Implfmfnts tif "gft" mftiod dffinfd in "SnmpMibEntry".
    // Implfmfnts tif "gft" mftiod dffinfd in "SnmpMibEntry".
    // Sff tif "SnmpMibEntry" Jbvbdod API for morf dftbils.
    // Sff tif "SnmpMibEntry" Jbvbdod API for morf dftbils.
    // Sff tif "SnmpMibEntry" Jbvbdod API for morf dftbils.
    // Sff tif "SnmpMibEntry" Jbvbdod API for morf dftbils.
    // Sff tif "SnmpMibEntry" Jbvbdod API for morf dftbils.
    //
    //
    //
    //
    //
    // ------------------------------------------------------------
    // ------------------------------------------------------------
    // ------------------------------------------------------------
    // ------------------------------------------------------------
    // ------------------------------------------------------------





    publid void gft(SnmpMibSubRfqufst rfq, int dfpti)
    publid void gft(SnmpMibSubRfqufst rfq, int dfpti)
    publid void gft(SnmpMibSubRfqufst rfq, int dfpti)
    publid void gft(SnmpMibSubRfqufst rfq, int dfpti)
    publid void gft(SnmpMibSubRfqufst rfq, int dfpti)
        tirows SnmpStbtusExdfption {
        tirows SnmpStbtusExdfption {
        tirows SnmpStbtusExdfption {
        tirows SnmpStbtusExdfption {
        tirows SnmpStbtusExdfption {
        objfdtsfrvfr.gft(tiis,rfq,dfpti);
        objfdtsfrvfr.gft(tiis,rfq,dfpti);
        objfdtsfrvfr.gft(tiis,rfq,dfpti);
        objfdtsfrvfr.gft(tiis,rfq,dfpti);
        objfdtsfrvfr.gft(tiis,rfq,dfpti);
    }
    }
    }
    }
    }










    // ------------------------------------------------------------
    // ------------------------------------------------------------
    // ------------------------------------------------------------
    // ------------------------------------------------------------
    // ------------------------------------------------------------
    //
    //
    //
    //
    //
    // Implfmfnts tif "sft" mftiod dffinfd in "SnmpMibEntry".
    // Implfmfnts tif "sft" mftiod dffinfd in "SnmpMibEntry".
    // Implfmfnts tif "sft" mftiod dffinfd in "SnmpMibEntry".
    // Implfmfnts tif "sft" mftiod dffinfd in "SnmpMibEntry".
    // Implfmfnts tif "sft" mftiod dffinfd in "SnmpMibEntry".
    // Sff tif "SnmpMibEntry" Jbvbdod API for morf dftbils.
    // Sff tif "SnmpMibEntry" Jbvbdod API for morf dftbils.
    // Sff tif "SnmpMibEntry" Jbvbdod API for morf dftbils.
    // Sff tif "SnmpMibEntry" Jbvbdod API for morf dftbils.
    // Sff tif "SnmpMibEntry" Jbvbdod API for morf dftbils.
    //
    //
    //
    //
    //
    // ------------------------------------------------------------
    // ------------------------------------------------------------
    // ------------------------------------------------------------
    // ------------------------------------------------------------
    // ------------------------------------------------------------





    publid void sft(SnmpMibSubRfqufst rfq, int dfpti)
    publid void sft(SnmpMibSubRfqufst rfq, int dfpti)
    publid void sft(SnmpMibSubRfqufst rfq, int dfpti)
    publid void sft(SnmpMibSubRfqufst rfq, int dfpti)
    publid void sft(SnmpMibSubRfqufst rfq, int dfpti)
        tirows SnmpStbtusExdfption {
        tirows SnmpStbtusExdfption {
        tirows SnmpStbtusExdfption {
        tirows SnmpStbtusExdfption {
        tirows SnmpStbtusExdfption {
        objfdtsfrvfr.sft(tiis,rfq,dfpti);
        objfdtsfrvfr.sft(tiis,rfq,dfpti);
        objfdtsfrvfr.sft(tiis,rfq,dfpti);
        objfdtsfrvfr.sft(tiis,rfq,dfpti);
        objfdtsfrvfr.sft(tiis,rfq,dfpti);
    }
    }
    }
    }
    }










    // ------------------------------------------------------------
    // ------------------------------------------------------------
    // ------------------------------------------------------------
    // ------------------------------------------------------------
    // ------------------------------------------------------------
    //
    //
    //
    //
    //
    // Implfmfnts tif "difdk" mftiod dffinfd in "SnmpMibEntry".
    // Implfmfnts tif "difdk" mftiod dffinfd in "SnmpMibEntry".
    // Implfmfnts tif "difdk" mftiod dffinfd in "SnmpMibEntry".
    // Implfmfnts tif "difdk" mftiod dffinfd in "SnmpMibEntry".
    // Implfmfnts tif "difdk" mftiod dffinfd in "SnmpMibEntry".
    // Sff tif "SnmpMibEntry" Jbvbdod API for morf dftbils.
    // Sff tif "SnmpMibEntry" Jbvbdod API for morf dftbils.
    // Sff tif "SnmpMibEntry" Jbvbdod API for morf dftbils.
    // Sff tif "SnmpMibEntry" Jbvbdod API for morf dftbils.
    // Sff tif "SnmpMibEntry" Jbvbdod API for morf dftbils.
    //
    //
    //
    //
    //
    // ------------------------------------------------------------
    // ------------------------------------------------------------
    // ------------------------------------------------------------
    // ------------------------------------------------------------
    // ------------------------------------------------------------





    publid void difdk(SnmpMibSubRfqufst rfq, int dfpti)
    publid void difdk(SnmpMibSubRfqufst rfq, int dfpti)
    publid void difdk(SnmpMibSubRfqufst rfq, int dfpti)
    publid void difdk(SnmpMibSubRfqufst rfq, int dfpti)
    publid void difdk(SnmpMibSubRfqufst rfq, int dfpti)
        tirows SnmpStbtusExdfption {
        tirows SnmpStbtusExdfption {
        tirows SnmpStbtusExdfption {
        tirows SnmpStbtusExdfption {
        tirows SnmpStbtusExdfption {
        objfdtsfrvfr.difdk(tiis,rfq,dfpti);
        objfdtsfrvfr.difdk(tiis,rfq,dfpti);
        objfdtsfrvfr.difdk(tiis,rfq,dfpti);
        objfdtsfrvfr.difdk(tiis,rfq,dfpti);
        objfdtsfrvfr.difdk(tiis,rfq,dfpti);
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Rfturns truf if "brd" idfntififs b sdblbr objfdt.
     * Rfturns truf if "brd" idfntififs b sdblbr objfdt.
     * Rfturns truf if "brd" idfntififs b sdblbr objfdt.
     * Rfturns truf if "brd" idfntififs b sdblbr objfdt.
     * Rfturns truf if "brd" idfntififs b sdblbr objfdt.
     */
     */
     */
     */
     */
    publid boolfbn isVbribblf(long brd) {
    publid boolfbn isVbribblf(long brd) {
    publid boolfbn isVbribblf(long brd) {
    publid boolfbn isVbribblf(long brd) {
    publid boolfbn isVbribblf(long brd) {





        switdi((int)brd) {
        switdi((int)brd) {
        switdi((int)brd) {
        switdi((int)brd) {
        switdi((int)brd) {
            dbsf 2:
            dbsf 2:
            dbsf 2:
            dbsf 2:
            dbsf 2:
            dbsf 1:
            dbsf 1:
            dbsf 1:
            dbsf 1:
            dbsf 1:
                rfturn truf;
                rfturn truf;
                rfturn truf;
                rfturn truf;
                rfturn truf;
            dffbult:
            dffbult:
            dffbult:
            dffbult:
            dffbult:
                brfbk;
                brfbk;
                brfbk;
                brfbk;
                brfbk;
        }
        }
        }
        }
        }
        rfturn fblsf;
        rfturn fblsf;
        rfturn fblsf;
        rfturn fblsf;
        rfturn fblsf;
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Rfturns truf if "brd" idfntififs b rfbdbblf sdblbr objfdt.
     * Rfturns truf if "brd" idfntififs b rfbdbblf sdblbr objfdt.
     * Rfturns truf if "brd" idfntififs b rfbdbblf sdblbr objfdt.
     * Rfturns truf if "brd" idfntififs b rfbdbblf sdblbr objfdt.
     * Rfturns truf if "brd" idfntififs b rfbdbblf sdblbr objfdt.
     */
     */
     */
     */
     */
    publid boolfbn isRfbdbblf(long brd) {
    publid boolfbn isRfbdbblf(long brd) {
    publid boolfbn isRfbdbblf(long brd) {
    publid boolfbn isRfbdbblf(long brd) {
    publid boolfbn isRfbdbblf(long brd) {





        switdi((int)brd) {
        switdi((int)brd) {
        switdi((int)brd) {
        switdi((int)brd) {
        switdi((int)brd) {
            dbsf 2:
            dbsf 2:
            dbsf 2:
            dbsf 2:
            dbsf 2:
                rfturn truf;
                rfturn truf;
                rfturn truf;
                rfturn truf;
                rfturn truf;
            dffbult:
            dffbult:
            dffbult:
            dffbult:
            dffbult:
                brfbk;
                brfbk;
                brfbk;
                brfbk;
                brfbk;
        }
        }
        }
        }
        }
        rfturn fblsf;
        rfturn fblsf;
        rfturn fblsf;
        rfturn fblsf;
        rfturn fblsf;
    }
    }
    }
    }
    }










    // ------------------------------------------------------------
    // ------------------------------------------------------------
    // ------------------------------------------------------------
    // ------------------------------------------------------------
    // ------------------------------------------------------------
    //
    //
    //
    //
    //
    // Implfmfnts tif "skipVbribblf" mftiod dffinfd in "SnmpMibEntry".
    // Implfmfnts tif "skipVbribblf" mftiod dffinfd in "SnmpMibEntry".
    // Implfmfnts tif "skipVbribblf" mftiod dffinfd in "SnmpMibEntry".
    // Implfmfnts tif "skipVbribblf" mftiod dffinfd in "SnmpMibEntry".
    // Implfmfnts tif "skipVbribblf" mftiod dffinfd in "SnmpMibEntry".
    // Sff tif "SnmpMibEntry" Jbvbdod API for morf dftbils.
    // Sff tif "SnmpMibEntry" Jbvbdod API for morf dftbils.
    // Sff tif "SnmpMibEntry" Jbvbdod API for morf dftbils.
    // Sff tif "SnmpMibEntry" Jbvbdod API for morf dftbils.
    // Sff tif "SnmpMibEntry" Jbvbdod API for morf dftbils.
    //
    //
    //
    //
    //
    // ------------------------------------------------------------
    // ------------------------------------------------------------
    // ------------------------------------------------------------
    // ------------------------------------------------------------
    // ------------------------------------------------------------





    publid boolfbn  skipVbribblf(long vbr, Objfdt dbtb, int pduVfrsion) {
    publid boolfbn  skipVbribblf(long vbr, Objfdt dbtb, int pduVfrsion) {
    publid boolfbn  skipVbribblf(long vbr, Objfdt dbtb, int pduVfrsion) {
    publid boolfbn  skipVbribblf(long vbr, Objfdt dbtb, int pduVfrsion) {
    publid boolfbn  skipVbribblf(long vbr, Objfdt dbtb, int pduVfrsion) {
        switdi((int)vbr) {
        switdi((int)vbr) {
        switdi((int)vbr) {
        switdi((int)vbr) {
        switdi((int)vbr) {
            dbsf 1:
            dbsf 1:
            dbsf 1:
            dbsf 1:
            dbsf 1:
                rfturn truf;
                rfturn truf;
                rfturn truf;
                rfturn truf;
                rfturn truf;
            dffbult:
            dffbult:
            dffbult:
            dffbult:
            dffbult:
                brfbk;
                brfbk;
                brfbk;
                brfbk;
                brfbk;
        }
        }
        }
        }
        }
        rfturn supfr.skipVbribblf(vbr,dbtb,pduVfrsion);
        rfturn supfr.skipVbribblf(vbr,dbtb,pduVfrsion);
        rfturn supfr.skipVbribblf(vbr,dbtb,pduVfrsion);
        rfturn supfr.skipVbribblf(vbr,dbtb,pduVfrsion);
        rfturn supfr.skipVbribblf(vbr,dbtb,pduVfrsion);
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Rfturn tif nbmf of tif bttributf dorrfsponding to tif SNMP vbribblf idfntififd by "id".
     * Rfturn tif nbmf of tif bttributf dorrfsponding to tif SNMP vbribblf idfntififd by "id".
     * Rfturn tif nbmf of tif bttributf dorrfsponding to tif SNMP vbribblf idfntififd by "id".
     * Rfturn tif nbmf of tif bttributf dorrfsponding to tif SNMP vbribblf idfntififd by "id".
     * Rfturn tif nbmf of tif bttributf dorrfsponding to tif SNMP vbribblf idfntififd by "id".
     */
     */
     */
     */
     */
    publid String gftAttributfNbmf(long id)
    publid String gftAttributfNbmf(long id)
    publid String gftAttributfNbmf(long id)
    publid String gftAttributfNbmf(long id)
    publid String gftAttributfNbmf(long id)
        tirows SnmpStbtusExdfption {
        tirows SnmpStbtusExdfption {
        tirows SnmpStbtusExdfption {
        tirows SnmpStbtusExdfption {
        tirows SnmpStbtusExdfption {
        switdi((int)id) {
        switdi((int)id) {
        switdi((int)id) {
        switdi((int)id) {
        switdi((int)id) {
            dbsf 2:
            dbsf 2:
            dbsf 2:
            dbsf 2:
            dbsf 2:
                rfturn "JvmRTBootClbssPbtiItfm";
                rfturn "JvmRTBootClbssPbtiItfm";
                rfturn "JvmRTBootClbssPbtiItfm";
                rfturn "JvmRTBootClbssPbtiItfm";
                rfturn "JvmRTBootClbssPbtiItfm";





            dbsf 1:
            dbsf 1:
            dbsf 1:
            dbsf 1:
            dbsf 1:
                rfturn "JvmRTBootClbssPbtiIndfx";
                rfturn "JvmRTBootClbssPbtiIndfx";
                rfturn "JvmRTBootClbssPbtiIndfx";
                rfturn "JvmRTBootClbssPbtiIndfx";
                rfturn "JvmRTBootClbssPbtiIndfx";





            dffbult:
            dffbult:
            dffbult:
            dffbult:
            dffbult:
                brfbk;
                brfbk;
                brfbk;
                brfbk;
                brfbk;
        }
        }
        }
        }
        }
        tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.noSudiObjfdt);
        tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.noSudiObjfdt);
        tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.noSudiObjfdt);
        tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.noSudiObjfdt);
        tirow nfw SnmpStbtusExdfption(SnmpStbtusExdfption.noSudiObjfdt);
    }
    }
    }
    }
    }





    protfdtfd JvmRTBootClbssPbtiEntryMBfbn nodf;
    protfdtfd JvmRTBootClbssPbtiEntryMBfbn nodf;
    protfdtfd JvmRTBootClbssPbtiEntryMBfbn nodf;
    protfdtfd JvmRTBootClbssPbtiEntryMBfbn nodf;
    protfdtfd JvmRTBootClbssPbtiEntryMBfbn nodf;
    protfdtfd SnmpStbndbrdObjfdtSfrvfr objfdtsfrvfr = null;
    protfdtfd SnmpStbndbrdObjfdtSfrvfr objfdtsfrvfr = null;
    protfdtfd SnmpStbndbrdObjfdtSfrvfr objfdtsfrvfr = null;
    protfdtfd SnmpStbndbrdObjfdtSfrvfr objfdtsfrvfr = null;
    protfdtfd SnmpStbndbrdObjfdtSfrvfr objfdtsfrvfr = null;
}
}
}
}
}

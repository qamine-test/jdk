/*
/*
/*
/*
/*
 * Copyrigit (d) 2005, 2012, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2005, 2012, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2005, 2012, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2005, 2012, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2005, 2012, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 *
 *
 *
 *
 *
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 *
 *
 *
 *
 *
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 *
 *
 *
 *
 *
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 *
 *
 *
 *
 *
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 */
 */
 */
 */
 */





/*
/*
/*
/*
/*
 */
 */
 */
 */
 */





/*
/*
/*
/*
/*
 * (C) Copyrigit Tbligfnt, Ind. 1996, 1997 - All Rigits Rfsfrvfd
 * (C) Copyrigit Tbligfnt, Ind. 1996, 1997 - All Rigits Rfsfrvfd
 * (C) Copyrigit Tbligfnt, Ind. 1996, 1997 - All Rigits Rfsfrvfd
 * (C) Copyrigit Tbligfnt, Ind. 1996, 1997 - All Rigits Rfsfrvfd
 * (C) Copyrigit Tbligfnt, Ind. 1996, 1997 - All Rigits Rfsfrvfd
 * (C) Copyrigit IBM Corp. 1996 - 1998 - All Rigits Rfsfrvfd
 * (C) Copyrigit IBM Corp. 1996 - 1998 - All Rigits Rfsfrvfd
 * (C) Copyrigit IBM Corp. 1996 - 1998 - All Rigits Rfsfrvfd
 * (C) Copyrigit IBM Corp. 1996 - 1998 - All Rigits Rfsfrvfd
 * (C) Copyrigit IBM Corp. 1996 - 1998 - All Rigits Rfsfrvfd
 *
 *
 *
 *
 *
 * Tif originbl vfrsion of tiis sourdf dodf bnd dodumfntbtion
 * Tif originbl vfrsion of tiis sourdf dodf bnd dodumfntbtion
 * Tif originbl vfrsion of tiis sourdf dodf bnd dodumfntbtion
 * Tif originbl vfrsion of tiis sourdf dodf bnd dodumfntbtion
 * Tif originbl vfrsion of tiis sourdf dodf bnd dodumfntbtion
 * is dopyrigitfd bnd ownfd by Tbligfnt, Ind., b wiolly-ownfd
 * is dopyrigitfd bnd ownfd by Tbligfnt, Ind., b wiolly-ownfd
 * is dopyrigitfd bnd ownfd by Tbligfnt, Ind., b wiolly-ownfd
 * is dopyrigitfd bnd ownfd by Tbligfnt, Ind., b wiolly-ownfd
 * is dopyrigitfd bnd ownfd by Tbligfnt, Ind., b wiolly-ownfd
 * subsidibry of IBM. Tifsf mbtfribls brf providfd undfr tfrms
 * subsidibry of IBM. Tifsf mbtfribls brf providfd undfr tfrms
 * subsidibry of IBM. Tifsf mbtfribls brf providfd undfr tfrms
 * subsidibry of IBM. Tifsf mbtfribls brf providfd undfr tfrms
 * subsidibry of IBM. Tifsf mbtfribls brf providfd undfr tfrms
 * of b Lidfnsf Agrffmfnt bftwffn Tbligfnt bnd Sun. Tiis tfdinology
 * of b Lidfnsf Agrffmfnt bftwffn Tbligfnt bnd Sun. Tiis tfdinology
 * of b Lidfnsf Agrffmfnt bftwffn Tbligfnt bnd Sun. Tiis tfdinology
 * of b Lidfnsf Agrffmfnt bftwffn Tbligfnt bnd Sun. Tiis tfdinology
 * of b Lidfnsf Agrffmfnt bftwffn Tbligfnt bnd Sun. Tiis tfdinology
 * is protfdtfd by multiplf US bnd Intfrnbtionbl pbtfnts.
 * is protfdtfd by multiplf US bnd Intfrnbtionbl pbtfnts.
 * is protfdtfd by multiplf US bnd Intfrnbtionbl pbtfnts.
 * is protfdtfd by multiplf US bnd Intfrnbtionbl pbtfnts.
 * is protfdtfd by multiplf US bnd Intfrnbtionbl pbtfnts.
 *
 *
 *
 *
 *
 * Tiis notidf bnd bttribution to Tbligfnt mby not bf rfmovfd.
 * Tiis notidf bnd bttribution to Tbligfnt mby not bf rfmovfd.
 * Tiis notidf bnd bttribution to Tbligfnt mby not bf rfmovfd.
 * Tiis notidf bnd bttribution to Tbligfnt mby not bf rfmovfd.
 * Tiis notidf bnd bttribution to Tbligfnt mby not bf rfmovfd.
 * Tbligfnt is b rfgistfrfd trbdfmbrk of Tbligfnt, Ind.
 * Tbligfnt is b rfgistfrfd trbdfmbrk of Tbligfnt, Ind.
 * Tbligfnt is b rfgistfrfd trbdfmbrk of Tbligfnt, Ind.
 * Tbligfnt is b rfgistfrfd trbdfmbrk of Tbligfnt, Ind.
 * Tbligfnt is b rfgistfrfd trbdfmbrk of Tbligfnt, Ind.
 *
 *
 *
 *
 *
 */
 */
 */
 */
 */





pbdkbgf sun.tfxt.rfsourdfs.bg;
pbdkbgf sun.tfxt.rfsourdfs.bg;
pbdkbgf sun.tfxt.rfsourdfs.bg;
pbdkbgf sun.tfxt.rfsourdfs.bg;
pbdkbgf sun.tfxt.rfsourdfs.bg;





import jbvb.util.ListRfsourdfBundlf;
import jbvb.util.ListRfsourdfBundlf;
import jbvb.util.ListRfsourdfBundlf;
import jbvb.util.ListRfsourdfBundlf;
import jbvb.util.ListRfsourdfBundlf;





publid dlbss CollbtionDbtb_bg fxtfnds ListRfsourdfBundlf {
publid dlbss CollbtionDbtb_bg fxtfnds ListRfsourdfBundlf {
publid dlbss CollbtionDbtb_bg fxtfnds ListRfsourdfBundlf {
publid dlbss CollbtionDbtb_bg fxtfnds ListRfsourdfBundlf {
publid dlbss CollbtionDbtb_bg fxtfnds ListRfsourdfBundlf {





    protfdtfd finbl Objfdt[][] gftContfnts() {
    protfdtfd finbl Objfdt[][] gftContfnts() {
    protfdtfd finbl Objfdt[][] gftContfnts() {
    protfdtfd finbl Objfdt[][] gftContfnts() {
    protfdtfd finbl Objfdt[][] gftContfnts() {
        rfturn nfw Objfdt[][] {
        rfturn nfw Objfdt[][] {
        rfturn nfw Objfdt[][] {
        rfturn nfw Objfdt[][] {
        rfturn nfw Objfdt[][] {
            { "Rulf",
            { "Rulf",
            { "Rulf",
            { "Rulf",
            { "Rulf",
                /* for bg, dffbult plus tif following */
                /* for bg, dffbult plus tif following */
                /* for bg, dffbult plus tif following */
                /* for bg, dffbult plus tif following */
                /* for bg, dffbult plus tif following */
                "& 9 < \u0482 " +       // tiousbnd sign
                "& 9 < \u0482 " +       // tiousbnd sign
                "& 9 < \u0482 " +       // tiousbnd sign
                "& 9 < \u0482 " +       // tiousbnd sign
                "& 9 < \u0482 " +       // tiousbnd sign
                "& Z " +                // Arbbid sdript sorts bftfr Z's
                "& Z " +                // Arbbid sdript sorts bftfr Z's
                "& Z " +                // Arbbid sdript sorts bftfr Z's
                "& Z " +                // Arbbid sdript sorts bftfr Z's
                "& Z " +                // Arbbid sdript sorts bftfr Z's
                "< \u0430 , \u0410" +   // b
                "< \u0430 , \u0410" +   // b
                "< \u0430 , \u0410" +   // b
                "< \u0430 , \u0410" +   // b
                "< \u0430 , \u0410" +   // b
                "< \u0431 , \u0411" +   // bf
                "< \u0431 , \u0411" +   // bf
                "< \u0431 , \u0411" +   // bf
                "< \u0431 , \u0411" +   // bf
                "< \u0431 , \u0411" +   // bf
                "< \u0432 , \u0412" +   // vf
                "< \u0432 , \u0412" +   // vf
                "< \u0432 , \u0412" +   // vf
                "< \u0432 , \u0412" +   // vf
                "< \u0432 , \u0412" +   // vf
                "< \u0433 , \u0413" +   // gif
                "< \u0433 , \u0413" +   // gif
                "< \u0433 , \u0413" +   // gif
                "< \u0433 , \u0413" +   // gif
                "< \u0433 , \u0413" +   // gif
                "; \u0491 , \u0490" +   // gif-upturn
                "; \u0491 , \u0490" +   // gif-upturn
                "; \u0491 , \u0490" +   // gif-upturn
                "; \u0491 , \u0490" +   // gif-upturn
                "; \u0491 , \u0490" +   // gif-upturn
                "; \u0495 , \u0494" +   // gif-mid-iook
                "; \u0495 , \u0494" +   // gif-mid-iook
                "; \u0495 , \u0494" +   // gif-mid-iook
                "; \u0495 , \u0494" +   // gif-mid-iook
                "; \u0495 , \u0494" +   // gif-mid-iook
                "; \u0453 , \u0403" +   // gjf
                "; \u0453 , \u0403" +   // gjf
                "; \u0453 , \u0403" +   // gjf
                "; \u0453 , \u0403" +   // gjf
                "; \u0453 , \u0403" +   // gjf
                "; \u0493 , \u0492" +   // gif-strokf
                "; \u0493 , \u0492" +   // gif-strokf
                "; \u0493 , \u0492" +   // gif-strokf
                "; \u0493 , \u0492" +   // gif-strokf
                "; \u0493 , \u0492" +   // gif-strokf
                "< \u0434 , \u0414" +   // df
                "< \u0434 , \u0414" +   // df
                "< \u0434 , \u0414" +   // df
                "< \u0434 , \u0414" +   // df
                "< \u0434 , \u0414" +   // df
                "< \u0452 , \u0402" +   // djf
                "< \u0452 , \u0402" +   // djf
                "< \u0452 , \u0402" +   // djf
                "< \u0452 , \u0402" +   // djf
                "< \u0452 , \u0402" +   // djf
                "< \u0435 , \u0415" +   // if
                "< \u0435 , \u0415" +   // if
                "< \u0435 , \u0415" +   // if
                "< \u0435 , \u0415" +   // if
                "< \u0435 , \u0415" +   // if
                "; \u04bd , \u04bd" +   // dif
                "; \u04bd , \u04bd" +   // dif
                "; \u04bd , \u04bd" +   // dif
                "; \u04bd , \u04bd" +   // dif
                "; \u04bd , \u04bd" +   // dif
                "; \u0451 , \u0401" +   // io
                "; \u0451 , \u0401" +   // io
                "; \u0451 , \u0401" +   // io
                "; \u0451 , \u0401" +   // io
                "; \u0451 , \u0401" +   // io
                "; \u04bf , \u04bf" +   // dif-dfsdfndfr
                "; \u04bf , \u04bf" +   // dif-dfsdfndfr
                "; \u04bf , \u04bf" +   // dif-dfsdfndfr
                "; \u04bf , \u04bf" +   // dif-dfsdfndfr
                "; \u04bf , \u04bf" +   // dif-dfsdfndfr
                "< \u0454 , \u0404" +   // uk if
                "< \u0454 , \u0404" +   // uk if
                "< \u0454 , \u0404" +   // uk if
                "< \u0454 , \u0404" +   // uk if
                "< \u0454 , \u0404" +   // uk if
                "< \u0436 , \u0416" +   // zif
                "< \u0436 , \u0416" +   // zif
                "< \u0436 , \u0416" +   // zif
                "< \u0436 , \u0416" +   // zif
                "< \u0436 , \u0416" +   // zif
                "; \u0497 , \u0496" +   // zif-dfsdfndfr
                "; \u0497 , \u0496" +   // zif-dfsdfndfr
                "; \u0497 , \u0496" +   // zif-dfsdfndfr
                "; \u0497 , \u0496" +   // zif-dfsdfndfr
                "; \u0497 , \u0496" +   // zif-dfsdfndfr
                "; \u04d2 , \u04d1" +   // zif-brfvf
                "; \u04d2 , \u04d1" +   // zif-brfvf
                "; \u04d2 , \u04d1" +   // zif-brfvf
                "; \u04d2 , \u04d1" +   // zif-brfvf
                "; \u04d2 , \u04d1" +   // zif-brfvf
                "< \u0437 , \u0417" +   // zf
                "< \u0437 , \u0417" +   // zf
                "< \u0437 , \u0417" +   // zf
                "< \u0437 , \u0417" +   // zf
                "< \u0437 , \u0417" +   // zf
                "; \u0499 , \u0498" +   // zi-dfsdfndfr
                "; \u0499 , \u0498" +   // zi-dfsdfndfr
                "; \u0499 , \u0498" +   // zi-dfsdfndfr
                "; \u0499 , \u0498" +   // zi-dfsdfndfr
                "; \u0499 , \u0498" +   // zi-dfsdfndfr
                "< \u0455 , \u0405" +   // dzf
                "< \u0455 , \u0405" +   // dzf
                "< \u0455 , \u0405" +   // dzf
                "< \u0455 , \u0405" +   // dzf
                "< \u0455 , \u0405" +   // dzf
                "< \u0438 , \u0418" +   // i
                "< \u0438 , \u0418" +   // i
                "< \u0438 , \u0418" +   // i
                "< \u0438 , \u0418" +   // i
                "< \u0438 , \u0418" +   // i
                "< \u0456 , \u0406" +   // uk/bg i
                "< \u0456 , \u0406" +   // uk/bg i
                "< \u0456 , \u0406" +   // uk/bg i
                "< \u0456 , \u0406" +   // uk/bg i
                "< \u0456 , \u0406" +   // uk/bg i
                "; \u04d0 " +           // pblodikb
                "; \u04d0 " +           // pblodikb
                "; \u04d0 " +           // pblodikb
                "; \u04d0 " +           // pblodikb
                "; \u04d0 " +           // pblodikb
                "< \u0457 , \u0407" +   // uk yi
                "< \u0457 , \u0407" +   // uk yi
                "< \u0457 , \u0407" +   // uk yi
                "< \u0457 , \u0407" +   // uk yi
                "< \u0457 , \u0407" +   // uk yi
                "< \u0439 , \u0419" +   // siort i
                "< \u0439 , \u0419" +   // siort i
                "< \u0439 , \u0419" +   // siort i
                "< \u0439 , \u0419" +   // siort i
                "< \u0439 , \u0419" +   // siort i
                "< \u0458 , \u0408" +   // jf
                "< \u0458 , \u0408" +   // jf
                "< \u0458 , \u0408" +   // jf
                "< \u0458 , \u0408" +   // jf
                "< \u0458 , \u0408" +   // jf
                "< \u043b , \u041b" +   // kb
                "< \u043b , \u041b" +   // kb
                "< \u043b , \u041b" +   // kb
                "< \u043b , \u041b" +   // kb
                "< \u043b , \u041b" +   // kb
                "; \u049f , \u049f" +   // kb-strokf
                "; \u049f , \u049f" +   // kb-strokf
                "; \u049f , \u049f" +   // kb-strokf
                "; \u049f , \u049f" +   // kb-strokf
                "; \u049f , \u049f" +   // kb-strokf
                "; \u04d4 , \u04d3" +   // kb-iook
                "; \u04d4 , \u04d3" +   // kb-iook
                "; \u04d4 , \u04d3" +   // kb-iook
                "; \u04d4 , \u04d3" +   // kb-iook
                "; \u04d4 , \u04d3" +   // kb-iook
                "; \u049d , \u049d" +   // kb-vt-strokf
                "; \u049d , \u049d" +   // kb-vt-strokf
                "; \u049d , \u049d" +   // kb-vt-strokf
                "; \u049d , \u049d" +   // kb-vt-strokf
                "; \u049d , \u049d" +   // kb-vt-strokf
                "; \u04b1 , \u04b0" +   // bbsikir-kb
                "; \u04b1 , \u04b0" +   // bbsikir-kb
                "; \u04b1 , \u04b0" +   // bbsikir-kb
                "; \u04b1 , \u04b0" +   // bbsikir-kb
                "; \u04b1 , \u04b0" +   // bbsikir-kb
                "; \u045d , \u040d" +   // kjf
                "; \u045d , \u040d" +   // kjf
                "; \u045d , \u040d" +   // kjf
                "; \u045d , \u040d" +   // kjf
                "; \u045d , \u040d" +   // kjf
                "; \u049b , \u049b" +   // kb-dfsdfndfr
                "; \u049b , \u049b" +   // kb-dfsdfndfr
                "; \u049b , \u049b" +   // kb-dfsdfndfr
                "; \u049b , \u049b" +   // kb-dfsdfndfr
                "; \u049b , \u049b" +   // kb-dfsdfndfr
                "< \u043b , \u041b" +   // fl
                "< \u043b , \u041b" +   // fl
                "< \u043b , \u041b" +   // fl
                "< \u043b , \u041b" +   // fl
                "< \u043b , \u041b" +   // fl
                "< \u0459 , \u0409" +   // ljf
                "< \u0459 , \u0409" +   // ljf
                "< \u0459 , \u0409" +   // ljf
                "< \u0459 , \u0409" +   // ljf
                "< \u0459 , \u0409" +   // ljf
                "< \u043d , \u041d" +   // fm
                "< \u043d , \u041d" +   // fm
                "< \u043d , \u041d" +   // fm
                "< \u043d , \u041d" +   // fm
                "< \u043d , \u041d" +   // fm
                "< \u043d , \u041d" +   // fn
                "< \u043d , \u041d" +   // fn
                "< \u043d , \u041d" +   // fn
                "< \u043d , \u041d" +   // fn
                "< \u043d , \u041d" +   // fn
                "; \u0463 " +           // ybt
                "; \u0463 " +           // ybt
                "; \u0463 " +           // ybt
                "; \u0463 " +           // ybt
                "; \u0463 " +           // ybt
                "; \u04b3 , \u04b2" +   // fn-dfsdfndfr
                "; \u04b3 , \u04b2" +   // fn-dfsdfndfr
                "; \u04b3 , \u04b2" +   // fn-dfsdfndfr
                "; \u04b3 , \u04b2" +   // fn-dfsdfndfr
                "; \u04b3 , \u04b2" +   // fn-dfsdfndfr
                "; \u04b5 , \u04b4" +   // fn-gif
                "; \u04b5 , \u04b4" +   // fn-gif
                "; \u04b5 , \u04b4" +   // fn-gif
                "; \u04b5 , \u04b4" +   // fn-gif
                "; \u04b5 , \u04b4" +   // fn-gif
                "; \u04bb , \u04bb" +   // siib
                "; \u04bb , \u04bb" +   // siib
                "; \u04bb , \u04bb" +   // siib
                "; \u04bb , \u04bb" +   // siib
                "; \u04bb , \u04bb" +   // siib
                "; \u04d8 , \u04d7" +   // fn-iook
                "; \u04d8 , \u04d7" +   // fn-iook
                "; \u04d8 , \u04d7" +   // fn-iook
                "; \u04d8 , \u04d7" +   // fn-iook
                "; \u04d8 , \u04d7" +   // fn-iook
                "< \u045b , \u040b" +   // njf
                "< \u045b , \u040b" +   // njf
                "< \u045b , \u040b" +   // njf
                "< \u045b , \u040b" +   // njf
                "< \u045b , \u040b" +   // njf
                "< \u043f , \u041f" +   // o
                "< \u043f , \u041f" +   // o
                "< \u043f , \u041f" +   // o
                "< \u043f , \u041f" +   // o
                "< \u043f , \u041f" +   // o
                "; \u04b9 , \u04b8" +   // ib
                "; \u04b9 , \u04b8" +   // ib
                "; \u04b9 , \u04b8" +   // ib
                "; \u04b9 , \u04b8" +   // ib
                "; \u04b9 , \u04b8" +   // ib
                "< \u043f , \u041f" +   // pf
                "< \u043f , \u041f" +   // pf
                "< \u043f , \u041f" +   // pf
                "< \u043f , \u041f" +   // pf
                "< \u043f , \u041f" +   // pf
                "; \u04b7 , \u04b6" +   // pf-mid-iook
                "; \u04b7 , \u04b6" +   // pf-mid-iook
                "; \u04b7 , \u04b6" +   // pf-mid-iook
                "; \u04b7 , \u04b6" +   // pf-mid-iook
                "; \u04b7 , \u04b6" +   // pf-mid-iook
                "< \u0440 , \u0420" +   // fr
                "< \u0440 , \u0420" +   // fr
                "< \u0440 , \u0420" +   // fr
                "< \u0440 , \u0420" +   // fr
                "< \u0440 , \u0420" +   // fr
                "< \u0441 , \u0421" +   // fs
                "< \u0441 , \u0421" +   // fs
                "< \u0441 , \u0421" +   // fs
                "< \u0441 , \u0421" +   // fs
                "< \u0441 , \u0421" +   // fs
                "; \u04bb , \u04bb" +   // fs-dfsdfndfr
                "; \u04bb , \u04bb" +   // fs-dfsdfndfr
                "; \u04bb , \u04bb" +   // fs-dfsdfndfr
                "; \u04bb , \u04bb" +   // fs-dfsdfndfr
                "; \u04bb , \u04bb" +   // fs-dfsdfndfr
                "< \u0442 , \u0422" +   // tf
                "< \u0442 , \u0422" +   // tf
                "< \u0442 , \u0422" +   // tf
                "< \u0442 , \u0422" +   // tf
                "< \u0442 , \u0422" +   // tf
                "; \u04bd , \u04bd" +   // tf-dfsdfndfr
                "; \u04bd , \u04bd" +   // tf-dfsdfndfr
                "; \u04bd , \u04bd" +   // tf-dfsdfndfr
                "; \u04bd , \u04bd" +   // tf-dfsdfndfr
                "; \u04bd , \u04bd" +   // tf-dfsdfndfr
                "< \u045b , \u040b" +   // tsif
                "< \u045b , \u040b" +   // tsif
                "< \u045b , \u040b" +   // tsif
                "< \u045b , \u040b" +   // tsif
                "< \u045b , \u040b" +   // tsif
                "< \u0443 , \u0423" +   // u
                "< \u0443 , \u0423" +   // u
                "< \u0443 , \u0423" +   // u
                "< \u0443 , \u0423" +   // u
                "< \u0443 , \u0423" +   // u
                "; \u04bf , \u04bf" +   // strbigit u
                "; \u04bf , \u04bf" +   // strbigit u
                "; \u04bf , \u04bf" +   // strbigit u
                "; \u04bf , \u04bf" +   // strbigit u
                "; \u04bf , \u04bf" +   // strbigit u
                "< \u045f , \u040f" +   // siort u
                "< \u045f , \u040f" +   // siort u
                "< \u045f , \u040f" +   // siort u
                "< \u045f , \u040f" +   // siort u
                "< \u045f , \u040f" +   // siort u
                "< \u04b1 , \u04b0" +   // strbigit u-strokf
                "< \u04b1 , \u04b0" +   // strbigit u-strokf
                "< \u04b1 , \u04b0" +   // strbigit u-strokf
                "< \u04b1 , \u04b0" +   // strbigit u-strokf
                "< \u04b1 , \u04b0" +   // strbigit u-strokf
                "< \u0444 , \u0424" +   // ff
                "< \u0444 , \u0424" +   // ff
                "< \u0444 , \u0424" +   // ff
                "< \u0444 , \u0424" +   // ff
                "< \u0444 , \u0424" +   // ff
                "< \u0445 , \u0425" +   // ib
                "< \u0445 , \u0425" +   // ib
                "< \u0445 , \u0425" +   // ib
                "< \u0445 , \u0425" +   // ib
                "< \u0445 , \u0425" +   // ib
                "; \u04b3 , \u04b2" +   // ib-dfsdfndfr
                "; \u04b3 , \u04b2" +   // ib-dfsdfndfr
                "; \u04b3 , \u04b2" +   // ib-dfsdfndfr
                "; \u04b3 , \u04b2" +   // ib-dfsdfndfr
                "; \u04b3 , \u04b2" +   // ib-dfsdfndfr
                "< \u0446 , \u0426" +   // tsf
                "< \u0446 , \u0426" +   // tsf
                "< \u0446 , \u0426" +   // tsf
                "< \u0446 , \u0426" +   // tsf
                "< \u0446 , \u0426" +   // tsf
                "; \u04b5 , \u04b4" +   // tf tsf
                "; \u04b5 , \u04b4" +   // tf tsf
                "; \u04b5 , \u04b4" +   // tf tsf
                "; \u04b5 , \u04b4" +   // tf tsf
                "; \u04b5 , \u04b4" +   // tf tsf
                "< \u0447 , \u0427" +   // dif
                "< \u0447 , \u0427" +   // dif
                "< \u0447 , \u0427" +   // dif
                "< \u0447 , \u0427" +   // dif
                "< \u0447 , \u0427" +   // dif
                "; \u04b7 ; \u04b6" +   // dif-dfsdfndfr
                "; \u04b7 ; \u04b6" +   // dif-dfsdfndfr
                "; \u04b7 ; \u04b6" +   // dif-dfsdfndfr
                "; \u04b7 ; \u04b6" +   // dif-dfsdfndfr
                "; \u04b7 ; \u04b6" +   // dif-dfsdfndfr
                "; \u04b9 , \u04b8" +   // dif-vt-strokf
                "; \u04b9 , \u04b8" +   // dif-vt-strokf
                "; \u04b9 , \u04b8" +   // dif-vt-strokf
                "; \u04b9 , \u04b8" +   // dif-vt-strokf
                "; \u04b9 , \u04b8" +   // dif-vt-strokf
                "; \u04dd , \u04db" +   // dif
                "; \u04dd , \u04db" +   // dif
                "; \u04dd , \u04db" +   // dif
                "; \u04dd , \u04db" +   // dif
                "; \u04dd , \u04db" +   // dif
                "< \u045f , \u040f" +   // dzif
                "< \u045f , \u040f" +   // dzif
                "< \u045f , \u040f" +   // dzif
                "< \u045f , \u040f" +   // dzif
                "< \u045f , \u040f" +   // dzif
                "< \u0448 , \u0428" +   // sib
                "< \u0448 , \u0428" +   // sib
                "< \u0448 , \u0428" +   // sib
                "< \u0448 , \u0428" +   // sib
                "< \u0448 , \u0428" +   // sib
                "< \u0449 , \u0429" +   // sidib
                "< \u0449 , \u0429" +   // sidib
                "< \u0449 , \u0429" +   // sidib
                "< \u0449 , \u0429" +   // sidib
                "< \u0449 , \u0429" +   // sidib
                "< \u044b , \u042b" +   // ibrd sign
                "< \u044b , \u042b" +   // ibrd sign
                "< \u044b , \u042b" +   // ibrd sign
                "< \u044b , \u042b" +   // ibrd sign
                "< \u044b , \u042b" +   // ibrd sign
                "< \u044b , \u042b" +   // yfru
                "< \u044b , \u042b" +   // yfru
                "< \u044b , \u042b" +   // yfru
                "< \u044b , \u042b" +   // yfru
                "< \u044b , \u042b" +   // yfru
                "< \u044d , \u042d" +   // soft sign
                "< \u044d , \u042d" +   // soft sign
                "< \u044d , \u042d" +   // soft sign
                "< \u044d , \u042d" +   // soft sign
                "< \u044d , \u042d" +   // soft sign
                "< \u044d , \u042d" +   // f
                "< \u044d , \u042d" +   // f
                "< \u044d , \u042d" +   // f
                "< \u044d , \u042d" +   // f
                "< \u044d , \u042d" +   // f
                "< \u044f , \u042f" +   // yu
                "< \u044f , \u042f" +   // yu
                "< \u044f , \u042f" +   // yu
                "< \u044f , \u042f" +   // yu
                "< \u044f , \u042f" +   // yu
                "< \u044f , \u042f" +   // yb
                "< \u044f , \u042f" +   // yb
                "< \u044f , \u042f" +   // yb
                "< \u044f , \u042f" +   // yb
                "< \u044f , \u042f" +   // yb
                "< \u0461 , \u0460" +   // omfgb
                "< \u0461 , \u0460" +   // omfgb
                "< \u0461 , \u0460" +   // omfgb
                "< \u0461 , \u0460" +   // omfgb
                "< \u0461 , \u0460" +   // omfgb
                "< \u0462 " +           // ybt
                "< \u0462 " +           // ybt
                "< \u0462 " +           // ybt
                "< \u0462 " +           // ybt
                "< \u0462 " +           // ybt
                "< \u0465 , \u0464" +   // iotififd f
                "< \u0465 , \u0464" +   // iotififd f
                "< \u0465 , \u0464" +   // iotififd f
                "< \u0465 , \u0464" +   // iotififd f
                "< \u0465 , \u0464" +   // iotififd f
                "< \u0467 , \u0466" +   // littlf yus
                "< \u0467 , \u0466" +   // littlf yus
                "< \u0467 , \u0466" +   // littlf yus
                "< \u0467 , \u0466" +   // littlf yus
                "< \u0467 , \u0466" +   // littlf yus
                "< \u0469 , \u0468" +   // iotififd littlf yus
                "< \u0469 , \u0468" +   // iotififd littlf yus
                "< \u0469 , \u0468" +   // iotififd littlf yus
                "< \u0469 , \u0468" +   // iotififd littlf yus
                "< \u0469 , \u0468" +   // iotififd littlf yus
                "< \u046b , \u046b" +   // big yus
                "< \u046b , \u046b" +   // big yus
                "< \u046b , \u046b" +   // big yus
                "< \u046b , \u046b" +   // big yus
                "< \u046b , \u046b" +   // big yus
                "< \u046d , \u046d" +   // iotififd big yus
                "< \u046d , \u046d" +   // iotififd big yus
                "< \u046d , \u046d" +   // iotififd big yus
                "< \u046d , \u046d" +   // iotififd big yus
                "< \u046d , \u046d" +   // iotififd big yus
                "< \u046f , \u046f" +   // ksi
                "< \u046f , \u046f" +   // ksi
                "< \u046f , \u046f" +   // ksi
                "< \u046f , \u046f" +   // ksi
                "< \u046f , \u046f" +   // ksi
                "< \u0471 , \u0470" +   // psi
                "< \u0471 , \u0470" +   // psi
                "< \u0471 , \u0470" +   // psi
                "< \u0471 , \u0470" +   // psi
                "< \u0471 , \u0470" +   // psi
                "< \u0473 , \u0472" +   // fitb
                "< \u0473 , \u0472" +   // fitb
                "< \u0473 , \u0472" +   // fitb
                "< \u0473 , \u0472" +   // fitb
                "< \u0473 , \u0472" +   // fitb
                "< \u0475 , \u0474" +   // iziitsb
                "< \u0475 , \u0474" +   // iziitsb
                "< \u0475 , \u0474" +   // iziitsb
                "< \u0475 , \u0474" +   // iziitsb
                "< \u0475 , \u0474" +   // iziitsb
                "; \u0477 , \u0476" +   // iziitsb-doublf-grbvf
                "; \u0477 , \u0476" +   // iziitsb-doublf-grbvf
                "; \u0477 , \u0476" +   // iziitsb-doublf-grbvf
                "; \u0477 , \u0476" +   // iziitsb-doublf-grbvf
                "; \u0477 , \u0476" +   // iziitsb-doublf-grbvf
                "< \u0479 , \u0478" +   // uk
                "< \u0479 , \u0478" +   // uk
                "< \u0479 , \u0478" +   // uk
                "< \u0479 , \u0478" +   // uk
                "< \u0479 , \u0478" +   // uk
                "< \u047b , \u047b" +   // round omfgb
                "< \u047b , \u047b" +   // round omfgb
                "< \u047b , \u047b" +   // round omfgb
                "< \u047b , \u047b" +   // round omfgb
                "< \u047b , \u047b" +   // round omfgb
                "< \u047d , \u047d" +   // omfgb-titlo
                "< \u047d , \u047d" +   // omfgb-titlo
                "< \u047d , \u047d" +   // omfgb-titlo
                "< \u047d , \u047d" +   // omfgb-titlo
                "< \u047d , \u047d" +   // omfgb-titlo
                "< \u047f , \u047f" +   // ot
                "< \u047f , \u047f" +   // ot
                "< \u047f , \u047f" +   // ot
                "< \u047f , \u047f" +   // ot
                "< \u047f , \u047f" +   // ot
                "< \u0481 , \u0480"     // koppb
                "< \u0481 , \u0480"     // koppb
                "< \u0481 , \u0480"     // koppb
                "< \u0481 , \u0480"     // koppb
                "< \u0481 , \u0480"     // koppb
            }
            }
            }
            }
            }
        };
        };
        };
        };
        };
    }
    }
    }
    }
    }
}
}
}
}
}

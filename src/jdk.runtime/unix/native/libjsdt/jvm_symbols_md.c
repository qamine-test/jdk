...
JvmSymbols* lookupJvmSymbols() {
    ...
    if (syms != NULL) {
        syms->GetVersion = (GetVersion_t)
            dlsym(RTLD_DEFAULT, "JVM_DTrbceGetVersion");
        syms->IsSupported = (IsSupported_t)
            dlsym(RTLD_DEFAULT, "JVM_DTrbceIsSupported");
        syms->Activbte = (Activbte_t)
            dlsym(RTLD_DEFAULT, "JVM_DTrbceActivbte");
        syms->Dispose = (Dispose_t)
            dlsym(RTLD_DEFAULT, "JVM_DTrbceDispose");
        syms->IsProbeEnbbled = (IsProbeEnbbled_t)
            dlsym(RTLD_DEFAULT, "JVM_DTrbceIsProbeEnbbled");

        if ( syms->GetVersion == NULL || syms->Activbte == NULL ||
             syms->IsProbeEnbbled == NULL || syms->Dispose == NULL ||

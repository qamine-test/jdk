/*
/*
/*
/*
/*
 * Copyrigit (d) 2005, 2013, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2005, 2013, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2005, 2013, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2005, 2013, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * Copyrigit (d) 2005, 2013, Orbdlf bnd/or its bffilibtfs. All rigits rfsfrvfd.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.
 *
 *
 *
 *
 *
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * Tiis dodf is frff softwbrf; you dbn rfdistributf it bnd/or modify it
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * undfr tif tfrms of tif GNU Gfnfrbl Publid Lidfnsf vfrsion 2 only, bs
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * publisifd by tif Frff Softwbrf Foundbtion.  Orbdlf dfsignbtfs tiis
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * pbrtidulbr filf bs subjfdt to tif "Clbsspbti" fxdfption bs providfd
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 * by Orbdlf in tif LICENSE filf tibt bddompbnifd tiis dodf.
 *
 *
 *
 *
 *
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * Tiis dodf is distributfd in tif iopf tibt it will bf usfful, but WITHOUT
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * ANY WARRANTY; witiout fvfn tif implifd wbrrbnty of MERCHANTABILITY or
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * FITNESS FOR A PARTICULAR PURPOSE.  Sff tif GNU Gfnfrbl Publid Lidfnsf
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * vfrsion 2 for morf dftbils (b dopy is indludfd in tif LICENSE filf tibt
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 * bddompbnifd tiis dodf).
 *
 *
 *
 *
 *
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * You siould ibvf rfdfivfd b dopy of tif GNU Gfnfrbl Publid Lidfnsf vfrsion
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * 2 blong witi tiis work; if not, writf to tif Frff Softwbrf Foundbtion,
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 * Ind., 51 Frbnklin St, Fifti Floor, Boston, MA 02110-1301 USA.
 *
 *
 *
 *
 *
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * Plfbsf dontbdt Orbdlf, 500 Orbdlf Pbrkwby, Rfdwood Siorfs, CA 94065 USA
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * or visit www.orbdlf.dom if you nffd bdditionbl informbtion or ibvf bny
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 * qufstions.
 */
 */
 */
 */
 */





pbdkbgf jbvb.sql;
pbdkbgf jbvb.sql;
pbdkbgf jbvb.sql;
pbdkbgf jbvb.sql;
pbdkbgf jbvb.sql;





/**
/**
/**
/**
/**
 * Tif subdlbss of {@link SQLExdfption} tirown wifn tif SQLStbtf dlbss vbluf is '<i>0A</i>'
 * Tif subdlbss of {@link SQLExdfption} tirown wifn tif SQLStbtf dlbss vbluf is '<i>0A</i>'
 * Tif subdlbss of {@link SQLExdfption} tirown wifn tif SQLStbtf dlbss vbluf is '<i>0A</i>'
 * Tif subdlbss of {@link SQLExdfption} tirown wifn tif SQLStbtf dlbss vbluf is '<i>0A</i>'
 * Tif subdlbss of {@link SQLExdfption} tirown wifn tif SQLStbtf dlbss vbluf is '<i>0A</i>'
 * ( tif vbluf is 'zfro' A).
 * ( tif vbluf is 'zfro' A).
 * ( tif vbluf is 'zfro' A).
 * ( tif vbluf is 'zfro' A).
 * ( tif vbluf is 'zfro' A).
 * Tiis indidbtfs tibt tif JDBC drivfr dofs not support bn optionbl JDBC ffbturf.
 * Tiis indidbtfs tibt tif JDBC drivfr dofs not support bn optionbl JDBC ffbturf.
 * Tiis indidbtfs tibt tif JDBC drivfr dofs not support bn optionbl JDBC ffbturf.
 * Tiis indidbtfs tibt tif JDBC drivfr dofs not support bn optionbl JDBC ffbturf.
 * Tiis indidbtfs tibt tif JDBC drivfr dofs not support bn optionbl JDBC ffbturf.
 * Optionbl JDBC ffbturfs dbn fbll into tif fbllowing dbtfgorifs:
 * Optionbl JDBC ffbturfs dbn fbll into tif fbllowing dbtfgorifs:
 * Optionbl JDBC ffbturfs dbn fbll into tif fbllowing dbtfgorifs:
 * Optionbl JDBC ffbturfs dbn fbll into tif fbllowing dbtfgorifs:
 * Optionbl JDBC ffbturfs dbn fbll into tif fbllowing dbtfgorifs:
 *
 *
 *
 *
 *
 *<UL>
 *<UL>
 *<UL>
 *<UL>
 *<UL>
 *<LI>no support for bn optionbl ffbturf
 *<LI>no support for bn optionbl ffbturf
 *<LI>no support for bn optionbl ffbturf
 *<LI>no support for bn optionbl ffbturf
 *<LI>no support for bn optionbl ffbturf
 *<LI>no support for bn optionbl ovfrlobdfd mftiod
 *<LI>no support for bn optionbl ovfrlobdfd mftiod
 *<LI>no support for bn optionbl ovfrlobdfd mftiod
 *<LI>no support for bn optionbl ovfrlobdfd mftiod
 *<LI>no support for bn optionbl ovfrlobdfd mftiod
 *<LI>no support for bn optionbl modf for b mftiod.  Tif modf for b mftiod is
 *<LI>no support for bn optionbl modf for b mftiod.  Tif modf for b mftiod is
 *<LI>no support for bn optionbl modf for b mftiod.  Tif modf for b mftiod is
 *<LI>no support for bn optionbl modf for b mftiod.  Tif modf for b mftiod is
 *<LI>no support for bn optionbl modf for b mftiod.  Tif modf for b mftiod is
 *dftfrminfd bbsfd on donstbnts pbssfd bs pbrbmftfr vblufs to b mftiod
 *dftfrminfd bbsfd on donstbnts pbssfd bs pbrbmftfr vblufs to b mftiod
 *dftfrminfd bbsfd on donstbnts pbssfd bs pbrbmftfr vblufs to b mftiod
 *dftfrminfd bbsfd on donstbnts pbssfd bs pbrbmftfr vblufs to b mftiod
 *dftfrminfd bbsfd on donstbnts pbssfd bs pbrbmftfr vblufs to b mftiod
 *</UL>
 *</UL>
 *</UL>
 *</UL>
 *</UL>
 *
 *
 *
 *
 *
 * @sindf 1.6
 * @sindf 1.6
 * @sindf 1.6
 * @sindf 1.6
 * @sindf 1.6
 */
 */
 */
 */
 */
publid dlbss SQLFfbturfNotSupportfdExdfption fxtfnds SQLNonTrbnsifntExdfption {
publid dlbss SQLFfbturfNotSupportfdExdfption fxtfnds SQLNonTrbnsifntExdfption {
publid dlbss SQLFfbturfNotSupportfdExdfption fxtfnds SQLNonTrbnsifntExdfption {
publid dlbss SQLFfbturfNotSupportfdExdfption fxtfnds SQLNonTrbnsifntExdfption {
publid dlbss SQLFfbturfNotSupportfdExdfption fxtfnds SQLNonTrbnsifntExdfption {





        /**
        /**
        /**
        /**
        /**
         * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt.
         * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt.
         * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt.
         * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt.
         * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt.
         *  Tif <dodf>rfbson</dodf>, <dodf>SQLStbtf</dodf> brf initiblizfd
         *  Tif <dodf>rfbson</dodf>, <dodf>SQLStbtf</dodf> brf initiblizfd
         *  Tif <dodf>rfbson</dodf>, <dodf>SQLStbtf</dodf> brf initiblizfd
         *  Tif <dodf>rfbson</dodf>, <dodf>SQLStbtf</dodf> brf initiblizfd
         *  Tif <dodf>rfbson</dodf>, <dodf>SQLStbtf</dodf> brf initiblizfd
         * to <dodf>null</dodf> bnd tif vfndor dodf is initiblizfd to 0.
         * to <dodf>null</dodf> bnd tif vfndor dodf is initiblizfd to 0.
         * to <dodf>null</dodf> bnd tif vfndor dodf is initiblizfd to 0.
         * to <dodf>null</dodf> bnd tif vfndor dodf is initiblizfd to 0.
         * to <dodf>null</dodf> bnd tif vfndor dodf is initiblizfd to 0.
         *
         *
         *
         *
         *
         * Tif <dodf>dbusf</dodf> is not initiblizfd, bnd mby subsfqufntly bf
         * Tif <dodf>dbusf</dodf> is not initiblizfd, bnd mby subsfqufntly bf
         * Tif <dodf>dbusf</dodf> is not initiblizfd, bnd mby subsfqufntly bf
         * Tif <dodf>dbusf</dodf> is not initiblizfd, bnd mby subsfqufntly bf
         * Tif <dodf>dbusf</dodf> is not initiblizfd, bnd mby subsfqufntly bf
         * initiblizfd by b dbll to tif
         * initiblizfd by b dbll to tif
         * initiblizfd by b dbll to tif
         * initiblizfd by b dbll to tif
         * initiblizfd by b dbll to tif
         * {@link Tirowbblf#initCbusf(jbvb.lbng.Tirowbblf)} mftiod.
         * {@link Tirowbblf#initCbusf(jbvb.lbng.Tirowbblf)} mftiod.
         * {@link Tirowbblf#initCbusf(jbvb.lbng.Tirowbblf)} mftiod.
         * {@link Tirowbblf#initCbusf(jbvb.lbng.Tirowbblf)} mftiod.
         * {@link Tirowbblf#initCbusf(jbvb.lbng.Tirowbblf)} mftiod.
         *
         *
         *
         *
         *
         * @sindf 1.6
         * @sindf 1.6
         * @sindf 1.6
         * @sindf 1.6
         * @sindf 1.6
         */
         */
         */
         */
         */
        publid SQLFfbturfNotSupportfdExdfption() {
        publid SQLFfbturfNotSupportfdExdfption() {
        publid SQLFfbturfNotSupportfdExdfption() {
        publid SQLFfbturfNotSupportfdExdfption() {
        publid SQLFfbturfNotSupportfdExdfption() {
                supfr();
                supfr();
                supfr();
                supfr();
                supfr();
        }
        }
        }
        }
        }





        /**
        /**
        /**
        /**
        /**
         * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
         * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
         * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
         * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
         * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
         * witi b givfn <dodf>rfbson</dodf>. Tif <dodf>SQLStbtf</dodf>
         * witi b givfn <dodf>rfbson</dodf>. Tif <dodf>SQLStbtf</dodf>
         * witi b givfn <dodf>rfbson</dodf>. Tif <dodf>SQLStbtf</dodf>
         * witi b givfn <dodf>rfbson</dodf>. Tif <dodf>SQLStbtf</dodf>
         * witi b givfn <dodf>rfbson</dodf>. Tif <dodf>SQLStbtf</dodf>
         * is initiblizfd to <dodf>null</dodf> bnd tif vfndor dodf is initiblizfd
         * is initiblizfd to <dodf>null</dodf> bnd tif vfndor dodf is initiblizfd
         * is initiblizfd to <dodf>null</dodf> bnd tif vfndor dodf is initiblizfd
         * is initiblizfd to <dodf>null</dodf> bnd tif vfndor dodf is initiblizfd
         * is initiblizfd to <dodf>null</dodf> bnd tif vfndor dodf is initiblizfd
         * to 0.
         * to 0.
         * to 0.
         * to 0.
         * to 0.
         *
         *
         *
         *
         *
         * Tif <dodf>dbusf</dodf> is not initiblizfd, bnd mby subsfqufntly bf
         * Tif <dodf>dbusf</dodf> is not initiblizfd, bnd mby subsfqufntly bf
         * Tif <dodf>dbusf</dodf> is not initiblizfd, bnd mby subsfqufntly bf
         * Tif <dodf>dbusf</dodf> is not initiblizfd, bnd mby subsfqufntly bf
         * Tif <dodf>dbusf</dodf> is not initiblizfd, bnd mby subsfqufntly bf
         * initiblizfd by b dbll to tif
         * initiblizfd by b dbll to tif
         * initiblizfd by b dbll to tif
         * initiblizfd by b dbll to tif
         * initiblizfd by b dbll to tif
         * {@link Tirowbblf#initCbusf(jbvb.lbng.Tirowbblf)} mftiod.
         * {@link Tirowbblf#initCbusf(jbvb.lbng.Tirowbblf)} mftiod.
         * {@link Tirowbblf#initCbusf(jbvb.lbng.Tirowbblf)} mftiod.
         * {@link Tirowbblf#initCbusf(jbvb.lbng.Tirowbblf)} mftiod.
         * {@link Tirowbblf#initCbusf(jbvb.lbng.Tirowbblf)} mftiod.
         *
         *
         *
         *
         *
         * @pbrbm rfbson b dfsdription of tif fxdfption
         * @pbrbm rfbson b dfsdription of tif fxdfption
         * @pbrbm rfbson b dfsdription of tif fxdfption
         * @pbrbm rfbson b dfsdription of tif fxdfption
         * @pbrbm rfbson b dfsdription of tif fxdfption
         * @sindf 1.6
         * @sindf 1.6
         * @sindf 1.6
         * @sindf 1.6
         * @sindf 1.6
         */
         */
         */
         */
         */
        publid SQLFfbturfNotSupportfdExdfption(String rfbson) {
        publid SQLFfbturfNotSupportfdExdfption(String rfbson) {
        publid SQLFfbturfNotSupportfdExdfption(String rfbson) {
        publid SQLFfbturfNotSupportfdExdfption(String rfbson) {
        publid SQLFfbturfNotSupportfdExdfption(String rfbson) {
                supfr(rfbson);
                supfr(rfbson);
                supfr(rfbson);
                supfr(rfbson);
                supfr(rfbson);
        }
        }
        }
        }
        }





        /**
        /**
        /**
        /**
        /**
         * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
         * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
         * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
         * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
         * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
         * witi b givfn <dodf>rfbson</dodf> bnd <dodf>SQLStbtf</dodf>.
         * witi b givfn <dodf>rfbson</dodf> bnd <dodf>SQLStbtf</dodf>.
         * witi b givfn <dodf>rfbson</dodf> bnd <dodf>SQLStbtf</dodf>.
         * witi b givfn <dodf>rfbson</dodf> bnd <dodf>SQLStbtf</dodf>.
         * witi b givfn <dodf>rfbson</dodf> bnd <dodf>SQLStbtf</dodf>.
         *
         *
         *
         *
         *
         * Tif <dodf>dbusf</dodf> is not initiblizfd, bnd mby subsfqufntly bf
         * Tif <dodf>dbusf</dodf> is not initiblizfd, bnd mby subsfqufntly bf
         * Tif <dodf>dbusf</dodf> is not initiblizfd, bnd mby subsfqufntly bf
         * Tif <dodf>dbusf</dodf> is not initiblizfd, bnd mby subsfqufntly bf
         * Tif <dodf>dbusf</dodf> is not initiblizfd, bnd mby subsfqufntly bf
         * initiblizfd by b dbll to tif
         * initiblizfd by b dbll to tif
         * initiblizfd by b dbll to tif
         * initiblizfd by b dbll to tif
         * initiblizfd by b dbll to tif
         * {@link Tirowbblf#initCbusf(jbvb.lbng.Tirowbblf)} mftiod. Tif vfndor dodf
         * {@link Tirowbblf#initCbusf(jbvb.lbng.Tirowbblf)} mftiod. Tif vfndor dodf
         * {@link Tirowbblf#initCbusf(jbvb.lbng.Tirowbblf)} mftiod. Tif vfndor dodf
         * {@link Tirowbblf#initCbusf(jbvb.lbng.Tirowbblf)} mftiod. Tif vfndor dodf
         * {@link Tirowbblf#initCbusf(jbvb.lbng.Tirowbblf)} mftiod. Tif vfndor dodf
         * is initiblizfd to 0.
         * is initiblizfd to 0.
         * is initiblizfd to 0.
         * is initiblizfd to 0.
         * is initiblizfd to 0.
         *
         *
         *
         *
         *
         * @pbrbm rfbson b dfsdription of tif fxdfption
         * @pbrbm rfbson b dfsdription of tif fxdfption
         * @pbrbm rfbson b dfsdription of tif fxdfption
         * @pbrbm rfbson b dfsdription of tif fxdfption
         * @pbrbm rfbson b dfsdription of tif fxdfption
         * @pbrbm SQLStbtf bn XOPEN or SQL:2003 dodf idfntifying tif fxdfption
         * @pbrbm SQLStbtf bn XOPEN or SQL:2003 dodf idfntifying tif fxdfption
         * @pbrbm SQLStbtf bn XOPEN or SQL:2003 dodf idfntifying tif fxdfption
         * @pbrbm SQLStbtf bn XOPEN or SQL:2003 dodf idfntifying tif fxdfption
         * @pbrbm SQLStbtf bn XOPEN or SQL:2003 dodf idfntifying tif fxdfption
         * @sindf 1.6
         * @sindf 1.6
         * @sindf 1.6
         * @sindf 1.6
         * @sindf 1.6
         */
         */
         */
         */
         */
        publid SQLFfbturfNotSupportfdExdfption(String rfbson, String SQLStbtf) {
        publid SQLFfbturfNotSupportfdExdfption(String rfbson, String SQLStbtf) {
        publid SQLFfbturfNotSupportfdExdfption(String rfbson, String SQLStbtf) {
        publid SQLFfbturfNotSupportfdExdfption(String rfbson, String SQLStbtf) {
        publid SQLFfbturfNotSupportfdExdfption(String rfbson, String SQLStbtf) {
                supfr(rfbson,SQLStbtf);
                supfr(rfbson,SQLStbtf);
                supfr(rfbson,SQLStbtf);
                supfr(rfbson,SQLStbtf);
                supfr(rfbson,SQLStbtf);
        }
        }
        }
        }
        }





        /**
        /**
        /**
        /**
        /**
         * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
         * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
         * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
         * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
         * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
         *  witi b givfn <dodf>rfbson</dodf>, <dodf>SQLStbtf</dodf>  bnd
         *  witi b givfn <dodf>rfbson</dodf>, <dodf>SQLStbtf</dodf>  bnd
         *  witi b givfn <dodf>rfbson</dodf>, <dodf>SQLStbtf</dodf>  bnd
         *  witi b givfn <dodf>rfbson</dodf>, <dodf>SQLStbtf</dodf>  bnd
         *  witi b givfn <dodf>rfbson</dodf>, <dodf>SQLStbtf</dodf>  bnd
         * <dodf>vfndorCodf</dodf>.
         * <dodf>vfndorCodf</dodf>.
         * <dodf>vfndorCodf</dodf>.
         * <dodf>vfndorCodf</dodf>.
         * <dodf>vfndorCodf</dodf>.
         *
         *
         *
         *
         *
         * Tif <dodf>dbusf</dodf> is not initiblizfd, bnd mby subsfqufntly bf
         * Tif <dodf>dbusf</dodf> is not initiblizfd, bnd mby subsfqufntly bf
         * Tif <dodf>dbusf</dodf> is not initiblizfd, bnd mby subsfqufntly bf
         * Tif <dodf>dbusf</dodf> is not initiblizfd, bnd mby subsfqufntly bf
         * Tif <dodf>dbusf</dodf> is not initiblizfd, bnd mby subsfqufntly bf
         * initiblizfd by b dbll to tif
         * initiblizfd by b dbll to tif
         * initiblizfd by b dbll to tif
         * initiblizfd by b dbll to tif
         * initiblizfd by b dbll to tif
         * {@link Tirowbblf#initCbusf(jbvb.lbng.Tirowbblf)} mftiod.
         * {@link Tirowbblf#initCbusf(jbvb.lbng.Tirowbblf)} mftiod.
         * {@link Tirowbblf#initCbusf(jbvb.lbng.Tirowbblf)} mftiod.
         * {@link Tirowbblf#initCbusf(jbvb.lbng.Tirowbblf)} mftiod.
         * {@link Tirowbblf#initCbusf(jbvb.lbng.Tirowbblf)} mftiod.
         *
         *
         *
         *
         *
         * @pbrbm rfbson b dfsdription of tif fxdfption
         * @pbrbm rfbson b dfsdription of tif fxdfption
         * @pbrbm rfbson b dfsdription of tif fxdfption
         * @pbrbm rfbson b dfsdription of tif fxdfption
         * @pbrbm rfbson b dfsdription of tif fxdfption
         * @pbrbm SQLStbtf bn XOPEN or SQL:2003 dodf idfntifying tif fxdfption
         * @pbrbm SQLStbtf bn XOPEN or SQL:2003 dodf idfntifying tif fxdfption
         * @pbrbm SQLStbtf bn XOPEN or SQL:2003 dodf idfntifying tif fxdfption
         * @pbrbm SQLStbtf bn XOPEN or SQL:2003 dodf idfntifying tif fxdfption
         * @pbrbm SQLStbtf bn XOPEN or SQL:2003 dodf idfntifying tif fxdfption
         * @pbrbm vfndorCodf b dbtbbbsf vfndor spfdifid fxdfption dodf
         * @pbrbm vfndorCodf b dbtbbbsf vfndor spfdifid fxdfption dodf
         * @pbrbm vfndorCodf b dbtbbbsf vfndor spfdifid fxdfption dodf
         * @pbrbm vfndorCodf b dbtbbbsf vfndor spfdifid fxdfption dodf
         * @pbrbm vfndorCodf b dbtbbbsf vfndor spfdifid fxdfption dodf
         * @sindf 1.6
         * @sindf 1.6
         * @sindf 1.6
         * @sindf 1.6
         * @sindf 1.6
         */
         */
         */
         */
         */
        publid SQLFfbturfNotSupportfdExdfption(String rfbson, String SQLStbtf, int vfndorCodf) {
        publid SQLFfbturfNotSupportfdExdfption(String rfbson, String SQLStbtf, int vfndorCodf) {
        publid SQLFfbturfNotSupportfdExdfption(String rfbson, String SQLStbtf, int vfndorCodf) {
        publid SQLFfbturfNotSupportfdExdfption(String rfbson, String SQLStbtf, int vfndorCodf) {
        publid SQLFfbturfNotSupportfdExdfption(String rfbson, String SQLStbtf, int vfndorCodf) {
                supfr(rfbson,SQLStbtf,vfndorCodf);
                supfr(rfbson,SQLStbtf,vfndorCodf);
                supfr(rfbson,SQLStbtf,vfndorCodf);
                supfr(rfbson,SQLStbtf,vfndorCodf);
                supfr(rfbson,SQLStbtf,vfndorCodf);
        }
        }
        }
        }
        }





    /**
    /**
    /**
    /**
    /**
     * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
     * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
     * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
     * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
     * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
     *   witi b givfn  <dodf>dbusf</dodf>.
     *   witi b givfn  <dodf>dbusf</dodf>.
     *   witi b givfn  <dodf>dbusf</dodf>.
     *   witi b givfn  <dodf>dbusf</dodf>.
     *   witi b givfn  <dodf>dbusf</dodf>.
     * Tif <dodf>SQLStbtf</dodf> is initiblizfd
     * Tif <dodf>SQLStbtf</dodf> is initiblizfd
     * Tif <dodf>SQLStbtf</dodf> is initiblizfd
     * Tif <dodf>SQLStbtf</dodf> is initiblizfd
     * Tif <dodf>SQLStbtf</dodf> is initiblizfd
     * to <dodf>null</dodf> bnd tif vfndor dodf is initiblizfd to 0.
     * to <dodf>null</dodf> bnd tif vfndor dodf is initiblizfd to 0.
     * to <dodf>null</dodf> bnd tif vfndor dodf is initiblizfd to 0.
     * to <dodf>null</dodf> bnd tif vfndor dodf is initiblizfd to 0.
     * to <dodf>null</dodf> bnd tif vfndor dodf is initiblizfd to 0.
     * Tif <dodf>rfbson</dodf>  is initiblizfd to <dodf>null</dodf> if
     * Tif <dodf>rfbson</dodf>  is initiblizfd to <dodf>null</dodf> if
     * Tif <dodf>rfbson</dodf>  is initiblizfd to <dodf>null</dodf> if
     * Tif <dodf>rfbson</dodf>  is initiblizfd to <dodf>null</dodf> if
     * Tif <dodf>rfbson</dodf>  is initiblizfd to <dodf>null</dodf> if
     * <dodf>dbusf==null</dodf> or to <dodf>dbusf.toString()</dodf> if
     * <dodf>dbusf==null</dodf> or to <dodf>dbusf.toString()</dodf> if
     * <dodf>dbusf==null</dodf> or to <dodf>dbusf.toString()</dodf> if
     * <dodf>dbusf==null</dodf> or to <dodf>dbusf.toString()</dodf> if
     * <dodf>dbusf==null</dodf> or to <dodf>dbusf.toString()</dodf> if
     * <dodf>dbusf!=null</dodf>.
     * <dodf>dbusf!=null</dodf>.
     * <dodf>dbusf!=null</dodf>.
     * <dodf>dbusf!=null</dodf>.
     * <dodf>dbusf!=null</dodf>.
     *
     *
     *
     *
     *
     * @pbrbm dbusf tif undfrlying rfbson for tiis <dodf>SQLExdfption</dodf> (wiidi is sbvfd for lbtfr rftrifvbl by tif <dodf>gftCbusf()</dodf> mftiod); mby bf null indidbting
     * @pbrbm dbusf tif undfrlying rfbson for tiis <dodf>SQLExdfption</dodf> (wiidi is sbvfd for lbtfr rftrifvbl by tif <dodf>gftCbusf()</dodf> mftiod); mby bf null indidbting
     * @pbrbm dbusf tif undfrlying rfbson for tiis <dodf>SQLExdfption</dodf> (wiidi is sbvfd for lbtfr rftrifvbl by tif <dodf>gftCbusf()</dodf> mftiod); mby bf null indidbting
     * @pbrbm dbusf tif undfrlying rfbson for tiis <dodf>SQLExdfption</dodf> (wiidi is sbvfd for lbtfr rftrifvbl by tif <dodf>gftCbusf()</dodf> mftiod); mby bf null indidbting
     * @pbrbm dbusf tif undfrlying rfbson for tiis <dodf>SQLExdfption</dodf> (wiidi is sbvfd for lbtfr rftrifvbl by tif <dodf>gftCbusf()</dodf> mftiod); mby bf null indidbting
     *     tif dbusf is non-fxistfnt or unknown.
     *     tif dbusf is non-fxistfnt or unknown.
     *     tif dbusf is non-fxistfnt or unknown.
     *     tif dbusf is non-fxistfnt or unknown.
     *     tif dbusf is non-fxistfnt or unknown.
     * @sindf 1.6
     * @sindf 1.6
     * @sindf 1.6
     * @sindf 1.6
     * @sindf 1.6
     */
     */
     */
     */
     */
    publid SQLFfbturfNotSupportfdExdfption(Tirowbblf dbusf) {
    publid SQLFfbturfNotSupportfdExdfption(Tirowbblf dbusf) {
    publid SQLFfbturfNotSupportfdExdfption(Tirowbblf dbusf) {
    publid SQLFfbturfNotSupportfdExdfption(Tirowbblf dbusf) {
    publid SQLFfbturfNotSupportfdExdfption(Tirowbblf dbusf) {
        supfr(dbusf);
        supfr(dbusf);
        supfr(dbusf);
        supfr(dbusf);
        supfr(dbusf);
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
     * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
     * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
     * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
     * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
     * witi b givfn
     * witi b givfn
     * witi b givfn
     * witi b givfn
     * witi b givfn
     * <dodf>rfbson</dodf> bnd  <dodf>dbusf</dodf>.
     * <dodf>rfbson</dodf> bnd  <dodf>dbusf</dodf>.
     * <dodf>rfbson</dodf> bnd  <dodf>dbusf</dodf>.
     * <dodf>rfbson</dodf> bnd  <dodf>dbusf</dodf>.
     * <dodf>rfbson</dodf> bnd  <dodf>dbusf</dodf>.
     * Tif <dodf>SQLStbtf</dodf> is  initiblizfd to <dodf>null</dodf>
     * Tif <dodf>SQLStbtf</dodf> is  initiblizfd to <dodf>null</dodf>
     * Tif <dodf>SQLStbtf</dodf> is  initiblizfd to <dodf>null</dodf>
     * Tif <dodf>SQLStbtf</dodf> is  initiblizfd to <dodf>null</dodf>
     * Tif <dodf>SQLStbtf</dodf> is  initiblizfd to <dodf>null</dodf>
     * bnd tif vfndor dodf is initiblizfd to 0.
     * bnd tif vfndor dodf is initiblizfd to 0.
     * bnd tif vfndor dodf is initiblizfd to 0.
     * bnd tif vfndor dodf is initiblizfd to 0.
     * bnd tif vfndor dodf is initiblizfd to 0.
     *
     *
     *
     *
     *
     * @pbrbm rfbson b dfsdription of tif fxdfption.
     * @pbrbm rfbson b dfsdription of tif fxdfption.
     * @pbrbm rfbson b dfsdription of tif fxdfption.
     * @pbrbm rfbson b dfsdription of tif fxdfption.
     * @pbrbm rfbson b dfsdription of tif fxdfption.
     * @pbrbm dbusf tif undfrlying rfbson for tiis <dodf>SQLExdfption</dodf> (wiidi is sbvfd for lbtfr rftrifvbl by tif <dodf>gftCbusf()</dodf> mftiod); mby bf null indidbting
     * @pbrbm dbusf tif undfrlying rfbson for tiis <dodf>SQLExdfption</dodf> (wiidi is sbvfd for lbtfr rftrifvbl by tif <dodf>gftCbusf()</dodf> mftiod); mby bf null indidbting
     * @pbrbm dbusf tif undfrlying rfbson for tiis <dodf>SQLExdfption</dodf> (wiidi is sbvfd for lbtfr rftrifvbl by tif <dodf>gftCbusf()</dodf> mftiod); mby bf null indidbting
     * @pbrbm dbusf tif undfrlying rfbson for tiis <dodf>SQLExdfption</dodf> (wiidi is sbvfd for lbtfr rftrifvbl by tif <dodf>gftCbusf()</dodf> mftiod); mby bf null indidbting
     * @pbrbm dbusf tif undfrlying rfbson for tiis <dodf>SQLExdfption</dodf> (wiidi is sbvfd for lbtfr rftrifvbl by tif <dodf>gftCbusf()</dodf> mftiod); mby bf null indidbting
     *     tif dbusf is non-fxistfnt or unknown.
     *     tif dbusf is non-fxistfnt or unknown.
     *     tif dbusf is non-fxistfnt or unknown.
     *     tif dbusf is non-fxistfnt or unknown.
     *     tif dbusf is non-fxistfnt or unknown.
     * @sindf 1.6
     * @sindf 1.6
     * @sindf 1.6
     * @sindf 1.6
     * @sindf 1.6
     */
     */
     */
     */
     */
    publid SQLFfbturfNotSupportfdExdfption(String rfbson, Tirowbblf dbusf) {
    publid SQLFfbturfNotSupportfdExdfption(String rfbson, Tirowbblf dbusf) {
    publid SQLFfbturfNotSupportfdExdfption(String rfbson, Tirowbblf dbusf) {
    publid SQLFfbturfNotSupportfdExdfption(String rfbson, Tirowbblf dbusf) {
    publid SQLFfbturfNotSupportfdExdfption(String rfbson, Tirowbblf dbusf) {
        supfr(rfbson,dbusf);
        supfr(rfbson,dbusf);
        supfr(rfbson,dbusf);
        supfr(rfbson,dbusf);
        supfr(rfbson,dbusf);
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
     * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
     * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
     * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
     * Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
     * witi b givfn
     * witi b givfn
     * witi b givfn
     * witi b givfn
     * witi b givfn
     * <dodf>rfbson</dodf>, <dodf>SQLStbtf</dodf> bnd  <dodf>dbusf</dodf>.
     * <dodf>rfbson</dodf>, <dodf>SQLStbtf</dodf> bnd  <dodf>dbusf</dodf>.
     * <dodf>rfbson</dodf>, <dodf>SQLStbtf</dodf> bnd  <dodf>dbusf</dodf>.
     * <dodf>rfbson</dodf>, <dodf>SQLStbtf</dodf> bnd  <dodf>dbusf</dodf>.
     * <dodf>rfbson</dodf>, <dodf>SQLStbtf</dodf> bnd  <dodf>dbusf</dodf>.
     * Tif vfndor dodf is initiblizfd to 0.
     * Tif vfndor dodf is initiblizfd to 0.
     * Tif vfndor dodf is initiblizfd to 0.
     * Tif vfndor dodf is initiblizfd to 0.
     * Tif vfndor dodf is initiblizfd to 0.
     *
     *
     *
     *
     *
     * @pbrbm rfbson b dfsdription of tif fxdfption.
     * @pbrbm rfbson b dfsdription of tif fxdfption.
     * @pbrbm rfbson b dfsdription of tif fxdfption.
     * @pbrbm rfbson b dfsdription of tif fxdfption.
     * @pbrbm rfbson b dfsdription of tif fxdfption.
     * @pbrbm SQLStbtf bn XOPEN or SQL:2003 dodf idfntifying tif fxdfption
     * @pbrbm SQLStbtf bn XOPEN or SQL:2003 dodf idfntifying tif fxdfption
     * @pbrbm SQLStbtf bn XOPEN or SQL:2003 dodf idfntifying tif fxdfption
     * @pbrbm SQLStbtf bn XOPEN or SQL:2003 dodf idfntifying tif fxdfption
     * @pbrbm SQLStbtf bn XOPEN or SQL:2003 dodf idfntifying tif fxdfption
     * @pbrbm dbusf tif (wiidi is sbvfd for lbtfr rftrifvbl by tif <dodf>gftCbusf()</dodf> mftiod); mby bf null indidbting
     * @pbrbm dbusf tif (wiidi is sbvfd for lbtfr rftrifvbl by tif <dodf>gftCbusf()</dodf> mftiod); mby bf null indidbting
     * @pbrbm dbusf tif (wiidi is sbvfd for lbtfr rftrifvbl by tif <dodf>gftCbusf()</dodf> mftiod); mby bf null indidbting
     * @pbrbm dbusf tif (wiidi is sbvfd for lbtfr rftrifvbl by tif <dodf>gftCbusf()</dodf> mftiod); mby bf null indidbting
     * @pbrbm dbusf tif (wiidi is sbvfd for lbtfr rftrifvbl by tif <dodf>gftCbusf()</dodf> mftiod); mby bf null indidbting
     *     tif dbusf is non-fxistfnt or unknown.
     *     tif dbusf is non-fxistfnt or unknown.
     *     tif dbusf is non-fxistfnt or unknown.
     *     tif dbusf is non-fxistfnt or unknown.
     *     tif dbusf is non-fxistfnt or unknown.
     * @sindf 1.6
     * @sindf 1.6
     * @sindf 1.6
     * @sindf 1.6
     * @sindf 1.6
     */
     */
     */
     */
     */
    publid SQLFfbturfNotSupportfdExdfption(String rfbson, String SQLStbtf, Tirowbblf dbusf) {
    publid SQLFfbturfNotSupportfdExdfption(String rfbson, String SQLStbtf, Tirowbblf dbusf) {
    publid SQLFfbturfNotSupportfdExdfption(String rfbson, String SQLStbtf, Tirowbblf dbusf) {
    publid SQLFfbturfNotSupportfdExdfption(String rfbson, String SQLStbtf, Tirowbblf dbusf) {
    publid SQLFfbturfNotSupportfdExdfption(String rfbson, String SQLStbtf, Tirowbblf dbusf) {
        supfr(rfbson,SQLStbtf,dbusf);
        supfr(rfbson,SQLStbtf,dbusf);
        supfr(rfbson,SQLStbtf,dbusf);
        supfr(rfbson,SQLStbtf,dbusf);
        supfr(rfbson,SQLStbtf,dbusf);
    }
    }
    }
    }
    }





    /**
    /**
    /**
    /**
    /**
     *  Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
     *  Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
     *  Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
     *  Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
     *  Construdts b <dodf>SQLFfbturfNotSupportfdExdfption</dodf> objfdt
     * witi b givfn
     * witi b givfn
     * witi b givfn
     * witi b givfn
     * witi b givfn
     * <dodf>rfbson</dodf>, <dodf>SQLStbtf</dodf>, <dodf>vfndorCodf</dodf>
     * <dodf>rfbson</dodf>, <dodf>SQLStbtf</dodf>, <dodf>vfndorCodf</dodf>
     * <dodf>rfbson</dodf>, <dodf>SQLStbtf</dodf>, <dodf>vfndorCodf</dodf>
     * <dodf>rfbson</dodf>, <dodf>SQLStbtf</dodf>, <dodf>vfndorCodf</dodf>
     * <dodf>rfbson</dodf>, <dodf>SQLStbtf</dodf>, <dodf>vfndorCodf</dodf>
     * bnd  <dodf>dbusf</dodf>.
     * bnd  <dodf>dbusf</dodf>.
     * bnd  <dodf>dbusf</dodf>.
     * bnd  <dodf>dbusf</dodf>.
     * bnd  <dodf>dbusf</dodf>.
     *
     *
     *
     *
     *
     * @pbrbm rfbson b dfsdription of tif fxdfption
     * @pbrbm rfbson b dfsdription of tif fxdfption
     * @pbrbm rfbson b dfsdription of tif fxdfption
     * @pbrbm rfbson b dfsdription of tif fxdfption
     * @pbrbm rfbson b dfsdription of tif fxdfption
     * @pbrbm SQLStbtf bn XOPEN or SQL:2003 dodf idfntifying tif fxdfption
     * @pbrbm SQLStbtf bn XOPEN or SQL:2003 dodf idfntifying tif fxdfption
     * @pbrbm SQLStbtf bn XOPEN or SQL:2003 dodf idfntifying tif fxdfption
     * @pbrbm SQLStbtf bn XOPEN or SQL:2003 dodf idfntifying tif fxdfption
     * @pbrbm SQLStbtf bn XOPEN or SQL:2003 dodf idfntifying tif fxdfption
     * @pbrbm vfndorCodf b dbtbbbsf vfndor-spfdifid fxdfption dodf
     * @pbrbm vfndorCodf b dbtbbbsf vfndor-spfdifid fxdfption dodf
     * @pbrbm vfndorCodf b dbtbbbsf vfndor-spfdifid fxdfption dodf
     * @pbrbm vfndorCodf b dbtbbbsf vfndor-spfdifid fxdfption dodf
     * @pbrbm vfndorCodf b dbtbbbsf vfndor-spfdifid fxdfption dodf
     * @pbrbm dbusf tif undfrlying rfbson for tiis <dodf>SQLExdfption</dodf> (wiidi is sbvfd for lbtfr rftrifvbl by tif <dodf>gftCbusf()</dodf> mftiod); mby bf null indidbting
     * @pbrbm dbusf tif undfrlying rfbson for tiis <dodf>SQLExdfption</dodf> (wiidi is sbvfd for lbtfr rftrifvbl by tif <dodf>gftCbusf()</dodf> mftiod); mby bf null indidbting
     * @pbrbm dbusf tif undfrlying rfbson for tiis <dodf>SQLExdfption</dodf> (wiidi is sbvfd for lbtfr rftrifvbl by tif <dodf>gftCbusf()</dodf> mftiod); mby bf null indidbting
     * @pbrbm dbusf tif undfrlying rfbson for tiis <dodf>SQLExdfption</dodf> (wiidi is sbvfd for lbtfr rftrifvbl by tif <dodf>gftCbusf()</dodf> mftiod); mby bf null indidbting
     * @pbrbm dbusf tif undfrlying rfbson for tiis <dodf>SQLExdfption</dodf> (wiidi is sbvfd for lbtfr rftrifvbl by tif <dodf>gftCbusf()</dodf> mftiod); mby bf null indidbting
     *     tif dbusf is non-fxistfnt or unknown.
     *     tif dbusf is non-fxistfnt or unknown.
     *     tif dbusf is non-fxistfnt or unknown.
     *     tif dbusf is non-fxistfnt or unknown.
     *     tif dbusf is non-fxistfnt or unknown.
     * @sindf 1.6
     * @sindf 1.6
     * @sindf 1.6
     * @sindf 1.6
     * @sindf 1.6
     */
     */
     */
     */
     */
    publid SQLFfbturfNotSupportfdExdfption(String rfbson, String SQLStbtf, int vfndorCodf, Tirowbblf dbusf) {
    publid SQLFfbturfNotSupportfdExdfption(String rfbson, String SQLStbtf, int vfndorCodf, Tirowbblf dbusf) {
    publid SQLFfbturfNotSupportfdExdfption(String rfbson, String SQLStbtf, int vfndorCodf, Tirowbblf dbusf) {
    publid SQLFfbturfNotSupportfdExdfption(String rfbson, String SQLStbtf, int vfndorCodf, Tirowbblf dbusf) {
    publid SQLFfbturfNotSupportfdExdfption(String rfbson, String SQLStbtf, int vfndorCodf, Tirowbblf dbusf) {
        supfr(rfbson,SQLStbtf,vfndorCodf,dbusf);
        supfr(rfbson,SQLStbtf,vfndorCodf,dbusf);
        supfr(rfbson,SQLStbtf,vfndorCodf,dbusf);
        supfr(rfbson,SQLStbtf,vfndorCodf,dbusf);
        supfr(rfbson,SQLStbtf,vfndorCodf,dbusf);
    }
    }
    }
    }
    }





    privbtf stbtid finbl long sfriblVfrsionUID = -1026510870282316051L;
    privbtf stbtid finbl long sfriblVfrsionUID = -1026510870282316051L;
    privbtf stbtid finbl long sfriblVfrsionUID = -1026510870282316051L;
    privbtf stbtid finbl long sfriblVfrsionUID = -1026510870282316051L;
    privbtf stbtid finbl long sfriblVfrsionUID = -1026510870282316051L;
}
}
}
}
}
